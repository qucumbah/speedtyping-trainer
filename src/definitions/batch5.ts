export default [
  "exothecium - The outer coat of the anther.",
  "pantisocratist - One who favors or supports the theory of a pantisocracy. Macaulay.",
  "biggen - To make or become big; to enlarge. [Obs. or Dial.] Steele.",
  "bibliopegistic - Pertaining to the art of binding books. [R.] Dibdin.",
  "seabeard - A green seaweed (Cladophora rupestris) growing in dense tufts.",
  "subarachnoid - Situated under the arachnoid membrane.",
  "parsonish - Appropriate to, or like, a parson; -- used in disparagement. [Colloq.]",
  "valeritrine - A base, C15H27N, produced together with valeridine, which it resembles.",
  "hurlbone - 1. See Whirlbone. 2. (Far.)  A bone near the middle of the buttock of a horse. Crabb.",
  "enfree - To set free. [Obs.] \"The enfreed Antenor.\" Shak.",
  "limpness - The quality or state of being limp.",
  "segmented - Divided into segments or joints; articulated.",
  "deshabille - An undress; a careless toilet.",
  "requisitor - One who makes reqisition; esp., one authorized by a requisition to investigate facts.",
  "bescorn - To treat with scorn. \"Then was he bescorned.\" Chaucer.",
  "admix - To mingle with something else; to mix. [R.]",
  "palestinean - Of or pertaining to Palestine.",
  "prerogatived - Endowed with a prerogative, or exclusive privilege. [R.] Shak.",
  "sunproof - Impervious to the rays of the sun. \"Darksome yew, sunproof.\" Marston.",
  "immurement - The act iif immuring, or the state of being immured; imprsonment.",
  "finlander - A native or inhabitant of Finland.",
  "stereotomical - Of or pertaining to stereotomy; performed by stereotomy.",
  "stereographically - In a stereographical manner; by delineation on a plane.",
  "levitically - After the manner of the Levites; in accordance with the levitical law.",
  "cerulescent - Tending to cerulean; light bluish.",
  "recelebrate - To celebrate again, or anew. -- Re*cel`e*bra\"tion, n.",
  "lauriferous - Producing, or bringing, laurel.",
  "electro-telegraphic - Pertaining to the electric telegraph, or by means of it.",
  "priest-ridden - Controlled or oppressed by priests; as, a priest-ridden people. Swift.",
  "unicostate - Having a single rib or strong nerve running upward from the base; -- said of a leaf.",
  "wit-cracker - One who breaks jests; a joker. [Obs.] Shak.",
  "newfangly - In a newfangled manner; with eagerness for novelty. [Obs.] Sir T. More.",
  "gardening - The art of occupation of laying out and cultivating gardens; horticulture.",
  "dulciana - A sweet-toned stop of an organ.",
  "autopsorin - That which is given under the doctrine of administering a patient's own virus.",
  "incantatory - Dealing by enchantment; magical. Sir T. Browne.",
  "curability - The state of being curable; curableness.",
  "physicological - Of or pertaining to physicologic. Swift.",
  "frenchism - A French mode or characteristic; an idiom peculiar to the French language. Earle.",
  "fasciculate - Grouped in a fascicle; fascicled.",
  "intervent - To thwart; to obstruct. [Obs.] Chapman.",
  "tralineate - To deviate; to stray; to wander. [Obs.] Dryden.",
  "monoecism - The state or condition of being monoecious.",
  "double dealer - One who practices double dealing; a deceitful, trickish person. L'Estrange.",
  "abjectness - The state of being abject; abasement; meanness; servility. Grew.",
  "gladiature - Swordplay; fencing; gladiatorial contest. Gayton.",
  "nonagrian - Any moth of the genus Nonagria and allied genera, as the spindleworm and stalk borer.",
  "shaffle - To hobble or limp; to shuffle. [Obs. or Prov.Eng.]",
  "pastorium - A parsonage; -- so called in some Baptist churches. [Southern U. S.]",
  "paradoxology - The use of paradoxes. [Obs.] Sir T. Browne.",
  "cushat - The ringdove or wood pigeon. Scarce with cushat's homely song can vie. Sir W. Scott.",
  "baby farm - A place where the nourishment and care of babies are offered for hire.",
  "sperre - To shut in; to support; to inclose; to fasten. [Obs.] \"To sperre the gate.\" Spenser.",
  "immoderacy - Immoderateness. [R.] Sir T. Browne.",
  "organizability - Quality of being organizable; capability of being organized.",
  "pachyote - One of a family of bats, including those which have thick external ears.",
  "moky - Misty; dark; murky; muggy. [Obs.]",
  "remastication - The act of masticating or chewing again or repeatedly.",
  "tortility - The quality or state of being tortile, twisted, or wreathed.",
  "formicate - Resembling, or pertaining to, an ant or ants.",
  "disdeign - To disdain. [Obs.] Guyon much disdeigned so loathly sight. Spenser.",
  "trijugate - In three pairs; as, a trijugate leaf, or a pinnate leaf with three pairs of leaflets.",
  "placentious - Pleasing; amiable. [Obs.] \"A placentious person.\" Fuller.",
  "sassorol - The rock pigeon. See under Pigeon.",
  "gargol - A distemper in swine; garget. Mortimer.",
  "bargeman - The man who manages a barge, or one of the crew of a barge.",
  "multeity - Multiplicity. [R.] Coleridge.",
  "tuberculose - Having tubercles; affected with, or characterized by, tubercles; tubercular.",
  "hyperbolism - The use of hyperbole. Jefferson.",
  "cigarette - A little cigar; a little fine tobacco rolled in paper for smoking.",
  "intersectional - Pertaining to, or formed by, intersections.",
  "nidor - Scent or savor of meat or food, cooked or cooking. [Obs.] Jer. Taylor.",
  "mortgagee - The person to whom property is mortgaged, or to whom a mortgage is made or given.",
  "sidewalk - A walk for foot passengers at the side of a street or road; a foot pavement. [U.S.]",
  "antilopine - Of or relating to the antelope.",
  "brownback - The dowitcher or red-breasted snipe. See Dowitcher.",
  "fuguist - A musician who composes or performs fugues. Busby.",
  "greekling - A little Greek, or one of small esteem or pretensions. B. Jonson.",
  "muffin - A light, spongy, cylindrical cake, used for breakfast and tea.",
  "dogship - The character, or individuality, of a dog.",
  "cone clutch - A friction clutch with conical bearing surfaces.",
  "thysanopterous - Of or pertaining to the Thysanoptera.",
  "extensiveness - The state of being extensive; wideness; largeness; extent; diffusiveness.",
  "fluidounce - See Fluid ounce, under Fluid.",
  "traitress - A woman who betrays her country or any trust; a traitoress. Dryden.",
  "ca ira - The refrain of a famous song of the French Revolution.",
  "chuffiness - The quality of being chuffy.",
  "houseless - Destitute of the shelter of a house; shelterless; homeless; as, a houseless wanderer.",
  "pullman car - A kind of sleeping car; also, a palace car; -- often shortened to Pullman.",
  "cithara - An ancient instrument resembling the harp.",
  "amused - 1. Diverted. 2. Expressing amusement; as, an amused look.",
  "abdest - Purification by washing the hands before prayer; -- a Mohammedan rite. Heyse.",
  "chapelry - The territorial disrict legally assigned to a chapel.",
  "emendatory - Pertaining to emendation; corrective. \"Emendatory criticism.\"\" Johnson.",
  "mughouse - An alehouse; a pothouse. Tickel.",
  "correlativeness - Quality of being correlative.",
  "serricated - Covered with fine silky down.",
  "immatureness - The state or quality of being immature; immaturity. Boyle.",
  "psychoanalysis - = Psychanalysis, Psychanalytic.",
  "gerant - The manager or acting partner of a company, joint-stock association, etc.",
  "photochromatic - Of or pertaining to photochromy; produced by photochromy.",
  "ovipara - An artifical division of vertebrates, including those that lay eggs; -opposed to Vivipara.",
  "grubworm - See Grub, n., 1. And gnats and grubworms crowded on his view. C. Smart.",
  "higher thought - See New thought, below.",
  "portiere - A curtain hanging across a doorway.",
  "wordplay - A more or less subtle playing upon the meaning of words.",
  "sterner - A director. [Obs. & R.] Dr. R. Clerke.",
  "turfite - A votary of the turf, or race course; hence, sometimes, a blackleg. [Colloq.] Thackeray.",
  "multarticulate - Having many articulations or joints.",
  "redigest - To digest, or reduce to form, a second time. Kent.",
  "shiny - Bright; luminous; clear; unclouded. Like distant thunder on a shiny day. Dryden.",
  "perbromic - Pertaining to, or designating, the highest oxygen acid, HBrO4, of bromine.",
  "severality - Each particular taken singly; distinction. [Obs.] Bp. Hall.",
  "ostentate - To make an ambitious display of; to show or exhibit boastingly. [R.] Jer. Taylor.",
  "rashful - Rash; hasty; precipitate. [Obs.]",
  "pancratical - Of or pertaining to the pancratium; athletic. Sir T. Browne",
  "rhizomatous - Having the nature or habit of a rhizome or rootstock.",
  "tottle - To walk in a wavering, unsteady manner; to toddle; to topple. [Colloq.]",
  "foot-sore - Having sore or tender feet, as by reason of much walking; as, foot-sore cattle.",
  "hagbutter - A soldier armed with a hagbut or arquebus. [Written also hackbutter.] Froude.",
  "conceptive - Capable of conceiving. Sir T. Browne",
  "belock - To lock, or fasten as with a lock. [Obs.] Shak.",
  "allegorist - One who allegorizes; a writer of allegory. Hume.",
  "pleaseman - An officious person who courts favor servilely; a pickthank. [Obs.] Shak.",
  "overhandle - To handle, or use, too much; to mention too often. Shak.",
  "vatical - Of or pertaining to a prophet; prophetical. Bp. Hall.",
  "office wire - Copper wire with a strong but light insulation, used in wiring houses, etc.",
  "omnicorporeal - Comprehending or including all bodies; embracing all substance. [R.] Cudworth.",
  "southernly - Somewhat southern. -- adv.  In a southerly manner or course; southward.",
  "acquainted - Personally known; familiar. See To be acquainted with, under Acquaint, v. t.",
  "gunstick - A stick to ram down the charge of a musket, etc.; a rammer or ramrod. [R.]",
  "thalamocoele - The cavity or ventricle of the thalamencephalon; the third ventricle.",
  "bothersome - Vexatious; causing bother; causing trouble or perplexity; troublesome.",
  "nubile - Of an age suitable for marriage; marriageable. Prior.",
  "affectional - Of or pertaining to the affections; as, affectional impulses; an affectional nature.",
  "shock-headed - Having a thick and bushy head of hair.",
  "ideographics - The system of writing in ideographic characters; also, anything so written.",
  "pericarpic - Of or pertaining to a pericarp.",
  "bridgey - Full of bridges. [R.] Sherwood.",
  "cappeline - A hood-shaped bandage for the head, the shoulder, or the stump of an amputated limb.",
  "trevet - A stool or other thing supported by three legs; a trivet.",
  "thearchy - Government by God; divine sovereignty; theocracy.",
  "cravatted - Wearing a cravat. The young men faultlessly appointed, handsomely cravatted. Thackeray.",
  "cocus wood - A West Indian wood, used for making flutes and other musical instruments.",
  "dicacious - Talkative; pert; saucy. [Obs.]",
  "habitator - A dweller; an inhabitant. [Obs.] Sir T. Browne.",
  "circumfusion - The act of pouring or spreading round; the state of being spread round. Swift.",
  "brahminist - An adherent of the religion of the Brahmans.",
  "isoperimetrical - Having equal perimeters of circumferences; as, isoperimetrical figures or bodies.",
  "scrow - 1. A scroll. [Obs.] Palsgrave. 2. A clipping from skins; a currier's cuttings.",
  "caudad - Backwards; toward the tail or posterior part.",
  "way-goose - See Wayz-goose, n., 2. [Eng.]",
  "utero - - (connection with, or relation to, the uterus; as in utro- ovarian.",
  "photogenic - Of or pertaining to photogeny; producing or generating light.",
  "parablastic - Of or pertaining to the parablast; as, the parablastic cells.",
  "plumcot - A cross between the plum and apricot.",
  "aculeated - Having a sharp point; armed with prickles; prickly; aculeate.",
  "estimableness - The quality of deserving esteem or regard.",
  "hectometer - A measure of length, equal to a hundred meters. It is equivalent to 328.09 feet.",
  "underhang - To hang under or down; to suspend. Holland.",
  "coccolite - A granular variety of pyroxene, green or white in color.",
  "stormful - Abounding with storms. \"The stormful east.\" Carlyle. -- Storm\"ful*ness, n.",
  "oculate - 1. Furnished with eyes. 2. Having spots or holes resembling eyes; ocellated.",
  "poak - Waste matter from the preparation of skins, consisting of hair, lime, oil, etc.",
  "emptying - 1. The act of making empty. Shak. 2. pl.  The lees of beer, cider, etc.; yeast. [U.S.]",
  "gynarchy - Government by a woman. Chesterfield.",
  "fellowlike - Like a companion; companionable; on equal terms; sympathetic. [Obs.] Udall.",
  "cadene - A species of inferior carpet imported from the Levant. McElrath.",
  "uratic - Of or containing urates; as, uratic calculi.",
  "minorate - To diminish. [R.] Sir T. Browne.",
  "purificative - Having power to purify; tending to cleanse. [R.]",
  "animoseness - Vehemence of temper. [Obs.]",
  "transpicuous - Transparent; pervious to the sight. [R.] \"The wide, transpicuous air.\" Milton.",
  "comitiva - A body of followers; -- applied to the lawless or brigand bands in Italy and Sicily.",
  "floroon - A border worked with flowers. Wright.",
  "disclusion - A shutting off; exclusion. [Obs.] Dr. H. More.",
  "white slaver - A person engaged in procuring or holding a woman or women for unwilling prostitution.",
  "pot-valiant - Having the courage given by drink. Smollett.",
  "bodian - A large food fish (Diagramma lineatum), native of the East Indies.",
  "subsultus - A starting, twitching, or convulsive motion.",
  "shunter - A person employed to shunt cars from one track to another.",
  "helicoidal - Same as Helicoid. -- Hel`i*coid\"al*ly, adv.",
  "sedgy - Overgrown with sedge. On the gentle Severn''s sedgy bank. Shak.",
  "ganglial - Relating to a ganglion; ganglionic.",
  "enmarble - To make hard as marble; to harden. [Obs.] Spenser.",
  "disaventure - Misfortune. [Obs.] Spenser.",
  "dasyurine - Pertaining to, or like, the dasyures.",
  "cicely - Any one of several umbelliferous plants, of the genera Myrrhis, Osmorrhiza, etc.",
  "mesially - In, near, or toward, the mesial plane; mesiad.",
  "bavardage - Much talking; prattle; chatter.  Byron.",
  "subcarbonate - A carbonate containing an excess of the basic constituent.",
  "fracas - An uproar; a noisy quarrel; a disturbance; a brawl.",
  "refrigerium - Cooling refreshment; refrigeration. [Obs.] South.",
  "weariful - Abounding in qualities which cause weariness; wearisome. -- Wea\"ri*ful*ly, adv.",
  "euphuism - An affectation of excessive elegance and refinement of language; high-flown diction.",
  "exprobrate - To charge upon with reproach; to upbraid. [R.] Sir T. Browne.",
  "forgetter - One who forgets; a heedless person. Johnson.",
  "problematize - To propose problems. [R.] \"Hear him problematize.\" B. Jonson.",
  "myriologist - One who composes or sings a myriologue.",
  "heterophyllous - Having leaves of more than one shape on the same plant.",
  "freieslebenite - A sulphide of antimony, lead, and silver, occuring in monoclinic crystals.",
  "omander wood - The wood of Diospyros ebenaster, a kind of ebony found in Ceylon.",
  "amnion - A thin membrane surrounding the embryos of mammals, birds, and reptiles.",
  "clausular - Consisting of, or having, clauses. Smart.",
  "attorneyship - The office or profession of an attorney; agency for another. Shak.",
  "culture features - The artificial features of a district as distinguished from the natural.",
  "serotine - The European long-eared bat (Vesperugo serotinus).",
  "dividual - Divided, shared, or participated in, in common with others. [R.] Milton.",
  "subministrate - To supply; to afford; to subminister. [Obs.] Harvey.",
  "nemophily - Fondness for forest scenery; love of the woods. [R.]",
  "hisingerite - A soft black, iron ore, nearly earthy, a hydrous silicate of iron.",
  "misween - To ween amiss; to misjudge; to distrust; to be mistaken. [Obs.] Spenser.",
  "yearth - The earth. [Obs.] \"Is my son dead or hurt or on the yerthe felled\" Ld. Berners.",
  "debacchate - To rave as a bacchanal. [R.] Cockeram.",
  "passiontide - The last fortnight of Lent.",
  "dogmaticalness - The quality of being dogmatical; positiveness.",
  "expletory - Serving to fill up; expletive; superfluous; as, an expletory word. Bp. Burnet.",
  "tufaceous - Pertaining to tufa; consisting of, or resembling, tufa.",
  "tranquillization - The act of tranquilizing, or the state of being tranquilized.",
  "waketime - Time during which one is awake. [R.] Mrs. Browning.",
  "signalist - One who makes signals; one who communicates intelligence by means of signals.",
  "antimasonry - Opposition to Freemasonry.",
  "chatwood - Little sticks; twigs for burning; fuel. Johnson.",
  "lateward - Somewhat late; backward. [Obs.] \"Lateward lands.\" Holland.",
  "sea gudgeon - The European black goby (Gobius niger).",
  "docetae - Ancient heretics who held that Christ's body was merely a phantom or appearance.",
  "stigmatically - With a stigma, or mark of infamy or deformity.",
  "byard - A piece of leather crossing the breast, used by the men who drag sledges in coal mines.",
  "preordinate - Preordained. [R.] Sir T. Elyot.",
  "quair - A quire; a book. [Obs.] \" The king's quhair.\" James I. (of Scotland).",
  "upwreath - To rise with a curling motion; to curl upward, as smoke. Longfellow.",
  "astun - To stun. [Obs.] \"Breathless and astunned.\" Somerville.",
  "cloud-compeller - Cloud-gatherer; -- an epithet applied to Zeus. [Poetic.] Pope.",
  "jungly - Consisting of jungles; abounding with jungles; of the nature of a jungle.",
  "monogoneutic - Having but one brood in a season.",
  "unbit - To remove the turns of (a rope or cable) from the bits; as, to unbit a cable. Totten.",
  "entropium - The inversion or turning in of the border of the eyelids.",
  "saurian - Of or pertaining to, or of the nature of, the Sauria. -- n.  One of the Sauria.",
  "piffara - A fife; also, a rude kind of oboe or a bagpipe with an inflated skin for reservoir.",
  "lychnoscope - Same as Low side window, under Low, a.",
  "terminant - Termination; ending. [R.] Puttenham.",
  "kantism - The doctrine or theory of Kant; the Kantian philosophy.",
  "archilochian - Of or pertaining to the satiric Greek poet Archilochus; as, Archilochian meter.",
  "severy - A bay or compartment of a vaulted ceiling. [Written also civery.]",
  "paintless - Not capable of being painted or described. \"In paintless patience.\" Savage.",
  "disconvenience - Unsuitableness; incongruity. [Obs.] Bacon.",
  "gaudygreen - Light green. [Obs.] Chaucer. Spenser.",
  "perisarc - The outer, hardened integument which covers most hydroids.",
  "sarcoid - Resembling flesh, or muscle; composed of sarcode.",
  "squamoid - Resembling a scale; also, covered with scales; scaly.",
  "frondesce - To unfold leaves, as plants.",
  "preceptorial - Of or pertaining to a preceptor.",
  "idiolatry - Self-worship; excessive self-esteem.",
  "pisciculture - Fish culture. See under Fish.",
  "thinness - The quality or state of being thin (in any of the senses of the word).",
  "mochila - A large leather flap which covers the saddletree. [Western U.S.]",
  "dermatophyte - A vegetable parasite, infesting the skin.",
  "befringe - To furnish with a fringe; to form a fringe upon; to adorn as with fringe. Fuller.",
  "decigramme - A weight in the metric system; one tenth of a gram, equal to 1.5432 grains avoirdupois.",
  "authorless - Without an author; without authority; anonymous.",
  "abjudication - Rejection by judicial sentence. [R.] Knowles.",
  "theatral - Of or pertaining to a theater; theatrical. [Obs.]",
  "childed - Furnished with a child. [Obs.]",
  "vesical - Of or pertaining to the bladder. Dunglison.",
  "sloakan - A species of seaweed. [Spelled also slowcawn.] See 3d Laver.",
  "immune - Exempt; protected by inoculation. -- Im*mu\"nize, v. t.",
  "providore - One who makes provision; a purveyor. [R.] De Foe.",
  "handkercher - A handkerchief. [Obs. or Colloq.] Chapman (1654). Shak.",
  "fipple - A stopper, as in a wind instrument of music. [Obs.] Bacon.",
  "genuflection - The act of bending the knee, particularly in worship. Bp. Stillingfleet.",
  "consolate - To console; to comfort. [Obs.] Shak.",
  "unforeseeable - Incapable of being foreseen. South.",
  "lowness - The state or quality of being low.",
  "ovulate - Containing an ovule or ovules.",
  "betumble - To throw into disorder; to tumble. [R.] From her betumbled couch she starteth. Shak.",
  "macrodont - Having large teeth. -- n.  A macrodont animal.",
  "philalethist - A lover of the truth. [Obs.] Brathwait.",
  "vacillating - Inclined to fluctuate; wavering. Tennyson. -- Vac\"il*la`ting*ly, adv.",
  "metoposcopical - Of or relating to metoposcopy.",
  "venditate - To cry up. as if for sale; to blazon. [Obs.] Holland.",
  "maltreatment - Ill treatment; ill usage; abuse.",
  "palustrine - Of, pertaining to, or living in, a marsh or swamp; marshy.",
  "well-wish - A wish of happiness. \"A well-wish for his friends.\" Addison.",
  "gingival - Of or pertaining to the gums. Holder.",
  "overpowering - Excelling in power; too powerful; irresistible. -- O`ver*pow\"er*ing*ly, adv.",
  "reentrance - The act entereing again; re Hooker.",
  "water mole - (a) The shrew mole. See under Shrew. (b) The duck mole. See under Duck.",
  "sexangular - Having six angles; hexagonal. [R.] Dryden.",
  "dangerful - Full of danger; dangerous. [Obs.] -- Dan\"ger*ful*ly, adv. [Obs.] Udall.",
  "cockneyish - Characteristic of, or resembling, cockneys.",
  "discontentment - The state of being discontented; uneasiness; inquietude. Bacon.",
  "mousseline de soie - A soft thin silk fabric with a weave like that of muslin.",
  "dietitian - One skilled in dietetics. [R.]",
  "conjecturable - Capable of being conjectured or guessed.",
  "sanguinity - The quality of being sanguine; sanguineness. Swift.",
  "supersensible - Beyond the reach of the senses; above the natural powers of perception.",
  "admixtion - A mingling of different things; admixture. Glanvill.",
  "prompt-book - The book used by a prompter of a theater.",
  "nautically - In a nautical manner; with reference to nautical affais.",
  "mary-bud - The marigold; a blossom of the marigold. Shak.",
  "exorbitate - To go out of the track; to deviate. [Obs.] Bentley.",
  "solidago - A genus of yellow-flowered composite perennial herbs; golden- rod.",
  "veiny - Full of veins; veinous; veined; as, veiny marble.",
  "liquorous - Eagerly desirous. See Lickerish. [Obs.] Marston.",
  "wodegeld - A geld, or payment, for wood. Burrill.",
  "angevine - Of or pertaining to Anjou in France. -- n. A native of Anjou.",
  "harborless - Without a harbor; shelterless.",
  "repairer - One who, or that which, repairs, restores, or makes amends.",
  "brinishness - State or quality of being brinish.",
  "starched - 1. Stiffened with starch. 2. Stiff; precise; formal. Swift.",
  "noctambulation - Somnambulism; walking in sleep. Quain.",
  "osteoma - A tumor composed mainly of bone; a tumor of a bone.",
  "globularity - The state of being globular; globosity; sphericity.",
  "furfuration - Falling of scurf from the head; desquamation.",
  "retaliatory - Tending to, or involving, retaliation; retaliative; as retaliatory measures.",
  "mulewort - A fern of the genus Hemionitis.",
  "hopyard - A field where hops are raised.",
  "iodizer - One who, or that which, iodizes.",
  "pickmire - The pewit, or black-headed gull. [Prov. Eng.]",
  "pterostigma - A thickened opaque spot on the wings of certain insects.",
  "aleger - Gay; cheerful; sprightly. [Obs.] Bacon.",
  "carbineer - A soldier armed with a carbine.",
  "nominalistic - Of or pertaining to the Nominalists.",
  "christianization - The act or process of converting or being converted to a true Christianity.",
  "disoxidate - To deoxidate; to deoxidize. [R.]",
  "strontic - Of or pertaining to strontium; containing, or designating the compounds of, strontium.",
  "uranographic - Of or pertaining to uranography; as, an uranographic treatise.",
  "migraine - Same as Megrim. -- Mi*grain\"ous, a.",
  "insubmergible - Not capable of being submerged; buoyant. [R.]",
  "cosmographical - Of or pertaining to cosmography.",
  "sparkful - Lively; brisk; gay. [Obs.] \"Our sparkful youth.\" Camden.",
  "toise - An old measure of length in France, containing six French feet, or about 6.3946 French feet.",
  "oculus - 1. An eye; (Bot.) a leaf bud. 2. (Arch.)  A round window, usually a small one.",
  "sea laces - A kind of seaweed (Chorda Filum) having blackish cordlike fronds, often many feet long.",
  "muriatiferous - Producing muriatic substances or salt. [Obs.]",
  "venantes - The hunting spiders, which run after, or leap upon, their prey.",
  "duality - The quality or condition of being two or twofold; dual character or usage.",
  "exchanger - One who exchanges; one who practices exchange. Matt.",
  "lanifice - Anything made of wool. [Obs.] Bacon.",
  "lithofracteur - An explosive compound of nitroglycerin. See Nitroglycerin.",
  "outscout - To overpower by disdain; to outface. [Obs.] Marston.",
  "deerhound - One of a large and fleet breed of hounds used in hunting deer; a staghound.",
  "sowdanesse - A sultaness. [Obs.] Chaucer.",
  "oberration - A wandering about. [Obs.] Jonhson.",
  "sporogony - The growth or development of an animal or a zooid from a nonsexual germ.",
  "deliverer - 1. One who delivers or rescues; a preserver. 2. One who relates or communicates.",
  "hewhole - The European green woodpecker. See Yaffle.",
  "deific - Making divine; producing a likeness to God; god-making. \"A deifical communion.\" Homilies.",
  "gala - Pomp, show, or festivity. Macaulay. Gala day, a day of mirth and festivity; a holiday.",
  "criticisable - Capable of being criticised.",
  "majoration - Increase; enlargement. [Obs.] Bacon.",
  "smoother - One who, or that which, smooths.",
  "high-strung - Strung to a high pitch; spirited; sensitive; as, a high-strung horse.",
  "kecklish - Inclined to vomit; squeamish. [R.] Holland.",
  "catstitch - To fold and sew down the edge of with a coarse zigzag stitch.",
  "counterwork - To work in oppositeion to; to counteract. That counterworksh folly and caprice. Pope.",
  "pedireme - A crustacean, some of whose feet serve as oars.",
  "advocateship - Office or duty of an advocate.",
  "cosmographic - Of or pertaining to cosmography.",
  "erinaceous - Of the Hedgehog family; like, or characteristic of, a hedgehog.",
  "exsiccate - To exhaust or evaporate moisture from; to dry up. Sir T. Browne.",
  "heptarchic - Of or pertaining to a heptarchy; constituting or consisting of a heptarchy. T. Warton.",
  "denotable - Capable of being denoted or marked. Sir T. Browne.",
  "palo - A pole or timber of any kind; -- in the names of trees. [Sp. Amer.]",
  "zanyism - State or character of a zany; buffoonery. Coleridge. H. Morley.",
  "pinafore - An apron for a child to protect the front part of dress; a tier.",
  "percase - Perhaps; perchance. [Obs.] Bacon.",
  "recapitulatory - Of the nature of a recapitulation; containing recapitulation.",
  "bedlamite - An inhabitant of a madhouse; a madman. \"Raving bedlamites.\" Beattie.",
  "phlebogram - A tracing (with the sphygmograph) of the movements of a vein, or of the venous pulse.",
  "hymnic - Relating to hymns, or sacred lyrics. Donne.",
  "godmother - A woman who becomes sponsor for a child in baptism. See Godfather",
  "mechanicalness - The state or quality of being mechanical.",
  "weighage - A duty or toil paid for weighing merchandise. Bouvier.",
  "uncardinal - To degrade from the cardinalship.",
  "peasweep - (a) The pewit, or lapwing. (b) The greenfinch.",
  "bifoliolate - Having two leaflets, as some compound leaves.",
  "decubation - Act of lying down; decumbence. [Obs.] Evelyn.",
  "pimaric - Pertaining to, or designating, an acid found in galipot, and isomeric with abietic acid.",
  "incognoscible - Incognizable. -- In`cog*nos\"ci*bil\"i*ty, n.",
  "magistrally - In a magistral manner. Abp. Bramhall.",
  "unweeting - Unwitting. [Obs.] Chaucer. Spenser. -- Un*weet\"ing*ly, adv. [Obs.] Milton.",
  "anemometrical - Of or pertaining to anemometry.",
  "hard-tack - A name given by soldiers and sailors to a kind of hard biscuit or sea bread.",
  "outrode - An excursion. [Obs.] \"Outrodes by the ways of Judea.\" Macc. xv. 41 (Geneva Bible).",
  "marvelousness - The quality or state of being marvelous; wonderfulness; strangeness.",
  "cambrasine - A kind of linen cloth made in Egypt, and so named from its resemblance to cambric.",
  "outdoors - Abread; out of the house; out of doors.",
  "overnice - Excessively nice; fastidious. Bp. Hall. -- O\"ver*nice\"ly, adv. -- O\"ver*nice\"ness, n.",
  "feminity - Womanliness; femininity. [Obs.] \"Trained up in true feminity.\" Spenser.",
  "shoaliness - The quality or state of being shoaly; little depth of water; shallowness.",
  "dumbledor - A bumblebee; also, a cockchafer. [Prov. Eng.]",
  "xenyl - The radical characteristic of xenylic compounds.",
  "chairmanship - The office of a chairman of a meeting or organized body.",
  "symphyseal - Of or pertaining to to symphysis.",
  "water monkey - A jar or bottle, as of porous earthenware, in which water is cooled by evaporation.",
  "trigynous - Having three pistils or styles; of or pertaining to the Trigynia.",
  "lapper - One who takes up food or liquid with his tongue.",
  "stuntness - Stuntedness; brevity. [R.] Earle.",
  "adsignify - To denote additionally. [R.] Tooke.",
  "enneaspermous - Having nine seeds; -- said of fruits.",
  "augurous - Full of augury; foreboding. [Obs.] \"Augurous hearts.\" Chapman.",
  "sedged - Made or composed of sedge. With your sedged crowns and ever-harmless looks. Shak.",
  "frostiness - State or quality of being frosty.",
  "tockay - A spotted lizard native of India.",
  "entomic - Relating to insects; entomological.",
  "dotish - Foolish; weak; imbecile. Sir W. Scott.",
  "forehearth - The forward extension of the hearth of a blast furnace under the tymp.",
  "water soldier - An aquatic European plant (Stratiotes aloides) with bayonet- shaped leaves.",
  "ectrotic - Having a tendency to prevent the development of anything, especially of a disease.",
  "materiated - Consisting of matter. [Obs.] Bacon.",
  "teest - A tinsmith's stake, or small anvil.",
  "self-action - Action by, or originating in, one's self or itself.",
  "waitress - A female waiter or attendant; a waiting maid or waiting woman.",
  "trumpets - A plant (Sarracenia flava) with long, hollow leaves.",
  "inhabited - Uninhabited. [Obs.] Brathwait.",
  "serio-comic - Having a mixture of seriousness and sport; serious and comical.",
  "sanation - The act of healing or curing. [Obs.] Wiseman.",
  "permittee - One to whom a permission or permit is given.",
  "self-dependent - Dependent on one's self; self-depending; self-reliant.",
  "amission - Deprivation; loss. [Obs.] Sir T. Browne.",
  "strainable - 1. Capable of being strained. 2. Violent in action. Holinshed.",
  "diffranchisement - See Disfranchise, Disfranchisement.",
  "disenroll - To erase from a roll or list. [Written also disenrol.] Donne.",
  "breastplough - A kind of plow, driven by the breast of the workman; -- used to cut or pare turf.",
  "oostegite - One of the plates which in some Crustacea inclose a cavity wherein the eggs are hatched.",
  "browbound - Crowned; having the head encircled as with a diadem. Shak.",
  "erubescent - Red, or reddish; blushing. Johnson.",
  "intermittingly - With intermissions; at intervals. W. Montagu.",
  "metallist - A worker in metals, or one skilled in metals.",
  "barillet - A little cask, or something resembling one. Smart.",
  "aplastic - Not plastic or easily molded.",
  "expugnation - The act of taking by assault; conquest. [R.] Sandys.",
  "retractate - To retract; to recant. [Obs.]",
  "authorizable - Capable of being authorized. Hammond.",
  "softish - Somewhat soft. De Witt Clinton.",
  "juneating - A kind of early apple. [Written also jenneting.]",
  "iniquous - Iniquitous. [Obs.] Sir T. Browne.",
  "notionist - One whose opinions are ungrounded notions. [R.] Bp. Hopkins.",
  "staller - A standard bearer. obtaining Fuller.",
  "epiphyllous - Growing upon, or inserted into, the leaf.",
  "midrash - A talmudic exposition of the Hebrew law, or of some part of it.",
  "neighborship - The state of being neighbors. [R.] J. Bailie.",
  "deave - To stun or stupefy with noise; to deafen. [Scot.]",
  "danubian - Pertainingto, or bordering on, the river Danube.",
  "suppeditate - To supply; to furnish. [Obs.] Hammond.",
  "monomachia - A duel; single combat. \"The duello or monomachia.\" Sir W. Scott.",
  "hern - A heron; esp., the common European heron. \"A stately hern.\" Trench.",
  "paneless - Without panes. To patch his paneless window. Shenstone.",
  "fossiliferous - Containing or composed of fossils.",
  "arvicole - A mouse of the genus Arvicola; the meadow mouse. There are many species.",
  "mordicant - Biting; acrid; as, the mordicant quality of a body. [R.] Boyle.",
  "fatiscence - A gaping or opening; state of being chinky, or having apertures. Kirwan.",
  "parallelopipedon - A parallelopiped. Hutton.",
  "irrebuttable - Incapable of being rebutted. Coleridge.",
  "disunionist - An advocate of disunion, specifically, of disunion of the United States.",
  "immiscibility - Incapability of being mixed, or mingled.",
  "dartos - A thin layer of peculiar contractile tissue directly beneath the skin of the scrotum.",
  "seldshewn - Rarely shown or exhibited. [Obs.] Shak.",
  "cortes geraes - See Legislature, Portugal.",
  "thoroughly - In a thorough manner; fully; entirely; completely.",
  "verst - A Russian measure of length containing 3,500 English feet. [Written also werst.]",
  "make and break - Any apparatus for making and breaking an electric circuit; a circuit breaker.",
  "stifftail - The ruddy duck. [Local, U.S.]",
  "hability - Ability; aptitude. [Obs.] Robynson. (More's Utopia).",
  "solacious - Affording solace; as, a solacious voice. [Obs.] Bale.",
  "considerance - Act of considering; consideration. [Obs.] Shak.",
  "archdeaconship - The office of an archdeacon.",
  "forold - Very old. [Obs.] A bear's skin, coal-black, forold. Chaucer.",
  "laplandish - Of or pertaining to Lapland.",
  "dysphonia - A difficulty in producing vocal sounds; enfeebled or depraved voice.",
  "rememoration - A recalling by the faculty of memory; remembrance. [Obs. & R.] Bp. Montagu.",
  "torquated - Having or wearing a torque, or neck chain.",
  "canon bit - That part of a bit which is put in a horse's mouth.",
  "kempe - Rough; shaggy. [Obs.] \"Kempe hairs.\" Chaucer.",
  "town-crier - A town officer who makes proclamations to the people; the public crier of a town.",
  "grantor - The person by whom a grant or conveyance is made.",
  "phrenism - See Vital force, under Vital.",
  "beleper - To infect with leprosy. [Obs.] Beau. & Fl.",
  "undersky - The lower region of the sky. Floating about the undersky. Tennyson.",
  "knightless - Unbecoming a knight. [Obs.] \"Knightless guile.\" Spenser.",
  "cambria - The ancient Latin name of Wales. It is used by modern poets.",
  "alated - Winged; having wings, or side appendages like wings.",
  "bracteal - Having the nature or appearance of a bract.",
  "bonify - To convert into, or make, good. To bonify evils, or tincture them with good. Cudworth.",
  "necrose - To affect with necrosis; to unergo necrosis. Quain.",
  "wolframic - Of or pertaining to wolframium. See Tungstic.",
  "belsire - A grandfather, or ancestor. \"His great belsire Brute.\" [Obs.] Drayton.",
  "mixen - A compost heap; a dunghill. Chaucer. Tennyson.",
  "outerly - 1. Utterly; entirely. [Obs.] Chaucer. 2. Toward the outside. [R.] Grew.",
  "shasta daisy - A large-flowered garden variety of the oxeye daisy.",
  "sitology - A treatise on the regulation of the diet; dietetics. [Written also sitiology.]",
  "antimason - One opposed to Freemasonry. -- An`ti*ma*son\"ic, a.",
  "keyseat - To form a key seat, as by cutting. See Key seat, under Key.",
  "november - The eleventh month of the year, containing thirty days.",
  "equisonance - An equal sounding; the consonance of the unison and its octaves.",
  "nibblingly - In a nibbling manner; cautiously.",
  "quaternate - Composed of, or arranged in, sets of four; quaternary; as, quaternate leaves.",
  "mutage - A process for checking the fermentation of the must of grapes.",
  "tardiness - The quality or state of being tardy.",
  "crumbly - EAsily crumbled; friable; brittle. \"The crumbly soil.\" Hawthorne.",
  "cymling - A scalloped or \"pattypan\" variety of summer squash.",
  "white-water - A dangerous disease of sheep.",
  "latescent - Slightly withdrawn from view or knowledge; as, a latescent meaning. Sir W. Hamilton.",
  "urbicolous - Of or pertaining to a city; urban. [R.]",
  "majestatal - Majestic. [Obs.] E. Pocock. Dr. J. Scott.",
  "scleragogy - Severe discipline. [Obs.] Bp. Hacket.",
  "thermotactic - Of or retaining to thermotaxis.",
  "cinchonic - Belonging to, or obtained from, cinchona. Mayne.",
  "multisyllable - A word of many syllables; a polysyllable. [R.] -- Mul`ti*syl*lab\"ic, a.",
  "sea holly - An evergeen seashore plant (Eryngium maritimum). See Eryngium.",
  "junkerism - The principles of the aristocratic party in Prussia.",
  "tray-trip - An old game played with dice. [Obs.] Shak.",
  "account book - A book in which accounts are kept. Swift.",
  "fleetness - Swiftness; rapidity; velocity; celerity; speed; as, the fleetness of a horse or of time.",
  "temporally - In a temporal manner; secularly. [R.] South.",
  "v hook - A gab at the end of an eccentric rod, with long jaws, shaped like the letter V.",
  "hare-hearted - Timorous; timid; easily frightened. Ainsworth.",
  "instop - To stop; to close; to make fast; as, to instop the seams. [Obs.] Dryden.",
  "importunacy - The quality of being importunate; importunateness.",
  "gauffer - To plait, crimp, or flute; to goffer, as lace. See Goffer.",
  "cloggy - Clogging, or having power to clog.",
  "jeames - A footman; a flunky. [Slang, Eng.] Thackeray.",
  "wet nurse - A nurse who suckles a child, especially the child of another woman. Cf. Dry nurse.",
  "kyanol - (a) Aniline. [Obs.] (b) A base obtained from coal tar. Ure.",
  "puna - A cold arid table-land, as in the Andes of Peru.",
  "bidigitate - Having two fingers or fingerlike projections.",
  "perempt - To destroy; to defeat. [R.] Ayliffe.",
  "sea language - The peculiar language or phraseology of seamen; sailor's cant.",
  "psychiatry - The application of the healing art to mental diseases. Dunglison.",
  "conversationism - A word or phrase used in conversation; a colloqualism.",
  "outfrown - To frown down; to overbear by frowning. Shak.",
  "whacking - Very large; whapping. [Colloq.]",
  "marmoreal - Pertaining to, or resembling, marble; made of marble.",
  "pugilist - One who fights with his fists; esp., a professional prize fighter; a boxer.",
  "sentimentality - The quality or state of being sentimental.",
  "karakul - Astrakhan, esp. in fine grades. Cf. Caracul.",
  "pulsific - Exciting the pulse; causing pulsation.",
  "paum - To palm off by fraud; to cheat at cards. [Obs.] Swift.",
  "anthroposophy - Knowledge of the nature of man; hence, human wisdom.",
  "stagery - Exhibition on the stage. [Obs.]",
  "meteorography - The registration of meteorological phenomena.",
  "effigiation - The act of forming in resemblance; an effigy. Fuller.",
  "lunulated - Resembling a small crescent. Gray.",
  "dungmeer - A pit where dung and weeds rot for manure.",
  "bacteroid - Resembling bacteria; as, bacteroid particles.",
  "frost-bitten - Nipped, withered, or injured, by frost or freezing.",
  "mycological - Of or relating to mycology, or the fungi.",
  "sacchariferous - Producing sugar; as, sacchariferous canes.",
  "mechanography - The art of mechanically multiplying copies of a writing, or any work of art.",
  "seniorize - To exercise authority; to rule; to lord it. [R.] Fairfax.",
  "innutritious - Not nutritious; not furnishing nourishment.",
  "frons - The forehead; the part of the cranium between the orbits and the vertex.",
  "oxysulphide - A ternary compound of oxygen and sulphur.",
  "crazing-mill - A mill for grinding tin ore.",
  "rockiness - The state or quality of being rocky.",
  "tenotome - A slender knife for use in the operation of tenotomy.",
  "amovability - Liability to be removed or dismissed from office. [R.] T. Jefferson.",
  "benignant - Kind; gracious; favorable. -- Be*nig\"nant*ly, adv.",
  "reedify - To edify anew; to build again after destruction. [R.] Milton.",
  "concite - To excite or stir up. [Obs.] Cotgrave.",
  "headsail - Any sail set forward of the foremast. Totten.",
  "galapee tree - The West Indian Sciadophyllum Brownei, a tree with very large digitate leaves.",
  "legitimately - In a legitimate manner; lawfully; genuinely.",
  "zooegrapher - One who describes animals, their forms and habits.",
  "generalty - Generality. [R.] Sir M. Hale.",
  "descensory - A vessel used in alchemy to extract oils.",
  "laggingly - In a lagging manner; loiteringly.",
  "endemiology - The science which treats of endemic affections.",
  "livingly - In a living state. Sir T. Browne.",
  "totemic - Of or pertaining to a totem, or totemism.",
  "rideau - A small mound of earth; ground slightly elevated; a small ridge.",
  "morulation - The process of cleavage, or segmentation, of the ovum, by which a morula is formed.",
  "measelry - Leprosy. [Obs.] R. of Brunne.",
  "osteology - The science which treats of the bones of the vertebrate skeleton.",
  "kingtruss - A truss, framed with a king-post; -- used in roofs, bridges, etc.",
  "turband - A turban. Balfour (Cyc. of Ind.).",
  "bicornous - Having two horns; two-horned; crescentlike.",
  "ceroplastics - The art of modeling in wax.",
  "dejeration - The act of swearing solemnly. [Obs.] Bp. Hall.",
  "exemptitious - Separable. [Obs.] \"Exemptitious from matter.\" Dr. H. More.",
  "outhess - Outcry; alarm. [Obs.] Chaucer.",
  "splendidness - The quality of being splendid.",
  "tensity - The quality or state of being tense, or strained to stiffness; tension; tenseness.",
  "centesimo - A copper coin of Italy and Spain equivalent to a centime.",
  "electrographic - Of or pertaining to an electrograph or electrography.",
  "flabile - Liable to be blown about. Bailey.",
  "epicondyle - A projection on the inner side of the distal end of the numerus; the internal condyle.",
  "phono- - A combining form from Gr. sound, tone; as, phonograph, phonology.",
  "bolognian - Bolognese. Bolognian stone. See Bologna stone, under Bologna.",
  "geomantic - Pertaining or belonging to geomancy.",
  "asbestic - Of, pertaining to, or resembling asbestus; inconsumable; asbestine.",
  "degerm - To extract the germs from, as from wheat grains.",
  "syrtic - Of or pertaining to a syrt; resembling syrt, or quicksand. [R.] Ed. Rev.",
  "devolvement - The act or process of devolving;; devolution.",
  "actinic - Of or pertaining to actinism; as, actinic rays.",
  "constructional - Pertaining to, or deduced from, construction or interpretation.",
  "driftage - 1. Deviation from a ship's course due to leeway. 2. Anything that drifts.",
  "calendrical - Of or pertaining to a calendar.",
  "ecclesiastically - In an ecclesiastical manner; according ecclesiastical rules.",
  "hag-taper - The great woolly mullein (Verbascum Thapsus).",
  "stirless - Without stirring; very quiet; motionless. \"Lying helpless and stirless.\" Hare.",
  "ronion - A mangy or scabby creature. \"Aroint thee, with!\" the rump-fed ronyon cries. Shak.",
  "decivilize - To reduce from civilization to a savage state. [R.] Blackwood's Mag.",
  "micrometry - The art of measuring with a micrometer.",
  "diastasis - A forcible of bones without fracture.",
  "kobellite - A blackish gray mineral, a sulphide of antimony, bismuth, and lead.",
  "fleury - Finished at the ends with fleurs-de-lis; -- said esp. a cross so decorated.",
  "immeritous - Undeserving. [Obs.] Milton.",
  "hymnologist - A composer or compiler of hymns; one versed in hymnology. Busby.",
  "upsyturvy - Upside down; topsy-turvy. [Obs.] Robert Greene.",
  "recubation - Recumbence. [Obs.] Sir T. Browne.",
  "oneiroscopist - One who interprets dreams.",
  "pouchong - A superior kind of souchong tea. De Colange.",
  "ladylikeness - The quality or state of being ladylike.",
  "angustation - The act or making narrow; a straitening or contacting. Wiseman.",
  "vegetarianism - The theory or practice of living upon vegetables and fruits.",
  "multiloquy - Excess of words or talk. [R.]",
  "trayful - As much as a tray will hold; enough to fill a tray.",
  "phatagin - The long-tailed pangolin (Manis tetradactyla); -- called also ipi.",
  "narcotical - Narcotic. -- Nar*cot\"ic*al*ly, adv.",
  "magnesic - Pertaining to, or containing, magnesium; as, magnesic oxide.",
  "cheeringly - In a manner to cheer or encourage.",
  "grandaunt - The aunt of one's father or mother.",
  "spirituosity - The quality or state of being spirituous; spirituousness. [R.]",
  "coctile - Made by baking, or exposing to heat, as a brick.",
  "incalescent - Growing warm; increasing in heat.",
  "bonhomie - good nature; pleasant and easy manner.",
  "prolificness - The quality or state of being prolific; fruitfulness; prolificacy.",
  "entermete - To interfere; to intermeddle. [Obs.] Chaucer.",
  "featness - Skill; adroitness. [Archaic] Johnson.",
  "scrofulide - Any affection of the skin dependent on scrofula.",
  "pigtailed - Having a tail like a pig's; as, the pigtailed baboon.",
  "bridesmaid - A female friend who attends on a bride at her wedding.",
  "meloplastic - Of or pertaining to meloplasty, or the artificial formation of a new cheek.",
  "erratical - Erratic. -- Er*rat\"ic*al*ly, adv. -- Er*rat\"ic*al*ness, n.",
  "catelectrode - The negative electrode or pole of a voltaic battery. Faraday.",
  "ecbasis - A figure in which the orator treats of things according to their events consequences.",
  "entreative - Used in entreaty; pleading. [R.] \"Entreative phrase.\" A. Brewer.",
  "incrimination - The act of incriminating; crimination.",
  "rheae - A suborder of struthious birds including the rheas.",
  "rakel - Hasty; reckless; rash. [Obs.] Chaucer. -- Ra\"kel*ness, n. [Obs.] Chaucer.",
  "tithonicity - The state or property of being tithonic; actinism. [R.]",
  "dog-headed - Having a head shaped like that of a dog; -- said of certain baboons.",
  "defectionist - One who advocates or encourages defection.",
  "minimization - The act or process of minimizing. Bentham.",
  "accuser - One who accuses; one who brings a charge of crime or fault.",
  "satiny - Like or composed of satin; glossy; as, to have a satiny appearance; a satiny texture.",
  "malcontented - Malcontent. -- Mal`con*tent\"ed*ly, adv. -- Mal`con*tent\"ed*ness, n.",
  "confeder - To confederate. [Obs.] Sir T. North.",
  "alienor - One who alienates or transfers property to another. Blackstone.",
  "shram - To cause to shrink or shrivel with cold; to benumb. [Prov. Eng.]",
  "eminently - In an eminent manner; in a high degree; conspicuously; as, to be eminently learned.",
  "nutrication - The act or manner of feeding. [Obs.] Sir T. Browne.",
  "daguerreotypy - The art or process of producing pictures by method of Daguerre.",
  "falsifier - One who falsifies, or gives to a thing a deceptive appearance; a liar.",
  "decoloration - The removal or absence of color. Ferrand.",
  "in vacuo - In a vacuum; in empty space; as, experiments in vacuo.",
  "mantologist - One who is skilled in mantology; a diviner. [R.]",
  "coinhere - To inhere or exist together, as in one substance. Sir W. Hamilton.",
  "incisory - Having the quality of cutting; incisor; incisive.",
  "handmaid - A maid that waits at hand; a female servant or attendant.",
  "neurosensiferous - Pertaining to, or forming, both nerves and sense organs.",
  "antasthmatic - Opposing, or fitted to relieve, asthma. -- n.  A remedy for asthma.",
  "zayat - A public shed, or portico, for travelers, worshipers, etc. [Burmah]",
  "imbound - To inclose in limits; to shut in. [Obs.] Shak.",
  "deep-laid - Laid deeply; formed with cunning and sagacity; as, deep-laid plans.",
  "adhibition - The act of adhibiting; application; use. Whitaker.",
  "consumptively - In a way tending to or indication consumption. Beddoes.",
  "diseasefulness - The quality of being diseaseful; trouble; trial. [R.] Sir P. Sidney.",
  "extuberant - Swollen out; protuberant. [R.] \"Extuberant lips.\" Gayton.",
  "phylloxanthin - A yellow coloring matter extracted from chlorophyll.",
  "strepsipterous - Of or pertaining to Strepsiptera.",
  "patellar - Of or pertaining to the patella, or kneepan.",
  "unceasable - Not capable of being ended; unceasing. [R.]",
  "pyrotechnics - The art of making fireworks; the manufacture and use of fireworks; pyrotechny.",
  "invitrifiable - Not admitting of being vitrified, or converted into glass. Kirwan.",
  "lucidness - The quality of being lucid; lucidity.",
  "tigress - The female of the tiger. Holland.",
  "outstay - To stay beyond or longer than. She concluded to outstay him. Mad. D' Arblay.",
  "obversant - Conversant; familiar. [Obs.] Bacon.",
  "clean-limbed - With well-proportioned, unblemished limbs; as, a clean-limbed young fellow. Dickens.",
  "availably - In an available manner; profitably; advantageously; efficaciously.",
  "polybromide - A bromide containing more than one atom of bromine in the molecule.",
  "queenship - The state, rank, or dignity of a queen.",
  "affectationist - One who exhibits affectation. [R.] Fitzed. Hall.",
  "unbeseem - To be unbecoming or unsuitable to; to misbecome.",
  "obuncous - Hooked or crooked in an extreme degree. Maunder.",
  "stuffiness - The quality of being stuffy.",
  "cavalero - A cavalier; a gallant; a libertine. Shak.",
  "surculate - To purne; to trim. [Obs.] Cockeram.",
  "chelidon - The hollow at the flexure of the arm.",
  "fluoroid - A tetrahexahedron; -- so called because it is a common form of fluorite.",
  "metapode - The posterior division of the foot in the Gastropoda and Pteropoda.",
  "sheetful - Enough to fill a sheet; as much as a sheet can hold.",
  "hydrophytology - The branch of botany which treats of water plants.",
  "taunting - from Taunt, v. Every kind of insolent and taunting reflection. Burke.",
  "whenever - At whatever time. \"Whenever that shall be.\" Milton.",
  "brazen-browed - Shamelessly impudent. Sir T. Browne.",
  "algidity - Chilliness; coldness; especially (Med.),  coldness and collapse.",
  "torril - A worthless woman; also, a worthless horse. [Prov. Eng.] Halliwell.",
  "unsignificant - Insignificant. [Obs.] Holland.",
  "panamanian - Of or pert. to Panama. -- n.  A native or citizen of Panama.",
  "chlorotic - Pertaining to, or affected by, chlorosis.",
  "magaziner - One who edits or writes for a magazine. [R.] Goldsmith.",
  "abuzz - In a buzz; buzzing. [Colloq.] Dickens.",
  "heretical - Containing heresy; of the nature of, or characterized by, heresy.",
  "snape - To bevel the end of a timber to fit against an inclined surface.",
  "nomocracy - Government in accordance with a system of law. Milman.",
  "repositor - An instrument employed for replacing a displaced organ or part.",
  "misattend - To misunderstand; to disregard. [Obs.] Milton.",
  "vitrescible - That may be vitrified; vitrifiable.",
  "myopathia - Any affection of the muscles or muscular system.",
  "interluder - An actor who performs in an interlude. B. Jonson.",
  "pilement - An accumulation; a heap. [Obs.] Bp. Hall.",
  "infernally - In an infernal manner; diabolically. \"Infernally false.\" Bp. Hacket.",
  "anti-trade - A tropical wind blowing steadily in a direction opposite to the trade wind.",
  "banal - Commonplace; trivial; hackneyed; trite.",
  "bundobust - System; discipline. [India] He has more bundobust than most men. Kipling.",
  "walty - Liable to roll over; crank; as, a walty ship. [R.] Longfellow.",
  "deflourer - One who deflours; a ravisher.",
  "pepperer - A grocer; -- formerly so called because he sold pepper. [Obs.]",
  "pagoda sleeve - A funnel-shaped sleeve arranged to show the sleeve lining and an inner sleeve.",
  "pentandrian - Of or pertaining to the class Pentadria; having five stamens.",
  "pomme - Having the ends terminating in rounded protuberances or single balls; -- said of a cross.",
  "perjurer - One who is guilty of perjury; one who perjures or forswears, in any sense.",
  "esox - A genus of fresh-water fishes, including pike and pickerel.",
  "superposable - Capable of being superposed, as one figure upon another.",
  "thetical - Laid down; absolute or positive, as a law. Dr. H. More.",
  "delightable - Capable of delighting; delightful. [Obs.] Many a spice delightable. Rom. of R.",
  "gravic - Pertaining to, or causing, gravitation; as, gravic forces; gravic attraction. [R.]",
  "polt-foot - Having a distorted foot, or a clubfoot or clubfeet. B. Jonson.",
  "interpetalary - Between the petals of a flower.",
  "appendectomy - Excision of the vermiform appendix.",
  "quadrisyllable - A word consisting of four syllables. De Quincey.",
  "resinic - Pertaining to, or obtained from, resin; as, the resinic acids.",
  "melancholiness - The state or quality of being melancholy. Hallywell.",
  "subligation - The act of binding underneath. [R.]",
  "burglariously - With an intent to commit burglary; in the manner of a burglar. Blackstone.",
  "waterie - The pied wagtail; -- so called because it frequents ponds.",
  "high-raised - 1. Elevated; raised aloft; upreared. 2. Elated with great ideas or hopes. Milton.",
  "intermedial - Lying between; intervening; intermediate. \"Intermedial colors.\" Evelyn.",
  "sanjak - A district or a subvision of a vilayet. [Turkey]",
  "annumerate - To add on; to count in. [Obs.] Wollaston.",
  "parturient - Bringing forth, or about to bring forth, young; fruitful. Jer. Tailor.",
  "aurin - A red coloring matter derived from phenol; -- called also, in commerce, yellow coralin.",
  "intrunk - To inclose as in a trunk; to incase. [R.] Ford.",
  "insultment - Insolent treatment; insult. [Obs.] \"My speech of insultment ended.\" Shak.",
  "polychloride - A chloride containing more than one atom of chlorine in the molecule.",
  "erectable - Capable of being erected; as, an erectable feather. Col. G. Montagu.",
  "trumpet-shaped - Tubular with one end dilated, as the flower of the trumpet creeper.",
  "wool-dyed - Dyed before being made into cloth, in distinction from piece- dyed; ingrain.",
  "emetical - Inducing to vomit; producing vomiting; emetic. -- E*met\"ic*al*ly, adv.",
  "deceitless - Free from deceit. Bp. Hall.",
  "water pipe - A pipe for conveying water.",
  "thyrohyoid - Of or pertaining to the thyroid cartilage of the larynx and the hyoid arch.",
  "color-blind - Affected with color blindness. See Color blindness, under Color, n.",
  "rattle-pated - Rattle-headed. \"A noisy, rattle-pated fellow.\" W. Irving.",
  "righter - One who sets right; one who does justice or redresses wrong. Shelton.",
  "feofor - One who enfeoffs or grants a fee.",
  "seorita - A Spanish title of courtesy given to a young lady; Miss; also, a young lady.",
  "equableness - Quality or state of being equable.",
  "cest - A woman's girdle; a cestus. [R.] Collins.",
  "consignificative - Consignificant; jointly significate. [R.]",
  "sarcorhamphi - A division of raptorial birds composing the vultures.",
  "hypostome - The lower lip of trilobites, crustaceans, etc.",
  "vaginitis - Inflammation of the vagina, or the genital canal, usually of its mucous living membrane.",
  "dolabra - A rude ancient ax or hatchet, seen in museums.",
  "pieno - Full; having all the instruments.",
  "smerky - Smart; jaunty; spruce. See Smirk, a. [Obs.] So smerk, so smooth, his pricked ears. Spenser.",
  "brittle star - Any species of ophiuran starfishes. See Ophiuroidea.",
  "sea pig - (a) A porpoise or dolphin. (b) A dugong.",
  "dinginess - Quality of being dingy; a dusky hue.",
  "dossier - A bundle containing the papers in reference to some matter.",
  "smorzando - Growing gradually fainter and softer; dying away; morendo.",
  "pittle-pattle - To talk unmeaningly; to chatter or prattle. [R.] Latimer.",
  "supersphenoidal - Situated above, or on the dorsal side of, the body of the sphenoid bone.",
  "toxin - A poisonous product formed by pathogenic bacteria, as a toxic proteid or poisonous ptomaine.",
  "impave - To pave. [Poetic] Impaved with rude fidelity Of art mosaic. Wordsworth.",
  "insolidity - Want of solidity; weakness; as, the insolidity of an argument. [R.] Dr. H. More.",
  "assuredly - Certainly; indubitably. \"The siege assuredly I'll raise.\" Shak.",
  "pinnock - (a) The hedge sparrow. [Prov. Eng.] (b) The tomtit.",
  "sallowness - The quality or condition of being sallow. Addison.",
  "futureless - Without prospect of betterment in the future. W. D. Howells.",
  "photophonic - Of or pertaining to photophone.",
  "plenipotent - Possessing full power. [R.] Milton.",
  "preyer - One who, or that which, preys; a plunderer; a waster; a devourer. Hooker.",
  "drawgloves - An old game, played by holding up the fingers. Herrick.",
  "melic grass - A genus of grasses (Melica) of little agricultural importance.",
  "philologue - A philologist. [R.] Carlyle.",
  "inequilobate - Unequally lobed; cut into lobes of different shapes or sizes.",
  "catechist - One who instructs by question and answer, especially in religions matters.",
  "reconsecrate - To consecrate anew or again.",
  "bicarburetted - Containing two atoms or equivalents of carbon in the molecule. [Obs. or R.]",
  "mundic - Iron pyrites, or arsenical pyrites; -- so called by the Cornish miners.",
  "blandiloquous - Fair-spoken; flattering.",
  "charioteer - 1. One who drives a chariot. 2. (Astron.)  A constellation. See Auriga, and Wagones.",
  "evaluation - Valuation; appraisement. J. S. Mill.",
  "infraterritorial - Within the territory of a state. Story.",
  "costumer - One who makes or deals in costumes, as for theaters, fancy balls, etc.",
  "naughtily - In a naughty manner; wickedly; perversely. Shak.",
  "parochiality - The state of being parochial. [R.] Sir J. Marriot.",
  "gemmuliferous - Bearing or producing gemmules or buds.",
  "preindispose - To render indisposed beforehand. Milman.",
  "subalternant - A universal proposition. See Subaltern, 2. Whately.",
  "testicular - Of or pertaining to the testicle.",
  "conformance - Conformity. [R.]  Marston.",
  "blickey - A tin dinner pail. [Local, U. S.] Bartlett.",
  "decardinalize - To depose from the rank of cardinal.",
  "fin-footed - (a) Having palmate feet. (b) Having lobate toes, as the coot and grebe.",
  "turriculate - Furnished with, or formed like, a small turret or turrets; somewhat turreted.",
  "kjoekken moeddings - See Kitchen middens.",
  "pusher - One who, or that which, pushes.",
  "quintic - Of the fifth degree or order. -- n. (Alg.)  A quantic of the fifth degree. See Quantic.",
  "religieux - A person bound by monastic vows; a nun; a monk.",
  "probe-pointed - Having a blunt or button-shaped extremity; -- said of cutting instruments.",
  "idealogue - One given to fanciful ideas or theories; a theorist; a spectator. [R.] Mrs. Browning.",
  "permissibility - The quality of being permissible; permissibleness; allowableness.",
  "techiness - The quality or state of being techy.",
  "pyrotartrate - A salt of pyrotartaric acid.",
  "philoprogenitive - Having the love of offspring; fond of children.",
  "querist - One who inquires, or asks questions. Swift.",
  "predeliberation - Previous deliberation.",
  "stockjobbing - The act or art of dealing in stocks; the business of a stockjobber.",
  "crinital - Same as Crinite, 1. He the star crinital adoreth. Stanyhurst.",
  "grossly - In a gross manner; greatly; coarsely; without delicacy; shamefully; disgracefully.",
  "loopie - Deceitful; cunning; sly. [Scot.]",
  "difflation - A blowing apart or away. [Obs.] Bailey.",
  "hallage - A fee or toll paid for goods sold in a hall.",
  "storeship - A vessel used to carry naval stores for a fleet, garrison, or the like.",
  "wonders - See Wondrous. [Obs.] They be wonders glad thereof. Sir T. More.",
  "interestingly - In an interesting manner.",
  "theorically - In a theoretic manner. [Obs.]",
  "exemplifier - One who exemplifies by following a pattern.",
  "quakerly - Resembling Quakers; Quakerlike; Quakerish. Macaulay.",
  "accretive - Relating to accretion; increasing, or adding to, by growth. Glanvill.",
  "ark shell - A marine bivalve shell belonging to the genus Arca and its allies.",
  "baken - p. p. of Bake. [Obs. or. Archaic]",
  "mistrial - A false or erroneous trial; a trial which has no result.",
  "helispherical - Spiral. Helispherical line (Math.). the rhomb line in navigation. [R.]",
  "cosmogonist - One who treats of the origin of the universe; one versed in cosmogony.",
  "nilometer - An instrument for measuring the rise of water in the Nile during its periodical flood.",
  "archival - Pertaining to, or contained in, archives or records. Tooke.",
  "intervert - To turn to another course or use. [Obs.] Sir H. Wotton.",
  "pheasantry - A place for keeping and rearing pheasants. Gwilt.",
  "calcography - The art of drawing with chalk.",
  "tryptic - Relating to trypsin or to its action; produced by trypsin; as, trypsin digestion.",
  "disappropriation - The act of disappropriating.",
  "interstinctive - Distinguishing. [Obs.] Wallis.",
  "appellativeness - The quality of being appellative. Fuller.",
  "spicate - Having the form of a spike, or ear; arranged in a spike or spikes. Lee.",
  "renouncement - The act of disclaiming or rejecting; renunciation. Shak.",
  "unconning - Not knowing; ignorant. [Obs.] Chaucer. -- n.  Ignorance. [Obs.]",
  "deviator - One who, or that which, deviates.",
  "phyllopodous - Of or pertaining to the Phyllopoda.",
  "gravimeter - (Physics) An instrument for ascertaining the specific gravity of bodies.",
  "homogeneousness - Sameness 9kind or nature; uniformity of structure or material.",
  "aftereye - To look after. [Poetic] Shak.",
  "arquated - Shaped like a bow; arcuate; curved. [R.]",
  "antivenereal - Good against venereal poison; antisyphilitic.",
  "kiefekil - A species of clay; meerschaum. [Also written keffekil.]",
  "mandil - A loose outer garment worn the 16th and 17th centuries.",
  "wheelwork - A combination of wheels, and their connection, in a machine or mechanism.",
  "reflower - To flower, or cause to flower, again. Sylvester.",
  "postremote - More remote in subsequent time or order.",
  "technicological - Technological; technical. [R.] Dr. J. Scott.",
  "fundholder - One who has money invested in the public funds. J. S. Mill.",
  "embillow - To swell or heave like a [R.] Lisle.",
  "moneran - Of or pertaining to the Monera. -- n.  One of the Monera.",
  "gurt - A gutter or channel for water, hewn out of the bottom of a working drift. Page.",
  "detersive - Cleansing; detergent. -- n.  A cleansing agent; a detergent.",
  "extraneity - State of being without or beyond a thing; foreignness. [Obs.]",
  "rewin - To win again, or win back. The Palatinate was not worth the rewinning. Fuller.",
  "stulm - A shaft or gallery to drain a mine. [Local, Eng.] Bailey.",
  "excrescential - Pertaining to, or resembling, an excrescence. [R.] Hawthorne.",
  "corniform - Having the shape of a horn; horn-shaped.",
  "dungy - Full of dung; filthy; vile; low. Shak.",
  "biologist - A student of biology; one versed in the science of biology.",
  "deligation - A binding up; a bandaging. Wiseman.",
  "plowable - Capable of being plowed; arable.",
  "seedlop - A vessel in which a sower carries the seed to be scattered. [Prov. Eng.]",
  "redeemableness - The quality or state of being redeemable; redeemability.",
  "succory - A plant of the genus Cichorium. See Chicory.",
  "ingot steel - Steel cast in ingots from the Bessemer converter or open-hearth furnace.",
  "anthraciferous - Yielding anthracite; as, anthraciferous strata.",
  "foretaster - One who tastes beforehand, or before another.",
  "cawky - Of or pertaining to cawk; like cawk.",
  "brize - The breeze fly. See Breeze. Shak.",
  "mistransport - To carry away or mislead wrongfully, as by passion. [Obs.] Bp. Hall.",
  "eyeful - Filling or satisfying the eye; visible; remarkable. [Obs.] \"Eyeful trophies.\" Chapman.",
  "stereotypography - The act or art of printing from stereotype plates.",
  "debeige - A kind of woolen or mixed dress goods. [Written also debage.]",
  "water bailiff - An officer of the customs, whose duty it is to search vessels. [Eng.]",
  "unmuzzle - To loose from a muzzle; to remove a muzzle from.",
  "reinterrogate - To interrogate again; to question repeatedly. Cotgrave.",
  "concurrently - With concurrence; unitedly.",
  "pulverizable - Admitting of being pulverized; pulverable. Barton.",
  "tohubohu - Chaos; confusion. Was ever such a tohubohu of people as there assembles Thuckeray.",
  "rhymeless - Destitute of rhyme. Bp. Hall.",
  "obimbricate - Imbricated, with the overlapping ends directed downward.",
  "casal - Of or pertaining to case; as, a casal ending.",
  "infallibly - In an infallible manner; certainly; unfailingly; unerringly. Blair.",
  "concreate - To create at the same time. If God did concreate grace with Adam. Jer. Taylor.",
  "histrionize - To act; to represent on the stage, or theatrically. Urquhart.",
  "globated - Having the form of a globe; spherical.",
  "pontile - Of or pertaining to the pons Varolii. See Pons.",
  "inauration - The act or process of gilding or covering with gold.",
  "trophied - Adorned with trophies. The trophied arches, storied halls, invade. Pope.",
  "plec-tognathous - Of or pertaining to the Plectognathi.",
  "londonism - A characteristic of Londoners; a mode of speaking peculiar to London.",
  "supernaturalize - To treat or regard as supernatural.",
  "underminister - To serve, or minister to, in a subordinate relation. [Obs.] Wyclif.",
  "decliner - He who declines or rejects. A studious decliner of honors. Evelyn.",
  "detestability - Capacity of being odious. [R.] Carlyle.",
  "chekelatoun - See Ciclatoun. [Obs.] Chaucer.",
  "tralatition - A change, as in the use of words; a metaphor.",
  "stibium - 1. (Chem.)  The technical name of antimony. 2. (Min.)  Stibnite. [Obs.]",
  "sea widgeon - (a) The scaup duck. (b) The pintail duck.",
  "outbid - To exceed or surpass in bidding. Prevent the greedy, and outbid the bold. Pope.",
  "ribbonism - The principles and practices of the Ribbonmen. See Ribbon Society, under Ribbon.",
  "dyewood - Any wood from which coloring matter is extracted for dyeing.",
  "devenustate - To deprive of beauty or grace. [Obs.]",
  "exultance - Exultation. [Obs.] Burton. Hammond.",
  "spinosity - The quality or state of being spiny or thorny; spininess.",
  "faffle - To stammer. [Prov. Eng.] Halliwell.",
  "wigher - To neigh; to whinny. [Obs.] Beau. & Fl.",
  "vicety - Fault; defect; coarseness. [Obs.] B. Jonson.",
  "glossographical - Of or pertaining to glossography.",
  "cranial - Of or pertaining to the cranium.",
  "cloddy - Consisting of clods; full of clods.",
  "nodulous - Having small nodes or knots; diminutively nodose.",
  "hexacapsular - Having six capsules or seed vessels.",
  "compassionable - Deserving compassion or pity; pitiable. [R.] Barrow.",
  "univalence - The quality or state of being univalent.",
  "stemmer - One who, or that which, stems (in any of the senses of the verbs).",
  "evanescently - ; imperceptibly. Chalmers.",
  "detractory - Defamatory by denial of desert; derogatory; calumnious. Sir T. Browne.",
  "regnal - Of or pertaining to the reign of a monarch; as, regnal years.",
  "mintman - One skilled in coining, or in coins; a coiner.",
  "distinctively - With distinction; plainly.",
  "aftermost - 1. Hindmost; -- opposed to foremost. 2. (Naut.)  Nearest the stern; most aft.",
  "bicallose - Having two callosities or hard spots. Gray.",
  "vulnerability - The quality or state of being vulnerable; vulnerableness.",
  "pegmatoid - Resembling pegmatite; pegmatic.",
  "herbose - Abounding with herbs. \"Fields poetically called herbose.\" Byrom.",
  "undermirth - Suppressed or concealed mirth. [Obs.] The Coronation.",
  "unflesh - To deprive of flesh; to reduce a skeleton. \"Unfleshed humanity.\" Wordsworth.",
  "scattergood - One who wastes; a spendthrift.",
  "embryological - Of or pertaining to embryology.",
  "incongealable - Not congealable; incapable of being congealed. -- In`con*geal\"a*ble*ness, n.",
  "antiphlogistian - An opposer of the theory of phlogiston.",
  "pentagonally - In the form of a pentagon; with five angles. Sir T. Browne.",
  "leze majesty - Any crime committed against the sovereign power.",
  "transcendentally - In a transcendental manner.",
  "rappage - The enlargement of a molt caused by rapping the pattern.",
  "capillaceous - Having long filaments; resembling a hair; slender. See Capillary.",
  "surquedous - Having or exhibiting surquedry; arrogant; insolent. [Obs.] Gower. James II. of Scot.",
  "antecursor - A forerunner; a precursor. [Obs.]",
  "ill-wisher - One who wishes ill to another; an enemy.",
  "trivialism - A trivial matter or method; a triviality. Carlyle.",
  "unburden - 1. To relieve from a burden. 2. To throw off, as a burden; to unload.",
  "uncomely - Not comely. -- adv. In an uncomely manner. 1 Cor. vii. 36.",
  "vitrifacture - The manufacture of glass and glassware.",
  "pacane - A species of hickory. See Pecan.",
  "profoundness - The quality or state of being profound; profundity; depth. Hooker.",
  "subdivine - Partaking of divinity; divine in a partial or lower degree. Bp. Hall.",
  "hery - To worship; to glorify; to praise. [Obs.] Chaucer. Spenser.",
  "gecarcinian - A land crab of the genus Gecarcinus, or of allied genera.",
  "aspartic - Pertaining to, or derived, asparagine; as, aspartic acid.",
  "reconciliatory - Serving or tending to reconcile. Bp. Hall.",
  "bracken - A brake or fern. Sir W. Scott.",
  "plunderage - The embezzlement of goods on shipboard. Wharton.",
  "dewrot - To rot, as flax or hemp, by exposure to rain, dew, and sun. See Dewretting.",
  "netty - Like a net, or network; netted. [R.]",
  "obfirm - To make firm; to harden in resolution. [Obs.] Bp. Hall. Sheldon.",
  "sancho - The nine of trumps in sancho pedro.",
  "heterogenetic - Relating to heterogenesis; as, heterogenetic transformations.",
  "nickeliferous - Containing nickel; as, nickelferous iron.",
  "intercommonage - The right or privilege of intercommoning.",
  "forlay - To lie in wait for; to ambush. An ambushed thief forlays a traveler. Dryden.",
  "rebucous - Rebuking. [Obs.] She gave unto him many rebucous words. Fabyan.",
  "earless - Without ears; hence, deaf or unwilling to hear. Pope.",
  "primity - Quality of being first; primitiveness. [Obs.] Bp. Pearson.",
  "silesian - Of or pertaining to Silesia. -- n.  A native or inhabitant of Silesia.",
  "syndesmography - A description of the ligaments; syndesmology.",
  "sternocostal - Of or pertaining to the sternum and the ribs; as, the sternocostal cartilages.",
  "thousandfold - Multiplied by a thousand.",
  "fruticant - Full of shoots. [Obs.] Evelyn.",
  "inanitiate - To produce inanition in; to exhaust for want of nourishment. [R.]",
  "anorthic - Having unequal oblique axes; as, anorthic crystals.",
  "coziness - The state or quality of being cozy.",
  "pluviograph - A self-registering rain gauge.",
  "circumjacent - Lying round; borderong on every side. T. Fuller.",
  "childbed - The state of a woman bringing forth a child, or being in labor; parturition.",
  "catfall - A rope used in hoisting the anchor to the cathead. Totten.",
  "crustily - In a crusty or surly manner; morosely.",
  "imperfectible - Incapable of being mad perfect. [R.]",
  "checkless - That can not be checked or restrained.",
  "underplant - To plant under; specif. (Forestry),  to plant (young trees) under an existing stand.",
  "despisement - A despising. [R.] Holland."
]