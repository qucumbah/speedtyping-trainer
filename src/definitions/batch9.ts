export default [
  "awlessness - The quality of being awless.",
  "websterite - A hydrous sulphate of alumina occurring in white reniform masses.",
  "fnese - To breathe heavily; to snort. [Obs.] Chaucer.",
  "subaduncate - Somewhat hooked or curved.",
  "weather-bitten - Eaten into, defaced, or worn, by exposure to the weather. Coleridge.",
  "clear-seeing - Having a clear physical or mental vision; having a clear understanding.",
  "rooty - Full of roots; as, rooty ground.",
  "myrrhic - Of, pertaining to, or obtained from, myrrh.",
  "obliqueness - Quality or state of being oblique.",
  "begrease - To soil or daub with grease or other oily matter.",
  "self-propagating - Propagating by one's self or by itself.",
  "apple-squire - A pimp; a kept gallant. [Obs.] Beau. & Fl.",
  "paramitome - The fluid portion of the protoplasm of a cell.",
  "disputableness - State of being disputable.",
  "autopsical - Pertaining to autopsy; autoptical. [Obs.]",
  "interdome - The open space between the inner and outer shells of a dome or cupola of masonry.",
  "squiffy - Somewhat intoxicated; tipsy. [Slang]  Kipling.",
  "cullyism - The state of being a cully. Less frequent instances of eminent cullyism. Spectator.",
  "stagecoachman - One who drives a stagecoach.",
  "nycthemeron - The natural day and night, or space of twenty-four hours.",
  "compendiousness - The state or quality of being compendious.",
  "mesodont - Having teeth of moderate size.",
  "calcinable - That may be calcined; as, a calcinable fossil.",
  "dryness - The state of being dry. See Dry.",
  "lungis - A lingerer; a dull, drowsy fellow. [Obs.]",
  "glore - To glare; to glower. [Obs.] Halliwell.",
  "asbestine - Of or pertaining to asbestus, or partaking of its nature; incombustible; asbestic.",
  "scholastically - In a scholastic manner.",
  "gutturine - Pertaining to the throat. [Obs.] \"Gutturine tumor.\" Ray.",
  "titubate - 1. To stumble. [Obs.] 2. To rock or roll, as a curved body on a plane.",
  "epulary - Of or pertaining to a feast or banquet. [Obs.] Smart.",
  "polynesians - The race of men native in Polynesia.",
  "dope-book - A chart of previous performances, etc., of race horses. [Race- track Slang]",
  "outdare - To surpass in daring; to overcome by courage; to brave. Shak. R. Browning.",
  "concupiscential - Relating to concupiscence. [Obs.] Johnson.",
  "self-consuming - Consuming one's self or itself.",
  "unrioted - Free from rioting. [Obs.] \"A chaste, unrioted house.\" May (Lucan).",
  "bemoil - To soil or encumber with mire and dirt. [Obs.] Shak.",
  "curvation - The act of bending or crooking.",
  "infamousness - The state or quality of being infamous; infamy.",
  "cementatory - Having the quality of cementating or uniting firmly.",
  "deplumate - Destitute or deprived of features; deplumed.",
  "nasicornous - Bearing a horn, or horns, on the nose, as the rhinoceros.",
  "electrograph - A mark, record, or tracing, made by the action of electricity.",
  "dal segno - A direction to go back to the sign Segno.",
  "yeldrine - The yellow-hammer; -- called also yeldrock, and yoldrin. [Prov. Eng.]",
  "outscouring - That which is scoured out o Buckland.",
  "tanagrine - Of or pertaining to the tanagers.",
  "gauziness - The quality of being gauzy; flimsiness. Ruskin.",
  "sky-high - , adv. & a. Very high. [Colloq.]",
  "preconize - To approve by preconization.",
  "preternaturality - Preternaturalness. [R.] Dr. John Smith.",
  "abodance - An omen; a portending. [Obs.]",
  "libidinosity - The state or quality of being libidinous; libidinousness. Skelton.",
  "subventaneous - Produced by the wind. [Obs.]",
  "organonymy - The designation or nomenclature of organs. B. G. Wilder.",
  "narrow-minded - Of narrow mental scope; illiberal; mean. -- Nar\"row-mind`ed*ness, n.",
  "robin goodfellow - A celebrated fairy; Puck. See Puck. Shak.",
  "hobbism - The philosophical system of Thomas Hobbes, an English materialist (",
  "moneral - Of or pertaining to the Monera.",
  "superdreadnought - See Dreadnought, above.",
  "deadhearted - Having a dull, faint heart; spiritless; listless. -- Dead\"*heart`ed*ness, n. Bp. Hall.",
  "undecagon - A figure having eleven angles and eleven sides.",
  "breastknot - A pin worn of the breast for a fastening, or for ornament; a brooch.",
  "spathal - Furnished with a spathe; as, spathal flowers. Howitt.",
  "godroon - An ornament produced by notching or carving a rounded molding.",
  "isatinic - Pertaining to, or derived from, isatin; as, isatic acid, which is also called trioxindol.",
  "painterly - Like a painter's work. [Obs.] \"A painterly glose of a visage.\" Sir P. Sidney.",
  "northmost - Lying farthest north; northernmost. Northmost part of the coast of Mozambique. De Foe.",
  "irrubrical - Contrary to the rubric; not rubrical.",
  "carnauba - The Brazilian wax palm. See Wax palm.",
  "lithiophilite - A phosphate of manganese and lithium; a variety of triphylite.",
  "coque - A small loop or bow of ribbon used in making hats, boas, etc.",
  "burgoo - A kind of oatmeal pudding, or thick gruel, used by seamen. [Written also burgout.]",
  "capping plane - A plane used for working the upper surface of staircase rails.",
  "interjacent - Lying or being between or among; intervening; as, interjacent isles. Sir W. Raleigh.",
  "centonism - The composition of a cento; the act or practice of composing a cento or centos.",
  "sambucus - A genus of shrubs and trees; the elder.",
  "hardship - That which is hard to hear, as toil, privation, injury, injustice, etc. Swift.",
  "dolcemente - Softly; sweetly; with soft, smooth, and delicate execution.",
  "anthropogeny - The science or study of human generation, or the origin and development of man.",
  "exulting - Rejoicing triumphantly or exceedingly; exultant. -- Ex*ult\"ing*ly, adv.",
  "exuperate - To excel; to surmount. [Obs.]",
  "trogonoid - Like or pertaining to the trogons.",
  "charry - Pertaining to charcoal, or partaking of its qualities.",
  "complimentative - Complimentary. [R.] Boswell.",
  "extine - The outer membrane of the grains of pollen of flowering plants.",
  "superethical - More than ethical; above ethics. Bolingbroke.",
  "glibly - In a glib manner; as, to speak glibly.",
  "attask - To take to task; to blame. Shak.",
  "interpellant - Interpelling; interrupting. -- n.  One who, or that which, interpels.",
  "spousess - A wife or bride. [Obs.] Fabyan.",
  "tapet - Worked or figured stuff; tapestry. [R.] Spenser.",
  "noctiferous - Bringing night. [Obs.] Johnson.",
  "addle-pate - A foolish or dull-witted fellow. [Colloq.]",
  "astrolater - A worshiper of the stars. Morley.",
  "desolatory - Causing desolation. [R.] Bp. Hall.",
  "epidemiology - That branch of science which treats of epidemics.",
  "morosaurus - An extinct genus of large herbivorous dinosaurs, found in Jurassic strata in America.",
  "brachyural - Of or pertaining to the Brachyura.",
  "tamperer - One who tampers; one who deals unfairly.",
  "brachylogy - Conciseness of expression; brevity.",
  "clart - To daub, smear, or spread, as with mud, etc. [Prov. Eng.] Halliwell.",
  "totemist - One belonging to a clan or tribe having a totem. -- To`tem*is\"tic, a.",
  "enseint - With child; pregnant. See Enceinte. [Obs.]",
  "curdy - Like curd; full of curd; coagulated. \"A curdy mass.\" Arbuthnot.",
  "coleopterous - Having wings covered with a case or sheath; belonging to the Coleoptera.",
  "mammalian - Of or pertaining to the Mammalia or mammals.",
  "aeronef - A power-driven, heavier-than-air flying machine.",
  "verisimility - Verisimilitude. [Obs.] The verisimility or probable truth. Sir T. Browne.",
  "emgalla - The South African wart hog. See Wart hog.",
  "andron - The apartment appropriated for the males. This was in the lower part of the house.",
  "minoration - A diminution. [R.] Sir T. Browne.",
  "beneficential - Relating to beneficence.",
  "verbalization - The act of verbalizing, or the state of being verbalized.",
  "concavation - The act of making concave.",
  "dungyard - A yard where dung is collected.",
  "kitchenmaid - A woman employed in the kitchen. Shak.",
  "amigo - A friend; -- a Spanish term applied in the Philippine Islands to friendly natives.",
  "absorbing - Swallowing, engrossing; as, an absorbing pursuit. -- Ab*sorb\"ing, adv.",
  "frustrable - Capable of beeing frustrated or defeated.",
  "inadequation - Want of exact correspondence. [Obs.] Puller.",
  "antislavery - Opposed to slavery. -- n.  Opposition to slavery.",
  "cerasinous - 1. Pertaining to, or containing, cerasin. 2. Of a cherry color.",
  "basal-nerved - Having the nerves radiating from the base; -- said of leaves.",
  "langate - A linen roller used in dressing wounds.",
  "conduciveness - The quality of conducing.",
  "clammily - In a clammy manner. \"Oozing so clammily.\" Hood.",
  "ballistite - A smokeless powder containing equal parts of soluble nitrocellulose and nitroglycerin.",
  "zebrinny - A cross between a male horse and a female zebra.",
  "paragram - A pun. Puns, which he calls paragrams. Addison.",
  "evangelian - Rendering thanks for favors.",
  "tulip-eared - Having erect, pointed ears; prick-eared; -- said of certain dogs.",
  "intersticed - Provided with interstices; having interstices between; situated at intervals.",
  "illabile - Incapable of falling or erring; infalliable. [Obs.] -- Il`la*bil\"i*ty, n. [Obs.]",
  "chartographer - Same as Cartographer, Cartographic, Cartography, etc.",
  "natatorious - Adapted for swimming; -- said of the legs of certain insects.",
  "peltry - Pelts or skins, collectively; skins with the fur on them; furs.",
  "instantaneity - Quality of being instantaneous. Shenstone.",
  "motionless - Without motion; being at rest.",
  "addititious - Additive. [R.] Sir J. Herschel.",
  "repulseless - Not capable of being repulsed.",
  "undershirt - A shirt worn next the skin, under another shirt; -- called also undervest.",
  "overliness - The quality or state of being overly; carelessness. [Obs.] Bp. Hall.",
  "rhino - Gold and silver, or money. [Cant] W. Wagstaffe. As long as the rhino lasted. Marryat.",
  "peach-colored - Of the color of a peach blossom. \"Peach-colored satin.\" Shak.",
  "enquicken - To quicken; to make alive. [Obs.] Dr. H. More.",
  "oxybutyric - Hydroxybutyric; designating any one of a group of metameric acids (C3H6.OH.CO2H).",
  "resell - To sell again; to sell what has been bought or sold; to retail.",
  "figgum - A juggler's trick; conjuring. [Obs.] The devil is the author of wicked figgum. B. Jonson.",
  "salagane - The esculent swallow. See under Esculent.",
  "amphiarthrodial - Characterized by amphiarthrosis.",
  "riparian - Of or pertaining to the bank of a river; as, riparian rights.",
  "unlash - To loose, as that which is lashed or tied down.",
  "spiculigenous - Producing or containing spicules.",
  "smithereens - Fragments; atoms; smithers. [Colloq.] W. Black.",
  "skirlcrake - The turnstone. [Prev. Eng.]",
  "furoin - A colorless, crystalline substance, C10H8O4, from furfurol.",
  "lamellirostral - Having a lamellate bill, as ducks and geese.",
  "xanthosis - The yellow discoloration often observed in cancerous tumors.",
  "israelitish - Of or pertaining to Israel, or to the Israelites; Jewish; Hebrew.",
  "plagiocephalic - Having an oblique lateral deformity of the skull.",
  "utterless - Incapable of being uttered. [Obs.] A clamoring debate of utterless things. Milton.",
  "mesoblastic - Relating to the mesoblast; as, the mesoblastic layer.",
  "sifflement - The act of whistling or hissing; a whistling sound; sibilation. [Obs.] A. Brewer.",
  "occrustate - To incrust; to harden. [Obs.] Dr. H. More.",
  "moate - To void the excrement, as a bird; to mute. [Obs.]",
  "subacetate - An acetate containing an excess of the basic constituent.",
  "distinguishedly - In a distinguished manner. [R.] Swift.",
  "fluter - 1. One who plays on the flute; a flutist or flautist. 2. One who makes grooves or flutings.",
  "unsadden - To relieve from sadness; to cheer. [R.] Whitlock.",
  "isabella grape - A favorite sweet American grape of a purple color. See Fox grape, under Fox.",
  "rhinocerical - Of or pertaining to the rhinoceros; resembling the rhinoceros, or his horn. Tatler.",
  "chubby - Like a chub; plump, short, and thick. \"Chubby faces.\" I. Taylor.",
  "incomprehension - Want of comprehension or understanding. \"These mazes and incomprehensions.\" Bacon.",
  "orthis - An extinct genus of Brachiopoda, abundant in the Paleozoic rocks.",
  "arthrodic - Of or pertaining to arthrodia.",
  "biblical - Pertaining to, or derived from, the Bible; as, biblical learning; biblical authority.",
  "draughty - Pertaining to a draught, or current of air; as, a draughtly, comfortless room.",
  "spargefaction - The act of sprinkling. [Obs.] Swift.",
  "tressy - Abounding in tresses. J. Baillie.",
  "clinometry - That art or operation of measuring the inclination of strata.",
  "scraggedness - Quality or state of being scragged.",
  "gailliarde - A lively French and Italian dance.",
  "neoplastic - of or pertaining to neoplasty, or neoplasia.",
  "arithmetically - Conformably to the principles or methods of arithmetic.",
  "glutinousness - The quality of being glutinous.",
  "conjunct - 1. United; conjoined; concurrent. [Archaic] 2. (Her.)  Same as Conjoined.",
  "gossiper - One given to gossip. Beaconsfield.",
  "tannin - Same as Tannic acid, under Tannic.",
  "incorporated - United in one body; formed into a corporation; made a legal entity.",
  "allusively - Figuratively [Obs.]; by way of allusion; by implication, suggestion, or insinuation.",
  "inheritor - One who inherits; an heir. Born inheritors of the dignity. Milton.",
  "medication - The act or process of medicating.",
  "univocation - Agreement of name and meaning. [Obs.] Whiston.",
  "selfness - Selfishness. [Obs.] Sir. P. Sidney.",
  "antiae - The two projecting feathered angles of the forehead of some birds; the frontal points.",
  "armiferous - Bearing arms or weapons. [R.]",
  "upbreathe - To breathe up or out; to exhale. [Obs.] Marston.",
  "pyretology - A discourse or treatise on fevers; the doctrine of fevers. Hooper.",
  "impallid - To make pallid; to blanch. [Obs.] Feltham.",
  "corneule - One of the corneas of a compound eye in the invertebrates. Carpenter.",
  "sea pool - A pool of salt water. Spenser.",
  "synchronization - The act of synchronizing; concurrence of events in respect to time.",
  "sensigenous - Causing or exciting sensation. Huxley.",
  "foreknowable - That may be foreknown. Dr. H. More.",
  "daguerreian - Pertaining to Daguerre, or to his invention of the daguerreotype.",
  "octopoda - (a) Same as Octocerata. (b) Same as Arachnida.",
  "stromboid - Of, pertaining to, or like, Strombus.",
  "sea robin - See under Robin, and Illustration in Appendix.",
  "repaint - To paint anew or again; as, to repaint a house; to repaint the ground of a picture.",
  "begnaw - To gnaw; to eat away; to corrode. The worm of conscience still begnaw thy soul. Shak.",
  "adelopod - An animal having feet that are not apparent.",
  "outlinear - Of or pertaining to an outline; being in, or forming, an outline. Trench.",
  "opolchenie - See Army organization, above.",
  "doctrinally - In a doctrinal manner or for; by way of teaching or positive direction.",
  "monogrammic - Of, pertaining to, or resembling, a monogram.",
  "aquarellist - A painter in thin transparent water colors.",
  "indistinguishably - In a indistinguishable manner. Sir W. Scott.",
  "diurnalist - A journalist. [Obs.] Bp. Hall.",
  "seepy - Oozy; -- applied to land under cultivation that is not well drained.",
  "principate - Principality; supreme rule. [Obs.] Barrow.",
  "preconizate - To proclaim; to publish; also, to summon; to call. [Obs.] Bp. Burnet.",
  "quinizine - any one of a series of nitrogenous bases, certain of which are used as antipyretics.",
  "shecklaton - A kind of gilt leather. See Checklaton. [Obs.] Spenser.",
  "cataphonics - That branch of acoustics which treats of reflested sounds; catacoustics.",
  "rageful - Full of rage; expressing rage. [Obs.] \"Rageful eyes.\" Sir P. Sidney.",
  "chierte - Love; tender regard. [Obs.] Chaucer.",
  "southness - A tendency in the end of a magnetic needle to point toward the south pole. Faraday.",
  "corradiate - To converge to one point or focus, as light or rays.",
  "calumniatory - Containing calumny; slanderous. Montagu.",
  "centerfire cartridge - See under Cartridge.",
  "snottery - Filth; abomination. [Obs.] To purge the snottery of our slimy time. Marston.",
  "regional - Of or pertaining to a particular region; sectional.",
  "heartbroken - Overcome by crushing sorrow; deeply grieved.",
  "resurgence - The act of rising again; resurrection.",
  "ecderon - See Ecteron. -- Ec`der*on\"ic, a.",
  "oneirocritics - The art of interpreting dreams.",
  "reafforest - To convert again into the forest, as a region of country.",
  "azotous - : Nitrous; as, azotous acid. [R.]",
  "croconate - A salt formed by the union of croconic acid with a base.",
  "erration - A wandering; a roving about. [Obs.] Cockeram.",
  "liage - Union by league; alliance. [Obs.]",
  "newsman - 1. One who brings news. [Obs.] Spenser. 2. A man who distributes or sells newspapers.",
  "subprior - The vicegerent of a prior; a claustral officer who assists the prior.",
  "uberty - Fruitfulness; copiousness; abundance; plenty. [Obs.] Florio.",
  "grassplot - A plot or space covered with grass; a lawn. \"Here on this grassplot.\" Shak.",
  "treeless - Destitute of trees. C. Kingsley.",
  "posthumously - It a posthumous manner; after one's decease.",
  "decapodal - Belonging to the decapods; having ten feet; ten-footed.",
  "colature - The process of straining; the matter strained; a strainer. [R.]",
  "omnivagant - Wandering anywhere and everywhere. [R.]",
  "phantasmagoric - Of or pertaining to phantasmagoria; phantasmagorial. Hawthorne.",
  "bibcock - A cock or faucet having a bent down nozzle. Knight.",
  "herpetism - See Dartrous diathesis, under Dartrous.",
  "io - An exclamation of joy or triumph; -- often interjectional.",
  "concupiscent - Having sexual lust; libidinous; lustful; lecherous; salacious. Johnson.",
  "bricklayer - One whose pccupation is to build with bricks. Bricklayer's itch. See under Itch.",
  "plangency - The quality or state of being plangent; a beating sound. [R.]",
  "disfurnishment - The act of disfurnishing, or the state of being disfurnished. Daniel.",
  "aquarial - Of or pertaining to an aquarium.",
  "leucophlegmatic - Having a dropsical habit of body, with a white bloated skin.",
  "symbological - Pertaining to a symbology; versed in, or characterized by, symbology.",
  "payer - One who pays; specifically, the person by whom a bill or note has been, or should be, paid.",
  "supra - Over; above; before; also, beyond; besides; -- much used as a prefix.",
  "anchoretic - Pertaining to an anchoret or hermit; after the manner of an anchoret.",
  "marchman - A person living in the marches between England and Scotland or Wales.",
  "pelleted - Made of, or like, pellets; furnished with pellets. [R.] \"This pelleted storm.\" Shak.",
  "rattleweed - Any plant of the genus Astragalus. See Milk vetch.",
  "parallelly - In a parallel manner; with parallelism. [R.] Dr. H. More.",
  "basisolute - Prolonged at the base, as certain leaves.",
  "incoincidence - The quality of being incoincident; want of coincidence. [R.]",
  "banteng - The wild ox of Java (Bibos Banteng).",
  "cephalometer - An instrument measuring the dimensions of the head of a fetus during delivery.",
  "choliamb - A verse having an iambus in the fifth place, and a spondee in the sixth or last.",
  "clerkship - State, quality, or business of a clerk.",
  "panification - The act or process of making bread. Ure.",
  "predeterminable - Capable of being determined beforehand. Coleridge.",
  "piercel - A kind of gimlet for making vents in casks; -- called also piercer.",
  "karagane - A species of gray fox found in Russia.",
  "abderian - Given to laughter; inclined to foolish or incessant merriment.",
  "chrestomathic - Teaching what is useful. \"A chrestomathic school.\" Southey.",
  "crossbred - Produced by mixing distinct breeds; mongrel.",
  "jovialness - Noisy mirth; joviality. Hewyt.",
  "parkleaves - A European species of Saint John's-wort; the tutsan. See Tutsan.",
  "squamduck - The American eider duck. [Local, U.S.]",
  "turgidity - The quality or state of being turgid.",
  "conculcate - To tread or trample under foot. [Obs.] Bp. Montagu -- Con`cul*ca\"tion (, n. [Obs.]",
  "lobelin - A yellowish green resin from Lobelia, used as an emetic and diaphoretic.",
  "snobling - A little snob. [Jocose] Thackeray.",
  "photic region - The uppermost zone of the sea, which receives the most light.",
  "extraversion - The act of throwing out; the state of being turned or thrown out. [Obs.] Boyle.",
  "speller - 1. One who spells. 2. A spelling book. [U. S.]",
  "properness - 1. The quality of being proper. 2. Tallness; comeliness. [Obs.] Udall.",
  "preresolve - To resolve beforehand; to predetermine. Sir E. Dering.",
  "diegesis - A narrative or history; a recital or relation.",
  "chalcedonic - Of or pertaining to chalcedony.",
  "speedful - Full of speed (in any sense). [Obs.]",
  "dustbrush - A brush of feathers, bristles, or hair, for removing dust from furniture.",
  "schoolbook - A book used in schools for learning lessons.",
  "endermically - By the endermic method; as, applied endermically.",
  "manumotor - A small wheel carriage, so constructed that a person sitting in it may move it.",
  "rhodanate - A salt of rhodanic acid; a sulphocyanate. [Obsoles.]",
  "fordone - Undone; ruined. [Obs.] Spenser.",
  "inergetical - Having no energy; sluggish. [R.] Boyle.",
  "imploring - That implores; beseeching; entreating. -- Im*plor\"ing*ly, adv.",
  "zoographic - Of or pertaining to the description of animals.",
  "welaway - Alas! [Obs.] Then welaway, for she undone was clean. Wyatt.",
  "preconquer - To conquer in anticipation. [R.] Fuller.",
  "areometry - The art or process of measuring the specific gravity of fluids.",
  "nonylene - Any one of a series of metameric, unsaturated hydrocarbons C9H18 of the ethylene series.",
  "white-limed - Whitewashed or plastered with lime. \"White-limed walls.\" Shak.",
  "roughwrought - Wrought in a rough, unfinished way; worked over coarsely.",
  "renewedness - The state of being renewed.",
  "peripteral - Having columns on all sides; -- said of an edifice. See Apteral.",
  "mistide - To happen or come to pass unfortunately; also, to suffer evil fortune. [Obs.]",
  "antemosaic - Being before the time of Moses.",
  "rewardless - Having, or affording, no reward.",
  "dramseller - One who sells distilled liquors by the dram or glass.",
  "ginglymoidal - Pertaining to, or resembling, a ginglymus, or hinge joint; ginglyform.",
  "terebrant - Boring, or adapted for boring; -- said of certain Hymenoptera, as the sawflies.",
  "transregionate - Foreign. [Obs.] Holinshed.",
  "flowerage - State of flowers; flowers, collectively or in general. Tennyson.",
  "collembola - The division of Thysanura which includes Podura, and allied forms.",
  "unconfounded - Not confounded. Bp. Warburton.",
  "fluorated - Combined with fluorine; subjected to the action of fluoride. [R.]",
  "pushpin - A child's game played with pins. L. Estrange.",
  "kilnhole - The mouth or opening of an oven or kiln. Shak.",
  "casualist - One who believes in casualism.",
  "dispondee - A double spondee; a foot consisting of four long syllables.",
  "exaltate - Exercising its highest influence; -- said of a planet. [Obs.] Chaucer.",
  "autochthonal - Aboriginal; indigenous; native.",
  "tombstone - A stone erected over a grave, to preserve the memory of the deceased.",
  "forky - Opening into two or more parts or shoots; forked; furcated. \"Forky tongues.\" Pope.",
  "spouseless - Destitute of a spouse; unmarried.",
  "tinternell - A certain old dance. [Obs.] Halliwell.",
  "interdependence - Mutual dependence. \"The interdependence of virtue and knowledge.\" M. Arnold.",
  "deisticalness - State of being deistical.",
  "radicular - Of or performance to roots, or the root of a plant.",
  "triangularly - In a triangular manner; in the form of a triangle. Dampier.",
  "palaverer - One who palavers; a flatterer.",
  "shoon - pl. of Shoe. [Archaic] Chaucer. They shook the snow from hats and shoon. Emerson.",
  "thrashel - An instrument to thrash with; a flail. [Obs. or Prov. Eng.] Halliwell.",
  "anarchism - The doctrine or practice of anarchists.",
  "countercast - A trick; a delusive contrivance. [Obs.] Spenser.",
  "willy nilly - See Will I, nill I, etc., under 3d Will.",
  "pewterer - One whose occupation is to make utensils of pewter; a pewtersmith. Shak.",
  "unwitting - Not knowing; unconscious; ignorant. -- Un*wit\"ting*ly, adv.",
  "hang-by - A dependent; a hanger-on; -- so called in contempt. B. Jonson.",
  "originality - The quality or state of being original. Macaulay.",
  "tonus - Tonicity, or tone; as, muscular tonus.",
  "scorpioidal - 1. Having the inflorescence curved or circinate at the end, like a scorpion's tail.",
  "mannitol - The technical name of mannite. See Mannite.",
  "amphigonic - Pertaining to amphigony; sexual; as, amphigonic propagation. [R.]",
  "far-about - A going out of the way; a digression. [Obs.] Fuller.",
  "antiquist - An antiquary; a collector of antiques. [R.] Pinkerton.",
  "strenuity - Strenuousness; activity. [Obs.] Chapman.",
  "electrization - The act of electrizing; electrification.",
  "polygastrian - One of the Polygastrica. [Obs.]",
  "hematology - The science which treats of the blood.",
  "bemean - To make mean; to lower. C. Reade.",
  "querpo - The inner or body garments taken together. See Cuerpo. Dryden.",
  "gastropodous - Of or pertaining to the Gastropoda.",
  "stelliform - Like a star; star-shaped; radiated.",
  "natation - The act of floating on the water; swimming. Sir T. Browne.",
  "befuddle - To becloud and confuse, as with liquor.",
  "half-clammed - Half-filled. [Obs.] Lions' half-clammed entrails roar food. Marston.",
  "conductible - Capable of being conducted.",
  "boatful - The quantity or amount that fills a boat.",
  "infectiously - In an infectious manner. Shak.",
  "amuletic - Of or pertaining to an amulet; operating as a charm.",
  "impoison - To poison; to imbitter; to impair.",
  "columbatz fly - See Buffalo fly, under Buffalo.",
  "misthought - Erroneous thought; mistaken opinion; error. [Obs.] Spenser.",
  "plenipotency - The quality or state of being plenipotent. [R.]",
  "divorcer - The person or cause that produces or effects a divorce. Drummond.",
  "pigeon-hearted - Timid; easily frightened; chicken-hearted. Beau. & Fl.",
  "comicality - The quality of being comical; something comical.",
  "perdurability - Durability; lastingness. [Archaic] Chaucer.",
  "potently - With great force or energy; powerfully; efficaciously. \"You are potently opposed.\" Shak.",
  "vantbrass - Armor for the arm; vambrace. Milton.",
  "depolish - To remove the polish or glaze from.",
  "nonetto - A composition for nine instruments, rarely for nine voices.",
  "epulation - A feasting or feast; banquet. [Obs.] Sir T. Browne.",
  "perfumatory - Emitting perfume; perfuming. [R.] Sir E. Leigh.",
  "throdden - To grow; to thrive. [Prov. Eng.] Grose.",
  "sifilet - The six-shafted bird of paradise. See Paradise bird, under Paradise.",
  "equalizer - One who, or that which, equalizes anything.",
  "green-broom - A plant of the genus Genista (G. tinctoria); dyer's weed; -- called also greenweed.",
  "hamate - Hooked; bent at the end into a hook; hamous.",
  "scelet - A mummy; a skeleton. [Obs.] olland.",
  "teyne - A thin plate of metal. [Obs.] \"A teyne of silver.\" Chaucer.",
  "botanist - One skilled in botany; one versed in the knowledge of plants.",
  "repetitious - Repeating; containing repetition. [U.S.] Dr. T. Dwight.",
  "hooven - Affected with hoove; as, hooven, or hoven, cattle.",
  "unmasculate - To emasculate. [Obs.] Fuller.",
  "delineable - Capable of being, or liable to be, delineated. Feltham.",
  "cornicle - A little horn. [Obs.] Sir T. Browne.",
  "crustific - Producing or forming a crust or skin. [R.]",
  "recoiler - One who, or that which, recoils.",
  "unsoldiered - Not equipped like a soldier; unsoldierlike. [Obs.] J. Fletcher.",
  "vivisectional - Of or pertaining to vivisection.",
  "baked-meat - A pie; baked food. [Obs.] Gen. xl. 17. Shak.",
  "socman - One who holds lands or tenements by socage; a socager. Cowell.",
  "immerited - Unmerited. [Obs.] Charles I.",
  "diswarn - To dissuade from by previous warning. [Obs.]",
  "sailmaker - One whose occupation is to make or repair sails. -- Sail\"mak`ing, n.",
  "smickering - Amorous glance or inclination. [Obs.] \"A smickering to our young lady.\" Dryden.",
  "squaloid - Like or pertaining to a shark or sharks.",
  "garter stitch - The simplest stitch in knitting.",
  "cetewale - Same as Zedoary. [Obs.] Chaucer.",
  "deletory - That which blots out. [Obs.] \"A deletory of sin.\" Jer. Taylor.",
  "anthropotomical - Pertaining to anthropotomy, or the dissection of human bodies.",
  "diuturnal - Of long continuance; lasting. [R.] Milton.",
  "statehood - The condition of being a State; as, a territory seeking Statehood.",
  "admirative - Relating to or expressing admiration or wonder. [R.] Earle.",
  "fitly - In a fit manner; suitably; properly; conveniently; as, a maxim fitly applied.",
  "conjunctival - 1. Joining; connecting. 2. (Anat.)  Of or pertaining to the conjunctiva.",
  "exalbuminous - Having no albumen about the embryo; -- said of certain seeds.",
  "demiculverin - A kind of ordnance, carrying a ball weighing from nine to thirteen pounds.",
  "metachronism - An error committed in chronology by placing an event after its real time.",
  "trawlboat - A boat used in fishing with trawls or trawlnets.",
  "actinosome - The entire body of a coelenterate.",
  "gastrophrenic - Pertaining to the stomach and diaphragm; as, the gastrophrenic ligament.",
  "differentiator - One who, or that which, differentiates.",
  "indisturbance - Freedom from disturbance; calmness; repose; apathy; indifference.",
  "monocule - A small crustacean with one median eye.",
  "unportunate - Importunate; troublesome with requests. [Obs.] Golden Boke.",
  "chouicha - The salmon of the Columbia River or California. See Quinnat.",
  "nonpreparation - Neglect or failure to prepare; want of preparation.",
  "salvatory - A place where things are preserved; a repository. [R.] Sir M. Hale.",
  "rusticated - resembling rustic work. See Rustic work (a), under Rustic.",
  "attractor - One who, or that which, attracts. Sir T. Browne",
  "behove - , and derivatives. See Behoove, & c.",
  "ungular - Of or pertaining to a hoof, claw, or talon; ungual.",
  "tender-hefted - Having great tenderness; easily moved. [Obs.] Shak.",
  "phaseless - Without a phase, or visible form. [R.] \"A phaseless and increasing gloom.\" Poe.",
  "swingtree - The bar of a carriage to which the traces are fastened; the whiffletree.",
  "bandala - A fabric made in Manilla from the older leaf sheaths of the abaca (Musa textilis).",
  "pellile - The redshank; -- so called from its note. [Prov. Eng.]",
  "adnation - The adhesion or cohesion of different floral verticils or sets of organs.",
  "soko - An African anthropoid ape, supposed to be a variety of the chimpanzee.",
  "annihilable - Capable of being annihilated.",
  "syndactyl - Having two or more digits wholly or partly united. See Syndactylism.",
  "sprawls - Small branches of a tree; twigs; sprays. [Prov. Eng.] Halliwell.",
  "orthodromy - The act or art of sailing on a great circle.",
  "daguerreotypist - One who takes daguerreotypes.",
  "tyrociny - The state of being a tyro, or beginner; apprenticeship. [Obs.] Blount.",
  "aphrodisiacal - Exciting venereal desire; provocative to venery.",
  "varicosis - The formation of varices; varicosity.",
  "outtaken - or prep. Excepted; save. [Obs.] Wyclif. Chaucer.",
  "omissive - Leaving out; omitting. Bp. Hall. -- O*mis\"sive*ly, adv.",
  "expetible - Worthy of being wished for; desirable. [Obs.] Puller.",
  "hippophagy - The act or practice of feeding on horseflesh.",
  "refutability - The quality of being refutable.",
  "decigram - A weight in the metric system; one tenth of a gram, equal to 1.5432 grains avoirdupois.",
  "featherbone - A substitute for whalebone, made from the quills of geese and turkeys.",
  "gallotannic - Pertaining to the tannin or nutgalls. Gallotannic acid. See Tannic acid, under Tannic.",
  "pyrotechnist - One skilled in pyrotechny; one who manufactures fireworks. Steevens.",
  "mericarp - One carpel of an umbelliferous fruit. See Cremocarp.",
  "sculpturesque - After the manner of sculpture; resembling, or relating to, sculpture.",
  "pleomorphic - Pertaining to pleomorphism; as, the pleomorphic character of bacteria.",
  "perennity - The quality of being perennial. [R.] Derham.",
  "rebaptisation - A second baptism. [Obs.] Hooker.",
  "ebionitism - The system or doctrine of the Ebionites.",
  "emiction - 1. The voiding of urine. 2. What is voided by the urinary passages; urine.",
  "imbosture - Embossed or raised work. [Obs.] Beau. & Fl.",
  "lactation - A giving suck; the secretion and yielding of milk by the mammary gland.",
  "gymnocytode - A cytode without either a cell wall or a nucleus. Haeckel.",
  "cavitary - Containing a body cavity; as, the cavitary or nematoid worms.",
  "germiparity - Reproduction by means of germs.",
  "amygdalic - Of or pertaining to almonds; derived from amygdalin; as, amygdalic acid.",
  "monothalama - A division of Foraminifera including those that have only one chamber.",
  "inevitableness - The state of being unavoidable; certainty to happen. Prideaux.",
  "multidigitate - Having many fingers, or fingerlike processes.",
  "sea slater - Any isopod crustacean of the genus Ligia.",
  "tidder - To use with tenderness; to fondle. [Obs. or Prov. Eng.]",
  "circumventive - Tending to circumvent; deceiving by artifices; deluding.",
  "stag-evil - A kind of palsy affecting the jaw of a horse. Crabb.",
  "simpless - Simplicity; silliness. [Obs.] Spenser.",
  "toxophilite - A lover of archery; one devoted to archery.",
  "provand - Provender or food. [Obs.] One pease was a soldier's provant a whole day. Beau. & Fl.",
  "manitrunk - The anterior segment of the thorax in insects. See Insect.",
  "lusory - Used in play; sportive; playful. [Obs.] Bp. Sanderson.",
  "strip-leaf - Tobacco which has been stripped of its stalks before packing.",
  "unacquaintedness - Unacquaintance. Whiston.",
  "hebraist - One versed in the Hebrew language and learning.",
  "apyrexy - The absence or intermission of fever.",
  "baffy - A short wooden club having a deeply concave face, seldom used.",
  "niobium - A later name of columbium. See Columbium.",
  "septifarious - Turned in seven different ways.",
  "disintegrator - A machine for grinding or pulverizing by percussion.",
  "gadolinic - Pertaining to or containing gadolinium.",
  "lithogenesy - The doctrine or science of the origin of the minerals composing the globe.",
  "reinless - Not having, or not governed by, reins; hence, not checked or restrained.",
  "get-penny - Something which gets or gains money; a successful affair. [Colloq.] Chapman.",
  "tenuious - Rare or subtile; tenuous; -- opposed to dense. [Obs.] Glanvill.",
  "sciurine - Of or pertaining to the Squirrel family. -- n.  A rodent of the Squirrel family.",
  "turgescent - Becoming turgid or inflated; swelling; growing big.",
  "arachnidial - (a) Of or pertaining to the Arachnida. (b) Pertaining to the arachnidium.",
  "vitiousness - See Vicious, Viciously, Viciousness.",
  "crastination - Procrastination; a putting off till to-morrow. [Obs.]",
  "bayed - Having a bay or bays. \"The large bayed barn.\" Drayton.",
  "rewardable - Worthy of reward. -- Re*ward\"a*ble*ness, n. -- Re*ward\"a*bly, adv.",
  "piscicapture - Capture of fishes, as by angling. [R.] W. H. Russell.",
  "toughness - The quality or state of being tough.",
  "gemote - A meeting; -- used in combination, as, Witenagemote, an assembly of the wise men.",
  "whiggery - The principles or practices of the Whigs; Whiggism.",
  "heightener - One who, or that which, heightens.",
  "benzonaphtol - A white crystalline powder used as an intestinal antiseptic; beta-naphthol benzoate.",
  "formulization - The act or process of reducing to a formula; the state of being formulized.",
  "embrown - To give a brown color to; to imbrown. Summer suns embrown the laboring swain. Fenton.",
  "irreligion - The state of being irreligious; want of religion; impiety.",
  "fastish - Rather fast; also, somewhat dissipated. [Colloq.] Thackeray.",
  "proprietorial - Of or pertaining to ownership; proprietary; as, proprietorial rights.",
  "worshipable - Capable of being worshiped; worthy of worship. [R.] Carlyle.",
  "enterparlance - Mutual talk or conversation; conference. [Obs.] Sir J. Hayward.",
  "culasse - The lower faceted portion of a brilliant-cut diamond.",
  "doughiness - The quality or state of being doughy.",
  "untent - To bring out of a tent. [R.] Shak.",
  "conferrable - Capable of being conferred.",
  "anachronistic - Erroneous in date; containing an anachronism. T. Warton.",
  "accreditation - The act of accrediting; as, letters of accreditation.",
  "endrudge - To make a drudge or slave of. [Obs.] Bp. Hall.",
  "retrocopulant - Copulating backward, or from behind.",
  "astructive - Building up; constructive; -- opposed to destructive. [Obs.]",
  "proteolysis - The digestion or dissolving of proteid matter by proteolytic ferments.",
  "staynil - The European starling. [Prov. Eng.]",
  "conciliabule - An obscure ecclesiastical council; a conciliable. Milman.",
  "bunch-backed - Having a bunch on the back; crooked. \"Bunch-backed toad.\" Shak.",
  "pasturer - One who pastures; one who takes cattle to graze. See Agister.",
  "fortunately - In a fortunate manner; luckily; successfully; happily.",
  "outrank - To exceed in rank; hence, to take precedence of.",
  "smilet - A little smile. [R.] Those happy smilets That played on her ripe lip. Shak.",
  "coastwise - By way of, or along, the coast.",
  "sevennight - A week; any period of seven consecutive days and nights. See Sennight.",
  "therebiforn - Before that time; beforehand. [Obs.] Many a winter therebiforn. Chaucer.",
  "indiscussed - Not discussed. [Obs.] Donne.",
  "pseudoneuropterous - Of or pertaining to the Pseudoneuroptera.",
  "yellowtop - A kind of grass, perhaps a species of Agrostis.",
  "essenism - The doctrine or the practices of the Essenes. De Quincey.",
  "cruive - A kind of weir or dam for trapping salmon; also, a hovel. [Scot.]",
  "sociologic - Of or pertaining to sociology, or social science. -- So`ci*o*log\"ic*al*ly, adv.",
  "larrup - To beat or flog soundly. [Prov. Eng. & Colloq. U.S.] Forby.",
  "augurship - The office, or period of office, of an augur. Bacon.",
  "sportless - Without sport or mirth; joyless.",
  "overbuy - 1. To buy too much. 2. To buy at too dear a rate. Dryden.",
  "cephalotribe - An obstetrical instrument for performing cephalotripsy.",
  "bysmottered - Bespotted with mud or dirt. [Obs.] Chaucer.",
  "admove - To move or conduct to or toward. [Obs.] Sir T. Browne.",
  "phototypy - The art or process of producing phototypes.",
  "divertise - To divert; to entertain. [Obs.] Dryden.",
  "preglacial - Prior to the glacial or drift period.",
  "thermomotor - A heat engine; a hot-air engine.",
  "grooving - The act of forming a groove or grooves; a groove, or collection of grooves.",
  "mutteringly - With a low voice and indistinct articulation; in a muttering manner.",
  "engyn - Variant of Engine. [Obs.] Chaucer.",
  "seposit - To set aside; to give up. [Obs.]",
  "monochrome - A painting or drawing in a single color; a picture made with a single color.",
  "retreatful - Furnishing or serving as a retreat. [R.] \"Our retreatful flood.\" Chapman.",
  "pipestone - A kind of clay slate, carved by the Indians into tobacco pipes. Cf. Catlinite.",
  "veiled - Covered by, or as by, a veil; hidden. \"Words used to convey a veiled meaning.\" Earle.",
  "gassy - Full of gas; like gas. Hence: [Colloq.] Inflated; full of boastful or insincere talk.",
  "minatorily - In a minatory manner; with threats.",
  "right-hearted - Having a right heart or disposition. -- Right\"-heart`ed*ness, n.",
  "flossy - Pertaining to, made of, or resembling, floss; hence, light; downy.",
  "mistakable - Liable to be mistaken; capable of being misconceived. Sir T. Browne.",
  "argutation - Caviling; subtle disputation. [Obs.]",
  "dephlegm - To rid of phlegm or water; to dephlegmate. [Obs.] Boyle.",
  "enter- - A prefix signifying between, among, part.",
  "abscind - To cut off. [R.] \"Two syllables . . . abscinded from the rest.\" Johnson.",
  "fluxible - Capable of being melted or fused, as a mineral. Holland. -- Flux\"i*ble*ness, n.",
  "sea bow - See Marine rainbow, under Rainbow.",
  "horseplay - Rude, boisterous play. Too much given to horseplay in his raillery. Dryden.",
  "anachronical - Characterized by, or involving, anachronism; anachronistic.",
  "rammish - Like a ram; hence, rank; lascivious. \"Their savor is so rammish.\" Chaucer.",
  "lairdship - The state of being a laird; an estate; landed property. [Scot.] Ramsay.",
  "untile - To take the tiles from; to uncover by removing the tiles.",
  "incessable - Unceasing; continual. [Obs.] Shelton. -- In*ces\"sa*bly, adv. [Obs.]",
  "filamentary - Having the character of, or formed by, a filament.",
  "demonstrance - Demonstration; proof. [Obs.] Holland.",
  "cuckoldly - Having the qualities of a cuckold; mean-spirited; sneaking. Shak.",
  "cenobitism - The state of being a cenobite; the belief or practice of a cenobite. Milman.",
  "meteoroidal - Of or pertaining to a meteoroid or to meteoroids.",
  "nonresemblance - Want of resemblance; unlikeness; dissimilarity.",
  "whimsically - In a whimsical manner; freakishly.",
  "woodcut - An engraving on wood; also, a print from it. Same as Wood cut, under Wood.",
  "basify - To convert into a salifiable base.",
  "cenobitical - Of or pertaining to a cenobite.",
  "diversiloquent - Speaking in different ways. [R.]",
  "mulierty - Condition of being a mulier; position of one born in lawful wedlock.",
  "expilator - One who pillages; a plunderer; a pillager. [Obs.] Sir T. Browne.",
  "abatable - Capable of being abated; as, an abatable writ or nuisance.",
  "bureaucratical - Of, relating to, or resembling, a bureaucracy.",
  "ginglymoid - Pertaining to, or resembling, a ginglymus, or hinge joint; ginglyform.",
  "prosaism - That which is in the form of prose writing; a prosaic manner. Coleridge.",
  "emicant - Beaming forth; flashing. [R.] Which emicant did this and that way dart. Blackmore.",
  "transelementate - To change or transpose the elements of; to transubstantiate. [Obs.] Jer. Taylor.",
  "correctable - Capable of being corrected.",
  "snakeweed - (a) A kind of knotweed (Polygonum Bistorta). (b) The Virginia snakeroot. See Snakeroot.",
  "overcloy - To fill beyond satiety. Shak.",
  "pappoose - Same as Papoose. Pappoose root. (Bot.) See Cohosh.",
  "grapeless - Wanting grapes or the flavor of grapes.",
  "subsignation - The act of writing the name under something, as for attestation. [R.] Shelton.",
  "johannean - Of or pertaining to John, esp. to the Apostle John or his writings. M. Stuart.",
  "legerity - Lightness; nimbleness [Archaic] Shak.",
  "glebous - Pertaining to the glebe; turfy; cloddy; fertile; fruitful. \"Gleby land.\" Prior.",
  "webster - A weaver; originally, a female weaver. [Obs.] Brathwait.",
  "enfierce - To make fierce. [Obs.] Spenser.",
  "tectrices - The wing coverts of a bird. See Covert, and Illust. of Bird.",
  "spectroscopy - The use of the spectroscope; investigations made with the spectroscope.",
  "dunnock - The hedge sparrow or hedge accentor. [Local, Eng.]",
  "inviter - One who, or that which, invites.",
  "angerly - Angrily. [Obs. or Poetic] Why, how now, Hecate! you look angerly. Shak.",
  "cognatus - A person cinnected through cognation.",
  "defiatory - Bidding or manifesting defiance. [Obs.] Shelford.",
  "deliquiate - To melt and become liquid by absorbing water from the air; to deliquesce. Fourcroy.",
  "perchance - By chance; perhaps; peradventure.",
  "homogony - The condition of having homogonous flowers.",
  "philatory - A kind of transparent reliquary with an ornamental top.",
  "hydrometeorological - Of or pertaining to hydrometeorology, or to rain, clouds, storms, etc.",
  "malacissant - Softening; relaxing. [Obs.]",
  "parvitude - Littleness. [Obs.] Glanvill. Ray.",
  "conjuror - One bound by a common cath with others. [Obs.]",
  "cognitive - Knowing, or apprehending by the understanding; as, cognitive power. South.",
  "flotery - Wavy; flowing. [Obs.] With flotery beard. Chaucer.",
  "torturous - Involving, or pertaining to, torture. [R.] \"The torturous crucifixion.\" I. Disraeli.",
  "unanswerability - The quality of being unanswerable; unanswerableness.",
  "cometographer - One who describes or writes about comets.",
  "millinet - A stiff cotton fabric used by milliners for lining bonnets.",
  "bletting - A form of decay seen in fleshy, overripe fruit. Lindley.",
  "claque - A collection of persons employed to applaud at a theatrical exhibition.",
  "endophyllous - Wrapped up within a leaf or sheath.",
  "corresponsive - Corresponding; conformable; adapted. Shak. -- Cor`re*spon\"sive*ly, adv.",
  "addeem - To award; to adjudge. [Obs.] \"Unto him they did addeem the prise.\" Spenser.",
  "knaveship - A small due, in meal, established by usage, which is paid to the under miller. [Scot.]",
  "entosthoblast - The granule within the nucleolus or entoblast of a nucleated cell. Agassiz.",
  "finicality - The quality of being finical; finicalness.",
  "vitric - Having the nature and qualities of glass; glasslike; -- distinguished from ceramic.",
  "leod - People; a nation; a man. [Obs.] Piers Plowman. Bp. Gibson.",
  "dishaunt - To leave; to quit; to cease to haunt. Halliwell.",
  "reacher - 1. One who reaches. 2. An exaggeration. [Obs.] Fuller.",
  "wind-up - Act of winding up, or closing; a concluding act or part; the end.",
  "mudwort - A small herbaceous plant growing on muddy shores (Limosella aquatica).",
  "octoyl - A hypothetical radical (C8H15O), regarded as the essential residue of octoic acid.",
  "stringency - The quality or state of being stringent.",
  "bepinch - To pinch, or mark with pinches. Chapman.",
  "porime - A theorem or proposition so easy of demonstration as to be almost self-evident. [R.] Crabb.",
  "dehusk - To remove the husk from. [Obs.] \"Wheat dehusked upon the floor.\" Drant.",
  "dermatopathic - Of or pertaining to skin diseases, or their cure.",
  "alogy - Unreasonableness; absurdity. [Obs.]",
  "stitchery - Needlework; -- in comtempt. Shak.",
  "melanure - A small fish of the Mediterranean; a gilthead. See Gilthead (a).",
  "sarculation - A weeding, as with a hoe or a rake.",
  "feroher - A symbol of the solar deity, found on monuments exhumed in Babylon, Nineveh, etc.",
  "encowl - To make a monk (or wearer of a cowl) of. [R.] Drayton.",
  "brinish - Like brine; somewhat salt; saltish. \"Brinish tears.\" Shak.",
  "enthymematic - Pertaining to, or of the form of, an enthymeme.",
  "feveret - A slight fever. [Obs.] Ayliffe.",
  "pulverate - To beat or reduce to powder or dust; to pulverize. [R.]",
  "alertness - The quality of being alert or on the alert; briskness; nimbleness; activity.",
  "monopolylogue - An exhibition in which an actor sustains many characters.",
  "commissionship - The office of commissioner. Sir W. Scott.",
  "subact - To reduce; to subdue. [Obs.] Bacon.",
  "inorganity - Quality of being inorganic. [Obs.] \"The inorganity of the soul.\" Sir T. Browne.",
  "supple-chapped - Having a limber tongue. [R.] \"A supple-chapped flatterer.\" Marston.",
  "exosculate - To kiss; especially, to kiss repeatedly or fondly. [Obs.]",
  "heriotable - Subject to the payment of a heriot. Burn.",
  "quinquefoliated - Having five leaves or leaflets. Gray.",
  "coadjuting - Mutually assisting. [Obs.] Drayton.",
  "round-shouldered - Having the shoulders stooping or projecting; round-backed.",
  "spectrometer - A spectroscope fitted for measurements of the luminious spectra observed with it.",
  "chassepot - A kind of breechloading, center-fire rifle, or improved needle gun.",
  "mundification - The act or operation of cleansing.",
  "squaw vine - The partridge berry (Mitchella repens).",
  "reconsolidation - The act or process of reconsolidating; the state of being reconsolidated.",
  "truncheoneer - A person armed with a truncheon. [Written also truncheoner.]",
  "mezzotinter - One who engraves in mezzotint.",
  "contradance - A dance in which the partners are arranged face to face, or in opposite lines.",
  "nematocyst - A lasso cell, or thread cell. See Lasso cell, under Lasso.",
  "nonimportation - Want or failure of importation; a not importing of commodities.",
  "irretraceable - Incapable of being retraced; not retraceable.",
  "hypercatalectic - Having a syllable or two beyond measure; as, a hypercatalectic verse.",
  "permistion - The act of mixing; the state of being mingled; mixture. [Written also permixtion.]",
  "speechify - To make a speech; to harangue. [Used derisively or humorously.]",
  "gravitative - Causing to gravitate; tending to a center. Coleridge.",
  "solecistically - In a solecistic manner.",
  "rowdy - One who engages in rows, or noisy quarrels; a ruffianly fellow. M. Arnold.",
  "nondelivery - A neglect or failure of delivery; omission of delivery.",
  "hopperings - Gravel retaining in the hopper of a cradle.",
  "bister - A dark brown pigment extracted from the soot of wood.",
  "ishmaelitish - Of, pertaining to, or resembling, an Ishmaelite or the Ishmaelites.",
  "articled - Bound by articles; apprenticed; as, an articled clerk.",
  "electrotonize - To cause or produce electrotonus.",
  "unbarrel - To remove or release from a barrel or barrels.",
  "astrologize - To apply astrology to; to study or practice astrology.",
  "burghmote - A court or meeting of a burgh or borough; a borough court held three times yearly.",
  "simpleness - The quality or state of being simple; simplicity. Shak.",
  "calcigerous - Holding lime or other earthy salts; as, the calcigerous cells of the teeth.",
  "gallantness - The quality of being gallant.",
  "granatin - Mannite; -- so called because found in the pomegranate.",
  "bedchair - A chair with adjustable back, for the sick, to support them while sitting up in bed.",
  "wire-heel - A disease in the feet of a horse or other beast.",
  "hunger-bitten - Pinched or weakened by hunger. [Obs.] Milton.",
  "principally - In a principal manner; primarily; above all; chiefly; mainly.",
  "tearer - One who tears or rends anything; also, one who rages or raves with violence.",
  "thaumaturgic - Of or pertaining to thaumaturgy; magical; wonderful. Burton.",
  "fibrillous - Pertaining to, or composed of, fibers.",
  "correctory - Containing or making correction; corrective.",
  "reversibility - The quality of being reversible. Tyndall.",
  "regelate - To freeze together again; to undergo regelation, as ice.",
  "repack - To pack a second time or anew; as, to repack beef; to repack a trunk.",
  "outbray - 1. To exceed in braying. 2. To emit with great noise. [Obs.] Fairfax.",
  "apprecatory - Praying or wishing good. [Obs.]\"Apprecatory benedictions.\" Bp. Hall.",
  "haybird - (a) The European spotted flycatcher. (b) The European blackcap.",
  "mediatorship - The office or character of a mediator.",
  "ritardando - Retarding; -- a direction for slower time; rallentado.",
  "ceremonialism - Adherence to external rites; fondness for ceremony.",
  "disaccommodation - A state of being unaccommodated or unsuited. [R.] Sir M. Hale.",
  "aconddylose - Being without joints; jointless.",
  "clidastes - A genus of exinct marine reptiles, allied to the Mosasaurus. See Illust. in Appendix.",
  "regularize - To cause to become regular; to regulate. [R.]",
  "antibrachial - Of or pertaining to the antibrachium, or forearm.",
  "gubernation - The act of governing; government [Obs.] I. Watts.",
  "ragout - A dish made of pieces of meat, stewed, and highly seasoned; as, a ragout of mutton.",
  "titanitic - Pertaining to, or containing, titanium; as, a titanitic mineral.",
  "doss - A place to sleep in; a bed; hence, sleep. [Slang]",
  "despection - A looking down; a despising. [R.] W. Montagu.",
  "bothrenchyma - Dotted or pitted ducts or vessels forming the pores seen in many kinds of wood.",
  "tolerability - The quality or state of being tolerable. [R.] Fuller. Wordsworth.",
  "ruiniform - Having the appearance of ruins, or of the ruins of houses; -- said of certain minerals.",
  "guttersnipe - (a) A small poster, suitable for a curbstone. (b) A curbstone broker. [U. S.]",
  "oligochaeta - An order of Annelida which includes the earthworms and related species.",
  "trigrammatic - Containing three letters or characters, or three sets of letters or characters.",
  "dandyise - To make, or to act, like a dandy; to dandify.",
  "precondition - A previous or antecedent condition; a preliminary condition.",
  "denyingly - In the manner of one denies a request. Tennyson.",
  "malacostracous - Belonging to the Malacostraca.",
  "phototonus - A motile condition in plants resulting from exposure to light. -- Pho`to*ton\"ic, a.",
  "pyxis - 1. A box; a pyx. 2. (Bot.)  A pyxidium. 3. (Anat.)  The acetabulum. See Acetabulum, 2.",
  "mormonite - A Mormon. -- a.  Mormon. \"Mormonite religion.\" F. W. Newman.",
  "patricianism - The rank or character of patricians.",
  "endolymph - The watery fluid contained in the membranous labyrinth of the internal ear.",
  "epinicion - A song of triumph. [Obs.] T. Warton.",
  "procurable - Capable of being procured; obtainable. Boyle.",
  "evacuant - Emptying; evacuative; purgative; cathartic. -- n. (Med.)  A purgative or cathartic.",
  "underpropper - One who, or that which, underprops or supports.",
  "heliotropism - The phenomenon of turning toward the light, seen in many leaves and flowers.",
  "diplogenic - Partaking of the nature of two bodies; producing two substances. Wright.",
  "wieldance - The act or power of wielding. [Obs.] \"Our weak wieldance.\" Bp. Hall.",
  "infecund - Unfruitful; not producing young; barren; infertile. [Obs.] Evelyn.",
  "siegework - A temporary fort or parallel where siege guns are mounted.",
  "coudee - A measure of length; the distance from the elbow to the end of the middle finger; a cubit.",
  "collineation - The act of aiming at, or directing in a line with, a fixed object. [R.] Johnson.",
  "maidenliness - The quality of being maidenly; the behavior that becomes a maid; modesty; gentleness.",
  "unhive - 1. To drive or remove from a hive. 2. To deprive of habitation or shelter, as a crowd.",
  "dehydrogenate - To deprive of, or free from, hydrogen.",
  "mysterize - To make mysterious; to make a mystery of.",
  "unheard-of - New; unprecedented; unparalleled. Swift.",
  "revilement - The act of reviling; also, contemptuous language; reproach; abuse. Spenser.",
  "paludose - Growing or living in marshy places; marshy.",
  "self-willedness - Obstinacy. Sir W. Scott.",
  "galvanology - A treatise on galvanism, or a description of its phenomena.",
  "musketeer - A soldier armed with a musket.",
  "absolvatory - Conferring absolution; absolutory.",
  "lactucarium - The inspissated juice of the common lettuce, sometimes used as a substitute for opium.",
  "capableness - The quality or state of being capable; capability; adequateness; competency.",
  "semibull - A bull issued by a pope in the period between his election and coronation.",
  "spermococcus - The nucleus of the sperm cell.",
  "smittlish - Infectious; catching. [Scot. & Prov. Eng.] H. Kingsley.",
  "thermoregulator - A device for the automatic regulation of temperature; a thermostat.",
  "postscapula - The part of the scapula behind or below the spine, or mesoscapula.",
  "puer - The dung of dogs, used as an alkaline steep in tanning. Simmonds.",
  "remblai - Earth or materials made into a bank after having been excavated.",
  "transfix - To pierce through, as with a pointed weapon; to impale; as, to transfix one with a dart.",
  "veiling - A veil; a thin covering; also, material for making veils.",
  "meritmonger - One who depends on merit for salvation. [Obs.] Milner.",
  "retry - To try (esp. judicially) a second time; as, to retry a case; to retry an accused person.",
  "buck-basket - A basket in which clothes are carried to the wash. Shak.",
  "exorable - Capable of being moved by entreaty; pitiful; tender. Milton.",
  "individed - Undivided. [R.] Bp. Patrick.",
  "inalienability - The quality or state of being inalienable.",
  "malobservation - Erroneous observation. J. S Mill.",
  "lepismoid - Like or pertaining to the Lepisma.",
  "compear - 1. To appear. [Obs.] 2. (Law)  To appear in court personally or by attorney. [Scot]",
  "nauseant - A substance which produces nausea.",
  "inductrical - Acting by, or in a state of, induction; relating to electrical induction.",
  "diserty - Expressly; clearly; eloquently. [Obs.] Holland.",
  "melanocomous - Having very dark or black hair; black-haired. Prichard.",
  "folkmoter - One who takes part in a folkmote, or local court. [Obs.] Milton.",
  "astucity - Craftiness; astuteness. [R.] Carlyle.",
  "etherealization - An ethereal or spiritlike state. J. H. Stirling.",
  "bacteriological - Of or pertaining to bacteriology; as, bacteriological studies.",
  "misdoing - A wrong done; a fault or crime; an offense; as, it was my misdoing.",
  "reflectingly - With reflection; also, with censure; reproachfully. Swift.",
  "busky - See Bosky, and 1st Bush, n. Shak.",
  "humbles - Entrails of a deer. [Written also umbles.] Johnson.",
  "kerana - A kind of long trumpet, used among the Persians. Moore (Encyc. of Music).",
  "astriferous - Bearing stars. [R.] Blount.",
  "ophiologic - Of or pertaining to ophiology.",
  "floater - 1. One who floats or swims. 2. A float for indicating the height of a liquid surface.",
  "overest - Uppermost; outermost. Full threadbare was his overeste courtepy. Chaucer.",
  "asperation - The act of asperating; a making or becoming rough. Bailey.",
  "protosomite - One of the primitive segments, or metameres, of an animal.",
  "latigo - A strap for tightening a saddle girth. [Western U. S. & Sp. Amer.]",
  "brewhouse - A house or building appropriated to brewing; a brewery.",
  "actinology - The science which treats of rays of light, especially of the actinic or chemical rays.",
  "railleur - A banterer; a jester; a mocker. [R.] Wycherley.",
  "poculiform - Having the shape of a goblet or drinking cup.",
  "duramen - The heartwood of an exogenous tree.",
  "hylicist - A philosopher who treats chiefly of matter; one who adopts or teaches hylism.",
  "opinioned - Opinionated; conceited. His opinioned zeal which he thought judicious. Milton.",
  "moxie - 1. energy; pep. 2. courage, determination. 3. Know-how, expertise. MW10.",
  "niggardness - Niggardliness. Sir P. Sidney.",
  "retailer - One who retails anything; as, a retailer of merchandise; a retailer of gossip.",
  "ringman - The ring finger. [Obs.] Ascham",
  "revibrate - To vibrate back or in return. -- Re`vi*bra\"tion, n.",
  "clogginess - The state of being clogged.",
  "administrable - Capable of being administered; as, an administrable law.",
  "icebird - An Arctic sea bird, as the Arctic fulmar.",
  "arthropathy - Any disease of the joints.",
  "belligerency - The quality of being belligerent; act or state of making war; warfare.",
  "nott-headed - Having the hair cut close. [Obs.] Chapman.",
  "interhaemal - Between the hemal arches or hemal spines. -- n.  An interhemal spine or cartilage.",
  "thankly - Thankfully. [Obs.] Sylvester (Du Bartas).",
  "magisteriality - Magisterialness; authoritativeness. [R.] Fuller.",
  "sarigue - A small South American opossum (Didelphys opossum), having four white spots on the face.",
  "beery - Of or resembling beer; affected by beer; maudlin.",
  "wieldy - Capable of being wielded; manageable; wieldable; -- opposed to unwieldy. [R.] Johnson.",
  "cuminol - A liquid, C3H7.C6H4.CHO, obtained from oil of caraway; -- called also cuminic aldehyde.",
  "subnect - To tie or fasten beneath; to join beneath. [R.] Pope.",
  "lepidopterist - One who studies the Lepidoptera.",
  "agrappes - Hooks and eyes for armor, etc. Fairholt.",
  "anthropomorphitic - to anthropomorphism. Kitto.",
  "julienne - A kind of soup containing thin slices or shreds of carrots, onions, etc.",
  "anus - The posterior opening of the alimentary canal, through which the excrements are expelled.",
  "cunningman - A fortune teller; one who pretends to reveal mysteries. [Obs.] Hudibras.",
  "melampode - The black hellebore. [Obs.] Spenser.",
  "prolongate - To prolong; to extend in space or in time. [R.]",
  "passless - Having no pass; impassable. Cowley.",
  "catholicize - To make or to become catholic or Roman Catholic.",
  "fashion-monger - One who studies the fashions; a fop; a dandy. Marston.",
  "catlinite - A red clay from the Upper Missouri region, used by the Indians for their pipes.",
  "epiglottic - Pertaining to, or connected with, the epiglottis.",
  "impermeability - The quality of being impermeable.",
  "sea tang - A kind of seaweed; tang; tangle. To their nests of sedge and sea tang. Longfellow.",
  "tenpins - A game resembling ninepins, but played with ten pins. See Ninepins. [U. S.]",
  "saffrony - Having a color somewhat like saffron; yellowish. Lord (1630).",
  "transmeate - To pass over or beyond. [Obs.]",
  "decene - One of the higher hydrocarbons, C10H20, of the ethylene series.",
  "diphtherial - Relating to diphtheria; diphtheritic.",
  "hosier - One who deals in hose or stocking, or in goods knit or woven like hose.",
  "underpoise - To weigh, estimate, or rate below desert; to undervalue. [R.] Marston.",
  "vagal - Of or pertaining to the vagus, or pneumogastric nerves; pneumogastric.",
  "brandlin - Same as Branlin, fish and worm.",
  "inconcrete - Not concrete. [R.] L. Andrews.",
  "circumvection - The act of carrying anything around, or the state of being so carried.",
  "torsion head - That part of a torsion balance from which the wire or filament is suspended.",
  "disliker - One who dislikes or disrelishes.",
  "icing - A coating or covering resembling ice, as of sugar and milk or white of egg; frosting.",
  "certifier - One who certifies or assures.",
  "facilitation - The act of facilitating or making easy.",
  "antiquatedness - Quality of being antiquated.",
  "otopathy - A diseased condition of the ear.",
  "calcareo-argillaceous - consisting of, or containing, calcareous and argillaceous earths.",
  "lessee - The person to whom a lease is given, or who takes an estate by lease. Blackstone.",
  "octoroon - The offspring of a quadroon and a white person; a mestee.",
  "thrusher - The song thrush. [Prov. Eng.]",
  "kage - A chantry chapel inclosed with lattice or screen work.",
  "lachrymation - The act of shedding tears; weeping.",
  "procere - Of high stature; tall. [Obs.] Evelyn.",
  "synosteology - That part of anatomy which treats of joints; arthrology.",
  "aphilanthropy - Want of love to mankind; -- the opposite of philanthropy. Coxe.",
  "curvirostres - A group of passerine birds, including the creepers and nuthatches.",
  "pithless - Destitute of pith, or of strength; feeble. Dryden. \"Pithless argumentation.\" Glandstone.",
  "miscellanist - A writer of miscellanies; miscellanarian.",
  "edificant - Building; constructing. [R.] Dugard.",
  "accidentalism - Accidental character or effect. Ruskin.",
  "sodomitical - Pertaining to, or of the nature of, sodomy. -- Sod`om*it\"ic*al*ly, adv.",
  "chargeful - Costly; expensive. [Obs.] The fineness of the gold and chargeful fashion. Shak.",
  "improvisatorial - Of or pertaining to improvisation or extemporaneous composition.",
  "queintise - See Quaintise. [Obs.] Chaucer.",
  "effrenation - Unbridled license; unruliness. [Obs.] Cockeram.",
  "druxey - Having decayed spots or streaks of a whitish color; -- said of timber. Weale.",
  "self-regulative - Tending or serving to regulate one's self or itself. Whewell.",
  "cerebrate - To exhibit mental activity; to have the brain in action.",
  "perorate - To make a peroration; to harangue. [Colloq.]",
  "zooetic - Containing the remains of organized bodies; -- said of rock or soil.",
  "irenic - Fitted or designed to promote peace; pacific; conciliatory; peaceful. Bp. Hall.",
  "heartsick - Sick at heart; extremely depressed in spirits; very despondent.",
  "chirurgical - Surgical [Obs.] \"Chirurgical lore\" Longfellow.",
  "chloridic - Of or pertaining to a chloride; containing a chloride.",
  "educt - That which is educed, as by analysis. Sir W. Hamilton.",
  "unqualify - To disqualify; to unfit. Swift.",
  "repetitionary - Of the nature of, or containing, repetition. [R.]",
  "discommendation - Blame; censure; reproach. [R.] Ayliffe.",
  "treebeard - A pendulous branching lichen (Usnea barbata); -- so called from its resemblance to hair.",
  "aristophanic - Of or pertaining to Aristophanes, the Athenian comic poet.",
  "ecstatical - 1. Ecstatic. Bp. Stillingfleet. 2. Tending to external objects. [R.] Norris.",
  "goldenly - In golden terms or a golden manner; splendidly; delightfully. [Obs.] Shak.",
  "obluctation - A struggle against; resistance; opposition. [Obs.] Fotherby.",
  "ninnyhammer - A simpleton; a silly person. [Colloq.] Addison.",
  "relationship - The state of being related by kindred, affinity, or other alliance. Mason.",
  "merocele - Hernia in the thigh; femoral hernia .",
  "shepherdish - Resembling a shepherd; suiting a shepherd; pastoral. Sir T. Sidney.",
  "improbatory - Implying, or tending to, improbation.",
  "belly-pinched - Pinched with hunger; starved. \"The belly-pinched wolf.\" Shak.",
  "subadvocate - An under or subordinate advocate.",
  "clarre - Wine with a mixture of honey and species. [Obs.] Chaucer.",
  "confusive - Confusing; having a tendency to confusion.  Bp. Hall.",
  "insusurration - The act of whispering into something. [Obs.] Johnson.",
  "kingstone - The black angel fish. See Angel fish, under Angel.",
  "tank ship - A vessel fitted with tanks for the carrying of oil or other liquid in bulk.",
  "coprophagous - Feeding upon dung, as certain insects.",
  "refar - To go over again; to repeat. [Obs.] To him therefore this wonder done refar. Fairfax.",
  "chiropody - The art of treating diseases of the hands and feet.",
  "gigantomachy - A war of giants; especially, the fabulous war of the giants against heaven.",
  "whipstaff - A bar attached to the tiller, for convenience in steering.",
  "antacrid - Corrective of acrimony of the humors.",
  "aglow - In a glow; glowing; as, cheeks aglow; the landscape all aglow.",
  "encomberment - Hindrance; molestation.[Obs.] Spenser.",
  "jarringly - In a jarring or discordant manner.",
  "sliminess - The quality or state of being slimy.",
  "aforementioned - Previously mentioned; before-mentioned. Addison.",
  "tabbinet - A fabric like poplin, with a watered surface. [Written also tabinet.]",
  "perspirability - The quality or state of being perspirable.",
  "transire - A customhouse clearance for a coasting vessel; a permit.",
  "corniplume - A hornlike tuft of feathers on the head of some birds.",
  "careenage - (a) Expense of careening ships. (b) A place for careening.",
  "vespa - A genus of Hymenoptera including the common wasps and hornets.",
  "eupryion - A contrivance for obtaining a light instantaneous, as a lucifer match. Brande & C.",
  "lankness - The state or quality of being lank.",
  "philister - A Philistine; -- a cant name given to townsmen by students in German universities.",
  "snowshoer - One who travels on snowshoes; an expert in using snowshoes. W. G. Beers.",
  "cowherd - One whose occupation is to tend cows.",
  "shatter-brained - Disordered or wandering in intellect; hence, heedless; wild. J. Goodman.",
  "equisetiform - Having the form of the equisetum.",
  "blubbered - Swollen; turgid; as, a blubbered lip. Spenser.",
  "moistener - One who, or that which, moistens. Johnson.",
  "abrahamic - Pertaining to Abraham, the patriarch; as, the Abrachamic covenant.",
  "zooegony - The doctrine of the formation of living beings.",
  "felicify - To make happy; to felicitate. [Obs.] Quarles.",
  "superheater - An apparatus for superheating steam.",
  "thacker - See Thatch, Thatcher. [Obs. or Prov. Eng. & Scot.]",
  "bluewing - The blue-winged teal. See Teal.",
  "blond metal - A variety of clay ironstone, in Staffordshire, England, used for making tools.",
  "peristalsis - Peristaltic contraction or action.",
  "finos - Second best wool from Merino sheep. Gardner.",
  "munga - See Bonnet monkey, under Bonnet.",
  "piscary - The right or privilege of fishing in another man's waters. Blackstone.",
  "pipistrelle - A small European bat (Vesperugo pipistrellus); -- called also flittermouse.",
  "millesimal - Thousandth; consisting of thousandth parts; as, millesimal fractions.",
  "guidebook - A book of directions and information for travelers, tourists, etc.",
  "beleave - To leave or to be left. [Obs.] May.",
  "deoxidize - To deprive of oxygen; to reduce from the state of an oxide.",
  "deflorate - Past the flowering state; having shed its pollen. Gray.",
  "nott-pated - Same as Nott-headed. [Obs.] Shak.",
  "lifeboat - A strong, buoyant boat especially designed for saving the lives of shipwrecked people.",
  "rheotome - An instrument which periodically or otherwise interrupts an electric current. Wheatstone.",
  "discommendable - Deserving, disapprobation or blame. -- Dis`com*mend\"a*ble*ness, n.",
  "nighttime - The time from dusk to dawn; -- opposed to Ant: daytime.",
  "pneumonitic - Of or pertaining to pneumonitis.",
  "amboyna button - A chronic contagious affection of the skin, prevalent in the tropics.",
  "disponee - The person to whom any property is legally conveyed.",
  "transhape - To transshape. [R.] J. Webster (1623).",
  "rhinitis - Infllammation of the nose; esp., inflammation of the mucous membrane of the nostrils.",
  "waterworn - Worn, smoothed, or polished by the action of water; as, waterworn stones.",
  "enfreeze - To freeze; to congeal. [Obs.] Thou hast enfrozened her disdainful breast. Spenser.",
  "enactor - One who enacts a law; one who decrees or establishes as a law. Atterbury.",
  "prickliness - The quality of being prickly, or of having many prickles.",
  "spondyl - A joint of the backbone; a vertebra.",
  "mesologarithm - A logarithm of the cosine or cotangent. [Obs.] Kepler. Hutton.",
  "urethrotome - An instrument for cutting a urethral stricture.",
  "birthdom - The land of one's birth; one's inheritance. [R.] Shak.",
  "extrinsical - Extrinsic. -- Ex*trin\"sic*al*ly(#), adv.",
  "authenticalness - The quality of being authentic; authenticity. [R.] Barrow.",
  "subdial - Of or pertaining to the open air; being under the open sky. [R.] N. Bacon.",
  "gypseous - Resembling or containing gypsum; partaking of the qualities of gypsum.",
  "septiform - Having the form of a septum.",
  "trapanner - One who trapans, or insnares.",
  "popeling - 1. A petty or deputy pope. 2. An adherent of the pope. [R.] Marlowe.",
  "fy - A word which expresses blame, dislike, disapprobation, abhorrence, or contempt. See Fie.",
  "experimentalist - One who makes experiments; an experimenter. Whaterly.",
  "aurited - Having lobes like the ear; auriculate.",
  "exercitation - exercise; practice; use. [R.] Sir T. Browne.",
  "gnomologic - Pertaining to, of the nature of, or resembling, a gnomology.",
  "pulmometry - The determination of the capacity of the lungs.",
  "futurely - In time to come. [Obs.] Raleigh.",
  "woundable - Capable of being wounded; vulnerable. [R.] Fuller.",
  "jealousness - State or quality of being jealous.",
  "wheezy - Breathing with difficulty and with a wheeze; wheezing. Used also figuratively.",
  "seecatch - A full-grown male fur seal. [Alaska]",
  "lampadist - One who gained the prize in the lampadrome.",
  "insectation - The act of pursuing; pursuit; harassment; persecution. [Obs.] Sir T. More.",
  "dejerate - To swear solemnly; to take an oath. [Obs.] Cockeram.",
  "hispanicism - A Spanish idiom or mode of speech. Keightley.",
  "clotter - To concrete into lumps; to clot. [Obs.] \"Clottered blood.\" Chapman.",
  "forewend - To go before. [Obs.] Spenser.",
  "abaca - The Manila-hemp plant (Musa textilis); also, its fiber. See Manila hemp under Manila.",
  "postpalatine - Situated behind the palate, or behind the palatine bones."
]