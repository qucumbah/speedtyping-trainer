export default [
  "shemitic - Of or pertaining to Shem, the son of Noah, or his descendants. See Semitic.",
  "transcendentness - Same as Transcendence.",
  "inscroll - To write on a scroll; to record. [Written also inscrol.] Shak.",
  "roughdry - in laundry work, to dry without smoothing or ironing.",
  "separative - Causing, or being to cause, separation. \"Separative virtue of extreme cold.\" Boyle.",
  "altruist - One imbued with altruism; -- opposed to egoist.",
  "chaomancy - Divination by means of apperances in the air.",
  "coagment - To join together. [Obs.] Glanvill.",
  "space bar - A bar or key, in a typewriter or typesetting machine, used for spacing between letters.",
  "implemental - Pertaining to, or characterized by, implements or their use; mechanical.",
  "deads - The substances which inclose the ore on every side.",
  "indogen - A complex, nitrogenous radical, C8H5NO, regarded as the essential nucleus of indigo.",
  "parenchymal - Of, pertaining to, or consisting of, parenchyma.",
  "polyadelphian - Belonging to the class Polyadelphia; having stamens united in three or more bundles.",
  "senhorita - A Spanish title of courtesy given to a young lady; Miss; also, a young lady.",
  "fucated - Painted; disguised with paint, or with false show.",
  "tequila - An intoxicating liquor made from the maguey in the district of Tequila, Mexico.",
  "obumbrant - Overhanging; as, obumbrant feathers.",
  "symbologist - One who practices, or who is versed in, symbology.",
  "naphthalize - To mingle, saturate, or impregnate, with naphtha.",
  "bituminiferous - Producing bitumen. Kirwan.",
  "persuadable - That may be persuaded. -- Per*suad\"a*ble*ness, n. -- Per*suad\"a*bly, adv.",
  "aventre - To thrust forward (at a venture), as a spear. [Obs.] Spenser.",
  "celliferous - Bearing or producing cells.",
  "hektogram - Same as Hectare, Hectogram, Hectoliter, and Hectometer.",
  "affectionately - With affection; lovingly; fondly; tenderly; kindly.",
  "butyrone - A liquid ketone obtained by heating calcium butyrate.",
  "secale - A genus of cereal grasses including rye.",
  "ossuary - A place where the bones of the dead are deposited; a charnel house. [Obs.] Sir T. Browne.",
  "pederast - One guilty of pederasty; a sodomite.",
  "sadda - A work in the Persian tongue, being a summary of the Zend- Avesta, or sacred books.",
  "cyclostomous - Pertaining to the Cyclostomi.",
  "obfirmate - To make firm; to harden in resolution. [Obs.] Bp. Hall. Sheldon.",
  "ossifying - Changing into bone; becoming bone; as, the ossifying process.",
  "dropmeal - By drops or small portions. [Obs.] Distilling dropmeal, a little at once. Holland.",
  "concisely - In a concise manner; briefly.",
  "overfull - Too full; filled to overflowing; excessively full; surfeited. Shak.",
  "handbook - A book of reference, to be carried in the hand; a manual; a guidebook.",
  "felonous - Wicked; felonious. [Obs.] Spenser.",
  "mandibulohyoid - Pertaining both to the mandibular and the hyoid arch, or situated between them.",
  "raspy - Like a rasp, or the sound made by a rasp; grating. R. D. Blackmore.",
  "circumjovial - One of the moons or satellites of the planet Jupiter. [Obs.] Derham.",
  "paragrapher - A writer of paragraphs; a paragraphist.",
  "initially - In an initial or incipient manner or degree; at the beginning. Barrow.",
  "postable - Capable of being carried by, or as by, post. [Obs.] W. Montagu.",
  "hydrosulphite - A saline compound of hydrosulphurous acid and a base. [R.]",
  "misology - Hatred of argument or discussion; hatred of enlightenment. G. H. Lewes.",
  "iodyrite - Silver iodide, a mineral of a yellowish color.",
  "photophobia - A dread or intolerance of light. Sir T. Watson.",
  "eschatological - Pertaining to the last or final things.",
  "unidimensional - Having but one dimension. See Dimension.",
  "chrome steel - Same as Chromium steel, under Steel.",
  "perruquier - A marker of perukes or wigs.",
  "knappish - Snappish; peevish. [Obs.] Grafton.",
  "deraign - To prove or to refute by proof; to clear (one's self). [Obs.]",
  "stearyl - The hypothetical radical characteristic of stearic acid.",
  "periastral - Among or around the stars. \"Comets in periastral passage.\" R. A. Proctor.",
  "woodrock - A compact woodlike variety of asbestus.",
  "encephalous - Having a head; -- said of most Mollusca; -- opposed to acephalous.",
  "sourly - In a sour manner; with sourness.",
  "clangor - A sharp, harsh, ringing sound. Dryden.",
  "dissymmetrical - Not having symmetry; asymmetrical; unsymmetrical.",
  "dubitative - Tending to doubt; doubtful. [R.] -- Du\"bi*ta*tive*ly, adv. [R.] . Eliot.",
  "tarot - A game of cards; -- called also taroc. Hoyle.",
  "yieldless - Without yielding; unyielding. [Obs.]",
  "smorsato - Growing gradually fainter and softer; dying away; morendo.",
  "integrality - Entireness. [Obs.] Whitaker.",
  "three-pointed - Having three acute or setigerous points; tricuspidate.",
  "puristical - Of or pertaining to purists or purism.",
  "preconstitute - To constitute or establish beforehand.",
  "profanely - In a profane manner. The character of God profanely impeached. Dr. T. Dwight.",
  "rowboat - A boat designed to be propelled by oars instead of sails.",
  "disillusionment - The act of freeing from an illusion, or the state of being freed therefrom.",
  "drivebolt - A drift; a tool for setting bolts home.",
  "sheepshank - A hitch by which a rope may be temporarily shortened.",
  "omniform - Having every form or shape. Berkeley.",
  "existency - Existence. [R.] Sir M. Hale.",
  "plage - A region; country. [Obs.] \"The plages of the north.\" Chaucer.",
  "colleagueship - Partnership in office. Milton.",
  "asteroidal - Of or pertaining to an asteroid, or to the asteroids.",
  "antitheism - The doctrine of antitheists. -- An`ti*the*is\"tic, a.",
  "polymathy - The knowledge of many arts and sciences; variety of learning. Johnson.",
  "barometrically - By means of a barometer, or according to barometric observations.",
  "ambassage - Same as Embassage. [Obs. or R.] Luke xiv. 32.",
  "ichthyic - Like, or pertaining to, fishes.",
  "calycle - A row of small bracts, at the base of the calyx, on the outside.",
  "fritting - The formation of frit or slag by heat with but incipient fusion.",
  "hogget - 1. A young boar of the second year. 2. A sheep or colt alter it has passed its first year.",
  "count-wheel - The wheel in a clock which regulates the number of strokes.",
  "ruttier - A chart of a course, esp. at sea. [Obs.]",
  "teatish - Peevish; tettish; fretful; -- said of a child. See Tettish. [Obs.] Beau. & Fl.",
  "pyrouric - Pertaining to, or designating, an acid now called cyanuric acid. See Cyanuric.",
  "dracunculus - (a) A fish; the dragonet. (b) The Guinea worm (Filaria medinensis).",
  "distractedness - A state of being distracted; distraction. Bp. Hall.",
  "drey - A squirrel's nest. See Dray. [Obs.]",
  "pretervection - The act of carrying past or beyond. [R.] Abp. Potter.",
  "archtreasurer - A chief treasurer. Specifically, the great treasurer of the German empire.",
  "dowable - Capable of being endowed; entitled to dower. Blackstone.",
  "righteously - In a righteous manner; as, to judge righteously.",
  "chabouk - A long whip, such as is used in the East in the infliction of punishment. Balfour.",
  "exundate - To overflow; to inundate. [Obs.] Bailey.",
  "larvated - Masked; clothed as with a mask.",
  "didine - Like or pertaining to the genus Didus, or the dodo.",
  "hyposulphate - A salt of hyposulphuric acid.",
  "pic - A Turkish cloth measure, varying from 18 to 28 inches.",
  "planipennate - Of or pertaining to Planipennia.",
  "interworld - A world between other worlds. Holland.",
  "volcanity - The quality or state of being volcanic, or volcanic origin; volcanicity. [R.]",
  "damnification - That which causes damage or loss.",
  "self-abased - Humbled by consciousness of inferiority, unworthiness, guilt, or shame.",
  "inter - To deposit and cover in the earth; to bury; to inhume; as, to inter a dead body. Shak.",
  "cross-days - The three days preceding the Feast of the Ascension.",
  "bezoardic - Pertaining to, or compounded with, bezoar. -- n.  A medicine containing bezoar.",
  "pyrotungstic - Polytungstic. See Metatungstic.",
  "vengement - Avengement; penal retribution; vengeance. [Obs.] Spenser.",
  "chicken-hearted - Timid; fearful; cowardly. Bunyan.",
  "flammivomous - Vomiting flames, as a volcano. W. Thompson. (1745).",
  "dilettantism - Same as Dilettanteism. F. Harrison.",
  "sagination - The act of fettening or pampering. [R.] Topsell.",
  "syrianism - A Syrian idiom, or a peculiarity of the Syrian language; a Syriacism. Paley.",
  "oppleted - Filled; crowded. [Obs.] Johnson.",
  "vermiculous - Containing, or full of, worms; resembling worms.",
  "exophyllous - Not sheathed in another leaf.",
  "accusement - Accusation. [Obs.] Chaucer.",
  "sheriffdom - The office or jurisdiction of sheriff. See Shrievalty.",
  "succeedant - Succeeding one another; following.",
  "incoact - Not compelled; unconstrained. [Obs.] Coles.",
  "discide - To divide; to cleave in two. [Obs.] Spenser.",
  "prehend - To lay hold of; to seize. [Obs.] Middleton.",
  "bipennated - Having two wings. \"Bipennated insects.\" Derham.",
  "titubation - The act of stumbling, rocking, or rolling; a reeling. Quain.",
  "journey-bated - Worn out with journeying. [Obs.] Shak.",
  "gangrenous - Affected by, or produced by, gangrene; of the nature of gangrene.",
  "photometrist - A specialist in photometry.",
  "anchylotic - Of or pertaining to anchylosis.",
  "solenoglyph - Pertaining to the Selenoglypha. See Ophidia. -- n.  One of the Selenoglypha.",
  "haliotis - A genus of marine shells; the ear-shells. See Abalone.",
  "tetrathionic - Of, pertaining to, or designating, a thionic derivative, H",
  "artificious - Artificial. [Obs.] Johnson.",
  "lymphadenitis - Inflammation of the lymphatic glands; -- called also lymphitis.",
  "cataphonic - Of or relating to cataphonics; catacoustic.",
  "rixatrix - A scolding or quarrelsome woman; a scold. Burrill.",
  "emplead - To accuse; to indict. See Implead.",
  "anlace - A broad dagger formerly worn at the girdle. [Written also anelace.]",
  "thwartness - The quality or state of being thwart; obliquity; perverseness.",
  "amorwe - 1. In the morning. [Obs.] Chaucer. 2. On the following morning. [Obs.] Chaucer.",
  "asteriscus - The smaller of the two otoliths found in the inner ear of many fishes.",
  "screechy - Like a screech; shrill and harsh.",
  "exquisiteness - Quality of being exquisite.",
  "trollopee - A kind of loose dress for women. [Obs.] Goldsmith.",
  "physiogony - The birth of nature. [R.] Coleridge.",
  "abstention - The act of abstaining; a holding aloof. Jer. Taylor.",
  "saw-toothed - Having a tooth or teeth like those of a saw; serrate.",
  "encorporing - Incorporation. [Obs.] Chaucer.",
  "woundily - In a woundy manner; excessively; woundy. [Obs.]",
  "retromingency - The quality or state of being retromingent. Sir T. Browne.",
  "arreption - The act of taking away. [Obs.] \"This arreption was sudden.\" Bp. Hall.",
  "remean - To give meaning to; to explain the meaning of; to interpret. [Obs.] Wyclif.",
  "stairway - A flight of stairs or steps; a staircase. \"A rude and narrow stairway.\" Moore.",
  "rickstand - A flooring or framework on which a rick is made.",
  "ruminator - One who ruminates or muses; a meditator.",
  "overground - Situated over or above ground; as, the overground portion of a plant.",
  "mycologist - One who is versed in, or who studies, mycology.",
  "ravehook - A tool, hooked at the end, for enlarging or clearing seams for the reception of oakum.",
  "olidous - Having a strong, disagreeable smell; fetid. [Obs.] Boyle. Sir T. Browne.",
  "short-handed - Short of, or lacking the regular number of, servants or helpers.",
  "plotter - One who plots or schemes; a contriver; a conspirator; a schemer. Dryden.",
  "seducible - Capable of being seduced; corruptible.",
  "epibolic - Growing or covering over; -- said of a kind of invagination. See under Invagination.",
  "upend - To end up; to set on end, as a cask.",
  "marge - Border; margin; edge; verge. [Poetic] Tennyson. Along the river's stony marge. Wordsworth.",
  "punctualist - One who is very exact in observing forms and ceremonies. Milton.",
  "armamentary - An armory; a magazine or arsenal. [R.]",
  "oppletion - The act of filling up, or the state of being filled up; fullness. [Obs.]",
  "gonoblastid - A reproductive bud of a hydroid; a simple gonophore.",
  "choke-full - Full to the brim; quite full; chock-full.",
  "schene - An Egyptian or Persian measure of length, varying from thirthy- two to sixty stadia.",
  "overlash - To drive on rashly; to go to excess; hence, to exaggerate; to boast. [Obs.] Barrow.",
  "undersparred - Having spars smaller than the usual dimension; -- said of vessels.",
  "water pox - A variety of chicken pox, or varicella. Dunglison.",
  "redressment - The act of redressing; redress. Jefferson.",
  "craftily - With craft; artfully; cunningly.",
  "nephalism - Total abstinence from spirituous liquor.",
  "endable - That may be ended; terminable.",
  "pipe-line - To convey by a pipe line; to furnish with a pipe line or pipe lines.",
  "billard - An English fish, allied to the cod; the coalfish. [Written also billet and billit.]",
  "displosion - Explosion. The vast displosion dissipates the clouds. Young.",
  "adjument - Help; support; also, a helper. [Obs.] Waterhouse.",
  "didactylous - Having only two digits; two-toed.",
  "duck-legged - Having short legs, like a waddling duck; short-legged. Dryden.",
  "bidarka - A portable boat made of skins stretched on a frame. [Alaska] The Century.",
  "fan-tailed - Having an expanded, or fan-shaped, tail; as, the fan-tailed pigeon.",
  "stulp - A short, stout post used for any purpose, a to mark a boundary. [Prov. Eng.] Halliwell.",
  "pronubial - Presiding over marriage. [R.]",
  "multijugate - Having many pairs of leaflets.",
  "garous - Pertaining to, or resembling, garum. Sir T. Browne.",
  "glider - One who, or that which, glides.",
  "extortionate - Characterized by extortion; oppressive; hard.",
  "gossiprede - The relationship between a person and his sponsors. [Obs.]",
  "spathaceous - Having a spathe; resembling a spathe; spathal.",
  "chanticleer - A cock, so called from the clearness or loundness of his voice in crowing.",
  "abominably - In an abominable manner; very odiously; detestably.",
  "spriggy - Full of sprigs or small branches.",
  "begirdle - To surround as with a girdle.",
  "antimetabole - A figure in which the same words or ideas are repeated in transposed order.",
  "silveriness - The state of being silvery.",
  "workbench - A bench on which work is performed, as in a carpenter's shop.",
  "kersey - A kind of coarse, woolen cloth, usually ribbed, woven from wool of long staple.",
  "oxshoe - A shoe for oxen, consisting of a flat piece of iron nailed to the hoof.",
  "contemperate - To temper; to moderate. [Obs.] Moisten and contemperate the air. Sir T. Browne.",
  "boragewort - Plant of the Borage family.",
  "enneahedria - A figure having nine sides; a nonagon.",
  "paleologist - One versed in paleology; a student of antiquity.",
  "to-break - To break completely; to break in pieces. [Obs.] With nose and mouth to-broke. Chaucer.",
  "inflatingly - In a manner tending to inflate.",
  "caterwauling - The cry of cats; a harsh, disagreeable noise or cry like the cry of cats. Shak.",
  "megalesian - Pertaining to, or in honor of, Cybele; as, the Megalesian games at Rome.",
  "kyloes - The cattle of the Hebrides, or of the Highlands. [Scot.] Sir W. Scott.",
  "infound - To pour in; to infuse. [Obs.] Sir T. More.",
  "henbit - A weed of the genus Lamium (L. amplexicaule) with deeply crenate leaves.",
  "sea willow - A gorgonian coral with long flexible branches.",
  "foundationer - One who derives support from the funds or foundation of a college or school. [Eng.]",
  "detractress - A female detractor. Addison.",
  "swellfish - Any plectognath fish that dilates itself, as the bur fish, puffer, or diodon.",
  "erosive - That erodes or gradually eats away; tending to erode; corrosive. Humble.",
  "abstorted - Wrested away. [Obs.] Bailey.",
  "pococurante - A careless person; a trifler. [R.]",
  "alpenstock - A long staff, pointed with iron, used in climbing the Alps. Cheever.",
  "coarticulation - The unoin or articulation of bones to form a joint.",
  "bromide - A compound of bromine with a positive radical.",
  "decastich - A poem consisting of ten lines.",
  "self-taught - Taught by one's own efforts.",
  "tetrose - A monosaccharide derived from a certain alcohol.",
  "vowelish - Of the nature of a vowel. [R.] \"The power [of w] is always vowelish.\" B. Jonson.",
  "participantly - In a participant manner.",
  "luctation - Effort to overcome in contest; struggle; endeavor. [R.] Farindon.",
  "separatistic - Of or pertaining to separatists; characterizing separatists; schismatical.",
  "letheed - Caused by Lethe. \" Letheed dullness.\" [Obs.] Shak.",
  "wolframium - The technical name of the element tungsten. See Tungsten.",
  "serpigo - A dry, scaly eruption on the skin; especially, a ringworm.",
  "distensibility - The quality or capacity of being distensible. [R.]",
  "returnless - Admitting no return. Chapman.",
  "paretic - Of or pertaining to paresis; affected with paresis.",
  "transferography - The act or process of copying inscriptions, or the like, by making transfers.",
  "acquirability - The quality of being acquirable; attainableness. [R.] Paley.",
  "termatarium - Any nest or dwelling of termes, or white ants.",
  "dehortation - Dissuasion; advice against something. [R.]",
  "amelcorn - A variety of wheat from which starch is produced; -- called also French rice.",
  "whitlow-wort - Same as Whitlow grass, under Whitlow.",
  "obliterative - Tending or serving to obliterate.",
  "micrograph - An instrument for executing minute writing or engraving.",
  "monstration - The act of demonstrating; proof. [Obs.] A certain monstration. Grafton.",
  "infashionable - Unfashionable. [Obs.] Beau. & Fl.",
  "diureticalness - The quality of being diuretical; diuretic property.",
  "tracheid - A wood cell with spiral or other markings and closed throughout, as in pine wood.",
  "energizer - One who, or that which, gives energy, or acts in producing an effect.",
  "uterogestation - Gestation in the womb from conception to birth; pregnancy. Pritchard.",
  "christianlike - Becoming to a Christian. A virtuous and a Christianlike conclusion. Shak.",
  "phlegmatically - In a phlegmatic manner.",
  "reapplication - The act of reapplying, or the state of being reapplied.",
  "centurial - Of or pertaining to a century; as, a centurial sermon. [R.]",
  "unrecuring - Incurable. [Obs.] \"Some unrecuring wound.\" Shak.",
  "decarburize - To deprive of carbon; to remove the carbon from.",
  "spiritualistic - Relating to, or connected with, spiritualism.",
  "chub-faced - Having a plump, short face.",
  "petunse - Powdered fledspar, kaolin, or quartz, used in the manufacture of porcelain.",
  "flammation - The act of setting in a flame or blaze. [Obs.] Sir. T. Browne.",
  "concorporation - Union of things in one mass or body. [R.] Dr. H. More.",
  "paradigmatical - Exemplary. -- Par`a*dig*mat\"ic*al*ly, adv. [Obs.]",
  "spongoid - Resembling sponge; like sponge.",
  "scious - Knowing; having knowledge. \"Brutes may be and are scious.\" Coleridge.",
  "iamatology - Materia Medica; that branch of therapeutics which treats of remedies.",
  "accelerator - One who, or that which, accelerates. Also as an adj.; as, accelerator nerves.",
  "abuttal - The butting or boundary of land, particularly at the end; a headland. Spelman.",
  "prostatitis - Inflammation of the prostate.",
  "sachemship - Office or condition of a sachem.",
  "laceration - 1. The act of lacerating. 2. A breach or wound made by lacerating. Arbuthnot.",
  "caroteel - A tierce or cask for dried fruits, etc., usually about 700 lbs. Simmonds.",
  "checkstring - A cord by which a person in a carriage or horse car may signal to the driver.",
  "scarefire - 1. An alarm of fire. [Obs.] 2. A fire causing alarm. [Obs.] Fuller.",
  "anxiousness - The quality of being anxious; great solicitude; anxiety.",
  "imperforation - The state of being without perforation.",
  "chivachie - A cavalry raid; hence, a military expedition. [Obs.] Chaucer.",
  "poggy - (a) See Porgy. (b) A small whale.",
  "vermicule - A small worm or insect larva; also, a wormlike body. [R.] Derham.",
  "discerningly - In a discerning manner; with judgment; judiciously; acutely. Garth.",
  "gonys - The keel or lower outline of a bird's bill, so far as the mandibular rami are united.",
  "inimicous - Inimical; hurtful. [Obs.] Evelyn.",
  "pishu - The Canada lynx. [Written also peeshoo.]",
  "criminology - A treatise on crime or the criminal population. -- Crim`i*nol\"o*gist (-j, n.",
  "metallographist - One who writes on the subject of metals.",
  "placeful - In the appointed place. [Obs.]",
  "embeam - To make brilliant with beams. [R.] G. Fletcher.",
  "fromward - A way from; -- the contrary of toward. [Obs.] Towards or fromwards the zenith. Cheyne.",
  "trusting - Having or exercising trust; confiding; unsuspecting; trustful. -- Trust\"ing*ly, adv.",
  "sackage - The act of taking by storm and pillaging; sack. [R.] H. Roscoe.",
  "acherontic - Of or pertaining to Acheron; infernal; hence, dismal, gloomy; moribund.",
  "miltwaste - A small European fern (Asplenium Ceterach) formerly used in medicine.",
  "calculable - That may be calculated or ascertained by calculation.",
  "hemiplegia - A palsy that affects one side only of the body. -- Hem`i\"pleg\"ic, a.",
  "sorediferous - Bearing soredia; sorediate.",
  "analogic - Of or belonging to analogy. Geo. Eliot.",
  "comicry - The power of exciting mirth; comicalness. [R.] H. Giles.",
  "fictive - Feigned; counterfeit. \"The fount of fictive tears.\" Tennyson.",
  "repatriate - To restore to one's own country.",
  "latitancy - Act or state of lying hid, or lurking. [R.] Sir T. Browne.",
  "mouser - 1. A cat that catches mice. 2. One who pries about on the lookout for something.",
  "assassination - The act of assassinating; a killing by treacherous violence.",
  "revealer - One who, or that which, reveals.",
  "glasswork - Manufacture of glass; articles or ornamentation made of glass.",
  "bordereau - A note or memorandum, esp. one containing an enumeration of documents.",
  "shittleness - Instability; inconstancy. [Obs.] The vain shittlenesse of an unconstant head. Baret.",
  "young one - A young human being; a child; also, a young animal, as a colt.",
  "funiliform - Resembling a cord in toughness and flexibility, as the roots of some endogenous trees.",
  "chiefage - A tribute by the head; a capitation tax. [Written also chevage and chivage.] [Obs.]",
  "ichnographical - Of or pertaining to ichonography; describing a ground plot.",
  "repealment - Recall, as from banishment. [Obs.]",
  "subendocardial - Situated under the endocardium.",
  "disrober - One who, or that which, disrobes.",
  "punition - Punishment. [R.] Mir. for Mag.",
  "broncho-pneumonia - Inflammation of the bronchi and lungs; catarrhal pneumonia.",
  "boragineous - Relating to the Borage tribe; boraginaceous.",
  "feather-pated - Feather-headed; frivolous. [Colloq.] Sir W. Scott.",
  "intranscalent - Impervious to heat; adiathermic.",
  "traducingly - In a traducing manner; by traduction; slanderously.",
  "unshiftable - 1. That may 2. Shiftless; helpless. [Obs.]",
  "anticness - The quality of being antic. Ford.",
  "stupration - Violation of chastity by force; rape. [R.] Sir T. Browne.",
  "odontostomatous - Having toothlike mandibles; -- applied to certain insects.",
  "exemplariness - The state or quality of being exemplary; fitness to be an example.",
  "coyote state - South Dakota; -- a nickname.",
  "anathematical - Pertaining to, or having the nature of, an anathema. -- A*nath`e*mat\"ic*al*ly, adv.",
  "papa - 1. A child's word for father. 2. A parish priest in the Greek Church. Shipley.",
  "misseldine - The mistletoe. [Obs.] Baret.",
  "miting - A little one; -- used as a term of endearment. [Obs.] Skelton.",
  "psalmograph - A writer of psalms; a psalmographer.",
  "cabinetmaking - The art or occupation of making the finer articles of household furniture.",
  "naseberry - A tropical fruit. See Sapodilla. [Written also nisberry.]",
  "smaragdine - Of or pertaining to emerald; resembling emerald; of an emerald green.",
  "ethylic - Pertaining to, derived from, or containing, ethyl; as, ethylic alcohol.",
  "rapturize - To put, or be put, in a state of rapture. [R.]",
  "epulis - A hard tumor developed from the gums.",
  "fretter - One who, or that which, frets.",
  "highflying - Extravagant in opinions or ambition. \"Highflying, arbitrary kings.\" Dryden.",
  "non est factum - The plea of the general issue in an action of debt on bond.",
  "ichthyocolla - Fish glue; isinglass; a glue prepared from the sounds of certain fishes.",
  "fourb - A trickly fellow; a cheat. [Obs.] Evelyn. Denham.",
  "intermeation - A flowing between. [Obs.] Bailey.",
  "zebrula - A cross between a male zebra and a female horse.",
  "legislatress - A woman who makes laws. Shaftesbury.",
  "thesmothete - A lawgiver; a legislator; one of the six junior archons at Athens.",
  "kercher - A kerchief. [Obs.] He became . . . white as a kercher. Sir T. North.",
  "wong - A field. [Obs.] Spelman. \"Woods and wonges.\" Havelok the Dane.",
  "undecyl - The radical regarded as characteristic of undecylic acid.",
  "seven-up - The game of cards called also all fours, and old sledge. [U. S.]",
  "superangelic - Superior to the angels in nature or rank. [R.] Milman.",
  "menstruation - The discharge of the menses; also, the state or the period of menstruating.",
  "thwarter - A disease in sheep, indicated by shaking, trembling, or convulsive motions.",
  "liturgiologist - One versed in liturgiology.",
  "sacrosanct - Sacred; inviolable. [R.] Dr. H. More.",
  "amphiblastic - Segmenting unequally; -- said of telolecithal ova with complete segmentation.",
  "antiodontalgic - Efficacious in curing toothache. -- n.  A remedy for toothache.",
  "geitonogamy - Fertilization of flowers by pollen from other flowers on the same plant.",
  "anthropomorpha - The manlike, or anthropoid, apes.",
  "seriality - The quality or state of succession in a series; sequence. H. Spenser.",
  "anticlinorium - The upward elevation of the crust of the earth, resulting from a geanticlinal.",
  "becripple - To make a cripple of; to cripple; to lame. [R.] Dr. H. More.",
  "stylographical - Same as Stylographic, 1. -- Sty`lo*graph\"ic*al*ly, adv.",
  "restrainable - Capable of being restrained; controllable. Sir T. Browne.",
  "papized - Conformed to popery. [Obs.] \"Papized writers.\" Fuller.",
  "pibcorn - A wind instrument or pipe, with a horn at each end, -- used in Wales.",
  "flog - To beat or strike with a rod or whip; to whip; to lash; to chastise with repeated blows.",
  "personator - One who personates. \"The personators of these actions.\" B. Jonson.",
  "caraboid - Like, or pertaining to the genus Carabus.",
  "anthropophagy - The eating of human flesh; cannibalism.",
  "pin-fire - Having a firing pin to explode the cartridge; as, a pin-fire rifle.",
  "quar - A quarry. [Prov. Eng.] B. Jonson.",
  "unsatiability - Quality of being unsatiable; insatiability. [Obs.]",
  "inexpertness - Want of expertness or skill.",
  "bulbous - Having or containing bulbs, or a bulb; growing from bulbs; bulblike in shape or structure.",
  "concord buggy - A kind of buggy having a body with low sides, and side springs.",
  "mater - See Alma mater, Dura mater, and Pia mater.",
  "mesocolon - The fold of peritoneum, or mesentery, attached to the colon. -- Mes`o*col\"ic, a.",
  "loups - The Pawnees, a tribe of North American Indians whose principal totem was the wolf.",
  "reproducer - One who, or that which, reproduces. Burke.",
  "lachrymable - Lamentable. Martin Parker.",
  "turbinite - A petrified shell resembling the genus Turbo. [R.]",
  "trimness - The quality or state of being trim; orderliness; compactness; snugness; neatness.",
  "torturingly - So as to torture. Beau. & Fl.",
  "matriarchal - Of or pertaining to a matriarch; governed by a matriarch.",
  "progressional - Of or pertaining to progression; tending to, or capable of, progress.",
  "astel - An arch, or ceiling, of boards, placed over the men's heads in a mine.",
  "reprover - One who, or that which, reproves.",
  "mesoarium - The fold of peritoneum which suspends the ovary from the dorsal wall of the body cavity.",
  "anconeal - Of or pertaining to the ancon or elbow. \"The olecranon on anconeal process.\" Flower.",
  "denitration - A disengaging, or removal, of nitric acid.",
  "armor-bearer - One who carries the armor or arms of another; an armiger. Judg. ix. 54.",
  "darkle - To grow dark; to show indistinctly. Thackeray.",
  "nodous - Nodose; knotty; knotted. [Obs.]",
  "disobligement - Release from obligation. [Obs.]",
  "agnoiology - The doctrine concerning those things of which we are necessarily ignorant.",
  "self-color - A color not mixed or variegated.",
  "theophilanthropic - Pertaining to theophilanthropy or the theophilanthropists.",
  "stemmy - Abounding in stems, or mixed with stems; -- said of tea, dried currants, etc. [Colloq.]",
  "disacryl - A white amorphous substance obtained as a polymeric modification of acrolein.",
  "incinerable - Capable of being incinerated or reduced to ashes. Sir T. Browne.",
  "dolphinet - A female dolphin. [R.] Spenser.",
  "anidiomatic - Not idiomatic. [R.] Landor.",
  "unculture - Want of culture. \"Idleness, ill husbandry . . . unculture.\" Bp. Hall.",
  "intreasure - To lay up, as in a treasury; to hoard. [Obs.] Shak.",
  "cogwheel - A wheel with cogs or teeth; a gear wheel. See Illust. of Gearing.",
  "scaglia - A reddish variety of limestone.",
  "vertiginate - Turned round; giddy. [R.] Coleridge.",
  "fuzzle - To make drunk; to intoxicate; to fuddle. [Obs.] Burton.",
  "self-deceived - Deceived or misled respecting one's self by one's own mistake or error.",
  "polygoneutic - Having two or more broods in a season.",
  "taxability - The quality or state of being taxable; taxableness.",
  "rudenture - Cabling. See Cabling. gwilt.",
  "hydrotellurate - A salt formed by the union of hydrotelluric acid and the base.",
  "old-maidism - The condition or characteristics of an old maid. G. Eliot.",
  "caledonite - A hydrous sulphate of copper and lead, found in some parts of Caledonia or Scotland.",
  "demicadence - An imperfect or half cadence, falling on the dominant instead of on the key note.",
  "torturer - One who tortures; a tormentor.",
  "unfailable - Infallible. [Obs.] \"This unfailable word of truth.\" Bp. Hall.",
  "unwoman - To deprive of the qualities of a woman; to unsex. [R.] R. Browning.",
  "foresay - To foretell. [Obs.] Her danger nigh that sudden change foresaid. Fairfax.",
  "frapler - A blusterer; a rowdy. [Obs.] Unpolished, a frapler, and base. B. Jonson.",
  "porphyrize - To cause to resemble porphyry; to make spotted in composition, like porphyry.",
  "holethnos - A parent stock or race of people, not yet divided into separate branches or tribes.",
  "dueness - Quality of being due; debt; what is due or becoming. T. Goodwin.",
  "kloof - A glen; a ravine closed at its upper end. [South Africa]",
  "laboredly - In a labored manner; with labor.",
  "technicist - One skilled to technics or in one or more of the practical arts.",
  "apishness - The quality of being apish; mimicry; foppery.",
  "grallatorial - Of or pertaining to the Grallatores, or waders.",
  "calin - An alloy of lead and tin, of which the Chinese make tea canisters.",
  "inappetency - Want of appetency; want of desire.",
  "imbowment - act of imbowing; an arch; a vault. Bacon.",
  "beach comber - A long, curling wave rolling in from the ocean. See Comber. [Amer.]",
  "vacillancy - The quality or state of being vacillant, or wavering. [R.] Dr. H. More.",
  "exestuate - To be agitated; to boil up; to effervesce. [Obs.]",
  "malapropos - Unseasonable or unseasonably; unsuitable or unsuitably.",
  "occultness - State or quality of being occult.",
  "featly - Neatly; dexterously; nimbly. [Archaic] Foot featly here and there. Shak.",
  "blushingly - In a blushing manner; with a blush or blushes; as, to answer or confess blushingly.",
  "butterbird - The rice bunting or bobolink; -- so called in the island of Jamaica.",
  "sergeancy - The office of a sergeant; sergeantship. [Written also serjeancy.]",
  "deerstalker - One who practices deerstalking.",
  "surrenderee - The person to whom a surrender is made. Mozley & W.",
  "calefactor - A heater; one who, or that which, makes hot, as a stove, etc.",
  "coralligerous - Producing coral; coraliferous.",
  "cabotage - Navigation along the coast; the details of coast pilotage.",
  "westmost - Lying farthest to the west; westernmost.",
  "apositic - Destroying the appetite, or suspending hunger.",
  "birding - Birdcatching or fowling. Shak. Birding piece, a fowling piece. Shak.",
  "elegy - A mournful or plaintive poem; a funereal song; a poem of lamentation. Shak.",
  "tabific - Producing tabes; wasting; tabefying.",
  "subjectness - Quality of being subject. [R.]",
  "reputedly - In common opinion or estimation; by repute.",
  "cedrine - Of or pertaining to cedar or the cedar tree.",
  "lesses - The leavings or dung of beasts.",
  "aberr - To wander; to stray. [Obs.] Sir T. Browne.",
  "sulphionide - A binary compound of sulphion, or one so regarded; thus, sulphuric acid, Hsulphionide.",
  "ensure - 1. To make sure. See Insure. 2. To betroth. [Obs.] Sir T. More.",
  "hierography - Sacred writing. [R.] Bailey.",
  "educational - Of or pertaining to education. \"His educational establishment.\" J. H. Newman.",
  "yesteryear - The year last past; last year.",
  "chomp - To chew loudly and greedily; to champ. [Prov. Eng. & Colloq. U. S.] Halliwell.",
  "afterwise - Wise after the event; wise or knowing, when it is too late.",
  "demigration - Emigration. [Obs.] Bp. Hall.",
  "disendow - To deprive of an endowment, as a church. Gladstone.",
  "overskip - To skip or leap over; to treat with indifference. Shak.",
  "sea poppy - The horn poppy. See under Horn.",
  "agatiferous - Containing or producing agates. Craig.",
  "maundy thursday - The Thursday in Passion week, or next before Good Friday.",
  "accubation - The act or posture of reclining on a couch, as practiced by the ancients at meals.",
  "starproof - Impervious to the light of the stars; as, a starproof elm. [Poetic] Milton.",
  "tetrapod - An insect characterized by having but four perfect legs, as certain of the butterflies.",
  "oligoclase - A triclinic soda-lime feldspar. See Feldspar.",
  "serpentinize - To convert (a magnesian silicate) into serpentine. -- Ser`pen*tin`i*za\"tion, n.",
  "mylodon - An extinct genus of large slothlike American edentates, allied to Megatherium.",
  "humblesse - Humbleness; abasement; low obeisance. [Obs.] Chaucer. Spenser.",
  "self-sacrificing - Yielding up one's own interest, ffeelings, etc; sacrificing one's self.",
  "disflesh - To reduce the flesh or obesity of. [Obs.] Shelton.",
  "thribble - Triple; treble; threefold. [Prov. Eng. or Colloq.] Halliwell.",
  "cochleated - Having the form of a snail shell; spiral; turbinated.",
  "fittedness - The state or quality of being fitted; adaptation. [Obs.] Dr. H. More.",
  "chopchurch - An exchanger or an exchange of benefices. [Cant]",
  "transdialect - To change or translate from one dialect into another. [R.] Bp. Warburton.",
  "momentariness - The state or quality of being momentary; shortness of duration.",
  "gangetic - Pertaining to, or inhabiting, the Ganges; as, the Gangetic shark.",
  "end- - A combining form signifying within; as, endocarp, endogen, endocuneiform, endaspidean.",
  "eternization - The act of eternizing; the act of rendering immortal or famous.",
  "thysbe - A common clearwing moth (Hemaris thysbe).",
  "interchangeability - The state or quality of being interchangeable; interchangeableness.",
  "gloried - Illustrious; honorable; noble. [Obs.] Milton.",
  "self-moving - Moving by inherent power, without the aid of external impulse.",
  "impudency - Impudence. [Obs.] Burton. Audacious without impudency. Shak.",
  "ushership - The office of an usher; usherdom.",
  "superfuse - To pour (something) over or on something else. [Obs.] Evelyn.",
  "crocetin - A dyestuff, obtained from the Chinese croicin, which produces a brilliant yellow.",
  "tetrylene - Butylene; -- so called from the four carbon atoms in the molecule.",
  "vastitude - 1. Vastness; immense extent. [R.] 2. Destruction; vastation. [Obs.] Joye.",
  "miscarriageable - Capable of miscarrying; liable to fail. [R.] Bp. Hall.",
  "solifidianism - The state of Solifidians.",
  "stratographic - Of or pertaining to stratography.",
  "beswike - To lure; to cheat. [Obs.] Gower.",
  "declaredness - The state of being declared.",
  "attendancy - The quality of attending or accompanying; attendance; an attendant. [Obs.]",
  "epigenetic - Of or pertaining to the epigenesis; produced according to the theory of epigenesis.",
  "onerary - Fitted for, or carrying, a burden. Johnson.",
  "citiner - One who is born or bred in a city; a citizen. [Obs.] Champan.",
  "invitement - Invitation. [Obs.] Chapman.",
  "d valve - A kind of slide valve. See Slide valve, under Slide.",
  "methide - A binary compound of methyl with some element; as, aluminium methide, Al2(CH3)6.",
  "trisyllable - A word consisting of three syllables only; as, a-ven-ger.",
  "hemselven - Themselves; -- used reflexively. [Obs.] Chaucer.",
  "flautist - A player on the flute; a flutist.",
  "octachord - An instrument of eight strings; a system of eight tones. [Also written octochord.]",
  "tangentially - In the direction of a tangent.",
  "frigate-built - Built like a frigate with a raised quarter-deck and forecastle.",
  "homing - Home-returning; -- used specifically of carrier pigeons.",
  "patin - A plate. See Paten. \"Inlaid with patines of bright gold.\" Shak.",
  "negotiatrix - A woman who negotiates. Miss Edgeworth.",
  "cooky - A small, flat, sweetened cake of various kinds.",
  "mus - A genus of small rodents, including the common mouse and rat.",
  "overweigh - To exceed in weight; to overbalance; to weigh down. Drayton. Hooker.",
  "intentiveness - Closeness of attention or application of mind; attentiveness. [Obs.] W. Montagu.",
  "choroidal - Pertaining to the choroid coat.",
  "dissyllabify - To form into two syllables. Ogilvie.",
  "surviver - One who survives; a survivor.",
  "low-studded - Furnished or built with short studs; as, a low-studded house or room.",
  "perky - Perk; pert; jaunty; trim. There amid perky larches and pines. Tennyson.",
  "invader - One who invades; an assailant; an encroacher; an intruder.",
  "peripneumonic - Of or pertaining to peripneumonia.",
  "hospitage - Hospitality. [Obs.] Spenser.",
  "motacil - Any singing bird of the genus Motacilla; a wagtail.",
  "anthophorous - Flower bearing; supporting the flower.",
  "hylophagous - Eating green shoots, as certain insects do.",
  "fibril - A small fiber; the branch of a fiber; a very slender thread; a fibrilla. Cheyne.",
  "bandore - A musical stringed instrument, similar in form to a guitar; a pandore.",
  "cerise - Cherry-colored; a light bright red; --- applied to textile fabrics, especially silk.",
  "mitt - A mitten; also, a covering for the wrist and hand and not for the fingers.",
  "phlebolith - A small calcareous concretion formed in a vein; a vein stone.",
  "attonce - At once; together. [Obs.] Spenser.",
  "disembowered - Deprived of, or removed from, a bower. [Poetic] Bryant.",
  "terremote - An earthquake. [Obs.] Gower.",
  "transcension - The act of transcending, or surpassing; also, passage over. [Obs.] Chapman.",
  "capnomancy - Divination by means of the ascent or motion of smoke.",
  "inveterateness - Inveteracy. Sir T. Browne.",
  "ergal - Potential energy; negative value of the force function.",
  "sincereness - Same as Sincerity. Beau & Fl.",
  "bicipitous - Having two heads; bicipital. \"Bicipitous serpents.\" Sir T. Browne.",
  "quinizarin - A yellow crystalline substance produced artificially. It is isomeric with alizarin.",
  "corsepresent - An offering made to the church at the interment of a dead body. Blackstone.",
  "radioli - The barbs of the radii of a feather; barbules.",
  "prostibulous - Of or pertaining to prostitutes or prostitution; meretricious. [Obs.] Bale.",
  "cardioid - An algebraic curve, so called from its resemblance to a heart.",
  "productibility - The state of being productible; producibility. Ruskin.",
  "dyscrasite - A mineral consisting of antimony and silver.",
  "amusing - Giving amusement; diverting; as, an amusing story. -- A*mus\"ing*ly, adv.",
  "decagramme - A weight of the metric system; ten grams, equal to about 154.32 grains avoirdupois.",
  "fissigemmation - A process of reproduction intermediate between fission and gemmation.",
  "unidiomatical - Not idiomatic. [R.] Landor.",
  "hag-ridden - Ridden by a hag or witch; hence, afflicted with nightmare. Beattie. Cheyne.",
  "degloried - Deprived of glory; dishonored. [Obs.] \"With thorns degloried.\" G. Fletcher.",
  "blow-out - The cleaning of the flues of a boiler from scale, etc., by a blast of steam.",
  "eliquament - A liquid obtained from fat, or fat fish, by pressure.",
  "blesser - One who blesses; one who bestows or invokes a blessing.",
  "transcorporate - To transmigrate. [Obs.] Sir T. Browne.",
  "defigure - To delineate. [Obs.] These two stones as they are here defigured. Weever.",
  "sarcophagous - Feeding on flesh; flesh-eating; carnivorous.",
  "synochal - Of or pertaining to synocha; like synocha. [Obs.]",
  "prepay - To pay in advance, or beforehand; as, to prepay postage.",
  "cygnus - A constellation of the northern hemisphere east of, or following, Lyra; the Swan.",
  "bushelman - A tailor's assistant for repairing garments; -- called also busheler. [Local, U.S.]",
  "unboy - To divest of the traits of a boy. [R.] Clarendon.",
  "monotomous - Having a distinct cleavage in a single direction only.",
  "coestablishment - Joint establishment. Bp. Watson.",
  "scabbiness - The quality or state of being scabby.",
  "interaulic - Existing between royal courts. [R.] \"Interaulic politics.\" Motley.",
  "bookcase - A case with shelves for holding books, esp. one with glazed doors.",
  "nitrifier - An agent employed in nitrification.",
  "continuable - Capable of being continued [R.]",
  "neglective - Neglectful. [R.] \"Neglective of their own children.\" Fuller.",
  "combatable - Such as can be, or is liable to be, combated; as, combatable foes, evils, or arguments.",
  "agre - In good part; kindly. [Obs.] Rom. of R.",
  "disempower - To deprive of power; to divest of strength. H. Bushnell.",
  "sputation - The act of spitting; expectoration. Harvey.",
  "asexually - In an asexual manner; without sexual agency.",
  "diaphaned - Transparent or translucent. [R.]",
  "ewery - An office or place of household service where the ewers were formerly kept. [Enq.] Parker.",
  "lamaistic - Of or pertaining to Lamaism.",
  "omophagic - Eating raw flesh; using uncooked meat as food; as, omophagic feasts, rites.",
  "holmium - A rare element said to be contained in gadolinite. -- Hol\"mic, a.",
  "marsupite - A fossil crinoid of the genus Marsupites, resembling a purse in form.",
  "aboma - A large South American serpent (Boa aboma).",
  "favoring - That favors. -- Fa\"vor*ing*ly, adv.",
  "pelure - A crisp, hard, thin paper, sometimes used for postage stamps.",
  "chicky - A chicken; -- used as a diminutive or pet name, especially in calling fowls.",
  "delibate - To taste; to take a sip of; to dabble in. [Obs.]",
  "lanuginous - Covered with down, or fine soft hair; downy.",
  "breastbeam - The front transverse beam of a locomotive.",
  "cachepot - An ornamental casing for a flowerpot, of porcelain, metal, paper, etc.",
  "traditioner - One who adheres to tradition.",
  "pouncet box - A box with a perforated lid, for sprinkling pounce, or for holding perfumes. Shak.",
  "distemperment - Distempered state; distemperature. [Obs.] Feltham.",
  "fore part - The part most advanced, or first in time or in place; the beginning.",
  "pinguid - Fat; unctuous; greasy. [Obs.] \"Some clays are more pinguid.\" Mortimer.",
  "recursion - The act of recurring; return. [Obs.] Boyle.",
  "irresilient - Not resilient; not recoiling or rebounding; inelastic.",
  "mycothrix - The chain of micrococci formed by the division of the micrococci in multiplication.",
  "coalgoose - The cormorant; -- so called from its black color.",
  "degenerous - Degenerate; base. [Obs.] \"Degenerous passions.\" Dryden. \"Degenerous practices.\" South.",
  "pisolitic - Composed of, containing, or resembling, pisolite.",
  "prostatic - Of or pertaining to the prostate gland. Prostatic catheter. (Med.) See under Catheter.",
  "swayed - Bent down, and hollow in the back; sway-backed; -- said of a horse. Shak.",
  "beatificate - To beatify. [Obs.] Fuller.",
  "welsome - Prosperous; well. [Obs.] Wyclif. -- Wel\"some*ly, adv. Wyclif.",
  "acquaintant - An acquaintance. [R.] Swift.",
  "photic - Relating to the production of light by the lower animals.",
  "dishevele - Disheveled. [Obs.] Dishevele, save his cap, he rode all bare. Chaucer.",
  "auspicial - Of or pertaining to auspices; auspicious. [R.]",
  "souterrain - A grotto or cavern under ground. [Obs.] Arbuthnot.",
  "rosulate - Arranged in little roselike clusters; -- said of leaves and bracts.",
  "mistful - Clouded with, or as with, mist.",
  "dukedom - 1. The territory of a duke. 2. The title or dignity of a duke. Shak.",
  "controllership - The office of a controller.",
  "levirostres - A group of birds, including the hornbills, kingfishers, and related forms.",
  "gullible - Easily gulled; that may be duped. -- Gul\"li*bii`i*ty, n. Burke.",
  "nudation - The act of stripping, or making bare or naked.",
  "thunderproof - Secure against the effects of thunder or lightning.",
  "submuscular - Situated underneath a muscle or muscles.",
  "autocratorical - Pertaining to an autocrator; absolute. [Obs.] Bp. Pearson.",
  "subtegulaneous - Under the roof or eaves; within doors. [R.]",
  "dorsoventral - From the dorsal to the ventral side of an animal; as, the dorsoventral axis.",
  "inequal - Unequal; uneven; various. [Obs.] Chaucer.",
  "shoplike - Suiting a shop; vulgar. B. Jonson.",
  "gladiate - Sword-shaped; resembling a sword in form, as the leaf of the iris, or of the gladiolus.",
  "epihyal - A segment next above the ceratohyal in the hyoidean arch.",
  "flacket - A barrel-shaped bottle; a flagon.",
  "ludificatory - Making sport; tending to excite derision. [Obs.]",
  "snying - A curved plank, placed edgewise, to work in the bows of a vessel. R. H. Dana, Jr.",
  "-et - A noun suffix with a diminutive force; as in baronet, pocket, facet, floweret, latchet.",
  "puddler - One who converts cast iron into wrought iron by the process of puddling.",
  "mattages - A shrike or butcher bird; -- written also matagasse. [Prov. Eng.]",
  "proportionless - Without proportion; unsymmetrical.",
  "ethnographically - In an ethnographical manner.",
  "nototrema - The pouched, or marsupial, frog of South America.",
  "arace - To tear up by the roots; to draw away. [Obs.] Wyatt.",
  "uralic - Of or relating to the Ural Mountains.",
  "autokinesis - Spontaneous or voluntary movement; movement due to an internal cause.",
  "azurite - Blue carbonate of copper; blue malachite.",
  "ligniperdous - Wood-destroying; -- said of certain insects.",
  "plerome - The central column of parenchyma in a growing stem or root.",
  "revolutionism - The state of being in revolution; revolutionary doctrines or principles.",
  "baby farming - The business of keeping a baby farm.",
  "tubipore - Any species of the genus Tubipora.",
  "disposedness - The state of being disposed or inclined; inclination; propensity. [R.]",
  "attemperation - The act of attempering or regulating. [Archaic] Bacon.",
  "avenious - Being without veins or nerves, as the leaves of certain plants.",
  "cucujo - The fire beetle of Mexico and the West Indies.",
  "coadaptation - Mutual adaption. R. Owen.",
  "abuser - One who abuses [in the various senses of the verb].",
  "trochiscus - A kind of tablet or lozenge; a troche.",
  "ootocoid - A half oviparous, or an oviparous, mammal; a marsupial or monotreme.",
  "daughter-in-law - The wife of one's son.",
  "educable - Capable of being educated. \"Men are educable.\" M. Arnold.",
  "agrostographical - Pertaining to agrostography.",
  "absonous - Discordant; inharmonious; incongruous. [Obs.] \"Absonous to our reason.\" Glanvill.",
  "himselven - Themselves. See Hemself. [Obs.] Chaucer.",
  "biometry - Measurement of life; calculation of the probable duration of human life.",
  "transelement - To change or transpose the elements of; to transubstantiate. [Obs.] Jer. Taylor.",
  "athermanous - Not transmitting heat; -- opposed to diathermanous.",
  "spyboat - A boat sent to make discoveries and bring intelligence. Arbuthnot.",
  "dumal - Pertaining to, or set with, briers or bushes; brambly. [R.]",
  "pillery - Plunder; pillage. [Obs.] Daniel.",
  "monasticon - A book giving an account of monasteries.",
  "adjuster - One who, or that which, adjusts.",
  "riparious - Growing along the banks of rivers; riparian.",
  "cruelly - 1. In a cruel manner. 2. Extremly; very. [Colloq.] Spectator.",
  "soorma - A preparation of antimony with which Mohammedan men anoint their eyelids.",
  "callat - Same as Callet. [Obs.] A callat of boundless tongue. Shak.",
  "towrope - A rope used in towing vessels.",
  "cispadane - On the hither side of the river Po with reference to Rome; that is, on the south side.",
  "systematically - In a systematic manner; methodically.",
  "repugnable - Capable of being repugned or resisted. [R.] Sir T. North.",
  "stereotypic - Of or pertaining to stereotype, or stereotype plates.",
  "cystotome - A knife or instrument used in cystotomy.",
  "scintillously - In a scintillant manner. [R.]",
  "demote - To reduce to a lower grade, as in school.",
  "orcharding - 1. The cultivation of orchards. 2. Orchards, in general.",
  "applier - He who, or that which, applies.",
  "canular - See Cannula, Cannular, and Cannulated.",
  "condensative - Having the property of condensing.",
  "ambidextrousness - The quality of being ambidextrous; ambidexterity.",
  "spitfire - A violent, irascible, or passionate person. [Colloq.] Grose.",
  "cablet - A little cable less than ten inches in circumference.",
  "unreality - The quality or state of being unreal; want of reality.",
  "gaudiness - The quality of being gaudy. Whitlock.",
  "oliverian - An adherent of Oliver Cromwell. Macaulay.",
  "spadeful - As much as a spade will hold or lift.",
  "ululate - To howl, as a dog or a wolf; to wail; as, ululating jackals. Sir T. Herbert.",
  "sulphotungstate - A salt of sulphotungstic acid.",
  "outblush - To exceed in blushing; to surpass in rosy color. T. Shipman.",
  "thylacine - The zebra wolf. See under Wolf.",
  "hefty - Moderately heavy. [Colloq. U. S.]",
  "intertwiningly - By intertwining or being intertwined.",
  "planticle - A young plant, or plant in embryo. E. Darwin.",
  "inscriptible - Capable of being inscribed; inscribable.",
  "begrudge - To grudge; to envy the possession of.",
  "overprompt - Too prompt; too ready or eager; precipitate. -- O`ver*prompt\"ness, n.",
  "wineless - destitute of wine; as, wineless life.",
  "favosite - Like or pertaining to the genus Favosites.",
  "angioscope - An instrument for examining the capillary vessels of animals and plants. Morin.",
  "disculpatory - Tending to exculpate; exculpatory.",
  "sacrificable - Capable of being offered in sacrifice. [R.] Sir T. Browne.",
  "presentient - Feeling or perceiving beforehand.",
  "by-walk - secluded or private walk. He moves afterward in by-walks. Dryden.",
  "five-leafed - Having five leaflets, as the Virginia creeper.",
  "plumicorn - An ear tuft of feathers, as in the horned owls.",
  "monolithic - Of or pertaining to a monolith; consisting of a single stone.",
  "refulgence - The quality of being refulgent; brilliancy; splender; radiance.",
  "enform - To form; to fashion. [Obs.] Spenser.",
  "lackadaisical - Affectedly pensive; languidly sentimental. -- Lack`a*dai\"si*cal*ly, adv.",
  "redsear - To be brittle when red-hot; to be red-short. Moxon.",
  "cursoriness - The quality of being cursory; superficial performance; as, cursoriness of view.",
  "corymbiferous - Bearing corymbs of flowers or fruit.",
  "fratrage - A sharing among brothers, or brothers' kin. [Obs.] Crabb.",
  "goa - A species of antelope (Procapra picticauda), inhabiting Thibet.",
  "appel - A tap or stamp of the foot as a warning of intent to attack; -- called also attack.",
  "mottoed - Bearing or having a motto; as, a mottoed coat or device.",
  "hereinbefore - In the preceding part of this (writing, document, book, etc.).",
  "sententiarist - A sententiary. Barnas Sears (Life of Luther).",
  "forted - Furnished with, or guarded by, forts; strengthened or defended, as by forts. [R.] Shak.",
  "complexed - Complex, complicated. [Obs.] \"Complexed significations.\" Sir T. Browne.",
  "low-mindedness - The quality of being lowminded; meanness; baseness.",
  "balsamine - The Impatiens balsamina, or garden balsam.",
  "excarnate - To deprive or clear of flesh. Grew.",
  "gainsayer - One who gainsays, contradicts, or denies. \"To convince the gainsayers.\" Tit. i. 9.",
  "physico-philosophy - The philosophy of nature.",
  "tetravalent - Having a valence of four; tetratomic; quadrivalent.",
  "exungulate - To pare off, as nails, the hoof, etc. [R.]",
  "plenist - One who holds that all space is full of matter.",
  "hostilize - To make hostile; to cause to become an enemy. [Obs.] A. Seward.",
  "wallbird - The spotted flycatcher. [Prov. Eng.]",
  "senectitude - Old age. [R.] \"Senectitude, weary of its toils.\" H. Miller.",
  "translatory - Serving to translate; transferring. [R.] Arbuthnot.",
  "encloister - To shut up in a cloister; to cloister.",
  "melanic - 1. Melanotic. 2. (Ethnol.)  Of or pertaining to the black-haired races. Prichard.",
  "diaconal - Of or pertaining to a deacon.",
  "lickerous - Lickerish; eager; lustful. [Obs.] -- Lick\"er*ous*ness, n. [Obs.] Chaucer.",
  "trafficless - Destitute of traffic, or trade.",
  "fullery - The place or the works where the fulling of cloth is carried on.",
  "foreworn - Worn out; wasted; used up. [Archaic] Old foreworn stories almost forgotten. Brydges.",
  "tajacu - The common, or collared, peccary.",
  "oppignerate - To pledge; to pawn. [Obs.] Bacon.",
  "muscularity - The state or quality of being muscular. Grew.",
  "chalder - A kind of bird; the oyster catcher.",
  "plausibleness - Quality of being plausible.",
  "graminaceous - Pertaining to, or resembling, the grasses; gramineous; as, graminaceous plants.",
  "helminthologic - Of or pertaining to helminthology.",
  "seafaring - Following the business of a mariner; as, a seafaring man.",
  "unvote - To reverse or annul by vote, as a former vote. [R.] Bp, Burnet.",
  "otalgic - Of or pertaining to otalgia. -- n.  A remedy for otalgia.",
  "meerkat - A South African carnivore (Cynictis penicillata), allied to the ichneumons.",
  "fluminous - Pertaining to rivers; abounding in streama.",
  "millenniarism - Belief in, or expectation of, the millennium; millenarianism.",
  "derth - Dearth; scarcity. [Obs.] Spenser.",
  "invertedly - In an inverted order. Derham.",
  "epauletted - Wearing epaulets; decorated with epaulets.",
  "indifulvin - A reddish resinous substance, obtained from indican.",
  "fraternizer - One who fraternizes. Burke.",
  "despiser - One who despises; a contemner; a scorner.",
  "conferential - Relating to conference. [R.] Clarke.",
  "corruptress - A woman who corrupts. Thou studied old corruptress. Beau & Fl.",
  "castellany - The lordship of a castle; the extent of land and jurisdiction appertaining to a castle.",
  "mugget - The small entrails of a calf or a hog.",
  "pightel - A small inclosure. [Written also pightle.] [Obs. or Prov. Eng.]",
  "do-nothing - Doing nothing; inactive; idle; lazy; as, a do-nothing policy.",
  "negotiousness - The state of being busily occupied; activity. [R.] D. Rogers.",
  "overslop - An outer garment, or slop. [Obs.] Chaucer.",
  "incorporality - Incorporeality. [Obs.] Bailey.",
  "leede - A caldron; a copper kettle. [Obs.] \"A furnace of a leed.\" Chaucer.",
  "bibliopolic - Of or pertaining to the sale of books. \"Bibliopolic difficulties.\" Carlyle.",
  "newsmonger - One who deals in news; one who is active in hearing and telling news.",
  "dramaturgist - One versed in dramaturgy. Carlyle.",
  "lawgiver - One who makes or enacts a law or system of laws; a legislator.",
  "selenitic - Of or pertaining to selenite; resembling or containing selenite.",
  "trisulphide - A sulphide containing three atoms of sulphur.",
  "mussite - A variety of pyroxene, from the Mussa Alp in Piedmont; diopside.",
  "chekmak - A turkish fabric of silk and cotton, with gold thread interwoven.",
  "inopinate - Not expected or looked for. [Obs.]",
  "wheen - A quantity; a goodly number. [Scot.] \"A wheen other dogs.\" Sir W. Scott.",
  "hirudine - Of or pertaining to the leeches.",
  "aestho-physiology - The science of sensation in relation to nervous action. H. Spenser.",
  "necrolatry - The worship of the dead; manes worship. H. Spenser.",
  "brame - Sharp passion; vexation. [Obs.] Heart-burning brame. Spenser.",
  "tenotomy - The division of a tendon, or the act of dividing a tendon.",
  "squirm - To twist about briskly with contor",
  "moya - Mud poured out from volcanoes during eruptions; -- so called in South America.",
  "supersemination - The sowing of seed over seed previously sown. [Obs.] Abp. Bramhall.",
  "conventionally - In a conventional manner.",
  "lacert - A muscle of the human body. [Obs.] Chaucer.",
  "placentiferous - Having or producing a placenta.",
  "seventy-four - A naval vessel carrying seventy-four guns.",
  "bescumber - To discharge ordure or dung upon. [Obs.] B. Jonson.",
  "consubstantialism - The doctrine of consubstantiation.",
  "zyme - 1. A ferment. 2. (Med.)  The morbific principle of a zymotic disease. Quain.",
  "bathetic - Having the character of bathos. [R.]",
  "septemfluous - Flowing sevenfold; divided into seven streams or currents. [R.] Fuller.",
  "theorization - The act or product of theorizing; the formation of a theory or theories; speculation.",
  "mechlin - A kind of lace made at, or originating in, Mechlin, in Belgium.",
  "threnody - A song of lamentation; a threnode. Sir T. Herbert.",
  "sagebrush state - Nevada; -- a nickname.",
  "scratching - With the action of scratching.",
  "tileseed - Any plant of the genus Geissois, having seeds overlapping like tiles on a roof.",
  "tatch - A spot or stain; also, a trick. [Obs.] Sir T. Elyot.",
  "quint - 1. A set or sequence of five, as in piquet. 2. (Mus.)  The interval of a fifth.",
  "muxy - Soft; sticky, and dirty. [Prov. Eng.] See Mucky.",
  "introreception - The act of admitting into or within. Hammond.",
  "ferri- - A combining form indicating ferric iron as an ingredient; as, ferricyanide.",
  "hexapodous - Having six feet; belonging to the Hexapoda.",
  "socialize - 1. To render social. 2. To subject to, or regulate by, socialism.",
  "vively - In a lively manner. [Obs.] If I see a thing vively represented on the stage. B. Jonson.",
  "holily - 1. Piously; with sanctity; in a holy manner. 2. Sacredly; inviolably. [R.] Shak.",
  "swingebuckler - A swashbuckler; a bully; a roiserer. [Obs.] Shak.",
  "starchness - Of or pertaining to starched or starch; stiffness of manner; preciseness.",
  "iambize - To satirize in iambics; to lampoon. [R.]",
  "bryology - That part of botany which relates to mosses.",
  "punnet - A broad, shallow basket, for displaying fruit or flowers.",
  "hematinometer - A form of hemoglobinometer.",
  "statehouse - The building in which a State legislature holds its sessions; a State capitol. [U. S.]",
  "tewhit - The lapwing; -- called also teewheep. [Prov. Eng.]",
  "unrivet - To take out, or loose, the rivets of; as, to unrivet boiler plates.",
  "islamitic - Of or pertaining to Islam; Mohammedan.",
  "caliculate - Relating to, or resembling, a cup; also improperly used for calycular, calyculate.",
  "constitutively - In a constitutive manner.",
  "imbar - To bar in; to secure. [Obs.] To imbar their crooked titles. Shak.",
  "imperviability - The quality of being imperviable.",
  "resenter - One who resents. Sir H. Wotton.",
  "magnetotherapy - The treatment of disease by the application of magnets to the surface of the body.",
  "scabbard plane - See Scaleboard plane, under Scaleboard.",
  "kittlish - Ticklish; kittle. Sir W. Scott.",
  "inflationist - One who favors an increased or very large issue of paper money. [U.S.]",
  "kiosk - A Turkish open summer house or pavilion, supported by pillars.",
  "zax - A tool for trimming and puncturing roofing states. [Written also sax.]",
  "belate - To retard or make too late. Davenant.",
  "phthisipneumony - Pulmonary consumption.",
  "adversifoliate - Having opposite leaves, as plants which have the leaves so arranged on the stem.",
  "electrifiable - Capable of receiving electricity, or of being charged with it.",
  "vinosity - The quality or state of being vinous.",
  "beaucatcher - A small flat curl worn on the temple by women. [Humorous]",
  "argentiferous - Producing or containing silver; as, argentiferous lead ore or veins.",
  "anomy - Disregard or violation of law. [R.] Glanvill.",
  "counterirritant - See Counter irritant, etc., under Counter, a.",
  "carbonate - A salt or carbonic acid, as in limestone, some forms of lead ore, etc.",
  "plainness - The quality or state of being plain.",
  "uniserial - Having only one row or series.",
  "cherubical - Of or pertaining to cherubs; angelic. \"The cherubic host.\" Milton.",
  "near-legged - Having the feet so near together that they interfere in traveling. Shak.",
  "purgery - The part of a sugarhouse where the molasses is drained off from the sugar.",
  "bombazette - A sort of thin woolen cloth. It is of various colors, and may be plain or twilled.",
  "layship - The condition of being a layman. [Obs.] Milton.",
  "illegibility - The state or quality of being illegible.",
  "inestimably - In a manner, or to a degree, above estimation; as, things inestimably excellent.",
  "wastage - Loss by use, decay, evaporation, leakage, or the like; waste.",
  "disinclose - To free from being inclosed.",
  "water inch - Same as Inch of water, under Water.",
  "dairyman - A man who keeps or takes care of a dairy.",
  "wisly - Certainly. [Obs.] \"God so wisly have mercy on me.\" Chaucer.",
  "megaphyton - An extinct genus of tree ferns with large, two-ranked leaves, or fronds.",
  "counterclaim - A claim made by a person as an offset to a claim made on him.",
  "spermary - An organ in which spermatozoa are developed; a sperm gland; a testicle.",
  "anthologist - One who compiles an anthology.",
  "examinable - Capable of being examined or inquired into. Bacon.",
  "discruciate - To torture; to excruciate. [Obs.] Discruciate a man in deep distress. Herrick.",
  "gallicize - To conform to the French mode or idiom.",
  "emotioned - Affected with emotion. [R.] \"The emotioned soul.\" Sir W. Scott.",
  "bescummer - To discharge ordure or dung upon. [Obs.] B. Jonson.",
  "eddaic - Relating to the Eddas; resembling the Eddas.",
  "repellence - The principle of repulsion; the quality or capacity of repelling; repulsion.",
  "figulate - Made of potter's clay; molded; shaped. [R.] Johnson.",
  "bifold - Twofold; double; of two kinds, degrees, etc. Shak.",
  "gaddish - Disposed to gad. -- Gad\"dish*nes, n. \"Gaddishness and folly.\" Abp. Leighton.",
  "subprehensile - Somewhat prehensile; prehensile in an inferior degree.",
  "repaganize - To paganize anew; to bring back to paganism.",
  "healful - Tending or serving to heal; healing. [Obs.] Ecclus. xv. 3.",
  "makebate - One who excites contentions and quarrels. [Obs.]",
  "timocratic - Belonging to, or constituted by, timocracy. Sir G. C. Lewis.",
  "licheniform - Having the form of a lichen.",
  "designful - Full of design; scheming. [R.] -- De*sign\"ful*ness, n. [R.] Barrow.",
  "mandarinate - The collective body of officials or persons of rank in China. S. W. Williams.",
  "arabical - Relating to Arabia; Arabic. -- A*rab\"ic*al*ly, adv.",
  "lochia - The discharge from the womb and vagina which follows childbirth.",
  "reviction - Return to life. [Obs.] Sir T. Browne.",
  "ordonnant - Of or pertaining to ordonnance. Dryden.",
  "empanoplied - Completely armed; panoplied. Tennyson.",
  "doty - Half-rotten; as, doty timber. [Local, U. S.]",
  "distracted - Mentally disordered; unsettled; mad. My distracted mind. Pope.",
  "infecter - One who, or that which, infects.",
  "sinapisin - A substance extracted from mustard seed and probably identical with sinalbin. [Obs.]",
  "ingloriously - In an inglorious manner; dishonorably; with shame; ignominiously; obscurely.",
  "tintinnabulation - A tinkling sound, as of a bell or bells. Poe.",
  "shucker - One who shucks oysters or clams",
  "bel-esprit - A fine genius, or man of wit. \"A man of letters and a bel esprit.\" W. Irving.",
  "assaultable - Capable of being assaulted.",
  "contradistinct - Distinguished by opposite qualities. J. Goodwin.",
  "commoration - The act of staying or residing in a place. [Obs.] Bp. Hall.",
  "clientage - 1. State of being client. 2. A body of clients. E. Everett.",
  "corrugent - Drawing together; contracting; -- said of the corrugator. [Obs.]",
  "rajahship - The office or dignity of a rajah.",
  "whitmonday - The day following Whitsunday; -- called also Whitsun Monday.",
  "cleaner - One who, or that which, cleans.",
  "conversible - Capable of being converted or reversed. Hammond.",
  "perforator - One who, or that which, perforates; esp., a cephalotome.",
  "rankly - With rank or vigorous growth; luxuriantly; hence, coarsely; grossly; as, weeds grow rankly.",
  "emphysematous - Pertaining to, or of the nature of, emphysema; swelled; bloated.",
  "seacoast - The shore or border of the land adjacent to the sea or ocean. Also used adjectively.",
  "dvergr - A dwarf supposed to dwell in rocks and hills and to be skillful in working metals.",
  "adiposity - The state of being fat; fatness.",
  "perineoplasty - The act or process of restoring an injured perineum.",
  "exilic - Pertaining to exile or banishment, esp. to that of the Jews in Babylon. Encyc. Dict.",
  "jalapin - A glucoside found in the stems of the jalap plant and scammony. It is a strong purgative.",
  "confidently - With confidence; with strong assurance; positively.",
  "chasselas - A white grape, esteemed for the table.",
  "vying - a. & n. from Vie. -- Vy\"ing*ly, adv.",
  "hyperboliform - Having the form, or nearly the form, of an hyperbola.",
  "tenno - Lit., King of Heaven; -- a title of the emperor of Japan as the head of the Shinto religion.",
  "trilobitic - Of, pertaining to or containing, trilobites; as, trilobitic rocks.",
  "misgrowth - Bad growth; an unnatural or abnormal growth.",
  "slopeness - State of being slope. Sir H. Wotton.",
  "almightful - All-powerful; almighty. [Obs.] Udall.",
  "siriasis - (a) A sunstroke. (b) The act of exposing to a sun bath. [Obs.] Cf. Insolation.",
  "subjugator - One who subjugates; a conqueror.",
  "absentation - The act of absenting one's self. Sir W. Hamilton.",
  "cedarn - Of or pertaining to the cedar or its wood. [R.]",
  "dustman - One whose employment is to remove dirt and defuse. Gay.",
  "mytilus - A genus of marine bivalve shells, including the common mussel. See Illust. under Byssus.",
  "planiform - Having a plane surface; as, a planiform, gliding, or arthrodial articulation.",
  "organographist - One versed in organography.",
  "rulable - That may be ruled; subject to rule; accordant or conformable to rule. Bacon.",
  "fungite - A fossil coral resembling Fungia.",
  "dissociative - Tending or leading to dissociation.",
  "statable - That can be stated; as, a statablegrievance; the question at issue is statable.",
  "slabberer - One who slabbers, or drools; hence, an idiot.",
  "themis - The goddess of law and order; the patroness of existing rights.",
  "comportation - A bringing together. [Obs.] Bp. Richardson.",
  "oftensith - Frequently; often. [Obs.] For whom I sighed have so oftensith. Gascoigne.",
  "calamist - One who plays upon a reed or pipe. [Obs.] Blount.",
  "leiotrichous - Having smooth, or nearly smooth, hair.",
  "thundery - Accompanied with thunder; thunderous. [R.] \"Thundery weather.\" Pennant.",
  "sarcomatous - Of or pertaining to sarcoma; resembling sarcoma.",
  "rehibitory - Of or relating to rehibition; as, a rehibitory action.",
  "textualist - A textman; a textuary. Lightfoot.",
  "by-respect - Private end or view; by-interest. [Obs.] Dryden.",
  "commatism - Conciseness in writing. Bp. Horsley.",
  "anagrammatize - To transpose, as the letters of a word, so as to form an anagram. Cudworth.",
  "rescription - A writing back; the answering of a letter. Loveday.",
  "ancienty - 1. Age; antiquity. [Obs.] Martin. 2. Seniority. [Obs.]",
  "baculometry - Measurement of distance or altitude by a staff or staffs.",
  "dunner - One employed in soliciting the payment of debts.",
  "ploughboy - A boy that drives or guides a team in plowing; a young rustic.",
  "self-culture - Culture, training, or education of one's self by one's own efforts.",
  "questioner - One who asks questions; an inquirer. \"Little time for idle questioners.\" Tennyson.",
  "cockcrowing - The time at which cooks first crow; the early morning.",
  "negritic - Of or pertaining to negroes; composed of negroes. Keary.",
  "carmelin - Of or pertaining to the order of Carmelites.",
  "disinthrall - To free from thralldom; to disenthrall. [Written also disinthral.]",
  "unsubstantiation - A divesting of substantiality.",
  "pulverizer - One who, or that which, pulverizes.",
  "nous - Intellect; understanding; talent; -- used humorously.",
  "heptamerous - Consisting of seven parts, or having the parts in sets of sevens. Gray.",
  "exundation - An overflow, or overflowing abundance. [R.] Ray.",
  "astragalar - Of or pertaining to the astragalus.",
  "hyperchromatism - The condition of having an unusual intensity of color.",
  "isopogonous - Having the two webs equal in breath; -- said of feathers.",
  "galactopoietic - Increasing the flow of milk; milk-producing. -- n. A galactopoietic substance.",
  "brusher - One who, or that which, brushes.",
  "parosteal - Of or pertaining to parostosis; as, parosteal ossification.",
  "gith - The corn cockle; also anciently applied to the Nigella, or fennel flower.",
  "spinigerous - Bearing a spine or spines; thorn-bearing.",
  "obligee - The person to whom another is bound, or the person to whom a bond is given. Blackstone.",
  "saltatorious - Capable of leaping; formed for leaping; saltatory; as, a saltatorious insect or leg.",
  "perfunctorily - In a perfunctory manner; formally; carelessly. Boyle.",
  "baya - The East Indian weaver bird (Ploceus Philippinus).",
  "tronator - An officer in London whose duty was to weigh wool. [Obs.]",
  "practicalize - To render practical. [R.] \"Practicalizing influences.\" J. S. Mill.",
  "intercident - Falling or coming between; happening accidentally. [Obs.] Boyle.",
  "hemerobian - A neuropterous insect of the genus Hemerobius, and allied genera.",
  "tracheitis - Inflammation of the trachea, or windpipe.",
  "muslinet - A sort of coarse or light cotton cloth.",
  "impulsor - One who, or that which, impels; an inciter. [R.] Sir T. Browne.",
  "hoarsely - With a harsh, grating sound or voice.",
  "reboation - Repetition of a bellow. [R.] Bp. Patrick.",
  "world-wide - Extended throughout the world; as, world-wide fame. Tennyson.",
  "physicology - Physics. [R.] -- Phys`i*col\"o*gist, n. [R.]",
  "non assumpsit - The general plea or denial in an action of assumpsit.",
  "degustation - Tasting; the appreciation of sapid qualities by the taste organs. Bp. Hall.",
  "revengement - Revenge. [Obs.] He 'll breed revengement and a scourge for me. Shak."
]