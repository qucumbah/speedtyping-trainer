export default [
  "rooter - One who, or that which, roots; one that tears up by the roots.",
  "astyllen - A small dam to prevent free passage of water in an adit or level.",
  "periclitation - 1. Trial; experiment. [Obs.] 2. The state of being in peril. [Obs.]",
  "dejecter - One who casts down, or dejects.",
  "metantimonate - A salt of metantimonic acid.",
  "truculency - The quality or state of being truculent; savageness of manners; ferociousness.",
  "bidentate - Having two teeth or two toothlike processes; two-toothed.",
  "courtelle - a wool-like fabric. [WordNet 1.5]",
  "equipollently - With equal power. Barrow.",
  "bergylt - The Norway haddock. See Rosefish.",
  "partheniad - A poem in honor of a virgin. [Obs.]",
  "fleckless - Without spot or blame. [R.] My consnience will not count me fleckless. Tennyson.",
  "functionalize - To assign to some function or office. [R.]",
  "octopetalous - Having eight petals or flower leaves.",
  "papilliform - Shaped like a papilla; mammilliform.",
  "underbuy - To buy at less than the real value or worth; to buy cheaper than. [R.] J. Fletcher.",
  "ferricyanate - A salt of ferricyanic acid; a ferricyanide.",
  "appendaged - Furnished with, or supplemented by, an appendage.",
  "cepaceous - Of the nature of an onion, as in odor; alliaceous.",
  "dezincify - To deprive of, or free from, zinc.",
  "exemplarity - Exemplariness. [R.] The exemplarity of Christ's life. Abp. Sharp.",
  "court-craft - The artifices, intrigues, and plottings, at courts.",
  "assyriology - The science or study of the antiquities, language, etc., of ancient Assyria.",
  "arsenicism - A diseased condition produced by slow poisoning with arsenic.",
  "pediluvy - The bathing of the feet, a bath for the feet. [Obs.]",
  "sidehill - The side or slope of a hill; sloping ground; a descent. [U. S.]",
  "cyclo- - A combining form meaning circular, of a circle or wheel.",
  "unembarrassment - Freedom from embarrassment.",
  "nonvernacular - Not vernacular. A nonvernacular expression. Sir W. Hamilton.",
  "taeniafuge - A remedy to expel tapeworms.",
  "repristinate - To restore to an original state. [R.] Shedd.",
  "bosset - A rudimental antler of a young male of the red deer.",
  "gmelinite - A rhombohedral zeolitic mineral, related in form and composition to chabazite.",
  "sudd - A tangled mass of floating vegetal matter obstructing navigation. [Central Africa]",
  "emanatory - Emanative; of the nature of an emanation. Dr. H. More.",
  "scatteringly - In a scattering manner; dispersedly.",
  "birdcatcher - One whose employment it is to catch birds; a fowler.",
  "aurivorous - Gold-devouring. [R.] H. Walpole.",
  "vanward - Being on, or towards, the van, or front. \"The vanward frontier.\" De Quincey.",
  "intermetatarsal - Between the metatarsal bones.",
  "telepheme - A message by a telephone. [Recent]",
  "brasse - A spotted European fish of the genus Lucioperca, resembling a perch.",
  "abligurition - Prodigal expense for food. [Obs.] Bailey.",
  "apprecation - Earnest prayer; devout wish. [Obs.] A solemn apprecation of good success. Bp. Hall.",
  "effectuose - Effective. [Obs.] B. Jonson.",
  "cloisterer - One belonging to, or living in, a cloister; a recluse.",
  "individuator - One who, or that which, individuates. Sir K. Digby.",
  "horse-leechery - The business of a farrier; especially, the art of curing the diseases of horses.",
  "quadrireme - A galley with four banks of oars or rowers.",
  "ametabolian - Of or pertaining to insects that do undergo any metamorphosis.",
  "errorist - One who encourages and propagates error; one who holds to error.",
  "acetin - A combination of acetic acid with glycerin. Brande & C.",
  "expertly - In a skillful or dexterous manner; adroitly; with readiness and accuracy.",
  "naphthyl - A hydrocarbon radical regarded as the essential residue of naphthalene.",
  "azygous - Odd; having no fellow; not one of a pair; single; as, the azygous muscle of the uvula.",
  "funambulist - A ropewalker or ropedancer.",
  "crowbar - A bar of iron sharpened at one end, and used as a lever.",
  "jumpy - Jumping, or inducing to jump; characterized by jumps; hence, extremely nervous.",
  "geographically - In a geographical manner or method; according to geography.",
  "overtrow - To be too trustful or confident; to trust too much. [Obs.] Wyclif",
  "encrimson - To give a crimson or red color to; to crimson. Shak.",
  "anfractuose - Anfractuous; as, anfractuose anthers.",
  "calistheneum - A gymnasium; esp. one for light physical exercise by women and children.",
  "unvoweled - Having no vowel sounds or signs. [Written also unvowelled.] Skinner.",
  "fatiloquent - Prophetic; fatidical. [Obs.] Blount.",
  "pactolian - Pertaining to the Pactolus, a river in ancient Lydia famous for its golden sands.",
  "electro-kinetic - Of or pertaining to electro-kinetics.",
  "repletive - Tending to make replete; filling. -- Re*ple\"tive*ly, adv.",
  "erythrism - A condition of excessive redness. See Erythrochroism.",
  "preexistimation - Previous esteem or estimation. [Obs.] Sir T. Browne.",
  "sam - Together. [Obs.] \"All in that city sam.\" Spenser.",
  "skeelgoose - The common European sheldrake. [Prov. Eng.]",
  "navarchy - Nautical skill or experience. [Obs.] ir W. Petty.",
  "opalesce - To give forth a play of colors, like the opal.",
  "portension - The act of foreshowing; foreboding. [R.] Sir T. Browne.",
  "informity - Want of regular form; shapelessness. [Obs.]",
  "perditionable - Capable of being ruined; worthy of perdition. [R.] Pollok.",
  "masculinity - The state or quality of being masculine; masculineness.",
  "crouton - Bread cut in various forms, and fried lightly in butter or oil, to garnish hashes, etc.",
  "vomerine - Of or pertaining to the vomer.",
  "permitter - One who permits. A permitter, or not a hinderer, of sin. J. Edwards.",
  "comprecation - A praying together. [Obs.] Bp. Wilkins.",
  "consol - A consolidated annuity (see Consols); -- chiefly in combination or attributively.",
  "gripeful - Disposed to gripe; extortionate.",
  "australian - Of or pertaining to Australia. -- n.  A native or an inhabitant of Australia.",
  "ex officio - From office; by virtue, or as a consequence, of an office; officially.",
  "unlove - To cease to love; to hate. [Obs.]",
  "cucquean - A woman whose husband is unfaithful to her. [Obs.]",
  "bezonian - A low fellow or scoundrel; a beggar. Great men oft die by vile bezonians. Shak.",
  "defoliated - Deprived of leaves, as by their natural fall.",
  "improperly - In an improper manner; not properly; unsuitably; unbecomingly.",
  "north star state - Minnesota; -- a nickname.",
  "en - Half an em, that is, half of the unit of space in measuring printed matter. See Em.",
  "subincusation - A slight charge or accusation. [Obs.] Bp. Hall.",
  "confessant - One who confesses to a priest. [Obs.] Bacon.",
  "chartometer - An instrument for measuring charts or maps.",
  "prefine - To limit beforehand. [Obs.] Knolles.",
  "inspirational - Pertaining to inspiration.",
  "truttaceous - Of, pertaining to, or resembling, a trout; as, fish of the truttaceous kind.",
  "penult - The last syllable but one of a word; the syllable preceding the final one.",
  "reverseless - Irreversible. [R.] A. SEward.",
  "incompetently - In an competent manner; inadequately; unsuitably.",
  "concentricity - The state of being concentric.",
  "superorder - A group intermediate in importance between an order and a subclass.",
  "extravasate - To force or let out of the proper vessels or arteries, as blood.",
  "counterman - A man who attends at the counter of a shop to sell goods. [Eng.]",
  "raash - The electric catfish. [Written also raasch.]",
  "palmidactyles - A group of wading birds having the toes webbed, as the avocet.",
  "euterpean - Of or pertaining to Euterpe or to music.",
  "antiscriptural - Opposed to, or not in accordance with, the Holy Scriptures.",
  "pilotism - Pilotage; skill in the duties of a pilot. [R.]",
  "fig-shell - A marine univalve shell of the genus Pyrula, or Ficula, resembling a fig in form.",
  "calisthenic - Of or pertaining to calisthenics.",
  "snoff - A short candle end used for igniting a fuse. Raymond.",
  "amenableness - The quality or state of being amenable; liability to answer charges; answerableness.",
  "succinamate - A salt of succinamic acid.",
  "laryngograph - An instrument for recording the larynx movements in speech.",
  "mechanize - To cause to be mechanical. Shelley.",
  "surrebut - To reply, as a plaintiff to a defendant's rebutter.",
  "chickabiddy - A chicken; a fowl; also, a trivial term of endearment for a child.",
  "interdigitation - The state of interdigitating; interdigital space. Owen.",
  "doop - A little copper cup in which a diamond is held while being cut.",
  "naufragous - causing shipwreck. [Obs.] r. Taylor.",
  "aurist - One skilled in treating and curing disorders of the ear.",
  "pieceless - Not made of pieces; whole; entire.",
  "volumenometry - The method or process of measuring volumes by means of the volumenometer.",
  "preambulous - See Perambulatory. [R.] Sir T. Browne.",
  "discommender - One who discommends; a dispraiser. Johnson.",
  "repulser - One who repulses, or drives back.",
  "obtunder - That which obtunds or blunts; especially, that which blunts sensibility.",
  "incentively - Incitingly; encouragingly.",
  "sauterelle - An instrument used by masons and others to trace and form angles.",
  "rabbinically - In a rabbinical manner; after the manner of the rabbins.",
  "chaldaism - An idiom or peculiarity in the Chaldee dialect.",
  "stratigraphy - That branch of geology which treats of the arrangement and succession of strata.",
  "scabredity - Roughness; ruggedness. [Obs.] Burton.",
  "pictograph - A picture or hieroglyph representing and expressing an idea. -- Pic`to*graph\"ic, a.",
  "preestablishment - Settlement beforehand.",
  "collocutor - One of the speakers in a dialogue. Derham.",
  "marechal niel - A kind of large yellow rose. [Written also Marshal Niel.]",
  "water god - A fabulous deity supposed to dwell in, and preside over, some body of water.",
  "photo- - A combining form from Gr. fw^s, fwto`s, light; as, photography, phototype, photometer.",
  "bondager - A field worker, esp. a woman who works in the field. [Scot.]",
  "orderliness - The state or quality of being orderly.",
  "sarsa - Sarsaparilla. [Written also sarza.]",
  "counterfesance - The act of forging; forgery. [Obs.] [Written also counterfaisance.]",
  "sempiternity - Future duration without end; the relation or state of being sempiternal. Sir M. Hale.",
  "enclitically - In an enclitic manner; by throwing the accent back. Walker.",
  "tintamar - A hideous or confused noise; an uproar. [Obs.] Howell.",
  "alary - Of or pertaining to wings; also, wing-shaped. The alary system of insects. Wollaston.",
  "claritude - Clearness; splendor. [Obs.] Beau. & Fl.",
  "plasmator - A former; a fashioner. [R.] \"The sovereign plasmator, God Almighty.\" Urquhart.",
  "suffumige - A medical fume. [Obs.] Harvey.",
  "fulham - A false die. [Cant] [Written also fullam.] Shak.",
  "martite - Iron sesquioxide in isometric form, probably a pseudomorph after magnetite.",
  "paeonine - An artifical red nitrogenous dyestuff, called also red coralline.",
  "boviform - Resembling an ox in form; ox-shaped. [R.]",
  "frequently - At frequent or short intervals; many times; often; repeatedly; commonly.",
  "woodcracker - The nuthatch. [Prov. Eng.]",
  "heptastich - A composition consisting of seven lines or verses.",
  "sovereignize - To exercise supreme authority. [Obs.] Sir T. Herbert.",
  "everich - each one; every one; each of two. See Every. [Obs.] Chaucer.",
  "tentaculate - Having tentacles, or organs like tentacles; tentacled.",
  "triploidite - A manganese phosphate near triplite, but containing hydroxyl instead of fluorine.",
  "dentate-ciliate - Having the margin dentate and also ciliate or fringed with hairs.",
  "semitangent - The tangent of half an arc.",
  "high-hearted - Full of courage or nobleness; high-souled. -- High\"-heart`ed*ness, n.",
  "diastema - A vacant space, or gap, esp. between teeth in a jaw.",
  "ebrillade - A bridle check; a jerk of one rein, given to a horse when he refuses to turn.",
  "polyphyllous - Many-leaved; as, a polyphyllous calyx or perianth.",
  "roc - A monstrous bird of Arabian mythology. [Written also rock, and rukh.] Brande & C.",
  "hemipteran - One of the Hemiptera; an hemipter.",
  "backer - One who, or that which, backs; especially one who backs a person or thing in a contest.",
  "unsophisticated - Not sophisticated; pure; innocent; genuine. -- Un`so*phis\"ti*ca`ted*ness, n.",
  "avant-guard - The van or advanced body of an army. See Vanguard.",
  "apothecium - The ascigerous fructification of lichens, forming masses of various shapes.",
  "caesious - Of the color of lavender; pale blue with a slight mixture of gray. Lindley.",
  "heroicomic - Combining the heroic and the ludicrous; denoting high burlesque; as, a heroicomic poem.",
  "crinitory - Of or relating to hair; as, a crinitory covering. T. Hook.",
  "aspermous - Destitute of seeds; aspermatous.",
  "aristulate - Pertaining a short beard or awn. Gray.",
  "targeted - Furnished, armed, or protected, with a target.",
  "chepster - The European starling. [Local, Eng.]",
  "corruptful - Tending to corrupt; full of corruption. [Obs.] \"Corruptful bribes.\" Spenser.",
  "sea scurf - Any bryozoan which forms rounded or irregular patches of coral on stones, seaweeds, etc.",
  "imperforated - Not perforated; having no opening or aperture. Sir J. Banks.",
  "fa - (a) A syllable applied to the fourth tone of the diatonic scale in solmization. (b) The tone F.",
  "paranthracene - An inert isomeric modification of anthracene.",
  "hogged - Broken or strained so as to have an upward curve between the ends. See Hog, v. i.",
  "dispoline - One of several isomeric organic bases of the quinoline series of alkaloids.",
  "seamed - Out of condition; not in good condition; -- said of a hawk.",
  "hypophyllous - Being or growing on the under side of a leaf, as the fruit dots of ferns.",
  "moroccan - Of or pertaining to Morocco, or its inhabitants.",
  "minglement - The act of mingling, or the state of being mixed.",
  "contemplatist - A contemplator. [R.] I. Taylor.",
  "invincibility - The quality or state of being invincible; invincibleness.",
  "pink stern - See Chebacco, and 1st Pink.",
  "peroxidize - To oxidize to the utmost degree, so as to form a peroxide.",
  "astrophotography - The application of photography to the delineation of the sun, moon, and stars.",
  "ingenerabillty - Incapacity of being engendered or produced. Cudworth.",
  "feateous - Dexterous; neat. [Obs.] Johnson. -- Feat\"e*ous*ly, adv.",
  "heartsome - Merry; cheerful; lively. [Scot.]",
  "emendator - One who emends or critically edits.",
  "swankie - An active and clever young fellow. [Scot.] Sir W. Scott.",
  "trullization - The act of laying on coats of plaster with a trowel.",
  "girlhood - State or time of being a girl.",
  "nazaritic - Of or pertaining to a Nazarite, or to Nazarites.",
  "anthoid - Resembling a flower; flowerlike.",
  "egyptize - To give an Egyptian character or appearance to. Fairbairn.",
  "xyloplastic - Formed of wood pulp by molds; relating to casts made of wood pulp in molds.",
  "tineman - An officer of the forest who had the care of vert and venison by night. [Obs.]",
  "adventual - Relating to the season of advent. Sanderson.",
  "irregularity - The state or quality of being irregular; that which is irregular.",
  "operatic - Of or pertaining to the opera or to operas; characteristic of, or resembling, the opera.",
  "panslavism - A scheme or desire to unite all the Slavic races into one confederacy.",
  "determinableness - Capability of being determined; determinability.",
  "devitrify - To deprive of glasslike character; to take away vitreous luster and transparency from.",
  "clotbur - 1. The burdock. [Prov. Engl.] Prior. 2. Same as Cocklebur.",
  "steamy - Consisting of, or resembling, steam; full of steam; vaporous; misty. Cowper.",
  "volcanicity - Quality or state of being volcanic; volcanic power.",
  "crookedly - In a curved or crooked manner; in a perverse or untoward manner.",
  "homotonous - Of the same tenor or tone; equable; without variation.",
  "examplary - Serving for example or pattern; exemplary. [Obs.] Hooker.",
  "lipothymy - A fainting; a swoon. Jer. Taylor.",
  "regrade - To retire; to go back. [Obs.] W. Hales.",
  "appealing - That appeals; imploring. -- Ap*peal\"*ing*ly, adv. -- Ap*peal\"ing*ness, n.",
  "contraplex - Pertaining to the sending of two messages in opposite directions at the same time.",
  "quintroon - The off-spring of an octoroon and a white person.",
  "plagueful - Abounding, or infecting, with plagues; pestilential; as, plagueful exhalations.",
  "gerundial - Pertaining to, or resembling, a gerund; as, a gerundial use.",
  "teamwork - Work done by a team, as distinguished from that done by personal labor.",
  "smock frock - A coarse frock, or shirt, worn over the other dress, as by farm laborers. Macaulay.",
  "osteoperiostitis - Inflammation of a bone and its periosteum.",
  "diluter - One who, or that which, dilutes or makes thin, more liquid, or weaker.",
  "contemptibly - In a contemptible manner.",
  "expunction - The act of expunging or erasing; the condition of being expunged. Milton.",
  "octangular - Having eight angles; eight-angled. -- Oc*tan\"gu*lar*ness, n.",
  "ambiloquy - Doubtful or ambiguous language. [Obs.] Bailey.",
  "basilicok - The basilisk. [Obs.] Chaucer",
  "turdus - A genus of singing birds including the true thrushes.",
  "captor - One who captures any person or thing, as a prisoner or a prize.",
  "unsociability - The quality or state of being unsociable; unsociableness.",
  "night-faring - Going or traveling in the night. Gay.",
  "sacrificant - One who offers a sacrifice. [R.]",
  "curvilinearly - In a curvilinear manner.",
  "disenchanter - One who, or that which, disenchants.",
  "naso- - A combining form denoting pertaining to, or connected with, the nose; as, nasofrontal.",
  "upholstery - The articles or goods supplied by upholsterers; the business or work of an upholsterer.",
  "maxillo-mandibular - Pertaining to the maxilla and mandible; as, the maxillo- mandibular nerve.",
  "gravigrade - Slow-paced. -- n.  One of the pachyderms.",
  "sleepwaking - The state of one mesmerized, or in a partial and morbid sleep.",
  "decarburization - The act, process, or result of decarburizing.",
  "compellative - The name by which a person is addressed; an appellative.",
  "commutual - Mutual; reciprocal; united. [R.] There, with commutual zeal, we both had strove. Pope.",
  "infinitesimally - By infinitesimals; in infinitely small quantities; in an infinitesimal degree.",
  "boutonniere - A bouquet worn in a buttonhole.",
  "fitweed - A plant (Eryngium foetidum) supposed to be a remedy for fits.",
  "kand - Fluor spar; -- so called by Cornish miners.",
  "blunge - To amalgamate and blend; to beat up or mix in water, as clay.",
  "gardenless - Destitute of a garden. Shelley.",
  "gemminess - The state or quality of being gemmy; spruceness; smartness.",
  "philoprogenitiveness - The love of offspring; fondness for children.",
  "hemicardia - A lateral half of the heart, either the right or left. B. G. Wilder.",
  "pleurobranch - Any one of the gills of a crustacean that is attached to the side of the thorax.",
  "boustrophedonic - Relating to the boustrophedon mode of writing.",
  "intervisible - Mutually visible, or in sight, the one from the other, as stations.",
  "furrier - A dealer in furs; one who makes or sells fur goods.",
  "supertuberation - The production of young tubers, as potatoes, from the old while still growing.",
  "wacke - A soft, earthy, dark-colored rock or clay derived from the alteration of basalt.",
  "oaky - Resembling oak; strong. Bp. Hall.",
  "unbelt - To remove or loose the belt of; to ungird.",
  "gaspereau - The alewife. [Local, Canada]",
  "cockmaster - One who breeds gamecocks. L'Estrange.",
  "ricinoleate - A salt of ricinoleic acid; -- formerly called palmate.",
  "y current - The current through one branch of the star arrangement of a three-phase circuit.",
  "glacious - Pertaining to, consisting of or resembling, ice; icy. Sir T. Browne.",
  "hydrina - The group of hydroids to which the fresh-water hydras belong.",
  "explicableness - Quality of being explicable.",
  "despiciency - A looking down; despection. [Obs.]",
  "depulsion - A driving or thrusting away. [R.] Speed.",
  "splashy - Full of dirty water; wet and muddy, so as be easily splashed about; slushy.",
  "babyism - 1. The state of being a baby. 2. A babyish manner of acting or speaking.",
  "attiguous - Touching; bordering; contiguous. [Obs.] -- At*tig\"u*ous*ness, n. [Obs.]",
  "parishioner - One who belongs to, or is connected with, a parish.",
  "ilicin - The bitter principle of the holly.",
  "epiphysial - Pertaining to, or having the nature of, an epiphysis.",
  "vantbrace - Armor for the arm; vambrace. Milton.",
  "berthierite - A double sulphide of antimony and iron, of a dark steel-gray color.",
  "humstrum - An instrument out of tune or rudely constructed; music badly played.",
  "senary - Of six; belonging to six; containing six. Dr. H. More.",
  "gaekwar - The title of the ruling Prince of Baroda, in Gujarat, in Bombay, India.",
  "poutingly - In a pouting, or a sullen, manner.",
  "immensible - Immeasurable. [Obs.] Davies.",
  "life-size - Of full size; of the natural size.",
  "distinctiveness - State of being distinctive.",
  "kilovolt - A unit of electromotive force equal to one thousand volts.",
  "decursion - A flowing; also, a hostile incursion. [Obs.] Sir M. Hale.",
  "leonese - Of or pertaining to Leon, in Spain. -- n. sing. & pl.  A native or natives of Leon.",
  "herdsman - The owner or keeper of a herd or of herds; one employed in tending a herd of cattle.",
  "thenardite - Anhydrous sodium sulphate, a mineral of a white or brown color and vitreous luster.",
  "enchafing - Heating; burning. [Obs.] The wicked enchaufing or ardure of this sin [lust]. Chaucer.",
  "canulated - See Cannula, Cannular, and Cannulated.",
  "lenitude - The quality or habit of being lenient; lenity. [Obs.] Blount.",
  "monogastric - Having but a single stomach.",
  "undershrieve - A low shrub; a woody plant of low stature.",
  "proller - Prowler; thief. [Obs.] Chapman.",
  "phosphaturia - The excessive discharge of phosphates in the urine.",
  "concussation - A violent shock or agitation. [Obs.] Bp. Hall.",
  "chimere - The upper robe worn by a bishop, to which lawn sleeves are usually attached. Hook.",
  "farmhouse - A dwelling house on a farm; a farmer's residence.",
  "indiligence - Want of diligence. [Obs.] B. Jonson.",
  "shipful - As much or as many as a ship will hold; enough to fill a ship.",
  "tricycle - A three-wheeled velocipede. See Illust. under Velocipede. Cf. Bicycle.",
  "paragrandine - An instrument to avert the occurrence of hailstorms. See Paragr. Knight.",
  "argot - A secret language or conventional slang peculiar to thieves, tramps, and vagabonds; flash.",
  "cribber - A horse that has the habit of cribbing.",
  "packfong - A Chinese alloy of nickel, zinc, and copper, resembling German silver.",
  "true-born - Of genuine birth; having a right by birth to any title; as, a true-born Englishman.",
  "desmology - The science which treats of the ligaments. [R.]",
  "jolly-boat - A boat of medium size belonging to a ship.",
  "oxbane - A poisonous bulbous plant (Buphane toxicaria) of the Cape of Good Hope.",
  "churchship - State of being a church. South.",
  "tetrapnuemonian - One of the Tetrapneumona.",
  "obiism - Belief in, or the practice of, the obi superstitions and rites.",
  "ovariole - One of the tubes of which the ovaries of most insects are composed.",
  "transferability - The quality or state of being transferable.",
  "awesomeness - The quality of being awesome.",
  "whitecoat - The skin of a newborn seal; also, the seal itself. [Sealers' Cant]",
  "confract - Broken in pieces; severed. [Obs.]",
  "gabardine - A coarse frock or loose upper garment formerly worn by Jews; a mean dress. Shak.",
  "disputeless - Admitting no dispute; incontrovertible. Bailey.",
  "soe - A large wooden vessel for holding water; a cowl. [Obs. or Prov. Eng.] Dr. H. More.",
  "half-hatched - Imperfectly hatched; as, half-hatched eggs. Gay.",
  "fluviatic - Belonging to rivers or streams; fluviatile. Johnson.",
  "languageless - Lacking or wanting language; speechless; silent. Shak.",
  "electro-chronographic - Belonging to the electro-chronograph, or recorded by the aid of it.",
  "lifesome - Animated; sprightly. [Poetic] Coleridge. -- Life\"some*ness, n.",
  "unguicular - Of or pertaining to a claw or a nail; ungual.",
  "half-yearly - Two in a year; semiannual. -- adv. Twice in a year; semiannually.",
  "prestigiation - Legerdemain; prestidigitation. [Obs.]",
  "tobogganist - One who practices tobogganing.",
  "footband - A band of foot soldiers. [Obs.]",
  "taintless - Free from taint or infection; pure.",
  "ironish - Resembling iron, as in taste. Wood.",
  "megaderm - Any one of several species of Old World blood-sucking bats of the genus Megaderma.",
  "entomotomy - The science of the dissection of insects.",
  "fibered - Having fibers; made up of fibers.",
  "poachiness - The state of being poachy; marshiness.",
  "preconscious - Of or pertaining to a state before consciousness.",
  "theopathetic - Of or pertaining to a theopathy.",
  "dextrously - Same as Dexterous, Dexterously, etc.",
  "battle-axe - A kind of broadax formerly used as an offensive weapon.",
  "dissite - Lying apart. [Obs.] Lands far dissite and remote asunder. Holland.",
  "disemboguement - The act of disemboguing; discharge. Mease.",
  "telarly - In a weblike manner. [Obs.] \"Telarly interwoven.\" Sir T. Browne.",
  "neogaean - Of or pertaining to the New World, or Western Hemisphere.",
  "rusticly - In a rustic manner; rustically. Chapman.",
  "cardiosphygmograph - A combination of cardiograph and shygmograph.",
  "fruticulose - Like, or pertaining to, a small shrub. Gray.",
  "surrenderor - One who makes a surrender, as of an estate. Bouvier.",
  "nathless - Nevertheless. [Archaic] Chaucer. Milton. E. Arnold.",
  "shebander - A harbor master, or ruler of a port, in the East Indies. [Written also shebunder.]",
  "oleiferous - Producing oil; as, oleiferous seeds.",
  "septulate - Having imperfect or spurious septa.",
  "honeysuckled - Covered with honeysuckles.",
  "intitule - To entitle; to give a title to. Selden.",
  "rizzar - To dry in the sun; as, rizzared haddock. [Scot.]",
  "anopla - One of the two orders of Nemerteans. See Nemertina.",
  "kibblings - Portions of small fish used for bait on the banks of Newfoundland.",
  "neckband - A band which goes around the neck; often, the part at the top of a garment.",
  "cross-crosslet - A cross having the three upper ends crossed, so as to from three small crosses.",
  "kingling - Same as Kinglet, 1. Churchill.",
  "meatotome - An instrument for cutting into the urethra so as to enlarge its orifice.",
  "vesselful - As much as a vessel will hold; enough to fill a vessel.",
  "unshutter - To open or remove the shutters of. T. Hughes.",
  "manstealer - A person who steals or kidnaps a human being or beings.",
  "urtica - A genus of plants including the common nettles. See Nettle, n.",
  "consecutively - In a consecutive manner; by way of sequence; successively.",
  "enrheum - To contract a rheum. [Obs.] Harvey.",
  "candroy - A machine for spreading out cotton cloths to prepare them for printing.",
  "bifurcous - See Bifurcate, a. [R.] Coles.",
  "anaglyptic - Relating to the art of carving, enchasing, or embossing in low relief.",
  "seeling - The rolling or agitation of a ship in a sterm. [Obs.] Sandys.",
  "theorize - To form a theory or theories; to form opinions solely by theory; to speculate.",
  "unmothered - Deprived of a mother; motherless.",
  "sarmentaceous - Bearing sarments, or runners, as the strawberry.",
  "deviltry - Diabolical conduct; malignant mischief; devilry. C. Reade.",
  "floriform - Having the form of a flower; flower-shaped.",
  "rabidness - The quality or state of being rabid.",
  "muscardin - The common European dormouse; -- so named from its odor. [Written also muscadine.]",
  "vaporiform - Existing in a vaporous form or state; as, steam is a vaporiform substance.",
  "cornic - Pertaining to, derived from, or resembling, the dogwood (Cornus florida).",
  "incurve - To bend; to curve; to make crooked.",
  "condignness - Agreeableness to deserts; suitableness.",
  "handygripe - Seizure by, or grasp of, the hand; also, close quarters in fighting. Hudibras.",
  "metropolitanate - The see of a metropolitan bishop. Milman.",
  "anew - Over again; another time; in a new form; afresh; as, to arm anew; to create anew. Dryden.",
  "alacriousness - Alacrity. [Obs.] Hammond.",
  "atavic - Pertaining to a remote ancestor, or to atavism.",
  "momentany - Momentary. [Obs.] Hooker. \"Momentany as a sound.\" Shak.",
  "ribes - A genus of shrubs including gooseberries and currants of many kinds.",
  "semiopal - A variety of opal not possessing opalescence.",
  "demonocracy - The power or government of demons. A demonocracy of unclean spirits. H. Taylor.",
  "ampersand - A word used to describe the character Halliwell.",
  "pronotum - The dorsal plate of the prothorax in insects. See Illust. of Coleoptera.",
  "preopinion - Opinion previously formed; prepossession; prejudice. Sir T. Browne.",
  "premunite - To fortify beforehand; to guard against objection. [Obs.] Fotherby.",
  "coinquinate - Topollute. [Obs.] Skelton.",
  "winningness - The quality or state of being winning. \"Winningness in style.\" J. Morley.",
  "cartwright - An artificer who makes carts; a cart maker.",
  "prognathi - A comprehensive group of mankind, including those that have prognathous jaws.",
  "exsuccous - Destitute of juice; dry; sapless. Latham.",
  "impracticableness - The state or quality of being impracticable; impracticability.",
  "stanniferous - Containing or affording tin.",
  "tephroite - A silicate of manganese of an ash-gray color.",
  "dentiferous - Bearing teeth; dentigerous.",
  "meliority - The state or quality of being better; melioration. [Obs.] Bacon.",
  "compesce - To hold in check; to restrain. [R.] Carlyle.",
  "self-imposture - Imposture practiced on one's self; self-deceit. South.",
  "tragus - The prominence in front of the external opening of the ear. See Illust. under Ear.",
  "provenient - Forthcoming; issuing. [Rare]",
  "plenitudinary - Having plenitude; full; complete; thorough. [Obs.]",
  "powderhorn - A horn in which gunpowder is carried.",
  "climatarchic - Presiding over, or regulating, climates.",
  "titrated - Standardized; determined or analyzed by titration; as, titrated solutions.",
  "enswathement - The act of enswathing, or the state of being enswathed.",
  "informative - Having power to inform, animate, or vivify. Dr. H. More.",
  "crystallographically - In the manner of crystallography.",
  "aquamarine - A transparent, pale green variety of beryl, used as a gem. See Beryl.",
  "bruckeled - Wet and dirty; begrimed. [Obs. or Dial.] Herrick.",
  "dispatchment - The act of dispatching. [Obs.] State Trials (1529).",
  "hierarchical - Pertaining to a hierarchy. -- Hi`er*arch`ic*al*ly, adv.",
  "iniquitously - In an iniquitous manner; unjustly; wickedly.",
  "organdy - A kind of transparent light muslin.",
  "abrogation - The act of abrogating; repeal by authority. Hume.",
  "malax - To soften by kneading or stirring with some thinner substance. [R.]",
  "brantail - The European redstart; -- so called from the red color of its tail.",
  "sexto - A book consisting of sheets each of which is folded into six leaves.",
  "betrustment - The act of intrusting, or the thing intrusted. [Obs.] Chipman.",
  "duan - A division of a poem corresponding to a canto; a poem or song. [R.]",
  "tripsis - (a) Trituration. [R.] (b) Shampoo. [R.]",
  "winker - 1. One who winks. Pope. 2. A horse's blinder; a blinker.",
  "dependingly - As having dependence. Hale.",
  "kidnapper - One who steals or forcibly carries away a human being; a manstealer.",
  "auto-inoculation - Inoculation of a person with virus from his own body.",
  "sogginess - The quality or state of being soggy; soddenness; wetness.",
  "boisterousness - The state or quality of being boisterous; turbulence; disorder; tumultuousness.",
  "fabler - A writer of fables; a fabulist; a dealer in untruths or falsehoods. Br. Hall.",
  "augmenter - One who, or that which, augments or increases anything.",
  "trais - Traces. [Obs.] Four white bulls in the trays. Chaucer.",
  "centerbit - An instrument turning on a center, for boring holes. See Bit, n., 3.",
  "forementioned - Mentioned before; already cited; aforementioned. Addison.",
  "demijohn - A glass vessel or bottle with a large body and small neck, inclosed in wickerwork.",
  "evagation - A wandering about; excursion; a roving. [R.] Ray.",
  "inconsecutiveness - The state or quality of not being consecutive. J. H. Newman.",
  "chymistry - See Chemic, Chemist, Chemistry.",
  "pommelion - The cascabel, or hindmost knob, of a cannon. [R.]",
  "parcel-mele - By parcels or parts. [Obs.] Chaucer.",
  "annueler - A priest employed in saying annuals, or anniversary Masses. [Obs.] Chaucer.",
  "humbler - One who, or that which, humbles some one.",
  "keverchief - A kerchief. [Obs.] Chaucer.",
  "forespeaking - A prediction; also, a preface. [Obs.] Camden. Huloet.",
  "auscultator - One who practices auscultation.",
  "nonextensile - Not extensile; incapable of being stretched.",
  "nim - To take; to steal; to filch. [Obs.] This canon it in his hand nam. Chaucer.",
  "adiposeness - The state of being fat; fatness.",
  "gnoscopine - An alkaloid existing in small quantities in opium.",
  "grammates - Rudiments; first principles, as of grammar. [Obs.] Ford.",
  "corbe - Crooked. [Obs.] \"Corbe shoulder.\" Spenser.",
  "looplight - A small narrow opening or window in a tower or fortified wall; a loophole.",
  "circulative - Promoting circulation; circulating. [R.] Coleridge.",
  "stuffer - One who, or that which, stuffs.",
  "thring - To press, crowd, or throng. [Obs.] Chaucer.",
  "infraocular - Situated below the eyes, as the antenna of certain insects.",
  "detractiveness - The quality of being detractive.",
  "frubish - To rub up: to furbish. [Obs.] Beau. c& Et.",
  "third-borough - An under constable. Shak. Johnson.",
  "dispiteous - Full of despite; cruel; spiteful; pitiless. Spenser. -- Dis*pit\"e*ous*ly, adv. [Obs.]",
  "oarfoot - Any crustacean of the genus Remipes.",
  "hebdomadal - Consisting of seven days, or occurring at intervals of seven days; weekly.",
  "lethiferous - Deadly; bringing death or destruction.",
  "hyalea - A pteroid of the genus Cavolina. See Pteropoda, and Illustration in Appendix.",
  "decoration day - = Memorial Day. [U. S.]",
  "inculpatory - Imputing blame; criminatory; compromising; implicating.",
  "semita - A fasciole of a spatangoid sea urchin.",
  "sorn - To obtrude one's self on another for bed and board. [Scot.] Sir W. Scott.",
  "mattamore - A subterranean repository for wheat.",
  "sea captain - The captain of a vessel that sails upon the sea.",
  "compassionately - In a compassionate manner; mercifully. Clarendon.",
  "glossographer - A writer of a glossary; a commentator; a scholiast. Hayward.",
  "bloodthirsty - Eager to shed blood; cruel; sanguinary; murderous. -- Blood\"thirst`i*ness (n.",
  "haar - A fog; esp., a fog or mist with a chill wind. [Scot.] T. Chalmers.",
  "hawser-laid - Made in the manner of a hawser. Cf. Cable-laid, and see Illust. of Cordage.",
  "rictus - The gape of the mouth, as of birds; -- often resricted to the corners of the mouth.",
  "articulary - A bone in the base of the lower jaw of many birds, reptiles, amphibians, and fishes.",
  "furilic - Pertaining to, or derived from, furile; as, furilic acid.",
  "epauleted - Wearing epaulets; decorated with epaulets.",
  "physostomous - (a) Having a duct to the air bladder. (b) Pertaining to the Physostomi.",
  "quob - To throb; to quiver. [Local & Vulgar]",
  "septentrio - The constellation Ursa Major.",
  "scorching - Burning; parching or shriveling with heat. -- Scorch\"ing*ly, adv. -- Scorch\"ing*ness, n.",
  "benightment - The condition of being benighted.",
  "stanchless - 1. Incapable of being stanched, or stopped. 2. Unquenchable; insatiable. [Obs.] Shak.",
  "funis - A cord; specifically, the umbilical cord or navel string.",
  "collutory - A medicated wash for the mouth.",
  "chabuk - A long whip, such as is used in the East in the infliction of punishment. Balfour.",
  "marinade - A brine or pickle containing wine and spices, for enriching the flavor of meat and fish.",
  "waid - Oppressed with weight; crushed; weighed down. [Obs.] Tusser.",
  "mesovarium - The fold of peritoneum connecting the ovary with the wall of the abdominal cavity.",
  "tobine - A stout twilled silk used for dresses.",
  "hoonoomaun - An Indian monkey. See Entellus. [Written also hoonuman.]",
  "tomboy - A romping girl; a hoiden. [Colloq.] J. Fletcher.",
  "circumnavigable - Capable of being sailed round. Ray.",
  "hexangular - Having six angles or corners.",
  "pyroarsenic - Pertaining to or designating, an acid of arsenic analogous to pyrophosphoric acid.",
  "necrophagous - Of or pertaining to the Necrophaga; eating carrion. See Necrophagan.",
  "unshut - To open, or throw open. [Obs.] Chaucer.",
  "hebraistic - Pertaining to, or resembling, the Hebrew language or idiom.",
  "deducibleness - The quality of being deducible; deducibility.",
  "fangless - Destitute of fangs or tusks. \"A fangless lion.\" Shak.",
  "subereous - Of or pertaining to cork; of the nature of cork; suberose.",
  "mismeasure - To measure or estimate incorrectly.",
  "schediasm - Cursory writing on a loose sheet. [R.]",
  "storge - Parental affection; the instinctive affection which animals have for their young.",
  "convincingly - in a convincing manner; in a manner to compel assent.",
  "provedore - A proveditor; a purveyor. Busied with the duties of a provedore. W. Irving.",
  "qualifiedness - The state of being qualified.",
  "palaeographer - See Paleographer, Paleographic, etc.",
  "zoanthodeme - The zooids of a compound anthozoan, collectively.",
  "horsehide - 1. The hide of a horse. 2. Leather made of the hide of a horse.",
  "beneme - To deprive (of), or take away (from). [Obs.]",
  "fenianism - The principles, purposes, and methods of the Fenians.",
  "ysame - Together. [Obs.] \"And in a bag all sorts of seeds ysame.\" Spenser.",
  "terre-tenant - One who has the actual possession of land; the occupant. [Written also ter-tenant.]",
  "missis - A mistress; a wife; -- so used by the illiterate. G. Eliot.",
  "entozoic - Pertaining to, or consisting of, the Entozoa.",
  "degenerative - Undergoing or producing degeneration; tending to degenerate.",
  "blastematic - Connected with, or proceeding from, the blastema; blastemal.",
  "perjurous - Guilty of perjury; containing perjury. [Obs.] Quarles. B. Johnson.",
  "intermutual - Mutual. [Obs.] Daniel. -- In`ter*mu\"tu*al*ly, adv. [Obs.]",
  "experimentalize - To make experiments (upon); to experiment. J. S. Mill.",
  "theopneusted - Divinely inspired; theopneustic. [R.]",
  "echinodermal - Relating or belonging to the echinoderms.",
  "antihypochondriac - Counteractive of hypochondria. -- n.  A remedy for hypochondria.",
  "lieutenantship - Same as Lieutenancy, 1.",
  "pulveraceous - Having a finely powdered surface; pulverulent.",
  "misremember - To mistake in remembering; not to remember correctly. Sir T. More.",
  "testamentation - The act or power of giving by testament, or will. [R.] Burke.",
  "appointive - Subject to appointment; as, an appointive office. [R.]",
  "engirdle - To surround as with a girdle; to girdle.",
  "deoperculate - Having the lid removed; -- said of the capsules of mosses.",
  "saxony yarn - A fine grade of woolen yarn twisted somewhat harder and smoother than zephyr yarn.",
  "fumosity - The fumes of drink. [Obs.] Chaucer.",
  "plathelminth - One of the Platyelminthes.",
  "cineritious - Like ashes; having the color of ashes, -- as the cortical substance of the brain.",
  "campanero - The bellbird of South America. See Bellbird.",
  "sulphonal - A substance employed as a hypnotic, produced by the union of mercaptan and acetone.",
  "slickness - The state or quality of being slick; smoothness; sleekness.",
  "chromatics - The science of colors; that part of optics which treats of the properties of colors.",
  "daintily - In a dainty manner; nicely; scrupulously; fastidiously; deliciously; prettily.",
  "nonlimitation - Want of limitation; failure to limit.",
  "recomforture - The act of recomforting; restoration of comfort. [Obs.] Shak.",
  "duds - 1. Old or inferior clothes; tattered garments. [Colloq.] 2. Effects, in general.[Slang]",
  "epilogism - Enumeration; computation. [R.] J. Gregory.",
  "isosulphocyanic - Pertaining to, or designating, an acid, HNCS, isomeric with sulphocyanic acid.",
  "effectible - Capable of being done or achieved; practicable; feasible. Sir T. Browne.",
  "spreynd - p. p. of Sprenge. Sprinkled. When spreynd was holy water. Chaucer.",
  "walter - To roll or wallow; to welter. [Obs. or Prov. Eng. & Scot.]",
  "onomatologist - One versed in the history of names. Southey.",
  "theosophism - Belief in theosophy. Murdock.",
  "laudability - Laudableness; praiseworthiness.",
  "styloglossal - Of or pertaining to styloid process and the tongue.",
  "motorpathic - Of or pertaining to motorpathy.",
  "precollection - A collection previously made. [R.]",
  "snippet - A small part or piece. To be cut into snippets and shreds. F. Harrison.",
  "cytogenous - Producing cells; -- applied esp. to lymphatic, or adenoid, tissue.",
  "spermoplasma - The protoplasm of the sperm cell. Haeckel.",
  "tartuffe - A hypocritical devotee. See the Dictionary of Noted Names in Fiction.",
  "morphosis - The order or mode of development of an organ or part.",
  "plighter - One who, or that which, plights.",
  "orientality - The quality or state of being oriental or eastern. Sir T. Browne.",
  "transmutual - Reciprocal; commutual. [R.] Coleridge.",
  "burnstickle - A stickleback (Gasterosteus aculeatus).",
  "pro rata - In proportion; proportionately; according to the share, interest, or liability of each.",
  "unitize - To reduce to a unit, or one whole; to form into a unit; to unify.",
  "grandnephew - The grandson of one's brother or sister.",
  "visioned - Having the power of seeing visions; inspired; also, seen in visions. [R.] Shelley.",
  "lymphogenic - Connected with, or formed in, the lymphatic glands.",
  "soho - Ho; -- a word used in calling from a distant place; a sportsman's halloo. Shak.",
  "chromolithography - Lithography adapted to printing in inks of various colors.",
  "glowingly - In a glowing manner; with ardent heat or passion.",
  "explorer - One who explores; also, an apparatus with which one explores, as a diving bell.",
  "cyathiform - In the form of a cup, a little widened at the top.",
  "overhele - To hele or cover over. [Obs.] B. Jonson.",
  "biologic - Of or relating to biology. -- Bi`o*log\"ic*al*ly, adv.",
  "clotty - Full of clots, or clods. \"Clotty matter.\" Harvey.",
  "riviere - A necklace of diamonds or other precious stones, esp. one of several strings.",
  "crinicultural - Relating to the growth of hair. [R.]",
  "fulcrate - 1. (Bot.)  Propped; supported by accessory organs. [R.] Gray. 2. Furnished with fulcrums.",
  "ambulacriform - Having the form of ambulacra.",
  "precursorship - The position or condition of a precursor. Ruskin.",
  "arrha - Money or other valuable thing given to evidence a contract; a pledge or earnest.",
  "inobedience - Disobedience. [Obs.] Wyclif. Chaucer.",
  "linarite - A hydrous sulphate of lead and copper occurring in bright blue monoclinic crystals.",
  "unusage - Want or lack of usage. [Obs.] Chaucer.",
  "cephalopodous - Belonging to, or resembling, the cephalopods.",
  "gynandrian - Having stamens inserted in the pistil; belonging to the class Gynandria.",
  "mayoralty - The office, or the term of office, of a mayor.",
  "desecrater - One who desecrates; a profaner. Harper's Mag.",
  "inconcludent - Not inferring a conclusion or consequence; not conclusive. [Obs.]",
  "brelan favori - In French games, a pair royal composed of 2 cards in the hand and the card turned.",
  "protreptical - Adapted to persuade; hortatory; persuasive. [Obs.] Bp. Ward.",
  "insurmountableness - The state or quality of being insurmountable; insurmountability.",
  "karmathian - One of a Mohammedan sect founded in the ninth century by Karmat.",
  "wellingtons - A kind of long boots for men.",
  "prelation - The setting of one above another; preference. [R.] Jer. Taylor.",
  "fashionableness - State of being fashionable.",
  "necrophobia - An exaggerated fear of death or horror of dead bodies.",
  "indagation - Search; inquiry; investigation. [Obs.]",
  "oathable - Capable of having an oath administered to. [Obs.] Shak.",
  "clowe-gilofre - Spice clove. [Obs.] Chaucer.",
  "exclusiveness - Quality of being exclusive.",
  "usherance - The act of ushering, or the state of being ushered in. [Obs.] Shaftesbury.",
  "antipetalous - Standing before a petal, as a stamen.",
  "knightliness - The character or bearing suitable for a knight; chivalry. Spenser.",
  "self-restraint - Restraint over one's self; self-control; self-command.",
  "abassis - A silver coin of Persia, worth about twenty cents.",
  "laburnine - A poisonous alkaloid found in the unripe seeds of the laburnum.",
  "essentiality - The quality of being essential; the essential part. Jer. Taylor.",
  "underkingdom - A subordinate or dependent kingdom. Tennyson.",
  "boniform - Sensitive or responsive to moral excellence. Dr. H. More.",
  "dualist - 1. One who believes in dualism; a ditheist. 2. One who administers two offices. Fuller.",
  "smilingness - Quality or state of being smiling. And made despair a smilingness assume. Byron.",
  "cold-hearted - Wanting passion or feeling; indifferent. -- Cold\"-heart`ed*ness, n.",
  "peccadillo - A slight trespass or offense; a petty crime or fault. Sir W. Scott.",
  "potableness - The quality of being drinkable.",
  "foveolate - Having small pits or depression, as the receptacle in some composite flowers.",
  "recarbonize - To restore carbon to; as, to recarbonize iron in converting it into steel.",
  "uncongeal - To thaw; to become liquid again. Tennyson.",
  "understroke - To underline or underscore. Swift.",
  "hydrargyrate - Of or pertaining to mercury; containing, or impregnated with, mercury. [R.]",
  "fleshliness - The state of being fleshly; carnal passions and appetites. Spenser.",
  "automatous - Automatic. [Obs.] \"Automatous organs.\" Sir T. Browne.",
  "dry-eyed - Not having tears in the eyes.",
  "wash sale - A sale made in washing. See Washing, n., 3, above.",
  "lues - Disease, especially of a contagious kind. Lues venerea, syphilis; -- called also simply lues.",
  "browdyng - Embroidery. [Obs.] Of goldsmithrye, of browdying, and of steel. Chaucer.",
  "palster - A pilgrim's staff. [Obs.] Halliwell.",
  "anelectrode - The positive pole of a voltaic battery.",
  "trowelful - As much as a trowel will hold; enough to fill a trowel.",
  "readvertency - The act of adverting to again, or of reviewing. [R.] Norris.",
  "inexsuperable - Not capable of being passed over; insuperable; insurmountable.",
  "broadness - The condition or quality of being broad; breadth; coarseness; grossness.",
  "deerlet - A chevrotain. See Kanchil, and Napu.",
  "overwear - To wear too much; to wear out. Drayton.",
  "immingle - To mingle; to mix; to unite; to blend. [R.] Thomson.",
  "beholdingness - , The state of being obliged or beholden. [Obs.] Sir P. Sidney.",
  "co-une - To combine or unite. [Obs.] \"Co-uned together.\" Feltham.",
  "high priest - A chief priest; esp., the head of the Jewish priesthood.",
  "redaction - The act of redacting; work produced by redacting; a digest.",
  "inobservant - Not observant; regardless; heedless. Bp. Hurd. -- In`ob*serv\"ant*ly, adv.",
  "spherosiderite - Siderite occuring in spheroidal masses.",
  "babyhouse - A place for children's dolls and dolls' furniture. Swift.",
  "bolete - any fungus of the family Boletaceae. [WordNet 1.5]",
  "embrute - To brutify; to imbrute. All the man embruted in the swine. Cawthorn.",
  "polytomy - A division into many members. F. Bowen.",
  "subconstellation - A subordinate constellation. Sir T. Browne.",
  "priestcap - A form of redan, so named from its shape; -- called also swallowtail.",
  "oesophageal - Same as Esophagus, Esophageal, etc.",
  "abaxile - Away from the axis or central line; eccentric. Balfour.",
  "decussative - Intersecting at acute angles. Sir T. Browne.",
  "aliene - To alien or alienate; to transfer, as title or property; as, to aliene an estate.",
  "crotaphite - The temple or temporal fossa. Also used adjectively.",
  "omniformity - The condition or quality of having every form. Dr. H. More.",
  "maistre - Mastery; superiority; art. See Mastery. [Obs.] Chaucer.",
  "pronunciatory - Of or pertaining to pronunciation; that pronounces.",
  "overoffice - To domineer over by virtue of office. [Obs.] Shak.",
  "astro- - The combining form of the Greek word 'a`stron, meaning star.",
  "reorient - Rising again. [R.] The life reorient out of dust. Tennyson.",
  "depositure - The act of depositing; deposition. [Obs.] Sir T. Browne.",
  "urinarium - A reservoir for urine, etc., for manure.",
  "correctly - In a correct manner; exactly; acurately; without fault or error.",
  "untrunked - Separated from its trunk or stock. [Obs.]",
  "woodwork - Work made of wood; that part of any structure which is wrought of wood.",
  "predestinarianism - The system or doctrine of the predestinarians.",
  "despisedness - The state of being despised.",
  "chicalote - A Mexican prickly poppy (Argemone platyceras), which has migrated into California.",
  "debarment - Hindrance from approach; exclusion.",
  "grogram - A coarse stuff made of silk and mohair, or of coarse silk.",
  "nake - ,v.t. To make naked. [Obs.] Chaucer. Come, be ready, nake your swords. Old Play.",
  "sanatorium - An establishment for the treatment of the sick; a resort for invalids. See Sanitarium.",
  "malleolar - Of or pertaining to the malleolus; in the region of the malleoli of the ankle joint.",
  "physicking - p. pr. & vb. n. fr. Physic, v. t.",
  "restringe - To confine; to contract; to stringe. [Obs.]",
  "patrizate - To imitate one's father. [R.]",
  "blastophoral - Relating to the blastophore.",
  "paramorphous - Relating to paramorphism; exhibiting paramorphism.",
  "mislactation - Defective flow or vitiated condition of the milk.",
  "haggish - Like a hag; ugly; wrinkled. But on both did haggish age steal on. Shak.",
  "dronish - Like a drone; indolent; slow. Burke. -- Dron\"ish*ly, adv. -- Dron\"ish*ness, n.",
  "boar - The uncastrated male of swine; specifically, the wild hog.",
  "endochondral - Growing or developing within cartilage; -- applied esp. to developing bone.",
  "prescapula - The part of the scapula in front of, or above, the spine, or mesoscapula.",
  "libkin - A house or lodging. [Old Slang] B. Jonson.",
  "miasmatist - One who has made a special study of miasma.",
  "questmonger - One who lays informations, and encourages petty lawsuits. [Obs.] Bacon.",
  "scenario - A preliminary sketch of the plot, or main incidents, of an opera.",
  "sea bear - (a) Any fur seal. See under Fur. (b) The white bear.",
  "saphead - A weak-minded, stupid fellow; a milksop. [Low]",
  "complicity - The state of being an accomplice; participation in guilt.",
  "basaltic - Pertaining to basalt; formed of, or containing, basalt; as basaltic lava.",
  "dietine - A subordinate or local assembly; a diet of inferior rank.",
  "ecclesiastes - One of the canonical books of the Old Testament.",
  "afternoon - The part of the day which follows noon, between noon and evening.",
  "al segno - A direction for the performer to return and recommence from the sign",
  "welfaring - Faring well; prosperous; thriving. [Obs.] \"A welfaring person.\" Chaucer.",
  "tozy - Soft, like wool that has been teased. -- To\"zi*ness, n.",
  "amylopsin - The diastase of the pancreatic juice.",
  "mischiefable - Mischievous. [R.] Lydgate.",
  "postulant - One who makes a request or demand; hence, a candidate.",
  "frivol - To act frivolously; to trifle. Kipling. -- Friv\"ol*er (#), Friv\"ol*ler, n. [All Colloq.]",
  "marketer - One who attends a market to buy or sell; one who carries goods to market.",
  "fivefold - In fives; consisting of five in one; five repeated; quintuple.",
  "exanthesis - An eruption of the skin; cutaneous efflorescence.",
  "mandarinism - A government mandarins; character or spirit of the mandarins. F. Lieder.",
  "planarida - A division of Turbellaria; the Dendrocoela.",
  "tulle - A kind of silk lace or light netting, used for veils, etc.",
  "phlebotomist - One who practiced phlebotomy.",
  "mauther - A girl; esp., a great, awkward girl; a wench. [Prov. Eng.]",
  "acold - Cold. [Obs.] \"Poor Tom's acold.\" Shak.",
  "volkslied - A popular song, or national air.",
  "tracer - One who, or that which, traces.",
  "brambled - Overgrown with brambles. Forlorn she sits upon the brambled floor. T. Warton.",
  "resuscitant - One who, or that which resuscitates. Also used adjectively.",
  "musrol - The nose band of a horse's bridle.",
  "turiole - The golden oriole. [Prov. Eng.]",
  "otiosity - Leisure; indolence; idleness; ease. [R.] Thackeray.",
  "turtle peg - A sharp steel spear attached to a cord, used in taking sea turtles. -- Turtle pegging.",
  "outrede - To surpass in giving rede, or counsel. [Obs.] See Atrede. Chaucer.",
  "uncourtliness - Absence of courtliness; rudeness; rusticity. Addison.",
  "discipleship - The state of being a disciple or follower in doctrines and precepts. Jer. Taylor.",
  "pacifical - Of or pertaining to peace; pacific. [R.] Sir H. Wotton. -- Pa*cif\"ic*al*ly, adv. [R.]",
  "dairying - The business of conducting a dairy.",
  "elkwood - The soft, spongy wood of a species of Magnolia (M. Umbrella).",
  "compurgatorial - Relating to a compurgator or to compurgation. \"Their compurgatorial oath.\" Milman.",
  "incommunicability - The quality or state of being incommunicable, or incapable of being imparted.",
  "expostulatory - Containing expostulation or remonstrance; as, an expostulatory discourse or letter.",
  "golyardeys - A buffoon. See Gollard. [Obs.] Chaucer.",
  "clubbist - A member of a club; a frequenter of clubs. [R.] Burke.",
  "melanesian - Of or pertaining to Melanesia.",
  "misanthropic - Hating or disliking mankind.",
  "resolvability - The quality or condition of being resolvable; resolvableness.",
  "guardenage - Guardianship. [Obs. & R.] \" His tuition and guardenage.\" Holland.",
  "aptote - A noun which has no distinction of cases; an indeclinable noun.",
  "passive balloon - One unprovided with motive power.",
  "liberatory - Tending, or serving, to liberate. [R.]",
  "cinchonaceous - Allied or pertaining to cinchona, or to the plants that produce it.",
  "overanxious - Anxious in an excessive or needless degree. -- O\"ver*anx\"ious*ly, adv.",
  "geoponical - Pertaining to tillage of the earth, or agriculture.",
  "triptote - A noun having three cases only.",
  "prestigiatory - Consisting of impostures; juggling. [Obs.] Barrow.",
  "overlinger - To cause to linger; to detain too long. [Obs.] Fuller.",
  "carminic - Of or pertaining to, or derived from, carmine. Carminic acid. Same as Carmine, 3.",
  "inexactness - Incorrectness; want of exactness.",
  "tricliniary - Of or pertaining to a triclinium, or to the ancient mode of reclining at table.",
  "squarroso-dentate - Having the teeth bent out of the plane of the lamina; -- said of a leaf.",
  "unleavened - Not leavened; containing no leaven; as, unleavened bread.",
  "endoctrine - To teach; to indoctrinate. [Obs.] Donne.",
  "blondness - The state of being blond. G. Eliot.",
  "optimity - The state of being best. [R.] Bailey.",
  "correlatable - Such as can be correlated; as, correlatable phenomena.",
  "premotion - Previous motion or excitement to action.",
  "dissentient - Disagreeing; declaring dissent; dissenting. -- n.  One who dissents. Macaulay.",
  "archelogy - The science of, or a treatise on, first principles. Fleming.",
  "hyperkinetic - Of or pertaining to hyperkinesis.",
  "barnabite - A member of a religious order, named from St. Barnabas.",
  "waterboard - A board set up to windward in a boat, to keep out water. Ham. Nav. Encyc.",
  "berbe - An African genet (Genetta pardina). See Genet.",
  "hoosier - A nickname given to an inhabitant of the State of Indiana. [U.S.]",
  "schizogenesis - reproduction by fission. Haeckel.",
  "univalved - Having one valve; as, a univalve shell or pericarp.",
  "sprayer - One that sprays; any instrument for vaporizing and spraying liquids.",
  "instrumentalist - One who plays upon an instrument of music, as distinguished from a vocalist.",
  "swarthily - In a swarthy manner; with a tawny hue; duskily.",
  "quadrumanous - Having four hands; of or pertaining to the Quadrumana.",
  "myeloid - Resembling marrow in appearance or consistency; as, a myeloid tumor.",
  "towery - Having towers; adorned or defended by towers. [R.] \"Towery cities.\" Pope.",
  "chondropterygian - Having a cartilaginous skeleton. -- n.  One of the Chondropterygii.",
  "tradesfolk - People employed in trade; tradesmen. [R.] Swift.",
  "bockey - A bowl or vessel made from a gourd. [Local, New York] Bartlett.",
  "affronter - One who affronts, or insults to the face.",
  "lap-welded - Having edges or ends united by a lap weld; as, a lap-welded pipe.",
  "ill-starred - Fated to be unfortunate; unlucky; as, an ill-starred man or day.",
  "draining - The art of carrying off surplus water, as from land. Draining tile. Same as Draintile.",
  "pantography - A general description; entire view of an object.",
  "accruer - The act of accruing; accretion; as, title by accruer.",
  "calamistration - The act or process of curling the hair. [Obs.] burton.",
  "colleterium - An organ of female insects, containing a cement to unite the ejected ova.",
  "spodomancy - Divination by means of ashes.",
  "imaginarily - In a imaginary manner; in imagination. B. Jonson.",
  "obovate - Inversely ovate; ovate with the narrow end downward; as, an obovate leaf.",
  "opportunist - One who advocates or practices opportunism. [Recent]",
  "spadassin - A bravo; a bully; a duelist. Ld. Lytton.",
  "phalangal - Of or pertaining to the phalanges. See Phalanx, 2.",
  "overword - To say in too many words; to express verbosely. Hales.",
  "widewhere - Widely; far and wide. [Obs.] Chaucer.",
  "dodecane - Any one of a group of thick oily hydrocarbons, C12H26, of the paraffin series.",
  "micrographic - Of or pertaining to micrography.",
  "operant - Operative. [R.] Shak. -- n.  An operative person or thing. [R.] Coleridge.",
  "nonproduction - A failure to produce or exhibit.",
  "diamond-back - The salt-marsh terrapin of the Atlantic coast (Malacoclemmys palustris).",
  "gabarage - A kind of coarse cloth for packing goods. [Obs.]",
  "interferingly - By or with interference.",
  "downlooked - Having a downcast countenance; dejected; gloomy; sullen. [R.] Dryden.",
  "stratified - Having its substance arranged in strata, or layers; as, stratified rock.",
  "baselard - A short sword or dagger, worn in the fifteenth century. [Written also baslard.] Fairholt.",
  "eponymist - One from whom a race, tribe, city, or the like, took its name; an eponym.",
  "disesteemer - One who disesteems. Boyle.",
  "jesu - Jesus. [Poetical] Jesu, give the weary Calm and sweet repose. S. Baring-Gould.",
  "plagiarism - 1. The act or practice of plagiarizing. 2. That which plagiarized.",
  "skeletology - That part of anatomy which treats of the skeleton; also, a treatise on the skeleton.",
  "acanthocephalous - Having a spiny head, as one of the Acanthocephala.",
  "cleg - A small breeze or horsefly. [North of Eng. & Scot.] Jamieson.",
  "metempsychose - To translate or transfer, as the soul, from one body to another. [R.] Peacham.",
  "esemplastic - Shaped into one; tending to, or formative into, unity. [R.] Coleridge.",
  "erupt - To cause to burst forth; to eject; as, to erupt lava. Huxley.",
  "versatility - The quality or state of being versatile; versatileness.",
  "stream clock - An instrument for ascertaining the velocity of the blood in a vessel.",
  "xenium - A present given to a guest or stranger, or to a foreign ambassador.",
  "burghership - The state or privileges of a burgher.",
  "quinquefarious - Arranged in five vertical rows; pentastichous. Gray.",
  "sexuality - The quality or state of being distinguished by sex. Lindley.",
  "quarrelet - A little quarrel. See 1st Quarrel, 2. [Obs.] \"Quarrelets of pearl [teeth].\" Herrick.",
  "etiology - The science of causes. Same as tiology.",
  "electro-therapeutics - The branch of medical science which treats of the applications agent.",
  "tautoousian - Having the same essence; being identically of the same nature. [R.] Cudworth.",
  "lividity - The state or quality of being livid.",
  "deflow - To flow down. [Obs.] Sir T. Browne.",
  "undersetting - Something set or built under as a support; a pedestal. Sir H. Wotton.",
  "supernaturalistic - Of or pertaining to supernaturalism.",
  "diminutal - Indicating or causing diminution. Earle.",
  "sea wolf - (a) The wolf fish. (b) The European sea perch. (c) The sea elephant. (d) A sea lion.",
  "exscribe - To copy; to transcribe. [Obs.] B. Jonson.",
  "formeret - One of the half ribs against the walls in a ceiling vaulted with ribs.",
  "bespit - To daub or soil with spittle. Johnson.",
  "palatonares - The posterior nares. See Nares.",
  "frequentage - The practice or habit of frequenting. [R.] Southey.",
  "impressure - Dent; impression. [Obs.] Shak.",
  "keech - A mass or lump of fat rolled up by the butcher. [Obs.] Shak.",
  "efformation - The act of giving shape or form. [Obs.] Ray.",
  "pressgang - See Press gang, under Press.",
  "anan - An expression equivalent to What did you say Sir Eh [Obs.] Shak.",
  "kidney-form - Having the form or shape of a kidney; reniform; as, a kidney- shaped leaf. Gray.",
  "liableness - Quality of being liable; liability.",
  "pirl - 1. To spin, as a top. 2. To twist or twine, as hair in making fishing lines.",
  "soloist - One who sings or plays a solo.",
  "subscapular - Situated beneath the scapula; infrascapular; as, the subscapular muscle.",
  "ghazel - A kind of Oriental lyric, and usually erotic, poetry, written in recurring rhymes.",
  "abandum - Anything forfeited or confiscated.",
  "tigella - That part of an embryo which represents the young stem; the caulicle or radicle.",
  "tickle-footed - Uncertain; inconstant; slippery. [Obs. & R.] Beau. & Fl.",
  "dipaschal - Including two passovers. Carpenter.",
  "pay rock - Earth, rock, etc., which yields a profit to the miner. [Western U. S.]",
  "duad - A union of two; duality. [R.] Harris.",
  "heroner - A hawk used in hunting the heron. \"Heroner and falcon.\" Chaucer.",
  "ethnographical - pertaining to ethnography.",
  "entheasm - Inspiration; enthusiasm. [R.] \"Religious entheasm.\" Byron.",
  "lollop - To move heavily; to lounge or idle; to loll. [Law.] Charles Reade.",
  "synanthous - Having flowers and leaves which appear at the same time; -- said of certain plants.",
  "skute - A boat; a small vessel. [Obs.] Sir R. Williams.",
  "blink beer - Beer kept unbroached until it is sharp. Crabb.",
  "sweepy - Moving with a sweeping motion. The branches bend before their sweepy away. Dryden.",
  "affatuate - To infatuate. [Obs.] Milton.",
  "nape - The back part of the neck. Spenser.",
  "excreate - To spit out; to discharge from the throat by hawking and spitting. [Obs.] Cockeram.",
  "custodianship - Office or duty of a custodian.",
  "conceptibility - The quality of being conceivable; conceivableness. Cudworth.",
  "gemmification - The production of a bud or gem.",
  "hyperoxymuriatic - Perchloric; as, hyperoxymuriatic acid. [Obs.]",
  "misalter - To alter wrongly; esp., to alter for the worse. Bp. Hall.",
  "equipondious - Of equal weight on both sides; balanced. [Obs.] Glanvill.",
  "bridechamber - The nuptial appartment. Matt. ix. 15.",
  "workingman - A laboring man; a man who earns his daily support by manual labor.",
  "mazedness - The condition of being mazed; confusion; astonishment. [Obs.] Chaucer.",
  "fair-natured - Well-disposed. \"A fair-natured prince.\" Ford.",
  "cadgy - Cheerful or mirthful, as after good eating or drinking; also, wanton. [Scot. & Prov. Eng.]",
  "plectile - Woven; plaited. [Obs.] Sir T. Browne.",
  "luteolin - A yellow dyestuff obtained from the foliage of the dyer's broom (Reseda luteola).",
  "homodont - Having all the teeth similar in front, as in the porpoises; -- opposed to heterodont.",
  "catadrome - 1. A race course. 2. (Mach.)  A machine for raising or lowering heavy weights.",
  "hemisection - A division along the mesial plane; also, one of the parts so divided.",
  "meleagris - A genus of American gallinaceous birds, including the common and the wild turkeys.",
  "stowboard - A place into which rubbish is put. [Written also stowbord.]",
  "bubonic - Of or pertaining to a bubo or buboes; characterized by buboes.",
  "cometology - The department of astronomy relating to comets.",
  "pallid - Deficient in color; pale; wan; as, a pallid countenance; pallid blue. Spenser.",
  "gelastic - Pertaining to laughter; used in laughing. \"Gelastic muscles.\" Sir T. Browne.",
  "lux - To put out of joint; to luxate. [Obs.]",
  "thanage - The district in which a thane anciently had jurisdiction; thanedom.",
  "cilician - Of or pertaining to Cilicia in Asia Minor. -- n.  A native or inhabitant of Cilicia.",
  "skare - Wild; timid; shy. [Prov. Eng. & Scot.]",
  "laverock - The lark. [Old Eng. & Scot.] [Written also lavrock.] Gower.",
  "mycetes - A genus of South American monkeys, including the howlers. See Howler, 2, and Illust.",
  "dulce - To make sweet; to soothe. [Obs.]",
  "affiche - A written or printed notice to be posted, as on a wall; a poster; a placard.",
  "annumeration - Addition to a former number. [Obs.] Sir T. Browne.",
  "halophyte - A plant found growing in salt marshes, or in the sea.",
  "illtreat - To treat cruelly or improperly; to ill use; to maltreat.",
  "thalian - Of or pertaining to Thalia; hence, of or pertaining to comedy; comic.",
  "wennish - Having the nature of a wen; resembling a wen; as, a wennish excrescence.",
  "offuscate - See Obfuscate, Obfuscation. [Obs.]",
  "cayman - The south America alligator. See Alligator. [Sometimes written caiman.]",
  "empirically - By experiment or experience; without science; in the manner of quacks.",
  "irreformable - Incapable of being reformed; incorrigible. Joseph Cook.",
  "inexpugnably - So as to be inexpugnable; in an inexpugnable manner. Dr. H. More.",
  "supertemporal - That which is more than temporal; that which is eternal. [R.]",
  "spilter - Any one of the small branches on a stag's head. [Obs.] Howell.",
  "tactful - Full of tact; characterized by a discerning sense of what is right, proper, or judicious.",
  "unchaste - Not chaste; not continent; lewd. -- Un*chaste\"ly, adv. -- Un*chaste\"ness, n.",
  "petto - The breast. In petto, in the breast; hence, in secrecy; in reserve.",
  "supervolute - Having a plainted and convolute arrangement in the bud, as in the morning-glory.",
  "cicuration - The act of taming. [Obs.] Ray.",
  "homotypal - Of the same type of structure; pertaining to a homotype; as, homotypal parts.",
  "disacquaintance - Neglect of disuse of familiarity, or familiar acquaintance. [Obs.] South.",
  "hudge - An iron bucket for hoisting coal or ore. Raymond.",
  "bloodshedder - One who sheds blood; a manslayer; a murderer.",
  "armorist - One skilled in coat armor or heraldry. Cussans.",
  "natureless - Not in accordance with nature; unnatural. [Obs.] Milton.",
  "immanuel - God with us; -- an appellation of the Christ. Is. vii. 14. Matt. i. 23.",
  "counterscale - Counterbalance; balance, as of one scale against another. [Obs.] Howell.",
  "totipresence - Omnipresence. [Obs.] A. Tucker.",
  "polaris - The polestar. See North star, under North.",
  "dissymmetry - Absence or defect of symmetry; asymmetry.",
  "masterdom - Dominion; rule; command. [R.] Shak.",
  "engagedness - The state of being deeply interested; earnestness; zeal.",
  "acequia - A canal or trench for irrigating land. [Sp. Amer.]",
  "crannied - Having crannies, chinks, or fissures; as, a crannied wall. Tennyson.",
  "fellowfeel - To share through sympathy; to participate in. [R.] D. Rodgers.",
  "pahi - A large war canoe of the Society Islands.",
  "pathmaker - One who, or that which, makes a way or path.",
  "adscriptive - Attached or annexed to the glebe or estate and transferable with it. Brougham.",
  "behither - On this side of. [Obs.] Two miles behither Clifden. Evelyn.",
  "festoony - Pertaining to, consisting of, or resembling, festoons. Sir J. Herschel.",
  "bragget - A liquor made of ale and honey fermented, with spices, etc. [Obs.] B. Jonson.",
  "abnodate - To clear (tress) from knots. [R.] Blount.",
  "neglectingly - Carelessly; heedlessly. Shak.",
  "hypostoma - The lower lip of trilobites, crustaceans, etc.",
  "rowdyish - Resembling a rowdy in temper or conduct; characteristic of a rowdy.",
  "pseudologist - One who utters falsehoods; a liar.",
  "dystocia - Difficult delivery pr parturition.",
  "trilobation - The state of being trilobate.",
  "wretchedly - In a wretched manner; miserably; despicable.",
  "juwansa - The camel's thorn. See under Camel.",
  "varnishing - The act of laying on varnish; also, materials for varnish.",
  "self-heal - A blue-flowered labiate plant (Brunella vulgaris); the healall.",
  "setiferous - Producing, or having one or more, bristles.",
  "cameralistics - The science of finance or public revenue.",
  "bindingness - The condition or property of being binding; obligatory quality. Coleridge.",
  "toe hold - A hold in which the agressor bends back his opponent's foot.",
  "illusionist - One given to illusion; a visionary dreamer.",
  "dispurveyance - Want of provisions; [Obs.] Spenser.",
  "saxatile - Of or pertaining to rocks; living among rocks; as, a saxatile plant.",
  "symposion - A drinking together; a symposium. \"Our symposion last night.\" Sir W. Scott.",
  "fratricidal - Of or pertaining to fratricide; of the nature of fratricide.",
  "cumulation - The act of heaping together; a heap. See Accumulation.",
  "excruciable - Liable to torment. [R.] Bailey.",
  "hoverer - A device in an incubator for protecting the young chickens and keeping them warm.",
  "grovy - Pertaining to, or resembling, a grove; situated in, or frequenting, groves. Dampier.",
  "platiniferous - Yielding platinum; as, platiniferous sand.",
  "perishableness - The quality or state of being perishable; liability to decay or destruction. Locke.",
  "infecundity - Want of fecundity or fruitfulness; barrenness; sterility; unproductiveness.",
  "debauched - Dissolute; dissipated. \"A coarse and debauched look.\" Ld. Lytton.",
  "simous - Having a very flat or snub nose, with the end turned up.",
  "dijudication - The act of dijudicating; judgment. [R.] Cockeram.",
  "trisaccharide - A complex sugar, as raffinose, yielding by hydrolysis three simple sugar molecules.",
  "pupillometer - An instrument for measuring the size of the pupil of the pupil of the eye.",
  "broad-leafed - Having broad, or relatively broad, leaves. Keats.",
  "canaanitish - Of or pertaining to Canaan or the Canaanites.",
  "orbitonasal - Of or pertaining to the orbit and the nose; as, the orbitonasal, or ophthalmic, nerve.",
  "magnetician - One versed in the science of magnetism; a magnetist.",
  "sipy - Oozy; -- applied to land under cultivation that is not well drained.",
  "prosimetrical - Consisting both of prose and verse. Clarke.",
  "nonone - Any one of several metameric unsaturated hydrocarbons (C9H14) of the valylene series.",
  "corinne - The common gazelle (Gazella dorcas). See Gazelle. [Written also korin.]",
  "requisitory - Sought for; demanded. [R.] Summary on Du Bartas (1621).",
  "ammonitiferous - Containing fossil ammonites.",
  "contemplativeness - The state of being contemplative; thoughtfulness.",
  "baronial - Pertaining to a baron or a barony. \"Baronial tenure.\" Hallam.",
  "corruptingly - In a manner that corrupts.",
  "cyanide - A compound formed by the union of cyanogen with an element or radical.",
  "experimentally - By experiment; by experience or trial. J. S. Mill.",
  "saulie - A hired mourner at a funeral. [Scot.] Sir W. Scott.",
  "panoramic - Of, pertaining to, or like, a panorama. Panoramic camera. See under Camera.",
  "aristology - The science of dining. Quart. Rev.",
  "holcad - A large ship of burden, in ancient Greece. Mitford.",
  "phorminx - A kind of lyre used by the Greeks. Mrs. Browning.",
  "domiculture - The art of house-keeping, cookery, etc. [R.] R. Park.",
  "nonbituminous - Containing no bitumen; not bituminous.",
  "moonset - The descent of the moon below the horizon; also, the time when the moon sets.",
  "dialogism - An imaginary speech or discussion between two or more; dialogue. Fulke."
]