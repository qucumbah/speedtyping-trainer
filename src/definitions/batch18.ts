export default [
  "emborder - To furnish or adorn with a border; to imborder.",
  "water craft - Any vessel or boat plying on water; vessels and boats, collectively.",
  "vagabondize - To play the vagabond; to wander about in idleness.",
  "exaugurate - To annul the consecration of; to secularize; to unhellow. [Obs.] Holland.",
  "expiscation - The act of expiscating; a fishing. [R.] Chapman.",
  "cowquake - A genus of plants (Briza); quaking grass.",
  "warfarer - One engaged in warfare; a military man; a soldier; a warrior.",
  "antipathous - Having a natural contrariety; adverse; antipathetic. [Obs.] Beau. & Fl.",
  "dawsonite - A hydrous carbonate of alumina and soda, occuring in white, bladed crustals.",
  "affixture - The act of affixing, or the state of being affixed; attachment.",
  "quemeful - Kindly; merciful. [Obs.] Wyclif.",
  "reliability - The state or quality of being reliable; reliableness.",
  "set-to - A contest in boxing, in an argument, or the like. [Colloq.] Halliwell.",
  "lofter - An iron club used in lofting the ball; -- called also lofting iron.",
  "samisen - A Japanese musical instrument with three strings, resembling a guitar or banjo.",
  "sulphurwort - The hog's fennel. See under Fennel.",
  "ugliness - The quality or state of being ugly.",
  "groutnol - Same as Growthead. Beau. & Fl.",
  "scavenge - To cleanse, as streets, from filth. C. Kingsley.",
  "sylleptical - Of or pertaining to a syllepsis; containing syllepsis. -- Syl*lep\"tic*al*ly, adv.",
  "voyageable - That may be sailed over, as water or air; navigable.",
  "semicirque - A semicircular hollow or opening among trees or hills. Wordsworth.",
  "loafer - One who loafs; a lazy lounger. Lowell.",
  "illuxurious - Not luxurious. [R.] Orrery.",
  "cribrose - Perforated like a sieve; cribriform.",
  "extraaxillary - Growing outside of the axils; as, an extra-axillary bud.",
  "mataco - The three-banded armadillo (Tolypeutis tricinctus). See Illust. under Loricata.",
  "alacrify - To rouse to action; to inspirit.",
  "protosalt - A salt derived from a protoxide base. [Obs.]",
  "speckled-bill - The American white-fronted goose (Anser albifrons).",
  "elimate - To render smooth; to polish. [Obs.]",
  "deploredness - The state of being deplored or deplorable. [R.] Bp. Hail.",
  "lituate - Forked, with the points slightly curved outward.",
  "hymnographer - 1. One who writes on the subject of hymns. 2. A writer or composed of hymns.",
  "water cure - 1. (Med.)  Hydropathy. 2. A hydropathic institution.",
  "lageniform - Shaped like a bottle or flask; flag-shaped.",
  "kabook - A clay ironstone found in Ceylon.",
  "choregraphic - Pertaining to choregraphy.",
  "kers - A cress. [Obs.] Chaucer. Not worth a kers. See under Cress.",
  "supersensuous - 1. Supersensible. 2. Excessively sensuous.",
  "obsolescent - Going out of use; becoming obsolete; passing into desuetude.",
  "achaean - Of or pertaining to Achaia in Greece; also, Grecian. -- n.  A native of Achaia; a Greek.",
  "amioidei - An order of ganoid fishes of which Amis is type. See Bowfin and Ganoidei.",
  "achlamydeous - Naked; having no floral envelope, neither calyx nor corolla.",
  "viary - Of or pertaining to roads; happening on roads. [Obs.]",
  "attemptive - Disposed to attempt; adventurous. [Obs.] Daniel.",
  "lenitiveness - The quality of being lenitive.",
  "russophobia - Morbid dread of Russia or of Russian influence.",
  "hematoma - A circumscribed swelling produced by an effusion of blood beneath the skin.",
  "blaubok - The blue buck. See Blue buck, under Blue.",
  "flagitate - To importune; to demand fiercely or with passion. [Archaic] Carcyle.",
  "manifestness - The quality or state of being manifest; obviousness.",
  "vaniloquence - Vain or foolish talk. [Obs.]",
  "trechometer - An odometer for vehicles. Knight.",
  "excubitorium - A gallery in a church, where persons watched all night.",
  "seemliness - The quality or state of being seemly: comeliness; propriety.",
  "conny - Brave; fine; canny. [Prov. Eng.] Grose.",
  "gigerium - The muscular stomach, or gizzard, of birds.",
  "heptylic - Pertaining to, or derived from, heptyl or heptane; as, heptylic alcohol. Cf. .",
  "backdoor - Acting from behind and in concealment; as backdoor intrigues.",
  "sciurus - A genus of reodents comprising the common squirrels.",
  "osmotic - Pertaining to, or having the property of, osmose; as, osmotic force.",
  "culling - 1. The act of one who culls. 2. pl.  Anything separated or selected from a mass.",
  "photochromography - Art or process of printing colored photographs.",
  "imprescriptibility - The quality of being imprescriptible.",
  "spatangoid - Of or pertaining to the Spatangoidea. -- n.  One of the Spatangoidea.",
  "semined - Thickly covered or sown, as with seeds. [Obs.] B. Jonson.",
  "tartramide - An acid amide derivative of tartaric acid, obtained as a white crystalline substance.",
  "messiahship - The state or office of the Messiah.",
  "capillation - A capillary blood vessel. [Obs.] Sir T. Browne.",
  "stylography - A mode of writing or tracing lines by means of a style on cards or tablets.",
  "sparing - Spare; saving; frugal; merciful. Bacon. -- Spar\"ing*ly, adv. -- Spar\"ing*ness, n.",
  "absentaneous - Pertaining to absence. [Obs.]",
  "plugger - One who, or that which, plugs.",
  "hemipterous - Of or pertaining to the Hemiptera.",
  "ionian - Of or pertaining to Ionia or the Ionians; Ionic. -- n.  A native or citizen of Ionia.",
  "rattinet - A woolen stuff thinner than ratteen.",
  "irremediableness - The state or quality of being irremediable.",
  "cor - A Hebrew measure of capacity; a homer. [Written also core.]",
  "eggement - Instigation; incitement. [Obs.] Chaucer.",
  "loob - The clay or slimes washed from tin ore in dressing.",
  "hailstorm - A storm accompanied with hail; a shower of hail.",
  "terebratuliform - Having the general form of a terebratula shell.",
  "vengeful - Vindictive; retributive; revengeful. \"Vengeful ire.\" Milton. -- Venge\"ful*ly, adv.",
  "euchymy - A good state of he blood and other fluids of the body.",
  "athletics - The art of training by athletic exercises; the games and sports of athletes.",
  "ectal - Pertaining to, or situated near, the surface; outer; -- opposed to ental. B. G. Wilder.",
  "hariolation - Prognostication; soothsaying. [Obs.] Cockeram.",
  "duncify - To make stupid in intellect. [R.] Bp. Warburton.",
  "allopathically - In a manner conformable to allopathy; by allopathic methods.",
  "modernness - The quality or state of being modern; recentness; novelty. M. Arnold.",
  "otherwhere - In or to some other place, or places; elsewhere. Milton. Tennyson.",
  "portemonnaie - A small pocketbook or wallet for carrying money.",
  "disadventure - Misfortune; mishap. [Obs.] Sir W. Raleigh.",
  "exampler - A pattern; an exemplar. [Obs.]",
  "prenote - To note or designate beforehand. Foxe.",
  "double-barreled - Having two barrels; -- applied to a gun.",
  "affrontiveness - The quality that gives an affront or offense. [R.] Bailey.",
  "inhibitor - That which causes inhibitory action; esp., an inhibitory nerve.",
  "seasoner - One who, or that which, seasons, or gives a relish; a seasoning.",
  "camphoraceous - Of the nature of camphor; containing camphor. Dunglison.",
  "sludge acid - Impure dark-colored sulphuric acid that has been used in the refining of petroleum.",
  "phalangeal - Of or pertaining to the phalanges. See Phalanx, 2.",
  "barth - A place of shelter for cattle. [Prov. Eng.] Halliwell.",
  "especialness - The state of being especial.",
  "multilateral - Having many sides; many-sided.",
  "tivy - With great speed; -- a huntsman's word or sound. Dryden.",
  "shadowiness - The quality or state of being shadowy.",
  "besit - To suit; to fit; to become. [Obs.]",
  "preconcertion - The act of preconcerting; preconcert. Dr. T. Dwight.",
  "lenard tube - A tube for producing Lenard rays.",
  "septilateral - Having seven sides; as, a septilateral figure.",
  "corollet - A floret in an aggregate flower. [Obs.] Martyn.",
  "convectively - In a convective manner. Hare.",
  "megachile - A leaf-cutting bee of the genus Megachilus. See Leaf cutter, under Leaf.",
  "incask - To cover with a casque or as with a casque. Sherwood.",
  "cross-question - To cross-examine; to subject to close questioning.",
  "caviller - One who cavils. Cavilers at the style of the Scriptures. Boyle.",
  "pal - A mate; a partner; esp., an accomplice or confederate. [Slang]",
  "philosophation - Philosophical speculation and discussion. [Obs.] Sir W. Petty.",
  "dreynt - p. p., of Drench to drown. [Obs.] Chaucer.",
  "incerative - Cleaving or sticking like wax. Cotgrave.",
  "stives - Stews; a brothel. [Obs.] Chaucer.",
  "zootomy - The dissection or the anatomy of animals; -- distinguished from androtomy.",
  "bicorporate - Double-bodied, as a lion having one head and two bodies.",
  "palpiform - Having the form of a palpus.",
  "bouquet - 1. A nosegay; a bunch of flowers. 2. A perfume; an aroma; as, the bouquet of wine.",
  "earpick - An instrument for removing wax from the ear.",
  "opine - To have an opinion; to judge; to think; to suppose. South.",
  "almonry - The place where an almoner resides, or where alms are distributed.",
  "imburse - To supply or stock with money. [Obs.]",
  "wontless - Unaccustomed. [Obs.] Spenser.",
  "curtailment - The act or result of curtailing or cutting off. Bancroft.",
  "conominee - One nominated in conjunction with another; a joint nominee. Kirby.",
  "rhymist - A rhymer; a rhymester. Johnston.",
  "avoke - To call from or back again. [Obs.] Bp. Burnet.",
  "acetonuria - Excess of acetone in the urine, as in starvation or diabetes.",
  "story-telling - Being accustomed to tell stories. -- n.  The act or practice of telling stories.",
  "jan - One of intermediate order between angels and men.",
  "picus - A genus of woodpeckers, including some of the common American and European species.",
  "transformative - Having power, or a tendency, to transform.",
  "misentreat - To treat wrongfully. [Obs.] Grafton.",
  "starchwort - The cuckoopint, the tubers of which yield a fine quality of starch.",
  "tedesco - German; -- used chiefly of art, literature, etc.",
  "uncalm - To disturb; to disquiet. Dryden.",
  "evadible - Capable of being evaded. [R.]",
  "antichronical - Deviating from the proper order of time. -- An`ti*chron\"ic*al*ly, adv.",
  "scapegallows - One who has narrowly escaped the gallows for his crimes. [Colloq.] Dickens.",
  "deturn - To turn away. [Obs.] Sir K. Digby.",
  "beath - To bathe; also, to dry or heat, as unseasoned wood. [Obs.] Spenser.",
  "malengine - Evil machination; guile; deceit. [Obs.] Gower.",
  "circumnutate - To pass through the stages of circumnutation.",
  "sanctifier - One who sanctifies, or makes holy; specifically, the Holy Spirit.",
  "counterponderate - TO equal in weight; to counterpoise; to equiponderate.",
  "pac - A kind of moccasin, having the edges of the sole turned up and sewed to the upper. Knight.",
  "sceptic - etc. See Skeptic, Skeptical, Skepticism, etc.",
  "soleness - The state of being sole, or alone; singleness. [R.] Chesterfield.",
  "bedcord - A cord or rope interwoven in a bedstead so as to support the bed.",
  "uralitization - The change of pyroxene to amphibole by paramorphism.",
  "volubilate - Turning, or whirling; winding; twining; voluble.",
  "metallize - To impart metallic properties to; to impregnate with a metal. [R.]",
  "freshmanship - The state of being a freshman.",
  "troopfowl - The American scaup duck. [Local, U. S.]",
  "apologer - A teller of apologues. [Obs.]",
  "cunctative - Slow; tardy; dilatory; causing delay.",
  "stirk - A young bullock or heifer. [Prov. Eng. & Scot.] Sir W. Scott.",
  "maniglion - Either one of two handles on the back of a piece of ordnance.",
  "bord service - Service due from a bordar; bordage.",
  "supinity - Supineness. [Obs.] Sir T. Browne.",
  "variole - 1. A foveola. 2.  (Geol.) A spherule of a variolite.",
  "fineless - Endless; boundless. [Obs.] Shak.",
  "gibbostity - The state of being gibbous or gibbose; gibbousness.",
  "fringed - Furnished with a fringe. Fringed lear (Bot.), a leaf edged with soft parallel hairs.",
  "impropitious - Unpropitious; unfavorable. [Obs.] \"Dreams were impropitious.\" Sir H. Wotton.",
  "narcissine - Of or pertaining to Narcissus.",
  "nullah - A water course, esp. a dry one; a gully; a gorge; -- orig. an East Indian term.  E. Arnold.",
  "panne - A fabric resembling velvet, but having the nap flat and less close.",
  "artilleryman - A man who manages, or assists in managing, a large gun in firing.",
  "transmissibility - The quality of being transmissible.",
  "spicenut - A small crisp cake, highly spiced.",
  "motherliness - The state or quality of being motherly.",
  "calcitrant - Kicking. Hence: Stubborn; refractory.",
  "leanly - Meagerly; without fat or plumpness.",
  "molinism - The doctrines of the Molinists, somewhat resembling the tenets of the Arminians.",
  "araby - The country of Arabia. [Archaic & Poetic]",
  "alchemically - In the manner of alchemy.",
  "zarathustrian - Of or pertaining to Zarathustra, or Zoroaster; Zoroastrian. Tylor.",
  "tactility - The quality or state of being tactile; perceptibility by touch; tangibleness.",
  "gladiatorian - Of or pertaining to gladiators, or to contests or combatants in general.",
  "pindarical - Pindaric. Too extravagant and Pindarical for prose. Cowley.",
  "multilobar - Consisting of, or having, many lobes.",
  "weakener - One who, or that which, weakens. \"[Fastings] weakeners of sin.\" South.",
  "tetragynian - Belonging to the order Tetragynia; having four styles.",
  "demiurgic - Pertaining to a demiurge; formative; creative. \"Demiurgic power.\" De Quincey.",
  "gallego - A native or inhabitant of Galicia, in Spain; a Galician.",
  "monte - A favorite gambling game among Spaniards, played with dice or cards.",
  "cembalo - An old mname for the harpsichord.",
  "furculum - The wishbone or merrythought of birds, formed by the united clavicles.",
  "hayrick - A heap or pile of hay, usually covered with thatch for preservation in the open air.",
  "mixtly - With mixture; in a mixed manner; mixedly. Bacon.",
  "refill - To fill, or become full, again.",
  "coagulative - Having the power to cause coagulation; as, a coagulative agent. Boyle.",
  "enchafe - To chafe; to enrage; to heat. [Obs.] Shak.",
  "avellane - In the form of four unhusked filberts; as, an avellane cross.",
  "unbeguile - To set free from the influence of guile; to undeceive. \"Then unbeguile thyself.\" Donne.",
  "leggiero - Light or graceful; in a light, delicate, and brick style.",
  "visional - Of or pertaining to a vision.",
  "mellification - The making or production of honey.",
  "unreaved - Not torn, split, or parted; not torn to pieces. [Obs.] Bp. Hall.",
  "padder - 1. One who, or that which, pads. 2. A highwayman; a footpad. [Obs.]",
  "gladship - A state of gladness. [Obs.] Gower.",
  "jacobinize - To taint with, or convert to, Jacobinism. France was not then jacobinized. Burke.",
  "retroverted - In a state of retroversion.",
  "archenteric - Relating to the archenteron; as, archenteric invagination.",
  "favaginous - Formed like, or resembling, a honeycomb.",
  "ribaudred - Filthy; obscene; ribald. [Obs.]",
  "vaporable - Capable of being converted into vapor by the agency of heat; vaporizable.",
  "dramshop - A shop or barroom where spirits are sold by the dram.",
  "fungivorous - Eating fungi; -- said of certain insects and snails.",
  "wieldsome - Admitting of being easily wielded or managed. [Obs.] Golding.",
  "unpossess - To be without, or to resign, possession of. [Obs.]",
  "capsicine - A valatile alkaloid extracted from Capsicum annuum or from capsicin.",
  "foutra - A fig; -- a word of contempt. [Obs.] A foutra for the world and wordlings base! Shak.",
  "anisocoria - Inequality of the pupils of the eye.",
  "aslake - To mitigate; to moderate; to appease; to abate; to diminish. [Archaic] Chaucer.",
  "isodimorphism - Isomorphism between the two forms severally of two dimorphous substances.",
  "plumigerous - Feathered; having feathers. Bailey",
  "resiniferous - Yielding resin; as, a resiniferous tree or vessel.",
  "covertly - Secretly; in private; insidiously.",
  "imperforate - Not perforated; having no opening or aperture. Sir J. Banks.",
  "indiscerptibility - The state or quality of being indiscerpible. [Obs.] Dr. H. More.",
  "couscousou - A favorite dish in Barbary. See Couscous.",
  "misbede - To wrong; to do injury to. [Obs.] Who hath you misboden or offended Chaucer.",
  "astromantic - Of or pertaining to divination by means of the stars; astrologic. [R.] Dr. H. More.",
  "epaxial - Above, or on the dorsal side of, the axis of the skeleton; episkeletal.",
  "bookstore - A store where books are kept for sale; -- called in England a bookseller's shop.",
  "footbridge - A narrow bridge for foot passengers only.",
  "panegyris - A festival; a public assembly. [Obs.] S. Harris.",
  "shail - To walk sidewise. [Obs.] L'Estrange.",
  "chambray - A gingham woven in plain colors with linen finish.",
  "decarbonate - To deprive of carbonic acid.",
  "water measurer - Any one of numerous species of water; the skater. See Skater, n., 2.",
  "amyous - Wanting in muscle; without flesh.",
  "tribble - A frame on which paper is dried. Knight.",
  "briefless - Having no brief; without clients; as, a briefless barrister.",
  "wawaskeesh - The wapiti, or wapiti, or American elk.",
  "epididymitis - Inflammation of the epididymis, one of the common results of gonorrhea.",
  "memorability - The quality or state of being memorable.",
  "operance - The act of operating or working; operation. [R.]",
  "concelebrate - To celebrate together. [Obs.] Holland.",
  "dismarshal - To disarrange; to derange; to put in disorder. [R.] Drummond.",
  "phillygenin - A pearly crystalline substance obtained by the decomposition of phillyrin.",
  "andabatism - Doubt; uncertainty. [Obs.] Shelford.",
  "insanely - Without reason; madly; foolishly.",
  "mint-master - The master or superintendent of a mint. Also used figuratively.",
  "encheson - Occasion, cause, or reason. [Obs.] Chaucer.",
  "puritanically - In a puritanical manner.",
  "dysprosium - An element of the rare earth-group. Symbol Dy; at. wt., 162.5.",
  "indorsor - The person who indorses. [Written also endorser.]",
  "propagator - One who propagates; one who continues or multiplies.",
  "faldfee - A fee or rent paid by a tenant for the privilege of faldage on his own ground. Blount.",
  "senhor - A Spanish title of courtesy corresponding to the English Mr. or Sir; also, a gentleman.",
  "pork - The flesh of swine, fresh or salted, used for food.",
  "arthrochondritis - Chondritis of a joint.",
  "nonacceptance - A neglect or refusal to accept.",
  "beduck - To duck; to put the head under water; to immerse. \"Deep himself beducked.\" Spenser.",
  "incorrespondency - Want of correspondence; disagreement; disproportion. [R.]",
  "overcapable - Too capable. [R.] Overcapable of such pleasing errors. Hooker.",
  "unsadness - Infirmity; weakness. [Obs.] Wyclif.",
  "undauntable - Incapable of being daunted; intrepid; fearless; indomitable. Bp. Hall.",
  "stomatoplasty - Plastic surgery of the mouth.",
  "bordrag - An incursion upon the borders of a country; a raid. [Obs.] Spenser.",
  "feodality - Feudal tenure; the feudal system. See Feudality. Burke.",
  "unsphere - To remove, as a planet, from its sphere or orb. Shak.",
  "leisured - Having leisure. \"The leisured classes.\" Gladstone.",
  "osphradium - The olfactory organ of some Mollusca. It is connected with the organ of respiration.",
  "sailboat - A boat propelled by a sail or sails.",
  "phyllite - (a) A mineral related to ottrelite. (b) Clay slate; argillaceous schist.",
  "salpinx - The Eustachian tube, or the Fallopian tube.",
  "siccative - Drying; causing to dry. -- n.  That which promotes drying.",
  "skiddaw - The black guillemot. [Prov. Eng.]",
  "pomely - Dappled. [Obs.] \"Pomely gray.\" Chaucer.",
  "suricat - Same as Zenick. [Written also suricate, surikate.]",
  "consigne - (a) A countersign; a watchword. (b) One who is orders to keep within certain limits.",
  "pavon - A small triangular flag, esp. one attached to a knight's lance; a pennon.",
  "beslime - To daub with slime; to soil. [Obs.] B. Jonson.",
  "irritatory - Exciting; producing irritation; irritating. [R.] Hales.",
  "unsubstantial - Lacking in matter or substance; visionary; chimerical.",
  "isochronon - A clock that is designed to keep very accurate time.",
  "peculator - One who peculates. \"Peculators of the public gold.\" Cowper.",
  "dissheathe - To become unsheathed. [Obs.] Sir W. Raleigh.",
  "roset - A red color used by painters. Peacham.",
  "deductor - The pilot whale or blackfish.",
  "to-beat - To beat thoroughly or severely. [Obs.] Layamon.",
  "avowee - The person who has a right to present to a benefice; the patron; an advowee. See Advowson.",
  "glasshouse - A house where glass is made; a commercial house that deals in glassware.",
  "weaponry - Weapons, collectively; as, an array of weaponry. [Poetic]",
  "recapacitate - To qualify again; to confer capacity on again. Atterbury.",
  "biangular - Having two angles or corners.",
  "sopsavine - See Sops of wine, under Sop.",
  "dodecagynian - Of or pertaining to the Dodecagynia; having twelve styles.",
  "conylene - An oily substance, C8H14, obtained from several derivatives of conine.",
  "footboy - A page; an attendant in livery; a lackey. Shak.",
  "dermatine - Of or pertaining to the skin.",
  "diuturnity - Long duration; lastingness. [R.] Sir T. Browne.",
  "consanguineal - Of the same blood; related by birth. Sir T. Browne.",
  "mummify - To embalm and dry as a mummy; to make into, or like, a mummy. Hall (1646).",
  "obituarily - In the manner of an obituary.",
  "marinorama - A representation of a sea view.",
  "fueler - One who, or that which, supplies fuel. [R.] [Written also fueller.] Donne.",
  "codille - A term at omber, signifying that the game is won. Pope.",
  "bitterling - A roachlike European fish (Rhodima amarus).",
  "hurst - A wood or grove; -- a word used in the composition of many names, as in Hazlehurst.",
  "raglan - A loose overcoat with large sleeves; -- named from Lord Raglan, an English general.",
  "bogberry - The small cranberry (Vaccinium oxycoccus), which grows in boggy places.",
  "intramundane - Being within the material world; -- opposed to extramundane.",
  "pettifogulize - To act as a pettifogger; to use contemptible tricks. De Quincey.",
  "croquette - A ball of minced meat, fowl, rice, or other ingredients, highly seasoned, and fried.",
  "holometer - An instrument for making of angular measurements.",
  "sacker - One who sacks; one who takes part in the storm and pillage of a town.",
  "unsuccess - Want of success; failure; misfortune. Prof. Wilson.",
  "apple-john - A kind of apple which by keeping becomes much withered; -- called also Johnapple. Shak.",
  "agriologist - One versed or engaged in agriology.",
  "fengite - A kind of marble or alabaster, sometimes used for windows on account of its transparency.",
  "mullock - Rubbish; refuse; dirt. [Obs.] All this mullok [was] in a sieve ythrowe. Chaucer.",
  "sea girkin - Any small holothurian resembling in form a gherkin.",
  "beam tree - A tree (Pyrus aria) related to the apple.",
  "quadragenarious - Consisting of forty; forty years old.",
  "subsequency - The act or state of following; -- opposed to precedence.",
  "grand-ducal - Of or pertaining to a grand duke. H. James.",
  "precentorship - The office of a precentor.",
  "saginate - To make fat; to pamper. [R.] \"Many a saginated boar.\" Cowper.",
  "monesin - The acrid principle of Monesia, sometimes used as a medicine.",
  "celestify - To make like heaven. [Obs.] Sir T. Browne.",
  "saxifragous - Dissolving stone, especially dissolving stone in the bladder.",
  "clubber - 1. One who clubs. 2. A member of a club. [R.] Massinger.",
  "methylamine - See Methyl amine, under Methyl.",
  "confrere - Fellow member of a fraternity; intimate associate.",
  "siserary - A hard blow. [Prov. Eng.] Halliwell.",
  "martialness - The quality of being martial.",
  "orkneyan - Of or pertaining to the Orkney islands. \"Orkneyan skerries.\" Longfellow.",
  "sermocinator - One who makes sermons or speeches. [Obs.] Howell.",
  "boteless - Unavailing; in vain. See Bootless.",
  "untrammeled - Not hampered or impeded; free. [Written also untrammelled.]",
  "apograph - A copy or transcript. Blount.",
  "hylozoic - Of or pertaining to hylozoism.",
  "floramour - The plant love-lies-bleeding. [Obs.] Prior.",
  "chode - the old imp. of chide. See Chide.",
  "peptogenous - Capable of yielding, or being converted into, peptone.",
  "balneatory - Belonging to a bath. [Obs.]",
  "somewhen - At some indefinite time. [R.]",
  "venenation - 1. The act of poisoning. 2. Poison; venom. [Obs.] Sir T. Browne.",
  "twelvescore - Twelve times twenty; two hundred and forty.",
  "violative - Violating, or tending to violate.",
  "tortulous - Swelled out at intervals like a knotted cord.",
  "inarticulation - Inarticulateness. Chesterfield.",
  "intoxicating - Producing intoxication; as, intoxicating liquors.",
  "superioress - A woman who acts as chief in a convent, abbey, or nunnery; a lady superior.",
  "water agrimony - A kind of bur marigold (Bidens tripartita) found in wet places in Europe.",
  "heliotroper - The person at a geodetic station who has charge of the heliotrope.",
  "triturable - Capable of being triturated. Sir T. Browne.",
  "great-hearted - 1. High-spirited; fearless. [Obs.] Clarendon. 2. Generous; magnanimous; noble.",
  "ossifrage - (a) The lammergeir. (b) The young of the sea eagle or bald eagle. [Obs.]",
  "ach - A name given to several species of plants; as, smallage, wild celery, parsley. [Obs.] Holland.",
  "hypocritely - Hypocritically. [R.] Sylvester.",
  "brachium - The upper arm; the segment of the fore limb between the shoulder and the elbow.",
  "phosphide - A binary compound of phosphorus.",
  "hypnoscope - An instrument for ascertaining the susceptibility of a person to hypnotic influences.",
  "tola - A weight of British India. The standard tola is equal to 180 grains.",
  "empawn - To put in pawn; to pledge; to impawn. To sell, empawn, and alienate the estates. Milman.",
  "toastmaster - A person who presides at a public dinner or banquet, and announces the toasts.",
  "inexplicably - In an inexplicable manner.",
  "aculeolate - Having small prickles or sharp points. Gray.",
  "whiningly - In a whining manner; in a tone of mean complaint.",
  "alphabetically - In an alphabetic manner; in the customary order of the letters.",
  "kuda - The East Indian tapir. See Tapir.",
  "thyrsoid - Having somewhat the form of a thyrsus.",
  "cantab - A Cantabrigian. [Colloq.] Sir W. Scott.",
  "unalmsed - Not having received alms. [Obs. & R.] Pollock.",
  "provinciate - To convert into a province or provinces. [Obs.] Howell.",
  "snarer - One who lays snares, or entraps.",
  "fermillet - A buckle or clasp. [Obs.] Donne.",
  "fatigation - Weariness. [Obs.] W. Montaqu.",
  "tinnient - Emitting a clear sound. [Obs.]",
  "introductorily - By way of introduction.",
  "auxanometer - An instrument to measure the growth of plants. Goodale.",
  "hypermyriorama - A show or exhibition having a great number of scenes or views.",
  "distantly - At a distance; remotely; with reserve.",
  "readjourn - To adjourn a second time; to adjourn again.",
  "cavalierism - The practice or principles of cavaliers. Sir. W. Scott.",
  "elephantiac - Affected with elephantiasis; characteristic of elephantiasis.",
  "gelatiniform - Having the form of gelatin.",
  "crinal - Of or pertaining to the hair. [R.] Blount.",
  "locky - Having locks or tufts. [R.] Sherwood.",
  "patentable - Suitable to be patented; capable of being patented.",
  "excito-motion - Motion excited by reflex nerves. See Excito-motory.",
  "songless - Destitute of the power of song; without song; as, songless birds; songless woods.",
  "rhythmless - Being without rhythm. Coleridge.",
  "immundicity - Uncleanness; filthness. [R.] W. Montagu.",
  "lation - Transportation; conveyance. [Obs.]",
  "unproselyte - To convert or recover from the state of a proselyte. Fuller.",
  "brachydome - A dome parallel to the shorter lateral axis. See Dome.",
  "airless - Not open to a free current of air; wanting fresh air, or communication with the open air.",
  "mitrailleur - One who serves a mitrailleuse.",
  "irishry - The Celtic people of Ireland. \"The whole Irishry of rebels.\" Milton.",
  "chatteration - The act or habit of chattering. [Colloq.]",
  "eyeless - Without eyes; blind. \"Eyeless rage.\" Shak.",
  "interbreed - To breed by crossing different stocks of animals or plants.",
  "trothless - Faitless; false; treacherous. Thrall to the faithless waves and trothless sky. Fairfax.",
  "nectarean - Resembling nectar; very sweet and pleasant. \"nectarean juice.\" Talfourd.",
  "car wheel - A flanged wheel of a railway car or truck.",
  "degravation - The act of making heavy. [Obs.] Bailey.",
  "by-interest - Self-interest; private advantage. Atterbury.",
  "scalloper - One who fishes for scallops.",
  "bullace - (a) A small European plum (Prunus communis, var. insitita). See Plum. (b) The bully tree.",
  "codder - A gatherer of cods or peas. [Obs. or Prov.] Johnson.",
  "bacterioscopist - One skilled in bacterioscopic examinations.",
  "gossaniferous - Containing or producing gossan.",
  "transgressional - Of pertaining to transgression; involving a transgression.",
  "futuritial - Relating to what is to come; pertaining to futurity; future. [R.]",
  "dogmatically - In a dogmatic manner; positively; magisterially.",
  "disgorgement - The act of disgorging; a vomiting; that which is disgorged. Bp. Hall.",
  "magnality - A great act or event; a great attainment. [Obs.] Sir T. Browne.",
  "spectatorial - Of or pertaining to a spectator. Addison.",
  "tomtit - (a) A titmouse, esp. the blue titmouse. [Prov.eng.] (b) The wren. [Prov.eng.]",
  "precoetanean - One contemporary with, but older than, another. [Obs.] Fuller.",
  "hypersecretion - Morbid or excessive secretion, as in catarrh.",
  "theurgical - Of or pertaining to theurgy; magical. Theurgic hymns, songs of incantation.",
  "dactylotheca - The scaly covering of the toes, as in birds.",
  "ichthyophthira - A division of copepod crustaceans, including numerous species parasitic on fishes.",
  "entwinement - A twining or twisting together or round; union. Bp. Hacket.",
  "interminated - Interminable; interminate; endless; unending. [Obs.] Akenside.",
  "semivocal - Of or pertaining to a semivowel; half cocal; imperfectly sounding.",
  "immarcescibly - Unfadingly. [Obs.] Bp. Hall.",
  "guttulous - In droplike form. [Obs.] In its [hail's] guttulous descent from the air. Sir T. Browne.",
  "sternocoracoid - Of or pertaining to the sternum and the coracoid.",
  "daubreelite - A sulphide of chromium observed in some meteoric irons.",
  "walkable - Fit to be walked on; capable of being walked on or over. [R.] Swift.",
  "trithionate - A salt of trithionic acid.",
  "delassation - Fatigue. Able to continue without delassation. Ray.",
  "synthermal - Having the same degree of heat.",
  "preludious - Preludial. [R.] Dr. H. More.",
  "restringency - Quality or state of being restringent; astringency. [Obs.] Sir W. Petty.",
  "consertion - Junction; adaptation [R.] Consertion of design, how exquisite. Young.",
  "modulator - One who, or that which, modulates. Denham.",
  "dipetalous - Having two petals; two-petaled.",
  "mandragorite - One who habitually intoxicates himself with a narcotic obtained from mandrake.",
  "jasponyx - An onyx, part or all of whose layers consist of jasper.",
  "outdraw - To draw out; to extract. [R.] \"He must the teeth outdraw.\" Gower.",
  "arbustive - Containing copses of trees or shrubs; covered with shrubs. Bartram.",
  "aesculapius - The god of medicine. Hence, a physician.",
  "biscayan - Of or pertaining to Biscay in Spain. -- n.  A native or inhabitant of Biscay.",
  "rachitic - Of or pertaining to rachitis; affected by rachitis; rickety.",
  "bullon - A West Indian fish (Scarus Croicensis).",
  "lutarious - Of, pertaining to, or like, mud; living in mud. [Obs.] Grew.",
  "goring cloth - A piece of canvas cut obliquely to widen a sail at the foot.",
  "tenacy - Tenaciousness; obstinacy. [Obs.] Barrow.",
  "sleepiness - The quality or state of being sleepy.",
  "salamstone - A kind of blue sapphire brought from Ceylon. Dana.",
  "gaulish - Pertaining to ancient France, or Gaul; Gallic. [R.]",
  "polyarchy - A government by many persons, of whatever order or class. Cudworth.",
  "equivorous - Feeding on horseflesh; as, equivorous Tartars.",
  "steatomatous - Of the nature of steatoma.",
  "gutturalism - The quality of being guttural; as, the gutturalism of A [in the 16th cent.] Earle.",
  "accompanier - He who, or that which, accompanies. Lamb.",
  "mythopoetic - Making or producing myths or mythical tales.",
  "ahoy - A term used in hailing; as, \"Ship ahoy.\"",
  "abacist - One who uses an abacus in casting accounts; a calculator.",
  "permansion - Continuance. [Obs.] Sir T. Browne.",
  "augustine - A member of one of the religious orders called after St. Augustine; an Austin friar.",
  "alcohometer - Same as Alcoholometer, Alcoholometric.",
  "hornify - To horn; to cuckold. [Obs.] Beau. & Fl.",
  "zincographical - Of or pertaining to zincography; as, zincographic processes.",
  "laborless - Not involving labor; not laborious; easy.",
  "plashoot - A hedge or fence formed of branches of trees interlaced, or plashed. [Obs.] Carew.",
  "self-affrighted - Frightened at or by one's self. Shak.",
  "alpen - Of or pertaining to the Alps. [R.] \"The Alpen snow.\" J. Fletcher.",
  "painim - A pagan; an infidel; -- used also adjectively. [Written also panim and paynim.] Peacham.",
  "mundificative - Cleansing. -- n.  A detergent medicine or preparation.",
  "cillosis - A spasmodic trembling of the upper eyelid.",
  "blastogenesis - Multiplication or increase by gemmation or budding.",
  "gayal - A Southern Asiatic species of wild cattle (Bibos frontalis).",
  "macartney - A fire-backed pheasant. See Fireback.",
  "prezygapophysis - An anterior zygapophysis.",
  "suleah fish - A coarse fish of India, used in making a breakfast relish called burtah.",
  "surviving - Remaining alive; yet living or existing; as, surviving friends; surviving customs.",
  "intransmutable - Not capable of being transmuted or changed into another substance.",
  "inviolability - The quality or state of being inviolable; inviolableness.",
  "superconception - Superfetation. [R.] Sir T. Browne.",
  "inhearse - To put in, or as in, a hearse or coffin. Shak.",
  "transmarine - Lying or being beyond the sea. Howell.",
  "exsuscitate - To rouse; to excite. [Obs.] Johnson.",
  "debark - To go ashore from a ship or boat; to disembark; to put ashore.",
  "fiddlestring - One of the catgut strings of a fiddle.",
  "heraldically - In an heraldic manner; according to the rules of heraldry.",
  "piscinal - Belonging to a fishpond or a piscina.",
  "tooter - One who toots; one who plays upon a pipe or horn. B. Jonson.",
  "dejecta - Excrements; as, the dejecta of the sick.",
  "meteorographic - Of or pertaining to meteorography.",
  "retrogradingly - By retrograding; so as to retrograde.",
  "maledicent - Speaking reproachfully; slanderous. [Obs.] Sir E. Sandys.",
  "crassament - A semisolid mass or clot, especially that formed in coagulation of the blood.",
  "mannerchor - A German men's chorus or singing club.",
  "energumen - One possessed by an evil spirit; a demoniac.",
  "covinous - Deceitful; collusive; fraudulent; dishonest.",
  "double-eyed - Having a deceitful look. [R.] \"Deceitful meanings is double- eyed.\" Spenser.",
  "ex-official - Proceeding from office or authority.",
  "imploration - The act of imploring; earnest supplication. Bp. Hall.",
  "vomiting - The spasmodic ejection of matter from the stomach through the mouth.",
  "petuntze - Powdered fledspar, kaolin, or quartz, used in the manufacture of porcelain.",
  "recognosce - To recognize. [R. & Obs.] Boyle.",
  "acting - 1. Operating in any way. 2. Doing duty for another; officiating; as, an superintendent.",
  "compacted - Compact; pressed close; concentrated; firmly united.",
  "workwise - In a working position or manner; as, a T rail placed workwise, i.e., resting on its base.",
  "ecostate - Having no ribs or nerves; -- said of a leaf.",
  "oligarch - A member of an oligarchy; one of the rulers in an oligarchical government.",
  "agreeingly - In an agreeing manner (to); correspondingly; agreeably. [Obs.]",
  "iriscope - A philosophical toy for exhibiting the prismatic tints by means of thin films.",
  "repristination - Restoration to an original state; renewal of purity. [R.] R. Browning.",
  "cornemuse - A wind instrument nearly identical with the bagpipe. Drayton.",
  "incorrupted - Uncorrupted. [Obs.] Breathed into their incorrupted breasts. Sir J. Davies.",
  "another-guess - Of another sort. [Archaic] It used to go in another-guess manner. Arbuthnot.",
  "waferer - A dealer in the cakes called wafers; a confectioner. [Obs.] Chaucer.",
  "irenical - Fitted or designed to promote peace; pacific; conciliatory; peaceful. Bp. Hall.",
  "labially - In a labial manner; with, or by means of, the lips.",
  "preaortic - In front, or on the ventral side, of the aorta.",
  "preseance - Priority of place in sitting.[Obs.] Carew.",
  "tubercularize - To infect with tuberculosis. --Tu*ber`cu*lar*i*za\"tion (#), n.",
  "guesswork - Work performed, or results obtained, by guess; conjecture.",
  "barycentric - Of or pertaining to the center of gravity. See Barycentric calculus, under Calculus.",
  "vambrace - The piece designed to protect the arm from the elbow to the wrist.",
  "biparietal - Of or pertaining to the diameter of the cranium, from one parietal fossa to the other.",
  "cliquism - The tendency to associate in cliques; the spirit of cliques.",
  "bheestie - A water carrier, as to a household or a regiment. [India]",
  "saltcat - A mixture of salt, coarse meal lime, etc., attractive to pigeons.",
  "unspike - To remove a spike from, as from the vent of a cannon.",
  "edenic - Of or pertaining to Eden; paradisaic. \"Edenic joys.\" Mrs. Browning.",
  "directorship - The condition or office of a director; directorate.",
  "overread - To read over, or peruse. Shak.",
  "laas - A lace. See Lace. [Obs.] Chaucer.",
  "tetrahedrally - In a tetrahedral manner.",
  "interlineary - Interlinear. -- n.  A book containing interlineations. [R.]",
  "uncheckable - Not capable of being checked or stopped. [R.]",
  "lathreeve - Formerly, the head officer of a lathe. See 1st Lathe.",
  "snead - 1. A snath. 2. A line or cord; a string. [Prov. Eng.]",
  "shadiness - , n. Quality or state of being shady.",
  "abominableness - The quality or state of being abominable; odiousness. Bentley.",
  "euhemerist - One who advocates euhemerism.",
  "onomatope - An imitative word; an onomatopoetic word.",
  "soother - One who, or that which, soothes.",
  "foamy - Covered with foam; frothy; spumy. Behold how high the foamy billows ride! Dryden.",
  "negotiatory - Of or pertaining to negotiation.",
  "amort - As if dead; lifeless; spiritless; dejected; depressed. Shak.",
  "isostemonous - Having exactly as many stamens as petals.",
  "bluffy - 1. Having bluffs, or bold, steep banks. 2. Inclined to bo bluff; brusque.",
  "aloneness - A state of being alone, or without company; solitariness. [R.] Bp. Montagu.",
  "zain - A horse of a dark color, neither gray nor white, and having no spots. Smart.",
  "sulciform - Having the form of a sulcus; as, sulciform markings.",
  "olivaceous - Resembling the olive; of the color of the olive; olive-green.",
  "absinthial - Of or pertaining to wormwood; absinthian.",
  "hydromel - A liquor consisting of honey diluted in water, and after fermentation called mead.",
  "hurkaru - In India, a running footman; a messenger. [Written also hurkaroo.]",
  "reciprocality - The quality or condition of being reciprocal; reciprocalness. [R.]",
  "arrantly - Notoriously, in an ill sense; infamously; impudently; shamefully. L'Estrange.",
  "feoffer - One who enfeoffs or grants a fee.",
  "motility - Capability of motion; contractility.",
  "photomicrography - The art of producing photomicrographs.",
  "pupelo - Cider brandy. [Local, U. S.] Bartlett.",
  "marly - Consisting or partaking of marl; resembling marl; abounding with marl.",
  "lisne - A cavity or hollow.[Obs.] Sir M. Hale.",
  "orpharion - An old instrument of the lute or cittern kind. [Spelt also orpheoreon.]",
  "magdaleon - A medicine in the form of a roll, a esp. a roll of plaster.",
  "collow - Soot; smut. See 1st Colly. [Obs.]",
  "biocellate - Having two ocelli (eyelike spots); -- said of a wing, etc.",
  "spirituelle - Of the nature, or having the appearance, of a spirit; pure; refined; ethereal.",
  "moton - A small plate covering the armpit in armor of the 14th century and later.",
  "primatical - Of or pertaining to a primate. Barrow.",
  "tautoousious - Having the same essence; being identically of the same nature. [R.] Cudworth.",
  "footpad - A highwayman or robber on foot.",
  "calmer - One who, or that which, makes calm.",
  "protozoan - Of or pertaining to the Protozoa. -- n.  One of the Protozoa.",
  "coverage - The aggregate of risks covered by the terms of a contract of insurance.",
  "immenseness - The state of being immense.",
  "definitely - In a definite manner; with precision; precisely; determinately.",
  "spiration - The act of breathing. [Obs.] Barrow.",
  "salue - To salute. [Obs.] There was no \"good day\" and no saluyng. Chaucer.",
  "negotiosity - The state of being busy; multitude of business. [Obs.]",
  "parfay - By my faith; verily. [Obs.] Chaucer.",
  "re-mark - To mark again, or a second time; to mark anew.",
  "icosahedral - Having twenty equal sides or faces.",
  "fimble hemp - Light summer hemp, that bears no seed.",
  "sciagraphical - Pertaining to sciagraphy. -- Sci`a*graph\"ic*al*ly, adv.",
  "waterpot - A vessel for holding or conveying water, or for sprinkling water on cloth, plants, etc.",
  "ablaqueate - To lay bare, as the roots of a tree. [Obs.] Bailey.",
  "loverwise - As lovers do. As they sat down here loverwise. W. D. Howells.",
  "apomorphine - A crystalline alkaloid obtained from morphia. It is a powerful emetic.",
  "acrasy - Excess; intemperance. [Obs. except in Med.] Farindon.",
  "stomatoda - A division of Protozoa in which a mouthlike opening exists.",
  "lamination - The process of laminating, or the state of being laminated.",
  "pipewort - Any plant of a genus (Eriocaulon) of aquatic or marsh herbs with soft grass-like leaves.",
  "somberness - The quality or state of being somber; gloominess.",
  "possessionary - Of or pertaining to possession; arising from possession.",
  "circumnavigate - To sail completely round. Having circumnavigated the whole earth. T. Fuller.",
  "homiform - In human form. [Obs.] Cudworth.",
  "reachless - Being beyond reach; lofty. Unto a reachless pitch of praises hight. Bp. Hall.",
  "good-humoredly - With a cheerful spirit; in a cheerful or good-tempered manner.",
  "strangurious - Of or pertaining to strangury. Cheyne.",
  "associateship - The state of an associate, as in Academy or an office.",
  "ambo - A large pulpit or reading desk, in the early Christian churches. Gwilt.",
  "bedrite - The duty or privilege of the marriage bed. Shak.",
  "brachioganoid - One of the Brachioganoidei.",
  "unencumber - To free from incumbrance; to disencumber.",
  "antimetathesis - An antithesis in which the members are repeated in inverse order.",
  "interosseal - Situated between bones; as, an interosseous ligament.",
  "plebicolist - One who flatters, or courts the favor of, the common people; a demagogue. [R.]",
  "tombester - A female dancer. [Obs.] Chaucer.",
  "inquisitorious - Making strict inquiry; inquisitorial. [Obs.] Milton.",
  "lipaemia - A condition in which fat occurs in the blood.",
  "maguari - A South American stork (Euxenara maguari), having a forked tail.",
  "vitrification - Same as Vitrifaction. Sir T. Browne. Ure.",
  "doorstone - The stone forming a threshold.",
  "gnomonist - One skilled in gnomonics. Boyle.",
  "palmiped - Web-footed, as a water fowl. -- n.  A swimming bird; a bird having webbed feet.",
  "objector - One who objects; one who offers objections to a proposition or measure.",
  "paintership - The state or position of being a painter. [R.] Br. Gardiner.",
  "sweepage - The crop of hay got in a meadow. [Prov. Eng.]",
  "styca - An anglo-Saxon copper coin of the lowest value, being worth half a farthing. S. M. Leake.",
  "allotriophagy - A depraved appetite; a desire for improper food.",
  "exitious - Destructive; fatal. [Obs.] \"Exitial fevers.\" Harvey.",
  "coelenterate - Belonging to the Coelentra. -- n.  One of the Coelentera.",
  "kahani - A kind of notary public, or attorney, in the Levant.",
  "monodynamic - Possessing but one capacity or power. \"Monodynamic men.\" De Quincey.",
  "revindicate - To vindicate again; to reclaim; to demand and take back. Mitford.",
  "squalidity - The quality or state of being squalid; foulness; filthiness.",
  "discardure - Rejection; dismissal. [R.] Hayter.",
  "beadhouse - An almshouse for poor people who pray daily for their benefactors.",
  "lowborn - Born in a low condition or rank; -- opposed to highborn.",
  "currently - In a current manner; generally; commonly; as, it is currently believed.",
  "quackish - Like a quack; boasting; characterized by quackery. Burke.",
  "surnominal - Of or pertaining to a surname or surnames.",
  "equiradical - Equally radical. [R.] Coleridge.",
  "carucage - 1. (Old Eng. Law.)  A tax on every plow or plowland. 2. The act of plowing. [R.]",
  "fielding - The act of playing as a fielder.",
  "assibilate - To make sibilant; to change to a sibilant. J. Peile.",
  "horologiographic - Of or pertaining to horologiography. Chambers.",
  "northerliness - The quality or state of being northerly; direction toward the north.",
  "tentory - The awning or covering of a tent. [Obs.] Evelyn.",
  "goodyship - The state or quality of a goody or goodwife [Jocose] Hudibraus.",
  "uncivilization - The state of being uncivilized; savagery or barbarism. [R.]",
  "thelytokous - Producing females only; -- said of certain female insects.",
  "princified - Imitative of a prince. [R. & Colloq.] Thackeray.",
  "hebraic - Of or pertaining to the Hebrews, or to the language of the Hebrews.",
  "citizenship - The state of being a citizen; the status of a citizen.",
  "trochisk - See Trochiscus. [Obs.] Bacon.",
  "disconsolacy - The state of being disconsolate. [Obs.] Barrow.",
  "footlicker - A sycophant; a fawner; a toady. Cf. Bootlick. Shak.",
  "originable - Capable of being originated.",
  "anagogics - Mystical interpretations or studies, esp. of the Scriptures. L. Addison.",
  "closefisted - Covetous; niggardly. Bp. Berkeley. \"Closefisted contractors.\" Hawthorne.",
  "self-restrained - Restrained by one's self or itself; restrained by one's own power or will.",
  "adulate - To flatter in a servile way. Byron.",
  "stromeyerite - A steel-gray mineral of metallic luster. It is a sulphide of silver and copper.",
  "souvenir - That which serves as a reminder; a remembrancer; a memento; a keepsake.",
  "psalmodist - One who sings sacred songs; a psalmist.",
  "ecphoneme - A mark (!) used to indicate an exclamation. G. Brown.",
  "proximately - In a proximate manner, position, or degree; immediately.",
  "caulis - An herbaceous or woody stem which bears leaves, and may bear flowers.",
  "impignorate - To pledge or pawn. [Obs.] Laing.",
  "edgeshot - Having an edge planed, -- said of a board. Knight.",
  "pettywhin - The needle furze. See under Needle.",
  "filical - Belonging to the Filices, r ferns.",
  "shirt-waist suit - A costume consisting of a plain belted waist and skirt of the same material.",
  "persuasibility - Capability of being persuaded. Hawthorne.",
  "timesaving - Saving time; as, a timesaving expedient.",
  "lentiginose - Bearing numerous dots resembling freckles.",
  "pyritical - Of or pertaining to pyrites; consisting of, or resembling, pyrites.",
  "murphy - A potato. [Humorous] Thackeray.",
  "refractable - Capable of being refracted.",
  "evangelism - The preaching or promulgation of the gospel. Bacon.",
  "pedagogical - Of or pertaining to a pedagogue; suited to, or characteristic of, a pedagogue.",
  "eremitism - The state of a hermit; a living in seclusion from social life.",
  "bibble-babble - Idle talk; babble. Shak.",
  "polyadelphous - Belonging to the class Polyadelphia; having stamens united in three or more bundles.",
  "meiostemonous - Having fever stamens than the parts of the corolla.",
  "fascicule - A small bunch or bundle; a fascicle; as, a fascicule of fibers, hairs, or spines.",
  "ginny-carriage - A small, strong carriage for conveying materials on a railroad. [Eng.]",
  "patcher - One who patches or botches. Foxe.",
  "knitback - The plant comfrey; -- so called from its use as a restorative. Dr. Prier.",
  "disgracive - Disgracing. [Obs.] Feltham.",
  "accustomably - According to custom; ordinarily; customarily. Latimer.",
  "osteologist - One who is skilled in osteology; an osteologer.",
  "cruels - Glandular scrofulous swellings in the neck.",
  "talcose - Of or pertaining to talc; composed of, or resembling, talc.",
  "abraid - To awake; to arouse; to stir or start up; also, to shout out. [Obs.] Chaucer.",
  "lit - , a form of the imp. & p. p. of Light.",
  "cerealia - 1. (Antiq.)  Public festivals in honor of Ceres. 2. The cereals. Crabb.",
  "subconjunctival - Situated under the conjunctiva.",
  "fleawort - An herb used in medicine (Plantago Psyllium), named from the shape of its seeds. Loudon.",
  "semilenticular - Half lenticular or convex; imperfectly resembling a lens. Kirwan.",
  "concolor - Of the same color; of uniform color. [R.] \"Concolor animals.\" Sir T. Browne.",
  "antepaschal - Pertaining to the time before the Passover, or before Easter.",
  "catty - An East Indian Weight of 11/3 pounds.",
  "isochronism - The state or quality of being isochronous.",
  "turfen - Made of turf; covered with turf.",
  "fixure - Fixed position; stable condition; firmness. [Obs.] Shak.",
  "gleby - Pertaining to the glebe; turfy; cloddy; fertile; fruitful. \"Gleby land.\" Prior.",
  "spikebill - (a) The hooded merganser. (b) The marbled godwit (Limosa fedoa).",
  "trinkle - To act secretly, or in an underhand way; to tamper. [Obs.] Wright.",
  "pretemporal - Situated in front of the temporal bone.",
  "tribuneship - The office or power of a tribune.",
  "otoscope - An instrument for examining the condition of the ear.",
  "merchandry - Trade; commerce. [Obs.] Bp. Sanderson.",
  "cilice - A kind of haircloth undergarment. Southey.",
  "debased - Turned upside down from its proper position; inverted; reversed.",
  "foliature - 1. Foliage; leafage. [Obs.] Shuckford. 2. The state of being beaten into foil. Johnson.",
  "roue - One devoted to a life of sensual pleasure; a debauchee; a rake.",
  "tumescence - The act of becoming tumid; the state of being swollen; intumescence.",
  "epicarican - An isopod crustacean, parasitic on shrimps.",
  "self-assertive - Disposed to self-assertion; self-asserting.",
  "whipster - A nimble little fellow; a whippersnapper. Every puny whipster gets my sword. Shak.",
  "self-satisfied - Satisfied with one's self or one's actions; self-complacent.",
  "brassiness - The state, conditions, or quality of being brassy. [Colloq.]",
  "anthropopathite - One who ascribes human feelings to deity.",
  "sloppiness - The quality or state of being sloppy; muddiness.",
  "avant - The front of an army. [Obs.] See Van.",
  "post-tympanic - Situated behind the tympanum, or in the skull, behind the auditory meatus.",
  "cross-week - Rogation week, when the cross was borne in processions.",
  "gaoler - The keeper of a jail. See Jailer.",
  "wetbird - The chaffinch, whose cry is thought to foretell rain. [Prov. Eng.]",
  "foreknowingly - With foreknowledge. He who . . . foreknowingly loses his life. Jer. Taylor.",
  "arborist - One who makes trees his study, or who is versed in the knowledge of trees. Howell.",
  "gastrotrocha - A form of annelid larva having cilia on the ventral side.",
  "plaintless - Without complaint; unrepining. \"Plaintless patience.\" Savage.",
  "khanate - Dominion or jurisdiction of a khan.",
  "remercy - To thank. [Obs.] She him remercied as the patron of her life. Spenser.",
  "limitive - Involving a limit; as, a limitive law, one designed to limit existing powers. [R.]",
  "rochelime - Lime in the lump after it is burned; quicklime. [Eng.]",
  "bibliopolar - Of or pertaining to the sale of books. \"Bibliopolic difficulties.\" Carlyle.",
  "honestetee - Honesty; honorableness. [Obs.] Chaucer.",
  "puppyism - Extreme meanness, affectation, conceit, or impudence. A. Chalmers.",
  "unnotify - To retract or withdraw a notice of. Walpole.",
  "nonmembership - State of not being a member.",
  "scalpel - A small knife with a thin, keen blade, -- used by surgeons, and in dissecting.",
  "diffluence - A flowing off on all sides; fluidity. [R.]",
  "zebrine - Pertaining to, or resembling, the zebra.",
  "extemporanean - Extemporaneous. [Obs] Burton.",
  "omicron - Lit., the little, or short, O, o; the fifteenth letter of the Greek alphabet.",
  "aerodonetics - The science of gliding and soaring flight.",
  "enigmatography - The art of making or of solving enigmas.",
  "undergroan - To groan beneath. [Obs.] Earth undergroaned their high-raised feet. Chapman.",
  "togated - Dressed in a toga or gown; wearing a gown; gowned. [R.] Sir M. Sandys.",
  "geminiflorous - Having the flowers arranged in pairs.",
  "signorina - Miss; -- a title of address among the Italians.",
  "hatstand - A stand of wood or iron, with hooks or pegs upon which to hang hats, etc.",
  "pinnothere - A crab of the genus pinnotheres. See Oyster crab, under Oyster.",
  "congeable - Permissible; done lawfully; as, entry congeable.",
  "accommodable - That may be accommodated, fitted, or made to agree. [R.] I. Watts.",
  "hep - See Hip, the fruit of the dog-rose.",
  "lobately - As a lobe; so as to make a lobe; in a lobate manner.",
  "satyrical - Of or pertaining to satyrs; burlesque; as, satyric tragedy. P. Cyc.",
  "dolus - Evil intent, embracing both malice and fraud. See Culpa. Wharton.",
  "outtongue - To silence by talk, clamor, or noise. [R.] Shak.",
  "accresce - 1. To accrue. [R.] 2. To increase; to grow. [Obs.] Gillespie.",
  "succinyl - A hypothetical radical characteristic of succinic acid and certain of its derivatives.",
  "foul-spoken - Using profane, scurrilous, slanderous, or obscene language. Shak.",
  "interstition - An intervening period of time; interval. [Obs.] Gower.",
  "epipodite - The outer branch of the legs in certain Crustacea. See Maxilliped.",
  "clamjamphrie - Low, worthless people; the rabble. [Scot.] Jamieson.",
  "unpicked - Picked out; picked open. 2. Etym: [Pref. un- not + picked.]  Not picked. Milton.",
  "baldly - Nakedly; without reserve; inelegantly.",
  "cream-colored - Of the color of cream; light yellow. \"Cream-colored horses.\" Hazlitt.",
  "sybaritism - Luxuriousness; effeminacy; wantonness; voluptuousness.",
  "anakim - A race of giants living in Palestine.",
  "outthrow - 1. To throw out. Spenser. 2. To excel in throwing, as in ball playing.",
  "headily - In a heady or rash manner; hastily; rashly; obstinately.",
  "succision - The act of cutting down, as of trees; the act of cutting off. [R.]",
  "castrensial - Belonging to a camp. Sir T. Browne.",
  "pyromalate - A salt of pyromalic acid. [Obs.]",
  "goniometrical - Pertaining to, or determined by means of, a goniometer; trigonometric.",
  "predal - Of or pertaining to prey; plundering; predatory. [R.] Boyse.",
  "reverencer - One who regards with reverence. \"Reverencers of crowned heads.\" Swift.",
  "allomerous - Characterized by allomerism.",
  "unapparel - To divest of clothing; to strip. [Obs.] Donne.",
  "diphtheric - Relating to diphtheria; diphtheritic.",
  "rejective - Rejecting, or tending to reject.",
  "rudderhole - The hole in the deck through which the rudderpost passes.",
  "sanhita - A collection of vedic hymns, songs, or verses, forming the first part of each Veda.",
  "meekness - The quality or state of being meek.",
  "procreator - One who begets; a father or sire; a generator.",
  "playsome - Playful; wanton; sportive. [R.] R. Browning. -- Play\"some*ness, n. [R.]",
  "reciter - One who recites; also, a book of extracts for recitation.",
  "livelode - Course of life; means of support; livelihood. [Obs.]",
  "carnification - The act or process of turning to flesh, or to a substance resembling flesh.",
  "obcompressed - Compressed or flattened antero-posteriorly, or in a way opposite to the usual one.",
  "stallon - A slip from a plant; a scion; a cutting. [R.] Holished.",
  "cometarium - An instrument, intended to represent the revolution of a comet round the sun. Hutton.",
  "hurly - Noise; confusion; uproar. That, with the hurly, death itself awakes. Shak.",
  "unbutton - To loose the buttons of; to unfasten.",
  "oomiak - A long, broad boat used by the Eskimos.",
  "girtline - A gantline. Hammock girtline, a line rigged for hanging out hammocks to dry.",
  "pronouncing - Pertaining to, or indicating, pronunciation; as, a pronouncing dictionary.",
  "respirative - Of or pertaining to respiration; as, respirative organs.",
  "interess - To interest or affect. [Obs.] Hooker.",
  "dilative - Causing dilation; tending to dilate, on enlarge; expansive. Coleridge.",
  "day-labor - Labor hired or performed by the day. Milton.",
  "appositional - Pertaining to apposition; put in apposition syntactically. Ellicott.",
  "nothingness - 1. Nihility; nonexistence. 2. The state of being of no value; a thing of no value.",
  "antistrumous - Good against scrofulous disorders. Johnson. Wiseman.",
  "unnethe - With difficulty. See Uneath. [Obs.] Chaucer.",
  "odonata - The division of insects that includes the dragon flies.",
  "draughthouse - A house for the reception of waste matter; a privy. [Obs.] 2 Kings x. 27.",
  "missheathed - Sheathed by mistake; wrongly sheathed; sheathed in a wrong place. Shak.",
  "ergmeter - An instrument for measuring energy in ergs.",
  "querimony - A complaint or complaining. [Obs.] E. Hall.",
  "magnanimously - In a magnanimous manner; with greatness of mind.",
  "unipersonalist - One who believes that the Deity is unipersonal.",
  "double-charge - 1. To load with a double charge, as of gunpowder. 2. To overcharge. Shak.",
  "zed - The letter Z; -- called also zee, and formerly izzard. \"Zed, thou unnecessary letter!\" Shak.",
  "dispurpose - To dissuade; to frustrate; as, to dispurpose plots. [R.] A. Brewer.",
  "judicable - Capable of being judged; capable of being tried or decided upon. Jer. Taylor.",
  "nonconstat - It does not appear; it is not plain or clear; it does not follow.",
  "falsifiable - Capable of being falsified, counterfeited, or corrupted. Johnson.",
  "lionlike - Like a lion; brave as a lion.",
  "thack - See Thatch, Thatcher. [Obs. or Prov. Eng. & Scot.]",
  "hypocrateriform - hypocraterimorphous; salver-shaped. Wood.",
  "aduncity - Curvature inwards; hookedness. The aduncity of the beaks of hawks. Pope.",
  "valerone - A ketone of valeric acid obtained as an oily liquid.",
  "callithumpian - Of, pertaining to, or resembling, a callithump. [U. S.]",
  "decretorial - Decretory; authoritative. Sir T. Browne.",
  "anteal - Being before, or in front. [R.] J. Fleming.",
  "oscinine - Of or pertaining to the Oscines.",
  "parer - One who, or that which, pares; an instrument for paring.",
  "interfluent - Flowing between or among; intervening. Boyle.",
  "gummosity - Gumminess; a viscous or adhesive quality or nature. [R.] Floyer.",
  "tool-stock - The part of a toolrest in which a cutting tool is clamped.",
  "hackneyman - A man who lets horses and carriages for hire.",
  "deturpation - A making foul. [Obs.] Jer. Taylor.",
  "pecuniary - 1. Relating to money; monetary; as, a pecuniary penalty; a pecuniary reward. Burke.",
  "weighlock - A lock, as on a canal, in which boats are weighed and their tonnage is settled.",
  "sabulosity - The quality of being sabulous; sandiness; grittiness.",
  "bronchitis - Inflammation, acute or chronic, of the bronchial tubes or any part of them.",
  "sparhawk - The sparrow hawk. [Prov. Eng.]",
  "stannofluoride - Any one of a series of double fluorides of tin (stannum) and some other element.",
  "intercurrence - A passing or running between; occurrence. Boyle.",
  "polyphaser - A machine generating more than one pressure wave; a multiphaser.",
  "jambeux - In the Middle Ages, armor for the legs below the knees. [Written also giambeux.] Chaucer.",
  "sapientize - To make sapient. [R.] Coleridge.",
  "uprouse - To rouse up; to rouse from sleep; to awake; to arouse. Shak.",
  "vaccinal - Of or pertaining to vaccinia or vaccination.",
  "zootrophic - Of or pertaining to the nourishment of animals.",
  "stratus - A form of clouds in which they are arranged in a horizontal band or layer. See Cloud.",
  "diaphane - A woven silk stuff with transparent and colored figures; diaper work.",
  "batty - Belonging to, or resembling, a bat. \"Batty wings.\" Shak.",
  "overeager - Too eager; too impatient. -- O`ver*ea\"ger*ly, adv. -- O\"ver*ea\"ger*ness, n.",
  "tretable - Tractable; moderate. [Obs.] By nature debonaire and tretable. Chaucer.",
  "dastardliness - The quality of being dastardly; cowardice; base fear.",
  "lustring - A kind of glossy silk fabric. See Lutestring.",
  "gablock - A false spur or gaff, fitted on the heel of a gamecock. Wright.",
  "upstream - Toward the higher part of a stream; against the current.",
  "mammal - One of the Mammalia. Age of mammals. See under Age, n., 8.",
  "thermometrically - In a thermometrical manner; by means of a thermometer.",
  "suiogoths - The Scandinavian Goths. See the Note under Goths.",
  "lichenologist - One versed in lichenology.",
  "coverlid - A coverlet. All the coverlid was clocth of gold. Tennyson.",
  "delinition - A smearing. [Obs.] Dr. H. More.",
  "perversity - The quality or state of being perverse; perverseness.",
  "trumpeting - A channel cut behind the brick lining of a shaft. Raymond.",
  "febriferous - Causing fever; as, a febriferous locality.",
  "frumentarious - Of or pertaining to wheat or grain. [R.] Coles.",
  "redistribute - To distribute again. -- Re*dis`tri*bu\"tion (-tr, n.",
  "crippled - Lamed; lame; disabled; impeded. \"The crippled crone.\" Longfellow.",
  "comedienne - A women who plays in comedy.",
  "terminological - Of or pertaining to terminology. -- Ter`mi*no*log\"ic*al*ly, adv.",
  "staphylotomy - The operation of removing a staphyloma by cutting.",
  "darkener - One who, or that which, darkens.",
  "prologize - To deliver a Prologue. [R.] Whewell.",
  "expostulator - One who expostulates. Lamb.",
  "sprack - Quick; lively' alert. [Prov. Eng. & Scot.]",
  "didacticism - The didactic method or system.",
  "fibred - Having fibers; made up of fibers.",
  "postposit - To postpone. [Obs.] Feltham.",
  "litheness - The quality or state of being lithe; flexibility; limberness.",
  "insurmountability - The state or quality of being insurmountable.",
  "oryza - A genus of grasses including the rice plant; rice.",
  "sanctificate - To sanctify. [Obs.] Barrow.",
  "pouch-mouthed - Having a pouch mouth; blobber-lipped.",
  "columned - Having columns. Troas and Ilion's columned citadel. Tennyson.",
  "adorner - He who, or that which, adorns; a beautifier.",
  "prosternum - The ventral plate of the prothorax of an insect.",
  "pronator - A muscle which produces pronation.",
  "portcrayon - A metallic handle with a clasp for holding a crayon.",
  "camleted - Wavy or undulating like camlet; veined. Sir T. Herbert.",
  "oryctologist - One versed in oryctology. [Obs.]",
  "euclid - A Greek geometer of the 3d century",
  "listener - One who listens; a hearkener.",
  "biscutate - Resembling two bucklers placed side by side.",
  "corrodibility - The qualityof being corrodible. [R.] Johnson.",
  "taas - A heap. See Tas. [Obs.]  Chaucer.",
  "unwarranted - Not warranted; being without warrant, authority, or guaranty; unwarrantable.",
  "epitomist - One who makes an epitome; one who abridges; an epitomizer. Milton.",
  "haughtily - In a haughty manner; arrogantly.",
  "tres-tine - The third tine above the base of a stag's antler; the royal antler.",
  "bilobate - Divided into two lobes or segments.",
  "resilient - Leaping back; rebounding; recoling.",
  "foredeck - The fore part of a deck, or of a ship.",
  "manutenency - Maintenance. [Obs.] Abp. Sancroft.",
  "hopbine - The climbing stem of the hop. Blackstone.",
  "unprop - To remove a prop or props from; to deprive of support.",
  "disaccustom - To destroy the force of habit in; to wean from a custom. Johnson.",
  "obstetricy - Obstetrics. [R.] Dunglison.",
  "hierographical - Of or pertaining to sacred writing.",
  "auscultatory - Of or pertaining to auscultation. Dunglison.",
  "democratism - The principles or spirit of a democracy. [R.]",
  "siphoniferous - Siphon-bearing, as the shell of the nautilus and other cephalopods.",
  "stem-clasping - Embracing the stem with its base; amplexicaul; as a leaf or petiole.",
  "lill - To loll. [Obs. or Prov.] Spenser.",
  "pompet - The ball formerly used to ink the type.",
  "pedometric - Pertaining to, or measured by, a pedometer.",
  "lipogrammatist - One who makes a lipogram.",
  "twinling - A young or little twin, especially a twin lamb.",
  "repayment - 1. The act of repaying; reimbursement. Jer. Taylor. 2. The money or other thing repaid.",
  "treater - One who treats; one who handles, or discourses on, a subject; also, one who entertains.",
  "barghest - A goblin, in the shape of a large dog, portending misfortune. [Also written barguest.]",
  "equalitarian - One who believes in equalizing the condition of men; a leveler.",
  "tenosynovitis - Inflammation of the synovial sheath enveloping a tendon.",
  "electro-engraving - The art or process of engraving by means of electricity.",
  "extramundane - Beyond the material world. \"An extramundane being.\" Bp. Warburton.",
  "prelatureship - The state or dignity of a prelate; prelacy. Milman.",
  "shearman - One whose occupation is to shear cloth.",
  "unphilosophize - To degrade from the character of a philosopher. [R.] Pope.",
  "misarrange - To place in a wrong order, or improper manner.",
  "turrical - Of or pertaining to a turret, or tower; resembling a tower.",
  "dialyzer - The instrument or medium used to effect chemical dialysis.",
  "bultel - A bolter or bolting cloth; also, bran. [Obs.]",
  "sparsely - In a scattered or sparse manner.",
  "palfreyed - Mounted on a palfrey. Tickell.",
  "subdual - Act of subduing. Bp. Warburton.",
  "agricolation - Agriculture. [Obs.] Bailey.",
  "bourbonism - The principles of those adhering to the house of Bourbon; obstinate conservatism.",
  "waygate - The tailrace of a mill. Knight.",
  "acquaintanceship - A state of being acquainted; acquaintance. Southey.",
  "lenticular - Resembling a lentil in size or form; having the form of a double-convex lens.",
  "corrigibility - Quality of being corrigible; capability of being corrected; corrigibleness.",
  "disaventurous - Misadventurous; unfortunate. [Obs.] Spenser.",
  "realistically - In the realistic manner.",
  "caruncular - Of, pertaining to, or like, a caruncle; furnished with caruncles.",
  "unsin - To deprive of sinfulness, as a sin; to make sinless. [Obs.] Feltham.",
  "tandem engine - A steam engine having two or more steam cylinders in line, with a common piston rod.",
  "fungosity - The quality of that which is fungous; fungous excrescence. Dunglison.",
  "hoariness - The state of being hoary. Dryden.",
  "sarco - A combining form from Gr. flesh; as, sarcophagous, flesh- eating; sarcology.",
  "kyack - A pack sack to be swung on either side of a packsaddle. [Western U. S.]",
  "multicuspidate - Having many cusps or points.",
  "fanfaronade - A swaggering; vain boasting; ostentation; a bluster. Swift.",
  "hoodlum - A young rowdy; a rough, lawless fellow. [Colloq. U.S.]",
  "unadvisable - Not advisable; inadvisable; inexpedient. Lowth. -- Un`ad*vis\"a*bly, adv.",
  "stickiness - The quality of being sticky; as, the stickiness of glue or paste.",
  "swimmingly - In an easy, gliding manner, as if swimming; smoothly; successfully; prosperously.",
  "azonic - Confined to no zone or region; not local.",
  "inedible - Not edible; not fit for food. -- In*ed`i*bil\"i*ty (#), n.",
  "scathful - Harmful; doing damage; pernicious. Shak. -- Scath\"ful*ness, n.",
  "guilor - A deceiver; one who deludes, or uses guile. [Obs.] Spenser.",
  "kaliform - Formed like kali, or glasswort.",
  "licit - Lawful. \"Licit establishments.\" Carlyle. -- Lic\"it*ly, adv. -- Lic\"it*ness, n.",
  "snobbery - The quality of being snobbish; snobbishness.",
  "postprandial - Happening, or done, after dinner; after-dinner; as, postprandial speeches.",
  "extruction - A building up; construction. [Obs.] Cockeram.",
  "brand-new - Quite new; bright as if fresh from the forge.",
  "aggest - To heap up. [Obs.] The violence of the waters aggested the earth. Fuller.",
  "chevronel - A bearing like a chevron, but of only half its width.",
  "palmigrade - Putting the whole foot upon the ground in walking, as some mammals.",
  "phare - 1. A beacon tower; a lighthouse. [Obs.] 2. Hence, a harbor. Howell.",
  "egregiousness - The state of being egregious.",
  "subtransparent - Not perfectly transparent.",
  "biographic - Of or pertaining to biography; containing biography. -- Bi`o*graph\"ic*al*ly, adv.",
  "poudrette - A manure made from night soil, dried and mixed with charcoal, gypsum, etc.",
  "otacousticon - An instrument to facilitate hearing, as an ear trumpet.",
  "allonymous - Published under the name of some one other than the author.",
  "presphenoidal - Of or pertaining to the presphenoid bone; presphenoid.",
  "ruffianish - Having the qualities or manners of a ruffian; ruffianly.",
  "stannotype - A photograph taken upon a tin plate; a tintype.",
  "differently - In a different manner; variously.",
  "chymiferous - Bearing or containing chyme.",
  "convivialist - A person of convivial habits.",
  "pyrenoid - A transparent body found in the chromatophores of certain Infusoria.",
  "gastroenteritis - Inflammation of the lining membrane of the stomach and the intestines.",
  "vibices - More or less extensive patches of subcutaneous extravasation of blood.",
  "predestinative - Determining beforehand; predestinating. [R.] Coleridge.",
  "unconstancy - Inconstancy. [Obs.] \"The unconstancy of the foundation.\" Fuller.",
  "massiveness - The state or quality of being massive; massiness.",
  "surmountable - Capable of being surmounted or overcome; superable. -- Sur*mount\"a*ble*ness, n.",
  "aphonous - Without voice; voiceless; nonvocal.",
  "milkiness - State or quality of being milky.",
  "stubby - 1. Abounding with stubs. 2. Short and thick; short and strong, as bristles.",
  "suboperculum - The lower opercular bone in fishes.",
  "cognizee - One to whom a fine of land was ackowledged. Blackstone.",
  "secund - Arranged on one side only, as flowers or leaves on a stalk. Gray.",
  "leveret - A hare in the first year of its age.",
  "capreoline - Of or pertaining to the roebuck.",
  "debasement - The act of debasing or the state of being debased. Milton."
]