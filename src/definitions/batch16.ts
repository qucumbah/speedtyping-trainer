export default [
  "over-story - The clearstory, or upper story, of a building.",
  "narratively - In the style of narration.",
  "hyoglossus - A flat muscle on either side of the tongue, connecting it with the hyoid bone.",
  "riban - See Ribbon. [Obs.] Piers Plowman.",
  "uromere - Any one of the abdominal segments of an arthropod.",
  "hellenistically - According to the Hellenistic manner or dialect. J. Gregory.",
  "misteach - To teach wrongly; to instruct erroneously.",
  "crinkly - Having crinkles; wavy; wrinkly.",
  "incagement - Confinement in, or as in, cage. [Obs.] Shelton.",
  "linch - A ledge; a right-angled projection.",
  "adrogate - To adopt (a person who is his own master).",
  "voweled - Furnished with vowels. [Written also vowelled.] Dryden.",
  "hystricine - Like or pertaining to the porcupines.",
  "misimagination - Wrong imagination; delusion. Bp. Hall.",
  "crossflow - To flow across, or in a contrary direction. \"His crossflowing course.\" Milton.",
  "indefensibility - The quality or state of not being defensible. Walsh.",
  "transfigurate - To transfigure; to transform. [R.]",
  "chirographic - Of or pertaining to chirography.",
  "pharisean - Following the practice of Pharisees; Pharisaic. [Obs.] \"Pharisean disciples.\" Milton.",
  "completement - Act of completing or perfecting; completion. [Obs.] Dryden.",
  "unregenerate - Not regenerated; not renewed in heart; remaining or being at enmity with God.",
  "flatwise - With the flat side downward, or next to another object; not edgewise.",
  "innumerous - Innumerable. [Archaic] Milton.",
  "unbashful - Not bashful or modest; bold; impudent; shameless. Shak.",
  "unneighbored - Being without neigbors. Cowper.",
  "soiliness - Stain; foulness. [R.] Bacon.",
  "ihram - The peculiar dress worn by pilgrims to Mecca.",
  "elixation - A seething; digestion. [Obs.] Burton.",
  "roseal - resembling a rose in smell or color. [Obs.] Sir T. Elyot.",
  "chud - To champ; to bite. [Obs.] A. Stafford.",
  "reclude - To open; to unclose. [R.] Harvey.",
  "wheaten - Made of wheat; as, wheaten bread. Cowper.",
  "aphoristically - In the form or manner of aphorisms; pithily.",
  "ossifragous - Serving to break bones; bone-breaking.",
  "vestibulum - A cavity into which, in certain bryozoans, the esophagus and anus open.",
  "isonomic - The same, or equal, in law or right; one in kind or origin; analogous; similar. Dana.",
  "scullionly - Like a scullion; base. [Obs.] Milton.",
  "affinitative - Of the nature of affinity. -- Af*fin\"i*ta*tive*ly, adv.",
  "coleridgian - Pertaining to Samuel Taylor Coleridge, or to his poetry or metaphysics.",
  "self-possessed - Composed or tranquill in mind, manner, etc.; undisturbed.",
  "pompelmous - A shaddock, esp. one of large size.",
  "umbecast - To cast about; to consider; to ponder. [Obs.] Sir T. Malory.",
  "apolar - Having no radiating processes; -- applied particularly to certain nerve cells.",
  "hyracoid - Of or pertaining to the Hyracoidea. -- n.  One of the Hyracoidea.",
  "unhumanize - To render inhuman or barbarous. J. Barlow.",
  "ringtoss - A game in which the object is to toss a ring so that it will catch upon an upright stick.",
  "unresty - Causing unrest; disquieting; as, unresty sorrows. [Obs.] Chaucer.",
  "interorbital - Between the orbits; as, the interorbital septum.",
  "omniscience - The quality or state of being omniscient; -- an attribute peculiar to God. Dryden.",
  "unconversion - The state of being unconverted; impenitence. [R.]",
  "self-regulated - Regulated by one's self or by itself.",
  "iliofemoral - Pertaining to the ilium and femur; as, iliofemoral ligaments.",
  "prosternation - Dejection; depression. [Obs.] Wiseman.",
  "marrowfat - A rich but late variety of pea.",
  "auriscopy - Examination of the ear by the aid of the auriscope.",
  "parenetic - Hortatory; encouraging; persuasive. [R.] F. Potter.",
  "schizognath - Any bird with a schizognathous palate.",
  "nosography - A description or classification of diseases.",
  "mopsy - 1. A moppet. 2. A slatternly, untidy woman. Halliwell.",
  "compinge - To compress; to shut up. [Obs.] Burton.",
  "intrusional - Of or pertaining to intrusion.",
  "self-reproving - Reproving one's self; reproving by consciousness of guilt.",
  "neologistic - of or pertaining to neology; neological.",
  "collatitious - Brought together; contributed; done by contributions. [Obs.] Bailey.",
  "adduct - To draw towards a common center or a middle line. Huxley.",
  "libant - Sipping; touching lightly. [R.] Landor.",
  "magisterially - In a magisterial manner.",
  "eulachon - The candlefish. [Written also oulachan, oolacan, and ulikon.] See Candlefish.",
  "soddy - Consisting of sod; covered with sod; turfy. Cotgrave.",
  "tonsilotomy - The operation of removing the tonsil, or a portion thereof.",
  "baromacrometer - An instrument for ascertaining the weight and length of a newborn infant.",
  "neurology - The branch of science which treats of the nervous system.",
  "judaist - One who believes and practices Judaism.",
  "toyful - Full of trifling play. [Obs.] Donne.",
  "piston ring - A spring packing ring, or any of several such rings, for a piston.",
  "shyly - In a shy or timid manner; not familiarly; with reserve. [Written also shily.]",
  "semibarbarism - The quality or state of being half barbarous or uncivilized.",
  "foliolate - Of or pertaining to leaflets; -- used in composition; as, bi- foliolate. Gray.",
  "mistle - To fall in very fine drops, as rain.",
  "anisette - A French cordial or liqueur flavored with anise seeds. De Colange.",
  "wheresoever - In what place soever; in whatever place; wherever.",
  "beneficiate - To reduce (ores). -- Ben`e*fi`ci*a\"tion (n.",
  "seaware - Seaweed; esp., coarse seaweed. See Ware, and Sea girdles.",
  "infausting - The act of making unlucky; misfortune; bad luck. [Obs.] Bacon.",
  "narrowness - The condition or quality of being narrow.",
  "unpartial - Impartial. [Obs.] Bp. Sanderson. -- Un*par\"tial*ly, adv. [Obs.] Hooker.",
  "calyculated - Having a set of bracts resembling a calyx.",
  "white-hot - White with heat; heated to whiteness, or incandescence.",
  "exolete - Obsolete; out of use; state; insipid. [Obs.]",
  "outzany - To exceed in buffoonery. [Obs.] B. Jonson.",
  "pauhaugen - The menhaden; -- called also poghaden.",
  "rutylene - A liquid hydrocarbon, C10H18, of the acetylene series. It is produced artificially.",
  "cremaillere - An indented or zigzaged line of intrenchment.",
  "masticador - A part of a bridle, the slavering bit. [Written also mastigador.]",
  "amit - To lose. [Obs.] A lodestone fired doth presently amit its proper virtue. Sir T. Browne.",
  "bibler - A great drinker; a tippler. [Written also bibbler and bibbeler.]",
  "eavedrop - A drop from the eaves; eavesdrop. [R.] Tennyson.",
  "propitiator - One who propitiates or appeases.",
  "centifidous - Divided into a hundred parts.",
  "optation - The act of optating; a wish. [Obs.] Sir T. Browne.",
  "wantage - That which is wanting; deficiency.",
  "incessantly - Unceasingly; continually. Shak.",
  "abaciscus - One of the tiles or squares of a tessellated pavement; an abaculus.",
  "stormcock - (a) The missel thrush. (b) The fieldfare. (c) The green woodpecker.",
  "digitiform - Formed like a finger or fingers; finger-shaped; as, a digitiform root.",
  "sative - Sown; propagated by seed. [Obs.] Evelyn.",
  "dentilave - A wash for cleaning the teeth.",
  "fortifiable - Capable of being fortified. Johnson.",
  "homo- - A combining form from Gr. \"omo`s, one and the same, common, joint.",
  "gillhouse - A shop where gill is sold. Thee shall each alehouse, thee each gillhouse mourn. Pope.",
  "vitalize - To endow with life, or vitality; to give life to; to make alive; as, vitalized blood.",
  "quadrilobed - Having four lobes; as, a quadrilobate leaf.",
  "radiciform - Having the nature or appearance of a radix or root.",
  "sexually - In a sexual manner or relation.",
  "abele - The white polar (Populus alba). Six abeles i' the churchyard grow. Mrs. Browning.",
  "fluxionist - One skilled in fluxions. Berkeley.",
  "lakeweed - The water pepper (Polygonum Hydropiper), an aquatic plant of Europe and North America.",
  "confocal - Having the same foci; as, confocal quadrics.",
  "nathmore - Not the more; never the more. [Obs.] penser.",
  "bountyhood - Goodness; generosity. [Obs.] Spenser.",
  "unplained - Not deplored or bewailed; unlamented. [Obs.] Spenser.",
  "tietick - The meadow pipit. [Prov. Eng].",
  "self-evidence - The quality or state of being self-evident. Locke.",
  "yankeeism - A Yankee idiom, word, custom, or the like. Lowell.",
  "populism - The political doctrines advocated by the People's party.",
  "tripinnate - Having bipinnate leaflets arranged on each side of a rhachis.",
  "fraternism - Fraternization. [R.] Jefferson.",
  "extralimitary - Being beyond the limit or bounds; as, extraliminary land. Mitford.",
  "gon - imp. & p. p. of Go. [Obs.] Chaucer.",
  "whitneyite - an arsenide of copper from Lake Superior.",
  "bluntish - Somewhat blunt. -- Blunt\"ish*ness, n.",
  "guesser - One who guesses; one who forms or gives an opinion without means of knowing.",
  "thermometry - The estimation of temperature by the use of a thermometric apparatus.",
  "efform - To form; to shape. [Obs.] Efforming their words within their lips. Jer. Taylor.",
  "besprinkle - To sprinkle over; to scatter over. The bed besprinkles, and bedews the ground. Dryden.",
  "gillian - A girl; esp., a wanton; a gill. [Obs.] Beau. & Fl.",
  "overlip - The upper lip. [Obs.] Chaucer.",
  "enroot - To fix by the root; to fix fast; to implant deep. Shak.",
  "misexplanation - An erroneous explanation.",
  "substructure - 1. (Arch.)  Same as Substruction. 2. An under structure; a foundation; groundwork.",
  "externalistic - Pertaining to externalism North Am. Rev.",
  "plectognathic - Of or pertaining to the Plectognathi.",
  "juliform - Having the shape or appearance of a julus or catkin.",
  "propithecus - A genus including the long-tailed, or diadem, indris. See Indris.",
  "compagination - Union of parts; structure. [Obs.] Jer. Taylor.",
  "overgive - To give over; to surrender; to yield. [Obs.] Spenser.",
  "postillator - One who postillates; one who expounds the Scriptures verse by verse.",
  "stipel - The stipule of a leaflet. Gray.",
  "shufflewing - The hedg sparrow. [Prov. Eng.]",
  "plagionite - A sulphide of lead and antimony, of a blackish lead-gray color and metallic luster.",
  "superintellectual - Being above intellect.",
  "melancholian - A person affected with melancholy; a melancholic. [Obs.] Dr. J. Scott.",
  "supportful - Abounding with support. [Obs.] Chapman.",
  "cinereous - Like ashes; ash-colored; grayish.",
  "everyone - Everybody; -- commonly separated, every one.",
  "inservient - Conducive; instrumental. [Obs.] Sir T. Browne.",
  "goaves - Old workings. See Goaf. Raymond.",
  "incongruent - Incongruous. Sir T. Elyot.",
  "dilogical - Ambiguous; of double meaning. [Obs.] T. Adams.",
  "putidity - The quality or state of being putrid.",
  "archaeography - A description of, or a treatise on, antiquity or antiquities.",
  "knickknackatory - A collection of knickknacks. Richardson.",
  "half-faced - Showing only part of the face; wretched looking; meager. Shak.",
  "entomotomist - One who practices entomotomy.",
  "fleshless - Destitute of flesh; lean. Carlyle.",
  "antenuptial - Preceding marriage; as, an antenuptial agreement. Kent.",
  "submersed - Being or growing under water, as the leaves of aquatic plants.",
  "apostrophic - Pertaining to an apostrophe, grammatical or rhetorical.",
  "ensky - To place in the sky or in heaven. [R.] \"A thing enskied and sainted.\" Shak.",
  "lego-literary - Pertaining to the literature of law.",
  "speedfully - In a speedful manner. [Obs.]",
  "nitrogenize - To combine, or impregnate, with nitrogen or its compounds.",
  "calycinal - Pertaining to a calyx; having the nature of a calyx.",
  "petroleur - One who makes use of petroleum for incendiary purposes.",
  "greenth - The state or quality of being green; verdure. [R.] The greenth of summer. G. Eliot.",
  "queen truss - A truss framed with queen-posts; a queen-post truss.",
  "stypticity - The quality or state of being styptic; astringency.",
  "mastodonsaurus - A large extinct genus of labyrinthodonts, found in the European Triassic rocks.",
  "contemptibility - The quality of being contemptible; contemptibleness. Speed.",
  "vitrificate - To convert into glass; to vitrify. [Obs.] Bacon.",
  "displicence - Dislike; dissatisfaction; discontent. [Obs.] W. Montagu.",
  "codilla - The coarse tow of flax and hemp. McElrath.",
  "unsurety - Want of surety; uncertainty; insecurity; doubt. [Obs.] Sir T. More.",
  "sassorolla - The rock pigeon. See under Pigeon.",
  "oftentide - Frequently; often. [Obs.] Robert of Brunne.",
  "praiseworthiness - The quality or state of being praiseworthy.",
  "capapie - From head to foot; at all points. \"He was armed cap-a-pie.\" Prescott.",
  "godhood - Divine nature or essence; deity; godhead.",
  "mugwumpism - The acts and views of the mugwumps. [Political Cant, U.S.]",
  "cementitious - Of the nature of cement. [R.] Forsyth.",
  "falding - A frieze or rough-napped cloth. [Obs.]",
  "mussulmanic - Of, pertaining to, or like, the Mussulmans, or their customs: Mohammedan.",
  "seity - Something peculiar to one's self. [R.] Tatler.",
  "lawmaking - Enacting laws; legislative. -- n.  The enacting of laws; legislation.",
  "water witch - (a) The dabchick. (b) The stormy petrel. [Prov. Eng.]",
  "gloriosa - A genus of climbing plants with very showy lilylike blossoms, natives of India.",
  "arteriac - Of or pertaining to the windpipe.",
  "fordless - Without a ford. A deep and fordless river. Mallock.",
  "connation - Connection by birth; natural union. [Obs.] Dr. H. More.",
  "natty - Neat; tidy; spruce. [Colloq.] -- Nat\"ti*ly, adv. -- Nat\"ti*ness, n.",
  "self-imparting - Imparting by one's own, or by its own, powers and will. Norris.",
  "allium - A genus of plants, including the onion, garlic, leek, chive, etc.",
  "resumptive - Taking back; resuming, or tending toward resumption; as, resumptive measures.",
  "ghastful - Fit to make one aghast; dismal. [Obs.] -- Ghast\"ful*ly, adv.",
  "mormal - A bad sore; a gangrene; a cancer. [Obs.] [Written also morrimal and mortmal.] Chaucer.",
  "advocatory - Of or pertaining to an advocate. [R.]",
  "linguality - The quality of being lingual.",
  "verso - The reverse, or left-hand, page of a book or a folded sheet of paper; -- opposed to recto.",
  "uranoscopy - Observation of the heavens or heavenly bodies.",
  "ichthidin - A substance from the egg yolk of osseous fishes.",
  "inclavated - Set; fast; fixed. Dr. John Smith.",
  "dimorphic - Having the property of dimorphism; dimorphous.",
  "megacephalic - Large headed; -- applied to animals, and to plants when they have large flower heads.",
  "unmannerly - Not mannerly; ill-bred; rude. -- adv.  Uncivilly; rudely. -- Un*man\"ner*li*ness, n.",
  "didelphic - Having the uterus double; of or pertaining to the Didelphia.",
  "barology - The science of weight or gravity.",
  "conventionalization - (a) The act of making conventional. (b) The state of being conventional.",
  "quadrigenarious - Consisting of four hundred.",
  "possessival - Of or pertaining to the possessive case; as, a possessival termination. Earle.",
  "sanguineless - Destitute of blood; pale. [R.]",
  "discontinuable - Admitting of being discontinued. [R.]",
  "otologist - One skilled in otology; an aurist.",
  "jimson weed - . See Jamestown weed. [Local, U.S.]",
  "blek - To blacken; also, to defile. [Obs. or Dial.] Wyclif.",
  "overflowingly - In great abundance; exuberantly. Boyle.",
  "bearishness - Behavior like that of a bear.",
  "gentianine - A bitter, crystallizable substance obtained from gentian.",
  "semidiaphaneity - Half or imperfect transparency; translucency. [R.] Boyle.",
  "chiaroscurist - A painter who cares for and studies light and shade rather than color.",
  "ingestion - The act of taking or putting into the stomach; as, the ingestion of milk or other food.",
  "thooid - Of or pertaining to a group of carnivores, including the wovels and the dogs.",
  "deflectable - Capable of being deflected.",
  "bitumed - Smeared with bitumen. [R.] \"The hatches caulked and bitumed.\" Shak.",
  "adrad - Put in dread; afraid. [Obs.] Chaucer.",
  "eurasiatio - Of or pertaining to the continents of Europe and Asia combined.",
  "masonic - Of or pertaining to Freemasons or to their craft or mysteries.",
  "superincumbent - Lying or resting on something else.",
  "aphidivorous - Devouring aphides; aphidophagous.",
  "expediment - An expedient. [Obs.] A like expediment to remove discontent. Barrow.",
  "disenshrouded - Freed from a shroudlike covering; unveiled. The disenshrouded statue. R. Browning.",
  "vintner - One who deals in wine; a wine seller, or wine merchant.",
  "intraterritorial - Within the territory or a territory.",
  "bacchante - 1. A priestess of Bacchus. 2. A female bacchanal.",
  "eldritch - Hideous; ghastly; as, an eldritch shriek or laugh. [Local, Eng.]",
  "cany - Of or pertaining to cane or canes; abounding with canes. Milton.",
  "acropolis - The upper part, or the citadel, of a Grecian city; especially, the citadel of Athens.",
  "pachydermal - Of or relating to the pachyderms; as, pachydermal dentition.",
  "rosebush - The bush or shrub which bears roses.",
  "infirmness - Infirmity; feebleness. Boyle.",
  "prestissimo - Very quickly; with great rapidity.",
  "intercommunion - Mutual communion; as, an intercommunion of deities. Faber.",
  "factoress - A factor who is a woman. [R.]",
  "resect - To cut or pare off; to remove by cutting.",
  "apicular - Situated at, or near, the apex; apical.",
  "expressness - The state or quality of being express; definiteness. [Obs.] Hammond.",
  "plowtail - The hind part or handle of a plow.",
  "prevenience - The act of going before; anticipation. [R.]",
  "podosperm - The stalk of a seed or ovule.",
  "dissociability - Want of sociability; unsociableness. Bp. Warburton.",
  "diatryma - An extinct eocene bird from New Mexico, larger than the ostrich.",
  "locksmith - An artificer whose occupation is to make or mend locks.",
  "semiperspicuous - Half transparent; imperfectly clear; semipellucid.",
  "epicede - A funeral song or discourse; an elegy. [R.] Donne.",
  "halter-sack - A term of reproach, implying that one is fit to be hanged. [Obs.] Beau. & Fl.",
  "microscopy - The use of the microscope; investigation with the microscope.",
  "unimpairable - That can not be impaired. Hakewill.",
  "tessellation - The act of tessellating; also, the mosaic work so formed. J. Forsyth.",
  "anencephalous - Without a brain; brainless. Todd & B.",
  "waxworks - An exhibition of wax figures, or the place of exhibition.",
  "revendication - The act of revendicating. [R.] Vattel (Trans. )",
  "vitreousness - The quality or state of being vitreous.",
  "kidnaper - One who steals or forcibly carries away a human being; a manstealer.",
  "behead - To sever the head from; to take off the head of.",
  "fibrillar - Of or pertaining to fibrils or fibers; as, fibrillar twitchings.",
  "apperceive - To perceive; to comprehend. Chaucer.",
  "pontifice - Bridgework; structure or edifice of a bridge. [R.] Milton.",
  "consecute - To follow closely; to endeavor to overtake; to pursue. [Obs.] Bp. Burnet.",
  "preceptress - A woman who is the principal of a school; a female teacher.",
  "inexcitable - Not susceptible of excitement; dull; lifeless; torpid.",
  "burying ground - The ground or place for burying the dead; burial place.",
  "savorous - Having a savor; savory. [Obs.] Rom. of R.",
  "demisable - Capable of being leased; as, a demisable estate.",
  "torpitude - Torpidness. [Obs.] \"In a kind of torpitude, or sleeping state.\" Derham.",
  "ontologist - One who is versed in or treats of ontology. Edin. Rev.",
  "reynard - An appelation applied after the manner of a proper name to the fox. Same as Renard.",
  "aldermanic - Relating to, becoming to, or like, an alderman; characteristic of an alderman.",
  "predestinary - Predestinarian. [Obs.] Heylin.",
  "breechcloth - A cloth worn around the breech.",
  "fetichist - A believer in fetiches. He was by nature a fetichist. H. Holbeach.",
  "constate - To ascertain; to verify; to establish; to prove. F. P. Cobbe.",
  "jovially - In a jovial manner; merrily; gayly. B. Jonson.",
  "sipid - Having a taste or flavorl savory; sapid. [Obs.] Cockeram.",
  "cretaceously - In a chalky manner; as chalk.",
  "antepileptic - Good against epilepsy. -- n.  A medicine for epilepsy.",
  "subthalamic - Situated under the optic thalamus.",
  "sunlight - The light of the sun. Milton.",
  "unchristened - Not christened; as, an unchristened child.",
  "assistless - Without aid or help. [R.] Pope.",
  "elusion - Act of eluding; adroit escape, as by artifice; a mockery; a cheat; trickery.",
  "putrescence - The state of being putrescent; putrescent matter.",
  "phthor - Fluorine. [Written also phthor.]",
  "viewly - Pleasing to the sight; sightly. [Prov. Eng.]",
  "tabulation - The act of forming into a table or tables; as, the tabulation of statistics.",
  "trenchantly - In a trenchant, or sharp, manner; sharply; severely.",
  "trebly - In a treble manner; with a threefold number or quantity; triply. Swift.",
  "elohim - One of the principal names by which God is designated in the Hebrew Scriptures.",
  "vesicoprostatic - Of a pertaining to the bladder and the prostrate gland.",
  "slinger - One who slings, or uses a sling.",
  "wildish - Somewhat wild; rather wild. \"A wildish destiny.\" Wordsworth.",
  "concubinarian - Concubinary. The married and concubinarian, as well as looser clergy. Milman.",
  "electant - One who has the power of choosing; an elector. [R.]",
  "vantage game - The first game after the set is deuce. See Set, n., 9.",
  "explorator - One who explores; one who examines closely; a searcher.",
  "acinaciform - Scimeter-shaped; as, an acinaciform leaf.",
  "civilizer - One who, or that which, civilizes or tends to civilize.",
  "propendency - 1. Propensity. [R.] 2. Attentive deliberation. [R.] Sir M. Hale.",
  "sigillative - Fit to seal; belonging to a seal; composed of wax. [R.]",
  "allopathist - One who practices allopathy; one who professes allopathy.",
  "tulipomaniac - One who is affected with tulipomania.",
  "hemiglyph - The half channel or groove in the edge of the triglyph in the Doric order.",
  "dominicide - 1. The act of killing a master. 2. One who kills his master.",
  "inextirpable - Not capable of being extirpated or rooted out; ineradicable.",
  "vehiculate - To convey by means of a vehicle; to ride in a vehicle. Carlyle.",
  "disinfector - One who, or that which, disinfects; an apparatus for applying disinfectants.",
  "hackman - The driver of a hack or carriage for public hire.",
  "unslaked - Not slaked; unslacked; as, an unslaked thirst; unslaked lime.",
  "corollate - Having a corolla or corollas; like a corolla.",
  "annualist - One who writes for, or who edits, an annual. [R.]",
  "uncunning - Ignorant. [Obs.] I am young and uncunning, as thou wost [knowest]. Chaucer.",
  "snatch block - a kind of block with an opening in one side to receive the bight of a rope.",
  "husbandless - Destitute of a husband. Shak.",
  "sulphonium - A hypothetical radical, SH3, regarded as the type and nucleus of the sulphines.",
  "redeemer - 1. One who redeems. 2. Specifically, the Savior of the world, Jesus Christ.",
  "husbandage - The commission or compensation allowed to a ship's husband.",
  "sleepish - Disposed to sleep; sleepy; drowsy. Your sleepish, and more than sleepish, security. Ford.",
  "lupercalia - A feast of the Romans in honor of Lupercus, or Pan.",
  "rew - A row. [Obs.] Chaucer. \"A rew of sundry colored stones.\" Chapman.",
  "fugato - in the gugue style, but not strictly like a fugue. -- n.  A composition resembling a fugue.",
  "weirdness - The quality or state of being weird.",
  "fast-handed - Close-handed; close-fisted; covetous; avaricious. [Obs.] Bacon.",
  "duncical - Like a dunce; duncish. The most dull and duncical commissioner. Fuller.",
  "suingly - In succession; afterwards. [Obs.] Sir T. More.",
  "subgroup - A subdivision of a group, as of animals. Darwin.",
  "softling - A soft, effeminate person; a voluptuary. [R.] Bp. Woolton. .",
  "fumidness - The state of being fumid; smokiness.",
  "cataclysmic - Of or pertaining to a cataclysm.",
  "infrangibility - The quality or state of being infrangible; infrangibleness.",
  "worm-shaped - Shaped like a worm; as, a worm-shaped root.",
  "babyish - Like a baby; childish; puerile; simple. -- Ba\"by*ish*ly, adv. -- Ba\"by*ish*ness, n.",
  "sea bank - 1. The seashore. Shak. 2. A bank or mole to defend against the sea.",
  "pigeonfoot - The dove's-foot geranium (Geranium molle).",
  "place-kick - To make a place kick; to make (a goal) by a place kick. -- Place\"-kick`er, n.",
  "guest rope - The line by which a boat makes fast to the swinging boom. Ham. Nav. Encyc.",
  "sawdust - Dust or small fragments of wood 9or of stone, etc.) made by the cutting of a saw.",
  "prolifical - Producing young or fruit abundantly; fruitful; prolific. -- Pro*lif\"ic*al*ly, adv.",
  "disventure - A disadventure. [Obs.] Shelton.",
  "triatic - A term used in the phrase triatic stay. See under Stay.",
  "unsufferable - Insufferable. [Obs.] Hooker. -- Un*suf\"fer*a*bly, adv. [Obs.]",
  "interrogator - One who asks questions; a questioner.",
  "go-between - An intermediate agent; a broker; a procurer; -- usually in a disparaging sense. Shak.",
  "typocosmy - A representation of the world. [R.]",
  "insulse - Insipid; dull; stupid. [Obs.] Milton.",
  "killow - An earth of a blackish or deep blue color. Woodward.",
  "commissive - Relating to commission; of the nature of, or involving, commission. [R.]",
  "excerptor - One who makes excerpts; a picker; a culler.",
  "euharmonic - Producing mathematically perfect harmony or concord; sweetly or perfectly harmonious.",
  "choleraic - Relating to, or resulting from, or resembling, cholera.",
  "adjudgment - The act of adjudging; judicial decision; adjudication. Sir W. Temple.",
  "ill-boding - Boding evil; inauspicious; ill-omened. \"Ill-boding stars.\" Shak.",
  "artow - A contraction of art thou. [Obs.] Chaucer.",
  "perdy - Truly. See Parde. [Obs.] Ah, dame! perdy ye have not done me right. Spenser.",
  "fronted - Formed with a front; drawn up in line. \"Fronted brigades.\" Milton.",
  "violable - Capable of being violated, broken, or injured. -- Vi\"o*la*bly, adv.",
  "consignature - Joint signature. [R.] Colgrave.",
  "carbazotate - A salt of carbazotic or picric acid; a picrate.",
  "letterer - One who makes, inscribes, or engraves, alphabetical letters.",
  "ramed - Having the frames, stem, and sternpost adjusted; -- said of a ship on the stocks.",
  "polydactylism - The possession of more that the normal number of digits.",
  "goemin - A complex mixture of several substances extracted from Irish moss.",
  "permeance - Permeation; specif. (Magnetism),  the reciprocal of reluctance.",
  "colombier - A large size of paper for drawings. See under Paper.",
  "astipulate - To assent. [Obs.] Bp. Hall.",
  "purseful - All that is, or can be, contained in a purse; enough to fill a purse.",
  "piller - One who pills or plunders. [Obs.]",
  "appraisement - The act of setting the value; valuation by an appraiser; estimation of worth.",
  "absolutely - In an absolute, independent, or unconditional manner; wholly; positively.",
  "overproud - Exceedingly or unduly proud. \"Overproud of his victory.\" Milton.",
  "mediety - The middle part; half; moiety. [Obs.] Sir T. Browne.",
  "overbuild - 1. To build over. Milton. 2. To build too much; to build beyond the demand.",
  "militiate - To carry on, or prepare for, war. [Obs.] Walpole.",
  "capaciously - In a capacious manner or degree; comprehensively.",
  "procuratorial - Of or pertaining to a procurator, or proctor; made by a proctor. Ayliffe.",
  "bismillah - An adjuration or exclamation common among the Mohammedans. [Written also Bizmillah.]",
  "boroughmaster - The mayor, governor, or bailiff of a borough.",
  "bescreen - To cover with a screen, or as with a screen; to shelter; to conceal. Shak.",
  "canuck - 1. A Canadian. [Slang] 2. A small or medium-sized hardy horse, common in Canada. [Colloq.]",
  "cyclone pit - A cellar or excavation used for refuge from a cyclone, or tornado. [Middle U. S.]",
  "behoovable - Supplying need; profitable; advantageous. [Obs.] Udall.",
  "dreggish - Foul with lees; feculent. Harvey.",
  "awl-shaped - 1. Shaped like an awl. 2. (Nat. Hist.)  Subulate. See Subulate. Gray.",
  "supracondyloid - Situated above a condyle or condyles.",
  "cowlstaff - A staff or pole on which a vessel is supported between two persons. Suckling.",
  "oenanthone - The ketone of oenanthic acid.",
  "mistrustingly - With distrust or suspicion.",
  "craniometric - Pertaining to craniometry.",
  "midbrain - The middle segment of the brain; the mesencephalon. See Brain.",
  "susurringly - In the manner of a whisper. [Obs.]",
  "pitch-dark - Dark as a pitch; pitch-black.",
  "underpuller - One who underpulls. [Obs.]",
  "oxychloride - A ternary compound of oxygen and chlorine; as, plumbic oxychloride.",
  "pyrometry - The art of measuring degrees of heat, or the expansion of bodies by heat.",
  "afterward - At a later or succeeding time.",
  "peanism - The song or shout of praise, of battle, or of triumph. [R.]",
  "spalting knife - A knife used in splitting codfish. [Written also spalding knife.]",
  "starchly - In a starched or starch manner.",
  "somner - A summoner; esp., one who summons to an ecclesiastical court. [Obs.] Piers Plowman.",
  "billed - Furnished with, or having, a bill, as a bird; -- used in composition; as, broad-billed.",
  "ephoralty - The office of an ephor, or the body of ephors.",
  "accessibility - The quality of being accessible, or of admitting approach; receptibility. Langhorne.",
  "ghostliness - The quality of being ghostly.",
  "rhinology - The science which treats of the nose, and its diseases.",
  "englue - To join or close fast together, as with glue; as, a coffer well englued. Gower.",
  "organographical - Of or pertaining to organography.",
  "stateful - Full of state; stately. [Obs.] \"A stateful silence.\" Marston.",
  "apellous - Destitute of skin. Brande & C.",
  "edomite - One of the descendants of Esau or Edom, the brother of Jacob; an Idumean.",
  "orphanhood - The state or condition of being an orphan; orphanage.",
  "sippling - Sipping often. [Obs.] \"Taken after a sippling sort.\" Holland.",
  "ean - To bring forth, as young; to yean. \"In eaning time.\" Shak.",
  "encrinitic - Pertaining to encrinites; encrinal.",
  "clangorous - Making a clangor; having a ringing, metallic sound.",
  "procrusteanize - To stretch or contract according to some rule or standard.",
  "amorously - In an amorous manner; fondly.",
  "doubleness - 1. The state of being double or doubled. 2. Duplicity; insincerity. Chaucer.",
  "crewel - Worsted yarn,, slackly twisted, used for embroidery.",
  "flippantness - State or quality of being flippant.",
  "bosk - A thicket; a small wood. \"Through bosk and dell.\" Sir W. Scott.",
  "scenographic - Of or pertaining to scenography; drawn in perspective. -- Scen`o*graph\"ic*al*ly, adv.",
  "bairn - A child. [Scot. & Prov. Eng.] Has he not well provided for the bairn ! Beau. & Fl.",
  "galactophagist - One who eats, or subsists on, milk.",
  "parvity - Littleness. [Obs.] Glanvill. Ray.",
  "postlude - A voluntary at the end of a service.",
  "self-reproof - The act of reproving one's self; censure of one's conduct by one's own judgment.",
  "solfeggiare - To sol-fa. See Sol-fa, v. i.",
  "retecious - Resembling network; retiform.",
  "understock - To supply insufficiently with stock. A. Smith.",
  "underproof - Containing less alcohol than proof spirit. See Proof spirit, under Spirit.",
  "crystallogeny - The science which pertains to the production of crystals.",
  "unwarp - To restore from a warped state; to cause to be linger warped.",
  "antipathist - One who has an antipathy. [R.] \"Antipathist of light.\" Coleridge.",
  "proprietorship - The state of being proprietor; ownership.",
  "naughtly - Naughtily; wrongly. [Obs.] because my parents naughtly brought me up. Mir. for Mag.",
  "admonitive - Admonitory. [R.] Barrow. -- Ad*mon\"i*tive*ly, adv.",
  "awakener - One who, or that which, awakens.",
  "series motor - (a) A series-wound motor. (b) A motor capable of being used in a series circuit.",
  "encephalology - The science which treats of the brain, its structure and functions.",
  "goddaughter - A female for whom one becomes sponsor at baptism.",
  "rabot - A rubber of hard wood used in smoothing marble to be polished. Knight.",
  "lichwort - An herb, the wall pellitory. See Pellitory.",
  "climatic - Of or pertaining to a climate; depending on, or limited by, a climate.",
  "abstersiveness - The quality of being abstersive. Fuller.",
  "decolorize - To deprive of color; to whiten. Turner. -- De*col`or*i*za\"tion, n.",
  "guardianship - The office, duty, or care, of a guardian; protection; care; watch.",
  "hindooism - The religious doctrines and rites of the Hindoos; Brahmanism.",
  "great-granddaughter - A daughter of one's grandson or granddaughter.",
  "cruciation - The act of torturing; torture; torment. [Obs.] Bp. Hall.",
  "epidictical - Serving to explain; demonstrative.",
  "triarchy - Government by three persons; a triumvirate; also, a country under three rulers. Holland.",
  "obconical - Conical, but having the apex downward; inversely conical.",
  "battologize - To keep repeating needlessly; to iterate. Sir T. Herbert.",
  "preferableness - The quality or state of being preferable.",
  "erubescence - The act of becoming red; redness of the skin or surface of anything; a blushing.",
  "stomatopodous - Of or pertaining to the Stomatopoda.",
  "aphaniptera - A group of wingless insects, of which the flea in the type. See Flea.",
  "dicephalous - Having two heads on one body; double-headed.",
  "annunciatory - Pertaining to, or containing, announcement; making known. [R.]",
  "didelphian - Of or relating to the Didelphia. -- n.  One of the Didelphia.",
  "drudgingly - In a drudging manner; laboriously.",
  "shingly - Abounding with shingle, or gravel.",
  "iron-hearted - Hard-hearted; unfeeling; cruel; as, an iron-hearted master. Cowper.",
  "houss - A saddlecloth; a housing. [Obs.] Dryden.",
  "brained - Supplied with brains. If th' other two be brained like us. Shak.",
  "cetene - An oily hydrocarbon, C16H32, of the ethylene series, obtained from spermaceti.",
  "crabfaced - Having a sour, disagreeable countenance. Beau & Fl.",
  "equinal - See Equine. \"An equinal shape.\" Heywood.",
  "self-satisfaction - The quality or state of being self-satisfied.",
  "amplexicaul - Clasping or embracing a stem, as the base of some leaves. Gray.",
  "sheepcote - A small inclosure for sheep; a pen; a fold.",
  "semicolumn - A half column; a column bisected longitudinally, or along its axis.",
  "episternal - Of or pertaining to the episternum.",
  "murrey - A dark red color. -- a.  Of a dark red color. Bacon.",
  "pigwidgeon - A cant word for anything petty or small. It is used by Drayton as the name of a fairy.",
  "fecundify - To make fruitful; to fecundate. Johnson.",
  "nucleolated - Having a nucleole, or second inner nucleus.",
  "adynamia - Considerable debility of the vital powers, as in typhoid fever. Dunglison.",
  "xanthate - A salt of xanthic; a xanthogenate.",
  "supersulphate - An acid sulphate. [Obs.]",
  "platter-faced - Having a broad, flat face.",
  "soutache - A kind of narrow braid, usually of silk; -- also known as Russian braid.",
  "churchly - Pertaining to, or suitable for, the church; ecclesiastical.",
  "cowardize - To render cowardly. [Obs.] God . . . cowardizeth . . . insolent spirits. Bp. Hall.",
  "mayfish - A common American minnow (Fundulus majalis). See Minnow.",
  "irresolvableness - The quality or state of being irresolvable; irresolvability.",
  "apostolicism - The state or quality of being apostolical.",
  "stupose - Composed of, or having, tufted or matted filaments like tow; stupeous.",
  "piffero - A fife; also, a rude kind of oboe or a bagpipe with an inflated skin for reservoir.",
  "fellah - A peasant or cultivator of the soil among the Egyptians, Syrians, etc. W. M. Thomson.",
  "gerontocracy - Government by old men. [R.] Gladstone.",
  "splanchnology - That part of anatomy which treats of the viscera; also, a treatise on the viscera.",
  "drayage - 1. Use of a dray. 2. The charge, or sum paid, for the use of a dray.",
  "exegetics - The science of interpretation or exegesis.",
  "au gratin - With a crust made by browning in the oven; as, spaghetti may be served au gratin.",
  "interpilaster - The interval or space between two pilasters. Elmes.",
  "oxycymene - Hydroxy cymene. Same as Carvacrol.",
  "self-active - Acting of one's self or of itself; acting without depending on other agents.",
  "chartographic - Same as Cartographer, Cartographic, Cartography, etc.",
  "circumcenter - The center of a circle that circumscribes a triangle.",
  "regerminate - To germinate again. Perennial plants regerminate several years successively. J. Lee.",
  "tainture - Taint; tinge; difilement; stain; spot. [R.] Shak.",
  "lexicographist - A lexicographer. [R.] Southey.",
  "hault - Lofty; haughty. [Obs.] Through support of countenance proud and hault. Spenser.",
  "ripienist - A player in the ripieno portion of an orchestra. See Ripieno.",
  "breadbasket - The stomach. [Humorous] S. Foote.",
  "decine - One of the higher hydrocarbons, C10H15, of the acetylene series; -- called also decenylene.",
  "completeness - The state of being complete.",
  "impennous - Having no wings, as some insects.",
  "seerfish - A scombroid food fish of Maderia (Cybium Commersonii).",
  "emphaticalness - The quality of being emphatic; emphasis.",
  "proseminary - A seminary which prepares pupils for a higher institution. T. Warton.",
  "trilocular - Having three cells or cavities; as, a trilocular capsule; a trilocular heart.",
  "baaing - The bleating of a sheep. Marryat.",
  "violator - One who violates; an infringer; a profaner; a ravisher.",
  "irresolvability - The quality of being irresolvable; irresolvableness.",
  "pneumo- - A combining form from Gr. a lung; as, pneumogastric, pneumology.",
  "theorematist - One who constructs theorems.",
  "primitial - Being of the first production; primitive; original. [Obs.] Ainsworth.",
  "retrofract - Refracted; as, a retrofract stem.",
  "mootman - One who argued moot cases in the inns of court.",
  "stipuled - Furnished with stipules, or leafy appendages.",
  "woolert - The barn owl. [Prov. Eng.] [Written also oolert, and owlerd.]",
  "marquisdom - A marquisate. [Obs.] \"Nobles of the marquisdom of Saluce.\" Holinshed.",
  "acquaintable - Easy to be acquainted with; affable. [Obs.] Rom. of R.",
  "canarese - Pertaining to Canara, a district of British India.",
  "spindleshanks - A person with slender shanks, or legs; -- used humorously or in contempt.",
  "mountebankery - The practices of a mountebank; quackery; boastful and vain pretenses.",
  "oculated - 1. Furnished with eyes. 2. Having spots or holes resembling eyes; ocellated.",
  "loord - A dull, stupid fellow; a drone. [Obs.] Spenser.",
  "whistlewood - The moosewood, or striped maple. See Maple.",
  "mirabilis - A genus of plants. See Four-o'clock.",
  "gamosepalous - Formed of united sepals; monosepalous.",
  "glyphographic - Of or pertaining to glyphography.",
  "twibilled - Armed or provided with a twibil or twibils.",
  "electively - In an elective manner; by choice.",
  "supercarbonate - A bicarbonate. [Obsoles.]",
  "euchloric - Relating to, or consisting of, euchlorine; as, euchloric . Davy.",
  "gastrointestinal - Of or pertaining to the stomach and intestines; gastroenteric.",
  "biographical - Of or pertaining to biography; containing biography. -- Bi`o*graph\"ic*al*ly, adv.",
  "love-sickness - The state of being love-sick.",
  "acquiescency - The quality of being acquiescent; acquiescence.",
  "acentric - Not centered; without a center.",
  "rubiginose - Having the appearance or color of iron rust; rusty-looking.",
  "malacopterygian - One of the Malacopterygii.",
  "scrotum - The bag or pouch which contains the testicles; the cod.",
  "porcine - Of or pertaining to swine; characteristic of the hog. \"Porcine cheeks.\" G. Eliot.",
  "interpretament - Interpretation. [Obs.] Milton.",
  "temptationless - Having no temptation or motive; as, a temptationless sin. [R.] Hammond.",
  "underlaborer - An assistant or subordinate laborer. Locke.",
  "zighyr - Same as Sicker. [Prov. Eng.] Raymond.",
  "portance - See Port, carriage, demeanor. [Obs.] Spenser. Shak.",
  "whity-brown - Of a color between white and brown. Pegge.",
  "squanderingly - In a squandering manner.",
  "landstreight - A narrow strip of land. [Obs.]",
  "fluidrachm - See Fluid dram, under Fluid. Pharm. of the U. S.",
  "vasty - Vast; immense. [R.] I can call spirits from the vasty deep. Shak.",
  "textuist - A textualist; a textman. [Obs.] The crabbed textualists of his time. Milton.",
  "top-proud - Proud to the highest degree. [R.] \"This top-proud fellow.\" Shak.",
  "enervative - Having power, or a tendency, to enervate; weakening. [R.]",
  "nectareal - 1. Nectareous. 2. (Bot.)  Of or pertaining to a nectary.",
  "conflictive - Tending to conflict; conflicting.  Sir W. Hamilton.",
  "vacuation - The act of emptying; evacuation. [R.]",
  "phonic - Of or pertaining to sound; of the nature of sound; acoustic. Tyndall.",
  "numismatist - One skilled in numismatics; a numismatologist.",
  "thermogen - Caloric; heat; regarded as a material but imponderable substance.",
  "raptor - A ravisher; a plunderer. [Obs.]",
  "commandable - Capable of being commanded.",
  "acrisy - 1. Inability to judge. 2. (Med.)  Undecided character of a disease. [Obs.]",
  "haematitic - Of a blood-red color; crimson; (Bot.) brownish red.",
  "canakin - A little can or cup. \"And let me the canakin clink.\" Shak.",
  "nailless - Without nails; having no nails.",
  "sparve - The hedge sparrow. [Prov. Eng.]",
  "telesmatical - Of or pertaining to telesms; magical. J. Gregory.",
  "gangion - A short line attached to a trawl. See Trawl, n.",
  "lophobranchiate - Of or pertaining to the Lophobranchii.",
  "guilder - A Dutch silver coin worth about forty cents; -- called also florin and gulden.",
  "obolo - A copper coin, used in the Ionian Islands, about one cent in value.",
  "curtation - The interval by which the curtate distance of a planet is less than the true distance.",
  "essayist - A writer of an essay, or of essays. B. Jonson.",
  "anury - Nonsecretion or defective secretion of urine; ischury.",
  "cryptic - Hidden; secret; occult. \"Her [nature's] more cryptic ways of working.\" Glanvill.",
  "plutocrat - One whose wealth gives him power or influence; one of the plutocracy.",
  "amber fish - A fish of the southern Atlantic coast (Seriola Carolinensis.)",
  "malefactress - A female malefactor. Hawthorne.",
  "dried - of Day. Also adj.; as, dried apples.",
  "petrogale - Any Australian kangaroo of the genus Petrogale, as the rock wallaby (P. penicillata).",
  "unmould - To change the form of; to reduce from any form. \"Unmolding reason's mintage.\" Milton.",
  "curvated - Bent in a regular form; curved.",
  "xeraphim - An old money of account in Bombay, equal to three fifths of a rupee.",
  "gazon - One of the pieces of sod used to line or cover parapets and the faces of earthworks.",
  "focimeter - (Photog.) An assisting instrument for focusing an object in or before a camera. Knight.",
  "ditheist - One who holds the doctrine of ditheism; a dualist. Cudworth.",
  "outcompass - To exceed the compass or limits of. Bacon.",
  "ruminantly - In a ruminant manner; by ruminating, or chewing the cud.",
  "proxenet - A negotiator; a factor. [R.] Dr. H. More.",
  "erectness - Uprightness of posture or form.",
  "sclerogenous - Making or secreting a hard substance; becoming hard.",
  "lomatinous - Furnished with lobes or flaps.",
  "linkman - A boy or man that carried a link or torch to light passengers.",
  "scoptical - Jesting; jeering; scoffing. [Obs.] South. -- Scop\"tic*al*ly, adv. [Obs.]",
  "trompe - A trumpet; a trump. [Obs.] Chaucer.",
  "superincumbence - The quality or state of being superincumbent.",
  "iron-sided - Having iron sides, or very firm sides.",
  "appetition - Desire; a longing for, or seeking after, something. Holland.",
  "platanus - A genus of trees; the plane tree.",
  "schematize - To form a scheme or schemes.",
  "chloroplastid - A granule of chlorophyll; -- also called chloroleucite.",
  "pentacrinite - Any species of Pentacrinus.",
  "lentando - Slackening; retarding. Same as Rallentando.",
  "rug-headed - Having shaggy hair; shock-headed. [Obs.] Those rough rug-headed kerns. Shak.",
  "brimstony - Containing or resembling brimstone; sulphurous. B. Jonson.",
  "duffel bag - A sack to hold miscellaneous articles, as tools, supplies, or the like.",
  "arthrodesis - Surgical fixation of joints.",
  "cackler - 1. A fowl that cackles. 2. One who prattles, or tells tales; a tattler.",
  "seizor - One who seizes, or takes possession.",
  "waymaker - One who makes a way; a precursor. [R.] Bacon.",
  "hallier - A kind of net for catching birds.",
  "phragmosiphon - The siphon of a phragmocone.",
  "stayed - Staid; fixed; settled; sober; -- now written staid. See Staid. Bacon. Pope.",
  "incastellated - Confined or inclosed in a castle.",
  "sextoness - A female sexton; a sexton's wife.",
  "coagmentation - The act of joining, or the state of being joined, together; union. [Obs.] B. Jonson.",
  "undersuit - A suit worn under another suit; a suit of underclothes.",
  "semiradial engine - See Radial engine, above.",
  "cheliferous - Having cheliform claws, like a crab.",
  "incavated - Made hollow; bent round or in.",
  "amentia - Imbecility; total want of understanding.",
  "sea breach - A breaking or overflow of a bank or a dike by the sea. L'Estrange.",
  "antiquarianize - To act the part of an antiquary. [Colloq.]",
  "indigene - One born in a country; an aboriginal animal or plant; an autochthon. Evelyn. Tylor.",
  "dextrad - Toward the right side; dextrally.",
  "cleanser - One who, or that which, cleanses; a detergent. Arbuthnot.",
  "educated - Formed or developed by education; as, an educated man.",
  "hickway - The lesser spotted woodpecker (Dendrocopus minor) of Europe. [Prov. Eng.]",
  "skullfish - A whaler's name for a whale more than two years old.",
  "leave-taking - Taking of leave; parting compliments. Shak.",
  "antisocialist - One opposed to the doctrines and practices of socialists or socialism.",
  "grangerite - One who collects illustrations from various books for the decoration of one book.",
  "lignitiferous - Producing or containing lignite; lignitic.",
  "portability - The quality or state of being portable; fitness to be carried.",
  "arrectary - An upright beam. [Obs.] Bp. Hall.",
  "pongo - Any large ape; especially, the chimpanzee and the orang-outang.",
  "killigrew - The Cornish chough. See under Chough. [Prov. Eng. & Scot.]",
  "friarly - Like a friar; inexperienced. Bacon.",
  "bellower - One who, or that which, bellows.",
  "deplorableness - State of being deplorable.",
  "compassionateness - The quality or state of being compassionate.",
  "mortress - A dish of meats and other ingredients, cooked together; an ollapodrida. Chaucer. Bacon.",
  "fibrillated - Furnished with fibrils; fringed.",
  "inquisible - Admitting judicial inquiry. [Obs.] Sir M. Hale.",
  "berthing - The planking outside of a vessel, above the sheer strake. Smyth.",
  "colation - The act or process of straining or filtering. [R.]",
  "dissympathy - Lack of sympathy; want of interest; indifference. [R.]",
  "matamata - The bearded tortoise (Chelys fimbriata) of South American rivers.",
  "overaffect - To affect or care for unduly. [Obs.] Milton.",
  "tiredness - The state of being tired, or weary.",
  "hamous - Having the end hooked or curved.",
  "salsify - See Oyster plant (a), under Oyster.",
  "sparagrass - Obs. or corrupt forms of Asparagus.",
  "phrenic - Of or pertaining to the diaphragm; diaphragmatic; as, the phrenic nerve.",
  "baenopod - One of the thoracic legs of Arthropods.",
  "lucubrator - One who studies by night; also, one who produces lucubrations.",
  "pediatrics - That branch of medical science which treats of the hygiene and diseases of children.",
  "bemonster - To make monstrous or like a monster. [Obs.] Shak.",
  "extorsive - Serving or tending to extort. [R.] Johnson. -- Ex*tor\"sive*ly, adv. [R.]",
  "debutante - A person who makes his (or her) first appearance before the public.",
  "melancholist - One affected with melancholy or dejection. [Obs.] Glanvill.",
  "prefigurate - To prefigure. [R.] Grafton.",
  "exustion - The act or operation of burning up. Bailey.",
  "coagent - An associate in an act; a coworker. Drayton.",
  "endoplasm - The protoplasm in the interior of a cell.",
  "improlific - Not prolific. [Obs.] E. Waterhouse.",
  "pappy - Like pap; soft; succulent; tender. Ray.",
  "anoplura - A group of insects which includes the lice.",
  "androgynism - Union of both sexes in one individual; hermaphroditism.",
  "tillable - Capable of being tilled; fit for the plow; arable.",
  "conjecturalist - A conjecturer. [R.] Month. rev.",
  "horsemanship - The act or art of riding, and of training and managing horses; manege.",
  "nightman - One whose business is emptying privies by night.",
  "gowned - Dressed in a gown; clad. Gowned in pure white, that fitted to the shape. Tennyson.",
  "unilabiate - Having one lip only; as, a unilabiate corolla.",
  "acosmism - A denial of the existence of the universe as distinct from God.",
  "petrean - Of or pertaining to to rock. G. S. Faber.",
  "streetwalker - A common prostitute who walks the streets to find customers.",
  "thickening - Something put into a liquid or mass to make it thicker.",
  "northwester - A storm or gale from the northwest; a strong northwest wind.",
  "perspicil - An optical glass; a telescope. [Obs.] Crashaw.",
  "eupatorin - A principle or mixture of principles extracted from various species of Eupatorium.",
  "pimola - An olive stuffed with a kind of sweet red pepper, or pimiento.",
  "daguerreotyper - One who takes daguerreotypes.",
  "raia - A genus of rays which includes the skates. See Skate.",
  "judiciously - In a judicious manner; with good judgment; wisely.",
  "glumal - Characterized by a glume, or having the nature of a glume.",
  "ichthyomorphous - Fish-shaped; as, the ichthyomorphic idols of ancient Assyria.",
  "expansile - Expansible. Ether and alcohol are more expansile than water. Brande & C.",
  "analectic - Relating to analects; made up of selections; as, an analectic magazine.",
  "priced - Rated in price; valued; as, high-priced goods; low-priced labor.",
  "ender - One who, or that which, makes an end of something; as, the ender of my life.",
  "ghazal - A kind of Oriental lyric, and usually erotic, poetry, written in recurring rhymes.",
  "demonomagy - Magic in which the aid of demons is invoked; black or infernal magic. Bp. Hurd.",
  "flashily - In a flashy manner; with empty show.",
  "modifiable - Capable of being modified; liable to modification.",
  "nonagenarian - A person ninety years old.",
  "breastbone - The bone of the breast; the sternum.",
  "gonorrheal - Of or pertaining to gonorrhea; as, gonorrheal rheumatism.",
  "exaeresis - In old writers, the operations concerned in the removal of parts of the body.",
  "delenda - Things to be erased or blotted out.",
  "gastriloquist - One who appears to speak from his stomach; a ventriloquist.",
  "brabbler - A clamorous, quarrelsome, noisy fellow; a wrangler. [R] Shak.",
  "water chicken - The common American gallinule.",
  "embraid - 1. To braid up, as hair. [Obs.] Spenser. 2. To upbraid. [Obs.] Sir T. Elyot.",
  "firetail - The European redstart; -- called also fireflirt. [prov. Eng.]",
  "nemophilist - One who is fond of forest or forest scenery; a haunter of the woods. [R.]",
  "miscounsel - To counsel or advise wrongly.",
  "egritude - Sickness; ailment; sorrow. [Obs.] Sir T. Elyot.",
  "solubleness - Quality or state of being soluble.",
  "swiftly - In a swift manner; with quick motion or velocity; fleetly. Wyclif.",
  "windtight - So tight as to prevent the passing through of wind. Bp. Hall.",
  "flingdust - One who kicks up the dust; a streetwalker; a low manner. [Obs.] Beau. & Fl.",
  "unthriftihood - Untriftiness. [Obs.] Spenser.",
  "dentistic - Pertaining to dentistry or to dentists. [R.]",
  "weather-driven - Driven by winds or storms; forced by stress of weather. Carew.",
  "consortship - The condition of a consort; fellowship; partnership. Hammond.",
  "bahaism - The religious tenets or practices of the Bahais.",
  "barathea - A soft fabric with a kind of basket weave and a diapered pattern.",
  "abstrude - To thrust away. [Obs.] Johnson.",
  "spurling - A tern. [Obs. or Prov. Eng.] Tusser.",
  "suggestress - A woman who suggests. \"The suggestress of suicides.\" De Quincey.",
  "myophan - A contractile striated layer found in the bodies and stems of certain Infusoria.",
  "oxysalt - A salt of an oxyacid, as a sulphate.",
  "zinciferous - Containing or affording zinc.",
  "streamless - Destitute of streams, or of a stream, as a region of country, or a dry channel.",
  "uncling - To cease from clinging or adhering. [Obs.] Milton.",
  "phyllocyanin - A blue coloring matter extracted from chlorophyll. [Written also phyllocyanine.]",
  "seashell - The shell of any marine mollusk.",
  "egranulose - Having no granules, as chlorophyll in certain conditions. R. Brown.",
  "abandonee - One to whom anything is legally abandoned.",
  "barysphere - The heavy interior portion of the earth, within the lithosphere.",
  "crow-quill - A quill of the crow, or a very fine pen made from such a quill.",
  "modicity - Moderateness; smallness; meanness. [Obs.]",
  "strene - Race; offspring; stock; breed; strain. [Obs.] Chaucer.",
  "florescent - Expanding into flowers; blossoming.",
  "fruitery - 1. Fruit, taken collectively; fruitage. J. Philips. 2. A repository for fruit. Johnson.",
  "reafforestation - The act or process of converting again into a forest.",
  "citole - A musical instrument; a kind of dulcimer. [Obs.]",
  "negotiant - A negotiator. [R.] Sir W. Raleigh.",
  "pyelitis - Inflammation of the pelvis of the kidney.",
  "sea salmon - (a) A young pollock. (b) The spotted squeteague. (c) See Sea bass (b).",
  "testamentize - To make a will. [Obs.] Fuller.",
  "abaist - Abashed; confounded; discomfited. [Obs.] Chaucer.",
  "adverseness - The quality or state of being adverse; opposition.",
  "inamiable - Unamiable. [Obs.] -- In*a\"mi*a*ble*ness, n. [Obs.]",
  "calcareo-bituminous - Consisting of, or containing, lime and bitumen. Lyell.",
  "barcon - A vessel for freight; -- used in Mediterranean.",
  "coolish - Somewhat cool. The nights began to grow a little coolish. Goldsmith.",
  "regulable - Capable of being regulated. [R.]",
  "sirenize - To use the enticements of a siren; to act as a siren; to fascinate.",
  "phasm - An apparition; a phantom; an appearance. [R.] Hammond. Sir T. Herbert.",
  "vase-shaped - Formed like a vase, or like a common flowerpot.",
  "somnour - A summoner; an apparitor; a sompnour. [Obs.] Piers Plowman.",
  "liquidness - The quality or state of being liquid; liquidity; fluency.",
  "antiarin - A poisonous principle obtained from antiar. Watts.",
  "axially - In relation to, or in a line with, an axis; in the axial (magnetic) line.",
  "diaphoresis - Perspiration, or an increase of perspiration.",
  "overready - Too ready. -- O\"ver*read\"*i*ly, adv. -- O\"ver*read\"i*ness, n.",
  "fluentness - The quality of being fluent.",
  "alternatively - In the manner of alternatives, or that admits the choice of one out of two things.",
  "self-devouring - Devouring one's self or itself. Danham.",
  "acerb - Sour, bitter, and harsh to the taste, as unripe fruit; sharp and harsh.",
  "xanthopous - Having a yellow stipe, or stem.",
  "citified - Aping, or having, the manners of a city.",
  "hater - One who hates. An enemy to God, and a hater of all good. Sir T. Browne.",
  "angulo-dentate - Angularly toothed, as certain leaves.",
  "organista - Any one of several South American wrens, noted for the sweetness of their song.",
  "lopseed - A perennial herb (Phryma Leptostachya), having slender seedlike fruits.",
  "stalder - A wooden frame to set casks on. [Prov. Eng.]",
  "idolizer - One who idolizes or loves to the point of reverence; an idolater.",
  "sammier - A machine for pressing the water from skins in tanning. Knight.",
  "wilne - To wish; to desire. [Obs.] \"He willneth no destruction.\" Chaucer.",
  "octopede - An animal having eight feet, as a spider.",
  "lochial - Of or pertaining to the lochia.",
  "steg - A gander. [Written also stag.] [Prov. Eng.] Halliwell.",
  "senora - A Spanish title of courtesy given to a lady; Mrs.; Madam; also, a lady.",
  "cetological - Of or pertaining to cetology.",
  "kickup - The water thrush or accentor. [Local, West Indies]",
  "refect - To restore after hunger or fatique; to refresh. [Archaic] Sir T. Browne.",
  "endogamous - Marrying within the same tribe; -- opposed to exogamous.",
  "ischury - A retention or suppression of urine.",
  "interauricular - Between the auricles; as, the interauricular partition of the heart.",
  "imbastardize - To bastardize; to debase. [Obs.] Milton.",
  "sea dove - The little auk, or rotche. See Illust. of Rotche.",
  "seasonless - Without succession of the seasons.",
  "shopworn - Somewhat worn or damaged by having been kept for a time in a shop.",
  "vitilitigation - Cavilous litigation; cavillation. [Obs.] Hudibras.",
  "fil - imp. of Fall, v. i. Fell. Chaucer.",
  "frau - In Germany, a woman; a married woman; a wife; -- as a title, equivalent to Mrs., Madam.",
  "maa - The common European gull (Larus canus); -- called also mar. See New, a gull.",
  "footmanship - Art or skill of a footman.",
  "unbeseeming - Unbecoming; not befitting. -- Un`be*seem\"ing*ly, adv. -- Un`be*seem\"ing*ness, n.",
  "hexateuch - The first six books of the Old Testament.",
  "potsherd - A piece or fragment of a broken pot. Job ii. 8.",
  "sensualistic - 1. Sensual. 2. Adopting or teaching the doctrines of sensualism.",
  "widowerhood - The state of being a widower.",
  "epidemiography - A treatise upon, or history of, epidemic diseases.",
  "figurante - A female figurant; esp., a ballet girl.",
  "recompact - To compact or join anew. \"Recompact my scattered body.\" Donne.",
  "horseshoeing - The act or employment of shoeing horses.",
  "nap-taking - A taking by surprise; an unexpected onset or attack. Carew.",
  "exauthoration - Deprivation of authority or dignity; degration. [Obs.] Jer. Taylor.",
  "monarchist - An advocate of, or believer in, monarchy.",
  "hibernal - Belonging or relating to winter; wintry; winterish. Sir T. Browne.",
  "topically - In a topical manner; with application to, or limitation of, a particular place or topic.",
  "noumenal - Of or pertaining to the noumenon; real; -- opposed to phenomenal. G. H. Lewes.",
  "archway - A way or passage under an arch.",
  "phono - A South American butterfly (Ithonia phono) having nearly transparent wings.",
  "basifier - That which converts into a salifiable base.",
  "redoubting - Reverence; honor. [Obs.] In redoutyng of Mars and of his glory. Chaucer.",
  "usager - One who has the use of anything in trust for another. [Obs.] Daniel.",
  "rimau dahan - The clouded tiger cat (Felis marmorata) of Southern Asia and the East Indies.",
  "convalesced - Convalescent. [R.] He found the queen somewhat convalesced. J. Knox.",
  "icarian - Soaring too high for safety, like Icarus; adventurous in flight.",
  "libken - A house or lodging. [Old Slang] B. Jonson.",
  "rubefaction - The act or process of making red.",
  "revet - To face, as an embankment, with masonry, wood, or other material.",
  "impenetrableness - The quality of being impenetrable; impenetrability.",
  "declivitous - Descending gradually; moderately steep; sloping; downhill.",
  "bedevilment - The state of being bedeviled; bewildering confusion; vexatious trouble. [Colloq.]",
  "transfreight - To transfrete. [Obs.] Waterhouse.",
  "heyne - A wretch; a rascal. [Obs.] Chaucer.",
  "glossolalia - The gift of tongues. Farrar.",
  "blockage - The act of blocking up; the state of being blocked up.",
  "cabaret - A tavern; a house where liquors are retailed. [Obs. as an English word.]",
  "shipload - The load, or cargo, of a ship.",
  "tricostate - Three-ribbed; having three ribs from the base.",
  "nickeline - 1. (Chem.)  An alloy of nickel, a variety of German silver. 2. (Min.)  Niccolite.",
  "impleasing - Unpleasing; displeasing. [Obs.] Overbury.",
  "maximize - To increase to the highest degree. Bentham.",
  "explorable - That may be explored; as, an explorable region.",
  "ombrometer - An instrument for measuring the rain that falls; a rain gauge.",
  "mimographer - A writer of mimes. Sir T. Herbert.",
  "distractedly - Disjointedly; madly. Shak.",
  "mechanurgy - That branch of science which treats of moving machines.",
  "tomium - The cutting edge of the bill of a bird.",
  "clustery - Growing in, or full of, clusters; like clusters. Johnson.",
  "protestantly - Like a Protestant; in conformity with Protestantism. [R.] Milton.",
  "untongue - To deprive of a tongue, or of voice. [Obs.] Fuller.",
  "coryphodont - Pertaining to, or resembling, the genus Coryphodon.",
  "thermocurrent - A current, as of electricity, developed, or set in motion, by the action of heat.",
  "privado - A private friend; a confidential friend; a confidant. [Obs.] Fuller.",
  "toze - To pull violently; to touse. [Obs.]",
  "self-mettle - Inborn mettle or courage; one's own temper. [Obs.] Shak.",
  "miseased - Having discomfort or misery; troubled. [Obs.] Chaucer.",
  "balsa - A raft or float, used principally on the Pacific coast of South America.",
  "penninerved - Pinnately veined or nerved.",
  "recrementitious - Of or pertaining to recrement; consisting of recrement or dross. Boyle.",
  "speckled-belly - The gadwall. [Local, U.S.]",
  "scopulous - Full of rocks; rocky. [Obs.]",
  "forespeech - A preface. [Obs.] Sherwood.",
  "apotheosize - To exalt to the dignity of a deity; to declare to be a god; to deify; to glorify.",
  "unperplex - To free from perplexity. [R.] Donne.",
  "empyrosis - A general fire; a conflagration. [Obs.] Sir M. Hale.",
  "bombic - Pertaining to, or obtained from, the silkworm; as, bombic acid.",
  "spouter - One who, or that which, spouts.",
  "lapidific - Forming or converting into stone.",
  "thousand legs - A millepid, or galleyworm; -- called also thousand-legged worm.",
  "lectionary - A book, or a list, of lections, for reading in divine service.",
  "photomagnetic - Of or pertaining to photomagnetism.",
  "christianite - (a) Same as Anorthite. [R.] (b) See Phillipsite.",
  "stemless - Having no stem; (Bot.) acaulescent.",
  "phthisicky - Having phthisis, or some symptom of it, as difficulty in breathing.",
  "decussatively - Crosswise; in the form of an X. \"Anointed decussatively.\" Sir T. Browne.",
  "acarpous - Not producing fruit; unfruitful.",
  "discernibleness - The quality of being discernible.",
  "branchiomerism - The state of being made up of branchiate segments. R. Wiedersheim.",
  "espousement - The act of espousing, or the state of being espoused.",
  "fogger - One who fogs; a pettifogger. [Obs.] A beggarly fogger. Terence in English(1614)",
  "jennet - A small Spanish horse; a genet.",
  "extensively - To a great extent; widely; largely; as, a story is extensively circulated.",
  "ferrary - The art of working in iron. [Obs.] Chapman.",
  "understandable - Capable of being understood; intelligible. Chillingworth.",
  "thermotic - Of or pertaining to heat; produced by heat; as, thermotical phenomena. Whewell.",
  "forblack - Very black. [Obs.] As any raven's feathers it shone forblack. Chaucer.",
  "peppering - Hot; pungent; peppery. Swift.",
  "austro-hungarian - Of or pertaining to the monarchy composed of Austria and Hungary.",
  "agenesis - Any imperfect development of the body, or any anomaly of organization.",
  "quasimodo - The first Sunday after Easter; Low Sunday.",
  "male-spirited - Having the spirit of a male; vigorous; courageous. [R.] B. Jonson.",
  "confessionalist - A priest hearing, or sitting to hear, confession. [R.] Boucher",
  "chattelism - The act or condition of holding chattels; the state of being a chattel.",
  "encephalon - The contents of the cranium; the brain.",
  "medically - In a medical manner; with reference to healing, or to the principles of the healing art.",
  "visionariness - The quality or state of being visionary.",
  "angelify - To make like an angel; to angelize. [Obs.] Farindon (1647).",
  "lanated - Wooly; covered with fine long hair, or hairlike filaments.",
  "unsuffering - Inability or incapability of enduring, or of being endured. [Obs.] Wyclif.",
  "hirudo - A genus of leeches, including the common medicinal leech. See Leech.",
  "sporulation - The act or process of forming spores; spore formation. See Illust. of Bacillus, b.",
  "carroty - Like a carrot in color or in taste; -- an epithet given to reddish yellow hair, etc.",
  "veliferous - Carrying or bearing sails. [Obs.] \"Veliferous chariots.\" Evelyn.",
  "demulsion - The act of soothing; that which soothes. Feltham.",
  "backdown - A receding or giving up; a complete surrender. [Colloq.]",
  "exceedable - Capable of exceeding or surpassing. [Obs.] Sherwood.",
  "moderatorship - The office of a moderator.",
  "overbid - To bid or offer beyond, or in excess of. Dryden.",
  "dashy - Calculated to arrest attention; ostentatiously fashionable; showy. [Colloq.]",
  "agronomist - One versed in agronomy; a student of agronomy.",
  "wither-wrung - Injured or hurt in the withers, as a horse.",
  "sinistrorsal - Rising spirally from right to left (of the spectator); sinistrorse.",
  "diesinker - An engraver of dies for stamping coins, medals, etc.",
  "diradiation - The emission and diffusion of rays of light.",
  "foveola - A small depression or pit; a fovea.",
  "hyperorganic - Higher than, or beyond the sphere of, the organic. Sir W. Hamilton.",
  "antiphrastical - Pertaining to antiphrasis. -- An`ti*phras\"tic*al*ly, adv.",
  "zealed - Full of zeal; characterized by zeal. [Obs.] \"Zealed religion.\" Beau. & Fl.",
  "salework - Work or things made for sale; hence, work done carelessly or slightingly. Shak.",
  "chiromantic - Of or pertaining to chiromancy.",
  "modifiability - Capability of being modified; state or quality of being modifiable.",
  "allegorizer - One who allegorizes, or turns things into allegory; an allegorist.",
  "abacinate - To blind by a red-hot metal plate held before the eyes. [R.]",
  "definement - The act of defining; definition; description. [Obs.] Shak.",
  "demurity - Demureness; also, one who is demure. Sir T. Browne.",
  "leatherette - An imitation of leather, made of paper and cloth.",
  "paraboliform - Resembling a parabola in form.",
  "agrostologist - One skilled in agrostology.",
  "issuer - One who issues, emits, or publishes.",
  "microseismograph - A microseismometer; specif., a microseismometer producing a graphic record.",
  "pentangle - A pentagon. [R.] Sir T. Browne.",
  "boozy - A little intoxicated; fuddled; stupid with liquor; bousy. [Colloq.] C. Kingsley.",
  "self-considering - Considering in one's own mind; deliberating. Pope.",
  "sadducize - To adopt the principles of the Sadducees. Atterbury.",
  "debauchment - The act of corrupting; the act of seducing from virtue or duty.",
  "fiveling - A compound or twin crystal consisting of five individuals.",
  "lobulette - A little lobule, or subdivision of a lobule.",
  "nucleoidioplasma - Hyaline plasma contained in the nucleus of vegetable cells.",
  "abrase - Rubbed smooth. [Obs.] \"An abrase table.\" B. Jonson.",
  "helmet-shaped - Shaped like a helmet; galeate. See Illust. of Galeate.",
  "defectuous - Full of defects; imperfect. [Obs.] Barrow.",
  "torosity - The quality or state of being torose.",
  "liturgics - The science of worship; history, doctrine, and interpretation of liturgies.",
  "plantable - Capable of being planted; fit to be planted. B. Edwards.",
  "rubelle - A red color used in enameling. Weale.",
  "pockmarked - Marked by smallpox; pitted.",
  "rhinoscopic - Of or pertaining to rhinoscopy.",
  "herniotomy - A cutting for the cure or relief of hernia; celotomy.",
  "innumerability - State of being innumerable. Fotherby.",
  "strangleable - Capable of being strangled. [R.] Chesterfield.",
  "phrasal - Of the nature of a phrase; consisting of a phrase; as, a phrasal adverb. Earlc.",
  "appalling - Such as to appall; as, an appalling accident. -- Ap*pall\"ing*ly, adv.",
  "cracked - 1. Coarsely ground or broken; as, cracked wheat. 2. Crack-brained. [Colloq.]",
  "ambrose - A sweet-scented herb; ambrosia. See Ambrosia, 3. Turner.",
  "bewreke - To wreak; to avenge. [Obs.] Ld. Berners.",
  "cleverly - In a clever manner. Never was man so clever absurd. C. Smart.",
  "dandified - Made up like a dandy; having the dress or manners of a dandy; buckish.",
  "cross-vaulting - Vaulting formed by the intersection of two or more simple vaults.",
  "ring-streaked - Having circular streaks or lines on the body; as, ring-streaked goats.",
  "self-colored - Being of a single color; -- applied to flowers, animals, and textile fabrics.",
  "suburethral - Situated under the urethra, or under its orifice.",
  "incogitable - Not cogitable; inconceivable. Sir T. More.",
  "allhallows - 1. All the saints (in heaven). [Obs.] 2. All Saints' Day, November 1st. [Archaic]",
  "negoce - Business; occupation. [Obs.] Bentley.",
  "smokeless powder - A high-explosive gunpowder whose explosion produces little, if any, smoke.",
  "claire - A small inclosed pond used for gathering and greening oysters.",
  "gastight - So tightly fitted as to preclude the escape of gas; impervious to gas.",
  "largition - The bestowment of a largess or gift. [Obs.]",
  "inbarge - To embark; to go or put into a barge. [Obs.] Drayton.",
  "phytomeron - An organic element of a flowering plant; a phyton.",
  "sulpharsenite - A salt of sulpharsenious acid.",
  "intractile - Not tractile; incapable of being drawn out or extended. Bacon.",
  "unsheriff - To depose from the office of sheriff. [R.]",
  "audibleness - The quality of being audible.",
  "prongbuck - (a) The springbuck. (b) The pronghorn.",
  "impostorship - The condition, character, or practice of an impostor. Milton.",
  "progeniture - A begetting, or birth. [R.]",
  "stonebrash - A subsoil made up of small stones or finely-broken rock; brash.",
  "overpersuade - To persuade or influence against one's inclination or judgment. Pope.",
  "centiloquy - A work divided into a hundred parts. [R.] Burton.",
  "archaeozoic - Like or belonging to the earliest forms of animal life.",
  "uncapable - Incapable. [Obs.] \"Uncapable of conviction.\" Locke.",
  "cycling - The act, art, or practice, of riding a cycle, esp. a bicycle or tricycle.",
  "unbenevolence - Absence or want of benevolence; ill will.",
  "rackarock - A Sprengel explosive consisting of potassium chlorate and mono- nitrobenzene.",
  "underload starter - A motor starter provided with an underload switch."
]