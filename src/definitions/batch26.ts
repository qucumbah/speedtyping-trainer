export default [
  "steersmate - One who steers; steersman. [Obs.] Milton.",
  "binotonous - Consisting of two notes; as, a binotonous cry.",
  "cuneate - Wedge-shaped; (Bot.), wedge-shaped, with the point at the base; as, a cuneate leaf.",
  "thermotropic - Manifesting thermotropism.",
  "banderillero - One who thrusts in the banderillas in bullfighting.  W. D. Howells.",
  "membranology - The science which treats of membranes.",
  "chooser - One who chooses; one who has the power or right of choosing; an elector. Burke.",
  "breachy - Apt to break fences or to break out of pasture; unruly; as, breachy cattle.",
  "callisthenic - See Calisthenic, Calisthenics.",
  "cobourg - A thin worsted fabric for women's dresses.",
  "anchorless - Without an anchor or stay. Hence: Drifting; unsettled.",
  "azotite - A salt formed by the combination of azotous, or nitrous, acid with a base; a nitrite. [R.]",
  "redeless - Without rede or counsel. [Obs.]",
  "anatocism - Compound interest. [R.] Bouvier.",
  "punctuator - One who punctuates, as in writing; specifically, a punctator.",
  "majusculae - Capital letters, as found in manuscripts of the sixth century and earlier.",
  "perimysial - (a) Surrounding a muscle or muscles. (b) Of or pertaining to the perimysium.",
  "waterproofing - 1. The act or process of making waterproof. 2. Same as Waterproof, n., 1.",
  "ineligibility - The state or quality of being ineligible.",
  "herber - A garden; a pleasure garden. [Obs.] \"Into an herber green.\" Chaucer.",
  "plot-proof - Secure against harm by plots. Shak.",
  "supraocular - Above the eyes; -- said of certain scales of fishes and reptiles.",
  "circumfluence - A flowing round on all sides; an inclosing with a fluid.",
  "roborate - To give strength or support to; to confirm. [Obs.] Fuller.",
  "perceptivity - The quality or state of being perceptive; power of perception. Locke.",
  "disleal - Disloyal; perfidious. [Obs.] \"Disleal knight.\" Spenser.",
  "misbegotten - Unlawfully or irregularly begotten; of bad origin; pernicious. \"Valor misbegot.\" Shak.",
  "irrepentance - Want of repentance; impenitence. Bp. Montagu.",
  "exceptionless - Without exception. A universal, . . . exceptionless disqualification. Bancroft.",
  "sifac - The white indris of Madagascar. It is regarded by the natives as sacred.",
  "quinoxyl - The hypothetical radical of certain quinone derivatives related to rhodizonic acid.",
  "proant - Provender or food. [Obs.] One pease was a soldier's provant a whole day. Beau. & Fl.",
  "turgesce - To become turgid; to swell or be inflated. [R.]",
  "floriculturist - One skilled in the cultivation of flowers; a florist.",
  "disburgeon - To strip of burgeons or buds; to disbud. [R.] Holland.",
  "eperlan - The European smelt (Osmerus eperlanus).",
  "emmantle - To cover over with, or as with, a mantle; to put about as a protection. [Obs.] Holland.",
  "stonebird - The yellowlegs; -- called also stone snipe. See Tattler, 2. [Local, U.S.]",
  "encounterer - One who encounters; an opponent; an antagonist. Atterbury.",
  "without-door - Outdoor; exterior. [Obs.] \"Her without-door form.\" Shak.",
  "feyre - A fair or market. [Obs.] Chaucer.",
  "cheesy - Having the nature, qualities, taste, form, consistency, or appearance of cheese.",
  "interpretable - Admitting of interpretation; capable of being interpreted or explained.",
  "unifier - One who, or that which, unifies; as, a natural law is a unifier of phenomena.",
  "evolutional - Relating to evolution. \"Evolutional changes.\" H. Spenser.",
  "braveness - The quality of state or being brave.",
  "argillo-calcareous - Consisting of, or containing, clay and calcareous earth.",
  "lardy - Containing, or resembling, lard; of the character or consistency of lard.",
  "loftiness - The state or quality of being lofty.",
  "ichthyologist - One versed in, or who studies, ichthyology.",
  "logographic - Of or pertaining to logography.",
  "peperino - A volcanic rock, formed by the cementing together of sand, scoria, cinders, etc.",
  "antiphrastic - Pertaining to antiphrasis. -- An`ti*phras\"tic*al*ly, adv.",
  "-let - A noun suffix having a diminutive force; as in streamlet, armlet.",
  "promissorily - In a promissory manner. Sir T. Browne.",
  "goby - One of several species of small marine fishes of the genus Gobius and allied genera.",
  "ridgebone - The backbone. [Obs.] Blood . . . lying cluttered about the ridgebone. Holland.",
  "strunt - Spirituous liquor. [Scot.] Burns.",
  "machicolated - Having machicolations. \"Machicolated turrets.\" C. Kingsley.",
  "frugivorous - Feeding on fruit, as birds and other animals. Pennant.",
  "amplifier - One who or that which amplifies.",
  "winterweed - A kind of speedwell (Veronica hederifolia) which spreads chiefly in winter. Dr. Prior.",
  "brander - 1. One who, or that which, brands; a branding iron. 2. A gridiron. [Scot.]",
  "pisciform - Having the form of a fish; resembling a fish.",
  "intussuscepted - Received into some other thing or part, as a sword into a sheath; invaginated.",
  "hebraically - After the manner of the Hebrews or of the Hebrew language.",
  "jestful - Given to jesting; full of jokes.",
  "uranoplasty - The plastic operation for closing a fissure in the hard palate.",
  "archonts - The group including man alone.",
  "hast - , 2d pers. sing. pres. of. Fave, contr. of havest. [Archaic]",
  "jambee - A fashionable cane. [Obs.] Tatler.",
  "envoyship - The office or position of an envoy.",
  "podagra - Gout in the joints of the foot; -- applied also to gout in other parts of body.",
  "cheeked - Having a cheek; -- used in composition. \"Rose-cheeked Adonis.\" Shak.",
  "altazimuth - An instrument for taking azimuths and altitudes simultaneously.",
  "zumological - See Zymic, Zymological, etc.",
  "contrarotation - Circular motion in a direction contrary to some other circular motion.",
  "yearnful - Desirous. [Obs.] Ormulum. P. Fletcher.",
  "monarchic - Of or pertaining to a monarch, or to monarchy. Burke. -- Mo*nar\"chic*al*ly, adv.",
  "insatiately - Insatiably. Sir T. Herbert.",
  "irrefrangibility - The quality or state of being irrefrangible; irrefrangibleness.",
  "diphthongic - Of the nature of diphthong; diphthongal. H. Sweet.",
  "silicofluoride - A fluosilicate; a salt of silicofluoric acid.",
  "perpession - Suffering; endurance. [Obs.] Bp. Pearson.",
  "asthenic - Characterized by, or pertaining to, debility; weak; debilitating.",
  "adenosclerosis - The hardening of a gland.",
  "melanistic - Affected with melanism; of the nature of melanism.",
  "opiniator - One who is opinionated. [Obs.] South. Barrow.",
  "justicoat - Formerly, a close coat or waistcoat with sleeves.",
  "interconnection - Connection between; mutual connection.",
  "hydrous - 1. Containing water; watery. 2. (Chem.)  Containing water of hydration or crystallization.",
  "snacot - A pipefish of the genus Syngnathus. See Pipefish.",
  "tide-rode - Swung by the tide when at anchor; -- opposed to wind-rode.",
  "alleviator - One who, or that which, alleviaties.",
  "nagging - Fault-finding; teasing; persistently annoying; as, a nagging toothache. [Colloq.]",
  "baxter - A baker; originally, a female baker. [Old Eng. & Scotch]",
  "nitrocellulose - See Gun cotton, under Gun.",
  "persisting - Inclined to persist; tenacious of purpose; persistent. -- Per*sist\"ing*ly, adv.",
  "calid - Hot; burning; ardent. [Obs.] Bailey.",
  "anhungered - Ahungered; longing. [Archaic]",
  "splintery - Consisting of splinters; resembling splinters; as, the splintery fracture of a mineral.",
  "stacket - A stockade. [Scot.] Sir W. Scott.",
  "idolographical - Descriptive of idols. [R.] Southey.",
  "babe - 1. An infant; a young child of either sex; a baby. 2. A doll for children. Spenser.",
  "alcoate - Shortened forms of Alcoholate.",
  "histrionism - Theatrical representation; acting; affectation. Sir T. Browne.",
  "intrinsically - Internally; A lie is a thing absolutely and intrinsically evil. South.",
  "acetonic - Of or pertaining to acetone; as, acetonic bodies.",
  "hagiographer - One of the writers of the hagiographa; a writer of lives of the saints. Shipley.",
  "divellicate - To pull in pieces. [Obs. or R.]",
  "space key - A bar or key, in a typewriter or typesetting machine, used for spacing between letters.",
  "prismoidal - Having the form of a prismoid; as, prismoidal solids.",
  "lentiginous - Of or pertaining to lentigo; freckly; scurfy; furfuraceous.",
  "sloo - A slough; a run or wet place. See 2d Slough, 2.",
  "enfester - To fester. [Obs.] \"Enfestered sores.\" Davies (Holy Roode).",
  "windas - See 3d Windlass. [Obs.] Chaucer.",
  "bimembral - Having two members; as, a bimembral sentence. J. W. Gibbs.",
  "eanling - A lamb just brought forth; a yeanling. Shak.",
  "distrusting - That distrusts; suspicious; lacking confidence in. -- Dis*trust\"ing*ly, adv.",
  "flagman - One who makes signals with a flag.",
  "cleansable - Capable of being cleansed. Sherwood.",
  "epitrochlear - Relating to the epitrochlea.",
  "eurypteroid - Like, or pertaining to, the genus Euryperus.",
  "intensative - Adding intensity; intensifying.",
  "snow-broth - Snow and water mixed, or snow just melted; very cold liquor. Shak.",
  "vild - Vile. [Obs.] \"That vild race.\" Spenser. -- Vild\"ly, adv. [Obs.] Spenser.",
  "counterjumper - A salesman in a shop; a shopman; -- used contemtuously. [Slang]",
  "digamy - Act, or state, of being twice married; deuterogamy. [R.]",
  "copper works - A place where copper is wrought or manufactured. Woodward.",
  "modernity - Modernness; something modern. Walpole.",
  "tourist - One who makes a tour, or performs a journey in a circuit.",
  "glumelle - One of the pelets or inner chaffy scales of the flowers or spikelets of grasses.",
  "huswifery - The business of a housewife; female domestic economy and skill. Tusser.",
  "abuna - The Patriarch, or head of the Abyssinian Church.",
  "ampullated - Having an ampulla; flask-shaped; bellied.",
  "pyramidoid - A solid resembling a pyramid; -- called also pyramoid. Barlow.",
  "metastannate - A salt of metastannic acid.",
  "interfacial - Included between two plane surfaces or faces; as, an interfacial angle.",
  "unwroken - Not revenged; unavenged. [Obs.] Surrey.",
  "coheiress - A female heir who inherits with other heiresses; a joint heiress.",
  "domestication - The act of domesticating, or accustoming to home; the action of taming wild animals.",
  "generalness - The condition or quality of being general; frequency; commonness. Sir P. Sidney.",
  "proficuous - Profitable; advantageous; useful. [Obs.] Harvey.",
  "curstfully - Peevishly; vexatiously; detestably. [Obs.] \"Curstfully mad.\" Marston.",
  "benzoinated - Containing or impregnated with benzoin; as, benzoinated lard.",
  "complexness - The state of being complex; complexity. A. Smith.",
  "gneissose - Having the structure of gneiss.",
  "theatric - Theatrical. Woods over woods in gay, theatric pride. Goldsmith.",
  "actinophorous - Having straight projecting spines.",
  "edificatory - Tending to edification. Bp. Hall.",
  "impetiginous - Of the nature of, or pertaining to, impetigo.",
  "thermae - Springs or baths of warm or hot water.",
  "tourn - 1. A spinning wheel. [Prov. Eng.] 2. (O.Eng.Law)  The sheriff's turn, or court.",
  "arenation - A sand bath; application of hot sand to the body. Dunglison.",
  "comessation - A reveling; a rioting. [Obs.] Bp. Hall.",
  "unremembrance - Want of remembrance; forgetfulness. I. Watts.",
  "amoebea - That division of the Rhizopoda which includes the amoeba and similar forms.",
  "backwardness - The state of being backward.",
  "courtepy - A short coat of coarse cloth. [Obs.] Full threadbare was his overeste courtepy. Chaucer.",
  "wung-out - Having the sails set in the manner called wing-and-wing. [Sailors' slang]",
  "pseudostoma - A group of cells resembling a stoma, but without any true aperture among them.",
  "bunder - A boat or raft used in the East Indies in the landing of passengers and goods.",
  "theosophical - Of or pertaining to theosophy. -- The`o*soph\"ic*al*ly, adv.",
  "arnicin - An active principle of Arnica montana. It is a bitter resin.",
  "hortulan - Belonging to a garden. [Obs.] Evelyn.",
  "mulloid - Like or pertaining to the genus Mullus, which includes the surmullet, or red mullet.",
  "envolup - To wrap up; to envelop. [Obs.] Chaucer.",
  "silverling - A small silver coin. [Obs.] A thousand vines at a thousand silverings. Isa. vii. 23.",
  "pseudopodial - Of or pertaining to a pseudopod, or to pseudopodia. See Illust. of Heliozoa.",
  "jesuitry - Jesuitism; subtle argument. [R.] Carlyle.",
  "brusque - Rough and prompt in manner; blunt; abrupt; hluff; as, a brusque man; a brusque style.",
  "castillan - Of or pertaining to Castile, in Spain.",
  "tetrahexahedral - Pertaining to a tetrahexahedron.",
  "supersacral - Situated over, or on the dorsal side of, the sacrum.",
  "anna - An East Indian money of account, the sixteenth of a rupee, or about 2",
  "pilfering - Thieving in a small way. Shak. -- n.  Petty theft. -- Pil\"fer*ing*ly, adv.",
  "plain-hearted - Frank; sincere; artless. Milton. -- Plain\"-heart`ed*ness, n.",
  "helianthoid - Of or pertaining to the Helianthoidea.",
  "luskish - Inclined to be lazy. Marston. -- Lusk\"*ish*ly, adv. -Lusk\"ish*ness, n. [Obs.] Spenser.",
  "middle-aged - Being about the middle of the ordinary age of man; between 30 and 50 years old.",
  "areopagite - A member of the Areopagus. Acts xvii. 34.",
  "mislight - To deceive or lead astray with a false light. Herrick.",
  "teniafuge - A remedy to expel tapeworms.",
  "cyanosite - Native sulphate of copper. Cf. Blue vitriol, under Blue.",
  "monachism - The system and influences of a monastic life; monasticism.",
  "yttric - Pertaining to, derived from, or containing, yttrium.",
  "davyne - A variety of nephelite from Vesuvius.",
  "tuberosity - 1. The state of being tuberous. 2. An obtuse or knoblike prominence; a protuberance.",
  "emmove - To move; to rouse; to excite. [Obs.]",
  "retrait - A portrait; a likeness. [Obs.] Whose fair retrait I in my shield do bear. Spenser.",
  "holotricha - A group of ciliated Infusoria, having cilia all over the body.",
  "fumifugist - One who, or that which, drives away smoke or fumes.",
  "obduredness - Hardness. [Obs.] Bp. Hall.",
  "feverwort - See Fever root, under Fever.",
  "snithy - Sharp; piercing; cutting; -- applied to the wind. [Prov. Eng.]",
  "psittaci - The order of birds which comprises the parrots.",
  "puissantness - The state or quality of being puissant; puissance; power.",
  "seraphicism - The character, quality, or state of a seraph; seraphicalness. [R.] Cudworth.",
  "polymeniscous - Having numerous facets; -- said of the compound eyes of insects and crustaceans.",
  "ridgy - Having a ridge or ridges; rising in a ridge. \"Lifted on a ridgy wave.\" Pope.",
  "semele - A daughter of Cadmus, and by Zeus mother of Bacchus.",
  "difficilitate - To make difficult. [Obs.] W. Montagu.",
  "gymnoblastic - Of or pertaining to the Gymnoblastea.",
  "stronghold - A fastness; a fort or fortress; fortfield place; a place of security.",
  "chandoo - An extract or preparation of opium, used in China and India for smoking. Balfour.",
  "tetty - Testy; irritable. [Obs.] Burton.",
  "slamkin - A slut; a slatternly woman. [Obs. or Prov. Eng.]",
  "bedrench - To drench; to saturate with moisture; to soak. Shak.",
  "homomorphous - Characterized by homomorphism.",
  "lycopod - A plant of the genus Lycopodium.",
  "lengest - Longer; longest; -- obsolete compar. and superl. of long. Chaucer.",
  "hindermost - Furthest in or toward the rear; last. \"Rachel and Joseph hindermost.\" Gen. xxxiii. 2.",
  "crypturi - An order of flying, dromTinamou.",
  "osseous - Composed of bone; resembling bone; capable of forming bone; bony; ossific.",
  "cyanometer - An instrument for measuring degress of blueness.",
  "palissander - (a) Violet wood. (b) Rosewood.",
  "bishoplike - Resembling a bishop; belonging to a bishop. Fulke.",
  "intersesamoid - Between sesamoid bones; as, intersesamoid ligaments.",
  "sceptical - etc. See Skeptic, Skeptical, Skepticism, etc.",
  "annuary - Annual. [Obs.] -- n.  A yearbook.",
  "archpresbytery - The absolute dominion of presbytery. Milton.",
  "observership - The office or work of an observer.",
  "vilipendency - Disesteem; slight; disparagement. [R.] E. Waterhouse.",
  "its - Possessive form of the pronoun it. See It.",
  "transume - To change; to convert. [R.] Crashaw.",
  "defeudalize - To deprive of the feudal character or form.",
  "pyroarsenate - A salt of pyroarsenic acid.",
  "controllability - Capability of being controlled; controllableness.",
  "tum-tum - A dish made in the West Indies by beating boiled plantain quite soft in a wooden mortar.",
  "spiritousness - Quality of being spiritous. [R.]",
  "booked - 1. Registered. 2. On the way; destined. [Colloq.]",
  "entozoal - Pertaining to, or consisting of, the Entozoa.",
  "tradespeople - People engaged in trade; shopkeepers.",
  "otoscopeic - Of or pertaining to the otoscope or to otoscopy.",
  "paradised - Placed in paradise; enjoying delights as of paradise.",
  "ailuroidea - A group of the Carnivora, which includes the cats, civets, and hyenas.",
  "self-degradation - The act of degrading one's self, or the state of being so degraded.",
  "impecuniosity - The state of being impecunious. Thackeray. Sir W. Scott.",
  "schematic - Of or pertaining to a scheme or a schema.",
  "delices - Delicacies; delights. [Obs.] \"Dainty delices.\" Spenser.",
  "bod veal - Veal too immature to be suitable for food.",
  "pulsive - Tending to compel; compulsory. [R.] \"The pulsive strain of conscience.\" Marston.",
  "rejuvenescent - Becoming, or causing to become, rejuvenated; rejuvenating.",
  "creational - Of or pertaining to creation.",
  "rubywood - red sandalwood. See under Sandalwood.",
  "phenylic - Pertaining to, derived from, or containing, phenyl. Phenylic alcohol (Chem.), phenol.",
  "redented - Formed like the teeth of a saw; indented.",
  "demonologic - Of or Pertaining to demonology.",
  "overinfluence - To influence in an excessive degree; to have undue influence over.",
  "punction - A puncturing, or pricking; a puncture.",
  "arch stone - A wedge-shaped stone used in an arch; a voussoir.",
  "irenicon - A proposition or device for securing peace, especially in the church. South.",
  "tergant - Showing the back; as, the eagle tergant. [Written also tergiant.]",
  "claqueur - One of the claque employed to applaud at a theater.",
  "manginess - The condition or quality of being mangy.",
  "peerdom - Peerage; also, a lordship. [Obs.]",
  "adonean - Pertaining to Adonis; Adonic. \"Fair Adonean Venus.\" Faber.",
  "unadulterate - Not adulterated; pure. \"Unadulterate air.\" Cowper. -- Un`a*dul\"ter*ate*ly, adv.",
  "xylite - A liquid hydrocarbon found in crude wood spirits.",
  "pairer - One who impairs. [Obs.] Wyclif.",
  "arseniferous - Containing or producing arsenic.",
  "isographic - Of or pertaining to isography.",
  "drynurse - To feed, attend, and bring up without the breast. Hudibras.",
  "rhapsodomancy - Divination by means of verses.",
  "rebukeful - Containing rebuke; of the nature of rebuke. [Obs.] -- Re*buke\"ful*ly, adv. [Obs.]",
  "wamble-cropped - Sick at the stomach; also, crestfallen; dejected. [Slang]",
  "high-hoe - The European green woodpecker or yaffle. [Written also high- hoo.]",
  "pondering - Deliberating. -- Pon\"der*ing*ly, adv.",
  "extraregular - Not comprehended within a rule or rules. Jer. Taylor.",
  "deletitious - Of such a nature that anything may be erased from it; -- said of paper.",
  "mesiad - Toward, or on the side toward, the mesial plane; mesially; -- opposed to laterad.",
  "ruffin - Disordered. [Obs.] His ruffin rainment all was stained with blood. Spenser.",
  "suffraginous - Of or pertaining to the hock of a beast. [Obs.]",
  "polychronious - Enduring through a long time; chronic.",
  "chromolithographer - One who is engaged in chromolithography.",
  "pebbly - Full of pebbles; pebbled. \"A hard, pebbly bottom.\" Johnson.",
  "ragious - Raging; furious; rageful. [Obs.] -- Ra\"gious*ness, n. [Obs.]",
  "bawbee - A halfpenny. [Spelt also baubee.] [Scot. & Prov. Eng.]",
  "climatography - A description of climates.",
  "sea cow - (a) The mantee. (b) The dugong. (c) The walrus.",
  "soothness - Truth; reality. [Obs.] Chaucer.",
  "phlogistian - A believer in the existence of phlogiston.",
  "hydrarthrosis - An effusion of watery liquid into the cavity of a joint.",
  "blowzy - Coarse and ruddy-faced; fat and ruddy; high colored; frowzy.",
  "fidalgo - The lowest title of nobility in Portugal, corresponding to that of Hidalgo in Spain.",
  "hypercriticism - Excessive criticism, or unjust severity or rigor of criticism; zoilism.",
  "ket - Carrion; any filth. [Prob. Eng.] Halliwell.",
  "gastrostege - One of the large scales on the belly of a serpent.",
  "pequots - A tribe of Indians who formerly inhabited Eastern Connecticut. [Written also Pequods.]",
  "recadency - A falling back or descending a second time; a relapse. W. Montagu.",
  "sheldafle - A chaffinch. [Written also sheldapple, and shellapple.]",
  "excrete - To separate and throw off; to excrete urine. \"The mucus thus excreted.\" Hooper.",
  "therefor - For that, or this; for it. With certain officers ordained therefore. Chaucer.",
  "pendice - A sloping roof; a lean-to; a penthouse. [Obs.] Fairfax.",
  "aheight - Aloft; on high. [Obs.] \"Look up aheight.\" Shak.",
  "frislet - [Fraise a kind of defense; also Friz.) A kind of small ruffle. Halliwell.",
  "gynobasic - Pertaining to, or having, a gynobase.",
  "outwall - The exterior wall; the outside surface, or appearance. Shak.",
  "humiliant - Humiliating; humbling. \"Humiliant thoughts.\" [R.] Mrs. Browning.",
  "bounding - Moving with a bound or bounds. The bounding pulse, the languid limb. Montgomery.",
  "nidulation - The time of remaining in the nest. [R.] Sir T. Browne.",
  "pooping - The act or shock of striking a vessel's stern by a following wave or vessel.",
  "impossibly - Not possibly. Sir. T. North.",
  "dissolubleness - The quality of being dissoluble; dissolubility. Boyle.",
  "daboia - A large and highly venomous Asiatic viper (Daboia xanthica).",
  "estacade - A dike of piles in the sea, a river, etc., to check the approach of an enemy.",
  "fair-minded - Unprejudiced; just; judicial; honest. -- Fair\"*mind`ed*ness, n.",
  "solitariness - Condition of being solitary.",
  "unbegot - Not begot; not yet generated; also, having never been generated; self-existent; eternal.",
  "self-abnegation - Self-denial; self-renunciation; self-sacrifice.",
  "limpidness - Quality of being limpid; limpidity.",
  "suppleness - The quality or state of being supple; flexibility; pliableness; pliancy.",
  "invisibleness - The quality or state of being invisible; invisibility.",
  "pasilaly - A form of speech adapted to be used by all mankind; universal language.",
  "involuntarily - In an involuntary manner; not voluntarily; not intentionally or willingly.",
  "absorbedly - In a manner as if wholly engrossed or engaged.",
  "argulus - A genus of copepod Crustacea, parasitic of fishes; a fish louse. See Branchiura.",
  "praenares - The anterior nares. See Nares. B. G. Wilder.",
  "earlduck - The red-breasted merganser (Merganser serrator).",
  "subgeneric - Of or pertaining to a subgenus.",
  "presspack - To pack, or prepare for packing, by means of a press.",
  "herakline - A picrate compound, used as an explosive in blasting.",
  "half-fish - A salmon in its fifth year of growth. [Prov. Eng.]",
  "perineurial - Surrounding nerves or nerve fibers; of or pertaining to the perineurium.",
  "engendrure - The act of generation. [Obs.] Chaucer.",
  "ut - The first note in Guido's musical scale, now usually superseded by do. See Solmization.",
  "mainswear - To swear falsely. [Obs.] Blount.",
  "jerry-built - Built hastily and of bad materials; as, jerry-built houses. [Colloq. Eng.]",
  "pilferer - One who pilfers; a petty thief.",
  "plougher - One who plows; a plowman; a cultivator.",
  "proeguminal - Serving to predispose; predisposing; as, a proeguminal cause of disease.",
  "dichroscope - An instrument for examining the dichroism of crystals.",
  "beastliness - The state or quality of being beastly.",
  "extirp - To extirpate. [Obs.] It is impossible to extirp it quite, friar. Shak .",
  "convexly - In a convex form; as, a body convexly shaped.",
  "homogamy - The condition of being homogamous.",
  "sarcoblast - A minute yellowish body present in the interior of certain rhizopods.",
  "bezpopovtsy - A Russian sect. See Raskolnik.",
  "semitranslucent - Slightly clear; transmitting light in a slight degree.",
  "repassage - The act of repassing; passage back. Hakluyt.",
  "brevetcy - The rank or condition of a brevet officer.",
  "dispace - To roam. [Obs.] In this fair plot dispacing to and fro. Spenser.",
  "chastely - In a chaste manner; with purity.",
  "kiddow - The guillemot. [Written also kiddaw.] [Prov. Eng.]",
  "azymous - Unleavened; unfermented. \"Azymous bread.\" Dunglison.",
  "extradotal - Forming no part of the dowry; as, extradotal property.",
  "humoristic - Of, pertaining to, or resembling, a humorist.",
  "sorbet - A kind of beverage; sherbet. Smolett.",
  "trisagion - An ancient anthem, -- usually known by its Latin name tersanctus.See Tersanctus.",
  "recognitory - Pertaining to, or connected with, recognition.",
  "paideutics - The science or art of teaching.",
  "ectasia - A dilatation of a hollow organ or of a canal.",
  "metonymical - Used by way of metonymy. -- Met`o*nym\"ic*al*ly, adv.",
  "multivious - Having many ways or roads; by many ways. [Obs.]",
  "camisole - 1. A short dressing jacket for women. 2. A kind of straitjacket.",
  "nothingarian - One of no certain belief; one belonging to no particular sect.",
  "ozonoscopic - Serving to indicate the presence or the amount of ozone.",
  "outwrite - To exceed or excel in writing.",
  "concetto - Affected wit; a conceit. Chesterfield.",
  "oppositipetalous - Placed in front of a petal.",
  "suscitate - To rouse; to excite; to call into life and action. [Obs.]",
  "colloidal - Pertaining to, or of the nature of, colloids.",
  "baccated - 1. Having many berries. 2. Set or adorned with pearls. [Obs.]",
  "kinghood - The state of being a king; the attributes of a king; kingship. Gower.",
  "misaffection - An evil or wrong affection; the state of being ill affected. [Obs.] Bp. Hall.",
  "biliteralism - The property or state of being biliteral.",
  "attollent - Lifting up; raising; as, an attollent muscle. Derham.",
  "lodger - One who, or that which, lodges; one who occupies a hired room in another's house.",
  "indulger - One who indulges. W. Montagu.",
  "tithymal - Any kind of spurge, esp. Euphorbia Cyparissias.",
  "pendicle - An appendage; something dependent on another; an appurtenance; a pendant. Sir W. Scott.",
  "isostemony - The quality or state of being isostemonous.",
  "subapical - Being under the apex; of or pertaining to the part just below the apex.",
  "familiary - Of or pertaining to a family or household; domestic. [Obs.] Milton.",
  "wark - Work; a building. [Obs. or Scot.] Spenser.",
  "heteropodous - Of or pertaining to the Heteropoda.",
  "pastorage - The office, jurisdiction, or duty, of a pastor; pastorate.",
  "perennibranchiata - Those Batrachia which retain their gills through life, as the menobranchus.",
  "prodition - Disclosure; treachery; treason. [Obs.] Ainsworth.",
  "dull-eyed - Having eyes wanting brightness, liveliness, or vivacity. Shak.",
  "tullian - Belonging to, or in the style of, Tully (Marcus Tullius Cicero).",
  "congenially - In a congenial manner; as, congenially married or employed.",
  "idioted - Rendered idiotic; befooled. [R.] Tennyson.",
  "allophylic - Pertaining to a race or a language neither Aryan nor Semitic. J. Prichard.",
  "inseparately - Inseparably. [Obs.] Cranmer.",
  "laryngological - Of or pertaining to laryngology.",
  "intuitivism - The doctrine that the ideas of right and wrong are intuitive. J. Grote.",
  "anthropoidea - The suborder of primates which includes the monkeys, apes, and man.",
  "betrayer - One who, or that which, betrays.",
  "wost - 2d pers. sing. pres. of Wit, to know. [Obs.] Spenser.",
  "sketchily - In a sketchy or incomplete manner. \"Sketchily descriptive.\" Bartlett.",
  "overpraise - To praise excessively or unduly.",
  "disentwine - To free from being entwined or twisted. Shelley.",
  "grippleness - The quality of being gripple. [Obs.]",
  "poulaine - A long pointed shoe. See Cracowes.",
  "minuend - The number from which another number is to be subtracted.",
  "sorex - A genus of small Insectivora, including the common shrews.",
  "assurgent - Ascending; (Bot.)  rising obliquely; curving upward. Gray.",
  "practician - One who is acquainted with, or skilled in, anything by practice; a practitioner.",
  "deep-fet - Deeply fetched or drawn. [Obs.] \"Deep-fet groans.\" Shak.",
  "amenorrhoeal - Pertaining to amenorrhoea.",
  "sepaline - Relating to, or having the nature of, sepals.",
  "cloddish - Resembling clods; gross; low; stupid; boorish. Hawthorne. -- Clod\"dish*ness, n.",
  "tachymetry - The science or use of the tachymeter. -- Ta`chy*met\"ric (#), a.",
  "scrappily - In a scrappy manner; in scraps. Mary Cowden Clarke.",
  "custodial - Relating to custody or guardianship.",
  "potestate - A chief ruler; a potentate. [Obs.] Wyclif. \"An irous potestate.\" Chaucer.",
  "unclew - To unwind, unfold, or untie; hence, to undo; to ruin. Shak.",
  "philomel - Same as Philomela, the nightingale. [Poetic] Milton. Cowper.",
  "sempervirent - Always fresh; evergreen. [R.] Smart.",
  "vultern - The brush turkey (Talegallus Lathami) of Australia. See Brush turkey.",
  "leviration - Levirate marriage or marriages. Kitto.",
  "counterpassant - Passant in opposite directions; -- said of two animals.",
  "doorkeeper - One who guards the entrance of a house or apartment; a porter; a janitor.",
  "uncontinent - Not continent; incontinent. Wyclif (2 Tim. iii. 3).",
  "caulome - A stem structure or stem axis of a plant, viewed as a whole. -- Cau*lom\"ic (#), a.",
  "underworker - 1. One who underworks. 2. An inferior or subordinate workman. Waterland.",
  "fangot - A quantity of wares, as raw silk, etc., from one hundred weight.",
  "interreign - An interregnum. [Obs.] Bacon.",
  "forgettable - Liable to be, or that may be, forgotten. Carlyle.",
  "marriable - Marriageable. [R.] Coleridge.",
  "vertuous - Virtuous; powerful. [Obs.] Spenser.",
  "overlanguaged - Employing too many words; diffuse. Lowell.",
  "absorber - One who, or that which, absorbs.",
  "pyrroline - A nitrogenous base, C4H7N, obtained as a colorless liquid by the reduction of pyrrol.",
  "unbehovely - Not behooving or becoming; unseemly. [Obs. & R.] Gower.",
  "participator - One who participates, or shares with another; a partaker.",
  "self-convicted - Convicted by one's own consciousness, knowledge, avowal, or acts.",
  "endmost - Farthest; remotest; at the very end. Tylor.",
  "improvision - Improvidence. [Obs.] Sir T. Browne.",
  "hemadrometer - An instrument for measuring the velocity with which the blood moves in the arteries.",
  "allerion - Am eagle without beak or feet, with expanded wings. Burke.",
  "old-gentlemanly - Pertaining to an old gentleman, or like one. Byron.",
  "sermocination - The making of speeches or sermons; sermonizing. [Obs.] Peacham.",
  "megastome - One of a group of univalve shells, having a large aperture or mouth.",
  "pretenseless - Not having or making pretenses.",
  "servitorship - The office, rank, or condition of a servitor. Boswell.",
  "substractor - 1. One who subtracts. 2. A detractor; a slanderer. [Obs.] Shak.",
  "laborous - Laborious. [Obs.] Wyatt. -- La\"bor*ous*ly, adv. [Obs.] Sir T. Elyot.",
  "wenny - Having the nature of a wen; resembling a wen; as, a wennish excrescence.",
  "nigrescent - Growing black; changing to a black color; approaching to blackness. Johnson.",
  "spasmodical - Same as Spasmodic, a. -- Spas*mod\"ic*al*ly, adv.",
  "ravager - One who, or that which, ravages or lays waste; spoiler.",
  "acrobat - One who practices rope dancing, high vaulting, or other daring gymnastic feats.",
  "iatrochemical - Of or pertaining to iatrochemistry, or to the iatrochemists.",
  "frothiness - State or quality of being frothy.",
  "stairhead - The head or top of a staircase.",
  "threnetic - Pertaining to a threne; sorrowful; mournful.",
  "dilapidator - One who causes dilapidation. Strype.",
  "depriver - One who, or that which, deprives.",
  "biramous - Having, or consisting of, two branches.",
  "colonical - Of or pertaining to husbandmen. [Obs.]",
  "photogalvanography - The art or process of making photo-electrotypes. Sir D. Brewster.",
  "presbyteress - A female presbyter. Bale.",
  "dizz - To make dizzy; to astonish; to puzzle. [Obs.] Gayton.",
  "gramarye - Necromancy; magic. Sir W. Scott.",
  "lege - To allege; to assert. [Obs.] Bp. Fisher.",
  "subjicible - Capable of being subjected. [Obs.] Jer. Taylor.",
  "anadrom - A fish that leaves the sea and ascends rivers.",
  "disappreciate - To undervalue; not to esteem. -- Dis`ap*pre`ci*a\"tion, n.",
  "leno - A light open cotton fabric used for window curtains.",
  "wrynecked - Having a distorted neck; having the deformity called wryneck.",
  "meathe - A sweet liquor; mead. [Obs.] Chaucer. Milton.",
  "haurient - In pale, with the head in chief; -- said of the figure of a fish, as if rising for air.",
  "sylviculturist - One who cultivates forest trees, especially as a business.",
  "hastings - Early fruit or vegetables; especially, early pease. Mortimer.",
  "lesbianism - Unnatural sexual relations between women.",
  "ophiologist - One versed in the natural history of serpents.",
  "efflorescency - The state or quality of being efflorescent; efflorescence.",
  "nullibiety - The state or condition of being nowhere. [Obs.]",
  "interserttion - The act of interserting, or that which is interserted. [Obs.] Hammond.",
  "undispensed - 1. Not dispensed. 2. Not freed by dispensation. [R.] Tooker.",
  "vizierate - The office, dignity, or authority of a vizier.",
  "pilulous - Like a pill; small; insignificant. [R.] G. Eliot.",
  "acclamatory - Pertaining to, or expressing approval by, acclamation.",
  "tritheism - The opinion or doctrine that the Father, Son, and Holy Spirit are three distinct Gods.",
  "illegally - In a illegal manner; unlawfully.",
  "protrudable - That may be protruded; protrusile. Darwin.",
  "enchanting - Having a power of enchantment; charming; fascinating. -- En*chant\"ing*ly, adv.",
  "ellipsoidal - Pertaining to, or shaped like, an ellipsoid; as, ellipsoid or ellipsoidal form.",
  "judaistic - Of or pertaining to Judaism.",
  "muzarabic - Of or pertaining to Muzarabs; as, the Muzarabic liturgy. [Written also Mozarabic.]",
  "postnares - The posterior nares. See Nares.",
  "triphyllous - Having three leaves; three-leaved.",
  "abstrusity - Abstruseness; that which is abstruse. [R.] Sir T. Browne.",
  "empight - Fixed; settled; fastened. [Obs.] Spenser.",
  "unhallow - To profane; to desecrate. The vanity unhallows the virtue. L'Estrange.",
  "epithetic - Pertaining to, or abounding with, epithets. \"In epithetic measured prose.\" Lloyd.",
  "stage-struck - Fascinated by the stage; seized by a passionate desire to become an actor.",
  "cogger - A flatterer or deceiver; a sharper.",
  "oratress - A woman who makes public addresses. Warner.",
  "neckwear - A collective term for cravats, collars, etc. [Colloq. or trade name]",
  "engarrison - To garrison; to put in garrison, or to protect by a garrison. Bp. Hall.",
  "comate - Encompassed with a coma, or bushy appearance, like hair; hairy.",
  "outmarch - To surpass in marching; to march faster than, or so as to leave behind.",
  "parrhesia - Boldness or freedom of speech.",
  "bulbel - A separable bulb formed on some flowering plants.",
  "gourmet - A connoisseur in eating and drinking; an epicure.",
  "burkism - The practice of killing persons for the purpose of selling their bodies for dissection.",
  "arteriology - That part of anatomy which treats of arteries.",
  "teade - A torch. [Obs.] \"A burning teade.\" Spenser.",
  "viperish - Somewhat like a viper; viperous.",
  "astrofel - A bitter herb, probably the same as aster, or starwort. Spenser.",
  "baldwin - A kind of reddish, moderately acid, winter apple. [U.S.]",
  "atwo - In two; in twain; asunder. [Obs.] Chaucer.",
  "unhat - To take off the hat of; to remove one's hat, especially as a mark of respect. H. Spenser.",
  "bemangle - To mangle; to tear asunder. [R.] Beaumont.",
  "lawe - To cut off the claws and balls of, as of a dog's fore feet. Wright.",
  "langdak - A wolf (Canis pallipes), found in India, allied to the jackal.",
  "belcher - One who, or that which, belches.",
  "curr - To coo. [Scot.] The owlets hoot, the owlets curr. Wordsworth.",
  "oligomerous - Having few members in each set of organs; as, an oligomerous flower.",
  "swingel - The swinging part of a flail which falls on the grain in thrashing; the swiple.",
  "vestales - A group of butterflies including those known as virgins, or gossamer-winged butterflies.",
  "rimbase - A short cylinder connecting a trunnion with the body of a cannon. See Illust. of Cannon.",
  "fluky - Formed like, or having, a fluke.",
  "contemptibleness - The state or quality of being contemptible, or of being despised.",
  "podarthrum - The foot joint; in birds, the joint between the metatarsus and the toes.",
  "hockle - 1. To hamstring; to hock; to hough. Hanmer. 2. To mow, as stubble. Mason.",
  "ordinalism - The state or quality of being ordinal. [R.] Latham.",
  "stepson - A son of one's husband or wife by a former marriage.",
  "lecturer - One who lectures; an assistant preacher.",
  "stone-horse - Stallion. [Obs.] Mortimer.",
  "underchamberlain - A deputy chamberlain of the exchequer.",
  "cool-headed - Having a temper not easily excited; free from passion. -- Cool\"-head`ed*ness, n.",
  "reddish - Somewhat red; moderately red. -- Red\"dish*ness, n.",
  "unanimity - The quality or state of being unanimous.",
  "filatory - A machine for forming threads. [Obs.] W. Tooke.",
  "disprize - To do preciate. [R.] Cotton (Ode to Lydia).",
  "desulphurize - To desulphurate; to deprive of sulphur. -- De*sul`phur*i*za\"tion, n.",
  "warray - To make war upon. [Obs.] Fairfax. \"When a man warrayeth truth.\" Chaucer.",
  "travel-tainted - Harassed; fatigued with travel. [Obs.] Shak.",
  "mesomyodous - Having the intrinsic muscles of the larynx attached to the middle of the semirings.",
  "electrepeter - An instrument used to change the direction of electric currents; a commutator. [R.]",
  "assimilability - The quality of being assimilable. [R.] Coleridge.",
  "steenkirk - A kind of neckcloth worn in a loose and disorderly fashion.",
  "lusorious - Used in play; sportive; playful. [Obs.] Bp. Sanderson.",
  "defamer - One who defames; a slanderer; a detractor; a calumniator.",
  "kowtow - The same as Kotow. I have salaamed and kowtowed to him. H. James.",
  "sheep-shearing - 1. Act of shearing sheep. 2. A feast at the time of sheep-shearing. Shak.",
  "rearmouse - The leather-winged bat (Vespertilio murinus). [Written also reermouse.]",
  "quadrupedal - Having four feet; of or pertaining to a quadruped.",
  "syrma - A long dress, trailing on the floor, worn by tragic actors in Greek and Roman theaters.",
  "enavigate - To sail away or over. [Obs.] Cockeram.",
  "youthhood - The quality or state of being a youth; the period of youth. Cheyne.",
  "tue-irons - A pair of blacksmith's tongs.",
  "alcanna - An oriental shrub (Lawsonia inermis) from which henna is obtained.",
  "abnegate - To deny and reject; to abjure. Sir E. Sandys. Farrar.",
  "pouch-shell - A small British and American pond snail (Bulinus hypnorum).",
  "inventress - A woman who invents. Dryden.",
  "palatableness - The quality or state of being agreeable to the taste; relish; acceptableness.",
  "iron-cased - Cased or covered with iron, as a vessel; ironclad.",
  "interclose - To shut in; to inclose. [Obs.]",
  "forgeman - A skilled smith, who has a hammerer to assist him.",
  "sclerenchymatous - Pertaining to, or composed of, sclerenchyma.",
  "meetness - Fitness; suitableness; propriety.",
  "amusette - A light field cannon, or stocked gun mounted on a swivel.",
  "antambulacral - Away from the ambulacral region.",
  "cheater - 1. One who cheats. 2. An escheator. [R.] Shak.",
  "avouchment - The act of avouching; positive declaration. [Obs.] Milton.",
  "settlement - 1. The act of setting, or the state of being settled. Specifically: -",
  "ritually - By rites, or by a particular rite.",
  "compos-mentis - One who is compos mentis. [Colloq.]",
  "fosterer - One who, or that which, fosters.",
  "washerman - A man who washes clothes, esp. for hire, or for others.",
  "repkie - Any edible sea urchin. [Alaska]",
  "monomachy - A duel; single combat. \"The duello or monomachia.\" Sir W. Scott.",
  "enterpriser - One who undertakes enterprises. Sir J. Hayward.",
  "augurial - Relating to augurs or to augury. Sir T. Browne.",
  "wit-starved - Barren of wit; destitute of genius. Examiner.",
  "postglenoid - Situated behind the glenoid fossa of the temporal bone.",
  "niceness - Quality or state of being nice.",
  "hoggerel - A sheep of the second year. [Written also hogrel.] Ash.",
  "ligger - 1. A baited line attached to a float, for night fishing. See Leger, a. 2. See Ledger, 2.",
  "admittedly - Received as true or valid; acknowledged. -- Ad*mit\"ted*ly adv.  Confessedly.",
  "corporify - To embody; to form into a body. [Obs.] Boyle.",
  "mitigable - Admitting of mitigation; that may be mitigated.",
  "beknow - To confess; to acknowledge. [Obs.] Chaucer.",
  "key-cold - Cold as a metallic key; lifeless. [Formerly, a proverbial expression.] Shak. Milton.",
  "comprobate - To agree; to concur. [Obs.] Sir T. Elyot.",
  "cream-slice - A wooden knife with a long thin blade, used in handling cream or ice cream.",
  "decapitation - The act of beheading; beheading.",
  "graptolitic - Of or pertaining to graptolites; containing graptolites; as, a graptolitic slate.",
  "subobscurely - Somewhat obscurely or darkly. [R.] Donne.",
  "overpester - To pester exceedingly or excessively. Sir W. Raleigh.",
  "ash-oven - A furnace or oven for fritting materials for glass making.",
  "heart-wounded - Wounded to the heart with love or grief. Pope.",
  "outjest - To surpass in jesting; to drive out, or away, by jesting. [R.] Shak.",
  "digue - A bank; a dike. [Obs.] Sir W. Temple.",
  "gaduin - A yellow or brown amorphous substance, of indifferent nature, found in cod-liver oil.",
  "nonacquiescence - Refusal of acquiescence; failure to yield or comply.",
  "old-womanish - Like an old woman; anile. -- Old`-wom\"an*ish*ness, n.",
  "torchlight - The light of a torch, or of torches. Also adjectively; as, a torchlight procession.",
  "weatherproof - Proof against rough weather.",
  "disorb - To throw out of the proper orbit; to unsphere. Shak.",
  "revirescence - A growing green or fresh again; renewal of youth or vigor. [Obs.]",
  "picle - A small piece of land inclosed with a hedge; a close. [Obs.] [Written also pickle.]",
  "acock - In a cocked or turned up fashion.",
  "heretification - The act of hereticating or pronouncing heretical. London Times.",
  "elementarity - Elementariness. [Obs.] Sir T. Browne.",
  "debilitant - Diminishing the energy of organs; reducing excitement; as, a debilitant drug.",
  "melic - Of or pertaining to song; lyric; tuneful.",
  "trustworthy - Worthy of trust or confidence; trusty. -- Trust\"wor`thi*ness, n.",
  "meazling - Falling in small drops; mistling; mizzing. [Obs.] Arbuthnot.",
  "labyrinthical - Like or pertaining to a labyrinth.",
  "tendresse - Tender feeling; fondness. [Obs., except as a French word]",
  "ecossaise - A dancing tune in the Scotch style.",
  "gabber - 1. A liar; a deceiver. [Obs.] 2. One addicted to idle talk.",
  "sheelfa - The chaffinch; -- so named from its call note. [Prov. Eng.]",
  "prompture - Suggestion; incitement; prompting. [R.] Shak. Coleridge.",
  "neocarida - The modern, or true, Crustacea, as distinguished from the Merostomata.",
  "marbly - Containing, or resembling, marble.",
  "consulship - 1. The office of a consul; consulate. 2. The term of office of a consul.",
  "enthymematical - Pertaining to, or of the form of, an enthymeme.",
  "absorbable - Capable of being absorbed or swallowed up. Kerr.",
  "concubinal - Of or pertaining to concubinage.",
  "interchapter - An intervening or inserted chapter.",
  "chartist - A supporter or partisan of chartism. [Eng.]",
  "unscapable - Not be escaped; inevitable. [Obs.] Wyclif.",
  "sightfulness - The state of being sightful; perspicuity. [Obs.] Sir P. Sidney.",
  "top-heavy - Having the top or upper part too heavy for the lower part. Sir H. Wotton.",
  "spleened - 1. Deprived of the spleen. 2. Angered; annoyed. [Obs.] R. North.",
  "trichroic - Exhibiting trichroism; pleochroic; pleochroism.",
  "pyroxenite - A rock consisting essentially of pyroxene.",
  "pantry - An apartment or closet in which bread and other provisions are kept.",
  "grammaticism - A point or principle of grammar. Abp. Leighton.",
  "popularization - The act of making popular, or of introducing among the people.",
  "emmarble - To turn to marble; to harden. [Obs.] Thou dost emmarble the proud heart. Spenser.",
  "ferroprussiate - A ferrocyanate; a ferocyanide. [R.]",
  "lithoid - Like a stone; having a stony structure.",
  "ornithichnology - The branch of science which treats of ornithichnites. Hitchcock.",
  "toxical - Of or pertaining to poison; poisonous; as, toxic medicines.",
  "crispature - The state of being crispate.",
  "herdess - A shepherdess; a female herder. Sir P. Sidney. Chaucer.",
  "unfence - To strip of a fence; to remove a fence from.",
  "exasperater - One who exasperates or inflames anger, enmity, or violence.",
  "diabaterial - Passing over the borders. [R.] Mitford.",
  "tricorporate - Represented with three bodies conjoined to one head, as a lion.",
  "unrestraint - Freedom from restraint; freedom; liberty; license.",
  "kutauss - The India civet (Viverra zibetha).",
  "belord - 1. To act the lord over. 2. To address by the title of \"lord\".",
  "pennated - 1. Winged; plume-shaped. 2. (Bot.)  Same as Pinnate.",
  "leniency - The quality or state of being lenient; lenity; clemency.",
  "branular - Relating to the brain; cerebral. I. Taylor.",
  "overconfidence - Excessive confidence; too great reliance or trust.",
  "dupion - A double cocoon, made by two silkworms.",
  "colonial - Of or pertaining to a colony; as, colonial rights, traffic, wars.",
  "orval - A kind of sage (Salvia Horminum).",
  "telescopist - One who uses a telescope. R. A. Proctor.",
  "infuscated - Darkened with a blackish tinge.",
  "mealtime - The usual time of eating a meal.",
  "weighbeam - A kind of large steelyard for weighing merchandise; -- also called weighmaster's beam.",
  "unhoard - To take or steal from a hoard; to pilfer. Milton.",
  "hastif - Hasty. [Obs.] Chaucer. -- Has\"tif*ly, adv. [Obs.]",
  "withdrawer - One who withdraws; one who takes back, or retracts.",
  "seraskierate - The office or authority of a seraskier.",
  "immortalization - The act of immortalizing, or state of being immortalized.",
  "exfoliation - The scaling off of a bone, a rock, or a mineral, etc.; the state of being exfoliated.",
  "tenerity - Tenderness. [Obs.] Ainsworth.",
  "somniloquism - The act or habit of talking in one's sleep; somniloquy. Coleridge.",
  "fairylike - Resembling a fairy, or what is made or done be fairies; as, fairylike music.",
  "dacoit - One of a class of robbers, in India, who act in gangs.",
  "outfool - To exceed in folly. [R.] Young.",
  "picayunish - Petty; paltry; mean; as, a picayunish business. [Colloq. U.S.]",
  "gratified - Pleased; indulged according to desire. Syn. -- Glad; pleased. See Glad.",
  "nervy - Strong; sinewy. \"His nervy knees.\" Keats.",
  "tenuifolious - Having thin or narrow leaves.",
  "hallucal - Of or pertaining to the hallux.",
  "evolvent - The involute of a curve. See Involute, and Evolute.",
  "expounder - One who expounds or explains; an interpreter.",
  "sorrowed - Accompanied with sorrow; sorrowful. [Obs.] Shak.",
  "waviness - The quality or state of being wavy.",
  "rebukable - Worthy of rebuke or reprehension; reprehensible. Shak.",
  "infratemporal - Below the temple; below the temporal bone.",
  "previse - 1. To foresee. [R.] 2. To inform beforehand; to warn. Ld. Lytton.",
  "retinoscopy - The study of the retina of the eye by means of the ophthalmoscope.",
  "caprine - Of or pertaining to a goat; as, caprine gambols.",
  "particularment - A particular; a detail. [Obs.]",
  "three-decker - A vessel of war carrying guns on three decks.",
  "sault - A rapid in some rivers; as, the Sault Ste. Marie. [U.S.] Bartlett.",
  "topman - 1. See Topsman, 2. 2. (Naut.)  A man stationed in the top.",
  "enrapture - To transport with pleasure; to delight beyond measure; to enravish. Shenstone.",
  "black-eyed susan - (a) The coneflower, or yellow daisy (Rudbeckia hirta). (b) The bladder ketmie.",
  "scurfiness - 1. Quality or state of being scurfy. 2. (Bot.)  Scurf.",
  "wontedness - The quality or state of being accustomed. [R.] Eikon Basilike.",
  "incompatibility - The quality or state of being incompatible; inconsistency; irreconcilableness.",
  "laundryman - A man who follows the business of laundering.",
  "vermiformia - A tribe of worms including Phoronis. See Phoronis.",
  "subtenant - One who rents a tenement, or land, etc., of one who is also a tenant; an undertenant.",
  "debullition - A bubbling or boiling over. [Obs.] Bailey.",
  "sanctuarize - To shelter by means of a sanctuary or sacred privileges. [Obs.] Shak.",
  "thornless - Destitute of, or free from, thorns.",
  "desinence - Termination; ending. Bp. Hall.",
  "squirarch - One who belongs to the squirarchy. -- Squir\"arch*al, a.",
  "operetta - A short, light, musical drama.",
  "raguled - Notched in regular diagonal breaks; -- said of a line, or a bearing having such an edge.",
  "rapidness - Quality of being rapid; rapidity.",
  "disincorporation - Deprivation of the rights and privileges of a corporation. T. Warton.",
  "shandygaff - A mixture of strong beer and ginger beer. [Eng.]",
  "aliner - One who adjusts things to a line or lines or brings them into line. Evelyn.",
  "heartrending - Causing intense grief; overpowering with anguish; very distressing.",
  "karob - The twenty-fourth part of a grain; -- a weight used by goldsmiths. Crabb.",
  "thrombin - The fibrin ferment which produces the formation of fibrin from fibrinogen.",
  "subjugation - The act of subjugating, or the state of being subjugated.",
  "lithoglypher - One who curs or engraves precious stones.",
  "modificate - To qualify. [Obs.] Bp. Pearson.",
  "indissolubly - In an indissoluble manner. On they move, indissolubly firm. Milton.",
  "alienability - Capability of being alienated. \"The alienability of the domain.\" Burke.",
  "sutured - Having a suture or sutures; knit or united together. Pennant.",
  "whewellite - Calcium oxalate, occurring in colorless or white monoclinic crystals.",
  "provincialize - To render provincial. M. Arnold.",
  "asexualization - The act or process of sterilizing an animal or human being, as by vasectomy.",
  "bolsa - An exchange for the transaction of business. [Sp. Amer. & Phil. Islands]",
  "roughdraw - To draw or delineate rapidly and by way of a first sketch.",
  "syphilology - That branch of medicine which treats of syphilis.",
  "unbegun - Not yet begun; also, existing without a beginning.",
  "grecianize - To conform to the Greek custom, especially in speech.",
  "targe - A shield or target. [Obs. or Poetic] \"A buckler on a targe.\" Chaucer.",
  "myropolist - One who sells unguents or perfumery. [Obs.] Jonhson.",
  "day-coal - The upper stratum of coal, as nearest the light or surface.",
  "operculiform - Having the form of a lid or cover.",
  "prudhomme - A trustworthy citizen; a skilled workman. See Citation under 3d Commune, 1.",
  "pleonastic - Of or pertaining to pleonasm; of the nature of pleonasm; redundant.",
  "gratiolin - One of the essential principles of the hedge hyssop (Gratiola officinalis).",
  "au fond - At bottom; fundamentally; essentially.",
  "plethoretic - Plethoric. [Obs.] Johnson.",
  "assober - To make or keep sober. [Obs.] Gower.",
  "famosity - The state or quality of being famous. [Obs.] Johnson.",
  "swelldom - People of rank and fashion; the class of swells, collectively. [Jocose]",
  "hognut - (a) The pignut. See Hickory. (b) In England, the Bunium flexuosum, a tuberous plant.",
  "demorage - Demurrage. [Obs.] Pepys (1663).",
  "principiant - Relating to principles or beginnings. [R.] Jer. Taylor.",
  "virescence - The act or state of becoming green through the formation of chlorophyll.",
  "penologist - One versed in, or a student of, penology.",
  "sexdigitist - One who has six fingers on a hand, or six toes on a foot.",
  "pectineal - (a) Of or pertaining to the pecten. (b) Relating to, or connected with, the pubic bone.",
  "depuratory - Depurating; tending to depurate or cleanse; depurative.",
  "nauseation - The act of nauseating, or the state of being nauseated.",
  "downtrod - Trodden down; trampled down; abused by superior power. Shak.",
  "fluvial - Belonging to rivers; growing or living in streams or ponds; as, a fluvial plant.",
  "rockery - A mound formed of fragments of rock, earth, etc., and set with plants.",
  "controversion - Act of controverting; controversy. [Obs.] Hooker.",
  "re-creative - Creating anew; as, re-creative power.",
  "zooecytium - The common support, often branched, of certain species of social Infusoria.",
  "unline - To take the lining out of; hence, to empty; as, to unline one's purse.",
  "peterwort - See Saint Peter's-wort, under Saint.",
  "autobiographist - One who writes his own life; an autobiographer. [R.]",
  "deploration - The act of deploring or lamenting; lamentation. Speed.",
  "vocalism - 1. The exercise of the vocal organs; vocalization. 2. A vocalic sound. [R.]",
  "ditty-box - A small box to hold a sailor's thread, needless, comb, etc.",
  "overbulk - To oppress by bulk; to overtower. [Obs. & R.] Shak.",
  "mercat - Market; trade. [Obs.] Bp. Sprat.",
  "promisor - One who engages or undertakes; a promiser. Burrill.",
  "suggester - One who suggests. Beau. & Fl.",
  "faced - Having (such) a face, or (so many) faces; as, smooth-faced, two-faced.",
  "scorny - Deserving scorn; paltry. [Obs.]",
  "surgeless - Free from surges; smooth; calm.",
  "xylamide - An acid amide derivative of xylic acid, obtained as a white crystalline substance.",
  "springtide - The time of spring; springtime. Thomson.",
  "vermicious - Of or pertaining to worms; wormy.",
  "inarching - A method of ingrafting. See Inarch.",
  "cottolene - A product from cottonseed, used as lard.",
  "neglectedness - The state of being neglected.",
  "preterlapsed - Past; as, preterlapsed ages. [R.] Glanvill.",
  "pocky - Full of pocks; affected with smallpox or other eruptive disease. Bp. Hall.",
  "atheological - Opposed to theology; atheistic. Bp. Montagu.",
  "tomelet - All small tome, or volume. [R.]",
  "tectly - Covertly; privately; secretly. [Obs.] Holinshed.",
  "ophthalmite - An eyestalk; the organ which bears the compound eyes of decapod Crustacea.",
  "pylagore - a deputy of a State at the Amphictyonic council.",
  "ampelopsis - A genus formerly including the Virginia creeper.",
  "paginal - Consisting of pages. \"Paginal books.\" Sir T. Browne.",
  "lumbaginous - Of or pertaining to lumbago.",
  "ebb tide - The reflux of tide water; the retiring tide; -- opposed to flood tide.",
  "bilgy - Having the smell of bilge water.",
  "cushite - A descendant of Cush, the son of Ham and grandson of Noah.",
  "sieur - Sir; -- a title of respect used by the French.",
  "whey cure - Treatment with whey as a drink and in baths.",
  "quinquedentated - Five-toothed; as, a quinquedentate leaf.",
  "beholden - Obliged; bound in gratitude; indebted. But being so beholden to the Prince. Tennyson.",
  "egghot - A kind of posset made of eggs, brandy, sugar, and ale. Lamb.",
  "febrifuge - A medicine serving to mitigate or remove fever. -- a.  Antifebrile.",
  "chamfret - 1. (Carp.)  A small gutter; a furrow; a groove. 2. A chamfer.",
  "matabeles - A warlike South African Kaffir tribe.",
  "annexment - The act of annexing, or the thing annexed; appendage. [R.] Shak.",
  "decemdentate - Having ten points or teeth.",
  "resorter - One who resorts; a frequenter.",
  "clote - The common burdock; the clotbur. [Obs.] Wyclif.",
  "metapectic - Pertaining to, or designating, a supposed acid obtained from pectin.",
  "otosteal - An auditory ossicle. R. Owen.",
  "congruence - Suitableness of one thing to another; agreement; consistency. Holland.",
  "falcula - A curved and sharp-pointed claw.",
  "slow-witted - Dull of apprehension; not possessing quick intelligence.",
  "brioche - 1. A light cake made with flour, butter, yeast, and eggs. 2.  A knitted foot cushion.",
  "autonomic - Having the power of self-government; autonomous. Hickok.",
  "miscognize - To fail to apprehend; to misunderstand. [Obs.] Holland.",
  "squeasy - Queasy; nice; squeamish; fastidious; scrupulous. [Obs.] Bp. Earle.",
  "deflected - 1. Turned aside; deviating from a direct line or course. 2. Bent downward; deflexed.",
  "poyou - A South American armadillo (Dasypus sexcinctus). Called also sixbanded armadillo.",
  "opination - The act of thinking; a supposition. [Obs.]",
  "argonautic - Of or pertaining to the Argonauts.",
  "lametta - Foil or wire made of gold, silver, or brass. De Colange.",
  "torta - a flat heap of moist, crushed silver ore, prepared for the patio process.",
  "advowee - One who has an advowson. Cowell.",
  "sura - One of the sections or chapters of the Koran, which are one hundred and fourteen in number.",
  "colloquist - A speaker in a colloquy or dialogue. Malone.",
  "investigative - Given to investigation; inquisitive; curious; searching.",
  "roky - Misty; foggy; cloudy. [Prov. Eng.] Ray.",
  "besmirch - To smirch or soil; to discolor; to obscure. Hence: To dishonor; to sully. Shak.",
  "orn - To ornament; to adorn. [Obs.] Joye.",
  "jasey - A wig; -- so called, perhaps, from being made of, or resembling, Jersey yarn. Thackeray.",
  "unibranchiate - Having but one gill, as certain molluscs.",
  "bosomy - Characterized by recesses or sheltered hollows.",
  "cardol - A yellow oil liquid, extracted from the shell of the cashew nut.",
  "tercellene - A small male hawk. [R.] Sir T. Browne.",
  "cedule - A scroll; a writing; a schedule. [Obs.]",
  "photologic - Pertaining to photology, or the doctrine of light.",
  "bonne bouche - A delicious morsel or mouthful; a tidbit.",
  "arshine - A Russian measure of length = 2 ft. 4.246 inches.",
  "praescapula - Same as Preoral, Prepubis, Prescapula, etc.",
  "clubfisted - Having a large fist. Howell.",
  "lactide - A white, crystalline substance, obtained from also, by extension, any similar substance.",
  "latian - Belonging, or relating, to Latium, a country of ancient Italy. See Latin.",
  "frigorifical - Causing cold; producing or generating cold. Quincy.",
  "gentianella - A kind of blue color. Johnson.",
  "linener - A dealer in linen; a linen draper. [Obs.]",
  "carte - 1. Bill of fare. 2. Short for Carte de visite.",
  "anginous - Pertaining to angina or angina pectoris.",
  "triplex - Havingthree principal operative parts or motions, so as to produce a three-fold effect.",
  "penumbrala - Of or pertaining to a penumbra; resembling a penumbra; partially illuminated.",
  "depurator - One who, or that which, cleanses.",
  "malingery - The spirit or practices of a malingerer; malingering.",
  "diptote - A noun which has only two cases. Andrews.",
  "demersed - Situated or growing under water, as leaves; submersed.",
  "coucal - A large, Old World, ground cuckoo of the genus Centropus, of several species.",
  "stringless - Having no strings. His tongue is now a stringless instrument. Shak.",
  "curmudgeonly - Like a curmudgeon; niggardly; churlish; as, a curmudgeonly fellow.",
  "evitation - A shunning; avoidance. [Obs.] Bacon.",
  "circumscribable - Capable of being circumscribed.",
  "pedomotive - Moved or worked by the action of the foot or feet on a pedal or treadle.",
  "apyretic - Without fever; -- applied to days when there is an intermission of fever. Dunglison.",
  "kyaboca wood - (a) Amboyna wood. (b) Sandalwood (Santalum album).",
  "comtism - Positivism; the positive philosophy. See Positivism.",
  "collingual - Having, or pertaining to, the same language.",
  "pearl-eyed - Having a pearly speck in the eye; afflicted with the cataract.",
  "precrural - Situated in front of the leg or thigh; as, the precrural glands of the horse.",
  "cubation - The act of lying down; a reclining. [Obs.]",
  "praseolite - A variety of altered iolite of a green color and greasy luster.",
  "circuition - The act of going round; circumlocution. [R.]",
  "panidiomorphic - Having a completely idiomorphic structure; -- said of certain rocks.",
  "preambulate - To walk before. [R.] Jordan.",
  "acrolithic - Pertaining to, or like, an acrolith.",
  "benempt - of Bename. 1. Promised; vowed. [Obs.] Spenser. 2. Named; styled. [Archaic] Sir W. Scott.",
  "intrepidly - In an intrepid manner; courageously; resolutely.",
  "reparability - The quality or state of being reparable.",
  "intervital - Between two lives. [R.] Through all its [the spirit's] intervital gloom. Tennyson.",
  "deglutitory - Serving for, or aiding in, deglutition.",
  "ravisher - One who ravishes (in any sense).",
  "unpromise - To revoke or annul, as a promise. Chapman.",
  "isospore - (a) One of the spores produced by an isosporous organism. (b) A zygospore.",
  "insipient - Wanting wisdom; stupid; foolish. [R.] Clarendon. -- n.  An insipient person. [R.] Fryth.",
  "overdress - To dress or adorn to excess; to dress too much. Pope.",
  "retractation - The act of retracting what has been said; recantation.",
  "antiquarianism - Character of an antiquary; study or love of antiquities. Warburton.",
  "registrarship - The office of a registrar.",
  "carder - One who, or that which cards wool flax, etc. Shak.",
  "miswander - To wander in a wrong path; to stray; to go astray. [Obs.] Chaucer.",
  "krems - A variety of white lead. See Krems lead, under Lead, n.",
  "barbate - Bearded; beset with long and weak hairs.",
  "half-cracked - Half-demented; half-witted. [Colloq.]",
  "derworth - Precious. [Obs.] Piers Plowman.",
  "caseation - A degeneration of animal tissue into a cheesy or curdy mass.",
  "volplane - To glide in a flying machine.",
  "antipyresis - The condition or state of being free from fever.",
  "outpart - An outlying part. [R.] Ayliffe.",
  "brainish - Hot-headed; furious. [R.] Shak.",
  "feather-headed - Giddy; frivolous; foolish. [Colloq.] G. Eliot.",
  "laugher - 1. One who laughs. 2. A variety of the domestic pigeon.",
  "spermoderm - The covering of a seed; -- sometimes limited to the outer coat or testa. Lindley.",
  "annalist - A writer of annals. The monks . . . were the only annalists in those ages. Hume.",
  "complier - One who complies, yields, or obeys; one of an easy, yieldy temper. Swift.",
  "targumist - The writer of a Targum; one versed in the Targums.",
  "retainment - The act of retaining; retention. Dr. H. More.",
  "technician - A technicist; esp., one skilled particularly in the technical details of his work.",
  "ovariotomist - One who performs, or is skilled in, ovariotomy.",
  "delitigate - To chide; to rail heartily. [Obs.]",
  "besprinkling - The act of sprinkling anything; a sprinkling over.",
  "rushy - 1. Abounding with rushes. 2. Made of rushes. Me rushy couch and frugal fare. Goldsmith.",
  "hummocking - The process of forming hummocks in the collision of Arctic ice. Kane.",
  "insuavity - Want of suavity; unpleasantness. [Obs.] Burton.",
  "suncup - A yellow flowered evening primrose (Taraxia, syn. Onothera, ovata) native of California.",
  "bespice - To season with spice, or with some spicy drug. Shak.",
  "indefectibility - The quality of being indefectible. Barrow.",
  "macrospore - One of the specially large spores of certain flowerless plants, as Selaginella, etc.",
  "crenation - 1. (Bot.)  A rounded tooth on the edge of a leaf. 2. The condition of being crenate.",
  "adornment - An adorning; an ornament; a decoration.",
  "short-breathed - 1. Having short-breath, or quick respiration. 2. Having short life.",
  "als - 1. Also. [Obs.] Chaucer. 2. As. [Obs.] Chaucer.",
  "unmoved - Not moved; fixed; firm; unshaken; calm; apathetic. -- Un*mov\"ed*ly, adv.",
  "herpetotomist - One who dissects, or studies the anatomy of, reptiles.",
  "box tail - In a flying machine, a tail or rudder, usually fixed, resembling a box kite.",
  "zymologist - One who is skilled in zymology, or in the fermentation of liquors.",
  "lachrymiform - Having the form of a tear; tear-shaped.",
  "orchidologist - One versed in orchidology.",
  "immaturely - In an immature manner. Warburion.",
  "photoglyphy - Photoglyphic engraving. See under Photoglyphic.",
  "bethrall - To reduce to thralldom; to inthrall. [Obs.] Spenser.",
  "superponderate - To wiegh over and above. [Obs.]",
  "agnize - To recognize; to acknowledge. [Archaic] I do agnize a natural and prompt alacrity. Shak.",
  "tucuma - A Brazilian palm (Astrocaryum Tucuma) which furnishes an edible fruit.",
  "gog - Haste; ardent desire to go. [Obs.] Beau. & Fl.",
  "bated - Reduced; lowered; restrained; as, to speak with bated breath. Macaulay.",
  "enfeeblement - The act of weakening; enervation; weakness.",
  "exenteration - Act of exenterating. [R.]",
  "centrifugence - The property or quality of being centrifugal. R. W. Emerson.",
  "namer - One who names, or calls by name.",
  "bismuthous - Of, or containing, bismuth, when this element has its lower valence.",
  "bituminize - To prepare, treat, impregnate, or coat with bitumen.",
  "etiolated - Having a blanched or faded appearance, as birds inhabiting desert regions.",
  "hypochondres - The hypochondriac regions. See Hypochondrium.",
  "lethean - Of or pertaining to Lethe; resembling in effect the water of Lethe. Milton. Barrow.",
  "intinctivity - The want of the quality of coloring or tingeing other bodies. Kirwan.",
  "temporariness - The quality or state of being temporary; -- opposed to perpetuity.",
  "jurisprudential - Of or pertaining to jurisprudence. Stewart.",
  "corrigent - A substance added to a medicine to mollify or modify its action. Dunglison.",
  "davy lamp - See Safety lamp, under Lamp.",
  "graf - A German title of nobility, equivalent to earl in English, or count in French. See Earl.",
  "educability - Capability of being educated.",
  "burgess-ship - The state of privilege of a burgess. South.",
  "inconcluding - Inferring no consequence. [Obs.]",
  "disally - To part, as an alliance; to sunder. [R.] \"Disallied their nuptials.\" Milton.",
  "questant - One who undertakes a quest; a seeker. [Obs.] Shak.",
  "polder - A tract of low land reclaimed from the sea by of high embankments. [Holland & Belgium]",
  "lazarlike - Full of sores; leprous. Shak. Bp. Hall.",
  "dynastic - Of or relating to a dynasty or line of kings. Motley.",
  "detrimentalness - The quality of being detrimental; injuriousness.",
  "closereefed - Having all the reefs taken in; -- said of a sail.",
  "papyrine - Imitation parchment, made by soaking unsized paper in dilute sulphuric acid.",
  "purgatorial - Of or pertaining to purgatory; expiatory.",
  "asystole - A weakening or cessation of the contractile power of the heart.",
  "colosseum - The amphitheater of Vespasian in Rome. [Also written Coliseum.]",
  "lithaemia - A condition in which uric (lithic) acid is present in the blood.",
  "preconsign - To consign beforehand; to make a previous consignment of.",
  "phytological - Of or pertaining to phytology; botanical.",
  "ridgeling - A half-castrated male animal.",
  "onloft - Aloft; above ground. [Obs.] She kept her father's life onloft. Chaucer.",
  "oculo- - A combining form from L. oculus the eye.",
  "outrush - To rush out; to issue, or ru Garth.",
  "plainsman - One who lives in the plains.",
  "perceivance - Power of perceiving. [Obs.] \"The senses and common perceivance.\" Milton.",
  "friendliness - The condition or quality of being friendly. Sir P. Sidney.",
  "unkindred - Not kindred; not of the same kin. [Obs.] Rowe. -- Un*kin\"dred*ly, a.",
  "justificative - Having power to justify; justificatory.",
  "recursant - Displayed with the back toward the spectator; -- said especially of an eagle.",
  "anticatarrhal - Efficacious against catarrh. -- n.  An anticatarrhal remedy.",
  "hautgout - High relish or flavor; high seasoning.",
  "attry - Poisonous; malignant; malicious. [Obs.] Chaucer.",
  "yestermorn - The morning of yesterday. Coleridge.",
  "scribism - The character and opinions of a Jewish scribe in the time of Christ. F. W. Robertson.",
  "fluxive - Flowing; also, wanting solidity. B. Jonson.",
  "discretionarily - At discretion; according to one's discretion or judgment.",
  "troching - One of the small branches of a stag's antler.",
  "semblative - Resembling. [Obs.] And all is semblative a woman's part. Shak.",
  "professory - Of or pertaining to a professor; professorial. [R.] Bacon.",
  "upsidown - See Upsodown. [Obs. or Colloq.] Spenser.",
  "reloan - A second lending of the same thing; a renewal of a loan.",
  "scoffer - One who scoffs. 2 Pet. iii. 3.",
  "asaphus - A genus of trilobites found in the Lower Silurian formation. See Illust. in Append.",
  "londoner - A native or inhabitant of London. Shak.",
  "thenceforward - From that time onward; thenceforth.",
  "overexquisite - Too exquisite; too exact or nice; too careful.",
  "comptometer - A calculating machine; an arithmometer.",
  "vaginismus - A painful spasmodic contraction of the vagina, often rendering copulation impossible.",
  "carbuncular - Belonging to a carbuncle; resembling a carbuncle; red; inflamed.",
  "inosinic - Pertaining to, or derived from, inosite; as, inosinic acid.",
  "labyrinthian - Intricately winding; like a labyrinth; perplexed; labyrinthal.",
  "alhambra - The palace of the Moorish kings at Granada.",
  "contemplant - Given to contemplation; meditative. [R.] Coleridge.",
  "devaporation - The change of vapor into water, as in the formation of rain.",
  "hermitical - Pertaining to, or suited for, a hermit. Coventry.",
  "veering - Shifting. -- Veer\"ing*ly, adv.",
  "dilly-dally - To loiter or trifle; to waste time.",
  "isabelline - Of an isabel or isabella color.",
  "marikina - A small marmoset (Midas rosalia); the silky tamarin.",
  "interleaf - A leaf inserted between other leaves; a blank leaf inserted, as in a book.",
  "ozocerite - A waxlike mineral resin; -- sometimes called native paraffin, and mineral wax.",
  "pauldron - A piece of armor covering the shoulder at the junction of the body piece and arm piece.",
  "merriness - The quality or state of being merry; merriment; mirth; gayety, with laughter.",
  "subitaneous - Sudden; hasty. [Obs.] Bullokar. -- Sub`i*ta\"ne*ous*ness, n. [Obs.]",
  "proudness - The quality of being proud; pride. Set aside all arrogancy and proudness. Latimer.",
  "corruptless - Not susceptible of corruption or decay; incorruptible. Dryden.",
  "oss - To prophesy; to presage. [R. & Obs.] R. Edgeworth.",
  "prociduous - Falling from its proper place.",
  "rotifer - One of the Rotifera. See Illust. in Appendix.",
  "blackbirder - A slave ship; a slaver. [Colloq.]  F. T. Bullen.",
  "objicient - One who makes objection; an objector. [R.] Cardinal Wiseman.",
  "speculatorial - Speculatory; speculative. [Obs.]",
  "constructure - That which is constructed or formed; an edifice; a fabric. [Obs.]",
  "enamorment - The state of being enamored. [R.]",
  "etherol - An oily hydrocarbon regarded as a polymeric variety of ethylene, produced with etherin."
]