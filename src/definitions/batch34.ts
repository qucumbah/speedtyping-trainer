export default [
  "sphacelation - The process of becoming or making gangrenous; mortification.",
  "variform - Having different shapes or forms.",
  "hieromartyr - A priest who becomes a martyr.",
  "vasodentine - A modified form of dentine, which is permeated by blood capillaries; vascular dentine.",
  "transplendent - Resplendent in the highest degree. [R.] -- Tran*splen\"dent*ly, adv. [R.]",
  "proctorship - The office or dignity of a proctor; also, the term of his office. Clarendon.",
  "micrometrical - Belonging to micrometry; made by the micrometer. -- Mi`cro*met\"ric*al*ly, adv.",
  "dihedron - A figure with two sides or surfaces. Buchanan.",
  "fluvialist - One who exlpains geological phenomena by the action of streams. [R.]",
  "taqua-nut - A Central American name for the ivory nut.",
  "vestibuled train - Same as Vestibule  train, under Vestibule.",
  "geotic - Belonging to earth; terrestrial. [Obs.] Bailey.",
  "mammy - A child's name for mamma, mother.",
  "tranquillizer - One who, or that which, tranquilizes.",
  "unconstraint - Freedom from constraint; ease. Felton.",
  "digestibility - The quality of being digestible.",
  "stercoraceous - Of or pertaining to dung; partaking of the nature of, or containing, dung.",
  "polary - Tending to a pole; having a direction toward a pole. [R.] Sir T. Browne.",
  "drencher - 1. One who, or that which, west or steeps. 2. One who administers a drench.",
  "deontologist - One versed in deontology.",
  "oesophagus - Same as Esophagus, Esophageal, etc.",
  "inamorate - Enamored. Chapman. -- In*am\"o*rate*ly, adv. [R.]",
  "pistic - Pure; genuine. [R.] Jer. Taylor.",
  "arundiferous - Producing reeds or canes.",
  "obstringe - To constrain; to put under obligation. [R.] Bp. Gardiner.",
  "inergetic - Having no energy; sluggish. [R.] Boyle.",
  "signate - Having definite color markings.",
  "quadriliteral - Consisting of four letters.",
  "six-footer - One who is six feet tall. [Colloq. U.S.]",
  "defacer - One who, or that which, defaces or disfigures.",
  "hitherward - Toward this place; hither. Marching hitherward in proud array. Shak.",
  "stapler - 1. A dealer in staple goods. 2. One employed to assort wool according to its staple.",
  "chartomancy - Divination by written paper or by cards.",
  "materiate - Consisting of matter. [Obs.] Bacon.",
  "hypnology - A treatise on sleep; the doctrine of sleep.",
  "angustifolious - Having narrow leaves. Wright.",
  "cystolithic - Relating to stone in the bladder.",
  "epineural - Arising from the neurapophysis of a vertebra.",
  "hepatology - The science which treats of the liver; a treatise on the liver.",
  "lobcock - A dull, sluggish person; a lubber; a lob. [Low]",
  "phytoglyphy - See Nature printing, under Nature.",
  "capful - As much as will fill a cap. A capful of wind (Naut.), a light puff of wind.",
  "thaumaturgics - Feats of legerdemain, or magical performances.",
  "oxeyed - Having large, full eyes, like those of an ox. Burton.",
  "guardful - Cautions; wary; watchful. [Obs. or Poetic.] -- Guard\"ful*ly, adv.",
  "consension - Agreement; accord. Bentley.",
  "cymatium - A capping or crowning molding in classic architecture.",
  "stiff-tailed - Having the quill feathers of the tail somewhat rigid.",
  "dietetist - A physician who applies the rules of dietetics to the cure of diseases. Dunglison.",
  "mesonotum - The dorsal portion of the mesothorax of insects.",
  "disship - To dismiss from service on board ship. [Obs.] Hakluyt.",
  "coleslaw - A salad made of sliced cabbage.",
  "hunger-bit - Pinched or weakened by hunger. [Obs.] Milton.",
  "morphean - Of or relating to Morpheus, to dreams, or to sleep. Keats.",
  "selflessness - Quality or state of being selfless.",
  "mentor - A wise and faithful counselor or monitor.",
  "bequote - To quote constantly or with great frequency.",
  "bouleversement - Complete overthrow; disorder; a turning upside down.",
  "amphibiological - Pertaining to amphibiology.",
  "aspected - Having an aspect. [Obs.] B. Jonson.",
  "lavement - A washing or bathing; also, a clyster.",
  "rixdaler - A Dutch silver coin, worth about $1.00.",
  "stadtholderate - The office or position of a stadtholder.",
  "protegee - One under the care and protection of another.",
  "self-diffusive - Having power to diffuse itself; diffusing itself. Norris.",
  "agalactous - Lacking milk to suckle with.",
  "rhinological - Of or pertaining to rhinology.",
  "wittolly - Like a wittol; cuckoldly. [Obs.] Shak.",
  "graffer - a notary or scrivener. Bowvier.",
  "aidance - Aid. [R.] Aidance 'gainst the enemy. Shak.",
  "psalmistry - The use of psalms in devotion; psalmody.",
  "serpentinous - Relating to, or like, serpentine; as, a rock serpentinous in character.",
  "dishonorer - One who dishonors or disgraces; one who treats another indignity. Milton.",
  "pronunciator - One who pronounces; a pronouncer.",
  "chunk - A short, thick piece of anything. [Colloq. U. S. & Prov. Eng.]",
  "bewig - To cover (the head) with a wig. Hawthorne.",
  "incircumspect - Not circumspect; heedless; careless; reckless; impolitic. Tyndale.",
  "freedman - A man who has been a slave, and has been set free.",
  "soldiership - Military qualities or state; martial skill; behavior becoming a soldier. [R.] Shak.",
  "attercop - 1. A spider. [Obs.] 2. A peevish, ill-natured person. [North of Eng.]",
  "haminura - A large edible river fish (Erythrinus macrodon) of Guiana.",
  "leaning - The act, or state, of inclining; inclination; tendency; as, a leaning towards Calvinism.",
  "bardling - An inferior bard. J. Cunningham.",
  "nominally - In a nominal manner; by name; in name only; not in reality. Burke.",
  "hued - Having color; -- usually in composition; as, bright-hued; many- hued. Chaucer.",
  "anachronic - Characterized by, or involving, anachronism; anachronistic.",
  "anthracometric - Of or pertaining to an anthracometer.",
  "empyreumatize - To render empyreumatic. [R.]",
  "gord - An instrument of gaming; a sort of dice. [Obs.] Beau. & Fl.",
  "reappointment - The act of reappointing, or the state of being reappointed.",
  "byssine - Made of silk; having a silky or flaxlike appearance. Coles.",
  "inshell - To hide in a shell. [Obs.] Shak.",
  "disannex - To disunite; to undo or repeal the annexation of. State Trials (1608).",
  "thermobattery - A thermoelectric battery; a thermopile.",
  "sharpness - The quality or condition of being sharp; keenness; acuteness.",
  "urate - A salt of uric acid; as, sodium urate; ammonium urate.",
  "great-bellied - Having a great belly, bigbellied; pregnant; teeming. Shak.",
  "downtrodden - Trodden down; trampled down; abused by superior power. Shak.",
  "psychiatria - The application of the healing art to mental diseases. Dunglison.",
  "antiicteric - Good against jaundice. -- n.  A remedy for jaundice.",
  "approving - Expressing approbation; commending; as, an approving smile. -- Ap*prov\"ing*ly, adv.",
  "sugared - Sweetened. \"The sugared liquor.\" Spenser.  Also used figuratively; as, sugared kisses.",
  "merenchyma - Tissue composed of spheroidal cells.",
  "frizzly - Curled or crisped; as, frizzly, hair.",
  "misericorde - 1. Compassion; pity; mercy. [Obs.] 2. (Anc. Armor.)  Same as Misericordia, 2.",
  "pentateuchal - Of or pertaining to the Pentateuch.",
  "lowry - An open box car used on railroads. Compare Lorry.",
  "quinquennalia - Public games celebrated every five years.",
  "titmal - The blue titmouse. [Prov. Eng.]",
  "impulsiveness - The quality of being impulsive.",
  "gildale - A drinking bout in which every one pays an equal share. [Obs.]",
  "himalayan - Of or pertaining to the Himalayas, the great mountain chain in Hindostan.",
  "calorimetry - Measurement of the quantities of heat in bodies.",
  "camelopard - An African ruminant; the giraffe. See Giraffe.",
  "revisionary - Of or pertaining to revision; revisory.",
  "stone-cold - Cold as a stone. Stone-cold without, within burnt with love's flame. Fairfax.",
  "mollities - Unnatural softness of any organ or part. Dunglison.",
  "biscotin - A confection made of flour, sugar, marmalade, and eggs; a sweet biscuit.",
  "absentness - The quality of being absent-minded. H. Miller.",
  "laddie - A lad; a male sweetheart. [Scot.]",
  "balter - To stick together.[Obs.] Holland.",
  "snugness - The quality or state of being snug.",
  "ziment-water - A kind of water found in copper mines; water impregnated with copper.",
  "sillily - In a silly manner; foolishly. Dryden.",
  "ringbill - The ring-necked scaup duck; -- called also ring-billed blackhead. See Scaup.",
  "tallness - The quality or state of being tall; height of stature.",
  "phototelescope - A telescope adapted for taking photographs of the heavenly bodies.",
  "throatlatch - A strap of a bridle, halter, or the like, passing under a horse's throat.",
  "intercision - A cutting off, through, or asunder; interruption. [R.] Sir T. Browne.",
  "actinogram - A record made by the actinograph.",
  "lopeared - Having ears which lop or hang down.",
  "molendinaceous - Resembling the sails of a windmill.",
  "petrescent - Petrifying; converting into stone; as, petrescent water. Boyle.",
  "semi circumference - Half of a circumference.",
  "archipterygium - The primitive form of fin, like that of Ceratodus.",
  "privily - In a privy manner; privately; secretly. Chaucer. 2 Pet. ii. 1.",
  "strait-handed - Parsimonious; sparing; niggardly. [R.] -- Strait\"-hand`ed*ness, n. [R.]",
  "unvulgarize - To divest of vulgarity; to make to be not vulgar. Lamb.",
  "penible - Painstaking; assidous. [Obs.] Chaucer.",
  "recumb - To lean; to recline; to repose. [Obs.] J. Allen (1761).",
  "nuciform - Shaped like a nut; nut-shaped.",
  "pervertible - Capable of being perverted.",
  "alternant - Composed of alternate layers, as some rocks.",
  "breloque - A seal or charm for a watch chain. \"His chains and breloques.\" Thackeray.",
  "hesp - A measure of two hanks of linen thread. [Scot.] [Written also hasp.] Knight.",
  "stichometrical - Of or pertaining to stichometry; characterized by stichs, or lines.",
  "difformity - Irregularity of form; diversity of form; want of uniformity. [Obs.] Sir T. Browne.",
  "layering - A propagating by layers. Gardner.",
  "metrician - A composer of verses. [Obs.]",
  "entomophagous - Feeding on insects; insectivorous.",
  "defensative - That which serves to protect or defend.",
  "idiograph - A mark or signature peculiar to an individual; a trade-mark.",
  "mouther - One who mouths; an affected speaker.",
  "primipara - A woman who bears a child for the first time.",
  "monumentally - 1. By way of memorial. 2. By means of monuments.",
  "antimagistrical - Opposed to the office or authority of magistrates. [Obs.] South.",
  "unmortise - To loosen, unfix, or separate, as things mortised together. Tennyson.",
  "procacious - Pert; petulant; forward; saucy. [R.] Barrow.",
  "unweave - To unfold; to undo; to ravel, as what has been woven.",
  "acetabuliform - Shaped like a shallow; saucer-shaped; as, an acetabuliform calyx. Gray.",
  "apathistical - Apathetic; une motional. [R.]",
  "protectiveness - The quality or state of being protective. W. Pater.",
  "mesothorax - The middle segment of the thorax in insects. See Illust. of Coleoptera.",
  "circumnavigator - One who sails round. W. Guthrie.",
  "incendiarism - The act or practice of maliciously setting fires; arson.",
  "windbound - prevented from sailing, by a contrary wind. See Weatherbound.",
  "pretendership - The character, right, or claim of a pretender. Swift.",
  "captivation - The act of captivating. [R.] The captivation of our understanding. Bp. Hall.",
  "counterweigh - To weigh against; to counterbalance.",
  "bittering - A bitter compound used in adulterating beer; bittern.",
  "amortisable - Same as Amortize, Amortization, etc.",
  "orfrays - See Orphrey. [Obs.] Rom. of R.",
  "dealfish - A long, thin fish of the arctic seas (Trachypterus arcticus).",
  "pachydactyl - A bird or other animal having thick toes.",
  "northeastern - Of or pertaining to the northeast; northeasterly.",
  "prefulgency - Superior brightness or effulgency. [R.] Barrow.",
  "landholder - A holder, owner, or proprietor of land. -- Land\"hold`ing, n. & a.",
  "macrosporic - Of or pertaining to macrospores.",
  "nine-killer - The northern butcher bird.",
  "sea withwind - (Bot.) A kind of bindweed (Convolvulus Soldanella) growing on the seacoast of Europe.",
  "horridness - The quality of being horrid.",
  "inhumate - To inhume; to bury; to inter. Hedge.",
  "mercurism - A communication of news; an announcement. [Obs.] Sir T. Browne.",
  "prosiphon - A minute tube found in the protocon",
  "suitability - The quality or state of being suitable; suitableness.",
  "taboret - A small tabor. [Written also tabouret.]",
  "parigenin - A curdy white substance, obtained by the decomposition of parillin.",
  "scratch runner - One that starts from the scratch; hence, one of first-rate ability.",
  "impierce - To pierce; to penetrate. [Obs.] Drayton.",
  "sibylist - One who believes in a sibyl or the sibylline prophecies. Cudworth.",
  "interstratified - Stratified among or between other bodies; as, interstratified rocks.",
  "mailclad - Protected by a coat of mail; clad in armor. Sir W. Scott.",
  "pectinibranch - One of the Pectinibranchiata. Also used adjectively.",
  "protestator - One who makes protestation; a protester.",
  "supplial - The act of supplying; a supply. \"The supplial of a preposition.\" Fitzed. Hall.",
  "colonizer - One who promotes or establishes a colony; a colonist. Bancroft.",
  "dyas - A name applied in Germany to the Permian formation, there consisting of two principal groups.",
  "paranymphal - Bridal; nuptial. [R.] At some paranymphal feast. Ford.",
  "extirpable - Capable of being extirpated or eradicated; as, an extirpable plant. Evelyn.",
  "resignee - One to whom anything is resigned, or in whose favor a resignation is made.",
  "arraigner - One who arraigns. Coleridge.",
  "turbellarian - One of the Turbellaria. Also used adjectively.",
  "foraminous - Having foramina; full of holes; porous. Bacon.",
  "fluctuability - The capacity or ability to fluctuate. [R.] H. Walpole.",
  "glottological - Of or pertaining to glottology.",
  "astroite - A radiated stone or fossil; star-stone. [Obs.] [Written also astrite and astrion.]",
  "musculosity - The quality or state of being musculous; muscularity. [Obs.]",
  "judiciousness - The quality or state of being judicious; sagacity; s",
  "retection - Act of disclosing or uncovering something concealed. [Obs.] Boyle.",
  "cholate - A salt of cholic acid; as, sodium cholate.",
  "ploc - A mixture of hair and tar for covering the bottom of a ship.",
  "foodless - Without food; barren. Sandys.",
  "mandoline - A small and beautifully shaped instrument resembling the lute.",
  "bipectinated - Having two margins toothed like a comb.",
  "iridal - Of or pertaining to the iris or rainbow; prismatic; as, the iridal colors. Whewell.",
  "augustinianism - The doctrines held by Augustine or by the Augustinians.",
  "sugar-house - A building in which sugar is made or refined; a sugar manufactory.",
  "apogeotropic - Bending away from the ground; -- said of leaves, etc. Darwin.",
  "multiform - Having many forms, shapes, or appearances. A plastic and multiform unit. Hare.",
  "bescribble - To scribble over. \"Bescribbled with impertinences.\" Milton.",
  "witherling - A withered person; one who is decrepit. [Obs.] Chapman.",
  "radiative - Capable of radiating; acting by radiation. Tyndall.",
  "indoin - A substance resembling indigo blue, obtained artificially from certain isatogen compounds.",
  "pedestrianize - To practice walking; to travel on foot.",
  "concreture - A mass formed by concretion. [Obs.] Johnson.",
  "devil-diver - A small water bird. See Dabchick.",
  "reforest - To replant with trees; to reafforest; to reforestize.",
  "choriambic - Pertaining to a choriamb. -- n.  A choriamb.",
  "grippe - The influenza or epidemic catarrh. Dunglison.",
  "equivalvular - Same as Equivalve or Equivalved.",
  "heremitical - Of or pertaining to a hermit; solitary; secluded from society. Pope.",
  "absentment - The state of being absent; withdrawal. [R.] Barrow.",
  "inhoop - To inclose in a hoop, or as in a hoop. [R.] Shak.",
  "squawberry - A local name for the partridge berry; also, for the deerberry. [U. S.]",
  "water pennywort - Marsh pennywort. See under Marsh.",
  "periplast - Same as Periblast. -- Per`i*plas\"tic, a. Huxley.",
  "proportionateness - The quality or state of being proportionate. Sir M. Hale.",
  "enfeebler - One who, or that which, weakens or makes feeble.",
  "obtuse-angled - Having an obtuse angle; as, an obtuse-angled triangle.",
  "evolatic - Apt to fly away. [Obs. or R.] Blount.",
  "omniprevalent - Prevalent everywhere or in all things. Fuller.",
  "shepen - A stable; a shippen. [Obs.] The shepne brenning with the blacke smoke. Chaucer.",
  "wandy - Long and flexible, like a wand. [Prov. Eng.] Brockett.",
  "toggery - Clothes; garments; dress; as, fishing toggery. [Colloq.] togs",
  "hepatoscopy - Divination by inspecting the liver of animals.",
  "confuter - One who confutes or disproves.",
  "assignability - The quality of being assignable.",
  "assertorial - Asserting that a thing is; -- opposed to problematical and apodeictical.",
  "lattermath - The latter, or second, mowing; the aftermath.",
  "chancellery - Chancellorship. [Obs.] Gower.",
  "endothecium - The inner lining of an another cell.",
  "attachable - Capable of being attached; esp., liable to be taken by writ or precept.",
  "halfendeal - Half; by the part. [Obs.] Chaucer. -- n.  A half part. [Obs.] R. of Brunne.",
  "baldrib - A piece of pork cut lower down than the sparerib, and destitute of fat. [Eng.] Southey.",
  "little-ease - An old slang name for the pillory, stocks, etc., of a prison.[Eng.] Latimer.",
  "hyetography - The branch of physical science which treats of the geographical distribution of rain.",
  "exuviability - Capability of shedding the skin periodically. Craig.",
  "daturine - Atropine; -- called also daturia and daturina.",
  "limehound - A dog used in hunting the wild boar; a leamer. Spenser.",
  "illusively - In a illusive manner; falsely.",
  "misapprehend - To take in a wrong sense; to misunderstand. Locke.",
  "incensation - The offering of incense. [R.] Encyc. Brit.",
  "angeiology - Same as Angiology, Angiotomy, etc.",
  "sultanry - The dominions of a sultan. Bacon.",
  "scraber - (a) The Manx shearwater. (b) The black guillemot.",
  "impreventability - The state or quality of being impreventable. [R.]",
  "prelatess - A woman who is a prelate; the wife of a prelate. Milton.",
  "spuminess - The quality or condition of being spumy; spumescence.",
  "numerist - One who deals in numbers. [Obs.] Sir T. Browne.",
  "eagle-eyed - Sharp-sighted as an eagle. \"Inwardly eagle-eyed.\" Howell.",
  "rabbitry - A place where rabbits are kept; especially, a collection of hutches for tame rabbits.",
  "neatress - A woman who takes care of cattle. [R.] Warner.",
  "malm - A kind of brick of a light brown or yellowish color, made of sand, clay, and chalk.",
  "dinnerly - Of or pertaining to dinner. [R.] The dinnerly officer. Copley.",
  "suffocative - Tending or able to choke or stifle. \"Suffocative catarrhs.\" Arbuthnot.",
  "lames - Small steel plates combined together so as to slide one upon the form a piece of armor.",
  "molecast - A little elevation of earth made by a mole; a molehill. Mortimer.",
  "headman - A head or leading man, especially of a village community.",
  "pitchiness - Blackness, as of pitch; darkness.",
  "self-indignation - Indignation at one's own character or actions. Baxter.",
  "pseudoblepsis - False or depraved sight; imaginary vision of objects. Forsyth.",
  "threaden - Made of thread; as, threaden sails; a threaden fillet. [Obs.] Shak.",
  "moonless - Being without a moon or moonlight.",
  "thomas phosphate - Same as Basic slag, above.",
  "endearedly - With affection or endearment; dearly.",
  "toluylene - (a) Same as Stilbene. (b) Sometimes, but less properly, tolylene.",
  "asteism - Genteel irony; a polite and ingenious manner of deriding another.",
  "gelatinous - Of the nature and consistence of gelatin or the jelly; resembling jelly; viscous.",
  "bluffness - The quality or state of being bluff.",
  "ratifier - One who, or that which, ratifies; a confirmer. Shak.",
  "abstainer - One who abstains; esp., one who abstains from the use of intoxicating liquors.",
  "endocarditis - Inflammation of the endocardium.",
  "differentially - In the way of differentiation.",
  "microvolt - A measure of electro-motive force; the millionth part of one volt.",
  "limaille - Filings of metal. [Obs.] \"An ounce . . . of silver lymaille.\" Chaucer.",
  "qua-bird - The American night heron. See under Night.",
  "kettledrummer - One who plays on a kettledrum.",
  "planispheric - Of or pertaining to a planisphere.",
  "bull-necked - Having a short and thick neck like that of a bull. Sir W. Scott.",
  "cottrel - A trammel, or hook to support a pot over a fire. Knight.",
  "wallwort - The dwarf elder, or danewort (Sambucus Ebulus).",
  "recommencement - A commencement made anew.",
  "dubb - The Syrian bear. See under Bear. [Written also dhubb, and dub.]",
  "statesmanlike - Having the manner or wisdom of statesmen; becoming a statesman.",
  "thermosiphon - An arrangement of siphon tubes for assisting circulation in a liquid.",
  "scienter - Knowingly; willfully. Bouvier.",
  "acclimature - The act of acclimating, or the state of being acclimated. [R.] Caldwell.",
  "subcontractor - One who takes a portion of a contract, as for work, from the principal contractor.",
  "connaturalize - To bring to the same nature as something else; to adapt. [Obs.] Dr. J. Scott.",
  "alternity - Succession by turns; alternation. [R.] Sir T. Browne.",
  "obduct - To draw over; to cover. [Obs.] Sir T. Browne.",
  "nothingism - Nihility; nothingness. [R.]",
  "nominatively - In the manner of a nominative; as a nominative.",
  "zincify - To coat or impregnate with zinc.",
  "nidificate - To make a nest. Where are the fishes which nidificated in trees Lowell.",
  "ingrace - To ingratiate. [Obs.] G. Fletcher.",
  "nictation - the act of winking; nictitation.",
  "cloudlet - A little cloud. R. Browning. Eve's first star through fleecy cloudlet peeping. Coleridge.",
  "remuable - That may be removed; removable. [Obs.] Gower.",
  "aberrate - To go astray; to diverge. [R.] Their own defective and aberrating vision. De Quincey.",
  "poupeton - A puppet, or little baby. [Obs.] Palsgrave.",
  "montero - An ancient kind of cap worn by horsemen or huntsmen. Bacon.",
  "replait - To plait or fold again; to fold, as one part over another, again and again.",
  "trisect - 1. To cut or divide into three parts. 2. (Geom.)  To cut or divide into three equal parts.",
  "considerator - One who considers. [Obs.] Sir T. Browne.",
  "jalousie - A Venetian or slatted inside window blind.",
  "lapidation - The act of stoning. [Obs.] Bp. Hall.",
  "nourishingly - Nutritively; cherishingly.",
  "parodical - Having the character of parody. Very paraphrastic, and sometimes parodical. T. Warton.",
  "conservable - Capable of being preserved from decay or injury.",
  "indoxyl - A nitrogenous substance, C8H7NO, isomeric with oxindol, obtained as an oily liquid.",
  "loxodromics - The art or method of sailing on the loxodromic or rhumb line.",
  "self-ignorance - Ignorance of one's own character, powers, and limitations.",
  "democratically - In a democratic manner.",
  "stupeous - Resembling tow; having long, loose scales, or matted filaments, like tow; stupose.",
  "beggarhood - The condition of being a beggar; also, the class of beggars.",
  "imperforata - A division of Foraminifera, including those in which the shell is not porous.",
  "semilor - A yellowish alloy of copper and zinc. See Simplor.",
  "bowler - One who plays at bowls, or who rolls the ball in cricket or any other game.",
  "fedity - Turpitude; vileness. [Obs.] Bp. Hall.",
  "mammer - To hesitate; to mutter doubtfully. [Obs.]",
  "diffranchise - See Disfranchise, Disfranchisement.",
  "zooerythrine - A peculiar organic red coloring matter found in the feathers of various birds.",
  "perivascular - Around the blood vessels; as, perivascular lymphatics.",
  "erectly - In an erect manner or posture.",
  "bromyrite - Silver bromide, a rare mineral; -- called also bromargyrite.",
  "cullion - A mean wretch; a base fellow; a poltroon; a scullion. \"Away, base cullions.\" Shak.",
  "malacopterygious - Belonging to the Malacopterygii.",
  "sorus - One of the fruit dots, or small clusters of sporangia, on the back of the fronds of ferns.",
  "brodekin - A buskin or half-boot. [Written also brodequin.] [Obs.]",
  "quiesce - To be silent, as a letter; to have no sound. M. Stuart.",
  "regious - Regal; royal. [Obs.] Harrington.",
  "sandiness - The quality or state of being sandy, or of being of a sandy color.",
  "dradge - Inferior ore, separated from the better by cobbing. Raymond.",
  "adhortation - Advice; exhortation. [Obs.] Peacham.",
  "declamator - A declaimer. [R.] Sir T. Elyot.",
  "forswat - Spent with heat; covered with sweat. [Obs.] P. Sidney.",
  "spheroidity - The quality or state of being spheroidal.",
  "narratory - Giving an account of events; narrative; as, narratory letters. Howell.",
  "skerry - A rocky isle; an insulated rock. [Scot.]",
  "unaudienced - Not given an audience; not received or heard.",
  "monest - To warn; to admonish; to advise. [Obs.] Wyclif (2 Cor. v. 20).",
  "examining - Having power to examine; appointed to examine; as, an examining committee.",
  "tornaria - The peculiar free swimming larva of Balanoglossus. See Illust. in Append.",
  "reddour - Rigor; violence. [Obs.] Gower.",
  "centrolineal - Converging to a center; -- applied to lines drawn so as to meet in a point or center.",
  "yeared - Containing years; having existed or continued many years; aged. [Obs.] B. Jonson.",
  "negotiability - The quality of being negotiable or transferable by indorsement.",
  "germarium - An organ in which the ova are developed in certain Turbellaria.",
  "gravidated - Made pregnant; big. [Obs.] Barrow.",
  "homodynamy - The homology of metameres. See Metamere. Gegenbaur.",
  "circumfuse - To pour round; to spread round. His army circumfused on either wing. Milton.",
  "greek kalends - A time that will never come, as the Greeks had no calends.",
  "scandic - Of or pertaining to scandium; derived from, or containing, scandium.",
  "packman - One who bears a pack; a peddler.",
  "buntine - A thin woolen stuff, used chiefly for flags, colors, and ships' signals.",
  "alimentiveness - The instinct or faculty of appetite for food. [Chiefly in Phrenol.]",
  "weeding-rhim - A kind of implement used for tearing up weeds esp. on summer fallows. [Prov. Eng.]",
  "full-hearted - Full of courage or confidence. Shak.",
  "crebrisulcate - Marked with closely set transverse furrows.",
  "niggardise - Niggardliness. [Obs.] Spenser.",
  "scyphomedusa - Same as Acraspeda, or Discophora.",
  "whiner - One who, or that which, whines.",
  "torminous - Affected with tormina; griping.",
  "semiparabola - One branch of a parabola, being terminated at the principal vertex of the curve.",
  "synarchy - Joint rule or sovereignity. [R.] Stackhouse.",
  "misincline - To cause to have a wrong inclination or tendency; to affect wrongly.",
  "jeering - Mocking; scoffing. -- n.  A mocking utterance. -- Jeer\"ing*ly, adv.",
  "detruncate - To shorten by cutting; to cut off; to lop off.",
  "druery - Courtship; gallantry; love; an object of love. [Obs.] Chaucer.",
  "pastorly - Appropriate to a pastor. Milton.",
  "altimeter - An instrument for taking altitudes, as a quadrant, sextant, etc. Knight.",
  "wingmanship - Power or skill in flying. [R.] Duke of Argyll.",
  "acorn-shell - One of the sessile cirripeds; a barnacle of the genus Balanus. See Barnacle.",
  "avast - Cease; stop; stay. \"Avast heaving.\" Totten.",
  "lave-eared - Having large, pendent ears. [Obs.] Bp. Hall.",
  "scotch terrier - One of a breed of small terriers with long, rough hair.",
  "movableness - The quality or state of being movable; mobility; susceptibility of motion.",
  "dendroidal - Resembling a shrub or tree in form; treelike.",
  "zoogeography - The study or description of the geographical distribution of animals.",
  "powdike - A dike a marsh or fen. [Prov. Eng.] Halliwell.",
  "metabolian - An insect which undergoes a metamorphosis.",
  "cryptology - Secret or enigmatical language. Johnson.",
  "virgouleuse - An old French variety of pear, of little value.",
  "spawling - That which is spawled, or spit out.",
  "macrocosmic - Of or pertaining to the macrocosm. Tylor.",
  "astrogeny - The creation or evolution of the stars or the heavens. H. Spencer.",
  "pikelin - A light, thin cake or muffin. [Prov. Eng.] Wright.",
  "submergence - The act of submerging, or the state of being submerged; submersion.",
  "stancher - One who, or that which, stanches, or stops, the flowing, as of blood.",
  "hellenian - Of or pertaining to the Hellenes, or Greeks.",
  "queasiness - The state of being queasy; nausea; qualmishness; squeamishness. Shak.",
  "lowgh - strong imp. of Laugh. Etym: [Cf. 1st Low and 2d Lough.] Chaucer.",
  "vaticanist - One who strongly adheres to the papal authority; an ultramontanist.",
  "copartment - A compartment. [Obs.] T. Warton.",
  "thwite - To cut or clip with a knife; to whittle. [Obs. or Prov. Eng.] Chaucer.",
  "pyrosulphate - A salt of pyrosulphuric acid.",
  "placentary - Having reference to the placenta; as, the placentary system of classification.",
  "diarrhetic - Producing diarrhea, or a purging.",
  "bullfice - A kind of fungus. See Puffball.",
  "fangleness - Quality of being fangled. [Obs.] He them in new fangleness did pass. Spenser.",
  "vicarship - The office or dignity of a vicar.",
  "nale - Ale; also, an alehouse. [Obs.] Great feasts at the nale. Chaucer.",
  "barkeeper - One who keeps or tends a bar for the sale of liquors.",
  "declinometer - An instrument for measuring the declination of the magnetic needle.",
  "apoplexed - Affected with apoplexy. [Obs.] Shak.",
  "atresia - Absence or closure of a natural passage or channel of the body; imperforation.",
  "equinumerant - Equal as to number. [Obs.] Arbuthnot.",
  "batoidei - The division of fishes which includes the rays and skates.",
  "ungka-puti - The agile gibbon; -- called also ungka-pati, and ungka-etam. See Gibbon.",
  "sixscore - Six times twenty; one hundred and twenty.",
  "epenetic - Bestowing praise; eulogistic; laudatory. [Obs.] E. Phillips.",
  "cinnamone - A yellow crystalline substance, (C6H5.C2H2)2CO, the ketone of cinnamic acid.",
  "untooth - To take out the teeth of. Cowper.",
  "mordacity - The quality of being mordacious; biting severity, or sarcastic quality. Bacon.",
  "exossate - To deprive of bones; to take out the bones of; to bone. [Obs.] Bailey.",
  "canescent - Growing white, or assuming a color approaching to white.",
  "seak - Soap prepared for use in milling cloth.",
  "lochage - An officer who commanded a company; a captain. Mitford.",
  "fleming - A native or inhabitant of Flanders.",
  "pooh - Pshaw! pish! nonsense! -- an expression of scorn, dislike, or contempt.",
  "antiaphrodisiac - Same as Antaphrodisiac.",
  "hierogram - A form of sacred or hieratic writing.",
  "beardlessness - The state or quality of being destitute of beard.",
  "repeatedly - More than once; again and again; indefinitely.",
  "repetitive - Containing repetition; repeating. [R.]",
  "draughtboard - A checkered board on which draughts are played. See Checkerboard.",
  "entheal - Divinely inspired; wrought up to enthusiasm. [Obs.]",
  "depulsory - Driving or thrusting away; averting. [R.] Holland.",
  "newsy - Full of news; abounding in information as to current events. [Colloq.]",
  "swartness - The quality or state of being swart.",
  "unreformation - Want of reformation; state of being unreformed. [Obs.] Bp. Hall.",
  "liberalizer - One who, or that which, liberalizes. Emerson.",
  "redispose - To dispose anew or again; to readjust; to rearrange. A. Baxter.",
  "refind - To find again; to get or experience again. Sandys.",
  "smoulderingness - The state of smoldering.",
  "cowleeching - Healing the distemper of cows.",
  "premonstration - A showing beforehand; foreshowing.",
  "greenshank - A European sandpiper or snipe (Totanus canescens); -- called also greater plover.",
  "lamelliform - Thin and flat; scalelike; lamellar.",
  "coacervation - A heaping together. [R.] Bacon.",
  "eterminable - Interminable. [Obs.] Skelton.",
  "holster - A leather case for a pistol, carried by a horseman at the bow of his saddle.",
  "chessel - The wooden mold in which cheese is pressed. Simmonds.",
  "ulcerated - Affected with, or as with, an ulcer or ulcers; as, an ulcerated sore throat.",
  "posied - Inscribed with a posy. In poised lockets bribe the fair. Gay.",
  "kemelin - A tub; a brewer's vessel. [Obs.] Chaucer.",
  "substantiation - The act of substantiating or proving; evidence; proof.",
  "androgyny - Union of both sexes in one individual; hermaphroditism.",
  "immask - To cover, as with a mask; to disguise or conceal. [R.] Shak.",
  "subrogate - To put in the place of another; to substitute. Barrow.",
  "replantable - That may be planted again.",
  "visto - A vista; a prospect. [R.] Gay. Through the long visto of a thousand years. Young.",
  "ringmaster - One in charge of the performances (as of horses) within the ring in a circus.",
  "dilly - A kind of stagecoach. \"The Derby dilly.\" J. H. Frere.",
  "eclegm - A medicine made by mixing oils with sirups. John Quincy.",
  "inspectorial - Of or pertaining to an inspector or to inspection. [R.]",
  "incombine - To be incapable of combining; to disagree; to differ. [Obs.] Milton.",
  "arcadian - Of or pertaining to Arcadia; pastoral; ideally rural; as, Arcadian simplicity or scenery.",
  "miniment - A trifle; a trinket; a token. [Obs.] Spenser.",
  "plausibleize - To render plausible. [R.]",
  "mohammedan era - The era in use in Mohammedan countries. See Mohammedan year, below.",
  "scummy - Covered with scum; of the nature of scum. Sir P. Sidney.",
  "levari facias - A writ of execution at common law.",
  "flibbergib - A sycophant. [Obs. & Humorous.] \"Flatterers and flibbergibs.\" Latimer.",
  "photometric - Of or pertaining to photometry, or to a photometer.",
  "uraniscoraphy - Suture of the palate. See Staphyloraphy.",
  "cubile - The lowest course of stones in a building.",
  "giantize - To play the giant. [R.] Sherwood.",
  "gastrosplenic - Pertaining to the stomach and spleen; as, the gastrosplenic ligament.",
  "temulence - Intoxication; inebriation; drunkenness. [R.] \"Their temulency.\" Jer. Taylor.",
  "actable - Capable of being acted. Tennyson.",
  "perpetrator - One who perpetrates; esp., one who commits an offense or crime.",
  "witted - Having (such) a wit or understanding; as, a quick-witted boy.",
  "frustration - The act of frustrating; disappointment; defeat; as, the frustration of one's designs",
  "giantry - The race of giants. [R.] Cotgrave.",
  "barpost - A post sunk in the ground to receive the bars closing a passage into a field.",
  "catopter - A reflecting optical glass or instrument; a mirror. [Obs.]",
  "alkalimetrical - Of or pertaining to alkalimetry.",
  "membership - 1. The state of being a member. 2. The collective body of members, as of a society.",
  "beguilement - The act of beguiling, or the state of being beguiled.",
  "anemometric - Of or pertaining to anemometry.",
  "moodir - The governor of a province in Egypt, etc. [Written also mudir.]",
  "peripatetical - Peripatetic. [R.] Hales.",
  "catarrhal - Pertaining to, produced by, or attending, catarrh; of the nature of catarrh.",
  "hexapterous - Having six processes. Gray.",
  "rusma - A depilatory made of orpiment and quicklime, and used by the Turks. See Rhusma.",
  "croche - A little bud or knob at the top of a deer's antler.",
  "messianic - Of or relating to the Messiah; as, the Messianic office or character.",
  "welte - imp. of Weld, to wield. Chaucer.",
  "synaptase - A ferment resembling diastase, found in bitter almonds. Cf. Amygdalin, and Emulsin.",
  "by-lane - A private lane, or one opening out of the usual road.",
  "philanthropistic - Pertaining to, or characteristic of, a philanthropist. [R.] Carlyle.",
  "jugulum - The lower throat, or that part of the neck just above the breast.",
  "coeternity - Existence from eternity equally with another eternal being; equal eternity.",
  "transcendentality - The quality or state of being transcendental.",
  "sulphophosphite - A salt of sulphophosphorous acid.",
  "teaspoon - A small spoon used in stirring and sipping tea, coffee, etc., and for other purposes.",
  "misassay - To assay, or attempt, improperly or unsuccessfully. [Obs.] W. Browne.",
  "disvouch - To discredit; to contradict. [Obs.] Shak.",
  "egophonic - Belonging to, or resembling, egophony.",
  "resile - To start back; to recoil; to recede from a purpose. J. Ellis.",
  "bailor - One who delivers goods or money to another in trust.",
  "predicatory - Affirmative; positive. Bp. Hall.",
  "refusable - Capable of being refused; admitting of refusal.",
  "probationership - The state of being a probationer; novitiate. Locke.",
  "silver-gray - Having a gray color with a silvery luster; as, silver-gray hair.",
  "excusatory - Making or containing excuse or apology; apologetical; as, an excusatory plea.",
  "spring steel - A variety of steel, elastic, strong, and tough, rolled for springs, etc.",
  "honestation - The act of honesting; grace; adornment. [Obs.] W. Montagu.",
  "fieldy - Open, like a field. [Obs.] Wyclif.",
  "bident - An instrument or weapon with two prongs.",
  "routinist - One who habituated to a routine.",
  "retarder - One who, or that which, retards.",
  "effossion - A digging out or up. [R.] \"The effossion of coins.\" Arbuthnot.",
  "survise - To look over; to supervise. [Obs.] B. Jonson.",
  "tractile - Capable of being drawn out in length; ductile. Bacon.",
  "hibernianism - An idiom or mode of speech peculiar to the Irish. Todd.",
  "graveyard - A yard or inclosure for the interment of the dead; a cemetery.",
  "gutturality - The quality of being guttural. [R.] \"The old gutturality of k.\" Earle.",
  "vociferator - One who vociferates, or is clamorous. [R.]",
  "nicotinism - The morbid condition produced by the excessive use of tobacco.",
  "tranter - One who trants; a peddler; a carrier. [Written also traunter.] [Obs. or Prov. Eng.]",
  "purree - A yellow coloring matter. See Euxanthin.",
  "burring machine - A machine for cleansing wool of burs, seeds, and other substances.",
  "sayette - A mixed stuff, called also sagathy. See Sagathy.",
  "bearish - Partaking of the qualities of a bear; resembling a bear in temper or manners. Harris.",
  "abb - Among weaves, yarn for the warp. Hence, abb wool is wool for the abb.",
  "arboriculturist - One who cultivates trees.",
  "eugetinic - Pertaining to, or derived from, eugenol; as, eugetic acid.",
  "northwesterly - Toward the northwest, or from the northwest.",
  "standstill - A standing without moving forward or backward; a stop; a state or rest.",
  "belowt - To treat as a lout; to talk abusively to. [Obs.] Camden.",
  "hospitableness - The quality of being hospitable; hospitality. Barrow.",
  "viverra - A genus of carnivores which comprises the civets.",
  "free-martin - An imperfect female calf, twinborn with a male.",
  "gemination - A doubling; duplication; repetition. [R.] Boyle.",
  "guards - A body of picked troops; as, \"The Household Guards.\"",
  "uniliteral - Consisting of one letter only; as, a uniliteral word or sign.",
  "refaction - Recompense; atonemet; retribution. [Obs.] Howell.",
  "detestableness - The quality or state of being detestable.",
  "tetrandrous - Belonging to the class Tetrandria.",
  "scotoscope - An instrument that discloses objects in the dark or in a faint light. [Obs.] Pepys.",
  "elephantoidal - Resembling an elephant in form or appearance.",
  "cytogenetic - Of or pertaining to cytogenesis or cell development.",
  "flagelliform - Shaped like a whiplash; long, slender, round, flexible, and (comming) tapering.",
  "celebrator - One who celebrates; a praiser. Boyle.",
  "trades-unionist - A member of a trades union, or a supporter of trades unions.",
  "primula - The genus of plants including the primrose (Primula vera).",
  "stela - A small column or pillar, used as a monument, milestone, etc.",
  "aphorismatic - Pertaining to aphorisms, or having the form of an aphorism.",
  "minoress - See Franciscan Nuns, under Franciscan, a.",
  "enginer - A contriver; an inventor; a contriver of engines. [Obs.] Shak.",
  "enharden - To harden; to embolden. [Obs.] Howell.",
  "dentiloquy - The habit or practice of speaking through the teeth, or with them closed.",
  "regian - An upholder of kingly authority; a royalist. [Obs.] Fuller.",
  "prefigurative - Showing by prefiguration. \"The prefigurative atonement.\" Bp. Horne.",
  "amortisement - Same as Amortize, Amortization, etc.",
  "strived - Striven. Yea, so have I strived to preach the gospel. Rom. xv. 20.",
  "truculence - The quality or state of being truculent; savageness of manners; ferociousness.",
  "toluyl - Any one of the three hypothetical radicals corresponding to the three toluic acids.",
  "nape-crest - An African bird of the genus Schizorhis, related to the plantain eaters.",
  "raphaelite - One who advocates or adopts the principles of Raphaelism.",
  "tottlish - Trembling or tottering, as if about to fall; un steady. [Colloq. U.S.]",
  "mundificant - Serving to cleanse and heal. -- n.  A mundificant ointment or plaster.",
  "wald - A forest; -- used as a termination of names. See Weald.",
  "heritor - A proprietor or landholder in a parish. [Scot.]",
  "laxator - That which loosens; -- esp., a muscle which by its contraction loosens some part.",
  "expurgatorious - Expurgatory. [Obs.] \"Expurgatorious indexes.\" Milton.",
  "haematocrya - The cold-blooded vertebrates. Same as Hematocrya.",
  "outromance - To exceed in romantic character. [R.] Fuller.",
  "trollmydames - The game of nineholes. [Written also trolmydames.] [Obs.] Shak.",
  "agrope - In the act of groping. Mrs. Browning.",
  "premiership - The office of the premier.",
  "counselorship - The function and rank or office of a counselor. Bacon.",
  "joysome - Causing joyfulness. [R.] This all joysome grove. T. Browne.",
  "jocularly - In jest; for sport or mirth; jocosely.",
  "sea arrow - A squid of the genus Ommastrephes. See Squid.",
  "solidifiable - Capable of being solidified.",
  "strangeness - The state or quality of being strange (in any sense of the adjective).",
  "outtell - To surpass in telling, counting, or reckoning. \"I have outtold the clock.\" Beau. & Fl.",
  "abradant - A material used for grinding, as emery, sand, powdered glass, etc.",
  "hypersthenic - Composed of, or containing, hypersthene.",
  "nymphly - Resembling, or characteristic of, a nymph.",
  "novity - Newness; novelty. [Obs.] Sir T. Browne.",
  "histoid - Resembling the normal tissues; as, histoid tumors.",
  "phlogistication - The act or process of combining with phlogiston.",
  "multitubular - Having many tubes; as, a multitubular boiler.",
  "creasing - A layer of tiles forming a corona for a wall.",
  "corrigibleness - The state or quality of being corrigible; corrigibility.",
  "galactophorous - Milk-carrying; lactiferous; -- applied to the ducts of mammary glands.",
  "nectarial - Of or pertaining to the nectary of a plant.",
  "averseness - The quality of being averse; opposition of mind; unwillingness.",
  "embryonal - Pertaining to an embryo, or the initial state of any organ; embryonic.",
  "trophonian - Of or pertaining to Trophonius, his architecture, or his cave and oracle.",
  "ureteritis - Inflammation of the ureter. Dunglison.",
  "ampleness - The state or quality of being ample; largeness; fullness; completeness.",
  "adaptedness - The state or quality of being adapted; suitableness; special fitness.",
  "galvanization - The act of process of galvanizing.",
  "vamose - To depart quickly; to depart from. [Written also vamos, and vamoose.] [Slang, Eng. & U. S.]",
  "quantivalent - Of or pertaining to quantivalence. [Archaic]",
  "digynous - Of or pertaining to the Digynia; having two styles.",
  "bancal - An ornamental covering, as of carpet or leather, for a bench or form.",
  "maltalent - Ill will; malice. [Obs.] Rom. of R. Spenser.",
  "flawn - A sort of flat custard or pie. [Obs.] Tusser.",
  "deutzia - A genus of shrubs with pretty white flowers, much cultivated.",
  "medico-legal - Of or pertaining to law as affected by medical facts.",
  "tortuoslty - the quality or state of being tortuous.",
  "steatopygous - Having fat buttocks. Specimens of the steatopygous Abyssinian breed. Burton.",
  "pimpship - The office, occupation, or persom of a pimp. [R.]",
  "water-laid - Having a left-hand twist; -- said of cordage; as, a water-laid, or left-hand, rope.",
  "bicolligate - Having the anterior toes connected by a basal web.",
  "outweigh - To exceed in weight or value.",
  "nickar tree - Same as Nicker nut, Nicker tree.",
  "impleach - To pleach; to interweave. [Obs.] Shak.",
  "hickwall - The lesser spotted woodpecker (Dendrocopus minor) of Europe. [Prov. Eng.]",
  "oogenesis - The development, or mode of origin, of the ova.",
  "keuper - The upper division of the European Triassic. See Chart of Geology.",
  "effumability - The capability of flying off in fumes or vapor. [Obs.] Boyle.",
  "cat-eyed - Having eyes like a cat; hence, able to see in the dark.",
  "paralactic - Designating an acid called paralactic acid. See Lactic acid, under Lactic.",
  "moldable - Capable of being molded or formed.",
  "entastic - Relating to any disease characterized by tonic spasms.",
  "isospondyli - An extensive order of fishes, including the salmons, herrings, and many allied forms.",
  "arborary - Of or pertaining to trees; arboreal.",
  "overshake - To shake over or away; to drive away; to disperse. [Obs.] Chaucer.",
  "intertarsal - Between the tarsal bones; as, the intertarsal articulations.",
  "ramulose - Having many small branches, or ramuli.",
  "seapiece - A picture representing a scene at sea; a marine picture. Addison.",
  "weighboard - Clay intersecting a vein. Weale.",
  "argolic - Pertaining to Argolis, a district in the Peloponnesus.",
  "rescindable - Capable of being rescinded.",
  "croupade - A leap in which the horse pulls up his hind legs toward his belly.",
  "salina - 1. A salt marsh, or salt pond, inclosed from the sea. 2. Salt works.",
  "opalescent - Reflecting a milky or pearly light from the interior; having an opaline play of colors.",
  "microphonous - Serving to augment the intensity of weak sounds; microcoustic.",
  "ibidem - In the same place; -- abbreviated ibid. or ib.",
  "ceromancy - Divination by dropping melted wax in water.",
  "rubicelle - A variety of ruby of a yellowish red color, from Brazil.",
  "manrope - One of the side ropes to the gangway of a ship. Totten.",
  "osmanli - A Turkish official; one of the dominant tribe of Turks; loosely, any Turk.",
  "ritenuto - Held back; holding back; ritardando.",
  "linctus - Medicine taken by licking with the tongue.",
  "blackpoll - A warbler of the United States (Dendroica striata).",
  "deposable - Capable of being deposed or deprived of office. Howell.",
  "sanguinaceous - Of a blood-red color; sanguine.",
  "paletot - (a) An overcoat. Dickens. (b) A lady's outer garment, -- of varying fashion.",
  "portableness - The quality or state of being portable; portability.",
  "octofid - Cleft or separated into eight segments, as a calyx.",
  "acaridan - One of a group of arachnids, including the mites and ticks.",
  "cellepore - A genus of delicate branching corals, made up of minute cells, belonging to the Bryozoa.",
  "servable - 1. Capable of being served. 2. Etym: [L. servabilis.]  Capable of being preserved. [R.]",
  "buhrstone - A cellular, flinty rock, used for mill stones. [Written also burrstone.]",
  "jacketing - The material of a jacket; as, nonconducting jacketing.",
  "lerot - A small European rodent (Eliomys nitela), allied to the dormouse.",
  "lickpenny - A devourer or absorber of money. \"Law is a lickpenny.\" Sir W. Scott.",
  "dronkelewe - Given to drink; drunken. [Obs.] Chaucer.",
  "zoogeny - The doctrine of the formation of living beings.",
  "amphitheatric - Of, pertaining to, exhibited in, or resembling, an amphitheater.",
  "telega - A rude four-wheeled, springless wagon, used among the Russians.",
  "preoperculum - The anterior opercular bone in fishes.",
  "bis- - A form of Bi-, sometimes used before s, c, or a vowel.",
  "subarcuated - Having a figure resembling that of a bow; somewhat curved or arched.",
  "ethicist - One who is versed in ethics, or has written on ethics.",
  "craunch - To crush with the teeth; to chew with violence and noise; to crunch. Swift.",
  "stilet - 1. A stiletto. [R.] 2. (Surg.)  See Stylet, 2.",
  "sacculo-cochlear - pertaining to the sacculus and cochlea of the ear.",
  "lucernarian - Of or pertaining to the Lucernarida. -- n.  One of the Lucernarida.",
  "meedfully - According to merit; suitably.",
  "gallopin - An under servant for the kitchen; a scullion; a cook's errand boy. [Obs.] Halliwell.",
  "civilizable - Capable of being civilized.",
  "preregnant - One who reigns before another; a sovereign predecessor. [R.] Warner.",
  "impreventable - Not preventable; invitable.",
  "delusory - Delusive; fallacious. Glanvill.",
  "indiadem - To place or set in a diadem, as a gem or gems.",
  "intercedent - Passing between; mediating; pleading. [R.] -- In`ter*ced\"ent*ly, adv.",
  "draconin - A red resin forming the essential basis of dragon's blood; -- called also dracin.",
  "cedared - Covered, or furnished with, cedars.",
  "storify - To form or tell stories of; to narrate or describe in a story. [Obs.]",
  "camerate - 1. To build in the form of a vault; to arch over. 2. To divide into chambers.",
  "susceptivity - Capacity for receiving; susceptibility. [R.] Wollaston.",
  "saltish - Somewhat salt. -- Salt\"ish*ly, adv. -- Salt\"ish*ness, n.",
  "megalocephalia - The condition of having an abnormally large head. -- Meg`a*lo*ce*phal\"ic (#), a.",
  "geneagenesis - Alternate generation. See under Generation.",
  "double-barrelled - Having two barrels; -- applied to a gun.",
  "nervelessness - The state of being nerveless.",
  "faulter - One who commits a fault. [Obs.] Behold the faulter here in sight. Fairfax.",
  "correctible - Capable of being corrected.",
  "finestill - To distill, as spirit from molasses or some saccharine preparation.",
  "sphagnicolous - Growing in moss of the genus Sphagnum.",
  "dryfoot - The scent of the game, as far as it can be traced. [Obs.] Shak.",
  "loellingite - A tin-white arsenide of iron, isomorphous with arsenopyrite.",
  "protensive - Drawn out; extended. [R.] Time is a protensive quantity. Sir W. Hamilton.",
  "sudamina - Minute vesicles surrounded by an area of reddened skin, produced by excessive sweating.",
  "episkeletal - Above or outside of the endoskeleton; epaxial.",
  "myographical - Of or pertaining to myography.",
  "influxion - A flowing in; infusion. [R.] Bacon.",
  "troic - Pertaining to Troy; Trojan. Gladstone.",
  "appetibility - The quality of being desirable. Bramhall.",
  "indihumin - A brown amorphous substance resembling humin, and obtained from indican.",
  "vitaille - Food; victuals. [Obs.] Piers Plowman. Chaucer.",
  "high-churchism - The principles of the high-church party.",
  "hircine - 1. Goatlike; of or pertaining to a goat or the goats. 2. Of a strong goatish smell.",
  "afterings - The last milk drawn in milking; strokings. [Obs.] Grose.",
  "hierophantic - Of or relating to hierophants or their teachings.",
  "rascaldom - State of being a rascal; rascality; domain of rascals; rascals, collectively. Emerson.",
  "marmorated - Variegated like marble; covered or overlaid with marble. [R.]",
  "sperate - Hoped for, or to be hoped for. [R.] Bouvier.",
  "unconfidence - Absence of confidence; uncertainty; doubt.",
  "paunce - The pansy. \"The pretty paunce.\" Spenser.",
  "claret - The name firat given in England to the red wines of M",
  "incriminatory - Of or pertaining to crimination; tending to incriminate; criminatory.",
  "bryozoan - Of or pertaining to the Bryozoa. -- n.  One of the Bryozoa.",
  "wareroom - A room in which goods are stored or exhibited for sale.",
  "malaise - An indefinite feeling of uneasiness, or of being sick or ill at ease.",
  "wheatbird - A bird that feeds on wheat, especially the chaffinch.",
  "abay - Barking; baying of dogs upon their prey. See Bay. [Obs.]",
  "remonstrantly - In a remonstrant manner.",
  "insperse - To sprinkle; to scatter. [Obs.] Bailey.",
  "telestic - Tending or relating to a purpose or an end. [R.] Cudworth.",
  "marcidity - The state or quality of being withered or lean. [R.]",
  "navarch - The commander of a fleet. Mitford.",
  "strophiolated - Furnished with a strophiole, or caruncle, or that which resembles it. Gray.",
  "wagerer - One who wagers, or lays a bet.",
  "gracility - State of being gracilent; slenderness. Milman. \"Youthful gracility.\" W. D. Howells.",
  "yeanling - A lamb or a kid; an eanling. Shak.",
  "commensalism - The act of eating together; table fellowship.",
  "stalworthhood - The quality or state of being stalworth; stalwartness; boldness; daring. [Obs.]",
  "copestone - A stone for coping. See Coping.",
  "lumbering - The business of cutting or getting timber or logs from the forest for lumber. [U.S.]",
  "reliquary - A depositary, often a small box or casket, in which relics are kept.",
  "mercature - Commerce; traffic; trade. [Obs.]",
  "donary - A thing given to a sacred use. [R.] Burton.",
  "nogging - Rough brick masonry used to fill in the interstices of a wooden frame, in building.",
  "townish - Of or pertaining to the inhabitants of a town; like the town. [R.] Turbervile.",
  "planometer - An instrument for gauging or testing a plane surface. See Surface gauge, under Surface.",
  "prying - Inspecting closely or impertinently. Syn. -- Inquisitive; curious. See Inquisitive.",
  "podocephalous - Having a head of flowers on a long peduncle, or footstalk.",
  "soldierlike - Like a soldier; soldierly.",
  "adenose - Like a gland; full of glands; glandulous; adenous.",
  "weryangle - See Wariangle. [Obs.] Chaucer.",
  "exo - A prefix signifying out of, outside; as in exocarp, exogen, exoskeleton.",
  "chirper - One who chirps, or is cheerful.",
  "doll - A child's puppet; a toy baby for a little girl.",
  "bedded - Provided with a bed; as, double-bedded room; placed or arranged in a bed or beds.",
  "rushlight - A rush candle, or its light; hence, a small, feeble light.",
  "proemptosis - The addition of a day to the lunar calendar. [R.] See Metemptosis.",
  "opacate - To darken; to cloud. [Obs.] Boyle.",
  "lacerable - That can be lacerated or torn.",
  "physoclisti - An order of teleost in which the air bladder has no opening.",
  "pteridomania - A madness, craze, or strong fancy, for ferns. [R.] C. Kingsley.",
  "apophlegmatism - 1. (Med.)  The action of apophlegmatics. 2. An apophlegmatic. [Obs.] Bacon.",
  "polygamize - To practice polygamy; to marry several wives. Sylvester. Coleridge.",
  "wastor - A waster; a thief. [Obs. or R.] [Written also wastour.] Chaucer. Southey.",
  "parraqua - A curassow of the genus Ortalida, allied to the guan.",
  "trireme - An ancient galley or vessel with tree banks, or tiers, of oars.",
  "auricled - Having ear-shaped appendages or lobes; auriculate; as, auricled leaves.",
  "shabrack - The saddlecloth or housing of a cavalry horse.",
  "artfully - In an artful manner; with art or cunning; skillfully; dexterously; craftily.",
  "petromastoid - Of or pertaining to the petrous and mastoid parts of the temporal bone, periotic.",
  "acharnement - Savage fierceness; ferocity.",
  "flugel - A grand piano or a harpsichord, both being wing-shaped.",
  "furthermore - or conj. Moreover; besides; in addition to what has been said.",
  "laughsome - Exciting laughter; also, addicted to laughter; merry. [R.]",
  "revisional - Of or pertaining to revision; revisory.",
  "intemerateness - The state of being unpolluted; purity. [Obs.] Donne.",
  "transverberate - To beat or strike through. [Obs.]",
  "pooped - (a) Having a poop; furnished with a poop. (b) Struck on the poop.",
  "exploratory - Serving or intended to explore; searching; examining; explorative. Sir H. Wotton.",
  "decidement - Means of forming a decision. [Obs.] Beau. & Fl.",
  "moonlit - Illumined by the moon. \"The moonlit sea.\" Moore. \"Moonlit dells.\" Lowell.",
  "non sequitur - An inference which does not follow from the premises.",
  "infortuned - Unfortunate. [Obs.] I, woeful wretch and infortuned wight. Chaucer.",
  "loneness - Solitude; seclusion. [Obs.] Donne.",
  "finned - Having a fin, or fins, or anything resembling a fin. Mortimer.",
  "absorpt - Absorbed. [Arcahic.] \"Absorpt in care.\" Pope.",
  "plenicorn - A ruminant having solid horns or antlers, as the deer. Brande & C.",
  "cindery - Resembling, or composed of, cinders; full of cinders.",
  "podurid - Any species of Podura or allied genera. -- a.  Pertaining to the poduras.",
  "antestomach - A cavity which leads into the stomach, as in birds. Ray.",
  "fulciment - A prop; a fulcrum. [Obs.] Bp. Wilkins.",
  "transvasate - To pour out of one vessel into another. [Obs.] Cudworth.",
  "graphicness - The quality or state of being graphic.",
  "antisabbatarian - One of a sect which opposes the observance of the Christian Sabbath.",
  "snow-blind - Affected with blindness by the brilliancy of snow. -- Snow\"-blind`ness, n.",
  "jo - A sweetheart; a darling. [Scot.] Burns.",
  "pendulosity - The state or quality of being pendulous. Sir T. Browne.",
  "impliable - Not pliable; inflexible; inyielding.",
  "mesoscapular - Of or pertaining to the mesoscapula.",
  "epistolography - The art or practice of writing epistles.",
  "rendrock - A kind of dynamite used in blasting. [U.S.]",
  "sound-board - A sounding-board. To many a row of pipes the sound-board breathes. Milton.",
  "teend - To kindle; to burn. [Obs.] Herrick.",
  "foothalt - A disease affecting the feet of sheep.",
  "tabifical - Producing tabes; wasting; tabefying.",
  "floury - Of or resembling flour; mealy; covered with flour. Dickens.",
  "lossless - Free from loss. [Obs.] Milton.",
  "wair - A piece of plank two yard Bailey.",
  "geest - Alluvial matter on the surface of land, not of recent origin. R. Jameson.",
  "micro-chemical - Of or pertaining to micro-chemistry; as, a micro-chemical test.",
  "bonniness - The quality of being bonny; gayety [R.]",
  "anthropophagical - Relating to cannibalism or anthropophagy.",
  "contextured - Formed into texture; woven together; arranged; composed. [R.] Carlyle.",
  "damnability - The quality of being damnable; damnableness. Sir T. More.",
  "mynheer - The Dutch equivalent of Mr. or Sir; hence, a Dutchman.",
  "disarrange - To unsettle or disturb the order or due arrangement of; to throw out of order.",
  "interset - To set between or among. [R.]",
  "tile-drain - To drain by means of tiles; to furnish with a tile drain.",
  "glandered - Affected with glanders; as, a glandered horse. Yu",
  "atterration - The act of filling up with earth, or of forming land with alluvial earth. [Obs.]",
  "undiocesed - Unprovided with a diocese; having no diocese. Milton.",
  "gleed - A live or glowing coal; a glede. [Archaic] Chaucer. Longfellow.",
  "prewarn - To warn beforehand; to forewarn. [R.]",
  "polygamist - One who practices polygamy, or maintains its lawfulness.",
  "thermoelectric - Pertaining to thermoelectricity; as, thermoelectric currents.",
  "completive - Making complete. [R.] J. Harris.",
  "dreint - p. p. of Drench to drown. [Obs.] Chaucer.",
  "gnomological - Pertaining to, of the nature of, or resembling, a gnomology.",
  "skag - An additional piece fastened to the keel of a boat to prevent lateral motion. See Skeg.",
  "shieling - A hut or shelter for shepherds of fishers. See Sheeling. [Scot.]",
  "pentagynous - Of or pertaining to plants of the order Pentagyna; having five styles.",
  "interdictive - Having the power to prohibit; as, an interdictive sentence. Milton.",
  "molecularly - With molecules; in the manner of molecules. W. R. Grove.",
  "stirp - Stock; race; family. [Obs.] Bacon.",
  "alegge - To allay or alleviate; to lighten. [Obs.] That shall alegge this bitter blast. Spenser.",
  "drillstock - A contrivance for holding and turning a drill. Knight.",
  "tribometer - An instrument to ascertain the degree of friction in rubbing surfaces. Brande & C.",
  "unweldy - Unwieldy; unmanageable; clumsy. [Obs.] Our old limbs move [may] well be unweld. Chaucer.",
  "disgracious - Wanting grace; unpleasing; disagreeable. Shak.",
  "zif - The second month of the Jewish ecclesiastical year, corresponding to our May.",
  "navigerous - Bearing ships; capable of floating vessels. [R.] Blount.",
  "fire-set - A set of fire irons, including, commonly, tongs, shovel, and poker.",
  "crouse - Brisk; lively; bold; self-complacent. [Scot.] Burns.",
  "cryptography - The act or art of writing in secret characters; also, secret characters, or cipher.",
  "goatherd - One who tends goats. Spenser.",
  "systematology - The doctrine of, or a treatise upon, systems. Dunglison.",
  "extraphysical - Not subject to physical laws or methods.",
  "nomarch - The chief magistrate of a nome or nomarchy.",
  "novitious - Newly invented; recent; new. [Obs.] Bp. Pearson.",
  "scruze - To squeeze, compress, crush, or bruise. [Obs. or Low] Spenser.",
  "frowny - Frowning; scowling. [Obs.] Her frowny mother's ragged shoulder. Sir F. Palgrave.",
  "cretic - A poetic foot, composed of one short syllable between two long ones (-Bentley.",
  "coportion - Equal share. [Obs.] Myself will bear . . . coportion of your pack. Spenser.",
  "hydrogenation - The act of combining with hydrogen, or the state of being so combined.",
  "essoiner - An attorney who sufficiently excuses the absence of another.",
  "matutinal - Of or pertaining to the morning; early.",
  "merce - To subject to fine or amercement; to mulct; to amerce. [Obs.]",
  "duodenal - Of or pertaining to the duodenum; as, duodenal digestion.",
  "monoxylon - A canoe or boat made from one piece of timber.",
  "haulage - Act of hauling; as, the haulage of cars by an engine; charge for hauling.",
  "teagle - A hoisting apparatus; an elevator; a crane; a lift. [Prov. Eng.]",
  "zooenic - Of or pertaining to animals; obtained from animal substances.",
  "reniform - Having the form or shape of a kidney; as, a reniform mineral; a reniform leaf.",
  "boxhauling - A method of going from one tack to another. See Boxhaul.",
  "volcanize - To subject to, or cause to undergo, volcanic heat, and to be affected by its action.",
  "transmutability - The quality of being transmutable.",
  "by-corner - A private corner. Britain being a by-corner, out of the road of the world. Fuller.",
  "geodephagous - Living in the earth; -- applied to the ground beetles.",
  "corposant - St. Elmo's fire. See under Saint.",
  "reliever - One who, or that which, relieves.",
  "favillous - Of or pertaining to ashes. [Obs.] Light and favollous particles. Sir T. Browne.",
  "exclusory - Able to exclude; excluding; serving to exclude.",
  "folious - 1. Like a leaf; thin; unsubstantial. [R.] Sir T. Browne. 2. (Bot.)  Foliose. [R.]",
  "disburthen - To disburden; to relieve of a load. [Archaic]",
  "monomphalus - A form of double monster, in which two individuals are united by a common umbilicus.",
  "disworth - To deprive of worth; to degrade. [Obs.] Feltham.",
  "tercet - 1. (Mus.)  A triplet. Hiles. 2. (Poetry)  A triplet; a group of three lines.",
  "fanner - 1. One who fans. Jer. li. 2. 2. A fan wheel; a fan blower. See under Fan.",
  "conchiform - Shaped like one half of a bivalve shell; shell-shaped.",
  "ambagious - Circumlocutory; circuitous. [R.]",
  "guld - A flower. See Gold. [Obs.] Chaucer.",
  "soapiness - Quality or state of being soapy.",
  "pugilism - The practice of boxing, or fighting with the fist.",
  "cartomancy - The act of telling fortunes with cards.",
  "whitehead torpedo - A form of self-propelling torpedo.",
  "opsonation - A catering; a buying of provisions. [Obs.] Bailey.",
  "bigential - Including two tribes or races of men.",
  "water tu twist - Yarn made by the throstle, or water frame.",
  "vomition - The act or power of vomiting. Grew.",
  "treasury stock - Issued stock of an incorporated company held by the company itself.",
  "rouncy - A common hackney horse; a nag. [Obs.] he rode upon a rouncy as he could. Chaucer.",
  "lowing - The calling sound made by cows and other bovine animals.",
  "transplanter - One who transplants; also, a machine for transplanting trees.",
  "rickrack - A kind of openwork edging made of serpentine braid.",
  "thermotherapy - Treatment of disease by heat, esp. by hot air.",
  "songish - Consisting of songs. [R.] Dryden.",
  "whapping - Very large; monstrous; astonishing; as, a whapping story. [Colloq.]",
  "hybridism - The state or quality of being hybrid.",
  "rainbowed - Formed with or like a rainbow.",
  "adjutory - Serving to help or assist; helping. [Obs.]",
  "noviceship - The state of being a novice; novitiate.",
  "intercondylar - Between condyles; as, the intercondylar fossa or notch of the femur.",
  "charbocle - Carbuncle. [Written also Charboncle.] [Obs.] Chaucer.",
  "accordionist - A player on the accordion.",
  "unwarped - Not warped; hence, not biased; impartial.",
  "complacential - Marked by, or causing, complacence. [Obs.] \"Complacential love.\" Baxter.",
  "extraaxillar - Growing outside of the axils; as, an extra-axillary bud.",
  "fallowist - One who favors the practice of fallowing land. [R.] Sinclair.",
  "postfurca - One of the internal thoracic processes of the sternum of an insect.",
  "undecide - To reverse or recant, as a previous decision.",
  "cretism - A Cretan practice; iying; a falsehood.",
  "truite - Having a delicately crackled surface; --applied to porcelian, etc.",
  "raving - Talking irrationally and wildly; as, a raving lunatic. -- Rav\"ing*ly, adv.",
  "hylotheist - One who believes in hylotheism.",
  "pinchfist - A closefisted person; a miser.",
  "quassation - The act of shaking, or the state of being shaken. Gayton.",
  "inaudibility - The quality of being inaudible; inaudibleness.",
  "conspiringly - In the manner of a conspirator; by conspiracy. Milton.",
  "zwieback - A kind of biscuit or  rusk first baked in a loaf and afterwards cut and toasted.",
  "tetrapharmacom - A combination of wax, resin, lard, and pitch, composing an ointment. Brande & C.",
  "euphorbiaceous - Of, relating to, or resembling, the Euphorbia family.",
  "dictature - Office of a dictator; dictatorship. [R.] Bacon.",
  "hellhag - A hag of or fit for hell. Bp. Richardson.",
  "presager - One who, or that which, presages; a foreteller; a foreboder. Shak.",
  "enneagonal - Belonging to an enneagon; having nine angles.",
  "ineradicably - So as not to be eradicable.",
  "schatchen - A person whose business is marriage brokage; a marriage broker, esp. among certain Jews.",
  "tumescent - Slightly tumid; swollen, as certain moss capsules.",
  "frontated - Growing broader and broader, as a leaf; truncate.",
  "open-mouthed - Having the mouth open; gaping; hence, greedy; clamorous. L'Estrange.",
  "salol - A white crystalline substance consisting of phenol salicylate.",
  "unguestlike - In a manner not becoming to a guest. [R.] Milton.",
  "hypophysial - Of or pertaining to the hypophysis; pituitary.",
  "unfellowed - Being without a fellow; unmatched; unmated. Shak.",
  "alkalize - To render alkaline; to communicate the properties of an alkali to.",
  "coordain - To ordain or appoint for some purpose along with another.",
  "neatness - The state or quality of being neat.",
  "prenticeship - Apprenticeship. [Obs. or Colloq.] He served a prenticeship who sets up shop. Pope.",
  "free-spoken - Accustomed to speak without reserve. Bacon. -- Free\"-spo`ken-ness, n.",
  "contentless - Discontented; dissatisfied. [R.] Shak.",
  "butyryl - The radical (C4H7O) of butyric acid.",
  "paludamentum - Antiq.) A military cloak worn by a general and his principal officers.",
  "donzel - A young squire, or knight's attendant; a page. [Obs.] Beau. & Fl.",
  "presbytership - The office or station of a presbyter; presbyterate.",
  "myrmotherine - Feeding upon ants; -- said of certain birds.",
  "irrationality - The quality or state of being irrational. \"Brutish irrationaliity.\" South.",
  "superfamily - A group intermediate between a family and a suborder.",
  "tapering - Becoming gradually smaller toward one end. -- Ta\"per*ing*ly, adv.",
  "concessionary - Of or pertaining to a concession. -- n.; pl. -ries.  A concessionaire.",
  "flete - To float; to swim. [Obs.] \"Whether I sink or flete.\" Chaucer.",
  "transfuge - One who flees from one side to another; hence, a deserter; a turncoat; an apostate. [R.]",
  "torpescence - The quality or state or being torpescent; torpidness; numbness; stupidity.",
  "spasticity - 1. A state of spasm. 2. The tendency to, or capability of suffering, spasm.",
  "supracranial - Situated above, or in the roof of, the cranium.",
  "lascious - Loose; lascivious. [Obs.] \"To depaint lascious wantonness.\" Holland.",
  "victoria crape - A kind of cotton crape.",
  "impassion - To move or affect strongly with passion. [Archaic] Chapman.",
  "detenebrate - To remove darkness from. [Obs.] Ash.",
  "ling-bird - The European meadow pipit; -- called also titling.",
  "clivity - Inclination; ascent or descent; a gradient. [R.]",
  "incremation - Burning; esp., the act of burning a dead body; cremation.",
  "seedlip - A vessel in which a sower carries the seed to be scattered. [Prov. Eng.]",
  "parousia - (a) The nativity of our Lord. (b) The last day. Shipley.",
  "buttons - A boy servant, or page, -- in allusion to the buttons on his livry. [Colloq.] Dickens.",
  "allenarly - Solely; only. [Scot.] Sir W. Scott.",
  "mangonize - To furbish up for sale; to set off to advantage. [Obs. or R.] B. Jonson.",
  "half-decked - Partially decked. The half-decked craft . . . used by the latter Vikings. Elton.",
  "anatomically - In an anatomical manner; by means of dissection.",
  "gynecological - Of or pertaining to gynecology.",
  "heliotropic - Manifesting heliotropism; turning toward the sun.",
  "suckler - An animal that suckles its young; a mammal.",
  "ataraxia - Perfect peace of mind, or calmness.",
  "modius - A dry measure, containing about a peck.",
  "battailant - Prepared for battle; combatant; warlike. Spenser. -- n.  A combatant. Shelton.",
  "armless - 1. Without any arm or branch. 2. Destitute of arms or weapons.",
  "silkman - A dealer in silks; a silk mercer. Shak.",
  "fletiferous - Producing tears. [Obs.] Blount.",
  "dove-eyed - Having eyes like a dove; meekeyed; as, dove-eyed Peace.",
  "outermost - Being on the extreme external part; farthest outward; as, the outermost row. Boyle.",
  "wemless - Having no wem, or blemish; spotless. [Obs.] \"Virgin wemless.\" Chaucer.",
  "noggen - Made of hemp; hence, hard; rough; harsh. [Obs.] Johnson.",
  "hoddengray - Applied to coarse cloth made of undyed wool, formerly worn by Scotch peasants. [Scot.]",
  "enamelar - Consisting of enamel; resembling enamel; smooth; glossy. [R.] Craig.",
  "serang - The boatswain of a Lascar or East Ondian crew.",
  "transudation - 1. The act or process of transuding. 2. (Physics)  Same as Exosmose.",
  "anthracomancy - Divination by inspecting a burning coal.",
  "adunc - Hooked; as, a parrot has an adunc bill.",
  "polygynist - One who practices or advocates polygyny. H. Spenser.",
  "arteriography - A systematic description of the arteries.",
  "claggy - Adhesive; -- said of a roof in a mine to which coal clings."
]