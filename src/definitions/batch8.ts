export default [
  "plowhead - The clevis or draught iron of a plow.",
  "mistreading - Misstep; misbehavior. \"To punish my mistreadings.\" Shak.",
  "ichthyodorulite - One of the spiny plates foundon the back and tail of certain skates.",
  "nosopoetic - Producing diseases. [R.] Arbuthnot.",
  "multocular - Having many eyes, or more than two.",
  "erato - The Muse who presided over lyric and amatory poetry.",
  "engrailed - Indented with small concave curves, as the edge of a bordure, bend, or the like.",
  "macroglossia - Enlargement or hypertrophy of the tongue.",
  "tread-softly - Spurge nettle. See under Nettle.",
  "unswaddle - To take a swaddle from; to unswathe.",
  "imaret - A lodging house for Mohammedan pilgrims. Moore.",
  "quaere - Inquire; question; see; -- used to signify doubt or to suggest investigation.",
  "shorthead - A sucking whale less than one year old; -- so called by sailors.",
  "hucksterer - A huckster. Gladstone. Those hucksterers or money-jobbers. Swift.",
  "empuse - A phantom or specter. [Obs.] Jer. Taylor.",
  "hylobate - Any species of the genus Hylobates; a gibbon, or long-armed ape. See Gibbon.",
  "exequial - Of or pertaining to funerals; funereal.",
  "fovea - A slight depression or pit; a fossa.",
  "divorceless - Incapable of being divorced or separated; free from divorce.",
  "merl - The European blackbird. See Blackbird. Drayton.",
  "argentry - Silver plate or vessels. [Obs.] Bowls of frosted argentry. Howell.",
  "incorrigibleness - Incorrigibility. Dr. H. More.",
  "paleontography - The description of fossil remains.",
  "garancin - An extract of madder by sulphuric acid. It consists essentially of alizarin.",
  "drabber - One who associates with drabs; a wencher. Massinger.",
  "ceruminous - Pertaining to, or secreting, cerumen; as, the ceruminous glands.",
  "preciously - In a precious manner; expensively; extremely; dearly. Also used ironically.",
  "pigweed - A name of several annual weeds. See Goosefoot, and Lamb's- quarters.",
  "photospheric - Of or pertaining to the photosphere.",
  "unseemliness - The quality or state of being unseemly; unbecomingness. Udall.",
  "spatiate - To rove; to ramble. [Obs.] Bacon.",
  "ringlestone - The ringed dotterel, or ring plover. [Prov.Eng.]",
  "pacate - Appeased; pacified; tranquil. [R.]",
  "diminisher - One who, or that which, diminishes anything. Clerke (1637).",
  "glandiform - Having the form of a gland or nut; resembling a gland.",
  "immeasured - Immeasurable. [R.] Spenser.",
  "nonobedience - Neglect of obedience; failure to obey.",
  "moonsail - A sail sometimes carried in light winds, above a skysail. R. H. Dana, Jr.",
  "cunctator - One who delays or lingers. [R.]",
  "mollient - Serving to soften; assuaging; emollient.",
  "thible - A slice; a skimmer; a spatula; a pudding stick. [Obs. or Prov. Eng.] Ainsworth.",
  "dispauperize - To free a state of pauperism, or from paupers. J. S. Mill.",
  "somnipathy - Sleep from sympathy, or produced by mesmerism or the like. [Written also somnopathy.]",
  "organicalness - The quality or state of being organic.",
  "margaritaceous - Pertaining to, or resembling, pearl; pearly.",
  "incoexistence - The state of not coexisting. [Obs.] Locke.",
  "entrochite - A fossil joint of a crinoid stem.",
  "butylamine - A colorless liquid base, C4H9NH2, of which there are four isomeric varieties.",
  "patrocinate - To support; to patronize. [Obs.] Urquhart.",
  "heterophony - An abnormal state of the voice. Mayne.",
  "asclepiadaceous - Of, pertaining to, or resembling, plants of the Milkweed family.",
  "archetypical - Relating to an archetype; archetypal.",
  "echinulate - Set with small spines or prickles.",
  "everlastingly - In an everlasting manner.",
  "wayless - Having no road or path; pathless.",
  "disfeature - To deprive of features; to mar the features of. [R.]",
  "water opossum - See Yapock, and the Note under Opossum.",
  "knockstone - A block upon which ore is broken up.",
  "tripudiation - The act of dancing. [R.] Bacon. Carlyle.",
  "multiflue - Having many flues; as, a multiflue boiler. See Boiler.",
  "mackinaw - A thick blanket formerly in common use in the western part of the United States.",
  "rily - Roily. [Prov. Eng. & Colloq. U.S.]",
  "split key - A key split at one end like a split pin, for the same purpose.",
  "boyhood - The state of being a boy; the time during which one is a boy. Hood.",
  "williwaw - A whirlwind, or whirlwind squall, encountered in the Straits of Magellan. W. C. Russell.",
  "hydrogenide - A binary compound containing hydrogen; a hydride. [R.] See Hydride.",
  "discussional - Pertaining to discussion.",
  "presystolic - Preceding the systole or contraction of the heart; as, the presystolic friction sound.",
  "thirstiness - The state of being thirsty; thirst.",
  "agalaxy - Failure of the due secretion of milk after childbirth.",
  "cordate - Heart-shaped; as, a cordate leaf.",
  "well-meaner - One whose intention is good. \"Well-meaners think no harm.\" Dryden.",
  "orthostade - A chiton, or loose, ungirded tunic, falling in straight folds.",
  "drawbolt - A coupling pin. See under Coupling.",
  "unveracity - Want of veracity; untruthfulness; as, unveracity of heart. Carlyle.",
  "stickful - As much set type as fills a composing stick.",
  "abstractedly - In an abstracted manner; separately; with absence of mind.",
  "boutade - An outbreak; a caprice; a whim. [Obs.]",
  "fructescence - The maturing or ripening of fruit. [R.] Martyn.",
  "chloro- - A prefix denoting that chlorine is an ingredient in the substance named.",
  "cardiogram - The curve or tracing made by a cardiograph.",
  "concettism - The use of concetti or affected conceits. [R.] C. Kingsley.",
  "spot cash - Cash paid or ready for payment at once upon delivery of property purchased.",
  "nymphomania - Morbid and uncontrollable sexual desire in women, constituting a true disease.",
  "anthropotomist - One who is versed in anthropotomy, or human anatomy.",
  "inserve - To be of use to an end; to serve. [Obs.]",
  "prosal - Of or pertaining to prose; prosaic. [R.] Sir T. Browne.",
  "brockish - Beastly; brutal. [Obs.] Bale.",
  "latinist - One skilled in Latin; a Latin scholar. Cowper. He left school a good Latinist. Macaulay.",
  "jackslave - A low servant; a mean fellow. Shak.",
  "donat - A grammar. [Obs.] [Written also donet.]",
  "contagium - Contagion; contagious matter. \"Contagium of measles.\" Tyndall.",
  "papuars - The native black race of Papua or New Guinea, and the adjacent islands.",
  "macruroid - Like or pertaining to the Macrura.",
  "inexpiableness - Quality of being inexpiable.",
  "pyoxanthose - A greenish yellow crystalline coloring matter found with pyocyanin in pus.",
  "subsalt - A basic salt. See the Note under Salt.",
  "detrude - To thrust down or out; to push down with force. Locke.",
  "realmless - Destitute of a realm. Keats.",
  "blatterer - One who blatters; a babbler; a noisy, blustering boaster.",
  "megalith - A large stone; especially, a large stone used in ancient building. -- Meg`a*lith\"ic, a.",
  "schizopodous - Of or pertaining to a schizopod, or the Schizopoda.",
  "creatural - Belonging to a creature; having the qualities of a creature. [R.]",
  "semi-christianized - Half Christianized.",
  "healthsome - Wholesome; salubrious. [R.] \"Healthsome air.\" Shak.",
  "noiseful - Loud; clamorous. [Obs.] Dryden.",
  "cleverness - The quality of being clever; skill; dexterity; adroitness. Syn. -- See Ingenuity.",
  "grotto-work - Artificial and ornamental rockwork in imitation of a grotto. Cowper.",
  "acrostically - After the manner of an acrostic.",
  "doddered - Shattered; infirm. \"A laurel grew, doddered with age.\" Dryden.",
  "amenorrhoea - Retention or suppression of the menstrual discharge.",
  "princeliness - The quality of being princely; the state, manner, or dignity of a prince.",
  "driveway - A passage or way along or through which a carriage may be driven.",
  "cottonade - A somewhat stoun and thick fabric of cotton.",
  "precompose - To compose beforehand. Johnson.",
  "dreamily - As if in a dream; softly; slowly; languidly. Longfellow.",
  "hygiology - A treatise on, or the science of, the preservation of health. [R.]",
  "oenophilist - A lover of wine. [R.] Thackeray.",
  "perpotation - The act of drinking excessively; a drinking bout. [Obs.]",
  "devotement - The state of being devoted, or set apart by a vow. [R.] Bp. Hurd.",
  "impeller - One who, or that which, impels.",
  "varify - To make different; to vary; to variegate. [R.] Sylvester.",
  "aldermanship - The condition, position, or office of an alderman. Fabyan.",
  "logomachist - One who contends about words.",
  "de jure - By right; of right; by law; -- often opposed to be facto.",
  "grains - 1. See 5th Grain, n., 2 (b). 2. Pigeon's dung used in tanning. See Grainer. n., 1.",
  "maikong - A South American wild dog (Canis cancrivorus); the crab-eating dog.",
  "reassign - To assign back or again; to transfer back what has been assigned.",
  "acanthine - Of, pertaining to, or resembling, the plant acanthus.",
  "engild - To gild; to make splendent. Fair Helena, who most engilds the night. Shak.",
  "decretorily - In a decretory or definitive manner; by decree.",
  "acyl - An acid radical, as acetyl, malonyl, or benzoyl.",
  "dermatologist - One who discourses on the skin and its diseases; one versed in dermatology.",
  "sphenethmoidal - Relating to the sphenoethmoid bone; sphenoethmoid.",
  "albert ware - A soft ornamental terra-cotta pottery, sold in the biscuit state for decorating.",
  "baptizable - Capable of being baptized; fit to be baptized. Baxter.",
  "ductilimeter - An instrument for accurately determining the ductility of metals.",
  "disrank - 1. To degrade from rank. [Obs.] 2. To throw out of rank or into confusion. Decker.",
  "prestigious - Practicing tricks; juggling. [Obs.] Cotton Mather.",
  "idiocratic - Peculiar in constitution or temperament; idiosyncratic.",
  "subumbonal - Beneath or forward of the umbos of a bivalve shell.",
  "delighting - Giving delight; gladdening. -- De*light\"ing*ly, adv. Jer. Taylor.",
  "enfire - To set on fire. [Obs.] Spenser.",
  "fonde - To endeavor; to strive; to try. [Obs.] Chaucer.",
  "scillain - A glucoside extracted from squill (Scilla) as a light porous substance.",
  "hot-spirited - Having a fierly spirit; hot-headed.",
  "extradite - To deliver up by one government to another, as a fugitive from justice. See Extradition.",
  "dijudicant - One who dijudicates. [R.] Wood.",
  "unbury - To disinter; to exhume; fig., to disclose.",
  "unconfound - To free from a state of confusion, or of being confounded. Milton.",
  "nonattendance - A failure to attend; omission of attendance; nonappearance.",
  "solivagant - Wandering alone. [R.] T. Grander.",
  "septicity - Tendency to putrefaction; septic quality.",
  "polyorama - A view of many objects; also, a sort of panorama with dissolving views.",
  "wailment - Lamentation; loud weeping; wailing. [Obs.] Bp. Hacket.",
  "exiguity - Scantiness; smallness; thinness. [R.] Boyle.",
  "alligator wrench - A kind of pipe wrench having a flaring jaw with teeth on one side.",
  "dentistry - The art or profession of a dentist; dental surgery.",
  "repossession - The act or the state of possessing again.",
  "assuasive - Mitigating; tranquilizing; soothing. [R.] Music her soft assuasive voice applies. Pope.",
  "tinger - One who, or that which, tinges.",
  "tronage - A toll or duty paid for weighing wool; also, the act of weighing wool. [Obs.] Nares.",
  "consonous - Agreeing in sound; symphonious.",
  "verdoy - Charged with leaves, fruits, flowers, etc.; -- said of a border.",
  "fencible - A soldier enlisted for home service only; -- usually in the pl.",
  "self-suspicious - Suspicious or distrustful of one's self. Baxter.",
  "slidderly - Slippery. [Obs.] To a drunk man the way is slidder. Chaucer.",
  "organdie - A kind of transparent light muslin.",
  "francolite - A variety of apatite from Wheal Franco in Devonshire.",
  "nephelometer - An instrument for measuring or registering the amount of cloudiness.",
  "grecque - An ornament supposed to be of Greek origin, esp. a fret or meander,",
  "sporades - Stars not included in any constellation; -- called also informed, or unformed, stars.",
  "periganglionic - Surrounding a ganglion; as, the periganglionic glands of the frog.",
  "expeditive - Performing with speed. [Obs.] Bacon.",
  "unrig - To strip of rigging; as, to unrig a ship. Totten.",
  "grease cock - A cock or cup containing grease, to serve as a lubricator.",
  "reinvestigate - To investigate again. -- Re`in*ves`ti*ga\"tion (-g, n.",
  "vaunt-courier - See Van-courier. [Obs.] Shak.",
  "baccate - Pulpy throughout, like a berry; -- said of fruits. Gray.",
  "endenizen - To admit to the privileges of a denizen; to naturalize. [Obs.] B. Jonson.",
  "mispunctuate - To punctuate wrongly or incorrectly.",
  "ichthyolite - A fossil fish, or fragment of a fish.",
  "stooping - from Stoop. -- Stoop\"ing*ly, adv.",
  "hamose - Having the end hooked or curved.",
  "quinqueliteral - Consisting of five letters.",
  "enigmatist - One who makes, or talks in, enigmas. Addison.",
  "poristic - Of or pertaining to a porism; of the nature of a porism.",
  "virgulate - Shaped like a little twig or rod.",
  "desipient - Foolish; silly; trifling. [R.]",
  "arbitral - Of or relating to an arbiter or an arbitration. [R.]",
  "emphractic - Having the quality of closing the pores of the skin.",
  "florulent - Flowery; blossoming. [Obs.] Blount.",
  "melastomaceous - Belonging to the order of which Melastoma is the type.",
  "dubiety - Doubtfulness; uncertainty; doubt. [R.] Lamb. \"The dubiety of his fate.\" Sir W. Scott.",
  "theopneustic - Given by the inspiration of the Spirit of God.",
  "demesnial - Of or pertaining to a demesne; of the nature of a demesne.",
  "illeviable - Not leviable; incapable of being imposed, or collected. [R.] Sir M. Hale.",
  "labile - Liable to slip, err, fall, or apostatize. [Obs.] Cheyne.",
  "il- - . A form of the prefix in-, not, and in-, among. See In-.",
  "barbiton - An ancient Greek instrument resembling a lyre.",
  "hausse - A kind of graduated breech sight for a small arm, or a cannon.",
  "rescussor - One who makes an unlawful rescue; a rescuer. Burril.",
  "picra - The powder of aloes with canella, formerly officinal, employed as a cathartic.",
  "tharms - Twisted guts. [Obs. or Prov. Eng.] Ascham.",
  "allis - The European shad (Clupea vulgaris); allice shad. See Alose.",
  "ropiness - Quality of being ropy; viscosity.",
  "discomplexion - To change the complexion or hue of. [Obs.] Beau. & Fl.",
  "reechy - Smoky; reeky; hence, begrimed with dirt. [Obs.]",
  "inframarginal - Below the margin; submarginal; as, an inframarginal convolution of the brain.",
  "ichthulin - A substance from the yolk of salmon's egg.",
  "bethumb - To handle; to wear or soil by handling; as books. Poe.",
  "buat - A lantern; also, the moon. [Scot.] Sir W. Scott.",
  "damnably - 1. In a manner to incur sever 2. Odiously; detestably; excessively. [Low]",
  "audiometer - An instrument by which the power of hearing can be gauged and recorded on a scale.",
  "consanguined - Of kin blood; related. [R.] Johnson.",
  "perthiocyanogen - Same as Persulphocyanogen.",
  "untressed - Not tied up in tresses; unarranged; -- said of the hair. Chaucer.",
  "grume - A thick, viscid fluid; a clot, as of blood. Quincy.",
  "retroflexed - Reflexed; bent or turned abruptly backward.",
  "crouke - A crock; a jar. [Obs.] Chauser.",
  "excito-motor - Excitomotory; as, excito-motor power or causes.",
  "tuch - A dark-colored kind of marble; touchstone. [Obs.] Sir J. Harrington.",
  "xanthogenic - Producing a yellow color or compound; xanthic.  See Xanthic acid, under Xanthic.",
  "anticipant - Anticipating; expectant; -- with of. Wakening guilt, anticipant of hell. Southey.",
  "vesication - The process of vesicating, or of raising blisters.",
  "cousinship - The relationship of cousins; state of being cousins; cousinhood. G. Eliot.",
  "nemorous - Woody. [R.] Paradise itself was but a kind of nemorous temple. Evelyn.",
  "tralucent - Translucent. [Obs.] The air's tralucent gallery. Sir. J. Davies.",
  "boroglyceride - A compound of boric acid and glycerin, used as an antiseptic.",
  "postexist - To exist after; to live subsequently. [Obs. or R.]",
  "plagiostomatous - Same as Plagiostomous.",
  "feneration - The act of fenerating; interest. [Obs.] Sir T. Browne.",
  "tractive - Serving to draw; pulling; attracting; as, tractive power.",
  "aviado - One who works a mine with means provided by another. [Sp. Amer. & Southwestern U. S.]",
  "duelo - A duel; also, the rules of dueling. [Obs.] Shak.",
  "sun-struck - Overcome by, or affected with, sunstroke; as, sun-struck soldiers.",
  "bedtick - A tick or bag made of cloth, used for inclosing the materials of a bed.",
  "illustriousness - The state or quality of being eminent; greatness; grandeur; glory; fame.",
  "offuscation - See Obfuscate, Obfuscation. [Obs.]",
  "ripely - Maturely; at the fit time. Shak.",
  "nayward - The negative side. [R.] Howe'er you lean to the nayward. Shak.",
  "frim - Flourishing; thriving; fresh; in good case; vigorous. [Obs.] \"Frim pastures.\" Drayton.",
  "controversary - Controversial. [Obs.] Bp. Hall.",
  "tremulent - Tremulous; trembling; shaking. [R.] \" With tremulent white rod.\" Carlyle.",
  "ginhouse - A building where cotton is ginned.",
  "rudderhead - The upper end of the rudderpost, to which the tiller is attashed.",
  "wapacut - The American hawk owl. See under Hawk.",
  "unbegilt - Not gilded; hence, not rewarded with gold.",
  "percipiency - The faculty, act or power of perceiving; perception. Mrs. Browning.",
  "earnful - Full of anxiety or yearning. [Obs.] P. Fletcher.",
  "decidedly - In a decided manner; indisputably; clearly; thoroughly.",
  "maasha - An East Indian coin, of about one tenth of the weight of a rupee.",
  "siphuncular - Of or pertaining to the siphuncle.",
  "vortex filament - A vortex tube of infinitesimal cross section.",
  "ingeniousness - The quality or state of being ingenious; ingenuity.",
  "pandiculated - Extended; spread out; stretched.",
  "hesperidium - A large berry with a thick rind, as a lemon or an orange.",
  "harborage - Shelter; entertainment.[R.] Where can I get me harborage for the night Tennyson.",
  "arrhythmous - Being without rhythm or regularity, as the pulse.",
  "gutturalness - The quality of being guttural.",
  "friday - The sixth day of the week, following Thursday and preceding Saturday.",
  "chimango - A south American carrion buzzard (Milvago chimango). See Caracara.",
  "chaffing - The use of light, frivolous language by way of fun or ridicule; raillery; banter.",
  "apocopated - Shortened by apocope; as, an apocopate form.",
  "subulipalp - One of a group of carabid beetles having slender palpi.",
  "vesicovaginal - Of or pertaining to the bladder and the vagina.",
  "cascarillin - A white, crystallizable, bitter substance extracted from oil of cascarilla.",
  "escheatage - The right of succeeding to an escheat. Sherwood.",
  "reglement - Regulation. [Obs.] The reformation and reglement of usuary. Bacon.",
  "ineffably - In a manner not to be expressed in words; unspeakably. Milton.",
  "africanize - To place under the domination of Africans or negroes. [Amer.] Bartlett.",
  "corcule - The heart of the seed; the embryo or germ. [Obs.]",
  "vulnifical - Causing wounds; inflicting wounds; wounding.",
  "unsalable - Not salable; unmerchantable. -- n.  That which can not be sold. Byron.",
  "asura - An enemy of the gods, esp. one of a race of demons and giants.",
  "rearrange - To arrange again; to arrange in a different way.",
  "quadrilobate - Having four lobes; as, a quadrilobate leaf.",
  "anxiously - In an anxious manner; with painful uncertainty; solicitously.",
  "aculeous - Aculeate. [Obs.] Sir T. Browne.",
  "wavey - The snow goose. [Canadian, & Local U. S.]",
  "cledgy - Stiff, stubborn, clayey, or tenacious; as, a cledgy soil. Halliwell.",
  "lexigraphic - Of or pertaining to lexigraphy.",
  "breathableness - State of being breathable.",
  "musal - Of or pertaining to the Muses, or to Poetry. [R.]",
  "noule - The top of the head; the head or noll. [Obs.] Spenser.",
  "unmitre - To deprive of a miter; to depose or degrade from the rank of a bishop. Milton.",
  "vetoist - One who uses, or sustains the use of, the veto.",
  "scaturiginous - Abounding with springs. [Obs.]",
  "bountihead - Goodness; generosity. [Obs.] Spenser.",
  "elaidin - A solid isomeric modification of olein.",
  "hogskin - Leather tanned from a hog's skin. Also used adjectively.",
  "vadimony - A bond or pledge for appearance before a judge on a certain day. [Obs.]",
  "supernaturalness - The quality or state of being supernatural.",
  "ecclesiarch - An official of the Eastern Church, resembling a sacrist in the Western Church.",
  "genevanism - Strict Calvinism. Bp. Montagu.",
  "self-respect - Respect for one's self; regard for one's character; laudable self-esteem.",
  "thalloid - Resembling, or consisting of, thallus.",
  "igniferous - Producing fire. [R.] Blount.",
  "utterest - Uttermost. To the utterest proof of her courage. Chaucer.",
  "unyolden - Not yielded. [Obs.] \"[By] force . . . is he taken unyolden.\" Sir T. Browne.",
  "antidromous - Changing the direction in the spiral sequence of leaves on a stem.",
  "presignification - The act of signifying or showing beforehand.",
  "haziness - The quality or state of being hazy.",
  "nould - Would not. [Obs.] \"By those who nould repent.\" Fairfax.",
  "diffluent - Flowing apart or off; dissolving; not fixed. [R.] Bailey.",
  "usurary - Usurious. [Obs.] \"Usurarious contracts.\" Jer. Taylor. Bp. Hall.",
  "allodialist - One who holds allodial land.",
  "deviling - A young devil. [Obs.] Beau. & Fl.",
  "complexly - In a complex manner; not simply.",
  "lactyl - An organic residue or radical derived from lactic acid.",
  "gemarist - One versed in the Gemara, or adhering to its teachings.",
  "significantly - In a significant manner.",
  "lowh - strong imp. of Laugh. Etym: [Cf. 1st Low and 2d Lough.] Chaucer.",
  "pseudo-monocotyledonous - Having two coalescent cotyledons, as the live oak and the horse-chestnut.",
  "subepithelial - Situated under the epithelium.",
  "elucubration - See Lucubration. [Obs.] Evelyn.",
  "mugweed - A slender European weed (Galium Cruciata); -- called also crossweed.",
  "jiffy - A moment; an instant; as, I will be ready in a jiffy. [Colloq.] J. & H. Smith.",
  "hoker - Scorn; derision; abusive talk. [Obs.] -- Ho\"ker*ly, adv. [Obs.] Chaucer.",
  "insuppressive - Insuppressible. [Obs.] \"The insuppressive mettle of our spirits.\" Shak.",
  "florescence - A bursting into flower; a blossoming. Martyn.",
  "steeving - 1. The act or practice of one who steeves. 2. (Naut.)  See Steeve, n. (a).",
  "gerent - Bearing; carrying. [Obs.] Bailey.",
  "hypothenusal - Of or pertaining to hypothenuse. [R.]",
  "disaggregation - The separation of an aggregate body into its component parts.",
  "comose - Bearing a tuft of soft hairs or down, as the seeds of milkweed. Gray.",
  "sulphophosphate - A salt of sulphophosphoric acid.",
  "bez-antler - The second branch of a stag's horn.",
  "fleak - A flake; a thread or twist. [Obs.] Little long fleaks or threads of hemp. Dr. H. More.",
  "keepership - The office or position of a keeper. Carew.",
  "cemental - Of or pertaining to cement, as of a tooth; as, cemental tubes. R. Owen.",
  "pluckless - Without pluck; timid; faint-hearted.",
  "synosteosis - Union by means of bone; the complete closing up and obliteration of sutures.",
  "dilatoriness - The quality of being dilatory; lateness; slowness; tardiness; sluggishness.",
  "quadrifarious - Arranged in four rows or ranks; as, quadrifarious leaves. Loudon.",
  "perpension - Careful consideration; pondering. [Obs.] Sir T. Browne.",
  "adapter - 1. One who adapts. 2. (Chem.)  A connecting tube; an adopter.",
  "inconceptible - Inconceivable. [Obs.] Sir M. Hale.",
  "septangle - A figure which has seven angles; a heptagon. [R.]",
  "annoying - That annoys; molesting; vexatious. -- An*noy\"ing*ly, adv.",
  "vehemency - Vehemence. [R.] The vehemency of your affection. Shak.",
  "cosmometry - The art of measuring the world or the universe. Blount.",
  "pretty-spoken - Spoken or speaking prettily. [Colloq.]",
  "unthriftiness - The quality or state or being unthrifty; profuseness; lavishness. Udall.",
  "healthward - In the direction of health; as, a healthward tendency.",
  "synchronous - Happening at the same time; simultaneous. -- Syn\"chro*nous*ly, adv.",
  "dehiscent - Characterized by dehiscence; opening in some definite way, as the capsule of a plant.",
  "galoot - A noisy, swaggering, or worthless fellow; a rowdy. [Slang, U. S.]",
  "disregardful - Neglect; negligent; heedless; regardless.",
  "accomplishable - Capable of being accomplished; practicable. Carlyle.",
  "doveship - The possession of dovelike qualities, harmlessness and innocence. [Obs.] Bp. Hall.",
  "dividable - 1. Capable of being divided; divisible. 2. Divided; separated; parted. [Obs.] Shak.",
  "leafstalk - The stalk or petiole which supports a leaf.",
  "cavaliero - A cavalier; a gallant; a libertine. Shak.",
  "salpingitis - Inflammation of the salpinx.",
  "prefatorily - In a prefatory manner; by way of preface.",
  "flutterer - One who, or that which, flutters.",
  "backhouse - A building behind the main building. Specifically: A privy; a necessary.",
  "disavaunce - To retard; to repel; to do damage to. [Obs.] Chaucer.",
  "islandy - Of or pertaining to islands; full of islands. Cotgrave.",
  "tiring-room - The room or place where players dress for the stage.",
  "isiac - Pertaining to the goddess Isis; as, Isiac mysteries.",
  "froterer - One who frotes; one who rubs or chafes. [Obs.] Marston.",
  "backwash - To clean the oil from (wood) after combing.",
  "compendiate - To sum or collect together. [Obs.] Bp. King.",
  "incorruptive - Incorruptible; not liable to decay. Akenside.",
  "damnatory - Doo \"Damnatory invectives.\" Hallam.",
  "cloisteral - Cloistral. [Obs.] I. Walton.",
  "pyromorphous - Having the property of crystallizing by the agency of fire.",
  "trochilics - The science of rotary motion, or of wheel work. Wilkins.",
  "self-assured - Assured by or of one's self; self-reliant; complacent.",
  "bumbailiff - See Bound bailiff, under Bound, a.",
  "eddoes - The tubers of Colocasia antiquorum. See Taro.",
  "unwrinkle - To reduce from a wrinkled state; to smooth.",
  "travailous - Causing travail; laborious. [Obs.] Wyclif. -- Trav\"ail*ous*ly, adv. [Obs.] Wyclif.",
  "dictamen - A dictation or dictate. [R.] Falkland.",
  "madoqua - A small Abyssinian antelope (Neotragus Saltiana), about the size of a hare.",
  "paladin - A knight-errant; a distinguished champion; as, the paladins of Charlemagne. Sir W. Scott.",
  "rubescence - The quality or state of being rubescent; a reddening; a flush.",
  "vaporing - Talking idly; boasting; vaunting. -- Va\"por*ing*ly, adv.",
  "kerchiefed - Dressed; hooded; covered; wearing a kerchief. Milton.",
  "plushy - Like plush; soft and shaggy. H. Kingsley.",
  "self-assumed - Assumed by one's own act, or without authority.",
  "deplant - To take up (plants); to transplant. [R.]",
  "joculator - A jester; a joker. [Obs.] Strutt.",
  "nourisher - One who, or that which, nourishes. Milton.",
  "epipetalous - Borne on the petals or corolla.",
  "trackmaster - One who has charge of the track; --called also roadmaster.",
  "enwind - To wind about; to encircle. In the circle of his arms Enwound us both. Tennyson.",
  "bayadere - A female dancer in the East Indies. [Written also bajadere.]",
  "insulsity - Insipidity; stupidity; dullness. [Obs.] The insulsity of mortal tongues. Milton.",
  "bicarinate - Having two keel-like projections, as the upper palea of grasses.",
  "parhelic - Of or pertaining to parhelia.",
  "classicalist - One who adheres to what he thinks the classical canons of art. Ruskin.",
  "angeiotomy - Same as Angiology, Angiotomy, etc.",
  "bequeathment - The act of bequeathing, or the state of being bequeathed; a bequest.",
  "sheriffship - The office or jurisdiction of sheriff. See Shrievalty.",
  "dumfounder - To dumfound; to confound. [Written also dumbfounder.]",
  "dinar - 1. A petty money of accounts of Persia. 2. An ancient gold coin of the East.",
  "indemonstrability - The quality of being indemonstrable.",
  "mirabilite - Native sodium sulphate; Glauber's salt.",
  "puissantly - In a puissant manner; powerfully; with great strength.",
  "higher-up - A superior officer or official; -- used chiefly in pl. [Slang]",
  "pea-jacket - A thick loose woolen jacket, or coat, much worn by sailors in cold weather.",
  "breech-loading - Receiving the charge at the breech instead of at the muzzle.",
  "obsigillation - A sealing up. [Obs.] Maunder.",
  "strigate - Having transverse bands of color.",
  "differingly - In a differing or different manner. Boyle.",
  "photomagnetism - The branch of science which treats of the relation of magnetism to light.",
  "resentful - Inclined to resent; easily provoked to anger; irritable. -- Re*sent\"ful*ly, adv.",
  "troupe - A company or troop, especially the company pf performers in a play or an opera.",
  "discodactylia - A division of amphibians having suctorial disks on the toes, as the tree frogs.",
  "archipelagic - Of or pertaining to an archipelago.",
  "theistic - Of or pertaining to theism, or a theist; according to the doctrine of theists.",
  "octosyllabic - Consisting of or containing eight syllables.",
  "manovery - A contrivance or maneuvering to catch game illegally.",
  "ulnage - Measurement by the ell; alnage.",
  "methodization - The act or process of methodizing, or the state of being methodized.",
  "do-nothingism - Inactivity; habitual sloth; idleness. [Jocular] Carlyle. Miss Austen.",
  "figurial - Represented by figure or delineation. [R.] Craig.",
  "gruelly - Like gruel; of the consistence of gruel.",
  "vanquishment - The act of vanquishing, or the state of being vanquished. Bp. Hall.",
  "talmudistic - Resembling the Talmud; Talmudic.",
  "implate - To cover with plates; to sheathe; as, to implate a ship with iron.",
  "brothelry - Lewdness; obscenity; a brothel. B. Jonson.",
  "aerogun - A cannon capable of being trained at very high angles for use against aircraft.",
  "czarevna - The title of the wife of the czarowitz.",
  "unbecome - To misbecome. [Obs.] Bp. Sherlock.",
  "biotaxy - The classification of living organisms according to their structural character; taxonomy.",
  "inviolableness - The quality or state of being inviolable; as, the inviolableness of divine justice.",
  "reservist - A member of a reserve force of soldiers or militia. [Eng.]",
  "tranquilly - In a tranquil manner; calmly.",
  "catamenial - Pertaining to the catamenia, or menstrual discharges.",
  "tasco - A kind of clay for making melting pots. Percy Smith.",
  "backband - The band which passes over the back of a horse and holds up the shafts of a carriage.",
  "twiner - Any plant which twines about a support.",
  "edituate - To guard as a churchwarden does. [Obs.] J. Gregory.",
  "caroched - Placed in a caroche. [Obs.] Beggary rides caroched. Massenger.",
  "vaulter - One who vaults; a leaper; a tumbler. B. Jonson.",
  "myo- - A combining form of Gr. muscle; as, myograph, myochrome.",
  "chondrogenesis - The development of cartilage.",
  "fitchet - The European polecat (Putorius foetidus). See Polecat.",
  "reforestization - The act or process of reforestizing.",
  "squab-chick - A young chicken before it is fully fledged. [Prov. Eng.]",
  "insuitable - Unsuitable. [Obs.] -- In*suit`a*bil\"i*ty, n. [Obs.]",
  "preaudience - Precedence of rank at the bar among lawyers. Blackstone.",
  "mothery - Consisting of, containing, or resembling, mother (in vinegar).",
  "melam - A white or buff-colored granular powder,",
  "unartful - Lacking art or skill; artless. Congreve. -- Un*art\"ful*ly, adv. Swift. Burke.",
  "curation - Cure; healing. [Obs.] Chaucer.",
  "verdancy - The quality or state of being verdant.",
  "ferrumination - The soldering ir uniting of me [R.] Coleridge.",
  "counterwait - To wait or watch for; to be on guard against. [Obs.] Chaucer.",
  "alphabetism - The expression of spoken sounds by an alphabet. Encyc. Brit.",
  "philologize - To study, or make critical comments on, language. Evelyn.",
  "crimeless - Free from crime; innocent. Shak.",
  "catelectrotonic - Relating to, or characterized by, catelectrotonus.",
  "cc ira - The refrain of a famous song of the French Revolution.",
  "unappalled - Not appalled; not frightened; dauntless; undaunted. Milton.",
  "sarcelle - The old squaw, or long-tailed duck.",
  "topazolite - A topaz-yellow variety of garnet.",
  "runcinate - Pinnately cut with the lobes pointing downwards, as the leaf of the dandelion.",
  "stoichiometrical - Of or pertaining to stoichiometry; employed in, or obtained by, stoichiometry.",
  "protosulphuret - A protosulphide. [Obs.]",
  "multiloquent - Speaking much; very talkative; loquacious.",
  "intelligibleness - The quality or state of being intelligible; intelligibility. Locke.",
  "mischarge - To charge erroneously, as in account. -- n.  A mistake in charging.",
  "hyalography - Art of writing or engraving on glass.",
  "reinette - A name given to many different kinds of apples, mostly of French origin.",
  "inflictive - Causing infliction; acting as an infliction. Whitehead.",
  "unspeak - To retract, as what has been spoken; to recant; to unsay. [R.] Shak.",
  "declinate - Bent downward or aside; (Bot.) bending downward in a curve; declined.",
  "empyrical - Containing the combustible principle of coal. Kirwan.",
  "fractionate - To separate into different portions or fractions, as in the distillation of liquids.",
  "monogynian - Pertaining to the Monogynia; monogynous. -- n.  One of the Monogynia.",
  "dilection - Love; choice. [Obs.] T. Martin.",
  "tanyard - An inclosure where the tanning of leather is carried on; a tannery.",
  "sulphuret - A sulphide; as, a sulphuret of potassium. [Obsoles.]",
  "metathetic - Of or pertaining to metathesis.",
  "quittal - Return; requital; quittance. [Obs.]",
  "melolonthidian - A beetle of the genus Melolontha, and allied genera. See May beetle, under May.",
  "solver - One who, or that which, solves.",
  "condensible - Capable of being condensed; as, a gas condensible to a liquid by cold.",
  "ampullate - Having an ampulla; flask-shaped; bellied.",
  "plitt - An instrument of punishment or torture resembling the knout, used in Russia.",
  "agasp - In a state of gasping. Coleridge.",
  "thio- - A combining form (also used adjectively) denoting the presence of sulphur. See Sulpho-.",
  "academicals - The articles of dress prescribed and worn at some colleges and universities.",
  "embolismal - Pertaining to embolism; intercalary; as, embolismal months.",
  "acrolithan - Pertaining to, or like, an acrolith.",
  "javelinier - A soldier armed with a javelin. Holland.",
  "eclectically - In an eclectic manner; by an eclectic method.",
  "limitedness - The quality of being limited.",
  "gymnosomata - One of the orders of Pteropoda. They have no shell.",
  "phytoid - Resembling a plant; plantlike.",
  "morigerous - Obedient; obsequious. [Obs.] Brathwait.",
  "chelonian - Of or pertaining to animals of the tortoise kind. -- n.  One of the Chelonia.",
  "cazique - A chief or petty king among some tribes of Indians in America.",
  "oxheart - A large heart-shaped cherry, either black, red, or white.",
  "seclusive - Tending to seclude; keeping in seclusion; secluding; sequestering.",
  "maudle - To throw onto confusion or disorder; to render maudlin. [Obs.]",
  "web-toed - Having the toes united by a web for a considerable part of their length.",
  "confirmative - Tending confirm or establish. Sherwood. -- Con*firm\"a*tive*ly, adv.",
  "bicaudal - Having, or terminating in, two tails.",
  "passager - A passenger; a bird or boat of passage. [Obs.] Ld. Berners.",
  "goltschut - 1. A small ingot of gold. 2. A silver ingot, used in Japan as money.",
  "adjutrix - A female helper or assistant. [R.]",
  "saucepan - A small pan with a handle, in which sauce is prepared over a fire; a stewpan.",
  "perduration - Long continuance. [Archaic]",
  "windstorm - A storm characterized by high wind with little or no rain.",
  "euripize - To whirl hither and thither. [Obs.]",
  "pennate - 1. Winged; plume-shaped. 2. (Bot.)  Same as Pinnate.",
  "disarticulator - One who disarticulates and prepares skeletons.",
  "outplay - To excel or defeat in a game; to play better than; as, to be outplayed in tennis or ball.",
  "heavy-headed - Dull; stupid. \"Gross heavy-headed fellows.\" Beau. & Fl.",
  "mouldery - Covered or filled with mold; consisting of, or resembling, mold.",
  "decompoundable - Capable of being decompounded.",
  "subpleural - Situated under the pleural membrane.",
  "overjump - To jump over; hence, to omit; to ignore. Marston.",
  "graphically - In a graphic manner; vividly.",
  "tristigmatic - Having, or consisting of, three stigmas. Gray.",
  "horrendous - Fearful; frightful. [Obs.] I. Watts.",
  "redtail - (a) The red-tailed hawk. (b) The European redstart.",
  "readjustment - A second adjustment; a new or different adjustment.",
  "douar - A village composed of Arab tents arranged in streets.",
  "fructed - Bearing fruit; -- said of a tree or plant so represented upon an escutcheon. Cussans.",
  "marcasitic - Containing, or having the nature of, marcasite.",
  "absinthiate - To impregnate with wormwood.",
  "boxthorn - A plant of the genus Lycium, esp. Lycium barbarum.",
  "stoloniferous - Producing stolons; putting forth suckers.",
  "commaterial - Consisting of the same material. [Obs.] Bacon.",
  "invalidness - Invalidity; as, the invalidness of reasoning.",
  "thermoelectricity - Electricity developed in the action of heat. See the Note under Electricity.",
  "pentangular - Having five corners or angles. [R.]",
  "leucopathy - The state of an albino, or of a white child of black parents.",
  "self-perplexed - Perplexed by doubts originating in one's own mind.",
  "azoturia - Excess of urea or other nitrogenous substances in the urine.",
  "nez perces - A tribe of Indians, mostly inhabiting Idaho.",
  "dichastic - Capable of subdividing spontaneously.",
  "dagoba - A dome-shaped structure built over relics of Buddha or some Buddhist saint. [East Indies]",
  "treachetour - A traitor. [Obs.] \"Treachour full of false despite.\" Spenser.",
  "naphthene - A peculiar hydrocarbon occuring as an ingredient of Caucasian petroleum.",
  "ammoniated - Combined or impregnated with ammonia.",
  "mendelian - Pert. to Mendel, or to Mendel's law. -- Men*de\"li*an*ism (#), Men*del\"ism (#), n.",
  "nonruminant - Not ruminating; as, a nonruminant animal.",
  "algerian - Of or pertaining to Algeria. -- n.  A native of Algeria.",
  "ophthalmologist - One skilled in ophthalmology; an oculist.",
  "sea foam - 1. Foam of sea water. 2. (Min.)  Meerschaum; -- called also sea froth.",
  "bergstock - A long pole with a spike at the end, used in climbing mountains; an alpenstock.",
  "urostege - One of the plates on the under side of the tail of a serpent.",
  "casuistical - Of or pertaining to casuists or casuistry.",
  "antimonarchic - Opposed to monarchial government. Bp. Benson. Addison.",
  "melanochroic - Having a dark complexion; of or pertaining to the Melanochroi.",
  "metasome - One of the component segments of the body of an animal.",
  "legist - One skilled in the laws; a writer on law. Milman. J. Morley.",
  "suiting - Among tailors, cloth suitable for making entire suits of clothes.",
  "thanedom - The property or jurisdiction of a thane; thanage. Sir W. Scott.",
  "-ock - A suffix used to form diminutives; as, bullock, hillock.",
  "sea mat - Any bryozoan of the genus Flustra or allied genera which form frondlike corals.",
  "perverseness - The quality or state of being perverse. \"Virtue hath some perverseness.\" Donne.",
  "wealthful - Full of wealth; wealthy; prosperous. [R.] Sir T. More. -- Wealth\"ful*ly, adv. [R.]",
  "otolitic - Of or pertaining to otoliths.",
  "september - The ninth month of the year, containing thurty days.",
  "brachycephalism - The state or condition of being brachycephalic; shortness of head.",
  "crateriform - Having the form of a shallow bowl; -- said of a corolla.",
  "feticide - The act of killing the fetus in the womb; the offense of procuring an abortion.",
  "unburiable - Not ready or not proper to be buried. Tennyson.",
  "susception - The act of taking; reception.",
  "obedienciary - One yielding obedience. [Obs.] Foxe.",
  "craniometrical - Pertaining to craniometry.",
  "extravagantness - The state of being extravagant or in excess; excess; extravagance.",
  "unthinker - A person who does not think, or does not think wisely.",
  "cresylic - Pertaining to, or derived from, cresol, creosote, etc. Cresylic acid. (Chem.) See Cresol.",
  "poor-spirited - Of a mean spirit; cowardly; base. -- Poor\"-spir`it*ed*ness, n.",
  "poorliness - The quality or state of being poorly; ill health.",
  "ineffectiveness - Quality of being ineffective.",
  "nummary - Of or relating to coins or money.",
  "antitypy - Opposition or resistance of matter to force. [R.] Sir W. Hamilton.",
  "basaltoid - Formed like basalt; basaltiform.",
  "immitigable - Not capable of being mitigated, softened, or appeased. Coleridge.",
  "laterality - The state or condition of being lateral.",
  "umbrel - An umbrella. [Obs. or Colloq.] Each of them besides bore their umbrels. Shelton.",
  "haustorium - One of the suckerlike rootlets of such plants as the dodder and ivy. R. Brown.",
  "recaptor - One who recaptures; one who takes a prize which had been previously taken.",
  "lurchline - The line by which a fowling net was pulled over so as to inclose the birds.",
  "regularly - In a regular manner; in uniform order; methodically; in due order or time.",
  "underconduct - A lower conduit; a subterranean conduit. [Obs.] Sir H. Wotton.",
  "aboriginality - The quality of being aboriginal. Westm. Rev.",
  "invulnerability - Quality or state of being invulnerable.",
  "peristomial - Of or pertaining to a peristome.",
  "chylify - To make chyle of; to be converted into chyle.",
  "sea-ear - Any species of ear-shaped shells of the genus Haliotis. See Abalone.",
  "menticultural - Of or pertaining to mental culture; serving to improve or strengthen the mind. [R.]",
  "bagpiper - One who plays on a bagpipe; a piper. Shak.",
  "grahamite - One who follows the dietetic system of Graham. [U. S.]",
  "palely - In a pale manner; dimly; wanly; not freshly or ruddily. Thackeray.",
  "nyctalops - One afflicted with nyctalopia.",
  "commonition - Advice; warning; instruction. [Obs.] Bailey.",
  "threpe - To call; to term. [Obs.] \"Luna silver we threpe.\" Chaucer.",
  "babillard - The lesser whitethroat of Europe; -- called also babbling warbler.",
  "underbred - Not thoroughly bred; ill-bred; as, an underbred fellow. Goldsmith.",
  "graffage - The scarp of a ditch or moat. \"To clean the graffages.\" Miss Mitford.",
  "timbreled - Sung to the sound of the timbrel. \"In vain with timbreled anthems dark.\" Milton.",
  "disturbation - Act of disturbing; disturbance. [Obs.] Daniel.",
  "atheology - Antagonism to theology. Swift.",
  "manurement - Cultivation. [Obs.] W. Wotton.",
  "populicide - Slaughter of the people. [R.]",
  "sub - A subordinate; a subaltern. [Colloq.]",
  "burdon - A pilgrim's staff. [Written also burden.] Rom. of R.",
  "ir- - . A form of the prefix in-. See In-.",
  "decore - To decorate; to beautify. [Obs.] To decore and beautify the house of God. E. Hall.",
  "gentilly - In a gentle or hoble manner; frankly. [Obs.] Chaucer.",
  "triquetrum - One of the bones of the carpus; the cuneiform. See Cuneiform (b).",
  "stabbingly - By stabbing; with intent to injure covertly. Bp. Parker.",
  "compotator - One who drinks with another. [R.] Pope.",
  "fleshment - The act of fleshing, or the excitement attending a successful beginning. [R.] Shak.",
  "juncous - Full of rushes: resembling rushes; juncaceous. [R.] Johnson.",
  "pellibranchiata - A division of Nudibranchiata, in which the mantle itself serves as a gill.",
  "fomenter - One who foments; one who encourages or instigates; as, a fomenter of sedition.",
  "unprison - To take or deliver from prison.",
  "dispand - To spread out; to expand. [Obs.] Bailey.",
  "smallness - The quality or state of being small.",
  "olifant - 1. An elephant. [Obs.] 2. An ancient horn, made of ivory.",
  "trappean - Of or pertaining to trap; being of the nature of trap.",
  "ricker - A stout pole for use in making a rick, or for a spar to a boat.",
  "perforated - Pierced with a hole or holes, or with pores; having transparent dots resembling holes.",
  "spoonwood - The mountain laurel (Kalmia latifolia).",
  "letterless - 1. Not having a letter. 2. Illiterate. [Obs.] E. Waterhouse.",
  "dairywoman - A woman who attends to a dairy.",
  "castorin - A white crystalline substance obtained from castoreum.",
  "preteritive - Used only or chiefly in the preterit or past tenses, as certain verbs.",
  "gode-year - The venereal disease; -- often used as a mild oath. [Obs.] Shak.",
  "aggregative - 1. Taken together; collective. 2.  Gregarious; social. [R.] Carlyle.",
  "hippocras - A cordial made of spiced wine, etc.",
  "-grave - A final syllable signifying a ruler, as in landgrave, margrave. See Margrave.",
  "araceous - Of or pertaining to an order of plants, of which the genus Arum is the type.",
  "retroflex - Reflexed; bent or turned abruptly backward.",
  "onomancy - Divination by the letters of a name; nomancy. [R.] Camden.",
  "incuss - To form, or mold, by striking or stamping, as a coin or medal.",
  "spodomantic - Relating to spodomancy, or divination by means of ashes. C. Kingsley.",
  "gastrocolic - Pertaining to both the stomach and the colon; as, the gastrocolic, or great, omentum.",
  "turnbroach - A turnspit. [Obs.] \" One that was her turnbroach.\" Beau. & Fl.",
  "tiffish - Inclined to tiffs; peevish; petulant.",
  "entreater - One who entreats; one who asks earnestly; a beseecher.",
  "misimprovement - Ill use or employment; use for a bad purpose.",
  "sekes - A place in a pagan temple in which the images of the deities were inclosed.",
  "infucation - The act of painting or staining, especially of painting the face.",
  "choiceful - Making choices; fickle. [Obs.] His choiceful sense with every change doth fit. Spenser.",
  "rancidity - The quality or state of being rancid; a rancid scent or flavor, as of old oil. Ure.",
  "crystallometry - The art of measuring crystals.",
  "tramper - One who tramps; a stroller; a vagrant or vagabond; a tramp. Dickens.",
  "defluous - Flowing down; falling off. [Obs.] Bailey.",
  "deadhouse - A morgue; a place for the temporary reception and exposure of dead bodies.",
  "larixinic - Of, or derived from, the larch (Larix); as, larixinic acid.",
  "contiguate - Contiguous; touching. [Obs.] Holland.",
  "otherguise - Of another kind or sort; in another way. \"Otherguess arguments.\" Berkeley.",
  "filoplumaceous - Having the structure of a filoplume.",
  "overweener - One who overweens. [R.] The conceits of warmed or overweening brain. Locke.",
  "eutychianism - The doctrine of Eutyches and his followers.",
  "bedkey - An instrument for tightening the parts of a bedstead.",
  "revolutionist - One engaged in effecting a change of government; a favorer of revolution. Burke.",
  "graphicalness - The quality or state of being graphic.",
  "gaviae - The division of birds which includes the gulls and terns.",
  "cerebellar - Pertaining to the cerebellum.",
  "diluviate - To run as a flood. [Obs.] Sir E. Sandys.",
  "fanged - Having fangs or tusks; as, a fanged adder. Also used figuratively.",
  "fesels - See Phasel. [Obs.] May (Georgics).",
  "wapp - (a) A fair-leader. (b) A rope with wall knots in it with which the shrouds are set taut.",
  "recluseness - Quality or state of being recluse.",
  "distributional - Of or pertaining to distribution. Huxley.",
  "neese - To sneeze. [Obs.] [Written also neeze.]",
  "shooty - Sprouting or coming up freely and regularly. [Prev. Eng.] Grose.",
  "edentation - A depriving of teeth. [R.] Cockeram.",
  "vespertilionine - Of or pertaining to the Vespertiliones.",
  "upclimb - To climb up; to ascend. Upclomb the shadowy pine above the woven copse. Tennyson.",
  "vesiculation - The state of containing vesicles, or the process by which vesicles are formed.",
  "binny - A large species of barbel (Barbus bynni), found in the Nile, and much esteemed for food.",
  "rheeboc - The peele. [Written also reebok.]",
  "haemadromograph - An instrument for registering the velocity of the blood.",
  "kleptomaniac - A person affected with kleptomania.",
  "rabble-rout - A tumultuous crowd; a rabble; a noisy throng.",
  "preluder - One who, or that which, preludes; one who plays a prelude. Mason.",
  "allylene - A gaseous hydrocarbon, C3H4, homologous with acetylene; propine. CH3.C.CH",
  "irrepresentable - Not capable of being represented or portrayed.",
  "jobbernowl - A blockhead. [Colloq. & Obs.] H. Taylor.",
  "reticent - Inclined to keep silent; reserved; uncommunicative.",
  "multiloquous - Speaking much; very talkative; loquacious.",
  "aphasic - Pertaining to, or affected by, aphasia; speechless.",
  "bisectrix - The line bisecting the angle between the optic axes of a biaxial crystal.",
  "didactyl - An animal having only two digits.",
  "allomerism - Variability in chemical constitution without variation in crystalline form.",
  "tuberculous - Having tubercles; affected with, or characterized by, tubercles; tubercular.",
  "atropism - A condition of the system produced by long use of belladonna.",
  "anxietude - The state of being anxious; anxiety. [R.]",
  "self-binder - A reaping machine containing mechanism for binding the grain into sheaves.",
  "mayoral - The conductir of a mule team; also, a head shepherd.",
  "derival - Derivation. [R.] The derival of e from a. Earle.",
  "bicyclism - The art of riding a bicycle.",
  "metalogical - Beyond the scope or province of logic.",
  "metathetical - Of or pertaining to metathesis.",
  "stathmograph - A contrivance for recording the speed of a railway train. Knight.",
  "desparple - To scatter; to disparkle. [Obs.] Mandeville.",
  "semiproof - Half proof; evidence from the testimony of a single witness. [Obs.] Bailey.",
  "napoleonist - A supporter of the dynasty of the Napoleons.",
  "outsettler - One who settles at a distance, or away, from others.",
  "locustella - The European cricket warbler.",
  "anapaestic - Same as Anapest, Anapestic.",
  "kamtschadales - An aboriginal tribe inhabiting the southern part of Kamtschatka.",
  "disparition - Act of disappearing; disappearance. [Obs.] Bp. Hall.",
  "premeditation - The act of meditating or contriving beforehand; previous deliberation; forethought.",
  "bacterial - Of or pertaining to bacteria.",
  "odontograph - An instrument for marking or laying off the outlines of teeth of gear wheels.",
  "zoroastrism - Same as Zoroastrianism. Tylor.",
  "nonacquaintance - Want of acquaintance; the state of being unacquainted.",
  "provexity - Great advance in age. [Obs.]",
  "executer - One who performs or carries into effect. See Executor.",
  "sportulary - Subsisting on alms or charitable contributions. [Obs.] Bp. Hall.",
  "unsured - Not made sure. [Obs.] Thy now unsured assurance to the crown. Shak.",
  "almandine - The common red variety of garnet.",
  "decagram - A weight of the metric system; ten grams, equal to about 154.32 grains avoirdupois.",
  "hithermost - Nearest on this side. Sir M. Hale.",
  "brankursine - Bear's-breech, or Acanthus.",
  "spanpiece - The collar of a roof; sparpiece.",
  "darkening - Twilight; gloaming. [Prov. Eng. & Scot.] Wright.",
  "aplacentata - Mammals which have no placenta.",
  "priestism - The influence, doctrines, principles, etc., of priests or the priesthood. [R.]",
  "inhalent - Used for inhaling; as, the inhalent end of a duct. Dana.",
  "pulmobranchiate - Same as Pulmonibranchiata, -ate.",
  "wealsman - A statesman; a politician. [R.] Shak.",
  "adesmy - The division or defective coherence of an organ that is usually entire.",
  "ural - Pertaining to, or designating, the Urals, a mountain range between Europe and Asia.",
  "sonnish - Like the sun; sunny; golden. [Obs.] \"Her sonnish hairs.\" Chaucer.",
  "peracute - Very sharp; very violent; as, a peracute fever. [R.] Harvey.",
  "acromegaly - Chronic enlargement of the extremities and face.",
  "laminability - The quality or state of being laminable.",
  "ruffianage - Ruffians, collectively; a body of ruffians. \"The vilest ruffianage.\" Sir F. Palgrave.",
  "eyasmusket - An unfledged or young male sparrow hawk. [Obs.] Shak.",
  "unfurnish - To strip of furniture; to divest; to strip.",
  "mahometanize - To convert to the religion of Mohammed; to Mohammedanize.",
  "peeress - The wife of a peer; a woman ennobled in her own right, or by right of marriage.",
  "dexterously - In a dexterous manner; skillfully.",
  "immethodize - To render immethodical; to destroy the method of; to confuse. [R.]",
  "blushy - Like a blush; having the color of a blush; rosy. [R.] \"A blushy color.\" Harvey.",
  "unqueen - To divest of the rank or authority of queen. Shak.",
  "embryogony - The formation of an embryo.",
  "aerology - That department of physics which treats of the atmosphere.",
  "mesocuneiform - One of the bones of the tarsus. See 2d Cuneiform.",
  "alinasal - Pertaining to expansions of the nasal bone or cartilage.",
  "presension - Previous perception. [Obs.] Sir T. Browne.",
  "insurant - The person insured. Champness.",
  "self-abhorrence - Abhorrence of one's self.",
  "refluency - The quality of being refluent; a flowing back.",
  "a-tiptoe - On tiptoe; eagerly expecting. We all feel a-tiptoe with hope and confidence. F. Harrison.",
  "exscutellate - Without, or apparently without, a scutellum; -- said of certain insects.",
  "bacchanalianism - The practice of bacchanalians; bacchanals; drunken revelry.",
  "primevally - In a primeval manner; in or from the earliest times; originally. Darwin.",
  "strapper - 1. One who uses strap. 2. A person or thing of uncommon size. [Colloq.]",
  "bowbell - One born within hearing distance of Bow-bells; a cockney. Halliwell.",
  "matcher - One who, or that which, matches; a matching machine. See under 3d Match.",
  "pyrogallic - Pertaining to, derived from, or designating, an acid called pyrogallol. See Pyrogallol.",
  "unbarricadoed - Not obstructed by barricades; open; as, unbarricadoed streets. Burke.",
  "evangelistary - A selection of passages from the Gospels, as a lesson in divine service. Porson.",
  "formulize - To reduce to a formula; to formulate. Emerson.",
  "suroxidate - To combine with oxygen so as to form a suroxide or peroxide. [Obs.]",
  "embowl - To form like a bowl; to give a globular shape to. [Obs.] Sir P. Sidney.",
  "anthropophagite - A cannibal. W. Taylor.",
  "panurgy - Skill in all kinds of work or business; craft. [R.] Bailey.",
  "cyphonautes - The free-swimming, bivalve larva of certain Bryozoa.",
  "indo-english - Of or relating to the English who are born or reside in India; Anglo-Indian.",
  "papistry - The doctrine and ceremonies of the Church of Rome; popery. [R.] Whitgift.",
  "eulogize - To speak or write in commendation of (another); to extol in speech or writing; to praise.",
  "refreyd - To chill; to cool. [Obs.] Refreyded by sickness . . . or by cold drinks. Chaucer.",
  "pinax - A tablet; a register; hence, a list or scheme inscribed on a tablet. [R.] Sir T. Browne.",
  "postoblongata - The posterior part of the medulla oblongata. B. G. Wilder.",
  "theophilanthropy - Theophilanthropism. Macaulay.",
  "bipedal - 1. Having two feet; biped. 2. Pertaining to a biped.",
  "carneous - Consisting of, or like, flesh; carnous; fleshy. \"Carneous fibers.\" Ray.",
  "accipitrine - Like or belonging to the Accipitres; raptorial; hawklike.",
  "protoconch - The embryonic shell, or first chamber, of ammonites and other cephalopods.",
  "homelyn - The European sand ray (Raia maculata); -- called also home, mirror ray, and rough ray.",
  "zinnwaldite - A kind of mica containing lithium, often associated with tin ore.",
  "verruciform - Shaped like a wart or warts.",
  "half-bound - Having only the back and corners in leather, as a book.",
  "forehew - To hew or cut in front. [Obs.] Sackville.",
  "recommendatory - Serving to recommend; recommending; commendatory. Swift.",
  "entotic - Pertaining to the interior of the ear.",
  "pensiveness - The state of being pensive; serious thoughtfulness; seriousness. Hooker.",
  "dendrolite - A petrified or fossil shrub, plant, or part of a plant.",
  "verbarium - A game in word making. See Logomachy, 2.",
  "brand goose - A species of wild goose (Branta bernicla) usually called in America brant. See Brant.",
  "mountebankish - Like a mountebank or his quackery. Howell.",
  "personize - To personify. [R.] Milton has personized them. J. Richardson.",
  "otherguess - Of another kind or sort; in another way. \"Otherguess arguments.\" Berkeley.",
  "wittingly - Knowingly; with knowledge; by design.",
  "polewig - The European spotted goby (Gobius minutus); -- called also pollybait. [Prov. Eng.]",
  "stoechiology - See Stoichiology, Stoichiometry, etc.",
  "lusterless - Destitute of luster; dim; dull.",
  "hewe - A domestic servant; a retainer. [Obs.] \"False homely hewe.\" Chaucer.",
  "deducement - Inference; deduction; thing deduced. [R.] Dryden.",
  "cantabrian - Of or pertaining to Cantabria on the Bay of Biscay in Spain.",
  "sectiuncle - A little or petty sect. [R.] \"Some new sect or sectiuncle.\" J. Martineau.",
  "ballproof - Incapable of being penetrated by balls from firearms.",
  "oxycalcium - Of or pertaining to oxygen and calcium; as, the oxycalcium light. See Drummond light.",
  "sanitarium - A health station or retreat; a sanatorium. \"A sanitarium for troops.\" L. Oliphant.",
  "balneary - A bathing room. Sir T. Browne.",
  "macrophyllous - Having long or large leaves.",
  "predictional - Prophetic; prognostic. [R.]",
  "martagon - A lily (Lilium Martagon) with purplish red flowers, found in Europe and Asia.",
  "guilt-sick - Made sick by consciousness of guilt. \"A guilt-sick conscience.\" Beau. c& El.",
  "diathermancy - The property of transmitting radiant heat; the quality of being diathermous. Melloni.",
  "preachify - To discourse in the manner of a preacher. [Colloq.] Thackeray.",
  "recalcitrant - Kicking back; recalcitrating; hence, showing repugnance or opposition; refractory.",
  "bousy - Drunken; sotted; boozy. In his cups the bousy poet songs. Dryden.",
  "refrenation - The act of refraining. [Obs.]",
  "self-activity - The quality or state of being self-active; self-action.",
  "splanchnic - Of or pertaining to the viscera; visceral.",
  "misinterpretation - The act of interpreting erroneously; a mistaken interpretation.",
  "findfaulting - Apt to censure or cavil; faultfinding; captious. [Obs.] Whitlock.",
  "paridigitate - Having an evennumber of digits on the hands or the feet. Qwen.",
  "influencive - Tending toinfluence; influential.",
  "catallactics - The science of exchanges, a branch of political economy.",
  "disproperty - To cause to be no longer property; to dispossess of. [R.] Shak.",
  "churchlike - Befitting a church or a churchman; becoming to a clergyman. Shak.",
  "homocerebrin - A body similar to, or identical with, cerebrin.",
  "sneakiness - The quality of being sneaky.",
  "inheritability - The quality of being inheritable or descendible to heirs. Jefferson.",
  "hurden - A coarse kind of linen; -- called also harden. [Prov. Eng.]",
  "intertropical - Situated between or within the tropics. J. Morse.",
  "drupel - A small drupe, as one of the pulpy grains of the blackberry.",
  "romanism - The tenets of the Church of Rome; the Roman Catholic religion.",
  "semiopaque - Half opaque; only half transparent.",
  "draggle-tail - A slattern who suffers her gown to trail in the mire; a drabble-tail.",
  "epitrochlea - A projection on the outer side of the distal end of the humerus; the external condyle.",
  "water qualm - See Water brash, under Brash.",
  "misseem - 1. To make a false appearance. [Obs.] 2. To misbecome; to be misbecoming. [Obs.] Spenser.",
  "increst - To adorn with a crest. [R.] Drummond.",
  "panoramical - Of, pertaining to, or like, a panorama. Panoramic camera. See under Camera.",
  "pontlevis - The action of a horse in rearing repeatedly and dangerously.",
  "excretive - Having the power of excreting, or promoting excretion. Harvey.",
  "methylated - Impregnated with, or containing, methyl alcohol or wood spirit; as, methylated spirits.",
  "hypophosphite - A salt of hypophosphorous acid.",
  "bedabble - To dabble; to sprinkle or wet. Shak.",
  "sufflate - To blow up; to inflate; to inspire. [R.] T. Ward.",
  "insanitary - Not sanitary; unhealthy; as, insanitary conditions of drainage.",
  "figent - Fidgety; restless. [Obs.] Such a little figent thing. Beau. & Fl.",
  "anopsia - Want or defect of sight; blindness.",
  "saltbush - An Australian plant (Atriplex nummularia) of the Goosefoot family.",
  "vivisectionist - One who practices or advocates vivisection; a vivisector.",
  "odylic - Of or pertaining to odyle; odic; as, odylic force. [Archaic]",
  "trispast - A machine with three pulleys which act together for raising great weights. Brande & C.",
  "sea loach - The three-bearded rockling. See Rockling.",
  "transportment - The act of transporting, or the state of being transported; transportation. [R.]",
  "superreward - To reward to an excessive degree. Bacon.",
  "supersulphurize - To impregnate or combine with an excess of sulphur.",
  "overdye - To dye with excess of color; to put one color over (another). Shak.",
  "showerless - Rainless; freo from showers.",
  "froppish - Peevish; froward. [Obs.] Clarendon.",
  "elderly - Somewhat old; advanced beyond middle age; bordering on old age; as, elderly people.",
  "intransgressible - Incapable of being transgressed; not to be passes over or crossed. Holland.",
  "cora - The Arabian gazelle (Gazella Arabica), found from persia to North Africa.",
  "pantherine - Like a panther, esp. in color; as, the pantherine snake (Ptyas mucosus) of Brazil.",
  "chromascope - An instrument for showing the optical effects of color.",
  "exulceration - 1. Ulceration. Quincy. 2. A fretting; a festering; soreness. Hooker.",
  "plagueless - Free from plagues or the plague.",
  "morbifical - Causing disease; generating a sickly state; as, a morbific matter.",
  "unbundle - To release, as from a bundle; to disclose.",
  "circumflant - Blowing around. [Obs.] Evelyn.",
  "unroost - To drive from the roost. Shak.",
  "effable - Capable of being uttered or explained; utterable. Barrow.",
  "menorrhagia - (a) Profuse menstruation. (b) Any profuse bleeding from the uterus; Metrorrhagia.",
  "incompared - Peerless; incomparable. [Obs.] Spenser.",
  "themselves - The plural of himself, herself, and itself. See Himself, Herself, Itself.",
  "jack-a-lent - A small stuffed puppet to be pelted in Lent; hence, a simple fellow.",
  "periblem - Nascent cortex, or immature cellular bark.",
  "capote - A long cloak or overcoat, especially one with a hood.",
  "olivin - A complex bitter gum, found on the leaves of the olive tree; -- called also olivite.",
  "mousquetaire cuff - A deep flaring cuff.",
  "positivist - A believer in positivism. -- a.  Relating to positivism.",
  "trabeated - Furnished with an entablature.",
  "digitule - A little finger or toe, or something resembling one.",
  "timbrelled - Sung to the sound of the timbrel. \"In vain with timbreled anthems dark.\" Milton.",
  "unvessel - To cause to be no longer a vessel; to empty. [Obs.] Ford.",
  "arm-gret - Great as a man's arm. [Obs.] A wreath of gold, arm-gret. Chaucer.",
  "tortilla - An unleavened cake, as of maize flour, baked on a heated iron or stone.",
  "ethnologic - Of or pertaining to ethnology.",
  "sea fox - The thrasher shark. See Thrasher.",
  "diplostemonous - Having twice as many stamens as petals, as the geranium. R. Brown.",
  "viewsome - Pleasing to the sight; sightly. [Prov. Eng.]",
  "consound - A name applied loosely to several plants of different genera, esp. the comfrey.",
  "crewelwork - Embroidery in crewels, commonly done upon some plain material, such as linen.",
  "denizenize - To constitute (one) a denizen; to denizen. Abbott.",
  "scrobiculated - Having numerous small, shallow depressions or hollows; pitted.",
  "gulleting - A system of excavating by means of gullets or channels.",
  "corrivate - To cause to flow together, as water drawn from several streams. [Obs.] Burton.",
  "croceous - Of, pertaining to, or like, saffron; deep reddish yellow. [R.]",
  "azured - Of an azure color; sky-blue. \"The azured harebell.\" Shak.",
  "enticer - One who entices; one who incites or allures to evil. Burton.",
  "noisiness - The state or quality of being noisy.",
  "excretory - Having the quality of excreting, or throwing off excrementitious matter.",
  "oarsweed - Any large seaweed of the genus Laminaria; tangle; kelp.  See Kelp.",
  "sanability - The quality or state of being sanable; sanableness; curableness.",
  "suppalpation - The act of enticing by soft words; enticement. [Obs.]",
  "leperous - Leprous; infectious; corrupting; poisonous. \"The leperous distillment.\" Shak.",
  "stereo- - A combining form meaning solid, hard, firm, as in stereo- chemistry, stereography.",
  "pulkha - A Laplander's traveling sledge. See Sledge.",
  "cholesteric - Pertaining to cholesterin, or obtained from it; as, cholesteric acid. Ure.",
  "dementation - The act of depriving of reason; madness. Whitlock.",
  "justiceship - The office or dignity of a justice. Holland.",
  "compaction - The act of making compact, or the state of being compact. [Obs.] Bacon.",
  "tetracoccous - Having four cocci, or carpels.",
  "stratonic - Of or pertaining to an army. [R.]",
  "scholarity - Scholarship. [Obs.] . Jonson.",
  "extirpator - One who extirpates or roots out; a destroyer.",
  "statesmanship - The qualifications, duties, or employments of a statesman.",
  "resoluteness - The quality of being resolute.",
  "gratis - For nothing; without fee or recompense; freely; gratuitously.",
  "iron-fisted - Closefisted; stingy; mean.",
  "exscriptural - Not in accordance with the doctrines of Scripture; unscriptural.",
  "musrole - The nose band of a horse's bridle.",
  "cross-bearer - A subdeacon who bears a cross before an archbishop or primate on solemn occasions.",
  "snakish - Having the qualities or characteristics of a snake; snaky.",
  "fibrillation - The state of being reduced to fibers. Carpenter.",
  "anthokyan - The blue coloring matter of certain flowers. Same as Cyanin.",
  "feather-foil - An aquatic plant (Hottonia palustris), having finely divided leaves.",
  "rayah - A person not a Mohammedan, who pays the capitation tax. [Turkey.]",
  "succinite - (a) Amber. (b) A garnet of an amber color.",
  "glumaceous - Having glumes; consisting of glumes.",
  "iodide - A binary compound of iodine, or one which may be regarded as binary; as, potassium iodide.",
  "pentahedron - A solid figure having five sides.",
  "geoscopy - Knowledge of the earth, ground, or soil, obtained by inspection. Chambers.",
  "vanglo - Benne (Sesamum orientale); also, its seeds; -- so called in the West Indies.",
  "glass-gazing - Given to viewing one's self in a glass or mirror; finical. [Poetic] Shak.",
  "undampned - Uncondemned. [Obs.] Wyclif (Acts xvi. 37).",
  "side-wheel - Having a paddle wheel on each side; -- said of steam vessels; as, a side-wheel steamer.",
  "struvite - A crystalline mineral found in guano. It is a hydrous phosphate of magnesia and ammonia.",
  "ople tree - The witch-hazel. [Obs.] Ainsworth.",
  "squareness - The quality of being square; as, an instrument to try the squareness of work.",
  "char-a-banc - A long, light, open vehicle, with benches or seats running lengthwise.",
  "interstratify - To put or insert between other strata.",
  "panic-struck - Struck with a panic, or sudden fear. Burke.",
  "forhall - To harass; to torment; to distress. [Obs.] Spenser.",
  "displayer - One who, or that which, displays.",
  "pitying - Expressing pity; as, a pitying eye, glance, or word. -- Pit\"y*ing*ly, adv.",
  "adsignification - Additional signification. [R.] Tooke.",
  "mancipate - To enslave; to bind; to restrict. [Obs.] Sir M. Hale.",
  "puller - One who, or that which, pulls. Proud setter up and puller down of kings. Shak.",
  "armozeen - A thick plain silk, generally black, and used for clerical. Simmonds.",
  "phyllomania - An abnormal or excessive production of leaves.",
  "winsomeness - The characteristic of being winsome; attractiveness of manner. J. R. Green.",
  "balkish - Uneven; ridgy. [R.] Holinshed.",
  "businesslike - In the manner of one transacting business wisely and by right methods.",
  "-lite - Combining forms fr. Gr. li`qos a stone; -- used chiefly in naming minerals and rocks.",
  "inhauler - A rope used to draw in the jib boom, or flying jib boom.",
  "mesitylenate - A salt of mesitylenic acid.",
  "dastardly - Meanly timid; cowardly; base; as, a dastardly outrage.",
  "examen - Examination; inquiry. [R.] \"A critical examen of the two pieces.\" Cowper.",
  "neapolitan - Of of pertaining to Maples in Italy. -- n.  A native or citizen of Naples.",
  "ostend - To exhibit; to manifest. [Obs.] Mercy to mean offenders we'll ostend. J. Webster.",
  "starshine - The light of the stars. [R.] The starshine lights upon our heads. R. L. Stevenson.",
  "cordovan - Same as Cordwain. in England the name is applied to leather made from horsehide.",
  "sparadrap - 1. A cerecloth. [Obs.] 2. (Med.)  Any adhesive plaster.",
  "oblationer - One who makes an offering as an act worship or reverence. Dr. H. More.",
  "couple-beggar - One who makes it his business to marry beggars to each other. Swift.",
  "eisel - Vinegar; verjuice. [Obs.] Sir T. More.",
  "saccharonate - A salt of saccharonic acid.",
  "oxter - The armpit; also, the arm. [Prov. Eng. & Scot.]",
  "inshave - A plane for shaving or dressing the concave or inside faces of barrel staves.",
  "eupyrion - A contrivance for obtaining a light instantaneous, as a lucifer match. Brande & C.",
  "unsteel - To disarm; to soften. Richardson.",
  "armhole - 1. The cavity under the shoulder; the armpit. Bacon. 2. A hole for the arm in a garment.",
  "chancrous - Of the nature of a chancre; having chancre.",
  "oreodont - Resembling, or allied to, the genus Oreodon.",
  "portraitist - A portrait painter. [R.] Hamerton.",
  "professorship - The office or position of a professor, or public teacher. Walton.",
  "downpour - A pouring or streaming downwards; esp., a heavy or continuous shower.",
  "waught - A large draught of any liquid. [Scot.] Jamieson.",
  "extravenate - Let out of the veins. [Obs.] \"Extravenate blood.\" Glanvill.",
  "barebone - A very lean person; one whose bones show through the skin. Shak.",
  "veldt - A region or tract of land; esp., the open field; grass country. [South Africa]",
  "cenation - Meal-taking; dining or supping. [Obs.] Sir T. Browne.",
  "appearer - One who appears. Sir T. Browne.",
  "argillo-ferruginous - Containing clay and iron.",
  "bayatte - A large, edible, siluroid fish of the Nile, of two species (Bagrina bayad and B. docmac).",
  "meteorism - Flatulent distention of the abdomen; tympanites.",
  "palpability - The quality of being palpable, or perceptible by the touch. Arbuthnot.",
  "discommodity - Disadvantage; inconvenience. Bacon.",
  "kindergartner - One who teaches in a kindergarten.",
  "obsecrate - To beseech; to supplicate; to implore. [R.]. Cockerman.",
  "narrable - Capable of being narrated or told. [Obs.]",
  "endwise - 1. On end; erectly; in an upright position. 2. With the end forward.",
  "representationary - Implying representation; representative. [R.]",
  "spongiform - Resembling a sponge; soft and porous; porous.",
  "monkey-bread - The fruit of the Adansonia digitata; also, the tree. See Adansonia.",
  "nonregardance - Want of due regard; disregard; slight. [Obs.] Shak.",
  "half-penny - An English coin of the value of half a penny; also, the value of half a penny.",
  "boozer - One who boozes; a toper; a guzzler of alcoholic liquors; a bouser.",
  "dislimb - To tear limb from limb; to dismember. [Obs.] Bailey.",
  "unservice - Neglect of duty; idleness; indolence. [Obs.] Massinger.",
  "pearlings - A kind of lace of silk or thread. [Scot.] Sir W. Scott.",
  "reimprisonment - The act of reimprisoning, or the state of being reimprisoned."
]