export default [
  "ectodermal - Of or relating to the ectoderm.",
  "brushite - A white or gray crystalline mineral consisting of the acid phosphate of calcium.",
  "water beech - The American hornbeam. See Hornbeam.",
  "aspish - Pertaining to, or like, an asp.",
  "assentatory - Flattering; obsequious. [Obs.] -- As*sent\"a*to*ri*ly, adv. [Obs.]",
  "alternateness - The quality of being alternate, or of following by turns.",
  "dear-bought - Bought at a high price; as, dear-bought experience.",
  "bushiness - The condition or quality of being bushy.",
  "stereotypy - The art or process of making stereotype plates.",
  "radio-flagellata - A group of Protozoa having both flagella and pseudopodia.",
  "vitrine - A glass show case for displaying fine wares, specimens, etc.",
  "circummure - To encompass with a wall. Shak.",
  "unauthorize - To disown the authority of; to repudiate.",
  "conventioner - One who belongs to a convention or assembly.",
  "ascitic - Of, pertaining to, or affected by, ascites; dropsical.",
  "bimetallist - An advocate of bimetallism.",
  "cesspipe - A pipe for carrying off waste water, etc., from a sink or cesspool. Knight.",
  "lily-handed - Having white, delicate hands.",
  "backsheesh - In Egypt and the Turkish empire, a gratuity; a \"tip\".",
  "anoil - The anoint with oil. [Obs.] Holinshed.",
  "affiant - One who makes an affidavit. [U. S.] Burrill. Syn. -- Deponent. See Deponent.",
  "weathermost - Being farthest to the windward.",
  "newish - Somewhat new; nearly new. Bacon.",
  "interluded - Inserted in the manner of an interlude; having or containing interludes.",
  "cantonize - To divide into cantons or small districts.",
  "dispenser - One who, or that which, dispenses; a distributer; as, a dispenser of favors.",
  "knickknack - A trifle or toy; a bawble; a gewgaw.",
  "organogenic - Of or pertaining to organogenesis.",
  "muntin - Same as Mullion; -- especially used in joiner's work.",
  "longicorn - Long-horned; pertaining to the Longicornia. -- n.  One of the Longicornia.",
  "mysticete - Any right whale, or whalebone whale. See Cetacea.",
  "cobwebbed - Abounding in cobwebs. \"The cobwebbed cottage.\" Young.",
  "hemuse - The roebuck in its third year. [Prov. Eng.]",
  "dwine - To waste away; to pine; to languish. [Obs. or Prov. Eng.] Gower.",
  "marcato - In a marked emphatic manner; -- used adverbially as a direction.",
  "tinkerly - After the manner of a tinker. [R.]",
  "wire-drawer - One who draws metal into wire.",
  "attaste - To taste or cause to taste. [Obs.] Chaucer.",
  "francic - Pertaining to the Franks, or their language; Frankish.",
  "urtical - Resembling nettles; -- said of several natural orders allied to urticaceous plants.",
  "isomeromorphism - Isomorphism between substances that are isomeric.",
  "dilatator - A muscle which dilates any part; a dilator.",
  "panpharmacon - A medicine for all diseases; a panacea. [R.]",
  "mesenteric - Pertaining to a mesentery; mesaraic.",
  "godsend - Something sent by God; an unexpected acquisiton or piece of good fortune.",
  "intermutation - Interchange; mutual or reciprocal change.",
  "audaciousness - The quality of being audacious; impudence; audacity.",
  "desulphuration - The act or process of depriving of sulphur.",
  "carious - Affected with caries; decaying; as, a carious tooth.",
  "admirer - One who admires; one who esteems or loves greatly. Cowper.",
  "canape confident - A sofa having a seat at each end at right angles to the main seats.",
  "twittingly - In a twitting manner; with upbraiding.",
  "pebbled - Abounding in pebbles. Thomson.",
  "redemptionary - One who is, or may be, redeemed. [R.] Hakluyt.",
  "conjury - The practice of magic; enchantment. Motley.",
  "waybread - The common dooryard plantain (Plantago major).",
  "elocutive - Pertaining to oratorical expression. [Obs.] Feltham.",
  "unimplicate - Not implicated. \"Unimplicate in folly.\" R. Browning.",
  "discordful - Full of discord; contentious. [Obs.] \"His discordful dame.\" Spenser.",
  "sacciform - Having the general form of a sac.",
  "ennation - The ninth segment in insects.",
  "glabellum - The median, convex lobe of the head of a trilobite. See Trilobite.",
  "great-heartedness - The quality of being greathearted; high-mindedness; magnanimity.",
  "untuck - To unfold or undo, as a tuck; to release from a tuck or fold.",
  "detracter - One who detracts; a detractor. Other detracters and malicious writers. Sir T. North.",
  "undertow - The current that sets seaward near the bottom when waves are breaking upon the shore.",
  "apiary - A place where bees are kept; a stand or shed for bees; a beehouse.",
  "zooegraphic - Of or pertaining to the description of animals.",
  "torvity - Sourness or severity of countenance; sterness. [Obs.]",
  "dermobranchiate - Having the skin modified to serve as a gill.",
  "hocco - The crested curassow; -- called also royal pheasant. See Curassow.",
  "vastly - To a vast extent or degree; very greatly; immensely. Jer. Taylor.",
  "middy - A colloquial abbreviation of midshipman.",
  "cartilaginification - The act or process of forming cartilage. Wright.",
  "venatical - Of or pertaining to hunting; used in hunting. [R.] \" Venatical pleasure.\" Howell.",
  "neossine - The substance constituting the edible bird's nest.",
  "modificative - That which modifies or qualifies, as a word or clause.",
  "forsay - To forbid; to renounce; to forsake; to deny. [Obs.] Spenser.",
  "unshelve - To remove from, or as from, a shelf.",
  "servileness - Quality of being servile; servility.",
  "bay-antler - The second tine of a stag's horn. See under Antler.",
  "saxonism - An idiom of the Saxon or Anglo-Saxon language. T. Warton.",
  "quickly - Speedily; with haste or celerity; soon; without delay; quick.",
  "inconcocted - Imperfectly digested, matured, or ripened. [Obs.] Bacon.",
  "thecal - Of or pertaining to a theca; as, a thecal abscess.",
  "treeful - The quantity or number which fills a tree.",
  "exhalant - Having the quality of exhaling or evaporating.",
  "treckschuyt - A covered boat for goods and passengers, used on the Dutch and Flemish canals.",
  "nonemphatic - Having no emphasis; unemphatic.",
  "uvulatomy - The operation of removing the uvula.",
  "stander - 1. One who stands. 2. Same as Standel. [Obs.] Ascham.",
  "intubation - The introduction of a tube into an organ to keep it open, as into the larynx in croup.",
  "apostolicity - The state or quality of being apostolical.",
  "metastome - A median elevation behind the mouth in the arthropods.",
  "ecardines - An order of Brachiopoda; the Lyopomata. See Brachiopoda.",
  "pactional - Of the nature of, or by means of, a paction. Bp. Sanderson.",
  "dupery - The act or practice of duping. [R.]",
  "ceinture - A cincture, girdle, or belt; -- chiefly used in English as a dressmaking term.",
  "competible - Compatible; suitable; consistent. [Obs.] Sir M. Hale.",
  "glossography - The writing of glossaries, glosses, or comments for illustrating an author.",
  "xylographic - Of or pertaining to xylography, or wood engraving.",
  "constructer - One who, or that which, constructs or frames.",
  "eikon - An image or effigy; -- used rather in an abstract sense, and rarely for a work of art.",
  "seigh - obs. imp. sing. of See. Saw. Chaucer.",
  "ethide - Any compound of ethyl of a binary type; as, potassium ethide.",
  "falcer - One of the mandibles of a spider.",
  "semispheric - Having the figure of a half sphere. Kirwan.",
  "transvection - The act of conveying or carrying over. [R.]",
  "roosa oil - The East Indian name for grass oil. See under Grass.",
  "mya - A genus of bivalve mollusks, including the common long, or soft-shelled, clam.",
  "piddock - Any species of Pholas; a pholad. See Pholas.",
  "excalfactive - Serving to heat; warming. [Obs.] Cotgrave.",
  "rodomel - Juice of roses mixed with honey. Simmonds.",
  "sacking - Stout, coarse cloth of which sacks, bags, etc., are made.",
  "nephoscope - An instrument for observing the clouds and their velocity.",
  "peplis - A genus of plants including water purslane.",
  "urbane - Courteous in manners; polite; refined; elegant.",
  "aerotherapentics - Treatment of disease by the use of air or other gases.",
  "bobwhite - The common qua(Colinus, or Ortyx, Virginianus); -- so called from its note.",
  "truthy - Truthful; likely; probable. [R.] \"A more truthy import.\" W. G. Palgrave.",
  "passionateness - The state or quality of being passionate.",
  "dysodile - An impure earthy or coaly bitumen, which emits a highly fetid odor when burning.",
  "infuscate - To darken; to make black; to obscure.",
  "usant - Using; accustomed. [Obs.] \"Usant for to steal.\" Chaucer.",
  "unconsecrate - To render not sacred; to deprive of sanctity; to desecrate. [Obs.] South.",
  "piedmontite - A manganesian kind of epidote, from Piedmont. See Epidote.",
  "smokeless - Making or having no smoke. \"Smokeless towers.\" Pope.",
  "resultate - A result. [Obs.] \"The resultate of their counsil.\" BAcon.",
  "irremeable - Admitting no return; as, an irremeable way. [Obs.] Dryden.",
  "bicalcarate - Having two spurs, as the wing or leg of a bird.",
  "thanatology - A description, or the doctrine, of death. Dunglison.",
  "chieftainship - The rank, dignity, or office of a chieftain.",
  "oppositisepalous - Placed in front of a sepal.",
  "arrestee - The person in whose hands is the property attached by arrestment.",
  "filchingly - By pilfering or petty stealing.",
  "ouphe - A fairy; a goblin; an elf. [Obs.] \"Like urchins, ouphes, and fairies.\" Shak.",
  "overanxiety - The state of being overanxious; excessive anxiety.",
  "veneering - 1. The act or art of one who veneers. 2. Thin wood or other material used as a veneer.",
  "re-sign - To affix one's signature to, a second time; to sign again.",
  "endoneurium - The delicate bands of connective tissue among nerve fibers.",
  "somniloquence - The act of talking in one's sleep; somniloquism.",
  "humic - Pertaining to, or derived from, vegetable mold; as, humic acid. See Humin.",
  "remold - To mold or shape anew or again; to reshape.",
  "peorias - An Algonquin tribe of Indians who formerly inhabited a part of Illinois.",
  "resurrectionist - One who steals bodies from the grave, as for dissection. [Slang]",
  "destructibility - The quality of being capable of destruction; destructibleness.",
  "polygram - A figure consisting of many lines. [R.] Barlow.",
  "spahi - 1. Formerly, one of the Turkish cavalry. 2. An Algerian cavalryman in the French army.",
  "materiation - Act of forming matter. [Obs.] Sir T. Browne.",
  "indulgential - Relating to the indulgences of the Roman Catholic Church. Brevint.",
  "chlamys - A loose and flowing outer garment, worn by the ancient Greeks; a kind of cloak.",
  "historiology - A discourse on history. Cockeram.",
  "dimble - A bower; a dingle. [Obs.] Drayton.",
  "zonular - Of or pertaining to a zone; zone-shaped. \"The zonular type of a placenta.\" Dana.",
  "kupfernickel - Copper-nickel; niccolite. See Niccolite.",
  "superhive - A removable upper part of a hive. The word is sometimes contracted to super.",
  "bipolar - Doubly polar; having two poles; as, a bipolar cell or corpuscle.",
  "knapple - To break off with an abrupt, sharp noise; to bite; to nibble. [Obs. or Prov. Eng.]",
  "yestereve - The evening of yesterday; the evening last past.",
  "frigefaction - The act of making cold. [Obs.]",
  "demonstratively - In a manner fitted to demonstrate; clearly; convincingly; forcibly.",
  "irreconcile - To prevent from being reconciled; to alienate or disaffect. [Obs.]",
  "elver - A young eel; a young conger or sea eel; -- called also elvene.",
  "self-applying - Applying to or by one's self.",
  "connaturalness - Participation of the same nature; natural union. I. Walton.",
  "barful - Full of obstructions. [Obs.] Shak.",
  "vese - Onset; rush; violent draught or wind. [Obs.] Chaucer.",
  "heterarchy - The government of an alien. [Obs.] Bp. Hall.",
  "-ly - A suffix forming adjectives and adverbs, and denoting likeness or resemblance.",
  "term policy - A policy of term insurance.",
  "nall - An awl. [Obs. or Prov. Eng.] Tusser.",
  "proportionality - The state of being in proportion. Coleridge.",
  "recharge - 1. To charge or accuse in return. 2. To attack again; to attack anew. Dryden.",
  "appendicectomy - Excision of the vermiform appendix.",
  "unseen - 1. Not seen or discovered. 2. Unskilled; inexperienced. [Obs.] Clarendon.",
  "disreverence - To treat irreverently or with disrespect. [Obs.] Sir T. More.",
  "cylindriform - Having the form of a cylinder.",
  "overwrest - To wrest or force from the natural or proper position. Shak.",
  "irretrievably - In an irretrievable manner.",
  "corticated - Having a special outer covering of a nature unlike the interior part.",
  "lactoprotein - A peculiar albuminous body considered a normal constituent of milk.",
  "gunlock - The lock of a gun, for producing the discharge. See Lock.",
  "bevile - A chief broken or opening like a carpenter's bevel. Encyc. Brit.",
  "tomcat - A male cat, especially when full grown or of large size.",
  "viled - Abusive; scurrilous; defamatory; vile. [Obs.] \"Viled speeches.\" Hayward.",
  "agrin - In the act of grinning. \"His visage all agrin.\" Tennyson.",
  "uranographical - Of or pertaining to uranography; as, an uranographic treatise.",
  "abstruseness - The quality of being abstruse; difficulty of apprehension. Boyle.",
  "auriscope - An instrument for examining the condition of the ear.",
  "aggrate - To please. [Obs.] Each one sought his lady to aggrate. Spenser.",
  "green-stall - A stall at which greens and fresh vegetables are exposed for sale.",
  "subelongate - Not fully elongated; somewhat elongated.",
  "lacrymal - See Lachrymatory, n., and Lachrymal, a.",
  "antetemple - The portico, or narthex in an ancient temple or church.",
  "quadrifurcated - Having four forks, or branches.",
  "unplumb - To deprive of lead, as of a leaden coffin. [R.] Burke.",
  "ariolation - A soothsaying; a foretelling. [Obs.] Sir T. Browne.",
  "envermeil - To color with, or as with, vermilion; to dye red. [Obs.] Milton.",
  "basifugal - Tending or proceeding away from the base; as, a basifugal growth.",
  "exilition - A sudden springing or leaping out. [Obs.] Sir T. Browne.",
  "hyemal - Belonging to winter; done in winter. Sir T. Browne.",
  "causationist - One who believes in the law of universal causation.",
  "reversely - In a reverse manner; on the other hand; on the opposite. Bp. Pearson.",
  "drawfiling - The process of smooth filing by working the file sidewise instead of lengthwise.",
  "queenly - Like, becoming, or suitable to, a queen.",
  "chondrology - The science which treats of cartilages. Dunglison.",
  "atypical - That has no type; devoid of typical character; irregular; unlike the type.",
  "epinicial - Relating to victory. \"An epinicial song.\" T. Warton.",
  "adnominal - Pertaining to an adnoun; adjectival; attached to a noun. Gibbs. -- Ad*nom\"i*nal*ly, adv.",
  "megalonyx - An extinct quaternary mammal, of great size, allied to the sloth.",
  "levelism - The disposition or endeavor to level all distinctions of rank in society.",
  "fighter - One who fights; a combatant; a warrior. Shak.",
  "harpist - A player on the harp; a harper. W. Browne.",
  "adread - To dread. [Obs.] Sir P. Sidney.",
  "exesion - The act of eating out or through. [Obs.] Sir T. Browne.",
  "lobule - A small lobe; a subdivision of a lobe. Lobule of the ear. (Anat.) Same as Lobe of the ear.",
  "bordelais - Of or pertaining to Bordeaux, in France, or to the district around Bordeaux.",
  "deuterogamist - One who marries the second time.",
  "pulverous - Consisting of dust or powder; like powder.",
  "explorate - To explore. [Obs.] Sir. T. Browne.",
  "discodactylous - Having sucking disks on the toes, as the tree frogs.",
  "petalous - Having petals; petaled; -- opposed to Ant: apetalous.",
  "lucrific - Producing profit; gainful. [Obs.]",
  "afterpains - The pains which succeed childbirth, as in expelling the afterbirth.",
  "fulgidity - Splendor; resplendence; effulgence. [R.] Bailey.",
  "prohibitive - That prohibits; prohibitory; as, a tax whose effect is prohibitive.",
  "deckle-edged - Having a deckle edge; as, deckle-edged paper; a deckle-edged book.",
  "discorporate - Deprived of the privileges or form of a body corporate. [Obs.] Jas. II.",
  "perissodactyl - One of the Perissodactyla.",
  "conjunctly - In union; conjointly; unitedly; together. Sir W. Hamilton.",
  "encyclopedian - Embracing the whole circle of learning, or a wide range of subjects.",
  "glycerite - A medicinal preparation made by mixing or dissolving a substance in glycerin.",
  "exertive - Having power or a tendency to exert; using exertion.",
  "gristly - Consisting of, or containing, gristle; like gristle; cartilaginous.",
  "propounder - One who propounds, proposes, or offers for consideration. Chillingworth.",
  "attainableness - The quality of being attainable; attainability.",
  "peridium - The envelope or coat of certain fungi, such as the puffballs and earthstars.",
  "avenage - A quantity of oats paid by a tenant to a landlord in lieu of rent. Jacob.",
  "giusto - In just, correct, or suitable time.",
  "misedition - An incorrect or spurious edition. [Obs.] Bp. Hall.",
  "royston crow - See Hooded crow, under Hooded.",
  "perrier - A short mortar used formerly for throwing stone shot. Hakluyt.",
  "nizam - The title of the native sovereigns of Hyderabad, in India, since 1719.",
  "managership - The office or position of a manager.",
  "similize - To liken; to compare; as, to similize a person, thing, or act. Lowell.",
  "moorstone - A species of English granite, used as a building stone.",
  "hodman - A man who carries a hod; a mason's tender.",
  "unguentary - Like an unguent, or partaking of its qualities.",
  "parotitis - Inflammation of the parotid glands. Epidemic, or Infectious, parotitis, mumps.",
  "ponderability - The quality or state of being ponderable.",
  "mockbird - The European sedge warbler (Acrocephalus phragmitis).",
  "anthropophagi - Man eaters; cannibals. Shak.",
  "verificative - Serving to verify; verifying; authenciating; confirming.",
  "paleotherian - Of or pertaining to Paleotherium.",
  "chaped - Furnished with a chape or chapes. [Obs.] Chaucer.",
  "immit - To send in; to inject; to infuse; -- the correlative of emit. [R.] Boyle.",
  "nutritional - Of or pertaining to nutrition; as, nutritional changes.",
  "lampate - A supposed salt of lampic acid. [Obs.]",
  "attaintment - Attainder; attainture; conviction.",
  "hylism - A theory which regards matter as the original principle of evil.",
  "tanner - One whose occupation is to tan hides, or convert them into leather by the use of tan.",
  "dronepipe - One of the low-toned tubes of a bagpipe.",
  "unoperculated - Destitute of an operculum, or cover.",
  "asse - A small foxlike animal (Vulpes cama) of South Africa, valued for its fur.",
  "soredium - A patch of granular bodies on the surface of the thallus of lichens.",
  "direptitious - Characterized by direption. [R.] Encyc. Dict.",
  "emolumental - Pertaining to an emolument; profitable. [R.] Evelyn.",
  "fazzolet - A handkerchief. [R.] percival.",
  "feminineness - The quality of being feminine; womanliness; womanishness.",
  "truismatic - Of or pertaining to truisms; consisting of truisms. [R.]",
  "bubbling jock - The male wild turkey, the gobbler; -- so called in allusion to its notes.",
  "conspurcate - To pollute; to defile. [Obs.] Cockeram.",
  "incensurable - Not censurable. Dr. T. Dwight. -- In*cen\"sur*a*bly, adv.",
  "ire - Anger; wrath. [Poet.] Syn. -- Anger; passion; rage; fury. See Anger.",
  "rach - A dog that pursued his prey by scent, as distinguished from the greyhound.[Obs.]",
  "anthropogenic - Of or pertaining to anthropogeny.",
  "aphorismer - A dealer in aphorisms. [Used in derogation or contempt.] Milton.",
  "serfage - The state or condition of a serf.",
  "embrothel - To inclose in a brothel. [Obs.] Donne.",
  "recompensement - Recompense; requital. [Obs.] Fabyan.",
  "crumpet - A kind of large. thin muffin or cake, light and spongy, and cooked on a griddle or spider.",
  "minionlike - Like a minion; daintily. Camden.",
  "quellio - A ruff for the neck. [Obs.] B. Jonson.",
  "rifleman - A soldier armed with a rifle.",
  "primordialism - Devotion to, or persistence in, conditions of the primordial state. H. Spencer.",
  "tollage - Payment of toll; also, the amount or quantity paid as toll. Drayton.",
  "hyalograph - An instrument for tracing designs on glass.",
  "pomona - The goddess of fruits and fruit trees.",
  "caracoly - An alloy of gold, silver, and copper, of which an inferior quality of jewerly is made.",
  "ballista - An ancient military engine, in the form of a crossbow, used for hurling large missiles.",
  "heterogenist - One who believes in the theory of spontaneous generation, or heterogenesis. Bastian.",
  "midmain - The middle part of the main or sea. [Poetic] Chapman.",
  "companable - Companionable; sociable. [Obs.] Chaucer.",
  "inconsiderateness - The quality or state of being inconsiderate. Tillotson.",
  "stepsister - A daughter of one's stepfather or stepmother by a former marriage.",
  "demonstratory - Tending to demonstrate; demonstrative. Johnson.",
  "unswell - To sink from a swollen state; to subside. [Obs.] Chaucer.",
  "eagless - A female or hen eagle. [R.] Sherwood.",
  "pluripartite - Deeply divided into several portions.",
  "dunted - Beaten; hence, blunted. [Obs.] Fencer's swords . . . having the edge dunted. Fuller.",
  "extraforaneous - Pertaining to that which is out of doors. \"Extr occupations.\" Cowper.",
  "shortstop - The player stationed in the field bewtween the second and third bases.",
  "self-righteous - Righteous in one's own esteem; pharisaic.",
  "supersaturate - To add to beyond saturation; as, to supersaturate a solution.",
  "maidenlike - Like a maiden; modest; coy.",
  "approximator - One who, or that which, approximates.",
  "stealingly - By stealing, or as by stealing, furtively, or by an invisible motion. Sir P. Sidney.",
  "daughterliness - The state of a daughter, or the conduct becoming a daughter.",
  "notchboard - The board which receives the ends of the steps in a staircase.",
  "experrection - A waking up or arousing. [Obs.] Holland",
  "sublease - A lease by a tenant or lessee to another person; an underlease. Bouvier.",
  "interestedness - The state or quality of being interested; selfishness. Richardson.",
  "kyar - Cocoanut fiber, or the cordage made from it. See Coir.",
  "dauby - Smeary; viscous; glutinous; adhesive. \"Dauby wax.\"",
  "dropmele - By drops or small portions. [Obs.] Distilling dropmeal, a little at once. Holland.",
  "mellifluently - In a mellifluent manner.",
  "feuterer - A dog keeper. [Obs.] Massinger.",
  "repressible - Capable of being repressed.",
  "ghee - Butter clarified by boiling, and thus converted into a kind of oil. [India] Malcom.",
  "disenamor - To free from the captivity of love. Shelton.",
  "decemvirship - The office of a decemvir. Holland.",
  "licitation - The act of offering for sale to the highest bidder. [R.]",
  "pretendence - The act of pretending; pretense. [Obs.] Daniel.",
  "disseverance - The act of disserving; separation.",
  "anthozoic - Of or pertaining to the Anthozoa.",
  "interhemal - Between the hemal arches or hemal spines. -- n.  An interhemal spine or cartilage.",
  "dysluite - A variety of the zinc spinel or gahnite.",
  "gadder - One who roves about idly, a rambling gossip.",
  "gentilesse - Gentleness; courtesy; kindness; nobility. [Obs.] Chaucer.",
  "backbiter - One who backbites; a secret calumniator or detractor.",
  "comelily - In a suitable or becoming manner. [R.] Sherwood.",
  "judaization - The act of Judaizing; a conforming to the Jewish religion or ritual. [R.]",
  "self-suspended - Suspended by one's self or by itself; balanced. Southey.",
  "gallinacean - One of the Gallinae or gallinaceous birds.",
  "pellagrin - One who is afficted with pellagra. Chambers's Encyc.",
  "lithochromics - The art of printing colored pictures on canvas from oil paintings on stone.",
  "promulger - One who promulges or publishes what was before unknown. Atterbury.",
  "cotangent - The tangent of the complement of an arc or angle. See Illust. of Functions.",
  "indiscerpibility - The state or quality of being indiscerpible. [Obs.] Dr. H. More.",
  "predy - Cleared and ready for engagement, as a ship. Smart.",
  "misjudgment - A wrong or unjust judgment.",
  "confutative - Adapted or designed to confute.  Bp. Warburton",
  "swede - 1. A native or inhabitant of Sweden. 2. (Bot.)  A Swedish turnip. See under Turnip.",
  "turpentine state - North Carolina; -- a nickname alluding to its extensive production of turpentine.",
  "courtliness - The quality of being courtly; elegance or dignity of manners.",
  "galvanoplasty - The art or process of electrotypy.",
  "greeze - A step. See Gree, a step. [Obs.] The top of the ladder, or first greeze, is this. Latimer.",
  "balneal - Of or pertaining to a bath. Howell.",
  "lappaceous - Resembling the capitulum of burdock; covered with forked points.",
  "perispore - The outer covering of a spore.",
  "firms - The principal rafters of a roof, especially a pair of rafters taken together. [Obs.]",
  "agricultor - An agriculturist; a farmer. [R.]",
  "antitypical - Of or pertaining to an antitype; explaining the type. -- An`ti*typ\"ic*al*ly, adv.",
  "bending - The marking of the clothes with stripes or horizontal bands. [Obs.] Chaucer.",
  "grandinous - Consisting of hail; abounding in hail. [R.] Bailey.",
  "islamize - To conform, or cause to conform, to the religion of Islam.",
  "tridiapason - A triple octave, or twenty-second. Busby.",
  "estuate - To boil up; to swell and rage; to be agitated. Bacon.",
  "conducibility - The state or quality of being conducible; conducibleness. Bp. Wilkins.",
  "ghoulish - Characteristic of a ghoul; vampirelike; hyenalike.",
  "nouthe - Just now; at present. [Obs.] But thereof needeth not to speak as nouthe. Chaucer.",
  "unionistic - Of or pertaining to union or unionists; tending to promote or preserve union.",
  "towned - Having towns; containing many towns. [Obs.] Hakluyt.",
  "cement steel - Steel produced by cementation; blister steel.",
  "biannual - Occurring twice a year; half-yearly; semiannual.",
  "devata - A deity; a divine being; a good spirit; an idol. [Written also dewata.]",
  "abysm - An abyss; a gulf. \"The abysm of hell.\" Shak.",
  "fronded - Furnished with fronds. \"Fronded palms.\" Whittier.",
  "rightward - Toward the right. Rightward and leftward rise the rocks. Southey.",
  "recompenser - One who recompenses. A thankful recompenser of the benefits received. Foxe.",
  "histrionicism - The histronic art; stageplaying. W. Black.",
  "uterovaginal - Pertaining to both the uterus and the vagina.",
  "intolerability - The quality of being intolerable; intolerableness. [R.]",
  "water-standing - Tear-filled. [R.] \"Many an orphan's water-standing eye.\" Shak.",
  "abortifacient - Producing miscarriage. -- n.  A drug or an agent that causes premature delivery.",
  "anywise - In any wise or way; at all. \"Anywise essential.\" Burke.",
  "crase - To break in pieces; to crack. [Obs.] \"The pot was crased.\" Chaucer.",
  "haematology - The science which treats of the blood. Same as Hematology.",
  "mouthful - 1. As much as is usually put into the mouth at one time. 2. Hence, a small quantity.",
  "desquamate - To peel off in the form of scales; to scale off, as the skin in certain diseases.",
  "hoful - Careful; wary. [Obs.] Stapleton.",
  "hemispheroidal - Resembling, or approximating to, a hemisphere in form.",
  "distrainor - One who distrains; the party distraining goods or chattels. Blackstone.",
  "fulgid - Shining; glittering; dazzling. [R.] Pope.",
  "jactation - A throwing or tossing of the body; a shaking or agitation. Sir. W. Temple.",
  "manesheet - A covering placed over the upper part of a horse's head.",
  "rakehelly - Dissolute; wild; lewd; rakish. [Obs.] Spenser. B. Jonson.",
  "cephalometry - The measurement of the heads of living persons. -- Ceph`a*lo*met\"ric (#),a.",
  "stoneware - A species of coarse potter's ware, glazed and baked.",
  "pettily - In a petty manner; frivolously.",
  "zolaesque - In the style of Zola (see Zolaism).",
  "adventuresome - Full of risk; adventurous; venturesome. -- Ad*ven\"ture*some*ness, n.",
  "agrostology - That part of botany which treats of the grasses.",
  "natchnee - An annual grass (Eleusine coracona), cultivated in India as a food plant.",
  "water flounder - The windowpane (Pleuronectes maculatus). [Local, U. S.]",
  "autochthonous - Aboriginal; indigenous; native.",
  "antares - The principal star in Scorpio: -- called also the Scorpion's Heart.",
  "nucleated - Having a nucleus; nucleate; as, nucleated cells.",
  "cacotechny - A corruption or corrupt state of art. [R.]",
  "germ plasm - See Plasmogen, and Idioplasm.",
  "lent lily - the daffodil; -- so named from its blossoming in spring.",
  "paries - The triangular middle part of each segment of the shell of a barnacle.",
  "hagberry - A plant of the genus Prunus (P. Padus); the bird cherry. [Scot.]",
  "flitty - Unstable; fluttering. [Obs.] Dr. H. More.",
  "weakishness - Quality or state of being weakish.",
  "cockup - A large, highly esteemed, edible fish of India (Lates calcarifer); -- also called begti.",
  "linguacious - Given to the use of the tongue; loquacious. [Obs.]",
  "gubernatorial - Pertaining to a governor, or to government.",
  "meath - A sweet liquor; mead. [Obs.] Chaucer. Milton.",
  "calcivorous - Eroding, or eating into, limestone.",
  "dismail - To divest of coat of mail. Spenser.",
  "pteropodous - Of or pertaining to the Pteropoda.",
  "kee - See Kie, Ky, and Kine. [Prov. Eng.] Gay.",
  "predeterminate - Determined beforehand; as, the predeterminate counsel of God.",
  "fuscation - A darkening; obscurity; obfuscation. [R.] Blount.",
  "prepose - To place or set before; to prefix. [Obs.] Fuller.",
  "delicateness - The quality of being delicate.",
  "soaring - from Soar. -- Soar\"ing*ly, adv.",
  "lippitude - Soreness of eyes; the state of being blear-eyes; blearedness.",
  "microfarad - The millionth part of a farad.",
  "playwright - A maker or adapter of plays.",
  "bromize - To prepare or treat with bromine; as, to bromize a silvered plate.",
  "coppersmith - One whose occupation is to manufacture copper utensils; a worker in copper.",
  "thurghfare - Thoroughfare. [Obs.] This world is but a thurghfare full of woe. Chaucer.",
  "recoilingly - In the manner of a recoil.",
  "glucosuria - A condition in which glucose is discharged in the urine; diabetes mellitus.",
  "epigastrium - The upper part of the abdomen.",
  "arval - A funeral feast. [North of Eng.] Grose.",
  "rootstock - A perennial underground stem, producing leafly s",
  "alioth - A star in the tail of the Great Bear, the one next the bowl in the Dipper.",
  "lustihead - See Lustihood. [Obs.] Chaucer.",
  "flabbergastation - The state of being flabbergasted. [Jocular] London Punch.",
  "sheriffry - The office or jurisdiction of sheriff. See Shrievalty.",
  "waxberry - The wax-covered fruit of the wax myrtle, or bayberry. See Bayberry, and Candleberry tree.",
  "noyous - Annoying; disagreeable. [Obs.] Watch the noyous night, and wait for Spenser.",
  "immerit - Want of worth; demerit. [R.] Suckling.",
  "whiteboyism - The conduct or principle of the Whiteboys.",
  "ascribable - Capable of being ascribed; attributable.",
  "eludible - Capable of being eluded; evadible.",
  "dorsel - 1. A pannier. 2. Same as Dorsal, n.",
  "blood vessel - Any vessel or canal in which blood circulates in an animal, as an artery or vein.",
  "logometric - Serving to measure or ascertain chemical equivalents; stoichiometric. [R.]",
  "prescapular - (Anat.) Of or pertaining to the prescapula; supraspinous.",
  "sahib - A respectful title or appelation given to Europeans of rank. [India]",
  "sillyhow - A caul. See Caul, n., 3. [Obs. or Prov. Eng. & Scot.]",
  "bergander - A European duck (Anas tadorna). See Sheldrake.",
  "gatherable - Capable of being gathered or collected; deducible from premises. [R.] Godwin.",
  "invaluable - Valuable beyond estimation; inestimable; priceless; precious.",
  "unhap - Ill luck; misfortune. [Obs.] \"The cause of her unhap.\" Sir P. Sidney.",
  "carpellary - Belonging to, forming, or containing carpels.",
  "dod - To cut off, as wool from sheep's tails; to lop or clip off. Halliwell.",
  "hamulose - Bearing a small hook at the end. Gray.",
  "imbank - To inclose or defend with a bank or banks. See Embank.",
  "uroscopy - The diagnosis of diseases by inspection of urine. Sir T. Browne.",
  "disadventurous - Unprosperous; unfortunate. [Obs.] Spenser.",
  "chapareras - Same as Chaparajos. [Sp. Amer.]",
  "eupittonic - Pertaining to, or derived from, eupittone.",
  "ai - The three-toed sloth (Bradypus tridactylus) of South America. See Sloth.",
  "extrageneous - Belonging to another race or kind.",
  "depender - One who depends; a dependent.",
  "petaliform - Having the form of a petal; petaloid; petal-shaped.",
  "loanmonger - A dealer in, or negotiator of, loans. The millions of the loanmonger. Beaconsfield.",
  "nuisancer - One who makes or causes a nuisance.",
  "bridegroom - A man newly married, or just about to be married.",
  "indecence - See Indecency. [Obs.] \"An indecence of barbarity.\" Bp. Burnet.",
  "to-do - Bustle; stir; commotion; ado. [Colloq.]",
  "moot-hall - A hall for public meetings; a hall of judgment. [Obs.] \"The moot-hall of Herod.\" Wyclif.",
  "unbeget - To deprive of existence. Dryden.",
  "jippo - A waistcoat or kind of stays for women.",
  "premunition - The act of fortifying or guarding against objections. [Obs.]",
  "modernist - One who admires the moderns, or their ways and fashions.",
  "principia - First principles; fundamental beginnings; elements; as. Newton's Principia.",
  "sylvanium - An old name for tellurium. [Written also silvanium.]",
  "well-born - Born of a noble or respect able family; not of mean birth.",
  "sunstone - Aventurine feldspar. See under Aventurine.",
  "avengeress - A female avenger. [Obs.] Spenser.",
  "cagit - A king of parrot, of a beautiful green color, found in the Philippine Islands.",
  "recuperation - Recovery, as of anything lost, especially of the health or strength.",
  "zounds - An exclamation formerly used as an oath, and an expression of anger or wonder.",
  "boomdas - A small African hyracoid mammal (Dendrohyrax arboreus) resembling the daman.",
  "expectable - That may be expected or looked for. Sir T. Browne.",
  "nisus - A striving; an effort; a conatus. A nisus or energizing towards a presented object. Hickok.",
  "seemlyhed - Comely or decent appearance. [Obs.] Rom. of R. Spenser.",
  "lightless - Destitute of light; dark. Shak.",
  "vorticose - Vortical; whirling; as, a vorticose motion.",
  "balmify - To render balmy. [Obs.] Cheyne.",
  "conjubilant - Shouting together for joy; rejoicing together. [R.] Neale.",
  "affrontedly - Shamelessly. [Obs.] Bacon.",
  "hairbrush - A brush for cleansing and smoothing the hair.",
  "deputize - To appoint as one's deputy; to empower to act in one's stead; to depute.",
  "estimator - One who estimates or values; a valuer. Jer. Taylor.",
  "iconographer - A maker of images. Fairholt.",
  "continuously - In a continuous maner; without interruption. -- Con*tin\"u*ous*ness, n.",
  "brachycephaly - The state or condition of being brachycephalic; shortness of head.",
  "intercoming - The act of coming between; intervention; interference. [Obs.]",
  "bilocular - Divided into two cells or compartments; as, a bilocular pericarp. Gray.",
  "tautophonical - Pertaining to, or characterized by, tautophony; repeating the same sound.",
  "bordello - A brothel; a bawdyhouse; a house devoted to prostitution. [Obs.] B. Jonson.",
  "boswellian - Relating to, or characteristic of, Boswell, the biographer of Dr. Johnson.",
  "hornedness - The condition of being horned.",
  "obstetrication - The act of assisting as a midwife; delivery. [Obs.] Bp. Hall.",
  "bedote - To cause to dote; to deceive. [Obs.] Chaucer.",
  "mesometrium - The fold of the peritoneum supporting the oviduct.",
  "circumforaneous - Going about or abroad; walking or wandering from house to house. Addison.",
  "cornicular - A secretary or clerk. [Obs.] Chaucer.",
  "preprovide - To provide beforehand. \"The materials preprovided.\" Fuller.",
  "puntello - One of the points sometimes drilled as guides for cutting away superfluous stone.",
  "trapeziform - Having the form of a trapezium; trapezoid.",
  "temptress - A woman who entices. She was my temptress, the foul provoker. Sir W. Scott.",
  "affrontee - One who receives an affront. Lytton.",
  "sun-dried - Dried by the heat of the sun. \"Sun-dried brick.\" Sir T. Herbert.",
  "butter-scotch - A kind of candy, mainly composed of sugar and butter. [Colloq.] Dickens.",
  "quannet - A flat file having the handle at one side, so as to be used like a plane.",
  "gastornis - A genus of large eocene birds from the Paris basin.",
  "leitmotif - See Leading motive, under Leading, a.",
  "panhellenism - A scheme to unite all the Greeks in one political body.",
  "ultimity - The last stage or consequence; finality. [Obs.] Bacon.",
  "respirability - The quality or state of being respirable; respirableness.",
  "nasute - 1. Having a nice sense of smell. [Obs.] Evelyn. 2. Critically nice; captious. [Obs.] auden.",
  "lupuline - An alkaloid extracted from hops as a colorless volatile liquid.",
  "incute - To strike or stamp in. [Obs.] Becon.",
  "pantingly - With palpitation or rapid breathing. Shak.",
  "vibrograph - An instrument to observe and record vibrations.",
  "sleevehand - The part of a sleeve nearest the hand; a cuff or wristband. [Obs.] Shak.",
  "figary - A frolic; a vagary; a whim. [Obs.] Beau. & Fl.",
  "disadorn - To deprive of ornaments. Congreve.",
  "tillman - A man who tills the earth; a husbandman. [Obs.] Tusser.",
  "unpick - To pick out; to undo by picking.",
  "puce - Of a dark brown or brownish purple color.",
  "ostracean - Any one of a family of bivalves, of which the oyster is the type.",
  "sextonship - The office of a sexton. Swift.",
  "suability - Liability to be sued; the state of being subjected by law to civil process.",
  "furibundal - Full of rage. [Obs.] G. Harvey.",
  "affably - In an affable manner; courteously.",
  "exantlation - Act of drawing out ; exhaustion. [Obs.] Sir T. Browne.",
  "complotter - One joined in a plot. Dryden.",
  "egg-shaped - Resembling an egg in form; ovoid.",
  "rejudge - To judge again; to re Rejudge his acts, and dignify disgrace. Pope.",
  "miscellanarian - Of or pertaining to miscellanies. Shaftesbury. -- n.  A writer of miscellanies.",
  "triflorous - Three-flowered; having or bearing three flowers; as, a triflorous peduncle.",
  "misdoubtful - Misgiving; hesitating. [Obs.] \"Her misdoubtful mind.\" Spenser.",
  "musci - An order or subclass of cryptogamous plants; the mosses. See Moss, and Cryptogamia.",
  "mis - Wrong; amiss. [Obs.] \"To correcten that [which] is mis.\" Chaucer.",
  "uniaxal - Uniaxial. -- U`ni*ax\"al*ly, adv.",
  "achiote - Seeds of the annotto tree; also, the coloring matter, annotto.",
  "hopingly - In a hopeful manner. Hammond.",
  "mackinaw blanket - A thick blanket formerly in common use in the western part of the United States.",
  "albuminoidal - Of the nature of an albuminoid.",
  "zincode - The positive electrode of an electrolytic cell; anode. [R.] Miller.",
  "unlust - Listlessness; disinclination. [Obs.] \"Idleness and unlust.\" Chaucer.",
  "pinguitude - Fatness; a growing fat; obesity. [R.]",
  "copsewood - Brushwood; coppice. Macaulay.",
  "promiscuity - Promiscuousness; confusion. H. Spencer.",
  "dovelike - Mild as a dove; gentle; pure and lovable. Longfellow.",
  "fridge - To rub; to fray. [Obs.] Sterne.",
  "toyhouse - A house for children to play in or to play with; a playhouse.",
  "reaccess - A second access or approach; a return. Hakewill.",
  "superproportion - Overplus or excess of proportion. Sir K. Digby.",
  "bicrescentic - Having the form of a double crescent.",
  "arthrotome - A strong scalpel used in the dissection of joints.",
  "woodwall - The yaffle. [Written also woodwale, and woodwele.]",
  "bureaucratist - An advocate for , or supporter of, bureaucracy.",
  "blockader - 1. One who blockades. 2. (Naut.)  A vessel employed in blockading.",
  "athalamous - Not furnished with shields or beds for the spores, as the thallus of certain lichens.",
  "tetrasyllable - A word consisting of four syllables; a quadrisyllable.",
  "feitsui - The Chinese name for a highly prized variety of pale green jade. See Jade.",
  "immensely - In immense manner or degree.",
  "gaselier - A chandelier arranged to burn gas.",
  "indistinctive - Having nothing distinctive; common. -- In`dis*tinc\"tive*ness, n.",
  "gauffre - A gopher, esp. the pocket gopher.",
  "gairishness - Same as Garish, Garishly, Garishness.",
  "wull - See 2d Will. Pour out to all that wull. Spenser.",
  "adulator - A servile or hypocritical flatterer. Carlyle.",
  "dermaptera - See Dermoptera, Dermopteran.",
  "prismoid - A body that approaches to the form of a prism.",
  "tarpan - A wild horse found in the region of the Caspian Sea.",
  "polyphagy - The practice or faculty of subsisting on many kinds of food.",
  "vanfess - A ditch on the outside of the counterscarp, usually full of water.",
  "myelencephalic - Of or pertaining to the myelencephalon; cerebro-spinal.",
  "fordrive - To drive about; to drive here and there. [Obs.] Rom. of R.",
  "antependium - The hangings or screen in front of the altar; an altar cloth; the frontal. Smollett.",
  "gemmosity - The quality or characteristics of a gem or jewel. [Obs.] Bailey.",
  "bleyme - An inflammation in the foot of a horse, between the sole and the bone. [Obs.]",
  "parisyllabic - Having the same number of syllables in all its inflections.",
  "shaman - A priest of Shamanism; a wizard among the Shamanists.",
  "inerrableness - Exemption from error; inerrability; infallibility. Hammond.",
  "unseparable - Inseparable. [Obs.] \"In love unseparable.\" Shak.",
  "caroigne - Dead body; carrion. [Obs.] Chaucer.",
  "obnubilate - To cloud; to obscure. [Obs.] Burton. -- Ob*nu\"bi*la\"tion, n. [Obs.] Beddoes.",
  "signally - In a signal manner; eminently.",
  "auletic - Of or pertaining to a pipe (flute) or piper. [R.] Ash.",
  "dryly - In a dry manner; not succulently; without interest; without sympathy; coldly.",
  "hypodactylum - The under side of the toes.",
  "encrinital - Relating to encrinites; containing encrinites, as certain kinds of limestone.",
  "determinator - One who determines. [R.] Sir T. Browne.",
  "inconsummate - Not consummated; not finished; incomplete. Sir M. Hale. -- In`con*sum\"mate*ness, n.",
  "osages - A tribe of southern Sioux Indians, now living in the Indian Territory.",
  "gothamite - 1. A gothamist. 2. An inhabitant of New York city. [Jocular] Irving.",
  "bipartible - Capable of being divided into two parts.",
  "uncape - To remove a cap or cape from. [Obs.]",
  "albescence - The act of becoming white; whitishness.",
  "wit-snapper - One who affects repartee; a wit-cracker. [Obs.] Shak.",
  "speir - To ask. See Spere. Sir W. Scott.",
  "garum - A sauce made of small fish. It was prized by the ancients.",
  "taglia - A peculiar combination of pulleys. Brande & C.",
  "interwish - To wish mutually in regarded to each other. [Obs.] Donne.",
  "repeller - One who, or that which, repels.",
  "refluctuation - A flowing back; refluence.",
  "jetter - One who struts; one who bears himself jauntily; a fop. [Obs.] Palsgrave.",
  "transmutationist - One who believes in the transmutation of metals or of species.",
  "hydrometallurgy - The art or process of assaying or reducing ores by means of liquid reagents.",
  "teetotaler - One pledged to entire abstinence from all intoxicating drinks.",
  "damassin - A kind of modified damask or blocade.",
  "by-passage - A passage different from the usual one; a byway.",
  "quibbler - One who quibbles; a caviler; also, a punster.",
  "glazen - Resembling glass; glasslike; glazed. [Obs.] Wyclif.",
  "lobulate - Made up of, or divided into, lobules; as, a lobulated gland.",
  "exaggerated - Enlarged beyond bounds or the truth. -- Ex*ag\"ger*a`ted*ly, adv.",
  "opalotype - A picture taken on \"milky\" glass.",
  "castanea - A genus of nut-bearing trees or shrubs including the chestnut and chinquapin.",
  "introspect - To look into or within; to view the inside of. Bailey.",
  "armchair - A chair with arms to support the elbows or forearms. Tennyson.",
  "modiolus - The central column in the osseous cochlea of the ear.",
  "phalansterianism - A system of phalansteries proposed by Fourier; Fourierism.",
  "hypsometrical - Of or pertaining to hypsometry.",
  "luniform - Resembling the moon in shape.",
  "undwellable - Uninhabitable. [Obs.] \"A land undwellable.\" Wyclif.",
  "mortrew - A dish of meats and other ingredients, cooked together; an ollapodrida. Chaucer. Bacon.",
  "phenocryst - One of the prominent embedded crystals of a porphyry.",
  "triassic - Of the age of, or pertaining to, the Trias. -- n.  The Triassic formation.",
  "fretten - Rubbed; marked; as, pock-fretten, marked with the smallpox. [Obs.] Wright.",
  "latrine - A privy, or water-closet, esp. in a camp, hospital, etc.",
  "dendrologist - One versed in the natural history of trees.",
  "steatoma - A cyst containing matter like suet.",
  "uncult - Not cultivated; rude; illiterate. [Obs.]",
  "eyeshot - Range, reach, or glance of the eye; view; sight; as, to be out of eyeshot. Dryden.",
  "gastropod - One of the Gastropoda. [Written also gasteropod.]",
  "superchery - Deceit; fraud; imposition. [Obs. & R.]",
  "affranchisement - The act of making free; enfranchisement. [R.]",
  "azotize - To impregnate with azote, or nitrogen; to nitrogenize.",
  "opener - One who, or that which, opens. \"True opener of my eyes.\" Milton.",
  "designatory - Serving to designate; designative; indicating. [R.]",
  "chronogrammatic - Belonging to a chronogram, or containing one.",
  "prial - A corruption of pair royal. See under Pair, n.",
  "pragmaticalness - The quality or state of being pragmatical.",
  "apomecometer - An instrument for measuring the height of objects. Knight.",
  "transisthmian - Extending across an isthmus, as at Suez or Panama.",
  "sula - A genus of sea birds including the booby and the common gannet.",
  "toiler - One who toils, or labors painfully.",
  "flossification - A flowering; florification. [R.] Craig.",
  "electro-dynamometer - An instrument for measuring the strength of electro-dynamic currents.",
  "myaria - A division of bivalve mollusks of which the common clam (Mya) is the type.",
  "cartage - 1. The act of carrying in a cart. 2. The price paid for carting.",
  "squamula - One of the little hypogynous scales found in the flowers of grasses; a lodicule.",
  "phalangious - Of or pertaining to Phalangoidea.",
  "hellbroth - A composition for infernal purposes; a magical preparation. Shak.",
  "insection - A cutting in; incisure; incision.",
  "quatorzain - A poem of fourteen lines; a sonnet. R. H. Stoddard.",
  "attracter - One who, or that which, attracts.",
  "vacuolation - Formation into, or multiplication of, vacuoles.",
  "sectarianize - To imbue with sectarian feelings; to subject to the control of a sect.",
  "unthrifty - Not thrifty; profuse. Spenser.",
  "brickyard - A place where bricks are made, especially an inclosed place.",
  "unpassionate - Not passionate; dispassionate. -- Un*pas\"sion*ate*ly, adv.",
  "snobocracy - Snobs, collectively. [Hybrid & Recent] C. Kingsley.",
  "draperied - Covered or supplied with drapery. [R.] Byron.",
  "monarchism - The principles of, or preference for, monarchy.",
  "ponderation - The act of weighing. [R.] Arbuthnot.",
  "urethra - The canal by which the urine is conducted from the bladder and discharged.",
  "tetrapetalous - Containing four distinct petals, or flower leaves; as, a tetrapetalous corolla.",
  "invalidism - The condition of an invalid; sickness; infirmity.",
  "lampyris - A genus of coleopterous insects, including the glowworms.",
  "partialism - Partiality; specifically (Theol.), the doctrine of the Partialists.",
  "superexcination - Excessive, or more than normal, excitation.",
  "ozonoscope - An apparatus employed to indicate the presence, or the amount, of ozone.",
  "polyonomous - Having many names or titles; polyonymous. Sir W. Jones.",
  "rubblework - Masonry constructed of unsquared stones that are irregular in size and shape.",
  "hymenopterous - Like, or characteristic of, the Hymenoptera; pertaining to the Hymenoptera.",
  "cruentate - Smeared with blood. [Obs.] Glanwill.",
  "eductive - Tending to draw out; extractive.",
  "outspeed - To excel in speed. Outspeed the realized miracles of steam. Talfourd.",
  "vulnerableness - The quality or state of being vulnerable; vulnerability.",
  "pyridic - Related to, or formed from, pyridin or its homologues; as, the pyridic bases.",
  "catastaltic - Checking evacutions through astringent or styptic qualities.",
  "historize - To relate as history; to chronicle; to historicize. [R.] Evelyn.",
  "pyramidion - The small pyramid which crowns or completes an obelisk.",
  "irresistance - Nonresistance; passive submission.",
  "kiddier - A huckster; a cadger. [Obs.] Halliwell.",
  "syncotyledonous - Having united cotyledonous.",
  "discountenancer - One who discountenances; one who disfavors. Bacon.",
  "preassurance - Previous assurance. Coleridge.",
  "disunity - A state of separation or disunion; want of unity. Dr. H. More.",
  "departer - 1. One who refines metals by separation. [Obs.] 2. One who departs.",
  "superexalt - To exalt to a superior degree; to exalt above others. Barrow.",
  "unmerciless - Utterly merciless. [Obs.] Joye.",
  "sepic - Of or pertaining to sepia; done in sepia; as, a sepic drawing.",
  "cockfight - A match or contest of gamecocks.",
  "ook - Oak. [Obs.] \"A branched ook.\" Chaucer.",
  "bed-molding - The molding of a cornice immediately below the corona. Oxf. Gloss.",
  "confoundedness - The state of being confounded. Their witty descant of my confoundedness. Milton.",
  "carbimide - The technical name for isocyanic acid. See under Isocyanic.",
  "unpowerful - Not powerful; weak. Cowley.",
  "upwhirl - To rise upward in a whirl; to raise upward with a whirling motion.",
  "branchiostoma - The lancelet. See Amphioxus.",
  "fumishness - Choler; fretfulness; passion.",
  "amphicarpic - Producing fruit of two kinds, either as to form or time of ripening.",
  "frustulent - Abounding in fragments. [R.]",
  "subcentral - 1. Under the center. 2. Nearly central; not quite central.",
  "colloquialize - To make colloquial and familiar; as, to colloquialize one's style of writing.",
  "purveiaunce - Purveyance. [Obs.] Chaucer.",
  "garookuh - A small fishing vessel met with in the Persian Gulf.",
  "anhydrous - Destitute of water; as, anhydrous salts or acids.",
  "transmeation - The act of transmeating; a passing through or beyond. [Obs.]",
  "exudate - To exude. [Obs.] Sir T. Browne.",
  "microcosmography - Description of man as a microcosm.",
  "dracanth - A kind of gum; -- called also gum tragacanth, or tragacanth. See Tragacanth.",
  "sea slug - (a) A holothurian. (b) A nudibranch mollusk.",
  "erf - A garden plot, usually about half an acre. [Cape Colony]",
  "monadic - Of, pertaining to, or like, a monad, in any of its senses. See Monad, n. Dr. H. More.",
  "exannulate - Having the sporangium destitute of a ring; -- said of certain genera of ferns.",
  "achievable - Capable of being achieved. Barrow.",
  "slubberingly - In a slovenly, or hurried and imperfect, manner. [Low] Drayton.",
  "idiothermic - Self-heating; warmed, as the body of animal, by process going on within itself.",
  "indignancy - Indignation. [Obs.] Spenser.",
  "clavier - The keyboard of an organ, pianoforte, or harmonium. Note: Clavier (",
  "discriminateness - The state of being discriminated; distinctness.",
  "silliness - The quality or state of being silly.",
  "feather-veined - Having the veins (of a leaf) diverging from the two sides of a midrib.",
  "dulcorate - To sweeten; to make less acrimonious. [R.] Bacon.",
  "copalm - The yellowish, fragrant balsam yielded by the sweet gum; also, the tree itself.",
  "mausolean - Pertaining to a mausoleum; monumental.",
  "originant - Originating; original. [R.] An absolutely originant act of self will. Prof. Shedd.",
  "lacertus - A bundle or fascicle of muscular fibers.",
  "disconducive - Not conductive; impeding; disadvantageous. [R.]",
  "cockyoly bird - A pet name for any small bird.",
  "goody-goody - Mawkishly or weakly good; exhibiting goodness with silliness. [Colloq.]",
  "indomptable - Indomitable. [Obs.] Tooke.",
  "religiously - In a religious manner. Drayton.",
  "broadax - 1. An ancient military weapon; a battle-ax. 2. An ax with a broad edge, for hewing timber.",
  "brouded - Braided; broidered. [Obs.] Alle his clothes brouded up and down. Chaucer.",
  "immodesty - Want of modesty, delicacy, or decent reserve; indecency. \"A piece of immodesty.\" Pope.",
  "suddenty - Suddenness; a sudden. [Scot.] On a suddenty, on a sudden. [Scot.] Sir W. Scott.",
  "exhilarant - Exciting joy, mirth, or pleasure. -- n.  That which exhilarates.",
  "turbidness - The quality or state of being turbid; muddiness; foulness.",
  "gneissic - Relating to, or resembling, gneiss; consisting of gneiss.",
  "pocan - The poke (Phytolacca decandra); -- called also pocan bush.",
  "accusatory - Pertaining to, or containing, an accusation; as, an accusatory libel. Grote.",
  "eprouvette - An apparatus for testing or proving the strength of gunpowder.",
  "goarish - Patched; mean. [Obs.] Beau. & Fl.",
  "arrastre - A rude apparatus for pulverizing ores, esp. those containing free gold.",
  "lira - An Italian coin equivalent in value to the French franc.",
  "interestingness - The condition or quality of being interesting. A. Smith.",
  "anginose - Pertaining to angina or angina pectoris.",
  "evomition - The act of vomiting. [Obs.] Swift.",
  "fluophosphate - A double salt of fluoric and phosphoric acids.",
  "hierolatry - The worship of saints or sacred things. [R.] Coleridge.",
  "acanthopterygious - Having fins in which the rays are hard and spinelike; spiny- finned.",
  "aetheogamous - Propagated in an unusual way; cryptogamous.",
  "parisyllabical - Having the same number of syllables in all its inflections.",
  "empire state - New York; -- a nickname alluding to its size and wealth.",
  "penitentiaryship - The office or condition of a penitentiary of the papal court. [R.] Wood.",
  "devast - To devastate. [Obs.] Bolingbroke.",
  "verdingale - See Farthingale. [Spelled also verdingall.] [Obs.]",
  "glossoepiglottic - Pertaining to both tongue and epiglottis; as, glossoepiglottic folds.",
  "schoolhouse - A house appropriated for the use of a school or schools, or for instruction.",
  "cheesiness - The quality of being cheesy.",
  "harlock - Probably a corruption either of charlock or hardock. Drayton.",
  "querquedule - (a) A teal. (b) The pintail duck.",
  "tragedienne - A woman who plays in tragedy.",
  "sidepiece - The jamb, or cheek, of an opening in a wall, as of door or window.",
  "postrider - One who rides over a post road to carry the mails. Bancroft.",
  "fructure - Use; fruition; enjoyment. [Obs.] Cotgrave.",
  "reboant - Rebellowing; resounding loudly. [R.] Mrs. Browning.",
  "preventable - Capable of being prevented or hindered; as, preventable diseases.",
  "seminific - Forming or producing seed, or the male generative product of animals or of plants.",
  "debentured - Entitled to drawback or debenture; as, debentured goods.",
  "hedging bill - A hedge bill. See under Hedge.",
  "knapbottle - The bladder campion (Silene inflata).",
  "self-reverence - A reverent respect for one's self. Tennyson.",
  "pluteal - Of or pertaining to a pluteus.",
  "foralite - A tubelike marking, occuring in sandstone and other strata.",
  "pharmacologist - One skilled in pharmacology.",
  "undershrievalty - The office or position of an undersheriff.",
  "ensnare - To catch in a snare. See Insnare.",
  "olid - Having a strong, disagreeable smell; fetid. [Obs.] Boyle. Sir T. Browne.",
  "buttonweed - The name of several plants of the genera Spermacoce and Diodia, of the Madder family.",
  "derring - Daring or warlike. [Obs.] Drad for his derring doe and bloody deed. Spenser.",
  "unsped - Not performed; not dispatched. [Obs.] Garth.",
  "monodrama - A drama acted, or intended to be acted, by a single person.",
  "sheepy - Resembling sheep; sheepish. Testament of Love.",
  "anemometer - An instrument for measuring the force or velocity of the wind; a wind gauge.",
  "oviposition - The depositing of eggs, esp. by insects.",
  "honeysucker - See Honey eater, under Honey.",
  "papescent - Containing or producing pap; like pap. [R.] Arbuthnot.",
  "smickly - Smugly; finically. [Obs.] Ford.",
  "widow-wail - A low, narrowleaved evergreen shrub (Cneorum tricoccon) found in Southern Europe.",
  "diagraph - A drawing instrument, combining a protractor and scale.",
  "ambler - A horse or a person that ambles.",
  "improficiency - Want of proficiency. [R.] Bacon.",
  "papess - A female pope; i. e., the fictitious pope Joan. [Obs.] Bp. Hall.",
  "bibulously - In a bibulous manner; with profuse imbibition or absorption. De Quincey.",
  "muteness - The quality or state of being mute; speechlessness.",
  "vandyke beard - A trim, pointed beard, such as those often seen in pictures by Vandyke.",
  "acropodium - The entire upper surface of the foot.",
  "sleety - Of or pertaining to sleet; characterized by sleet; as, a sleety storm; sleety weather.",
  "liberalization - The act of liberalizing.",
  "indicolite - A variety of tourmaline of an indigo-blue color.",
  "neorama - A panorama of the interior of a building, seen from within.",
  "maleficience - The doing of evil, harm, or mischief.",
  "cope-chisel - A narrow chisel adapted for cutting a groove. Knight.",
  "syllabify - To form or divide into syllables.",
  "photothermic - Of or pertaining to both light and heat.",
  "olf - The European bullfinch. [Prov.Eng.]",
  "self-communion - Communion with one's self; thoughts about one's self.",
  "papaphobia - Intense fear or dread of the pope, or of the Roman Catholic Church. [R.]",
  "esteemable - Worthy of esteem; estimable. [R.] \"Esteemable qualities.\" Pope.",
  "arborized - Having a treelike appearance. \"An arborized or moss agate.\" Wright.",
  "arachnological - Of or pertaining to arachnology.",
  "bridgepot - The adjustable socket, or step, of a millstone spindle. Knight.",
  "weatherwise - Skillful in forecasting the changes of the weather. Hakluyt.",
  "paleobotanist - One versed in paleobotany.",
  "antibacchius - A foot of three syllables, the first two long, and the last short.",
  "softener - One who, or that which, softens. [Written also, less properly, softner.]",
  "stance - 1. A stanza. [Obs.] Chapman. 2. A station; a position; a site. [Scot.] Sir W. Scott.",
  "subquadrate - Nearly or approximately square; almost square.",
  "ballahoo - A fast-sailing schooner, used in the Bermudas and West Indies.",
  "astraddle - In a straddling position; astride; bestriding; as, to sit astraddle a horse.",
  "allah - The name of the Supreme Being, in use among the Arabs and the Mohammedans generally.",
  "ergon - (a) Work, measured in terms of the quantity of heat to which it is equivalent. (b) = Erg.",
  "superpraise - To praise to excess. To vow, and swear, and superpraise my parts. Shak.",
  "earthliness - The quality or state of being earthly; worldliness; grossness; perishableness.",
  "arrasene - A material of wool or silk used for working the figures in embroidery.",
  "meteorologist - A person skilled in meteorology.",
  "outlimb - , An extreme member or part of a thing; a limb. [Obs.] Fuller.",
  "neologistical - of or pertaining to neology; neological.",
  "polyzooen - One of the individual zooids forming the compound organism of a polyzoan.",
  "majesticness - The quality or state of being majestic. Oldenburg.",
  "gaseity - State of being gaseous. [R] Eng. Cyc.",
  "sclerotal - Sclerotic. -- n.  The optic capsule; the sclerotic coat of the eye. Owen.",
  "uncapper - An instrument for removing an explode cap from a cartridge shell.",
  "hermitary - A cell annexed to an abbey, for the use of a hermit. Howell.",
  "discompt - To discount. See Discount. Hudibras.",
  "colluder - One who conspires in a fraud.",
  "picturable - Capable of being pictured, or represented by a picture.",
  "incommodation - The state of being incommoded; inconvenience. [Obs.]",
  "horological - Relating to a horologe, or to horology.",
  "keddah - An inclosure constructed to entrap wild elephants; an elephant trap. [India]",
  "side-taking - A taking sides, as with a party, sect, or faction. Bp. Hall.",
  "vetchling - Any small leguminous plant of the genus Lathyrus, especially L. Nissolia.",
  "irremunerable - Not remunerable; not capable of remuneration.",
  "bacteriologist - One skilled in bacteriology.",
  "specifiable - Admitting specification; capable of being specified.",
  "jurisdictive - Having jurisdiction. Milton.",
  "agminate - Grouped together; as, the agminated glands of Peyer in the small intestine.",
  "diathermal - Freely permeable by radiant heat.",
  "fiendlike - Fiendish; diabolical. Longfellow.",
  "unordinate - Disorderly; irregular; inordinate. [R.] -- Un*or\"di*nate*ly, adv. [R.]",
  "quinquevalve - Having five valves, as a pericarp.",
  "pluriparous - Producing several young at a birth; as, a pluriparous animal.",
  "underhead - A blockhead, or stupid person; a dunderhead. [Obs.] Sir T. Browne.",
  "wrongdoing - Evil or wicked behavior or action.",
  "torpify - To make torpid; to numb, or benumb.",
  "inequitable - Not equitable; not just. Burke.",
  "tilbury - A kind of gig or two-wheeled carriage, without a top or cover. [Written also tilburgh.]",
  "neglection - The state of being negligent; negligence. [Obs.] Shak.",
  "orchardist - One who cultivates an orchard.",
  "entackle - To supply with tackle. [Obs.] Skelton.",
  "remissness - Quality or state of being remiss.",
  "mostwhat - For the most part. [Obs.] \"All the rest do mostwhat far amiss.\" Spenser.",
  "dented - Indented; impressed with little hollows.",
  "enswathe - To swathe; to envelop, as in swaddling clothes. Shak.",
  "relinquishment - The act of relinquishing.",
  "achatina - A genus of land snails, often large, common in the warm parts of America and Africa.",
  "alarmist - One prone to sound or excite alarms, especially, needless alarms. Macaulay.",
  "subpolygonal - Approximately polygonal; somewhat or almost polygonal.",
  "inoperculate - Having no operculum; -- said of certain gastropod shells.",
  "leptocardian - Of or pertaining to the Leptocardia. -- n.  One of the Leptocardia.",
  "tim-whiskey - A kind of carriage. See Whiskey. Southery.",
  "euganoidei - A group which includes the bony ganoids, as the gar pikes.",
  "creable - Capable of being created. [Obs.] I. Watts.",
  "chickaree - The American red squirrel (Sciurus Hudsonius); -- so called from its cry.",
  "oomiac - A long, broad boat used by the Eskimos.",
  "classmate - One who is in the same class with another, as at school or college.",
  "caravaneer - The leader or driver of the camels in caravan.",
  "tablement - A table. [Obs.] Tablements and chapters of pillars. Holland.",
  "remindful - Tending or adapted to remind; careful to remind. Southey.",
  "disbranch - To divest of a branch or branches; to tear off. Shak.",
  "sighing - Uttering sighs; grieving; lamenting. \"Sighing millions.\" Cowper. -- Sigh\"ing*ly, adv.",
  "convertibleness - The state of being convertible; convertibility.",
  "firebote - An allowance of fuel. See Bote.",
  "nappiness - The quality of having a nap; abundance of nap, as on cloth.",
  "surprisement - Surprisal. [Obs.] Daniel.",
  "waterless - Destitute of water; dry. Chaucer.",
  "trepanize - To trepan. [Obs.] \"By trepanizing the skull.\" Jer. Taylor.",
  "stout-hearted - Having a brave heart; courageous. -- Stout\"-heart\"ed*ness, n.",
  "reconstructive - Reconstructing; tending to reconstruct; as, a reconstructive policy.",
  "demonial - Of or pertaining to a demon. [Obs.] Cudworth.",
  "psalmography - The act or practice of writing psalms, or sacred songs.",
  "air brake - A railway brake operated by condensed air. Knight.",
  "achatour - Purveyor; acater. [Obs.] Chaucer.",
  "subworker - A subordinate worker or helper. South.",
  "disme - A tenth; a tenth part; a tithe. Ayliffe.",
  "zehner - An Austrian silver coin equal to ten kreutzers, or about five cents.",
  "fancywork - Ornamental work with a needle or hook, as embroidery, crocheting, netting, etc.",
  "marmose - A species of small opossum (Didelphus murina) ranging from Mexico to Brazil.",
  "hygrophanous - Having such a structure as to be diaphanous when moist, and opaque when dry.",
  "apostleship - The office or dignity of an apostle.",
  "huntress - A woman who hunts or follows the chase; as, the huntress Diana. Shak.",
  "ellenge - See Elenge, Elengeness. [Obs.]",
  "filmy - Composed of film or films. Whose filmy cord should bind the struggling fly. Dryden.",
  "immaterialize - To render immaterial or incorporeal. Immateralized spirits. Glanvill.",
  "autobiographer - One who writers his own life or biography.",
  "goodless - Having no goods. [Obs.] Chaucer.",
  "morkin - A beast that has died of disease or by mischance. [Obs.] Bp. Hall.",
  "tofus - 1. Tophus. 2. (Min.)  Tufa. See under Tufa, and Toph.",
  "cenotaphy - A cenotaph. [R.] Lord Cobham honored him with a cenotaphy. Macaulay.",
  "single-handed - Having but one hand, or one workman; also, alone; unassisted.",
  "uranitic - Of or pertaining to uranium; containing uranium.",
  "forestaller - One who forestalls; esp., one who forestalls the market. Locke.",
  "autotoxication - Same as Auto-intoxication.",
  "thermotypy - The art or process of obtaining thermotypes.",
  "galvanocautery - Cautery effected by a knife or needle heated by the passage of a galvanic current.",
  "orabassu - A South American monkey of the genus Callithrix, esp. C. Moloch.",
  "duodenary - Containing twelve; twelvefold; increasing by twelves; duodecimal.",
  "pandanus - A genus of endogenous plants. See Screw pine.",
  "interurban - Going between, or connecting, cities or towns; as, interurban electric railways.",
  "fool-largesse - Foolish expenditure; waste. [Obs.] Chaucer.",
  "by-room - A private room or apartment. \"Stand in some by-room\" Shak.",
  "clinanthium - The receptacle of the flowers in a composite plant; -- also called clinium.",
  "semi-arianism - The doctrines or tenets of the Semi-Arians.",
  "prima donna - The first or chief female singer in an opera.",
  "pulmoniferous - Having lungs; pulmonate.",
  "maat - Dejected; sorrowful; downcast. [Obs.] \"So piteous and so maat.\" Chaucer.",
  "percipience - The faculty, act or power of perceiving; perception. Mrs. Browning.",
  "unpersuasion - The state of not being persuaded; disbelief; doubt. [R.] Abp. Leighton.",
  "soldanrie - The country ruled by a soldan, or sultan. [Poet.] Sir W. Scott.",
  "forevouched - Formerly vouched or avowed; affirmed in advance. [R.] Shak.",
  "needlestone - Natrolite; -- called also needle zeolite.",
  "disordeined - Inordinate; irregular; vicious. [Obs.] Chaucer.",
  "baculine - Of or pertaining to the rod or punishment with the rod.",
  "lepered - Affected or tainted with leprosy.",
  "teemer - One who teems, or brings forth.",
  "splaymouth - A wide mouth; a mouth stretched in derision. Dryden.",
  "subcranial - Situated under, or on the ventral side of, the cranium; facial.",
  "sympathetically - In a sympathetic manner.",
  "disoccident - To turn away from the west; to throw out of reckoning as to longitude. [Obs.] Marvell.",
  "uncous - Hooklike; hooked. [Obs.] Sir T. Browne.",
  "influencer - One who, or that which, influences.",
  "shopboard - A bench or board on which work is performed; a workbench. South.",
  "speet - To stab. [Obs.] Gammer Gurton's Needle.",
  "monologist - One who soliloquizes; esp., one who monopolizes conversation in company. De Quincey.",
  "auxetic - Pertaining to, or containing, auxesis; amplifying.",
  "durylic - Pertaining to, allied to, or derived from, durene; as, durylic acid.",
  "unwonder - To divest of the quality of wonder or mystery; to interpret; to explain. [R.] Fuller.",
  "subhastation - A public sale or auction. [R.] Bp. Burnet.",
  "clergical - Of or pertaining to the clergy; clerical; clerkily; learned. [Obs.] Milton.",
  "constat - A certificate showing what appears upon record touching a matter in question.",
  "mispoint - To point improperly; to punctuate wrongly.",
  "nectostem - That portion of the axis which bears the nectocalyces in the Siphonophora.",
  "brahminism - The religion or system of doctrines of the Brahmans; the religion of Brahma.",
  "tellural - Of or pertaining to the earth. [R.]",
  "conchal - Pertaining to the concha, or external ear; as, the conchal cartilage.",
  "postnatal - After birth; subsequent to birth; as, postnatal infanticide; postnatal diseases.",
  "malleableness - Quality of being malleable.",
  "quicksilvering - The mercury and foil on the back of a looking-glass.",
  "restagnate - To stagnate; to cease to flow. [Obs.] Wiseman.",
  "spherulitic - Of or pertaining to a spherulite; characterized by the presence of spherulites.",
  "hydropathical - Of or pertaining to hydropathy.",
  "omniparity - Equality in every part; general equality.",
  "washbowl - A basin, or bowl, to hold water for washing one's hands, face, etc.",
  "aplomb - Assurance of manner or of action; self-possession.",
  "petrologic - Of or pertaining to petrology.",
  "perinephritis - Inflammation of the cellular tissue around the kidney. -- Per`i*ne*phrit\"ic, a.",
  "post- - A prefix signifying behind, back, after; as, postcommissure, postdot, postscript.",
  "phantasmatography - A description of celestial phenomena, as rainbows, etc.",
  "mesogastrium - (a) The umbilical region. (b) The mesogaster.",
  "rubiacin - A substance found in madder root, and probably identical with ruberythrinic acid.",
  "discerptible - Capable of being discerped. [R.]",
  "planching - The laying of floors in a building; also, a floor of boards or planks.",
  "water butt - A large, open-headed cask, set up on end, to contain water. Dickens.",
  "nonsolution - Failure of solution or explanation.",
  "tricurvate - Curved in three directions; as, a tricurvate spicule (see Illust. of Spicule).",
  "beton - The French name for concrete; hence, concrete made after the French fashion.",
  "barry - , Divided into bars; -- said of the field.",
  "comprehensively - In a comprehensive manner; with great extent of scope.",
  "typesetter - One who, or that which, sets type; a compositor; a machine for setting type.",
  "blond - Of a fair color; light-colored; as, blond hair; a blond complexion.",
  "anomalousness - Quality of being anomalous.",
  "lomentaceous - Of the nature of a loment; having fruits like loments.",
  "columbate - A salt of columbic acid; a niobate. See Columbium.",
  "basketful - As much as a basket will contain.",
  "scabling - A fragment or chip of stone. [Written also scabline.]",
  "tanning - The art or process of converting skins into leather. See Tan, v. t., 1.",
  "fourierite - One who adopts the views of Fourier.",
  "inthrong - To throng or collect together. [R.] Fairfax.",
  "reducibleness - Quality of being reducible.",
  "maturant - A medicine, or application, which promotes suppuration.",
  "moble - To wrap the head of in a hood. [Obs.] Shak.",
  "picoid - Like or pertaining to the Pici.",
  "emittent - Sending forth; emissive. Boyle."
]