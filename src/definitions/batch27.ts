export default [
  "rist - 3d pers. sing. pres. of Rise, contracted from riseth. Chaucer.",
  "browpost - A beam that goes across a building.",
  "spandogs - A pair of grappling dogs for hoisting logs and timber.",
  "getter - One who gets, gains, obtains, acquires, begets, or procreates.",
  "ectoplastic - Pertaining to, or composed of, ectoplasm.",
  "valerylene - A liquid hydrocarbon, C5H8; -- called also pentine.",
  "domable - Capable of being tamed; tamable.",
  "disinvigorate - To enervate; to weaken. [R.] Sydney Smith.",
  "beguiling - Alluring by guile; deluding; misleading; diverting. -- Be*guil\"ing*ly, adv.",
  "holostomata - An artificial division of gastropods, including those that have an entire aperture.",
  "chesterlite - A variety of feldspar found in crystals in the county of Chester, Pennsylvania.",
  "annuent - Nodding; as, annuent muscles (used in nodding).",
  "mammilloid - Like a mammilla or nipple; mammilliform.",
  "unqualitied - Deprived of the usual faculties. [Obs.] Shak.",
  "forbearing - Disposed or accustomed to forbear; patient; long-suffering. -- For*bear\"ing*ly, adv.",
  "unweighed - Not weighed; not pondered or considered; as, an unweighed statement.",
  "placidity - The quality or state of being placid; calmness; serenity. Hawthorne.",
  "outspread - To spread out; to expand; -- usually as a past part. or adj.",
  "vertilinear - Straight; rectilinear. [R.]",
  "inamissible - Incapable of being lost. [R.] Hammond. -- In`a*mis\"si*ble*ness, n. [R.]",
  "glozer - A flatterer. [Obs.] Gifford (1580).",
  "mismate - To mate wrongly or unsuitably; as, to mismate gloves or shoes; a mismated couple.",
  "elasmobranch - Of or pertaining to the Elasmobranchii. -- n.  One of the Elasmobranchii.",
  "bedust - To sprinkle, soil, or cover with dust. Sherwood.",
  "teleorganic - Vital; as, teleorganic functions.",
  "intreatable - Not to be entreated; inexorable.",
  "beforetime - Formerly; aforetime. [They] dwelt in their tents, as beforetime. 2 Kings xiii. 5.",
  "interlobar - Between lobes; as, the interlobar notch of the liver; the interlobar ducts of a gland.",
  "intransient - Not transient; remaining; permanent. Killingbeck.",
  "self-distrust - Want of confidence in one' self; diffidence.",
  "vitalic - Pertaining to life; vital. [R.]",
  "feoffee - The person to whom a feoffment is made; the person enfeoffed.",
  "self-sacrifice - The act of sacrificing one's self, or one's interest, for others; self-devotion.",
  "pendulate - To swing as a pendulum. [R.]",
  "alleyed - Furnished with alleys; forming an alley. \"An alleyed walk.\" Sir W. Scott.",
  "mucronulate - Having, or tipped with, a small point or points.",
  "wyclifite - A follower of Wyclif, the English reformer; a Lollard.",
  "presbyope - One who has presbyopia; a farsighted person.",
  "putage - Prostitution or fornication on the part of a woman.",
  "catlike - Like a cat; stealthily; noiselessly.",
  "temperative - Having power to temper. [R.] T. Granger.",
  "refocillation - Restoration of strength by refreshment. [Obs.] Middleton.",
  "breadcorn - Corn of grain of which bread is made, as wheat, rye, etc.",
  "dissocial - Unfriendly to society; contracted; selfish; as, dissocial feelings.",
  "philathea - An international, interdenominational organization of Bible classes of young women.",
  "scrutable - Discoverable by scrutiny, inquiry, or critical examination. [R.] r. H. More.",
  "illesive - Not injurious; harmless. [R.]",
  "circumterraneous - Being or dwelling around the earth. \"Circumterraneous demouns.\" H. Hallywell.",
  "metatarsal - Of or pertaining to the metatarsus. -- n.  A metatarsal bone.",
  "spine-finned - Having fine supported by spinous fin rays; -- said of certain fishes.",
  "cass - To render useless or void; to annul; to reject; to send away. [Obs.] Sir W. Raleing.",
  "demonic - Of or pertaining to a demon or to demons; demoniac. \"Demonic ambushes.\" Lowell.",
  "subepidermal - Situated immediately below the epidermis.",
  "serrulate - Finely serrate; having very minute teeth.",
  "strapple - To hold or bind with, or as with, a strap; to entangle. [Obs.] Chapman.",
  "tenuirostres - An artificial group of passerine birds having slender bills, as the humming birds.",
  "mammose - Having the form of the breast; breast-shaped.",
  "voe - An inlet, bay, or creek; -- so called in the Orkney and Shetland Islands. Jamieson.",
  "foggily - In a foggy manner; obscurely. Johnson.",
  "incredulously - In an incredulous manner; with incredulity.",
  "confirmer - One who, or that which, confirms, establishes, or ratifies; one who corroborates. Shak.",
  "autogenetic - Relating to autogenesis; self-generated.",
  "uraemia - Accumulation in the blood of the principles of the urine, producing dangerous disease.",
  "mordication - The act of biting or corroding; corrosion. [R.] Bacon.",
  "ranula - A cyst formed under the tongue by obstruction of the duct of the submaxillary gland.",
  "syphering - The lapping of chamfered edges of planks to make a smooth surface, as for a bulkhead.",
  "self-imposed - Voluntarily taken on one's self; as, self-imposed tasks.",
  "griffe - The offspring of a mulatto woman and a negro; also, a mulatto. [Local, U. S.]",
  "leucitic - Containing leucite; as, leucitic rocks.",
  "circled - Having the form of a circle; round. \"Monthly changes in her circled orb.\" Shak.",
  "stilar - Of or pertaining to the style of a dial. [Written also stylar.]",
  "crore - Ten millions; as, a crore of rupees (which is nearly $5,000,000). [East Indies] Malcolm.",
  "resemblant - Having or exhibiting resemblance; resembling. [R.] Gower.",
  "medalurgy - The art of making and striking medals and coins. [Written also medallurgy.]",
  "abscondence - Fugitive concealment; secret retirement; hiding. [R.] Phillips.",
  "cleromancy - A divination by throwing dice or casting lots.",
  "bromoiodism - Poisoning induced by large doses of bromine and iodine or of their compounds.",
  "grazioso - Gracefully; smoothly; elegantly.",
  "dermostosis - Ossification of the dermis.",
  "lovelorn - Forsaken by one's love. The lovelorn nightingale. Milton.",
  "dicotyledon - A plant whose seeds divide into two seed lobes, or cotyledons, in germinating.",
  "suing - The process of soaking through anything. [Obs.] Bacon.",
  "half-sword - Half the length of a sword; close fight. \"At half-sword.\" Shak.",
  "instanter - Immediately; instantly; at once; as, he left instanter.",
  "locusting - Swarming and devastating like locusts. [R.] Tennyson.",
  "pentadactyloid - Having the form of, or a structure modified from, a pentadactyl limb.",
  "air-built - Erected in the air; having no solid foundation; chimerical; as, an air-built castle.",
  "pelioma - 1. (Med.)  A livid ecchymosis. 2. (Min.)  See Peliom.",
  "baalist - A worshiper of Baal; a devotee of any false religion; an idolater.",
  "percaline - A fine kind of French cotton goods, usually of one color.",
  "monometallist - One who believes in monometallism as opposed to bimetallism, etc.",
  "reexperience - A renewed or repeated experience.",
  "woodbind - Woodbine. Dryden. A garland . . . of woodbind or hawthorn leaves. Chaucer.",
  "cartographical - Of or pertaining to cartography.",
  "heartbreak - Crushing sorrow or grief; a yielding to such grief. Shak.",
  "disreputability - The state of being disreputable. [R.]",
  "twinter - A domestic animal two winters old. [Prov. Eng.]",
  "pannose - Similar in texture or appearance to felt or woolen cloth.",
  "air engine - An engine driven by heated or by compressed air. Knight.",
  "toftman - The owner of a toft. See Toft, 3.",
  "musketry - 1. Muskets, collectively. 2. The fire of muskets. Motley.",
  "mutely - Without uttering words or sounds; in a mute manner; silently.",
  "testacea - Invertebrate animals covered with shells, especially mollusks; shellfish.",
  "factual - Relating to, or containing, facts. [R.]",
  "monocarbonic - Containing one carboxyl group; as, acetic acid is a monocarbonic acid.",
  "prosaist - A writer of prose; an unpoetical writer. \"An estimable prosaist.\" I. Taylor.",
  "scurfy - Having or producing scurf; covered with scurf; resembling scurf.",
  "infertile - Not fertile; not productive; barren; sterile; as, an infertile soil.",
  "pagod - 1. A pagoda. [R.] \"Or some queer pagod.\" Pope. 2. An idol. [Obs.] Bp. Stillingfleet.",
  "governmental - Pertaining to government; made by government; as, governmental duties.",
  "syrtis - A quicksand. Quenched in a boggy syrtis, neither sea Nor good dry land. Milton.",
  "water fox - The carp; -- so called on account of its cunning. Walton.",
  "fresnel lantern - A lantern having a lamp surrounded by a hollow cylindrical Fresnel lens.",
  "ferrocyanate - A salt of ferrocyanic acid; a ferrocyanide.",
  "longipennate - Having long wings, or quills.",
  "grucche - To murmur; to grumble. [Obs.] What aileth you, thus for grucche and groan. Chaucer.",
  "dominance - Predominance; ascendency; authority.",
  "manducation - The act of chewing. [R.] Jer. Taylor.",
  "hypodicrotous - Exhibiting retarded dicrotism; as, a hypodicrotic pulse curve.",
  "siderated - Planet-struck; blasted. [Obs.]",
  "pine-crowned - Clad or crowned with pine trees; as, pine-clad hills.",
  "psycho-motor - Of or pertaining to movement produced by action of the mind or will.",
  "pterygoquadrate - Of, pertaining to, or representing the pterygoid and quadrate bones or cartilages.",
  "smift - A match for firing a charge of powder, as in blasting; a fuse.",
  "note paper - Writing paper, not exceeding in size, when folded once, five by eight inches.",
  "champe - The field or ground on which carving appears in relief.",
  "kaolinization - The process by which feldspar is changed into kaolin.",
  "mancipation - Slavery; involuntary servitude. [Obs.] Johnson.",
  "light-legged - Nimble; swift of foot. Sir P. Sidney.",
  "sniffle - To snuffle, as one does with a catarrh. [Prov. Eng.]",
  "pedicular - Of or pertaining to lice; having the lousy distemper (phthiriasis); lousy. Southey.",
  "accusatively - 1. In an accusative manner. 2. In relation to the accusative case in grammar.",
  "finative - Conclusive; decisive; definitive; final. [Obs.] Greene (1593).",
  "pinule - One of the sights of an astrolabe. [Obs.]",
  "sculp - To sculpture; to carve; to engrave. [Obs. or Humorous.] Sandys.",
  "pluralization - The act of pluralizing. H. Spencer.",
  "legitimateness - The state or quality of being legitimate; lawfulness; genuineness.",
  "riderless - Having no rider; as, a riderless horse. H. Kingsley.",
  "accompliceship - The state of being an accomplice. [R.] Sir H. Taylor.",
  "anthropomorphosis - Transformation into the form of a human being.",
  "infractor - One who infracts or infringes; a violator; a breaker.",
  "typhomania - A low delirium common in typhus fever.",
  "droughtiness - A state of dryness of the weather; want of rain.",
  "thyrsoidal - Having somewhat the form of a thyrsus.",
  "pharyngitis - Inflammation of the pharynx.",
  "skin-deep - Not deeper than the skin; hence, superficial. Lowell.",
  "dolmen - A cromlech. See Cromlech. [Written also tolmen.]",
  "poake - Waste matter from the preparation of skins, consisting of hair, lime, oil, etc.",
  "discerptive - Tending to separate or disunite parts. Encys. Dict.",
  "pecunious - Abounding in money; wealthy; rich. [Obs.] Sherwood.",
  "metagastric - Of or pertaining to the two posterior gastric lobes of the carapace of crabs.",
  "rudmasday - Either of the feasts of the Holy Cross, occuring on May 3 and September 14, annually.",
  "etter pike - The stingfish, or lesser weever (Tranchinus vipera).",
  "gasconader - A great boaster; a blusterer.",
  "slippiness - Slipperiness. [R.] \"The slippiness of the way.\" Sir W. Scott.",
  "catamenia - The monthly courses of women; menstrual discharges; menses.",
  "gyroscopic - Pertaining to the gyroscope; resembling the motion of the gyroscope.",
  "headsman - An executioner who cuts off heads. Dryden.",
  "annotative - Characterized by annotations; of the nature of annotation.",
  "skilts - A kind of large, coarse, short trousers formerly worn. [Local, U. S.] Bartlett.",
  "algological - Of or pertaining to algology; as, algological specimens.",
  "optocoele - The cavity of one of the optic lobes of the brain in many animals. B. G. Wilder.",
  "evestigate - To investigate. [Obs.] Bailey.",
  "mitraille - Shot or bits of iron used sometimes in loading cannon.",
  "carunculated - Having a caruncle or caruncles; caruncular.",
  "toothbrush - A brush for cleaning the teeth.",
  "weepful - Full of weeping or lamentation; grieving. [Obs.] Wyclif.",
  "frustraneous - Vain; useless; unprofitable. [Obs.] South.",
  "orangeat - Candied orange peel; also, orangeade.",
  "blandise - To blandish any one. [Obs.] Chaucer.",
  "kyrielle - A litany beginning with the words. \"Kyrie eleison.\" Shipley.",
  "otocrane - The cavity in the skull in which the parts of the internal ear are lodged.",
  "disalliege - To alienate from allegiance. [Obs. & R.] Milton.",
  "cephalous - Having a head; -- applied chiefly to the Cephalata, a division of mollusks.",
  "glacialist - One who attributes the phenomena of the drift, in geology, to glaciers.",
  "staymaker - One whose occupation is to make stays.",
  "cato-cathartic - A remedy that purges by alvine discharges.",
  "perilymphatic - (a) Pertaining to, or containing, perilymph. (b) Perilymphangial.",
  "sauseflem - Having a red, pimpled face. [Obs.] [Written also sawceflem.] Chaucer.",
  "clamation - The act of crying out. Sir T. Browne.",
  "plenipotence - The quality or state of being plenipotent. [R.]",
  "acrimoniousness - The quality of being acrimonious; asperity; acrimony.",
  "schetical - Of or pertaining to the habit of the body; constitutional. [Obs.] Cudworth.",
  "analgesia - Absence of sensibility to pain. Quain.",
  "categorically - Absolutely; directly; expressly; positively; as, to affirm categorically.",
  "palpation - 1. Act of touching or feeling. 2. (Med.)  Examination of a patient by touch. Quain.",
  "amplexation - An embrace. [Obs.] An humble amplexation of those sacred feet. Bp. Hall.",
  "unsoul - To deprive of soul, spirit, or principle. [R.] Shelton.",
  "bluebreast - A small European bird; the blue-throated warbler.",
  "defrauder - One who defrauds; a cheat; an embezzler; a peculator.",
  "foredesign - To plan beforehand; to intend previously. Cheyne.",
  "unknown - Not known; not apprehended. -- Un*known\"ness, n. [R.] Camden.",
  "dubitancy - Doubt; uncertainty. [R.] Hammond.",
  "scaliness - The state of being scaly; roughness.",
  "strategus - The leader or commander of an army; a general.",
  "methoxyl - A hypothetical radical, CH3O, analogous to hydroxyl.",
  "argoile - Potter's clay. [Obs.] Chaucer.",
  "ballotation - Voting by ballot. [Obs.] Sir H. Wotton.",
  "fortnight - The space of fourteen days; two weeks.",
  "zoogony - The doctrine of the formation of living beings.",
  "amblygonal - Obtuse-angled. [Obs.] Hutton.",
  "pretended - Making a false appearance; unreal; false; as, pretended friend. -- Pre*tend\"ed*ly, adv.",
  "leptostraca - An order of Crustacea, including Nebalia and allied forms.",
  "presternum - The anterior segment of the sternum; the manubrium. -- Pre*ster\"nal, a.",
  "salineness - The quality or state of being salt; saltness.",
  "glace - Coated with icing; iced; glazed; -- said of fruits, sweetmeats, cake, etc.",
  "undulary - Moving like waves; undulatory. [Obs.] Sir T. Browne.",
  "surchargement - The act of surcharging; also, surcharge, surplus. [Obs.] Daniel.",
  "maharif - An African antelope (Hippotragus Bakeri). Its face is striped with black and white.",
  "incondensability - The quality or state of being incondensable.",
  "taenioidea - The division of cestode worms which comprises the tapeworms. See Tapeworm.",
  "lipothymous - Pertaining, or given, to swooning; fainting.",
  "sharer - One who shares; a participator; a partaker; also, a divider; a distributer.",
  "unfasten - To loose; to unfix; to unbind; to untie.",
  "unvicar - To deprive of the position or office a vicar. [R.] Strype.",
  "disagreeably - In a disagreeable manner; unsuitably; offensively.",
  "steersman - One who steers; the helmsman of a vessel. Milton.",
  "ambit - Circuit or compass. His great parts did not live within a small ambit. Milward.",
  "huswifely - Like a huswife; capable; economical; prudent. -- adv.  In a huswifely manner.",
  "laterifolious - Growing from the stem by the side of a leaf; as, a laterifolious flower.",
  "crib-biter - A horse that has the habit of cribbing.",
  "slik - Such. [Obs. or Scot.] Note: Used by Chaucer as of the Northern dialect.",
  "conciliary - Of or pertaining to, or issued by, a council. Jer. Taylor.",
  "shrill-gorged - Having a throat which produces a shrill note. [R.] Shak.",
  "herborough - A harbor. [Obs.] B. Jonson.",
  "varvel - In falconry, one of the rings secured to the ends of the jesses. [Written also vervel.]",
  "circumrotary - turning, rolling, or whirling round.",
  "unpeople - To deprive of inhabitants; to depopulate. Shak.",
  "polychromous - Of or pertaining to polychromy; many-colored; polychromatic.",
  "festerment - A festering. [R.] Chalmers.",
  "square-toes - A precise person; -- used contemptuously or jocularly. Thackeray.",
  "uphilt - To thrust in up to the hilt; as, to uphilt one's sword into an enemy. [R.] Stanyhurst.",
  "flagworm - A worm or grub found among flags and sedge.",
  "columbae - An order of birds, including the pigeons.",
  "indebtedness - 1. The state of being indebted. 2. The sum owed; debts, collectively.",
  "bijou - A trinket; a jewel; -- a word applied to anything small and of elegant workmanship.",
  "sejunction - The act of disjoining, or the state of being disjoined. [Obs.] Bp. Pearson.",
  "determinedly - In a determined manner; with determination.",
  "truckler - One who truckles, or yields servilely to the will of another.",
  "anciently - 1. In ancient times. 2. In an ancient manner. [R.]",
  "pansclavonian - See Panslavic, Panslavism, etc.",
  "throwster - One who throws or twists silk; a thrower.",
  "endenization - The act of naturalizing. [R.]",
  "caviler - One who cavils. Cavilers at the style of the Scriptures. Boyle.",
  "aniseed - The seed of the anise; also, a cordial prepared from it. \"Oil of aniseed.\" Brande & C.",
  "saintship - The character or qualities of a saint.",
  "amorphozoic - Of or pertaining to the Amorphozoa.",
  "resinate - Any one of the salts the resinic acids.",
  "innovative - Characterized by, or introducing, innovations. Fitzed. Hall.",
  "preterpluperfect - Old name of the tense also called pluperfect.",
  "nourishing - Promoting growth; nutritious,",
  "gigget - Same as Gigot. Cut the slaves to giggets. Beau. & Fl.",
  "discriminable - Capable of being discriminated. [Obs.] Bailey.",
  "cobaltite - A mineral of a nearly silver-white color, composed of arsenic, sulphur, and cobalt.",
  "pissabed - A name locally applied to various wild plants, as dandelion, bluet, oxeye daisy, etc.",
  "accessorial - Of or pertaining to an accessory; as, accessorial agency, accessorial guilt.",
  "exortive - Rising; relating to the east. [R.]",
  "municipally - In a municipal relation or condition.",
  "forenoon - The early part of the day, from morning to meridian, or noon.",
  "sensor - Sensory; as, the sensor nerves.",
  "splenography - A description of the spleen.",
  "trogue - A wooden trough, forming a drain. Raymond.",
  "despicability - Despicableness. [R.] Carlyle.",
  "superpurgation - Excessive purgation. Wiseman.",
  "supernaturality - The quality or state of being supernatural.",
  "columniation - The employment or arrangement of columns in a structure. Gwilt.",
  "provisorship - The office or position of a provisor. [R.] J. Webster.",
  "connutritious - Nutritious by force of habit; -- said of certain kinds of food. [Obs.] Crabb.",
  "wele - Prosperity; happiness; well-being; weal. [Obs.] Chaucer.",
  "planetic - Of or pertaining to planets. Sir T. Browne.",
  "salisburia - The ginkgo tree (Ginkgo biloba, or Salisburia adiantifolia).",
  "picturesquish - Somewhat picturesque. [R.]",
  "eyewinker - An eyelash. [A child's word.]",
  "collectorship - The office of a collector of customs or of taxes.",
  "freiherr - In Germany and Austria, a baron.",
  "indissolubleness - Indissolubility. Sir M. Hale.",
  "exility - Smallness; meagerness; slenderness; fineness, thinness. [R.] Paley.",
  "restringent - Restringing; astringent; styptic. [Obs.] -- n.  A restringent medicine. [Obs.] Harvey.",
  "pentapetalous - Having five petals, or flower leaves.",
  "recourseful - Having recurring flow and ebb; moving alternately. [Obs.] Drayton.",
  "subtense - A line subtending, or stretching across; a chord; as, the subtense of an arc.",
  "electrotypy - The process of producing electrotype plates. See Note under Electrotype, n.",
  "philomath - A lover of learning; a scholar. Chesterfield.",
  "crossopterygian - Of or pertaining to the Crossopterygii. -- n.  One of the Crossopterygii.",
  "sonnetist - A sonneter, or sonneteer. Bp. Hall.",
  "inflammableness - The quality or state of being inflammable; inflammability. Boyle.",
  "receivership - The state or office of a receiver.",
  "sarment - A prostrate filiform stem or runner, as of the strawbwrry. See Runner.",
  "thretty - Thirty. [Obs. or Scot.] Burns.",
  "gamma - The third letter (G) of the Greek alphabet.",
  "gladiatorism - The art or practice of a gladiator.",
  "elflock - Hair matted, or twisted into a knot, as if by elves.",
  "columbin - A white, crystalline, bitter substance. See Calumbin.",
  "ymel - Among. [Obs.] \"Ymel them all.\" Chaucer.",
  "pseudology - Falsehood of speech. Arbuthnot.",
  "zooegraphical - Of or pertaining to the description of animals.",
  "asian - Of or pertaining to Asia; Asiatic. \"Asian princes.\" Jer. Taylor. -- n.  An Asiatic.",
  "offerable - Capable of being offered; suitable or worthy to be offered.",
  "flustrate - To fluster. [Colloq.] Spectator.",
  "squir - To throw with a jerk; to throw edge foremost. [Obs.] [Written also squirr.] Addison.",
  "conirostral - Belonging to the Conirostres.",
  "indicatory - Serving to show or make known; showing; indicative; signifying; implying.",
  "solert - Skillful; clever; crafty. [Obs.] Cudworth.",
  "salsuginous - Growing in brackish places or in salt marches.",
  "ditheistic - Pertaining to ditheism; dualistic.",
  "vizier-azem - A grand vizier. See under Vizier.",
  "self-life - Life for one's self; living solely or chiefly for one's own pleasure or good.",
  "thoroughness - The quality or state of being thorough; completeness.",
  "ash-colored - Of the color of ashes; a whitish gray or brownish gray.",
  "magnifier - One who, or that which, magnifies.",
  "woaded - Colored or stained with woad. \"Man tattoed or woaded, winter- clad in skins.\" Tennyson.",
  "liveried - Wearing a livery. See Livery, 3. The liveried servants wait. Parnell.",
  "foundershaft - The first shaft sunk. Raymond.",
  "cervelat - An ancient wind instrument, resembling the bassoon in tone.",
  "abit - 3d sing. pres. of Abide. [Obs.] Chaucer.",
  "warriangle - See Wariangle. [Obs. or Prov. Eng.]",
  "cinctured - Having or wearing a cincture or gridle.",
  "coleopteral - Having wings covered with a case or sheath; belonging to the Coleoptera.",
  "keratogenous - Producing horn; as, the keratogenous membrane within the horny hoof of the horse.",
  "bestiality - 1. The state or quality of being bestial. 2. Unnatural connection with a beast.",
  "grandmotherly - Like a grandmother in age or manner; kind; indulgent.",
  "somnolence - Sleepiness; drowsiness; inclination to sleep.",
  "congealable - Capable of being congealed. --Con*geal\"a*ble*ness, n.",
  "vodka - A Russian drink distilled from rye.",
  "fiddledeedee - An exclamatory word or phrase, equivalent to nonsense! [Colloq.]",
  "correlatively - In a correlative relation.",
  "diamond state - Delaware; -- a nickname alluding to its small size.",
  "au revoir - Good-by until we meet again.",
  "quadrifid - Divided, or deeply cleft, into four parts; as, a quadrifid perianth; a quadrifid leaf.",
  "coagulability - The quality of being coagulable; capacity of being coagulated. Ure.",
  "orcein - A reddish brown amorphous dyestuff,",
  "grudgingness - The state or quality of grudging, or of being full of grudge or unwillingness.",
  "muscadel - See Muscatel, n. Quaffed off the muscadel. Shak.",
  "rubus - A genus of rosaceous plants, including the raspberry and blackberry.",
  "nicker tree - The plant producing nicker nuts. [Written also neckar tree and nickar tree.]",
  "significatory - Significant. -- n.  That which is significatory.",
  "rhabdosphere - A minute sphere composed of rhabdoliths.",
  "crotaphitic - Pertaining to the temple; temporal.",
  "uniquity - The quality or state of being unique; uniqueness. [R.] Walpole.",
  "puttee - Same as Putty, a kind of gaiter.",
  "imesatin - A dark yellow, crystalline substance, obtained by the action of ammonia on isatin.",
  "coincider - One who coincides with another in an opinion.",
  "erogation - The act of giving out or bestowing. [Obs.] Sir T. Elyot.",
  "preorbital - a. (Anat.) Situated in front or the orbit.",
  "sesquibasic - Containing, or acting as, a base in the proportions of a sesqui compound.",
  "embogue - To disembogue; to discharge, as a river, its waters into the sea or another river. [R.]",
  "upstir - Insurrection; commotion; disturbance. [Obs.] Sir J. Cheke.",
  "durante - During; as, durante vita, during life; durante bene placito, during pleasure.",
  "chitchat - Familiar or trifling talk; prattle.",
  "dedecorous - Disgraceful; unbecoming. [R.] Bailey.",
  "sojer - Var. of Soldier. [Dial. or Slang]",
  "sheld - Variegated; spotted; speckled; piebald. [Prov. Eng.]",
  "haycock - A conical pile or hear of hay in the field. The tanned haycock in the mead. Milton.",
  "lam - To beat soundly; to thrash. [Obs. or Low] Beau. & Fl.",
  "languente - In a languishing manner; pathetically.",
  "malonyl - A hydrocarbon radical, CH2.(CO)2, from malonic acid.",
  "malnutrition - Faulty or imperfect nutrition.",
  "interplace - To place between or among; as, to interplace a name. [R.] Daniel.",
  "obsess - To besiege; to beset. Sir T. Elyot.",
  "outsentry - A sentry who guards the entrance or approach to a place; an outguard.",
  "encave - To hide in, or as in, a cave or recess. \"Do but encave yourself.\" Shak.",
  "consignatory - One of several that jointly sign a written instrument, as a treaty. Fallows.",
  "iodhydrin - One of a series of compounds containing iodine, and analogous to the chlorhydrins.",
  "redweed - The red poppy (Papaver Rhoeas). Dr. Prior.",
  "formidolose - Very much afraid. [Obs.] Bailey.",
  "averroism - The tenets of the Averroists.",
  "relishable - Capable of being relished; agreeable to the taste; gratifying.",
  "weedery - Weeds, collectively; also, a place full of weeds or for growing weeds. [R.] Dr. H. More.",
  "cardiographic - Of or pertaining to, or produced by, a cardiograph.",
  "planisher - One who, or that which, planishes. Weale.",
  "unwellness - , n. Quality or state of being unwell.",
  "blue-john - A name given to fluor spar in Derbyshire, where it is used for ornamental purposes.",
  "dandify - To cause to resemble a dandy; to make dandyish.",
  "galsome - Angry; malignant. [Obs.] Bp. Morton.",
  "germanization - The act of Germanizing. M. Arnold.",
  "orthite - A variety of allanite occurring in slender prismatic crystals.",
  "deplorability - Deplorableness. Stormonth.",
  "prosemination - Propagation by seed. [Obs.] Sir M. Hale.",
  "pulpous - Containing pulp; pulpy. \" Pulpous fruit.\" J. Philips. -- Pulp\"ous*ness, n.",
  "two-foot - Measuring two feet; two feet long, thick, or wide; as, a two- foot rule.",
  "californian - Of or pertaining to California. -- n.  A native or inhabitant of California.",
  "franion - A paramour; a loose woman; also, a gay, idle fellow. [Obs.] Spenser.",
  "demi-island - Peninsula. [Obs.] Knolles.",
  "paramento - Ornament; decoration. Beau. & Fl.",
  "self-denying - Refusing to gratify one's self; self-sacrificing. -- Self`-de*ny\"ing*ly, adv.",
  "burstwort - A plant (Herniaria glabra) supposed to be valuable for the cure of hernia or rupture.",
  "prochronize - To antedate. Fitzed. Hall.",
  "mitis metal - The malleable iron produced by mitis casting; -- called also simply mitis.",
  "exploring - Employed in, or designed for, exploration. \"Exploring parties.\" Bancroft.",
  "coothay - A striped satin made in India. McElrath.",
  "incapableness - The quality or state of being incapable; incapability.",
  "dullard - A stupid person; a dunce. Shak. -- a.  Stupid. Bp. Hall.",
  "accustomance - Custom; habitual use. [Obs.] Boyle.",
  "humanics - The study of human nature. [R.] T. W. Collins.",
  "divident - Dividend; share. [Obs.] Foxe.",
  "amphoral - Pertaining to, or resembling, an amphora.",
  "overlander - One who travels over lands or countries; one who travels overland.",
  "crippler - A wooden tool used in graining leather. Knight.",
  "churchism - Strict adherence to the forms or principles of some church organization; sectarianism.",
  "removal - The act of removing, or the state of being removed.",
  "cornflower - A conspicuous wild flower (Centaurea Cyanus), growing in grainfields.",
  "cavernulous - Full of little cavities; as, cavernulous metal. Black.",
  "endothorax - An internal process of the sternal plates in the thorax of insects.",
  "catadicrotism - Quality or state of being catacrotic. -- Cat`a*di*crot\"ic (#), a.",
  "maholi - A South African lemur (Galago maholi), having very large ears. [Written also moholi.]",
  "niggardous - Niggardly. [Obs.] Covetous gathering and niggardous keeping. Sir T. More.",
  "oversorrow - To grieve or afflict to excess. [Obs.] Milton.",
  "tyrannicidal - Of or pertaining to tyrannicide, or the murder of a tyrant. Booth.",
  "affret - A furious onset or attack. [Obs.] Spenser.",
  "unextinguishable - Inextinguishable. -- Un`ex*tin\"guish*a*bly, adv.",
  "presbyteral - Of or pertaining to a presbyter or presbytery; presbyterial.",
  "dowress - A woman entitled to dower. Bouvier.",
  "cragged - Full of crags, or steep, broken Into its cragged rents descend. J. Baillie.",
  "sorbonical - Belonging to the Sorbonne or to a Sorbonist. Bale.",
  "impuissance - Lack of power; inability. Bacon. Their own impuissance and weakness. Holland.",
  "collaborator - An associate in labor, especially in literary or scientific labor.",
  "maternity - The state of being a mother; the character or relation of a mother.",
  "conder - One who watches shoals of fish; a balker. See Balker.",
  "paleographer - One skilled in paleography; a paleographist.",
  "fabulize - To invent, compose, or relate fables or fictions. G. S. Faber.",
  "atomically - In an atomic manner; in accordance with the atomic philosophy.",
  "autochronograph - An instrument for the instantaneous self-recording or printing of time. Knight.",
  "amicableness - The quality of being amicable; amicability.",
  "kinology - That branch of physics which treats of the laws of motion, or of moving bodies.",
  "incrystallizable - Not crystallizable; incapable of being formed into crystals.",
  "insatiably - In an insatiable manner or degree; unappeasably. \"Insatiably covetous.\" South.",
  "matinal - Relating to the morning, or to matins; matutinal.",
  "everych - each one; every one; each of two. See Every. [Obs.] Chaucer.",
  "cresting - An ornamental finish on the top of a wall or ridge of a roof.",
  "pureness - The state of being pure (in any sense of the adjective).",
  "domestically - In a domestic manner; privately; with reference to domestic affairs.",
  "anticonstitutional - Opposed to the constitution; unconstitutional.",
  "slimness - The quality or state of being slim.",
  "alepidote - Not having scales. -- n.  A fish without scales.",
  "cockamaroo - The Russian variety of bagatelle.",
  "cynorexia - A voracious appetite, like that of a starved dog.",
  "a-good - In earnest; heartily. [Obs.] \"I made her weep agood.\" Shak.",
  "bedquilt - A quilt for a bed; a coverlet.",
  "dismaw - To eject from the maw; to disgorge. [R.] Shelton.",
  "bootjack - A device for pulling off boots.",
  "retistene - A white crystalline hydrocarbon produced indirectly from retene.",
  "unchristianness - The quality or state of being unchristian. [R.] Eikon Basilike.",
  "inerrability - Freedom or exemption from error; infallibility. Eikon Basilike.",
  "conglaciate - To turn to ice; to freeze. [Obs.] Sir T. Browne.",
  "bonbonniere - A small fancy box or dish for bonbons.",
  "irrevealable - Incapable of being revealed. -- Ir`re*veal\"a*bly, adv.",
  "curvilineal - Consisting of, or bounded by, curved lines; as, a curvilinear figure.",
  "molossine - A bat of the genus Molossus, as the monk bat.",
  "securer - One who, or that which, secures.",
  "viviparity - The quality or condition of being viviparous. H. Spencer.",
  "commender - One who commends or praises.",
  "familistic - Pertaining to Familists. Baxter.",
  "dochmius - A foot of five syllables (usually",
  "inarticulately - In an inarticulate manner. Hammond.",
  "ect- - A combining form signifying without, outside, external.",
  "half-wit - A foolish; a dolt; a blockhead; a dunce. Dryden.",
  "mollification - The act of mollifying, or the state of being mollified; a softening. Chaucer.",
  "skayles - [sq. root159.] Skittles. [Obs.]",
  "perterebration - The act of boring through. [Obs.] Ainsworth.",
  "authenticness - The quality of being authentic; authenticity. [R.] Hammond.",
  "boat-tail - A large grackle or blackbird (Quiscalus major), found in the Southern United States.",
  "roboration - The act of strengthening. [Obs.] Coles.",
  "multisiliquous - Having many pods or seed vessels.",
  "anachoretical - See Anchoret, Anchoretic. [Obs.]",
  "cataloguer - A maker of catalogues; esp. one skilled in the making of catalogues.",
  "aristocratism - 1. The principles of aristocrats. Romilly. 2. Aristocrats, collectively. [R.]",
  "mercaptal - Any one of a series of compounds of mercaptans with aldehydes.",
  "portmote - In old English law, a court, or mote, held in a port town. [Obs.] Blackstone.",
  "befrill - To furnish or deck with a frill.",
  "brainpan - The bones which inclose the brain; the skull; the cranium.",
  "delibration - The act of stripping off the bark. [Obs.] Ash.",
  "insomnious - Restless; sleepless. Blount.",
  "piscation - Fishing; fishery. [Obs.] Sir T. Browne.",
  "natterjack - A European toad (Bufo calamita), having a yellow line along its back.",
  "effacement - The act if effacing; also, the result of the act.",
  "atrypa - A extinct genus of Branchiopoda, very common in Silurian limestones.",
  "addle-brain - A foolish or dull-witted fellow. [Colloq.]",
  "talukdar - A proprietor of a talook. [India]",
  "tangalung - An East Indian civet (Viverra tangalunga).",
  "warre - Worse. [Obs.] They say the world is much warre than it wont. Spenser.",
  "saleswoman - A woman whose occupation is to sell goods or merchandise.",
  "incorruptibly - In an incorruptible manner.",
  "celticize - To render Celtic; to assimilate to the Celts.",
  "classific - Characterizing a class or classes; relating to classification.",
  "maxim gun - A kind of machine gun; -- named after its inventor, Hiram S. Maxim.",
  "pilgrimize - To wander as a pilgrim; to go on a pilgrimage. [Obs.] B. Jonson.",
  "vendetta - A blood feud; private revenge for the murder of a kinsman.",
  "norimon - A Japanese covered litter, carried by men. B. Taylor.",
  "warkloom - A tool; an implement. [Scot.]",
  "dicer - A player at dice; a dice player; a gamester. As false as dicers' oaths. Shak.",
  "photological - Pertaining to photology, or the doctrine of light.",
  "conceitedness - The state of being conceited; conceit; vanity. Addison.",
  "analyser - Same as Analyze, Analyzer, etc.",
  "glucic - Pertaining to, or obtained from, sugar; as, glucic acid.",
  "trinitrocellulose - Gun cotton; -- so called because regarded as containing three nitro groups.",
  "roberdsman - A bold, stout robber, or night thief; -- said to be so called from Robin Hood.",
  "counterstep - A contrary method of procedure; opposite course of action.",
  "distributiveness - Quality of being distributive.",
  "rayless - Destitute of rays; hence, dark; not illuminated; blind; as, a rayless sky; rayless eyes.",
  "untime - An unseasonable time. [Obs.] A man shall not eat in untime. Chaucer.",
  "podagric - 1. Pertaining to the gout; gouty; caused by gout. 2. Afflicted with gout. Sir T. Browne.",
  "needleful - As much thread as is used in a needle at one time.",
  "dentately - In a dentate or toothed manner; as, dentately ciliated, etc.",
  "ruggy - Rugged; rough. [Obs.] \"With ruggy, ashy hairs.\" Chaucer.",
  "perspicable - Discernible. [Obs.] Herbert.",
  "irreligiously - In an irreligious manner.",
  "sware - imp. of Swear. [Obs. or Poetic] Cophetua sware a royal oath. Tennyson.",
  "cannibally - In the manner of cannibal. \"An he had been cannibally given.\" Shak.",
  "peregal - Fully equal. [Obs.] Chaucer. \"Peregal to the best.\" Spenser.",
  "pullicate - A kind of checked cotton or silk handkerchief.",
  "interseminate - To sow between or among. [R.]",
  "sutteeism - The practice of self-immolation of widows in Hindostan.",
  "generalizer - One who takes general or comprehensive views. Tyndall.",
  "daintify - To render dainty, delicate, or fastidious. \"Daintified emotion.\" Sat. rev.",
  "jane - 1. A coin of Genoa; any small coin. Chaucer. 2. A kind of twilled cotton cloth. See Jean.",
  "maud - A gray plaid; -- used by shepherds in Scotland.",
  "crucigerous - Bearing the cross; marked with the figure of a cross. Sir. T. Browne.",
  "mantuan - Of or pertaining to Mantua. -- n.  A native or inhabitant of Mantua.",
  "insomnia - Want of sleep; inability to sleep; wakefulness; sleeplessness.",
  "flanched - Having flanches; -- said of an escutcheon with those bearings.",
  "archiepiscopality - The station or dignity of an archbishop; archiepiscopacy. Fuller.",
  "elicitation - The act of eliciting. [Obs.] Abp. Bramhall.",
  "stud-horse - A stallion, esp. one kept for breeding.",
  "antevert - 1. To prevent. [Obs.] Bp. Hall. 2. (Med.)  To displace by anteversion.",
  "hauerite - Native sulphide of manganese a reddish brown or brownish black mineral.",
  "orfe - A bright-colored domesticated variety of the id. See Id.",
  "pervasion - The act of pervading, passing, or spreading through the whole extent of a thing. Boyle.",
  "sheafy - Pertaining to, or consisting of, a sheaf or sheaves; resembling a sheaf.",
  "decimally - By tens; by means of decimals.",
  "dim-sighted - Having dim sight; lacking perception. -- Dim\"-sight`ed*ness, n.",
  "lousily - In a lousy manner; in a mean, paltry manner; scurvily. [Vulgar]",
  "acrotarsium - The instep or front of the tarsus.",
  "scamblingly - In a scambling manner; with turbulence and noise; with bold intrusiveness.",
  "tenementary - Capable of being leased; held by tenants. Spelman.",
  "speculist - One who observes or considers; an observer. [R.] Goldsmith.",
  "sanhedrist - A member of the sanhedrin. Schaeffer (Lange's Com. ).",
  "actualization - A making actual or really existent. [R.] Emerson.",
  "spiritualization - The act of spiritualizing, or the state of being spiritualized.",
  "interlobular - Between lobules; as, the interlobular branches of the portal vein.",
  "diversiform - Of a different form; of varied forms.",
  "inefficaciousness - Want of effect, or of power to produce the effect; inefficacy.",
  "leading edge - same as Advancing edge, above.",
  "ostentator - One fond of display; a boaster. Sherwood.",
  "pimpinel - The burnet saxifrage. See under Saxifrage.",
  "chenomorphae - An order of birds, including the swans, ducks, geese, flamingoes and screamers.",
  "cloven-hoofed - Having the foot or hoof divided into two parts, as the ox.",
  "reservor - One who reserves; a reserver.",
  "self-elective - Having the right of electing one's self, or, as a body, of electing its own members.",
  "salamandrine - Of, pertaining to, or resembling, a salamander; enduring fire. Addison.",
  "ancestorially - With regard to ancestors.",
  "fellon - Variant of Felon. [Obs.] Those two were foes the fellonest on ground. Spenser.",
  "bureaucratic - Of, relating to, or resembling, a bureaucracy.",
  "curstness - Peevishness; malignity; frowardness; crabbedness; surliness. [Obs.] Shak.",
  "self-sufficiency - The quality or state of being self-sufficient.",
  "herbergeour - A harbinger. [Obs.] Chaucer.",
  "extraparochial - Beyond the limits of a parish. -- Ex`tra*pa*ro\"chi*al*ly, adv.",
  "majorcan - Of or pertaining to Majorca. -- n.  A native or inhabitant of Majorca.",
  "hexdecylic - Pertaining to, or derived from, hexdecyl or hecdecane; as, hexdecylic alcohol.",
  "commonty - A common; a piece of land in which two or more persons have a common right. Bell.",
  "sassanage - Stones left after sifting. Smart.",
  "expirable - That may expire; capable of being brought to an end.",
  "piningly - In a pining manner; droopingly. Poe.",
  "scrobiculate - Having numerous small, shallow depressions or hollows; pitted.",
  "desiccation - The act of desiccating, or the state of being desiccated.",
  "in loco - In the place; in the proper or natural place.",
  "nigraniline - The complex, nitrogenous, organic base and dyestuff called also aniline black.",
  "coffeepot - A covered pot im which coffee is prepared,",
  "ingroove - To groove in; to join in or with a groove. Tennyson.",
  "oxamethane - Ethyl oxamate, obtained as a white scaly crystalline powder.",
  "dimmish - Somewhat dim; as, dimmish eyes. \"Dimmy clouds.\" Sir P. Sidney.",
  "fraudless - Free from fraud. -- Fraud\"less*ly, adv. -- Fraud\"less*ness, n.",
  "rectilinearity - The quality or state of being rectilinear. Coleridge.",
  "internationally - In an international manner; from an international point of view.",
  "interstellar - Between or among the stars; as, interstellar space. Bacon.",
  "molluscan - Of or pertaining to mollusks. -- n.  A mollusk; one of the Mollusca.",
  "redeliberate - To deliberate again; to reconsider.",
  "husbandly - Frugal; thrifty. [R.] Tusser.",
  "aventail - The movable front to a helmet; the ventail.",
  "geniture - Generation; procreation; birth. Dryden.",
  "nuncupation - The act of nuncupating. [Obs.]",
  "marketableness - Quality of being marketable.",
  "party-coated - Having a motley coat, or coat of divers colors. Shak.",
  "yis - Yes. [Obs.] \"Yis, sir,\" quod he, \"yis, host.\" Chaucer.",
  "catholicly - In a catholic manner; generally; universally. Sir L. Cary.",
  "ventage - A small hole, as the stop in a flute; a vent. Shak.",
  "seismology - The science of earthquakes.",
  "bewitchedness - The state of being bewitched. Gauden.",
  "sinupalliate - Having a pallial sinus. See under Sinus.",
  "translavation - A laving or lading from one vessel to another. [Obs.] Holland.",
  "sarcolactic - relating to muscle and milk; as, sarcolactic acid. See Lactic acid, under Lactic.",
  "monanthous - Having but one flower; one-flowered. Gray.",
  "demulce - To soothe; to mollify; to pacify; to soften. [R.] Sir T. Elyot.",
  "forfered - Excessively alarmed; in great fear. [Obs.] \"Forfered of his death.\" Chaucer.",
  "bullirag - To intimidate by bullying; to rally contemptuously; to badger. [Low]",
  "impacable - Not to be appeased or quieted. [Obs.] Spenser. -- Im*pa\"ca*bly, adv.",
  "manducable - Such as can be chewed; fit to be eaten. [R.] Any manducable creature. Sir T. Herbert.",
  "episcopally - By episcopal authority; in an episcopal manner.",
  "swashy - Soft, like fruit that is too ripe; quashy; swash. [Prov. Eng.]",
  "potamology - A scientific account or discussion of rivers; a treatise on rivers; potamography.",
  "austromancy - Soothsaying, or prediction of events, from observation of the winds.",
  "footworn - Worn by, or weared in, the feet; as, a footworn path; a footworn traveler.",
  "downstream - Down the stream; as, floating downstream.",
  "sauroidichnite - The fossil track of a saurian.",
  "epigaeous - Growing on, or close to, the ground.",
  "transanimate - To animate with a soul conveyed from another body. [R.] Bp. J. King (1608).",
  "arbuscle - A dwarf tree, one in size between a shrub and a tree; a treelike shrub. Bradley.",
  "saccholactic - Of, pertaining to, or designating, an acid now called mucic acid; saccholic. [Obs.]",
  "peregrinator - One who peregrinates; one who travels about.",
  "slock - To quench; to allay; to slake. See Slake. [Obs. or Scot.]",
  "triformity - The state of being triform, or of having a threefold shape.",
  "eternify - To make eternal. [Obs.] Fame . . . eternifies the name. Mir. for Mag.",
  "subtreasurer - The public officer who has charge of a subtreasury. [U. S.]",
  "scariose - Thin, dry, membranous, and not green. Gray.",
  "inseam - To impress or mark with a seam or cicatrix. Pope.",
  "subzygomatic - Situated under the zygoma or zygomatic process.",
  "acoumeter - An instrument for measuring the acuteness of the sense of hearing. Itard.",
  "bendlet - A narrow bend, esp. one half the width of the bend.",
  "idiographical - Of or pertaining to an idiograph.",
  "boke - To poke; to thrust. [Obs. or Dial.]",
  "boanerges - Any declamatory and vociferous preacher or orator.",
  "vavasory - The quality or tenure of the fee held by a vavasor; also, the lands held by a vavasor.",
  "unbias - To free from bias or prejudice. Swift.",
  "satanist - A very wicked-person. [R.] Granger.",
  "calcic - Pertaining to, derived from, or containing, calcium or lime.",
  "engallant - To make a gallant of. [Obs.] B. Jonson.",
  "all hail - All health; -- a phrase of salutation or welcome.",
  "burgonet - A kind of helmet. [Written also burganet.] Shak.",
  "deification - The act of deifying; exaltation to divine honors; apotheosis; excessive praise.",
  "waveless - Free from waves; undisturbed; not agitated; as, the waveless sea.",
  "especially - In an especial manner; chiefly; particularly; peculiarly; in an uncommon degree.",
  "after-mentioned - Mentioned afterwards; as, persons after-mentioned (in a writing).",
  "lichenology - The science which treats of lichens.",
  "irresolvedly - Without settled determination; in a hesitating manner; doubtfully. [R.]",
  "constabless - The wife of a constable. [Obs.]",
  "boneless - Without bones. \"Boneless gums.\" Shak.",
  "another-gaines - Of another kind. [Obs.] Sir P. Sidney.",
  "shallow-hearted - Incapable of deep feeling. Tennyson.",
  "furioso - With great force or vigor; vehemently.",
  "inburnt - Burnt in; ineffaceable. Her inburnt, shamefaced thoughts. P. Fletcher.",
  "unwilled - Deprived of the faculty of will or volition. Mrs. Browning.",
  "refut - Refuge. \"Thou haven of refut.\" [Obs.] Chaucer.",
  "dissentany - Dissentaneous; inconsistent. [Obs.] Milton.",
  "lour - An Asiatic sardine (Clupea Neohowii), valued for its oil.",
  "intempestive - Out of season; untimely. [Obs.] Burton. Intempestive bashfulness gets nothing. Hales.",
  "self-annihilation - Annihilation by one's own acts; annihilation of one's desires. Addison.",
  "deva - A god; a deity; a divine being; an idol; a king.",
  "impugnable - Capable of being impugned; that may be gainsaid.",
  "cataclasm - A breaking asunder; disruption.",
  "macropetalous - Having long or large petals.",
  "maximization - The act or process of increasing to the highest degree. Bentham.",
  "cognac - A kind of French brandy, so called from the town of Cognac.",
  "alfalfa - The lucern (Medicago sativa); -- so called in California, Texas, etc.",
  "spigurnel - Formerly the title of the sealer of writs in chancery. Mozley & W.",
  "rakery - Debauchery; lewdness. The rakery and intrigues of the lewd town. R. North.",
  "unitarianize - To change or turn to Unitarian views.",
  "dissipativity - The rate at which palpable energy is dissipated away into other forms of energy.",
  "dahlin - A variety of starch extracted from the dahlia; -- called also inulin. See Inulin.",
  "isapostolic - Having equal, or almost equal, authority with the apostles of their teachings.",
  "paronomastic - Of or pertaining to paronomasia; consisting in a play upon words.",
  "beadleship - The state of being, or the personality of, a beadle. A. Wood.",
  "phytogenesis - The doctrine of the generation of plants.",
  "suasible - Capable of being persuaded; easily persuaded.",
  "chansonnette - A little song. These pretty little chansonnettes that he sung. Black.",
  "sensive - Having sense or sensibility; sensitive. [Obs.] Sir P. Sidney.",
  "slugger - One who strikes heavy blows; hence, a boxer; a prize fighter. [Cant or Slang]",
  "alarmable - Easily alarmed or disturbed.",
  "gawn - A small tub or lading vessel. [Prov. Eng.] Johnson.",
  "pentagraphic - Pantographic. See Pantograph.",
  "choralist - A singer or composer of chorals.",
  "overruler - One who, or that which, controls, governs, or determines. Sir P. Sidney.",
  "spermatorrhea - Abnormally frequent involuntary emission of the semen without copulation.",
  "cazic - A chief or petty king among some tribes of Indians in America.",
  "fanfaron - A bully; a hector; a swaggerer; an empty boaster. [R.] Dryden.",
  "glottidean - Of or pertaining to the glottis; glottal.",
  "sea reed - The sea-sand reed. See under Reed.",
  "superbiate - To make (a person) haughty. [Obs. & R.] Feltham.",
  "prefident - Trusting beforehand; hence, overconfident. [Obs.] Baxter.",
  "paraguayan - Of or pertaining to Paraguay. -- n.  A native or inhabitant of Paraguay.",
  "largely - In a large manner. Dryden. Milton.",
  "rakeshame - A vile, dissolute wretch. [Obs.] Milton.",
  "overmarch - To march too far, or too much; to exhaust by marching. Baker.",
  "cinque - Five; the number five in dice or cards.",
  "suprapedal - Situated above the foot of a mollusk; as, the suprapedal gland.",
  "blastocarpous - Germinating inside the pericarp, as the mangrove. Brande & C.",
  "bursch - A youth; especially, a student in a german university.",
  "disepalous - Having two sepals; two-sepaled.",
  "dicoccous - Composed pf two coherent, one-seeded carpels; as, a dicoccous capsule.",
  "roching cask - A tank in which alum is crystallized from a solution.",
  "pianissimo - Very soft; -- a direction to execute a passage as softly as possible. (Abbrev. pp.)",
  "eau forte - An etching or a print from an etched plate.",
  "eburnean - Made of or relating to ivory.",
  "blacksalter - One who,makes crude potash, or black salts.",
  "doughbird - The Eskimo curlew (Numenius borealis). See Curlew.",
  "subseptuple - Having the ratio of one to seven. Bp. Wilkins.",
  "expectorative - Same as Expectorant. Harvey.",
  "eponymic - Same as Eponymous. Tablets . . . which bear eponymic dates. I. Taylor (The Alphabet).",
  "batful - Rich; fertile. [Obs.] \"Batful valleys.\" Drayton.",
  "blatherskite - A blustering, talkative fellow. [Local slang, U. S.] Barllett.",
  "sea dragon - (a) A dragonet, or sculpin. (b) The pegasus.",
  "diverberate - To strike or sound through. [R.] Davies (Holy Roode).",
  "yfere - Together. See Ifere. [Obs.] As friends do when they be met yfere. Chaucer.",
  "guru - A spiritual teacher, guide, or confessor amoung the Hindoos. Malcom.",
  "extortioner - , One who practices extortion.",
  "civics - The science of civil government.",
  "precellency - Excellence; superiority. [Obs.] Sheldon.",
  "prebendship - A prebendaryship. [Obs.] Foxe.",
  "gunstock - The stock or wood to which the barrel of a hand gun is fastened.",
  "endophragmal - Of or pertaining to the endophragma.",
  "volva - A saclike envelope of certain fungi, which bursts open as the plant develops.",
  "tetrapharmacum - A combination of wax, resin, lard, and pitch, composing an ointment. Brande & C.",
  "inevitability - Impossibility to be avoided or shunned; inevitableness. Shelford.",
  "befit - To be suitable to; to suit; to become. That name best befits thee. Milton.",
  "infructuose - Not producing fruit; unfruitful; unprofitable. [R.] T. Adams.",
  "glomerulus - The bunch of looped capillary blood vessels in a Malpighian capsule of the kidney.",
  "seora - A Spanish title of courtesy given to a lady; Mrs.; Madam; also, a lady.",
  "quintuple-nerved - The same as Quinquenerved.",
  "doctorally - In the manner of a doctor.[R.]",
  "anaemia - A morbid condition in which the blood is deficient in quality or in quantity.",
  "esotericism - Esoteric doctrine or principles.",
  "anachronize - To refer to, or put into, a wrong time. [R.] Lowell.",
  "preparable - Capable of being prepared. \"Medicine preparable by art.\" Boyle.",
  "monopersonal - Having but one person, or form of existence.",
  "surturbrand - A fibrous brown coal or bituminous wood.",
  "parliamentarily - In a parliamentary manner.",
  "tanghinia - The ordeal tree. See under Ordeal.",
  "formell - The female of a hawk or falcon.",
  "protuberancy - The quality or state of being protuberant; protuberance; prominence.",
  "steve - To pack or stow, as cargo in a ship's hold. See Steeve.",
  "metoposcopist - One versed in metoposcopy.",
  "stoichiological - Of or pertaining to stoichiology.",
  "mitigative - Tending to mitigate; alleviating.",
  "audaciously - In an audacious manner; with excess of boldness; impudently.",
  "firedog - A support for wood in a fireplace; an andiron.",
  "prehensory - Adapted to seize or grasp; prehensile.",
  "paritor - An apparitor. \"Summoned by an host of paritors.\" Dryden.",
  "performable - Admitting of being performed, done, or executed; practicable.",
  "marconism - The theory or practice of Marconi's wireless telegraph system.",
  "shovelful - As much as a shovel will hold; enough to fill a shovel.",
  "hektometer - Same as Hectare, Hectogram, Hectoliter, and Hectometer.",
  "overlord - One who is lord over another or others; a superior lord; a master. Freeman.",
  "high-sounding - Pompous; noisy; ostentatious; as, high-sounding words or titles.",
  "stationariness - The quality or state of being stationary; fixity.",
  "deputator - One who deputes, or makes a deputation. [R.] Locke.",
  "life-giving - Giving life or spirit; having power to give life; inspiriting; invigorating.",
  "sickerly - Surely; securely. [Obs.] But sikerly, withouten any fable. Chaucer.",
  "eldern - Made of elder. [Obs.] He would discharge us as boys do eldern guns. Marston.",
  "onomatology - The science of names or of their classification.",
  "decharm - To free from a charm; to disenchant.",
  "quartane - Butane, each molecule of which has four carbon atoms.",
  "landflood - An overflowing of land by river; an inundation; a freshet. Clarendon.",
  "undeaf - To free from deafness; to cause to hear. [Obs.] Shak.",
  "reorganization - The act of reorganizing; a reorganized existence; as, reorganization of the troops.",
  "guessingly - By way of conjecture. Shak.",
  "arcuate - Bent or curved in the form of a bow. \"Arcuate stalks.\" Gray.",
  "brogan - A stout, coarse shoe; a brogue.",
  "drifty - Full of drifts; tending to form drifts, as snow, and the like.",
  "complicateness - Complexity. Sir M. Hale.",
  "offerture - Offer; proposal; overture. [Obs.] More offertures and advantages to his crown. Milton.",
  "dooryard - A yard in front of a house or around the door of a house.",
  "bareback - On the bare back of a horse, without using a saddle; as, to ride bareback.",
  "amateurism - The practice, habit, or work of an amateur.",
  "lette - To let; to hinder. See Let, to hinder. [Obs.] Chaucer.",
  "confated - Fated or decreed with something else. [R.] A. Tucker.",
  "wringing - a. & n. from Wring, v. Wringing machine, a wringer. See Wringer, 2.",
  "indelectable - Not delectable; unpleasant; disagreeable. [R.] Richardson.",
  "edenized - Admitted to a state of paradisaic happiness. [R.] Davies (Wit's Pilgr. ).",
  "althing - The national assembly or parliament of Iceland. See Thing, n., 8.",
  "cete - One of the Cetacea, or collectively, the Cetacea.",
  "awanting - Missing; wanting. [Prov. Scot. & Eng.] Sir W. Hamilton.",
  "unblindfold - To free from that which blindfolds. Spenser.",
  "nonoxygenous - Without oxygen; characterized by the absence of oxygen; as, a nonoxygenous alkaloid.",
  "trigonocerous - Having horns with three angles, like those of some species of goats.",
  "ceraunics - That branch of physics which treats of heat and electricity. R. Park.",
  "chiloma - The tumid upper lip of certain mammals, as of a camel.",
  "alterability - The quality of being alterable; alterableness.",
  "whethering - The retention of the afterbirth in cows. Gardner.",
  "moabitess - A female Moabite. Ruth i. 22.",
  "lustily - In a lusty or vigorous manner.",
  "intramolecular - Between molecules; situated, or acting, between the molecules of bodies.",
  "waveringness - The quality or state of wavering.",
  "anklet - An ornament or a fetter for the ankle; an ankle ring.",
  "uncanonize - 1. To deprive of canonical authority. 2. To reduce from the rank of a canonized saint.",
  "dropsicalness - State of being dropsical.",
  "datiscin - A white crystalline glucoside extracted from the bastard hemp (Datisca cannabina).",
  "trabeculate - Crossbarred, as the ducts in a banana stem.",
  "feelingly - In a feeling manner; pathetically; sympathetically.",
  "gymnochroa - A division of Hydroidea including the hydra. See Hydra.",
  "hurler - One who hurls, or plays at hurling.",
  "terrenity - Earthiness; worldliness. [Obs.] \"A dull and low terrenity.\" Feltham.",
  "exclusionism - The character, manner, or principles of an exclusionist.",
  "heliometrical - Of or pertaining to the heliometer, or to heliometry.",
  "self-devoted - Devoted in person, or by one's own will. Hawthorne.",
  "acclimatable - Capable of being acclimated.",
  "light-armed - Armed with light weapons or accouterments.",
  "abdominothoracic - Relating to the abdomen and the thorax, or chest.",
  "spriteliness - See Sprightful, Sprightfully, Sprightliness, Sprightly, etc.",
  "totally - In a total manner; wholly; entirely.",
  "dread-bolted - Armed with dreaded bolts. \"Dread-bolted thunder.\" [Poetic] Shak.",
  "pneumatologist - One versed in pneumatology.",
  "creaminess - The quality of being creamy.",
  "pharmaceutics - The science of preparing medicines.",
  "bezoartical - Having the qualities of an antidote, or of bezoar; healing. [Obs.]",
  "neve - The upper part of a glacier, above the limit or perpetual snow. See Galcier.",
  "strumstrum - A rude musical instrument somewhat like a cittern. [R.] Dampier.",
  "sarco- - A combining form from Gr. flesh; as, sarcophagous, flesh- eating; sarcology.",
  "plenariness - Quality or state of being plenary.",
  "hullabaloo - A confused noise; uproar; tumult. [Colloq.] Thackeray.",
  "eventless - Without events; tame; monotomous; marked by nothing unusual; uneventful.",
  "tailblock - A block with a tail. See Tail, 9.",
  "legalize - 1. To make legal. 2. (Theol.)  To interpret or apply in a legal spirit.",
  "oratorious - Oratorical. [Obs.] Jer. Taylor. -- Or`a*to\"ri*ous*ly, adv. [Obs.]",
  "volatilizable - Capable of being volatilized.",
  "corporas - The corporal, or communion cloth. [Obs.] Fuller.",
  "bell-mouthed - Expanding at the mouth; as, a bell-mouthed gun. Byron.",
  "adorement - The act of adoring; adoration. [Obs.] Sir T. Browne.",
  "degarnishment - The act of depriving, as of furniture, apparatus, or a garrison. [R.]",
  "styliferous - Bearing one or more styles.",
  "remonstrator - One who remonstrates; a remonsrant. Bp. Burnet.",
  "bullfist - A kind of fungus. See Puffball.",
  "seagirt - Surrounded by the water of the sea or ocean; as, a seagirt isle. Milton.",
  "basseting - The upward direction of a vein in a mine; the emergence of a stratum at the surface.",
  "flaxseed - The seed of the flax; linseed.",
  "sapsago - A kind of Swiss cheese, of a greenish color, flavored with melilot.",
  "treatiser - One who writes a treatise. [Obs.]",
  "expressage - The charge for carrying a parcel by express.",
  "enchantress - A woman versed in magical arts; a sorceress; also, a woman who fascinates. Shak.",
  "impalpability - The quality of being impalpable. Jortin.",
  "trappures - Trappings for a horse. [Obs.] Chaucer.",
  "chesteyn - The chestnut tree. [Obs.] Wilwe, elm, plane, assch, box, chesteyn. Chaucer.",
  "vaporizer - One who, or that which, vaporizes, or converts into vapor.",
  "curvilinear - Consisting of, or bounded by, curved lines; as, a curvilinear figure.",
  "circumclusion - Act of inclosing on all sides. [R.]",
  "anti-gallican - Opposed to what is Gallic or French.",
  "genista - A genus of plants including the common broom of Western Europe.",
  "zymolysis - The action of enzymes; also, the changes produced by such action. --Zy`mo*lyt\"ic (#), a.",
  "projet - A plan proposed; a draft of a proposed measure; a project.",
  "thick-knee - A stone curlew. See under Stone.",
  "swannery - A place where swans are bred. \"The largest swannery in England.\" Encyc. Brit.",
  "spherobacteria - See the Note under Microbacteria.",
  "co-religionist - One of the same religion with another.",
  "supercrescence - That which grows upon another growing thing; a parasite. [R.] Sir T. Browne.",
  "demented - Insane; mad; of unsound mind. -- De*ment\"ed*ness, n.",
  "inopulent - Not opulent; not affluent or rich.",
  "subverter - One who, or that which, subverts; an overthrower. Sir T. More.",
  "ruddiness - The quality or state of being ruddy; as, the ruddiness of the cheeks or the sky.",
  "halogenous - Of the nature of a halogen.",
  "hematitic - Of or pertaining to hematite, or resembling it.",
  "seneschalship - The office, dignity, or jurisdiction of a seneschal.",
  "smell-less - Destitute of smell; having no odor. Daisies smell-less, yet most quaint. Beau & Fl.",
  "-kin - A diminutive suffix; as, manikin; lambkin.",
  "thereon - On that or this. Chaucer. Then the king said, Hang him thereon. Esther vii. 9.",
  "morone - Maroon; the color of an unripe black mulberry.",
  "playfere - A playfellow. [Obs.] [Also, playfeer, playphere.] Holinsheld.",
  "polony - A kind of sausage made of meat partly cooked.",
  "mayan arch - A form of corbel arch employing regular small corbels.",
  "parapetalous - Growing by the side of a petal, as a stamen.",
  "pneumatophore - One of the Pneumonophora.",
  "pawnee - One or two whom a pledge is delivered as security; one who takes anything in pawn.",
  "zunis - A tribe of Pueblo Indians occupying a village in New Mexico, on the Zuni River.",
  "angiostomous - With a narrow mouth, as the shell of certain gastropods.",
  "cal - Wolfram, an ore of tungsten. Simmonds.",
  "gunnage - The number of guns carried by a ship of war.",
  "fibrillary - Of of pertaining to fibrils.",
  "disworkmanship - Bad workmanship. [Obs.] Heywood.",
  "pentandrous - Of or pertaining to the class Pentadria; having five stamens.",
  "enhydros - A variety of chalcedony containing water.",
  "hypophysis - 1. (Anat.)  See Pituitary body, under Pituitary. 2. (Med.)  Cataract.",
  "approachless - Impossible to be approached.",
  "epithite - A lazy, worthless fellow; a vagrant. [Obs.] Mason.",
  "good-den - A form of salutation. [Obs.] Shak.",
  "unmercied - Unmerciful; merciless. [Obs.] Drayton.",
  "petrologically - According to petrology.",
  "triethylamine - A tertiary amine analogous to trimethylamine.",
  "geoponics - The art or science of cultivating the earth; agriculture. Evelin.",
  "vacatur - An order of court by which a proceeding is set aside or annulled.",
  "wraprascal - A kind of coarse upper coat, or overcoat, formerly worn.",
  "inexecutable - Incapable of being executed or performed; impracticable; infeasible.",
  "sulcate - Scored with deep and regular furrows; furrowed or grooved; as, a sulcated stem.",
  "nerite - Any mollusk of the genus Nerita.",
  "scrunch - To scranch; to crunch. Dickens.",
  "organological - Of or relating to organology.",
  "dishwater - Water in which dishes have been washed. \"Suds and dishwater.\" Beau. & Fl.",
  "calamistrate - To curl or friz, as the hair. [Obs.] Cotgrave.",
  "misstayed - Having missed stays; -- said of a ship.",
  "overfree - Free to excess; too liberal; too familiar. -- O\"ver*free\"ly, adv.",
  "furbishable - Capable of being furbished.",
  "swipper - Nimble; quick. [Obs. or Prov. Eng. & Slang]",
  "skirlcock - The missel thrush; -- so called from its harsh alarm note. [Prev. Eng.]",
  "cogitability - The quality of being cogitable; conceivableness.",
  "forlornness - State of being forlorn. Boyle.",
  "zooephagan - A animal that feeds on animal food.",
  "focalization - The act of focalizing or bringing to a focus, or the state of being focalized.",
  "gynephobia - Hatred of women; repugnance to the society of women. Holmes.",
  "coagulator - That which causes coagulation. Hixley.",
  "fireboard - A chimney board or screen to close a fireplace when not in use.",
  "chromophore - Any chemical group or residue (as NO",
  "minaceous - Of the color of minium or red lead; miniate.",
  "georgical - Relating to agriculture and rural affairs.",
  "okenite - A massive and fibrous mineral of a whitish color, chiefly hydrous silicate of lime.",
  "foundress - A female founder; a woman who founds or establishes, or who endows with a fund.",
  "reductibility - The quality of being reducible; reducibleness.",
  "merestone - A stone designating a limit or boundary; a landmark. Bacon.",
  "calycular - Pertaining to, or resembling, the bracts of a calycle.",
  "provider - One who provides, furnishes, or supplies; one who procures what is wanted.",
  "rana - A genus of anurous batrachians, including the common frogs.",
  "excerptive - That excerpts, selects, or chooses. D. L. Mackenzie.",
  "princehood - Princeliness. [Obs.] E. Hall.",
  "adumbrative - Faintly representing; typical. Carlyle.",
  "impeditive - Causing hindrance; impeding. \"Cumbersome, and impeditive of motion.\" Bp. Hall.",
  "robustness - The quality or state of being robust.",
  "volleyed - Discharged with a sudden burst, or as if in a volley; as, volleyed thunder.",
  "oenomel - Wine mixed with honey; mead, [R.]",
  "spirketing - The planking from the waterways up to the port sills. Totten.",
  "disallowable - Not allowable; not to be suffered. Raleigh. -- Dis`al*low\"a*ble*ness, n.",
  "outtravel - To exceed in speed o Mad. D' Arblay.",
  "acervation - A heaping up; accumulation. [R.] Johnson.",
  "slackness - The quality or state of being slack.",
  "tentaculiferous - Producing or bearing tentacles.",
  "slavism - The common feeling and interest of the Slavonic race.",
  "decurtation - Act of cutting short. [Obs.]",
  "white-heart - A somewhat heart-shaped cherry with a whitish skin.",
  "prenasal - Situated in front of the nose, or in front of the nasal chambers.",
  "bootes - A northern constellation, containing the bright star Arcturus.",
  "affinal - Related by marriage; from the same source.",
  "executively - In the way of executing or performing.",
  "potamography - An account or description of rivers; potamology.",
  "foxhound - One of a special breed of hounds used for chasing foxes.",
  "universological - Of or pertaining to universology.",
  "frustratory - Making void; rendering null; as, a frustratory appeal. [Obs.] Ayliffe.",
  "open-air - Taking place in the open air; outdoor; as, an open-air game or meeting.",
  "backjoint - A rebate or chase in masonry left to receive a permanent slab or other filling.",
  "rhinolite - A concretion formed within the cavities of the nose.",
  "despotist - A supporter of despotism. [R.]",
  "dotary - A dotard's weakness; dotage. [Obs.] Drayton.",
  "periculous - Dangerous; full of peril. [Obs.]",
  "jorum - A large drinking vessel; also, its contents. [Colloq. Eng.] Forby.",
  "anas - A genus of water fowls, of the order Anseres, including certain species of fresh-water ducks.",
  "scripturalism - The quality or state of being scriptural; literal adherence to the Scriptures.",
  "march-mad - Extremely rash; foolhardy. See under March, the month. Sir W. Scott.",
  "longish - Somewhat long; moderately long.",
  "cosmopolitanism - The quality of being cosmopolitan; cosmopolitism.",
  "tripartible - Divisible into three parts.",
  "zoocytium - The common support, often branched, of certain species of social Infusoria.",
  "unspar - To take the spars, stakes, or bars from. [R.] Sir W. Scott.",
  "sensiferous - Exciting sensation; conveying sensation. Huxley.",
  "serviture - Servants, collectively. [Obs.]",
  "snakefish - (a) The band fish. (b) The lizard fish.",
  "vivific - Giving life; reviving; enlivening. [R.]",
  "picturer - One who makes pictures; a painter. [R.] Fuller.",
  "opisthobranchiate - Of or pertaining to the Opisthobranchiata. -- n.  One of the Opisthobranchiata.",
  "townhouse - A building devoted to the public used of a town; a townhall.",
  "frightfulness - The quality of being frightful.",
  "ambulate - To walk; to move about. [R.] Southey.",
  "tympanites - A flatulent distention of the belly; tympany.",
  "murmuring - Uttering murmurs; making low sounds; complaining. -- Mur\"mur*ing*ly, adv.",
  "trapezohedral - Of, pertaining to, or resembling, a trapezohedron.",
  "underget - To get under or beneath; also, to understand. [Obs.] R. of Gloucester.",
  "nubia - A light fabric of wool, worn on the head by women; a cloud.",
  "increpation - A chiding; rebuke; reproof. [Obs.] Hammond.",
  "submonish - To suggest; to prompt. [R.] \"The submonishing inclinations of my senses.\" T. Granger.",
  "disforest - 1. To disafforest. Fuller. 2. To clear or deprive of forests or trees.",
  "cestoid - Of or pertaining to the Cestoidea. -- n.  One of the Cestoidea.",
  "strull - A bar so placed as to resist weight.",
  "infrangibleness - The state or quality of being infrangible; infrangibility.",
  "dichromatism - The state of being dichromatic.",
  "unframe - To take apart, or destroy the frame of. Dryden.",
  "viage - A voyage; a journey. [Obs.] Chaucer. Gower.",
  "roundtop - A top; a platform at a masthead; -- so called because formerly round in shape.",
  "verifiable - Capable of being verified; confirmable. Bp. Hall.",
  "calceolate - Slipper-ahaped. See Calceiform.",
  "muddlehead - A stupid person. [Colloq.] C. Reade. -- Mud\"dle-head`ed, a. [Colloq.] Dickens.",
  "anaptotic - Having lost, or tending to lose, inflections by phonetic decay; as, anaptotic languages.",
  "dawdler - One who wastes time in trifling employments; an idler; a trifler.",
  "cup-gall - A kind of oak-leaf gall. See Gall.",
  "newfangledness - Affectation of, or fondness for, novelty; vain or affected fashion or form.",
  "sacrifical - Employed in sacrifice. [R.] Johnson.",
  "sea salt - Common salt, obtained from sea water by evaporation.",
  "self-interested - Particularly concerned for one's own interest or happiness.",
  "inexplicit - Not explicit; not clearly stated; indefinite; vague.",
  "maya arch - A form of corbel arch employing regular small corbels.",
  "unorder - To countermand an order for. [R.]",
  "sigma - The Greek letter S, or s). It originally had the form of the English C.",
  "nonslaveholding - Not possessing or holding slaves; as, a nonslaveholding State.",
  "bottle-neck frame - An inswept frame. [Colloq.]",
  "vein quartz - Quartz occurring as gangue in a vein.",
  "consonantness - The quality or condition of being consonant, agreeable, or consistent.",
  "amess - Amice, a hood or cape. See 2d Amice.",
  "ovine - Of or pertaining to sheep; consisting of sheep.",
  "haikwan tael - A Chinese weight ( 1/10 catty) equivalent to 1 1/3 oz. or 37.801 g.",
  "microbic - Of or pertaining to a microbe.",
  "bismite - Bismuth trioxide, or bismuth ocher.",
  "stone-still - As still as a stone. Shak.",
  "unsafety - The quality or state of being in peril; absence of safety; insecurity. Bacon.",
  "branny - Having the appearance of bran; consisting of or containing bran. Wiseman.",
  "self-kindled - Kindled of itself, or without extraneous aid or power. Dryden.",
  "photophilous - Light-loving; growing in strong light, as many plants.",
  "confortation - The act of strengthening. [Obs.]  Bacon.",
  "ineffaceably - So as not to be effaceable.",
  "aestuous - Glowing; agitated, as with heat.",
  "discommodate - To discommode. [Obs.] Howell."
]